...
"""
Case Type   : 分区表锁降级
Case Name   : alter exchange分区,同时相邻和不相邻分区均update数据
Description :
    1、分别创建同字段普通表、范围分区表,表中分别插入数据;
    2、session1中将普通表的数据交换到分区表对应分区中;
    3、session2中同时在相邻和不相邻分区中均update数据;
    4、查看分区数据;
    5、清理环境;
Expect      :
    1、创建普通表、范围分区表成功,普通表插入数据成功;
    2、session1中交换数据成功;
    3、session2中同时在相邻和不相邻分区中update数据成功，未阻塞;
    4、查看分区数据成功;
    5、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0059开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name_row = 't_locklevel_0059_row'
        self.table_name_part = 't_locklevel_0059_part'
        self.i_function_name = 'f_insert_locklevel_0059'
        self.u_function_name = 'f_update_locklevel_0059'
    ...
    def test_lock_level(self):
        ...
        func_sql2 = f'''update {self.table_name_part} set id2 = 8 
            where id1={func_var};'''
        ...
        self.assertTrue(
            ...
            and create_msg.count(self.i_function_name) == 2, '执行失败:' + text)
        ...
        text = '---step3 & step4:session2中同时在相邻分区中update数据; expect:更新数据成功---'
        self.log.info(text)
        select_cmd = f'''select {update_func}(20100);
            select * from {self.table_name_part} partition \
            for(20100) limit 1;'''
        session_2 = ComThread(self.comSQL.execut_db_sql, args=(select_cmd,))
        session_2.setDaemon(True)
        session_2.start()

        text = '---step3 & step4:session2中同时在不相邻分区中update数据; expect:更新数据成功---'
        self.log.info(text)
        select_cmd = f'''select {update_func}(40100);
                    select * from {self.table_name_part} partition \
                    for(40100) limit 1;'''
        session_3 = ComThread(self.comSQL.execut_db_sql, args=(select_cmd,))
        session_3.setDaemon(True)
        session_3.start()

        self.log.info('------session3执行结果------')
        session_3.join(55)
        session_3_res = session_3.get_result()
        self.log.info(session_3_res)
        self.assertTrue(self.u_function_name in session_3_res.splitlines()[0]
                        and '8' in session_3_res.splitlines()[-2],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            '执行失败' + text)
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0059执行完成--')
