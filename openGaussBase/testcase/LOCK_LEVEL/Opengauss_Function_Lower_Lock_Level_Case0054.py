...
"""
Case Type   : 分区表锁降级
Case Name   : alter drop分区,同时相邻分区select数据
Description :
    1、创建范围分区表;
    2、向分区2中插入数据;
    3、session1中删除分区1;
...
Expect      :
    ...
    2、插入数据成功;
    3、session1中删除分区1成功;
    4、session2中同时在相邻分区2中select数据成功，未阻塞;
    5、清理环境成功;
History     :
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0054开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0054'
        self.i_function_name = 'f_insert_locklevel_0054'
    ...
    def test_lock_level(self):
        ...
        text = '---step4:session2中同时在分区2中select数据; expect:查看数据成功---'
        self.log.info(text)
        select_cmd = f'''select * from {self.table_name} partition \
            for(20100) limit 20;
        ...
        self.assertTrue('(20 rows)' in session_2_res.splitlines()[-6]
                        and '101' in session_2_res.splitlines()[-2].strip(),
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0054执行完成------'
        self.log.info(text)
