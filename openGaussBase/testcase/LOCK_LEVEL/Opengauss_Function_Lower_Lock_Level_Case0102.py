...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0102开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0102'
        self.i_function_name = 'f_insert_locklevel_0102'
        self.u_function_name = 'f_update_locklevel_0102'
    ...
    def test_lock_level(self):
        ...
        sql = f'''select {insert_func}(5000);
            select {insert_func}(39999);
            select count(*) from {self.table_name};'''
        self.log.info(sql)
        msg = self.comsh.execut_db_sql(sql)
        self.log.info(msg)
        self.assertTrue('25001' in msg.splitlines()[-2].strip(),
                        '执行失败' + text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0102执行完成------'
        self.log.info(text)
