...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务alter add分区不提交,同时跨分区select数据
Description :
    1、创建范围分区表;
    2、session1中开启事务，新增分区不提交，待select完后再提交;
    3、session2中同时跨分区中select数据;
    4、清理环境;
Expect      :
    1、创建范围分区表成功;
    2、session1中新增分区成功;
    3、session2中同时跨分区中select成功，未阻塞;
    4、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0077开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0077'
        self.i_function_name = 'f_insert_locklevel_0077'
    ...
    def test_lock_level(self):
        ...
        text = '---step3 & step4:session2中同时跨分区select数据; expect:查看数据成功---'
        self.log.info(text)
        ...
        self.assertTrue('303' in session_2_res.splitlines()[2]
                        and '(20 rows)' in session_2_res.splitlines()[-1],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_TABLE_SUCCESS in drop_msg and
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0077执行完成------'
        self.log.info(text)
