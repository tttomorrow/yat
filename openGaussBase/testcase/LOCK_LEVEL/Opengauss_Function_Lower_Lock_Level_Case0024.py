...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区select数据,同时相邻不分区delete数据
Description :
    1、创建范围分区表;
    2、向分区1、分区2中插入数据;
    3、session1中在分区1中select数据;
    4、session2中同时向分区3中delete数据;
    5、查看分区数据;
    6、清理环境;
Expect      :
    1、创建范围分区表成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0024开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0024'
        self.i_function_name = 'f_insert_locklevel_0024'
        self.d_function_name = 'f_delete_locklevel_0024'
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0024执行完成------'
        self.log.info(text)
