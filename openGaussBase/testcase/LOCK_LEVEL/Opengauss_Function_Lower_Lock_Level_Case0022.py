...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区select数据,同时不相邻分区update数据
Description :
    1、创建范围分区表;
    2、向分区1、分区3中插入数据;
    3、session1中在分区1中select数据;
    4、session2中同时向分区3中update数据;
    5、查看分区数据;
    6、清理环境;
Expect      :
    1、创建范围分区表成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0022开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0022'
        self.i_function_name = 'f_insert_locklevel_0022'
        self.u_function_name = 'f_update_locklevel_0022'
    ...
    def test_lock_level(self):
        ...
        text = '---step4 & step5:session2中向分区3中update数据; expect:更新数据成功---'
        self.log.info(text)
        select_cmd = f'''select {update_func}(40100);
            select * from {self.table_name} partition for (59999) limit 1;'''
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0022执行完成------'
        self.log.info(text)
