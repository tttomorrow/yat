...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务alter add分区不提交,同时不相邻分区delete数据
Description :
    1、创建范围分区表;
    2、session1中开启事务，新增分区不提交，待delete完后再提交;
...
Expect      :
    1、创建范围分区表成功;
    2、session1中新增分区成功;
    3、session2中同时在不相邻分区中delete成功，未阻塞;
    4、查看分区数据成功;
    5、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0074开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0074'
        self.i_function_name = 'f_insert_locklevel_0074'
        self.d_function_name = 'f_delete_locklevel_0074'
    ...
    def test_lock_level(self):
        ...
        self.assertTrue(self.constant.CREATE_TABLE_SUCCESS in create_msg
                        and self.i_function_name in create_msg,
                        '执行失败:' + text)

        text = '------step2:session1中开启事务,新增一个分区; expect:成功------'
        self.log.info(text)
        add_cmd = f'''start transaction;
            alter table {self.table_name} add partition p03 \
        ...
        text = '---step3 & step4:session2中同时在分区2中delete数据; expect:delete数据成功---'
        self.log.info(text)
        select_cmd = f'''select {delete_func}(20100);
        ...
        self.assertTrue(self.constant.ALTER_TABLE_MSG in session_1_res
                        and 'p03' in session_1_res.splitlines()[7]
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            self.constant.DROP_TABLE_SUCCESS in drop_msg
            and drop_msg.count(self.constant.DROP_FUNCTION_SUCCESS_MSG) == 2,
            '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0074执行完成------'
        self.log.info(text)
