...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区delete数据,同时相邻分区select数据
Description :
    1、创建范围分区表;
    2、向分区1、分区2中插入数据;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0034开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0034'
        self.i_function_name = 'f_insert_locklevel_0034'
        self.d_function_name = 'f_delete_locklevel_0034'
    ...
    def test_lock_level(self):
        ...
        text = '---step4 & step5:session2中在分区2中select数据; expect:查看数据成功---'
        self.log.info(text)
        ...
        self.assertTrue('(20 rows)' in session_2_res.splitlines()[-1].strip(),
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0034执行完成------'
        self.log.info(text)
