...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区更新数据,相邻分区同时select数据
Description :
    1、创建范围分区表;
    2、向分区1、分区2中插入数据;
    3、session1中在分区1中更新数据;
    4、session2中同时在分区2中select数据;
    5、查看分区数据;
    6、清理环境;
Expect      :
    1、创建范围分区表成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0016开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0016'
        self.i_function_name = 'f_insert_locklevel_0016'
        self.u_function_name = 'f_update_locklevel_0016'
    ...
    def test_lock_level(self):
        ...
        text = '---step4 & step5:session2中在分区2中select查看数据; expect:查看数据成功---'
        self.log.info(text)
        ...
        self.assertTrue('101' in session_2_res.splitlines()[-2],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0016执行完成------'
        self.log.info(text)
