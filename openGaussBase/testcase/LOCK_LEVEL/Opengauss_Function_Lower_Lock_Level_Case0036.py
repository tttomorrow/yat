...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区delete数据,同时跨分区select数据
Description :
    1、创建范围分区表;
    2、向分区1、分区2、分区3中插入数据;
    3、session1中在分区1中delete数据;
    4、session2中同时跨分区select数据;
    5、查看分区数据;
    6、清理环境;
Expect      :
    1、创建范围分区表成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0036开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0036'
        self.i_function_name = 'f_insert_locklevel_0036'
        self.d_function_name = 'f_delete_locklevel_0036'
    ...
    def test_lock_level(self):
        ...
        text = '---step4 & step5:session2中在分区3中select数据; expect:查看数据成功---'
        self.log.info(text)
        ...
        self.assertTrue('(20 rows)' in session_2_res.splitlines()[-1].strip()
                        and '202' in session_2_res.splitlines()[2],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0036执行完成------'
        self.log.info(text)
