...
"""
Case Type   : 分区表锁降级
Case Name   : truncate表,同时delete数据
Description :
    1、创建范围分区表,插入数据;
    2、session1中truncate表中数据;
    3、session2中同时在某一分区delete数据;
    4、查看表中数据;
    5、清理环境;
Expect      :
    1、创建范围分区表成功,插入数据成功;
    2、session1中truncate表中数据成功;
    3、session2中同时delete数据成功，未阻塞;
    4、查看数据成功;
    5、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0067开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0067'
        self.i_function_name = 'f_insert_locklevel_0067'
        self.d_function_name = 'f_delete_locklevel_0067'
    ...
    def test_lock_level(self):
        ...
        text = '---step3 & step4:session2中同时在分区中delete数据; expect:删除数据成功---'
        self.log.info(text)
        ...
        self.assertTrue(self.d_function_name in session_2_res
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0067执行完成------'
        self.log.info(text)
