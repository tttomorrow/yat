...
"""
Case Type   : 分区表锁降级
Case Name   : alter exchange分区,同时不相邻分区select数据
Description :
...
Expect      :
    ...
    2、session1中交换数据成功;
    3、session2中同时在不相邻分区中select数据成功，未阻塞;
    4、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0063开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name_row = 't_locklevel_0063_row'
        self.table_name_part = 't_locklevel_0063_part'
        self.i_function_name = 'f_insert_locklevel_0063'
    ...
    def test_lock_level(self):
        ...
        text = '---step3:session2中同时在相邻分区中select数据; expect:查看数据成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0063执行完成------'
        self.log.info(text)
