...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务truncate分区,通过分区值所在分区删除不提交,其他分区查询的执行成功
Description :
    1、创建范围分区表,插入数据;
    2、session1中开启事务，通过分区值所在分区删除分区数据，待查询后提交;
    3、session2中同时执行对其他分区的查询计划;
    4、清理环境;
Expect      :
    1、创建范围分区表成功,插入数据成功;
    2、session1中清空分区数据成功,事务提交成功;
    3、session2中同时执行查询成功;
    4、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0121开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.tab_name = 't_locklevel_0121'
        self.i_function_name = 'f_insert_locklevel_0121'
    ...
    def test_lock_level(self):
        ...
        text = '---step2:session1中开启事务,通过分区值删除分区数据,不提交; expect:成功---'
        self.log.info(text)
        update_cmd = f'''start transaction;
            alter table {self.tab_name} truncate partition for(19999);
            select count(*) from {self.tab_name} partition for(19999);
            select pg_sleep(10);
            commit;'''
        ...
        text = '---step3:session2中执行全表的查询; expect:成功---'
        self.log.info(text)
        select_cmd = f'''select count(*) from {self.tab_name};'''
        ...
        self.assertTrue('202' in session_2_res.splitlines()[2],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0121执行完成------'
        self.log.info(text)
