...
"""
Case Type   : 分区表锁降级
Case Name   : truncate表,同时select数据
Description :
    1、创建范围分区表,插入数据;
    2、session1中truncate表中数据;
    3、session2中同时在分区表中select数据;
    4、清理环境;
Expect      :
    1、创建范围分区表成功,插入数据成功;
    2、session1中truncate表中数据成功;
    3、session2中同时select数据成功，未阻塞;
    4、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0068开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0068'
        self.i_function_name = 'f_insert_locklevel_0068'
    ...
    def test_lock_level(self):
        ...
        text = '---step3:session2中同时在分区表中select数据; expect:查看数据成功,无数据---'
        self.log.info(text)
        ...
        self.assertTrue('0' in session_2_res.splitlines()[-2].strip(),
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0068执行完成------'
        self.log.info(text)
