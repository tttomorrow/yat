...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区插入数据,相邻分区同时更新数据
Description :
    1、创建范围分区表;
    2、向分区2中插入数据;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0089开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0089'
        self.i_function_name = 'f_insert_locklevel_0089'
        self.u_function_name = 'f_update_locklevel_0089'
    ...
    def test_lock_level(self):
        ...
        self.log.info(msg)
        self.assertIn('101', msg.splitlines()[-2].strip(), '执行失败' + text)
        ...
        self.log.info(text)
        update_cmd = f'''select {update_func}(20100);
            select * from {self.table_name} partition for (20100) limit 1;'''
        session_2 = ComThread(self.comSQL.execut_db_sql, args=(update_cmd,))
        ...
        self.assertTrue(self.i_function_name in session_1_result and
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0089执行完成------'
        self.log.info(text)
