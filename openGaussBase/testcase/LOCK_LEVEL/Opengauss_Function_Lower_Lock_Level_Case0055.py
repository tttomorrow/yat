...
"""
Case Type   : 分区表锁降级
Case Name   : alter exchange分区,同时相邻分区insert数据
Description :
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0055开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name_row = 't_locklevel_0055_row'
        self.table_name_part = 't_locklevel_0055_part'
        self.i_function_name = 'f_insert_locklevel_0055'
    ...
    def test_lock_level(self):
        ...
        text = '---step3 & step4:session2中同时在相邻分区中insert数据; expect:插入数据成功---'
        self.log.info(text)
        ...
        self.assertTrue(self.constant.ALTER_TABLE_MSG in session_1_res
                        ...
                        and '0' in session_1_res.splitlines()[3],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0055执行完成------'
        self.log.info(text)
