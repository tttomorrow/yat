...
"""
Case Type   : 分区表锁降级
Case Name   : alter drop分区,同时不相邻分区update数据
Description :
    1、创建范围分区表;
    2、向分区3中插入数据;
    3、session1中删除分区;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0043开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0043'
        self.i_function_name = 'f_insert_locklevel_0043'
        self.u_function_name = 'f_update_locklevel_0043'
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0043执行完成------'
        self.log.info(text)
