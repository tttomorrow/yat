...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务alter drop分区不提交,同时相邻分区insert数据
Description :
    1、创建范围分区表;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0078开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0078'
        self.i_function_name = 'f_insert_locklevel_0078'
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_TABLE_SUCCESS in drop_msg and
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0078执行完成------'
        self.log.info(text)
