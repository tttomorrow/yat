...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务alter drop分区不提交,同时不相邻分区select数据
Description :
    1、创建范围分区表,插入数据;
    2、session1中开启事务，删除分区不提交，待select完后再提交;
    3、session2中同时在不相邻分区中select数据;
    4、清理环境;
Expect      :
    1、创建范围分区表成功,插入数据成功;
    2、session1中删除分区成功;
    3、session2中同时在不相邻分区中select成功，未阻塞;
    4、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0085开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0085'
        self.i_function_name = 'f_insert_locklevel_0085'
    ...
    def test_lock_level(self):
        ...
        text = '------step2:session1中删除一个分区; expect:成功------'
        self.log.info(text)
        ...
        text = '---step3:session2中同时在分区3中select数据; expect:查看数据成功---'
        self.log.info(text)
        ...
        self.assertTrue('101' in session_2_res.splitlines()[-2].strip(),
                        "执行失败:" + text)
        ...
        self.assertTrue(
            self.constant.START_TRANSACTION_SUCCESS_MSG in session_1_res
            and self.constant.ALTER_TABLE_MSG in session_1_res
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_TABLE_SUCCESS in drop_msg and
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0085执行完成------'
        self.log.info(text)
