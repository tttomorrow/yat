...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0009开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0009'
        self.function_name = 'f_insert_locklevel_0009'
    ...
    def test_lock_level(self):
        ...
        sql = f'''select {insert_func}(39999);
            select {insert_func}(59999);
            select count(*) from {self.table_name};'''
        self.log.info(sql)
        msg = self.comsh.execut_db_sql(sql)
        self.log.info(msg)
        self.assertTrue('40000' in msg.splitlines()[-2].strip(),
                        '执行失败' + text)
        ...
        text = '------step4:session2中查看所有分区数据; expect:查看数据成功------'
        self.log.info(text)
        ...
        self.assertTrue('60000' in session_2_result.splitlines()[-2],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0009执行完成------'
        self.log.info(text)
