...
"""
Case Type   : 分区表锁降级
Case Name   : alter exchange分区,同时跨分区select数据
Description :
    1、分别创建同字段普通表、范围分区表,表中分别插入数据;
    2、session1中将普通表的数据交换到分区表对应分区中;
    3、session2中同时跨分区select数据;
    4、清理环境;
Expect      :
    1、创建普通表、范围分区表成功,普通表插入数据成功;
    2、session1中交换数据成功;
    3、session2中同时跨分区select数据成功，未阻塞;
    4、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0064开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name_row = 't_locklevel_0064_row'
        self.table_name_part = 't_locklevel_0064_part'
        self.i_function_name = 'f_insert_locklevel_0064'
    ...
    def test_lock_level(self):
        ...
        text = '---step3:session2中同时跨分区select数据; expect:查看数据成功---'
        self.log.info(text)
        select_cmd = f'''select count(*) from {self.table_name_part};'''
        ...
        self.assertTrue('303' in session_2_res.splitlines()[-2].strip(),
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            and self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg,
            '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0064执行完成------'
        self.log.info(text)
