...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务truncate全表并提交,全表的查询计划执行成功
Description :
    1、创建范围分区表,插入数据;
    2、session1中开启事务，清空全表数据并提交;
    3、session2中同时执行对全表的查询计划;
    4、清理环境;
Expect      :
    1、创建范围分区表成功,插入数据成功;
    2、session1中清空数据成功,事务提交成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0119开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.tab_name = 't_locklevel_0119'
        self.i_function_name = 'f_insert_locklevel_0119'
    ...
    def test_lock_level(self):
        ...
        text = '------step2:session1中开启事务,清空表中数据并提交; expect:成功------'
        self.log.info(text)
        ...
        text = '---step3:session2中执行全表的查询计划; expect:成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0119执行完成------'
        self.log.info(text)
