...
"""
Case Type   : 分区表锁降级
Case Name   : alter add分区,同时不相邻分区update数据
Description :
    1、创建范围分区表;
    2、向分区2中插入数据;
    3、session1中新增分区;
...
Expect      :
    ...
    2、插入数据成功;
    3、session1中新增分区成功;
    4、session2中同时在不相邻分区2中update成功，未阻塞;
    5、查看分区数据成功;
    6、清理环境成功;
History     :
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0038开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0038'
        self.i_function_name = 'f_insert_locklevel_0038'
        self.u_function_name = 'f_update_locklevel_0038'
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0038执行完成------'
        self.log.info(text)
