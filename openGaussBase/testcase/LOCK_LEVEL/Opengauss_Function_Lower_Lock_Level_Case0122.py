...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务truncate分区并提交,其他分区查询的执行成功
Description :
    1、创建范围分区表,插入数据;
    2、session1中开启事务，删除分区数据并提交;
    3、session2中同时执行对其他分区的查询;
    4、清理环境;
Expect      :
    1、创建范围分区表成功,插入数据成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0122开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.tab_name = 't_locklevel_0122'
        self.i_function_name = 'f_insert_locklevel_0122'
    ...
    def test_lock_level(self):
        ...
        text = '---step2:session1中开启事务,删除分区数据并提交; expect:成功---'
        self.log.info(text)
        ...
        text = '---step3:session2中执行其他分区的查询; expect:成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0122执行完成------'
        self.log.info(text)
