...
"""
Case Type   : 分区表锁降级
Case Name   : alter drop分区,同时不相邻分区select数据
Description :
    1、创建范围分区表;
    2、向分区3中插入数据;
    3、session1中删除分区;
...
Expect      :
    ...
    2、插入数据成功;
    3、session1中删除分区成功;
    4、session2中同时在不相邻分区3中select数据成功，未阻塞;
    5、清理环境成功;
History     :
...
class LockLevel(unittest.TestCase):
    ...
    def test_lock_level(self):
        ...
        text = '---step4:session2中同时在分区3中select数据; expect:查看数据成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0045执行完成------'
        self.log.info(text)
