...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区更新数据,不相邻分区同时插入数据
Description :
    1、创建范围分区表;
    2、向分区1中插入数据;
    3、session1中在分区1中更新数据;
    4、session2中同时在分区3插入数据;
    5、查看分区数据;
    6、清理环境;
Expect      :
    1、创建范围分区表成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0011开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0011'
        self.i_function_name = 'f_insert_locklevel_0011'
        self.u_function_name = 'f_update_locklevel_0011'
    ...
    def test_lock_level(self):
        ...
        text = '------step4 & step5:session2中向分区3中插入数据,查看数据更新;  ' \
               'expect:插入数据成功,查看数据成功------'
        self.log.info(text)
        insert_cmd = f'''select {insert_func}(40100);
            select count(*) from {self.table_name} partition for (40100);'''
        session_2 = ComThread(self.comSQL.execut_db_sql, args=(insert_cmd,))
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0011执行完成------'
        self.log.info(text)
