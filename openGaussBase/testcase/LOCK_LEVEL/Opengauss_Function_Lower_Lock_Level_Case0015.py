...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区更新数据,不相邻分区同时删除数据
Description :
    1、创建范围分区表;
    2、向分区1、分区3中插入数据;
    3、session1中在分区1中更新数据;
    4、session2中同时在分区3删除数据;
    5、查看分区数据;
    6、清理环境;
Expect      :
    1、创建范围分区表成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0015开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0015'
        self.i_function_name = 'f_insert_locklevel_0015'
        self.u_function_name = 'f_update_locklevel_0015'
        self.d_function_name = 'f_delete_locklevel_0015'
    ...
    def test_lock_level(self):
        ...
        func_sql3 = f'''delete from {self.table_name} where id1={func_var};'''
        delete_func = self.comSQL.create_func(self.d_function_name,
                                              execute_sql=func_sql3,
                                              var=func_var,
                                              start=20000,
                                              end=80000,
                                              step=20000)
        ...
        text = '------step3:session1中在分区1中update数据; expect:成功------'
        self.log.info(text)
        update_cmd = f'''select {update_func}(100);
            select pg_sleep(15);            
            select * from {self.table_name} partition for (100) limit 1;'''
        session_1 = ComThread(self.comsh.execut_db_sql,
                              args=(update_cmd, ''))
        session_1.setDaemon(True)
        session_1.start()
        time.sleep(1)

        text = '------step4 & step5:session2中在分区3中delete数据,查看数据;  ' \
               'expect:删除数据成功,查看数据成功------'
        self.log.info(text)
        delete_cmd = f'''select {delete_func}(40100);
        ...
        self.assertTrue(self.d_function_name in session_2_res.splitlines()[0]
                        and '0' in session_2_res.splitlines()[-2],
                        "执行失败:" + text)
        ...
        self.assertTrue(self.u_function_name in session_1_res.splitlines()[0]
                        and '8' in session_1_res.splitlines()[-2],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            drop_msg.count(self.constant.DROP_FUNCTION_SUCCESS_MSG) == 3)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0015执行完成------'
        self.log.info(text)
