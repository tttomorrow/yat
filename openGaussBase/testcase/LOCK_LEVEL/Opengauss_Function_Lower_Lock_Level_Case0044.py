...
"""
Case Type   : 分区表锁降级
Case Name   : alter drop分区,同时不相邻分区delete数据
Description :
    1、创建范围分区表;
    2、向分区3中插入数据;
    3、session1中删除分区;
...
Expect      :
    ...
    2、插入数据成功;
    3、session1中删除分区成功;
    4、session2中同时在不相邻分区3中delete数据成功，未阻塞;
    5、查看分区数据成功;
    6、清理环境成功;
History     :
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0044开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0044'
        self.i_function_name = 'f_insert_locklevel_0044'
        self.d_function_name = 'f_delete_locklevel_0044'
    ...
    def test_lock_level(self):
        ...
        text = '---step4 & step5:session2中同时在分区3中delete数据; expect:删除数据成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0044执行完成------'
        self.log.info(text)
