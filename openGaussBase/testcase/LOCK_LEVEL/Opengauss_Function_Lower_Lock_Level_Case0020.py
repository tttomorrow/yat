...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区select数据,同时不相邻分区insert数据
Description :
    1、创建范围分区表;
    2、向分区1中插入数据;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0020开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0020'
        self.i_function_name = 'f_insert_locklevel_0020'
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0020执行完成------'
        self.log.info(text)
