...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务alter add分区不提交,同时相邻分区delete数据
Description :
    1、创建范围分区表;
    2、session1中开启事务，新增分区不提交，待delete完后再提交;
    3、session2中同时在相邻分区中delete数据;
    4、查看分区数据;
    5、清理环境;
Expect      :
    1、创建范围分区表成功;
    2、session1中新增分区成功;
    3、session2中同时在相邻分区中delete成功，未阻塞;
    4、查看分区数据成功;
    5、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0073开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0073'
        self.i_function_name = 'f_insert_locklevel_0073'
        self.d_function_name = 'f_delete_locklevel_0073'
    ...
    def test_lock_level(self):
        ...
        text = '---step3 & step4:session2中同时在分区3中delete数据; expect:delete数据成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0073执行完成------'
        self.log.info(text)
