...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务drop分区不提交,其他分区加入全局索引查询的计划执行成功
Description :
    1、创建范围分区表,创建全局索引;
    2、向分区1、分区2、分区3中插入数据;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0111开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.tab_name = 't_locklevel_0111'
        self.index_name = 'i_locklevel_0111'
        self.i_function_name = 'f_insert_locklevel_0111'
    ...
    def test_lock_level(self):
        ...
        select_cmd = f'''explain select * from {self.tab_name} \
            partition for(20100) where id1=36666;'''
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0111执行完成------'
        self.log.info(text)
