...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务truncate全表并提交,其他分区的查询计划执行成功
Description :
    1、创建范围分区表,插入数据;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0118开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.tab_name = 't_locklevel_0118'
        self.i_function_name = 'f_insert_locklevel_0118'
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0118执行完成------'
        self.log.info(text)
