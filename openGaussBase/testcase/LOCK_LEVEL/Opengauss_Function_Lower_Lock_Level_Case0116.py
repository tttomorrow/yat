...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务exchange分区并提交,全表的查询计划执行成功
Description :
    1、分别创建同字段普通表、范围分区表,普通表中插入数据;
    2、session1中开启事务，将普通表的数据交换到分区表对应分区中并提交;
    3、session2中同时执行对全表的查询计划;
    4、清理环境;
Expect      :
    1、创建普通表、范围分区表成功,普通表插入数据成功;
    2、session1中交换数据成功,事务提交成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0116开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.p_tab = 't_locklevel_0116_part'
        self.r_tab = 't_locklevel_0116_row'
    ...
    def test_lock_level(self):
        ...
        text = '------step2:session1中开启事务,将普通表的数据交换到分区表对应分区中并提交; ' \
               'expect:成功------'
        self.log.info(text)
        ...
        text = '---step3:session2中执行全表的查询计划,未提交时查询阻塞,提交后查询成功; expect:成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(drop_msg.count(self.constant.DROP_TABLE_SUCCESS) == 2,
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0116执行完成------'
        self.log.info(text)
