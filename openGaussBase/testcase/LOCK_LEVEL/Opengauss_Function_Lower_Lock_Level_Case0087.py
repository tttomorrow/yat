...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区插入数据,相邻分区同时也插入数据
Description :
    1、创建范围分区表;
    2、session1中开启事务，向分区1插入数据;
    3、session2中同时在分区2插入数据;
    4、查看分区数据;
    5、清理环境;
Expect      :
    1、创建范围分区表成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0087开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0087'
        self.function_name = 'f_locklevel_0087'
    ...
    def test_lock_level(self):
        ...
        text = '------step2:session1中开启事务,向分区1插入数据; expect:成功------'
        self.log.info(text)
        insert_cmd1 = f'''start transaction;
        ...
        text = '------step3 & step4:session2中向分区2插入数据,查看分区数据; ' \
               'expect:插入数据成功,查看分区数据成功------'
        self.log.info(text)
        insert_cmd2 = f'''select {insert_func}(20100);
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_TABLE_SUCCESS in drop_msg and
                        ...
                        '执行失败' + text)
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0087执行完成--')
