...
"""
Case Type   : 分区表锁降级
Case Name   : alter exchange分区,同时不相邻分区insert数据
Description :
...
Expect      :
    ...
    2、session1中交换数据成功;
    3、session2中同时在不相邻分区中插入数据成功，未阻塞;
    4、查看分区数据成功;
    5、清理环境成功;
History     :
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0056开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name_row = 't_locklevel_0056_row'
        self.table_name_part = 't_locklevel_0056_part'
        self.i_function_name = 'f_insert_locklevel_0056'
    ...
    def test_lock_level(self):
        ...
        text = '------step1:创建普通表、范围分区表,普通表插入数据; expect:成功------'
        self.log.info(text)
        ...
        text = '---step3 & step4:session2中同时在不相邻分区中insert数据; expect:插入数据成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg and
            drop_msg.count(self.constant.DROP_TABLE_SUCCESS) == 2)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0056执行完成------'
        self.log.info(text)
