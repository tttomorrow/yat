...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务drop分区并提交,其他分区查询的计划执行成功
Description :
    1、创建范围分区表;
    2、向分区1、分区2、分区3中插入数据;
    3、session1中开启事务，删除分区并提交;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0109开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0109'
        self.i_function_name = 'f_insert_locklevel_0109'
    ...
    def test_lock_level(self):
        ...
        text = '------step3:session1中开启事务,删除分区后提交; expect:成功------'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0109执行完成------'
        self.log.info(text)
