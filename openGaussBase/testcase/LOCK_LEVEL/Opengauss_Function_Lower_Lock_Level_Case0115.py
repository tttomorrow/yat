...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务exchange分区并提交,其他分区的查询计划执行成功
Description :
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0115开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.p_tab = 't_locklevel_0115_part'
        self.r_tab = 't_locklevel_0115_row'
    ...
    def test_lock_level(self):
        ...
        text = '---step3:session2中执行其他分区的查询计划; expect:成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(drop_msg.count(self.constant.DROP_TABLE_SUCCESS) == 2,
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0115执行完成------'
        self.log.info(text)
