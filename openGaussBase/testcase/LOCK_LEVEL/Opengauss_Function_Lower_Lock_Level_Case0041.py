...
"""
Case Type   : 分区表锁降级
Case Name   : alter add分区,同时跨分区select数据
Description :
    1、创建范围分区表;
    2、向分区表中插入数据;
    3、session1中新增分区;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0041开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0041'
        self.i_function_name = 'f_insert_locklevel_0041'
    ...
    def test_lock_level(self):
        ...
        text = '-----step2:首先向分区表中插入部分数据; expect:成功-----'
        self.log.info(text)
        ...
        text = '---step4 & step5:session2中同时跨分区select数据; expect:查看数据成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0041执行完成------'
        self.log.info(text)
