...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务add分区，不提交,多分区查询的计划执行成功
Description :
    1、创建范围分区表;
    2、向分区1、分区2、分区3中插入数据;
    3、session1中开启事务，新增分区，不提交;
    4、session2中同时执行对其他多个分区的查询计划;
    5、清理环境;
Expect      :
    1、创建范围分区表成功;
    2、插入数据成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0107开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0107'
        self.i_function_name = 'f_insert_locklevel_0107'
    ...
    def test_lock_level(self):
        ...
        text = '------step3:session1中开启事务,增加分区; expect:成功------'
        self.log.info(text)
        ...
        self.log.info(text)
        select_cmd = f'''explain select count(*) from {self.table_name};'''
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_TABLE_SUCCESS in drop_msg and
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0107执行完成------'
        self.log.info(text)
