...
"""
Case Type   : 分区表锁降级
Case Name   : alter exchange分区,同时相邻分区delete数据
Description :
...
Expect      :
    ...
    2、session1中交换数据成功;
    3、session2中同时在相邻分区中delete数据成功，未阻塞;
    4、查看分区数据成功;
    5、清理环境成功;
History     :
"""
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0060开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name_row = 't_locklevel_0060_row'
        self.table_name_part = 't_locklevel_0060_part'
        self.i_function_name = 'f_insert_locklevel_0060'
        self.d_function_name = 'f_delete_locklevel_0060'
    ...
    def test_lock_level(self):
        ...
        text = '---step3 & step4:session2中同时在相邻分区中delete数据; expect:删除数据成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            '执行失败' + text)
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0060执行完成--')
