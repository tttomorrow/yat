...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区插入数据,不相邻分区同时更新数据
Description :
    1、创建范围分区表;
    2、向分区3中插入数据;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0090开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0090'
        self.i_function_name = 'f_insert_locklevel_0090'
        self.u_function_name = 'f_update_locklevel_0090'
    ...
    def test_lock_level(self):
        ...
        text = '------step4 & step5:session2中在分区3中update数据,查看数据更新;  ' \
               'expect:update数据成功,数据更新成功------'
        self.log.info(text)
        update_cmd = f'''select {update_func}(40100);
            select * from {self.table_name} partition for (40100) limit 1;'''
        session_2 = ComThread(self.comsh.execut_db_sql, args=(update_cmd,))
        ...
        self.assertTrue(self.constant.START_TRANSACTION_SUCCESS_MSG
                        in session_1_result and
                        self.i_function_name in session_1_result and
                        self.constant.COMMIT_SUCCESS_MSG in session_1_result,
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0090执行完成------'
        self.log.info(text)
