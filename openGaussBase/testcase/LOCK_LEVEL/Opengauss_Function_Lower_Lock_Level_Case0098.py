...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区更新数据,相邻分区同时更新数据
Description :
    1、创建范围分区表;
    2、向分区1、分区2中插入数据;
    3、session1中开启事务，在分区1中更新数据;
    4、session2中同时在分区2更新数据;
    5、查看分区数据;
    6、清理环境;
Expect      :
    1、创建范围分区表成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0098开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0098'
        self.i_function_name = 'f_insert_locklevel_0098'
        self.u_function_name = 'f_update_locklevel_0098'
    ...
    def test_lock_level(self):
        ...
        self.log.info(text)
        sql = f'''select {insert_func}(5000);
            select {insert_func}(25000);
            select count(*) from {self.table_name};'''
        self.log.info(sql)
        msg = self.comsh.execut_db_sql(sql)
        self.log.info(msg)
        self.assertTrue('10002' in msg.splitlines()[-2].strip(),
                        '执行失败' + text)

        text = '------step3:session1中开启事务,在分区1中update数据; expect:成功------'
        self.log.info(text)
        update_cmd1 = f'''start transaction;
            select {update_func}(5000);
            select pg_sleep(5);
            select * from {self.table_name} partition for (5000) limit 1;
            commit;'''
        ...
        text = '------step4 & step5:session2中在分区2中update数据,查看数据更新;  ' \
               'expect:更新数据成功,查看数据成功------'
        self.log.info(text)
        update_cmd2 = f'''select {update_func}(25000);
            select * from {self.table_name} partition for (25000) limit 1;'''
        session_2 = ComThread(self.comSQL.execut_db_sql, args=(update_cmd2,))
        ...
        self.assertTrue('8' in session_2_result.splitlines()[-2],
                        "执行失败:" + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(
            ...
            '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0098执行完成------'
        self.log.info(text)
