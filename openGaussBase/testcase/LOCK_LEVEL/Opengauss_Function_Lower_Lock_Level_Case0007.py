...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区插入数据,相邻分区同时查看数据
Description :
    1、创建范围分区表;
    2、向分区2中插入数据;
    3、session1中向分区1插入数据;
    4、session2中同时在分区2查看数据;
    5、清理环境;
Expect      :
    1、创建范围分区表成功;
    2、插入数据成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0007开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0007'
        self.function_name = 'f_insert_locklevel_0007'
    ...
    def test_lock_level(self):
        ...
        text = '-----step2:首先向分区2中插入数据; expect:成功-----'
        self.log.info(text)
        sql = f'''select {insert_func}(39999);
            select count(*) from {self.table_name} partition for (39999);'''
        self.log.info(sql)
        msg = self.comsh.execut_db_sql(sql)
        self.log.info(msg)
        self.assertTrue('20000' in msg.splitlines()[-2].strip(),
                        '执行失败' + text)
        ...
        text = '------step4:session2中在分区2中查看数据; expect:查看数据成功------'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0007执行完成------'
        self.log.info(text)
