...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务alter add分区不提交,同时不相邻分区select数据
Description :
    1、创建范围分区表;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0076开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0076'
        self.i_function_name = 'f_insert_locklevel_0076'
    ...
    def test_lock_level(self):
        ...
        text = '---step3 & step4:session2中同时在分区2中select数据; expect:查看数据成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_TABLE_SUCCESS in drop_msg and
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0076执行完成------'
        self.log.info(text)
