...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区插入数据,不相邻分区同时删除数据
Description :
    1、创建范围分区表;
    2、向分区3中插入数据;
    3、session1中向分区1插入数据;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0006开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0006'
        self.i_function_name = 'f_insert_locklevel_0006'
        self.d_function_name = 'f_delete_locklevel_0006'
    ...
    def test_lock_level(self):
        ...
        text = '------step4 & step5:session2中在分区3中delete数据,查看数据;  ' \
               'expect:delete数据成功,数据删除成功------'
        self.log.info(text)
        delete_cmd = f'''select {delete_func}(45000);
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0006执行完成------'
        self.log.info(text)
