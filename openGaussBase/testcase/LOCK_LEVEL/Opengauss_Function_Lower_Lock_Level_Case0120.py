...
"""
Case Type   : 分区表锁降级
Case Name   : 显示事务truncate分区,通过分区键名删除不提交,其他分区查询的执行成功
Description :
    1、创建范围分区表,插入数据;
    2、session1中开启事务，通过分区键名删除分区数据，待查询执行后提交;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0120开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.tab_name = 't_locklevel_0120'
        self.i_function_name = 'f_insert_locklevel_0120'
    ...
    def test_lock_level(self):
        ...
        text = '---step2:session1中开启事务,通过分区键名删除分区数据,不提交; expect:成功---'
        self.log.info(text)
        update_cmd = f'''start transaction;
            alter table {self.tab_name} truncate partition p00;
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.constant.DROP_FUNCTION_SUCCESS_MSG in drop_msg
                        ...
                        '执行失败' + text)
        text = '------Opengauss_Function_Lower_Lock_Level_Case0120执行完成------'
        self.log.info(text)
