...
"""
Case Type   : 分区表锁降级
Case Name   : 分区表某一分区select数据,同时跨分区select数据
Description :
    1、创建范围分区表;
    2、向分区1、分区2、分区3中插入数据;
    3、session1中在分区1中select数据;
    4、session2中在所有分区中select数据;
    5、清理环境;
Expect      :
    1、创建范围分区表成功;
    2、插入数据成功;
...
class LockLevel(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Lower_Lock_Level_Case0027开始执行--')
        self.constant = Constant()
        self.comsh = CommonSH('PrimaryDbUser')
        self.comSQL = CommonSQL('PrimaryDbUser')
        self.table_name = 't_locklevel_0027'
        self.i_function_name = 'f_insert_locklevel_0027'
    ...
    def test_lock_level(self):
        ...
        text = '---step4:session2中在所有分区中select数据; expect:成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        text = '------Opengauss_Function_Lower_Lock_Level_Case0027执行完成------'
        self.log.info(text)
