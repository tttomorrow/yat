...
Case Name   : 对使用shufflemasking方式脱密的数据还原，查看数据还原成功
Description :
    1.poladmin用户创建表，并赋予表的所有操作权限给普通用户1
    2.poladmin用户将敏感字段加到资源标签
    3.poladmin用户设置脱敏策略shufflemasking,过滤用户1
...
class Security(unittest.TestCase):
    def setUp(self):
        text = '---Opengauss_Function_Security_Masking_Case0042 start---'
        logger.info(text)
        self.userNode = Node('PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.poladmin = 'poladmin_security_masking_0042'
        self.com_user = 'com_security_masking_0042'
        self.resource_label = 'rl_security_masking_0042'
        self.masking_policy = 'mp_security_masking_0042'
        self.table = f'{self.poladmin}.table_security_masking_0042'
    ...
    def test_masking(self):
        ...
        text = '---step2：poladmin用户将敏感字段加到资源标签,设置脱敏策略' \
               'shufflemasking,过滤用户1 expect:添加成功，脱敏策略设置成功---'
        logger.info(text)
        sql_cmd5 = f'drop resource label if exists {self.resource_label};' \
            ...
            f'shufflemasking on label({self.resource_label}) ' \
    ...
    def tearDown(self):
        ...
        logger.info(msg4)
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0042 finish---')
