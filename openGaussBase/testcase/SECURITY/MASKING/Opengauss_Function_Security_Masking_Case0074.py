...
Case Name   : 使用regexpmasking方式脱敏，过滤所有用户
Description :
    1.poladmin用户创建表
    2.poladmin用户将敏感字段加到资源标签
    3.poladmin用户设置脱敏策略regexpmasking,不过滤指定用户
    4.用户1连接数据库查看表string字段是否脱敏
    5.用户2连接数据库查看表string字段是否脱敏
Expect      :
    1.创表成功
    2.资源标签创建成功：CREATE RESOURCE LABEL
    3.脱敏策略添加成功
    4.查询到string字段的信息脱敏，第四位到第十位数字脱敏
    5.查询到string字段的信息脱敏，第四位到第十位数字脱敏
History     :
"""
import os
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        ...
        self.poladmin = 'pol_security_masking_0074'
        self.user1 = 'u01_security_masking_0074'
        self.user2 = 'u02_security_masking_0074'
        self.table = f'{self.poladmin}.table_security_masking_0074'
        self.res_label = 'rl_security_masking_0074'
        self.masking_policy = 'mp_security_masking_0074'
        self.common = Common()
    ...
    def test_masking(self):
        ...
        create_cmd = f'drop user if exists {self.poladmin};' \
            ...
            f'drop user if exists {self.user2};' \
            f'create user {self.poladmin} with POLADMIN password \'' \
            f'{macro.COMMON_PASSWD}\';' \
            f'create user {self.user1} with password ' \
            f'\'{macro.COMMON_PASSWD}\';' \
            f'create user {self.user2} with ' \
            f'password \'{macro.COMMON_PASSWD}\';' \
            f'grant all privileges to {self.user1};' \
            f'grant all privileges to {self.user2};'
        create_msg = self.sh_primy.execut_db_sql(create_cmd)
        self.logger.info(create_msg)
        self.assertTrue(
            create_msg.count(self.constant.CREATE_ROLE_SUCCESS_MSG) == 3,
            '执行失败' + text)
        self.assertTrue(
            create_msg.count(self.constant.ALTER_ROLE_SUCCESS_MSG) == 2,
            '执行失败' + text)
        ...
        sql_cmd3 = f'drop masking policy if exists {self.masking_policy};' \
            ...
            f'regexpmasking(\'[\d+]\',\'*\',2,10) on ' \
            f'label({self.res_label});'
        ...
        self.assertIn('13**-****-**54-8988-123', msg4, '执行失败' + text)
        self.assertIn('64**-****-**68-5477-580', msg4, '执行失败' + text)
        
        text = '--step5:用户2连接数据库查看表string字段是否脱敏;expect:成功--'
        self.logger.info(text)
        content3 = f'-U {self.user2} -W {macro.COMMON_PASSWD}'
        msg5 = self.sh_primy.execut_db_sql(sql_cmd4, sql_type=f'{content3}')
        self.logger.info(msg4)
        self.assertIn('13**-****-**54-8988-123', msg5, '执行失败' + text)
        self.assertIn('64**-****-**68-5477-580', msg5, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        drop_user_cmd = f'drop user {self.user1} cascade;' \
            f'drop user {self.user2} cascade;' \
        ...
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, drop_msg,
                      '执行失败' + text)
        self.assertTrue(
            drop_user_msg.count(self.constant.DROP_ROLE_SUCCESS_MSG) == 3,
            '执行失败' + text)
