...
Case Name   : 使用randommasking方式脱密，未经过滤的用户不受脱敏策略限制
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        text = '---Opengauss_Function_Security_Masking_Case0050 start---'
        logger.info(text)
        self.userNode = Node('PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.poladmin = 'poladmin_security_masking_0050'
        self.com_user = 'com_security_masking_0050'
        self.com_user02 = 'com02_security_masking_0050'
        self.resource_label = 'rl_security_masking_0050'
        self.masking_policy = 'mp_security_masking_0050'
        self.table = f'{self.poladmin}.table_security_masking_0050'
    
    def test_masking(self):
        text = '---step1：poladmin用户创建表，并赋予表的所有操作权限给普通用户1、2 ' \
               'expect:创建表成功，权限赋予成功-------'
        logger.info(text)
        ...
        sql_cmd4 = f'''drop table if exists {self.table};
                ...
                'Shanxi,Xian,yuhuazhai'),(2,'李四','543210',
                'Fujian');'''
    ...
    def tearDown(self):
        ...
        logger.info(msg4)
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0050 finish---')
