...
Case Name   : 对使用randommasking方式脱密的数据还原，查看数据还原成功
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        text = '---Opengauss_Function_Security_Masking_Case0049 start---'
        logger.info(text)
        self.userNode = Node('PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.poladmin = 'poladmin_security_masking_0049'
        self.com_user = 'com_security_masking_0049'
        self.resource_label = 'rl_security_masking_0049'
        self.masking_policy = 'mp_security_masking_0049'
        self.table = f'{self.poladmin}.table_security_masking_0049'
    ...
    def test_masking(self):
        ...
        sql_cmd4 = f'''drop table if exists {self.table};
                ...
                insert into {self.table} values(1,'张三','123456',
                'Shanxi,Xian,yuhuazhai'),(2,'李四','654321',
                'Fujian');'''
        ...
        assert_1 = msg6_list[2].strip() != '123456' and \
                   msg6_list[3].strip() != '654321'
        self.assertTrue(assert_1, '执行失败:' + text)
        text = '---step4：还原脱敏字段 expect:还原成功---'
        logger.info(text)
        sql_cmd7 = f'alter resource label {self.resource_label} ' \
            f'remove column({self.table}.string);'
        ...
        logger.info(msg7)
        assert_1 = self.constant.resource_label_alter_success_msg in msg7
        self.assertTrue(assert_1, '执行失败:' + text)
        text = '---step5：查询出来的内容是否显示完整 expect:内容显示完整---'
        logger.info(text)
        ...
        assert_1 = msg8_list[2].strip() == '123456' and \
                   msg8_list[3].strip() == '654321'
        self.assertTrue(assert_1, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        msg3 = self.sh_primy.execut_db_sql(sql_cmd3)
        logger.info(msg3)
        sql_cmd4 = f'drop user {self.com_user};drop user {self.poladmin};'
        msg4 = self.sh_primy.execut_db_sql(sql_cmd4)
        logger.info(msg4)
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0049 finish---')
