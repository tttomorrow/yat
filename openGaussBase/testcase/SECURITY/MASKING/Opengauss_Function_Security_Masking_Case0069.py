...
Case Name   : 使用regexpmasking方式脱敏，对身份证格式的文本类型脱敏
Description :
    1.poladmin用户创建表
...
Expect      :
    ...
    3.脱敏策略添加成功
    4.查询到string字段的信息脱敏，第四位到第十三位数字脱敏
History     :
"""
import os
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        ...
        self.poladmin = 'pol_security_masking_0069'
        self.user = 'u_security_masking_0069'
        self.table = f'{self.poladmin}.table_security_masking_0069'
        self.res_label = 'rl_security_masking_0069'
        self.masking_policy = 'mp_security_masking_0069'
        self.common = Common()
    ...
    def test_masking(self):
        ...
        sql_cmd1 = f'drop table if exists {self.table};' \
            ...
            f'char(18),address varchar(60));' \
            f'insert into {self.table} values(1,\'张三\',' \
            f'\'640233199512142423\',\'Shanxi,Xian,yuhuazhai\'),' \
            f'(2,\'李四\',\'140321198510161516\',\'Fujian\');'
        ...
        self.assertIn('640**********42423', msg4, '执行失败' + text)
        self.assertIn('140**********61516', msg4, '执行失败' + text)
