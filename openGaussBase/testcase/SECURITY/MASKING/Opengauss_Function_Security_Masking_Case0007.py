...
Case Name   : 对身份证格式的文本类型使用creditcardmasking方式脱密，查看数据脱敏成功
Description :
    1.管理员用户创建表，并赋予表的所有操作权限给用户user004
    2.poladmin用户将敏感字段加到资源标签
    3.poladmin用户设置脱敏策略creditcardmasking
    4.user004用户连接数据库查看person表identify字段是否脱敏
Expect      :
    1.创表成功，赋权成功
    2.资源标签创建成功：CREATE RESOURCE LABEL
    3.脱敏策略添加成功
    4.identify字段的信息已做脱敏处理，后四位之前的数字做了脱敏处理
History     :
"""
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Security_Masking_Case0007 start---')
    ...
    def test_masking(self):
        ...
        sql_cmd4 = '''create table public.person(id int,name char(10),
                ...
                insert into person values(1,'张三','623604198707121615',
                'Shanxi,Xian,yuhuazhai'),(2,'李四','423604199807223231',
                'Fujian');'''
        ...
        self.assertTrue(msg6_list[2].strip() == 'xxxxxxxxxxxxxx1615' and
                        msg6_list[3].strip() == 'xxxxxxxxxxxxxx3231')
    ...
    def tearDown(self):
        ...
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0007 finish---')
