...
Case Name   : 对使用regexpmasking方式脱敏的数据还原，查看数据还原成功
Description :
    1.poladmin用户创建表
...
class Security(unittest.TestCase):
    def setUp(self):
        ...
        self.poladmin = 'pol_security_masking_0072'
        self.user = 'u_security_masking_0072'
        self.table = f'{self.poladmin}.table_security_masking_0072'
        self.res_label = 'rl_security_masking_0072'
        self.masking_policy = 'mp_security_masking_0072'
        self.common = Common()
    ...
    def test_masking(self):
        ...
        text = '--step5:还原脱敏字段;expect:成功--'
        self.logger.info(text)
        sql_cmd5 = f'alter resource label {self.res_label} remove ' \
            f'column({self.table}.string);'
        msg5 = self.sh_primy.execut_db_sql(sql_cmd5, sql_type=f'{content1}')
        self.logger.info(msg5)
        self.assertIn(self.constant.resource_label_alter_success_msg, msg5,
                      '执行失败' + text)

        text = '--step6:查看字段还原成功，查询出来的内容是否显示完整;expect:成功--'
        self.logger.info(text)
        sql_cmd6 = f'select string from {self.table};'
        msg6 = self.sh_primy.execut_db_sql(sql_cmd6, sql_type=f'{content2}')
        self.logger.info(msg6)
        self.assertIn('6402-3372-0368-5477-580', msg6, '执行失败' + text)
        self.assertIn('1354-7676-6854-8988-123', msg6, '执行失败' + text)
