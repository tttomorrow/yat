...
Case Name   : 使用regexpmasking方式脱敏，过滤IP地址
Description :
    1.poladmin用户创建表
    2.poladmin用户将敏感字段加到资源标签
    3.poladmin用户设置脱敏策略regexpmasking,过滤IP地址
...
class Security(unittest.TestCase):
    def setUp(self):
        ...
        self.poladmin = 'pol_security_masking_0079'
        self.user = 'u_security_masking_0079'
        self.table = f'{self.poladmin}.table_security_masking_0079'
        self.res_label = 'rl_security_masking_0079'
        self.masking_policy = 'mp_security_masking_0079'
        self.common = Common()
    ...
    def test_masking(self):
        ...
        text = '-----step3:poladmin用户设置脱敏策略regexpmasking,过滤gsql客户端;' \
               'expect:成功-----'
        self.logger.info(text)
        host_ip = self.userNode.db_host
        sql_cmd3 = f'drop masking policy if exists {self.masking_policy};' \
            ...
            f'label({self.res_label}) filter on IP(\'{host_ip}\');'
        ...
        content2 = f'-U {self.user} -W {macro.COMMON_PASSWD} -h {host_ip}'
