...
Case Name   : 使用regexpmasking方式脱敏，未经过滤的用户不受脱敏策略限制
Description :
    1.poladmin用户创建表
...
class Security(unittest.TestCase):
    def setUp(self):
        ...
        self.poladmin = 'pol_security_masking_0073'
        self.user1 = 'u01_security_masking_0073'
        self.user2 = 'u02_security_masking_0073'
        self.table = f'{self.poladmin}.table_security_masking_0073'
        self.res_label = 'rl_security_masking_0073'
        self.masking_policy = 'mp_security_masking_0073'
        self.common = Common()
    ...
    def test_masking(self):
        ...
        sql_cmd3 = f'drop masking policy if exists {self.masking_policy};' \
            ...
            f'label({self.res_label}) filter on roles({self.user1});'
        ...
        text = '--step5:用户2连接数据库查看表string字段是否脱敏;expect:失败--'
        self.logger.info(text)
        content3 = f'-U {self.user2} -W {macro.COMMON_PASSWD}'
        msg5 = self.sh_primy.execut_db_sql(sql_cmd4, sql_type=f'{content3}')
        self.logger.info(msg4)
        self.assertIn('6402-3372-0368-5477-580', msg5, '执行失败' + text)
        self.assertIn('1354-7676-6854-8988-123', msg5, '执行失败' + text)
