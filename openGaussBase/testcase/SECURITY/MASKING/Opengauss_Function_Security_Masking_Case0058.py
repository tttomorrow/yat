...
Case Name   : 使用maskall方式脱密，对所有的用户设置脱敏策略
Description :
    1.poladmin用户创建表，并赋予表的所有操作权限给用户1、用户2
    2.poladmin用户将敏感字段加到资源标签
    3.poladmin用户设置脱敏策略maskall,过滤所有用户
    4.用户1、用户2分别连接数据库查看表的string字段是否脱敏
Expect      :
    1.创表成功，赋权成功
    2.资源标签创建成功：CREATE RESOURCE LABEL
    3.脱敏策略添加成功
    4.2个用户均查询到string字段的信息脱敏
History     :
"""
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        text = '---Opengauss_Function_Security_Masking_Case0058 start---'
        logger.info(text)
        self.userNode = Node('PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.poladmin = 'poladmin_security_masking_0058'
        self.com_user = 'com_security_masking_0058'
        self.com_user02 = 'com02_security_masking_0058'
        self.resource_label = 'rl_security_masking_0058'
        self.masking_policy = 'mp_security_masking_0058'
        self.table = f'{self.poladmin}.table_security_masking_0058'
    ...
    def test_masking(self):
        ...
        text = '---step2：poladmin用户将敏感字段加到资源标签,设置脱敏策略' \
               'maskall,不指定过滤用户 expect:添加成功，脱敏策略设置成功---'
        logger.info(text)
        ...
        self.assertTrue(assert_1 and assert_2, '执行失败:' + text)
        text = '---step3：用户1连接数据库查看表的string字段脱敏 expect:脱敏成功---'
        logger.info(text)
        ...
        text = '---step4：用户1连接数据库查看表的string字段脱敏 expect:脱敏成功---'
        logger.info(text)
        ...
        assert_1 = msg7_list[2].strip() == 'xxxxxxxxxxxxxxxxx' and \
                   msg7_list[3].strip() == 'xxxxxxxxxxxxxxxxx'
        self.assertTrue(assert_1, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        logger.info(msg4)
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0058 finish---')
