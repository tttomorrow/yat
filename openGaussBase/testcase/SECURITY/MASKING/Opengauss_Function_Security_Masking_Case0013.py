...
Case Name   : 对不支持的数据类型使用creditcardmasking方式脱密，查看数据脱敏失败
Description :
...
Expect      :
    ...
    3.脱敏策略添加成功
    4.creditcard字段的信息脱敏为0（默认使用maskall方式）
History     :
"""
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Security_Masking_Case0013 start---')
    ...
    def test_masking(self):
        ...
        sql_cmd4 = '''create table public.person(id int,name char(10),
                creditcard bigint,address varchar(60));
    ...
    def tearDown(self):
        ...
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0013 finish---')
