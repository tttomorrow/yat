...
Case Name   : 使用shufflemasking方式脱密，未经过滤的用户不受脱敏策略限制
Description :
    1.poladmin用户创建表，并赋予表的所有操作权限给用户1
    3.poladmin用户设置脱敏策略shufflemasking,过滤用户1
    4.用户1用户连接数据库查看表的string字段是否脱敏
    5.用户2用户连接数据库查看表的string字段是否脱敏
Expect      :
    1.创表成功，赋权成功
...
class Security(unittest.TestCase):
    def setUp(self):
        text = '---Opengauss_Function_Security_Masking_Case0045 start---'
        logger.info(text)
        self.userNode = Node('PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.poladmin = 'poladmin_security_masking_0045'
        self.com_user = 'com_security_masking_0045'
        self.com_user02 = 'com02_security_masking_0045'
        self.resource_label = 'rl_security_masking_0045'
        self.masking_policy = 'mp_security_masking_0045'
        self.table = f'{self.poladmin}.table_security_masking_0045'
    ...
    def test_masking(self):
        ...
        sql_cmd1 = f'drop user if exists {self.poladmin};' \
            ...
            f'create user {self.com_user02} with POLADMIN password \'' \
            f'{macro.COMMON_PASSWD}\';' \
        ...
        text = '---step4：登录用户2查看表的string字段脱敏 expect:未脱敏---'
        logger.info(text)
        sql_cmd6 = f'select string from {self.table};'
        excute_cmd6 = f'source {macro.DB_ENV_PATH};' \
            f'gsql -d {self.userNode.db_name} -p ' \
    ...
    def tearDown(self):
        ...
        logger.info(msg4)
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0045 finish---')
