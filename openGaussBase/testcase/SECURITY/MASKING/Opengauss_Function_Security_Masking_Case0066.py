...
Case Name   : 源表中脱敏列使用带有query子句的INSERT操作，插入的结果为脱敏后的值，不可还原
Description :
    1.poladmin用户创建表，并赋予表的所有操作权限给用户user004
    2.poladmin用户将敏感字段加到资源标签
    3.poladmin用户设置脱敏策略creditcardmasking
    4.源表中脱敏列使用带有query子句的INSERT操作，由被过滤的用户执行
    5.查看poladmin.person02
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Masking_Case0066 start---')
        self.userNode = Node('PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.com_user = 'com_security_masking_0066'
        self.poladmin = 'poladmin_security_masking_0066'
        self.resource_label = 'rl_security_masking_0066'
        self.masking_policy = 'mp_security_masking_0066'
        self.table_1 = f'{self.poladmin}.table_1_security_masking_0066'
        self.table_2 = f'{self.poladmin}.table_2_security_masking_0066'
    ...
    def test_masking(self):
        ...
        self.logger.info(text)
        sql_cmd4 = f'''drop table if exists {self.table_1};
                       drop table if exists {self.table_2};
                       create table {self.table_1}(id int,name char(10),
                       creditcard varchar(25));
                       create table {self.table_2}(id int,name char(10),
                       creditcard varchar(25));
                       insert into {self.table_1} values
        ...
        sql_cmd5 = f'drop resource label if exists {self.resource_label};' \
                   ...
                   f'add column({self.table_1}.creditcard);'
        ...
        text = '----step:4源表中脱敏列使用带有query子句的INSERT操作，' \
               '由被过滤的用户执行，expect:数据插入成功:INSERT----'
        self.logger.info(text)
        sql_cmd7 = f'insert into {self.table_2} ' \
                   f'(select * from {self.table_1} where id=1);'
        ...
        assert_7 = self.constant.INSERT_SUCCESS_MSG in msg7
        self.assertTrue(assert_7, '执行失败：' + text)

        text = '-----step5: 查看poladmin.person02,' \
               'except:creditcard 字段数据脱敏-----'
        self.logger.info(text)
        ...
        assert_8 = 'xxxx-xxxx-xxxx-xxxx-645' in msg8
    ...
    def tearDown(self):
        ...
        sql_cmd1 = f'drop masking policy {self.masking_policy};' \
                   ...
                   f'drop table {self.table_1};' \
                   f'drop table {self.table_2};'
        ...
        self.logger.info(msg1)
        sql_cmd2 = f'drop user {self.com_user};' \
        ...
        self.logger.info(
            '---Opengauss_Function_Security_Masking_Case0066 finish---')