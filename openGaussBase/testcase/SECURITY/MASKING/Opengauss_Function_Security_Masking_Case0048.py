...
Case Name   : 使用randommasking脱密策略，文本中为两个不同字符，原来的文本按字符随机脱敏
Description :
    1.poladmin用户创建表，并赋予表的所有操作权限给用户1
    2.poladmin用户将敏感字段加到资源标签
    3.poladmin用户设置脱敏策略randommasking,过滤user001
    4.用户1用户查看数据是否脱敏
Expect      :
    1.创表成功，赋权成功
...
class Security(unittest.TestCase):
    def setUp(self):
        text = '---Opengauss_Function_Security_Masking_Case0048 start---'
        logger.info(text)
        self.userNode = Node('PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.poladmin = 'poladmin_security_masking_0048'
        self.com_user = 'com_security_masking_0048'
        self.resource_label = 'rl_security_masking_0048'
        self.masking_policy = 'mp_security_masking_0048'
        self.table = f'{self.poladmin}.table_security_masking_0048'
    ...
    def test_masking(self):
        ...
        sql_cmd4 = f'''drop table if exists {self.table};
                ...
                insert into {self.table} values(1,'张三','qa',
                'Shanxi,Xian,yuhuazhai'),(2,'李四','st',
                'Fujian');'''
        ...
        assert_1 = (msg6_list[2].strip() != 'qa' and
                    msg6_list[3].strip() != 'st')
    ...
    def tearDown(self):
        ...
        logger.info(msg4)
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0048 finish---')
