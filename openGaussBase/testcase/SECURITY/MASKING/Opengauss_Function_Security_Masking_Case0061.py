...
Case Name   : 列存表的字段设置masking脱敏策略
Description :
    1.poladmin用户创建列存表，并赋予表的所有操作权限给用户1
...
class Security(unittest.TestCase):
    def setUp(self):
        text = '---Opengauss_Function_Security_Masking_Case0061 start---'
        logger.info(text)
        self.userNode = Node('PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.poladmin = 'poladmin_security_masking_0061'
        self.com_user = 'com_security_masking_0061'
        self.resource_label = f'rl_security_masking_0061'
        self.masking_policy = f'mp_security_masking_0061'
        self.table = f'{self.poladmin}.table_security_masking_0061'
    ...
    def test_masking(self):
        ...
        sql_cmd4 = f'''drop table if exists {self.table};
                ...
                varchar2(25),address varchar(60)) with (orientation =column);
        ...
        text = '---step2：poladmin用户将敏感字段加到资源标签 expect:添加成功---'
        logger.info(text)
    ...
    def tearDown(self):
        ...
        logger.info(msg4)
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0061 finish---')
