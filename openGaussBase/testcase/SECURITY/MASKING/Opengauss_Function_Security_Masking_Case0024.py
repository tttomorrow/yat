...
Case Name   : 使用basicemailmasking方式脱密，存量数据和新增数据均被脱敏处理
Description :
    1.poladmin用户创建表，并赋予表的所有操作权限给用户user001
    2.poladmin用户将敏感字段加到资源标签
    3.poladmin用户设置脱敏策略basicemailmasking,过滤user001
...
Expect      :
    ...
    5.查询到email字段的存量数据和新增数据均脱敏处理，后四位之前的数字做了脱敏处理
History     :
"""
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Security_Masking_Case0024 start---')
    ...
    def test_masking(self):
        ...
        sql_cmd5 = 'create resource label email_lable add column(' \
                   'public.person.email);' \
                   'create masking policy mask_card_pol basicemailmasking ' \
        ...
        self.assertTrue(msg6_list[2].strip() == 'xxxxxxxxx@qq.com' and
                        msg6_list[3].strip() == 'xxxxxxxxx@qq.com')
        logger.info('------插入新的数据------')
        sql_cmd7 = '''insert into person values(1,'王五','454356789@qq.com',
                'Shanxi,Xian,yuhuazhai'),(2,'朱七','876578324@qq.com',
                'Fujian');'''
        msg7 = self.sh_primy.execut_db_sql(sql_cmd4)
        logger.info(msg7)
        logger.info('------user001查看存量数据和增量数据均脱敏------')
        ...
        self.assertTrue(msg8_list[2].strip() == 'xxxxxxxxx@qq.com' and
                        msg8_list[3].strip() == 'xxxxxxxxx@qq.com' and
                        msg8_list[4].strip() == 'xxxxxxxxx@qq.com' and
                        msg8_list[5].strip() == 'xxxxxxxxx@qq.com')
    ...
    def tearDown(self):
        ...
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0024 finish---')
