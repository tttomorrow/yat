...
Case Name   : 使用alldigitsmasking方式脱密，存量数据和新增数据均被脱敏处理
Description :
...
Expect      :
    ...
    3.脱敏策略添加成功
    4.查询到email字段的信息脱敏，所有的字符顺序打乱
    5.查询到email字段的存量数据和新增数据均脱敏处理，所有的字符顺序打乱
History     :
"""
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Security_Masking_Case0038 start---')
    ...
    def test_masking(self):
        ...
        logger.info('---------表中插入新的数据----------')
        sql_cmd7 = '''insert into person values(1,'王五','tty9176@qq.com',
                'Shanxi,Xian,yuhuazhai'),(2,'朱七','iuo7896@qq.com',
                'Fujian');'''
        msg7 = self.sh_primy.execut_db_sql(sql_cmd7)
        logger.info(msg7)
        logger.info('-------查看存量数据和增量数据是否均被脱敏---------')
        ...
        self.assertTrue(msg8_list[2].strip() == 'qaz0000@qq.com' and
                        msg8_list[3].strip() == 'edc0000@qq.com' and
                        msg8_list[4].strip() == 'tty0000@qq.com' and
                        msg8_list[5].strip() == 'iuo0000@qq.com')
    ...
    def tearDown(self):
        ...
        logger.info(
            '---Opengauss_Function_Security_Masking_Case0038 finish---')