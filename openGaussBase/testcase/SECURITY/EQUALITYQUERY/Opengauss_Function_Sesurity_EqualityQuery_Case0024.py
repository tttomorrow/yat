...
"""
Case Type   : 全密态
Case Name   : 加密列加密类型为随机加密时，不支持密态等值查询
Description :
    1、配置全密态环境变量
    2、连接全密态数据库，分别创建cmk，cek
    3、创建加密表,对随机加密列类型进行查询操作
    4、清理环境
Expect      :
    1、配置成功
    2、成功
...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0024'
        self.cek = 'cek_security_qualityquery_0024'
        self.tab = 't_security_qualityquery_0024'
        self.key_path = 'keypath_security_qualityquery_0024'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '---step3:创建加密表,对随机加密列类型进行查询操作;expect:失败---'
        self.logger.info(text)
        sql_cmd = f''' drop table if exists {self.tab};
            ...
            select * from {self.tab} where credit_card = '123';'''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertTrue(self.Constant.TABLE_CREATE_SUCCESS in msg1)
        self.assertIn('not allowed', msg1, text + '执行失败')
