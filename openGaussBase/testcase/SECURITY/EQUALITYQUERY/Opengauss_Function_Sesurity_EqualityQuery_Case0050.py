...
Case Name   : 修改列名后插入新的数据，非密态客户端查看该加密表的存量数据和新增数据均加密
Description :
    1.gsql客户端-C连接数据库，创建cmk
    2.gsql客户端-C连接数据库，创建cek
    3.gsql客户端-C连接数据库，创建加密表
    4.gsql客户端不加-C连接数据库，查询加密数据
    5.修改列名，并更新数据，表中插入新的数据
    6.gsql客户端不加-C连接数据库，查询加增量数据和存量数据
Expect      :
    1.创建成功
    2.创建成功
    3.创建成功，数据插入完成
...
class Equality(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0050'
        self.cek = 'cek_security_qualityquery_0050'
        self.key_path = 'keypath_security_qualityquery_0050'
        self.table = 'table_security_qualityquery_0050'
    ...
    def test_security(self):
        ...
        text = '--step4:gsql客户端不加-C连接数据库，查询加密列信息;expect:成功--'
        self.logger.info(text)
        sql_cmd4 = f'select name from {self.table} where id_number=\'1\';'
        msg4 = self.sh_primy.execut_db_sql(sql_cmd4)
        self.logger.info(msg4)
        self.assertNotIn('joe', msg4.splitlines()[2], text + '执行失败')
        
        text = '--step5:修改列名，并更新数据，表中插入新的数据;expect:成功--'
        self.logger.info(text)
        sql_cmd5 = f'alter table {self.table} rename column name to newname;' \
            f'update {self.table} set newname = \'lucy\' where ' \
            f'id_number = 1;insert into {self.table} ' \
            f'values(2, \'joy\',\'6219985678349800033\');'
        msg5 = self.sh_primy.execut_db_sql(sql_cmd5, sql_type='-C')
        self.logger.info(msg5)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, msg5, text + '执行失败')
        self.assertIn(self.constant.ALTER_TABLE_MSG, msg5, text + '执行失败')
        
        text = '--step6:gsql客户端不加-C连接数据库，查询加增量数据和存量数据;' \
               'expect:成功--'
        self.logger.info(text)
        sql_cmd6 = f'select newname from {self.table};'
        msg6 = self.sh_primy.execut_db_sql(sql_cmd6)
        self.logger.info(msg6)
        self.assertTrue(msg6.count('\\x0') == 3, '执行失败:' + text)
