...
"""
Case Type   : 全密态
Case Name   : 对确定性加密表支持使用ANALYZE进行统计
Description :
    1、配置全密态环境变量
    2、连接数据库，分别创建cmk,cek
    3、对确定性加密表进行analyze操作
    4、清理环境
Expect      :
    1、配置成功
    2、成功
...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0023'
        self.cek = 'cek_security_qualityquery_0023'
        self.tab = 't_security_qualityquery_0023'
        self.key_path = 'keypath_security_qualityquery_0023'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '---step2:连接数据库，分别创建cmk,cek;expect:成功---'
        self.logger.info(text)
        ...
        text = '---step3:对确定性加密表进行analyze操作，expect：成功'
        self.logger.info(text)
        ...
        analyze verbose {self.tab};'''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertTrue('CREATE TABLE' in msg1)
        self.assertIn('ANALYZE', msg1, text + '执行失败')
