...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0010'
        self.cek = 'cek_security_qualityquery_0010'
        self.tab = 't_security_qualityquery_0010'
        self.key_path = 'keypath_security_qualityquery_0010'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '--step2:连接全密态数据库创建cmk,cek，建加密表;' \
               'expect:创建成功--'
        self.logger.info(text)
        ...
        text = '---step3:插入数据,执行(Var = Const),in,表达式过滤条件的查询;' \
               'expect:失败---'
        self.logger.info(text)
        sql_cmd = f'''insert into {self.tab} 
        values(11,'Nancy','322404188811094044'),
        (12,'Hellen','3224091991050650X2'),
        (14,'Lucy','489234902380240309');
        select * from {self.tab} where name = 'Nancy';
        select * from {self.tab} where name in ('Hellen','Nancy');
        select * from {self.tab} where credit_card in 
        (322311188811094013,'3224091991050650X2');
        select * from {self.tab} where name = lower('NANCY');'''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertTrue(self.Constant.INSERT_SUCCESS_MSG in msg1 and
                        'Nancy' in msg1 and 'Hellen'
                        in msg1 and 'not allowed' in msg1 and 'No' in msg1)
