...
"""
Case Type   : 全密态
Case Name   :支持通过客户端执行COPY 语法实现加密表中导入数据
Description :
...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0025'
        self.cek = 'cek_security_qualityquery_0025'
        self.tab = 't_security_qualityquery_0025'
        self.DB_INSTANCE_PATH = macro.DB_INSTANCE_PATH
        self.key_path = 'keypath_security_qualityquery_0025'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '---step2:连接全密态数据库，分别创建cmk，cek,创建加密表;expect:成功---'
        self.logger.info(text)
        ...
        text = '---step3:通过COPY命令往加密表中导入数据;expect:成功---'
        self.logger.info(text)
        ...
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertIn('Nancy', msg1, text + '执行失败')
