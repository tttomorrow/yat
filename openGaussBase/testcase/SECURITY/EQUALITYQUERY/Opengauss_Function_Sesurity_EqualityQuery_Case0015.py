...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0015'
        self.cek = 'cek_security_qualityquery_0015'
        self.tab = 't_security_qualityquery_0015'
        self.key_path = 'keypath_security_qualityquery_0015'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '---step3:插入数据，进行集合类操作INTERSECT;expect:成功---'
        self.logger.info(text)
        sql_cmd = f'''insert into {self.tab} values
            ...
            intersect select name from {self.tab} where id_number > 12;
            '''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertIn(self.Constant.INSERT_SUCCESS_MSG, msg1, text + '执行失败')
        self.assertIn('Fiona' and 'Amy' and 'Lucy', msg1, text + '执行失败')
