...
"""
Case Type   : 全密态
Case Name   : 加密表不支持加密列的Sort算子操作
Description :
    1、配置全密态环境变量
    2、连接全密态数据库，分别创建cmk，cek
    3、创建加密表,对加密列进行Sort算子操作
    4、清理环境
Expect      :
    1、配置成功
    2、成功
...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0019'
        self.cek = 'cek_security_qualityquery_0019'
        self.tab = 't_security_qualityquery_0019'
        self.key_path = 'keypath_security_qualityquery_0019'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '---step3:创建加密表,对加密列进行Sort算子操作;expect:失败---'
        self.logger.info(text)
        sql_cmd = f'''drop table if exists {self.tab};
            ...
            select * from {self.tab} order by name;
            '''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertTrue(self.Constant.TABLE_CREATE_SUCCESS in msg1
                        and self.Constant.INSERT_SUCCESS_MSG in msg1)
        self.assertIn('could not support', msg1, text + '执行失败')
