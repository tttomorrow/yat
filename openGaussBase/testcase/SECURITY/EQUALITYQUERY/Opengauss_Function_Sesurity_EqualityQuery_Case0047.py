...
Case Name   : 关闭密态开关，查询加密表的数据显示密文
Description :
    1.gsql客户端-C连接数据库，创建cmk
    2.gsql客户端-C连接数据库，创建cek
    3.gsql客户端-C连接数据库，创建加密表
    4.gsql客户端不加-C连接数据库，加密列作为查询条件进行等值查询
    5.gsql客户端不加-C连接数据库，查询加密列信息
Expect      :
    1.创建成功
    2.创建成功
    3.创建成功，数据插入完成
    4.ERROR:  invalid input syntax for type
    5.查询出来的值以密文显示
History     :
"""
import os
...
class Equality(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0047'
        self.cek = 'cek_security_qualityquery_0047'
        self.key_path = 'keypath_security_qualityquery_0047'
        self.table = 'table_security_qualityquery_0047'
