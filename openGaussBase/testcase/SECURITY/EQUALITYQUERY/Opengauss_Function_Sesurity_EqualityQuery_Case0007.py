...
"""
Case Type   : 全密态
Case Name   : 创建不支持加密的数据类型的加密表
Description :
    1、配置全密态环境变量
    2、连接全密态数据库，分别创建cmk，cek,创建不支持加密的数据类型的加密表
    3、清理环境
Expect      :
    1、配置成功
    2、创建表失败，合理提示报错信息
...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0007'
        self.cek = 'cek_security_qualityquery_0007'
        self.tab = 't_security_qualityquery_0007'
        self.key_path = 'keypath_security_qualityquery_0007'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '--step2:连接全密态数据库，创建cmk，cek,' \
               '创建不支持加密数据类型的加密表;expect:失败，合理提示报错信息--'
        self.logger.info(text)
        sql_cmd = f'''drop client master key if exists {self.cmk} cascade;
            ...
            create table {self.tab}(id_number int,date timestamp 
        ...
        self.assertTrue(self.Constant.create_cmk_success in msg1 and
                        ...
                        'not implemented' in msg1)

    def tearDown(self):
        text = '--step3:清理环境;expect:成功-------'
        self.logger.info(text)
        sql_cmd = f'drop column encryption key {self.cek} cascade;' \
