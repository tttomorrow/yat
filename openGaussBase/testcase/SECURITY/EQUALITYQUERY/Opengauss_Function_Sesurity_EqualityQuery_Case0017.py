...
"""
Case Type   : 全密态
Case Name   : SUID支持包含非加密列相关查询
Description :
    1、配置$GAUSSHOME/etc/localkms路径
    2、连接全密态数据库，分别创建cmk，cek，创建加密表
    3、插入数据，进行非加密列相关查询
    4、关闭密态数据库的开关，使用非密态客户端进行非加密列相关查询;
    5、清理环境
Expect      :
    1、配置成功
    2、成功
    3、成功
    4、成功，显示非加密列信息，加密列信息以密文显示
    5、成功
History     :
"""
import unittest
...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0017'
        self.cek = 'cek_security_qualityquery_0017'
        self.tab = 't_security_qualityquery_0017'
        self.key_path = 'keypath_security_qualityquery_0017'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '---step3:插入数据,进行非加密列相关查询;expect:成功---'
        self.logger.info(text)
        sql_cmd = f'''insert into {self.tab} values
            ...
            select * from {self.tab} where id_number = 12;
            '''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertTrue(self.Constant.INSERT_SUCCESS_MSG, msg1)
        self.assertIn('12 | Hellen | 3224091991050650X2', msg1, text + '执行失败')
        text = '---step4:关闭密态数据库的开关，进行非加密列相关查询;' \
               'expect:成功，显示非加密列信息，加密列信息以密文显示---'
        self.logger.info(text)
        sql_cmd = f'select * from {self.tab} where id_number = 12;'
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd)
        self.logger.info(msg1)
        self.assertIn('12 | \\x01', msg1, text + '执行失败')
