...
"""
Case Type   : 全密态
Case Name   : 创建加密列指定默认值的加密表
Description :
    1、配置$GAUSSHOME/etc/localkms路径
    2、连接全密态数据库，创建cmk，cek
    3、,创建加密列指定默认值的加密表,查看表结构
    4、插入数据，查看加密列指定默认值的加密表信息
    5、清理环境
Expect      :
    1、配置成功
    2、成功
...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0008'
        self.cek = 'cek_security_qualityquery_0008'
        self.tab = 't_security_qualityquery_0008'
        self.key_path = 'keypath_security_qualityquery_0008'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '---step3:创建加密列指定默认值的加密表,查看表结构;expect:成功---'
        self.logger.info(text)
        sql_cmd = f'''drop table if exists {self.tab};
            create table {self.tab}(product_no int default 1, name text 
            encrypted with(column_encryption_key = {self.cek} , 
            encryption_type = randomized),price 
            numeric encrypted with(column_encryption_key = {self.cek} , 
            encryption_type = randomized) default 10);
            \d {self.tab};'''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertTrue(self.Constant.TABLE_CREATE_SUCCESS in msg1
                        and 'default' in msg1)
        text = '--step4:插入数据，查看加密列指定默认值的加密表信息;expect:成功--'
        self.logger.info(text)
        sql_cmd = f'''insert into {self.tab} (name) values('lala');
            select * from {self.tab}'''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertIn(self.Constant.INSERT_SUCCESS_MSG, msg1, text + '执行失败')
        self.assertTrue('1' in msg1 and '10' in msg1)
