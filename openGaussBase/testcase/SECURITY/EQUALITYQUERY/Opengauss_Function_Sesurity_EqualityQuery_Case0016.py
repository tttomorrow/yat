...
class FullDensityState(unittest.TestCase):
    def setUp(self):
        ...
        self.cmk = 'cmk_security_qualityquery_0016'
        self.cek = 'cek_security_qualityquery_0016'
        self.tab = 't_security_qualityquery_0016'
        self.key_path = 'keypath_security_qualityquery_0016'
    ...
    def test_create_cmk_cek(self):
        ...
        text = '---step3:对数据集合类进行except操作;expect:成功---'
        self.logger.info(text)
        sql_cmd = f'''insert into {self.tab} values
            ...
            select name from {self.tab} where id_number < 16 
            except select name from {self.tab} where id_number > 12;
            prepare test_prepare as select * 
            from {self.tab} where name = \$1;
            execute test_prepare('Nancy');
            '''
        self.logger.info(sql_cmd)
        msg1 = self.sh_primy.execut_db_sql(sql_cmd, sql_type='-C')
        self.logger.info(msg1)
        self.assertTrue(self.Constant.INSERT_SUCCESS_MSG in msg1)
        self.assertIn('Hellen' and ' Nancy',  msg1, text + '执行失败')
        self.assertIn('PREPARE' and 'Nancy ', msg1, text + '执行失败')
