...
Case Name   : 行访问控制策略应用到delete操作
Description :
    1.创建用户
    2.创建表，并给用户赋予访问权限
    3.打开行级访问开关，创建行级访问策略
    4.用户1连接数据库执行delete语句
    5.查看删除的数据
    6.用户2连接数据库查看表数据
Expect      :
    1.创建成功
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_RowLevel_Case0007 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.user01 = 'u01_security_rowlevel_0007'
        self.user02 = 'u02_security_rowlevel_0007'
        self.user03 = 'u03_security_rowlevel_0007'
        self.table = 'table_security_rowlevel_0007'
        self.rowlevel = 'rls_ecurity_rowlevel_0007'
    ...
    def test_masking(self):
        ...
        sql_cmd2 = f'create table {self.table}(id int, role varchar(100), ' \
            ...
            f'grant select, delete on {self.table} to {self.user01}, ' \
        ...
        text = '----step4：用户1连接数据路数据库执行delete语句；expect：成功----'
        self.logger.info(text)
        sql_cmd4 = f'delete from {self.table} where role=\'{self.user01}\';'
        ...
        self.assertIn('DELETE 1', msg4, '执行失败:' + text)
        text = '----step5：查看删除的数据；expect：成功----'
        self.logger.info(text)
        ...
        self.common.equal_sql_mdg(msg5, 'data', '(0 rows)', flag='1')
    ...
    def tearDown(self):
        ...
        self.logger.info(msg1)
        self.logger.info(
            '---Opengauss_Function_Security_RowLevel_Case0007 finish---')
