...
Case Name   : 主节点账户自动锁定，备节点账户仍然可以连接
Description :
    1.创建用户
    create user user001 password '$password';
    2.主机：user001用户使用错误的密码连接数据库2次
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-----Opengauss_Function_Security_User_Case0016 start-----')
        self.common = Common()
        self.constant = Constant()
        self.userNode = Node('PrimaryDbUser')
        self.standbyNode = Node('Standby1DbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.com_user = 'com_security_user_0016'
        self.wrong_pw = 'wrong_pw_0016'
    ...
    def test_lockuser(self):
        ...
        text = '-----step2: 主机：user001用户使用错误的密码连接数据库2次,' \
               'except: user001账户被锁定-----'
        self.logger.info(text)
        ...
        assert_2 = self.constant.ACCOUNT_LOCKED_TYPE in msg2
        ...
        msg3 = self.standbyNode.sh(excute_cmd3).result()
        self.logger.info(msg3)
        assert_3 = f'List of roles' in msg3
        self.assertTrue(assert_3, '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '------检查参数，failed_login_attempts恢复默认配置------')
        check_res = self.sh_primy.execut_db_sql(self.sql_cmd)
        self.logger.info(check_res)
        ...
        self.logger.info(
            '----Opengauss_Function_Security_User_Case0016 finish----')