...
Case Name   : 查看password_lock_time默认值为1，被锁定的账户1天后自动解锁
Description :
    1.查看参数password_lock_time的值默认值
    2.多次输错密码导致账户锁定，登录数据库(gsql -d $data_name -p $data_port -U
    $user_name -W $passwd -r)
    3.将系统时间设置推后>1天，再次登录数据库(gsql -d $data_name -p $data_port -U
    $user_name -W $passwd -r)
Expect      :
    1.默认值为1
    2.多次登录侯，提示账号被锁定
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Security_Policy_Case0010 start')
    ...
    def test_policy(self):
        ...
        self.common.equal_sql_mdg(msg0, 'failed_login_attempts', '10', '(1 row)',
                                flag='1')
        self.logger.info('-------查看password_lock_time修改后的值为1-------')
        ...
        self.logger.info('------将系统时间设置推后>=1天，再次用户登录数据库------')
        ...
        excute_cmd6 = f'source {self.DB_ENV_PATH};' \
                      f'gsql -d {self.userNode.db_name} -p ' \
                      f'{self.userNode.db_port} -U wf ' \
                      f'-W {macro.COMMON_PASSWD} -c "{sql_cmd6}"'
    ...
    def tearDown(self):
        ...
        shell_cmd = f'hwclock -s'
        self.logger.info(shell_cmd)
        shell_cmd_msg = self.primaryRoot.sh(shell_cmd).result()
        self.logger.info(shell_cmd_msg)
        ...
        self.logger.info('Opengauss_Function_Security_Policy_Case0010 finish')
