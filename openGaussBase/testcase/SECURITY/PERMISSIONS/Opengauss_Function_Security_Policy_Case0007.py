...
Case Name   : 设置failed_login_attempts=1000,密码第12次错误，账号自动锁定
Description :
    1.在postgres.conf中设置failed_login_attempts=1000，重启数据库生效
    2.初始用户执行：create user wf with password '$PASSWORD';
    3.用wf用户登录，输入错误的密码，登录1001次
Expect      :
    1.设置成功，数据库重启成功
    2.CREATE ROLE
    3.第1001次登录时，账号被自动锁定
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info(
            '------Opengauss_Function_Security_Policy_Case0007 start------')
    ...
    def test_policy(self):
        ...
        sql_cmd = f'source {self.DB_ENV_PATH};' \
                  ...
                  f'-c "failed_login_attempts=1000"'
    ...
    def tearDown(self):
        logger.info('-----------清理环境-----------')
        sql_cmd = f'source {self.DB_ENV_PATH};' \
                  f'gs_guc reload -N all -I all -c "failed_login_attempts=10"'
        msg0 = self.userNode.sh(sql_cmd).result()
        logger.info(msg0)
        ...
        logger.info(msg1)
        logger.info(
            '-----Opengauss_Function_Security_Policy_Case0007 finish------')
