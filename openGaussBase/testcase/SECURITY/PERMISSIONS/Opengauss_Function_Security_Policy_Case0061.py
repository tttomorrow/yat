...
Case Name   : 密码不可重用天数为默认为60
Description :
    1.登录数据库gsql -d $database -p $port -r，查看默认值:show password_reuse_time;
    2.创建用户user002,create user user001 with password "******";
    3.修改3次用户密码，第3次将用户密码改回原密码
    4.修改系统时间为60天后，将密码修改为原密码
Expect      :
    1.数据库登录成功
    2.用户创建成功
    3.用户登录正常
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('-Opengauss_Function_Security_Policy_Case0061 start-')
    ...
    def test_policy(self):
        ...
        self.common.equal_sql_mdg(msg0, 'password_reuse_max', '0', '(1 row)',
                                flag='1')
        self.logger.info('----查看参数password_reuse_time默认值-----')
        ...
        self.logger.info('-------修改时间为60天后，再次修改用户密码为原密码--------')
        date_return_cmd = 'date -d "1441 hour" "+%Y-%m-%d %H:%M:%S"'
    ...
    def tearDown(self):
        ...
        self.logger.info('-Opengauss_Function_Security_Policy_Case0061finish-')
