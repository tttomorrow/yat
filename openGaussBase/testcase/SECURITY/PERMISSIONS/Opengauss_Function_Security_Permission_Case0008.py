...
Description :
    1.初始用户执行：create user wf with password '$PASSWORD';
                ...
                grant delete on security_table to wf;
    2.wf用户执行：delete from security_table where id2=5;
Expect      :
    1.CREATE ROLE
    CREATE TABLE
    GRANT
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info(
            '----Opengauss_Function_Security_Permission_Case0008 start-----')
    ...
    def test_default_permission(self):
        ...
        sql_cmd1 = f'create user wf with password \'{macro.COMMON_PASSWD}\';' \
                   ...
                   f'grant delete on security_table to wf;'
        msg1 = self.sh_primy.execut_db_sql(sql_cmd1)
        logger.info(msg1)
        self.assertIn(self.Constant.GRANT_SUCCESS_MSG, msg1)
        sql_cmd2 = '''delete from security_table where id2=5;'''
        ...
        logger.info(msg2)
        self.assertIn(self.Constant.DELETE_SUCCESS_MSG, msg2)
    ...
    def tearDown(self):
        ...
        logger.info(msg1)
        logger.info(
            '----Opengauss_Function_Security_Permission_Case0008 finish----')
