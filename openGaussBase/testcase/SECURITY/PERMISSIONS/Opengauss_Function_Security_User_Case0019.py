...
Case Name   : 备节点账户手动锁定，主节点账户仍然可以连接
Description :
    1.创建用户
    create user user001 password '$password';
    2.备机：锁定账户user001，连接数据库
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-----Opengauss_Function_Security_User_Case0019 start-----')
        self.common = Common()
        self.constant = Constant()
        self.userNode = Node('PrimaryDbUser')
        self.standbyNode = Node('Standby1DbUser')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.com_user = 'com_security_user_0019'
    ...
    def test_lockuser(self):
        ...
        text = '-----step2: 备机：锁定账户user001，连接数据库' \
        ...
        assert_2 = self.constant.ALTER_ROLE_SUCCESS_MSG in msg2
        ...
        assert_3 = self.constant.ACCOUNT_LOCKED_TYPE in msg3
        self.assertTrue(assert_2 and assert_3, '执行失败：' + text)
    ...
    def tearDown(self):
        text = '------step4:清理资源：删除用户，expect:删除用户成功-------'
        self.logger.info(text)
        ...
        self.logger.info(
            '----Opengauss_Function_Security_User_Case0019 finish----')