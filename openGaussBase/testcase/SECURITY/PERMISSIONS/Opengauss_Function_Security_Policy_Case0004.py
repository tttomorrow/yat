...
Case Name   : 设置failed_login_attempts=11,密码第12次错误，账号自动锁定
Description :
    1.在postgres.conf中设置failed_login_attempts=11，重启数据库生效
    2.初始用户执行：create user wf with password '$PASSWORD';
    3.用wf用户登录，输入错误的密码，登录12次
Expect      :
    1.设置成功，数据库重启成功
    2.CREATE ROLE
    3.第12次登录时，账号被自动锁定
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('----Opengauss_Function_Security_Policy_Case0004 start---')
    ...
    def tearDown(self):
        ...
        msg1 = self.sh_primy.execut_db_sql(sql_cmd1)
        logger.info(msg1)
        logger.info('---Opengauss_Function_Security_Policy_Case0004 finish---')
