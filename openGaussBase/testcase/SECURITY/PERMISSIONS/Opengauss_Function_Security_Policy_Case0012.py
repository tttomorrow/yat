...
Case Name   : 设置password_lock_time=2，到期后被锁定的账户自动解锁
Description :
    1.修改参数failed_login_attempts=2:gs_guc reload -N all -I all
    -c "password_lock_time=2"
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Security_Policy_Case0012 start')
    ...
    def test_policy(self):
        ...
        self.common.equal_sql_mdg(msg0, 'failed_login_attempts', '10', '(1 row)',
                                flag='1')
        self.logger.info("----------password_lock_time修改后的值为2--------")
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c "password_lock_time=2"'
        ...
        self.common.equal_sql_mdg(msg2, 'password_lock_time', '2d', '(1 row)',
                                flag='1')
        ...
        self.logger.info('------将系统时间设置推后>=2天，再次用户登录数据库-------')
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Security_Policy_Case0012 finish')
