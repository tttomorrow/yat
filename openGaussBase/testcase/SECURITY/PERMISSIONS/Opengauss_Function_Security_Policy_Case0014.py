...
Case Name   : 设置password_lock_time=365，到期后被锁定的账户自动解锁
Description :
    1.修改参数failed_login_attempts=2:gs_guc reload -N all -I all -c
    "password_lock_time=2"
    2.多次输错密码导致账户锁定，将系统时间设置推后2天，用户再次登录数据库
Expect      :
    1.参数修改成功
    2.修改时间后，再次登录成功
History     :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Security_Policy_Case0014 start')
    ...
    def test_policy(self):
        ...
        self.common.equal_sql_mdg(msg0, 'failed_login_attempts', '10', '(1 row)',
                                flag='1')
        self.logger.info('----------password_lock_time修改后的值为365--------')
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c ' \
                      f'"password_lock_time=365"'
        ...
        self.common.equal_sql_mdg(msg2, 'password_lock_time', '365d', '(1 row)',
                                flag='1')
        ...
        self.logger.info('-----将系统时间设置推后>=365天，再次用户登录数据库-----')
        date_return_cmd = 'date -d "+366 day" "+%Y-%m-%d %H:%M:%S"'
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Security_Policy_Case0014 finish')
