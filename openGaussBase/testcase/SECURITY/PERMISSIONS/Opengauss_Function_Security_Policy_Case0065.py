...
Case Name   : 密码不可重用天数为浮点数
Description :
    1.设置不可重用天数默认值为5.5天，gs_guc reload -N all -I all -c
    "password_reuse_time=5.5"，重启数据库
    2.创建用户user002,create user user001 with password "******";
    3.修改2次用户密码，第2次将用户密码改回原密码
    4.修改系统时间为5.5天后，将密码修改为原密码
Expect      :
    1.设置成功
    2.创建成功
    3.修改失败，提示密码不可重复
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Security_Policy_Case0065 start')
    ...
    def test_policy(self):
        ...
        self.common.equal_sql_mdg(msg0, 'password_reuse_max', '0', '(1 row)',
                                flag='1')
        self.logger.info('-----修改参数password_reuse_time值为5.5--------')
        excute_cmd2 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all ' \
                      f'-c "password_reuse_time=5.5"'
        ...
        self.logger.info(msg1)
        self.common.equal_sql_mdg(msg1, 'password_reuse_time', '5.5', '(1 row)',
                                flag='1')
        ...
        self.logger.info('------修改时间为5.5天后，再次修改用户密码为原密码------')
        date_return_cmd = 'date -d "+133 hour" "+%Y-%m-%d %H:%M:%S"'
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Security_Policy_Case0065 finish')
