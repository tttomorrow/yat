...
Case Name   : 创建用户时的加密算法sm3，认证方式trust，初始用户正确的密码连接数据
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_SM3_Case0027 start--')
    ...
    def test_encrypted(self):
        ...
        self.assertIn('local all all trust', msg3, '执行失败:' + text)
        text = '---step3:初始用户错误的密码登录数据库;expect:成功---'
        self.logger.info(text)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg2)
        self.logger.info(
            '----Opengauss_Function_Security_SM3_Case0027 finish----')
