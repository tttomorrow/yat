...
Case Name   : 创建用户时的加密算法sha256,认证方式sm3,初始用户正确的密码通过JDBC连接数据库
Description :
    1.修改password_encryption_type=2
    2.pg_hba.conf文件中修改认证方式为sm3
    3.初始用户通过JDBC正确的密码连接数据库
Expect      :
    1-2.参数设置成功
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0034 start--')
    ...
    def test_encrypted(self):
        ...
        self.assertIn(f'host {self.userNode.db_name} {self.userNode.ssh_user} '
                      f'{self.userNode.db_host}/32 sm3', msg3,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg3)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0034 finish--')
