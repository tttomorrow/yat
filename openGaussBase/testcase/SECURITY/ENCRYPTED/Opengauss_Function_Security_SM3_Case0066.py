...
"""
Case Type   : Interface
Case Name   : 创建用户时的加密算法sm3，认证方式sha256，初始用户正确的密码通过ODBC连接数据库
Description :
    1.安装unixODBC
    2.配置加密方式sm3,认证方式为sha256
    3.配置数据源
    4.检查依赖库
    5.在客户端配置环境变量,导入环境变量
    6.连接数据库
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_SM3_Case0066 start--')
    ...
    def test_encrypted(self):
        ...
        text = '---step2.3:pg_hba.conf文件中增加认证方式为sha256;expect:成功---'
        self.logger.info(text)
        ...
        odbc_content = f'[gaussodbc]\n' \
            ...
            f'Password={self.userNode.ssh_password}\n' \
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Security_SM3_Case0066 finish--')
