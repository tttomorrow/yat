...
"""
Case Type   : Interface
Case Name   : 加密算法md5+sha256，认证方式sm3，非初始用户正确的密码通过ODBC连接数据库
Description :
    1.安装unixODBC
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_SM3_Case0080 start--')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.common = Common()
        self.user = 'u_security_sm3_0080'
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Security_SM3_Case0080 finish--')
