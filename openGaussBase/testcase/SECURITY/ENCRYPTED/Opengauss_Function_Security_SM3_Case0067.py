...
"""
Case Type   : Interface
Case Name   : 创建用户时的加密算法sm3，认证方式sha256，初始用户错误密码通过ODBC连接数据库
Description :
    1.安装unixODBC
    2.配置加密方式sha256,认证方式为sm3
    3.配置数据源
    4.检查依赖库
    5.在客户端配置环境变量,导入环境变量
    6.连接数据库
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_SM3_Case0067 start--')
    ...
    def test_encrypted(self):
        ...
        text = '---step2.2:pg_hba.conf文件中增加认证方式为sha256;expect:成功---'
        self.logger.info(text)
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Security_SM3_Case0067 finish--')
