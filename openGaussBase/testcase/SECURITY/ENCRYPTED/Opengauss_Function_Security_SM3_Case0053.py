...
Case Name   : 创建用户时加密算法MD5+sha256，认证方式sm3，非初始用户错误密码JDBC连接数据库
Description :
    1.修改password_encryption_type=1
    2.pg_hba.conf文件中修改认证方式为sm3
    3.非初始用户错误的通过JDBC密码登录数据库
Expect      :
    1-2.参数设置成功
    3.数据库连接失败
History     :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0053 start--')
    ...
    def tearDown(self):
        ...
        msg4 = self.sh_primy.execut_db_sql(sql_cmd4)
        self.logger.info(msg4)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0053 finish--')
