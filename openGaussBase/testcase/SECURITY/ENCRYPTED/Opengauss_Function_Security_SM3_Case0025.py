...
Case Name   : 创建用户时的加密算法MD5+sha256，认证方式sm3，非初始用户错误的密码连接数据库
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0025 start--')
        self.userNode = Node('PrimaryDbUser')
        self.DB_ENV_PATH = macro.DB_ENV_PATH
        self.DB_INSTANCE_PATH = macro.DB_INSTANCE_PATH
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.user = 'u_security_sm3_0025'
    ...
    def test_encrypted(self):
        ...
        self.common.equal_sql_mdg(check_msg, 'password_encryption_type', '1',
                                  '(1 row)', flag='1')
        text = '----step2:g_hba.conf文件中增加认证方式为sm3;expect:成功---'
        self.logger.info(text)
        ...
        text = '----step3:非初始用户错误的密码登录数据库;expect:成功---'
        self.logger.info(text)
    ...
    def tearDown(self):
        self.logger.info('-------step4：清理环境，恢复配置------')
        ...
        self.logger.info(msg3)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0025 finish--')
