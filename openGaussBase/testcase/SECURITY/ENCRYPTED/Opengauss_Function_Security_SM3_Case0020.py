...
Case Name   : 创建用户时的加密算法MD5，认证方式sm3，非初始用户正确的密码连接数据库
Description :
    1.修改password_encryption_type=0
    2.pg_hba.conf文件中修改修改认证方式为sm3
    3.非初始用户正确的密码登录数据库
Expect      :
    1-2.参数设置成功
    3.数据库连接失败
...
class Security(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Security_sm3_Case0020 start---')
    ...
    def test_masking(self):
        logger.info('--------1.修改password_encryption_type=0--------')
    ...
    def tearDown(self):
        ...
        logger.info(msg3)
        logger.info('----Opengauss_Function_Security_sm3_Case0020 finish----')
