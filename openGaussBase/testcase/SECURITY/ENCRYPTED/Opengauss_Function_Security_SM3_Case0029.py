...
Case Name   : 创建用户时的加密算法sm3，认证方式trust，非初始用户错误的密码连接数据
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Security_sm3_Case0029 start---')
    ...
    def test_encrypted(self):
        ...
        self.common.equal_sql_mdg(check_msg, 'password_encryption_type', '3',
                                  '(1 row)', flag='1')
        logger.info('--------2.pg_hba.conf文件中增加认证方式为trust--------')
        exe_cmd2 = f'grep  "local.*all.*trust" {self.config}'
        msg2 = self.userNode.sh(exe_cmd2).result()
        logger.info(msg2)
        insert_messages = f"local {self.userNode.db_name} user001 trust"
    ...
    def tearDown(self):
        ...
        logger.info(msg3)
        logger.info('----Opengauss_Function_Security_sm3_Case0029 finish----')
