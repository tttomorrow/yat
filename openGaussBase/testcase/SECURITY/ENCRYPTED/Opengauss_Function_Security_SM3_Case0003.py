...
Case Name   : 创建用户时的加密算法与认证过程中的加密算法均为sm3,初始用户错误的密码连接数据库
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_SM3_Case0003 start--')
    ...
    def test_encrypted(self):
        ...
        self.common.equal_sql_mdg(check_msg, 'password_encryption_type', '3',
                                  '(1 row)', flag='1')
        text = '---step2:pg_hba.conf文件中增加认证方式为sm3---'
        self.logger.info(text)
        ...
        self.assertIn('local all all sm3', msg3, '执行失败:' + text)
        text = '---step3:初始用户错误的密码登录数据库;expect:失败---'
    ...
    def tearDown(self):
        ...
        self.logger.info(msg2)
        self.logger.info(
            '----Opengauss_Function_Security_SM3_Case0003 finish----')
