...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0057 start--')
        self.userNode = Node('PrimaryDbUser')
        self.primary_root = Node('PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.user = 'u_security_sm3_0057'
    ...
    def test_encrypted(self):
        text = '----step1：修改password_encryption_type=1;expect:成功----'
        ...
        self.logger.info('--step3：非初始用户正确的密码通过JDBC登录数据库--')
        ...
        text = '----step3.2：写入配置文件，用户1设置正确的密码;expect:成功-----'
        self.logger.info(text)
        ...
        config = f'echo "password={error_password}"> {self.properties}'
    ...
    def tearDown(self):
        ...
        msg4 = self.sh_primy.execut_db_sql(sql_cmd4)
        self.logger.info(msg4)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0057 finish--')
