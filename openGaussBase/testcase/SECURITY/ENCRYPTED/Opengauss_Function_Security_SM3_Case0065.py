...
"""
Case Type   : Interface
Case Name   : 创建用户时的加密算法sha256，认证方式sm3,非初始用户错误密码通过ODBC连接数据库
Description :
    1.安装unixODBC
    2.创建用户，配置加密方式sha256,认证方式为sm3
    3.配置数据源
    4.检查依赖库
    5.在客户端配置环境变量,导入环境变量
    6.连接数据库
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_SM3_Case0065 start--')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.common = Common()
        self.user = 'u_security_sm3_0065'
    ...
    def test_encrypted(self):
        ...
        text = '---step2.1:设置加密方式为sha256;expect:成功---'
        self.logger.info(text)
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Security_SM3_Case0065 finish--')
