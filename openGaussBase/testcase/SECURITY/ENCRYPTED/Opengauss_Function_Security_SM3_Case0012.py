...
Case Name   : 创建用户时的加密算法sm3，认证方式sha256，非初始用户正确的密码连接数据库
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Security_sm3_Case0009 start---')
    ...
    def test_masking(self):
        ...
        self.common.equal_sql_mdg(check_msg, 'password_encryption_type', '3',
                                  '(1 row)', flag='1')
        logger.info('--------2.pg_hba.conf文件中增加认证方式为sha256--------')
        exe_cmd2 = f'grep  "local.*all.*trust" {self.config}'
        msg2 = self.userNode.sh(exe_cmd2).result()
        logger.info(msg2)
        insert_messages = f"local {self.userNode.db_name} user001 sha256"
    ...
    def tearDown(self):
        ...
        logger.info(msg3)
        logger.info('----Opengauss_Function_Security_sm3_Case0012 finish----')
