...
Case Name   : 创建用户时的加密算法sm3，认证方式trust，初始用户正确的密码通过JDBC连接数据
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0054 start--')
    ...
    def test_encrypted(self):
        ...
        self.assertIn(f'host {self.userNode.db_name} {self.userNode.ssh_user} '
                      f'{self.userNode.db_host}/32 trust', msg3,
                      '执行失败:' + text)
        text = '---step3:初始用户通过JDBC正确密码连接数据库;expect:失败---'
        self.logger.info(text)
        ...
        config = f'echo "password=' \
            f'{self.userNode.ssh_password}"> {self.properties}'
        ...
        self.logger.info(result)
        assert2 = 'Forbid remote connection with trust method' in result and \
                  '连接失败' in result
    ...
    def tearDown(self):
        ...
        self.logger.info(msg3)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0054 finish--')
