...
Case Name   : 加密算法与认证过程中加密算法均为sm3,初始用户正确密码通过JDBC连接数据库
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0030 start--')
    ...
    def test_encrypted(self):
        ...
        self.assertIn(f'host {self.userNode.db_name} {self.userNode.ssh_user} '
                      ...
                      '执行失败:' + text)
        text = '---step3:初始用户通过JDBC连接数据库;expect:失败---'
        self.logger.info(text)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg3)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0030 finish--')
