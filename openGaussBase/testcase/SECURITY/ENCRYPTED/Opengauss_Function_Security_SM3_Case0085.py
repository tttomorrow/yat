...
"""
Case Type   : Interface
Case Name   : 创建用户时的加密算法sm3，认证方式trust，非初始用户错误的密码通过ODBC连接数据
Description :
    1.安装unixODBC
    2.创建用户，配置加密方式sm3,认证方式为trust
    3.配置数据源
    4.检查依赖库
    5.在客户端配置环境变量,导入环境变量
    6.连接数据库
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_SM3_Case0085 start--')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.common = Common()
        self.user = 'u_security_sm3_0085'
    ...
    def test_encrypted(self):
        ...
        text = '---step2.3:pg_hba.conf文件中增加认证方式为trust;expect:成功---'
        self.logger.info(text)
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Security_SM3_Case0085 finish--')
