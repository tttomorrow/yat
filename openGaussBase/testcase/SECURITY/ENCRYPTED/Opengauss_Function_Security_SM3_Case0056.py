...
Case Name   : 创建用户时的加密算法sm3，认证方式trust，非初始用户正确的密码通过JDBC连接数据
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0056 start--')
        self.userNode = Node('PrimaryDbUser')
        self.primary_root = Node('PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user = 'u_security_sm3_0056'
    ...
    def test_encrypted(self):
        text = '--------step1：修改password_encryption_type=1--------'
        ...
        self.common.equal_sql_mdg(check_msg, 'password_encryption_type', '1',
                                  '(1 row)', flag='1')
        text = '--------step2：pg_hba.conf文件中增加认证方式为trust--------'
        self.logger.info(text)
        ...
        assert_1 = self.constant.CREATE_ROLE_SUCCESS_MSG
        self.assertIn(assert_1, msg4, '执行失败:' + text)
        text = '----step3.2：写入配置文件，用户1设置正确的密码----'
        self.logger.info(text)
        ...
        assert_1 = 'password=' in result and 'port=' in result and \
                   'hostname=' in result and 'user=' in result and \
                   'dbname=' in result
        self.assertTrue(assert_1, '执行失败' + text)
        text = '----step3.3：编译java脚本;expect:成功----'
        self.logger.info(text)
        ...
        self.logger.info(result)
        text = '------step3.4 运行java脚本，数据库连接成功;expect:成功-------'
        self.logger.info(text)
    ...
    def tearDown(self):
        ...
        msg4 = self.sh_primy.execut_db_sql(sql_cmd4)
        self.logger.info(msg4)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0056 finish--')
