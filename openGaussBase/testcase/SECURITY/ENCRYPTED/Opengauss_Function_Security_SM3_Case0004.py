...
class Security(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Security_sm3_Case0004 start---')
    ...
    def test_masking(self):
        ...
        logger.info('---5.通过系统表（pg_authid）查看创建用户时使用的加密算法sm3---')
        sql_cmd6 = f'select rolname,rolpassword from pg_authid where ' \
            f'rolname=\'user001\';'
        sql_msg6 = self.sh_primy.execut_db_sql(sql_cmd6)
        logger.info(sql_msg6)
        self.assertTrue('sm3' in sql_msg6)
    ...
    def tearDown(self):
        ...
        logger.info(msg3)
        logger.info('----Opengauss_Function_Security_sm3_Case0004 finish----')
