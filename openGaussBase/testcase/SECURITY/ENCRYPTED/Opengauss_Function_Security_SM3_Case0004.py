...
Case Name   : 创建用户时的加密算法与认证过程中的加密算法均为sm3,初始用户正确的密码连接数据库
Description :
    1.修改password_encryption_type=3
    2.pg_hba.conf文件中修改认证方式为sm3
    3.创建普通用户{self.user}
    4.{self.user}用户登录数据库
    5.通过系统表（pg_authid）查看创建用户时使用的加密算法sm3
Expect      :
    1-2.参数设置成功
    3.用户创建成功
    4.数据连接成功
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0004 start--')
        self.userNode = Node('PrimaryDbUser')
        self.DB_ENV_PATH = macro.DB_ENV_PATH
        self.DB_INSTANCE_PATH = macro.DB_INSTANCE_PATH
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.user = 'u_security_sm3_0004'
    ...
    def test_encrypted(self):
        ...
        self.common.equal_sql_mdg(check_msg, 'password_encryption_type', '3',
                                  '(1 row)', flag='1')
        text = '---step2:修pg_hba.conf文件中增加认证方式为sm3;expect;成功---'
        self.logger.info(text)
        ...
        text = '---step5:通过系统表查看创建用户时使用的加密算法sm3;expect;成功---'
        self.logger.info(text)
        sql_cmd6 = f'select rolpassword from pg_authid where ' \
            f'rolname=\'{self.user}\';'
        sql_msg6 = self.sh_primy.execut_db_sql(sql_cmd6)
        self.logger.info(sql_msg6)
        self.assertIn('sm3', sql_msg6, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg3)
        self.logger.info(
            '----Opengauss_Function_Security_sm3_Case0004 finish----')
