...
Case Name   : 创建用户时的加密算法sm3，认证方式sha256，非初始用户错误的密码连接数据库
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0013 start--')
        self.userNode = Node('PrimaryDbUser')
        self.DB_ENV_PATH = macro.DB_ENV_PATH
        self.DB_INSTANCE_PATH = macro.DB_INSTANCE_PATH
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.user = 'u_security_sm3_0013'
    ...
    def test_encrypted(self):
        ...
        self.common.equal_sql_mdg(check_msg, 'password_encryption_type', '3',
                                  '(1 row)', flag='1')
        text = '---step2:pg_hba.conf文件中增加认证方式为sha256;expect:成功---'
        self.logger.info(text)
        exe_cmd2 = f'grep  "local.*all.*trust" {self.config}'
        msg2 = self.userNode.sh(exe_cmd2).result()
        self.logger.info(msg2)
        insert_messages = f"local {self.userNode.db_name} {self.user} sha256"
        ...
        text = '---step4:使用用户1错误密码登录数据库;expect:成功---'
        self.logger.info(text)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg3)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0013 finish--')
