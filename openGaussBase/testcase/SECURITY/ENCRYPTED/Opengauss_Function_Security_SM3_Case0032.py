...
Case Name   : 创建用户的加密算法与认证方式均为sm3,非初始用户正确的密码通过JDBC连接数据库
Description :
    1.修改password_encryption_type=3
    2.创建普通用户1
    3.pg_hba.conf文件中修改认证方式为sm3
    4.用户1使用正确的密码通过JDBC连接数据库
Expect      :
    1.参数设置成功
    2.用户创建成功
    3.参数设置成功
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_sm3_Case0032 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.user = 'u_security_sm3_0032'
    ...
    def test_encrypted(self):
        ...
        config = f'echo "password=' \
            f'{macro.COMMON_PASSWD}"> {self.properties}'
        ...
        cmd_compile = f"javac -encoding utf-8 -cp " \
            f"{os.path.join(self.targetpath, 'postgresql.jar')} " \
            f"{os.path.join(self.targetpath, f'{self.java_name}.java')}"
        self.logger.info(cmd_compile)
        result1 = self.primary_root.sh(cmd_compile).result()
        ...
        result2 = self.primary_root.sh(cmd_run).result()
        self.logger.info(result2)
        self.assertIn('临时表已删除0', result2, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg4)
        self.logger.info(
            '----Opengauss_Function_Security_sm3_Case0032 finish----')
