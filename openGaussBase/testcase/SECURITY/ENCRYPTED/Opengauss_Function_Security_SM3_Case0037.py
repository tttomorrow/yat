...
Case Name   : 创建用户时加密算法sha256，认证方式sm3,非初始用户正确的密码通过JDBC连接数据库
Description :
    1.修改password_encryption_type=2
    2.pg_hba.conf文件中修改认证方式为sm3
    3.创建普通用户{self.user}
    4.{self.user}用户正确的密码通过JDBC连接数据库
Expect      :
    1-2.参数设置成功
    3.用户创建成功
    4.数据库连接失败
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0037 start--')
    ...
    def test_encrypted(self):
        ...
        self.logger.info(result)
        self.logger.info("--------4.3 运行java脚本，数据库连接成功---------")
    ...
    def tearDown(self):
        ...
        msg4 = self.sh_primy.execut_db_sql(sql_cmd4)
        self.logger.info(msg4)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0037 finish--')
