...
Case Name   : 使用gs_guc set方式设置加密方式为sm3，初始化数据库带参数--auth=sm3
Description :
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Security_sm3_Case0089 start--')
    ...
    def test_encrypted(self):
        ...
        self.logger.info(text)
        exe_cmd2 = f'source {macro.DB_ENV_PATH};gs_initdb -D ' \
            f'{self.dir_path} --nodename=dn_new --auth=sm3'
        ...
        self.assertTrue(msg3_2.find('sm3') >= 2, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        msg4 = self.userNode.sh(exe_cmd4).result()
        self.logger.info(msg4)
        self.logger.info('--Opengauss_Function_Security_sm3_Case0089 finish--')
