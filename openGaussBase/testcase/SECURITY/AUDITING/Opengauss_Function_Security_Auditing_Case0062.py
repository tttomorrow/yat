...
Case Name   : 关闭数据库对象ROW LEVEL SECURITY的CREATE、DROP、ALTER操作审计功能，
                audit_system_object=16383
Description :
    1.设置gs_guc reload -N all -I all -c "audit_system_object=16383
...
Expect      :
    ...
    2.创建成功
    3.删除成功
    4.未查询到创建、删除ROW LEVEL SECURITY信息
History     :
"""
import time
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '====Opengauss_Function_Security_Auditing_Case0062 start====')
        self.common = Common()
    ...
    def test_policy(self):
        self.logger.info('---------设置参数audit_system_object=16383----------')
        excute_cmd0 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c ' \
                      f'"audit_system_object=16383"'
        ...
        data varchar(100));
            CREATE ROW LEVEL SECURITY POLICY all_data_rls ON all_data  
        ...
        self.assertTrue("CREATE ROW LEVEL SECURITY POLICY" in msg1 and
                        "ALTER ROW LEVEL SECURITY POLICY" in msg1 and
                        "DROP ROW LEVEL SECURITY POLICY" in msg1)
        time.sleep(3)
        ...
        self.assertFalse(
            msg2.find('CREATE ROW LEVEL SECURITY POLICY all_data_rls ON '
                      'all_data USING(role = CURRENT_USER') > -1)
        self.assertFalse(
            msg2.find(
                'ALTER ROW LEVEL SECURITY POLICY all_data_rls ON all_data'
                ' RENAME TO\
          all_data_new_rls') > -1)
        self.assertFalse(
            msg2.find('DROP ROW LEVEL SECURITY POLICY all_data_new_rls ON  '
                      'all_data') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg1)
        self.logger.info(
            '====Opengauss_Function_Security_Auditing_Case0062 finish====')
