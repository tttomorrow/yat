...
Case Name   : 开启数据库对象table的CREATE、DROP、ALTER操作审计功能，
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0041 start----')
    ...
    def test_security(self):
        excute_cmd = f'source {self.DB_ENV_PATH};' \
                     f'gs_guc reload -N all -I all -c "audit_system_object=15"'
        ...
        self.assertTrue(msg3.find('create table table02(name char(10));') > -1)
        self.assertTrue(msg3.find('alter table table02 add(id int)') > -1)
        self.assertTrue(msg3.find('truncate table table02') > -1)
        self.assertTrue(msg3.find('drop table table02;') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg1)
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0041 end----')
