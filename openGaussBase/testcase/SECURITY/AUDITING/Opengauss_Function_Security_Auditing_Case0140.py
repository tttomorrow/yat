...
Case Name   : 统一审计策略：审计DML操作ALL，过滤user
Description :
    1.系统管理员用户创建两个用户
    ...
    5.用户1登录数据库执行ALL语句
    6.用户2登录数据库执行ALL语句
    7.查看/var/log/postgresql日志中是否审计了用户1和用户2的ALL操作
Expect      :
    1.用户创建成功
    2.建表成功，赋予权限
    3.创建成功
    ...
    5.执行成功
    6.执行成功
    7.日志中审计到user001的ALL所有操作、未审计到user002的ALL所有操作
History     :
"""
import os
import re
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0140 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0140'
        self.user2 = 'u02_security_auditing_0140'
        self.table = 'table_security_auditing_0140'
        self.res_label = 'rl_security_auditing_0140'
        self.audit_policy = 'ap_security_auditing_0140'
        self.index_policy = 'in_security_auditing_0140'
        self.in_reason = 'insert_reason_0140'
    ...
    def test_encrypted(self):
        ...
        text = '---step1.3:syslog_facility=local0;expect:成功---'
        self.logger.info(text)
        ...
        text = '---step1.4:系统管理员用户创建两个用户;expect:成功---'
        self.logger.info(text)
        ...
        sql_cmd4 = f'drop audit policy if exists {self.audit_policy};' \
            ...
            f' update , delete, reindex, prepare, execute, deallocate,' \
            f' truncate ' \
        ...
        text = '---step5:用户1登录数据库执行ALL语句;expect:成功---'
        self.logger.info(text)
        sql_cmd5 = f'insert into {self.table} values(2,\'licy\');' \
                   f'update {self.table} set name=\'modify\' where id=1;' \
                   f'delete from {self.table} where id=1;' \
                   f'reindex  table {self.table};' \
                   f'prepare {self.in_reason}(int,char(10)) as ' \
                   f'insert into {self.table} values (\$1,\$2);' \
                   f'execute {self.in_reason}(5, \'avcv\'); ' \
                   f'deallocate {self.in_reason};'\
                   f'truncate {self.res_label};'
        ...
        text = '---step6:用户2登录数据库执行ALL语句;expect:成功---'
        self.logger.info(text)
        sql_cmd6 = f'insert into {self.table} values(2,\'licy\');' \
                   f'update {self.table} set name=\'modify\' where id=1;' \
                   f'delete from {self.table} where id=1;' \
            f'reindex  table {self.table};' \
            f'prepare {self.in_reason}(int,char(10)) as ' \
        ...
        text = '--step7:查看postgresql日志中是否审计了用户1的操作;expect:成功--'
        self.logger.info(text)
        sleep(2)
        ...
        assert1 = re.search(
            ...
            re.S)
        assert2 = re.search(
            f"user name.*{self.user1}.*access type.*[UPDATE].*[OK]", msg7,
            re.S)
        assert3 = re.search(
            f"user name.*{self.user1}.*access type.*[REINDEX].*[OK]", msg7,
            re.S)
        assert4 = re.search(
            f"user name.*{self.user1}.*access type.*[PREPARE].*[OK]", msg7,
            re.S)
        assert5 = re.search(
            f"user name.*{self.user1}.*access type.*[EXECUTE].*[OK]", msg7,
            re.S)
        assert6 = re.search(
            f"user name.*{self.user1}.*access type.*[DEALLOCATE].*[OK]",
            msg7, re.S)
        assert7 = re.search(
            f"user name.*{self.user1}.*access type.*[TRUNCATE].*[OK]",
            msg7, re.S)
        self.assertTrue(assert1, '执行失败:' + text)
        self.assertTrue(assert2, '执行失败:' + text)
        self.assertTrue(assert3, '执行失败:' + text)
        self.assertTrue(assert4, '执行失败:' + text)
        self.assertTrue(assert5, '执行失败:' + text)
        self.assertTrue(assert6, '执行失败:' + text)
        self.assertTrue(assert7, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        clear_cmd = f'drop audit policy if exists {self.audit_policy};' \
            ...
            f'drop table if exists {self.in_reason} cascade;' \
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0140 finish----')
