...
Case Name   : 统一审计策略：审计DDL操作CREATE，过滤user
Description :
    1.系统管理员用户创建两个用户
    2.系统管理员用户创建表，并赋予用户权限
    3.系统管理员用户创建资源标签
    4.系统管理员用户创建统一审计策略，过滤用户1
    5.用户1登录数据库执行CREATE语句
    6.用户2登录数据库执行CREATE语句
    7.查看/var/log/postgresql日志中是否审计了用户1和用户2的CREATE操作
    8.恢复参数及清理环境
Expect      :
    1.用户创建成功
    2.建表成功，赋予权限
    ...
    5.执行成功
    6.执行成功
    7.日志中审计到user001的CREATE操作、未审计到user002的CREATE操作
    8.恢复参数及清理环境成功
History     :
"""
import os
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0144 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0144'
        self.user2 = 'u02_security_auditing_0144'
        self.table = 'table_security_auditing_0144'
        self.res_label = 'rl_security_auditing_0144'
        self.audit_policy = 'ap_security_auditing_0144'
    ...
    def test_encrypted(self):
        ...
        assert1 = re.search(
            f"user name.*{self.user1}.*privilege type.*[CREATE].*[OK]", msg7,
            re.S)
        self.assertTrue(assert1, '执行失败:' + text)
        self.assertNotIn(self.user2, msg7, '执行失败:' + text)

    def tearDown(self):
        text1 = '--step8.1:恢复配置文件信息;expect:成功--'
        ...
        text2 = '--step8.2:恢复参数默认值;expect:成功--'
        ...
        text3 = '--step8.3:清理用户及表;expect:成功--'
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0144 finish----')
