...
Case Name   : 统一审计策略：对table对象的SELECT行为的审计，过滤user，指定ENABLE
Description :
    1.系统管理员用户创建两个用户
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0131 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0131'
        self.user2 = 'u02_security_auditing_0131'
        self.table = 'table_security_auditing_0131'
        self.res_label = 'rl_security_auditing_0131'
        self.audit_policy = 'ap_security_auditing_0131'
    ...
    def test_encrypted(self):
        ...
        assert1 = re.search(
            f"user name.*{self.user1}.*access type.*[SELECT].*[OK]", msg7,
            re.S)
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0131 finish----')
