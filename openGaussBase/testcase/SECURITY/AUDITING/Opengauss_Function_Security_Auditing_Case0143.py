...
Case Name   : 统一审计策略：审计DDL操作GRANT、REVOKE，过滤user
Description :
    1.系统管理员用户创建两个用户
    ...
    5.用户1登录数据库执行语句
    6.用户2登录数据库执行语句
    7.查看/var/log/postgresql日志中是否审计了用户1的GRANT、REVOKE操作
Expect      :
    1.用户创建成功
    2.建表成功，赋予权限
    3.创建成功
    ...
    5.执行成功
    6.执行成功
    7.日志中审计到user001的grant、revoke操作、未审计到user002的grant、revoke操作
History     :
"""
import os
import re
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0143 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0143'
        self.user2 = 'u02_security_auditing_0143'
        self.table = 'table_security_auditing_0143'
        self.res_label = 'rl_security_auditing_0143'
        self.audit_policy = 'ap_security_auditing_0143'
    ...
    def test_encrypted(self):
        ...
        sql_cmd4 = f'drop audit policy if exists {self.audit_policy};' \
            f'create audit policy {self.audit_policy} privileges grant, revoke'\
        ...
        text = '---step5:用户1登录数据库执行GRANT、REVOKE语句expect:成功---'
        self.logger.info(text)
        sql_cmd5 = f'grant insert on table {self.table} to {self.user1};'\
                   f'revoke insert on table {self.table} from {self.user1};'
        ...
        text = '---step6:用户2登录数据库执行GRANT、REVOKE语句;expect:成功---'
        self.logger.info(text)
        sql_cmd6 = f'grant insert on table {self.table} to {self.user2};'\
                   f'revoke insert on table {self.table} from {self.user2};'
        ...
        assert1 = re.search(
            f"user name.*{self.user1}.*privilege type.*[GRANT].*[OK]",
            msg7, re.S)
        assert2 = re.search(
            f"user name.*{self.user1}.*privilege type.*[REVOKE].*[OK]",
            msg7, re.S)
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0143 finish----')
