...
Case Name   : 统一审计策略：审计DML操作prepare，execute，deallocate审计，过滤user
Description :
    1.系统管理员用户创建两个用户
    ...
    5.用户1用户登录数据库执行语句
    6.用户2用户登录数据库执行语句
    7.查看/var/log/postgresql日志中是否审计了用户1的操作
Expect      :
    1.用户创建成功
    2.建表成功，赋予权限
    3.创建成功
    ...
    5.执行成功
    6.执行成功
    7.日志中审计到user001的PREPARE, EXECUTE, DEALLOCATE操作，
      未审计到user002的PREPARE, EXECUTE, DEALLOCATE操作
History     :
"""
import os
import re
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0138 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0138'
        self.user2 = 'u02_security_auditing_0138'
        self.table = 'table_security_auditing_0138'
        self.res_label = 'rl_security_auditing_0138'
        self.audit_policy = 'ap_security_auditing_0138'
        self.in_reason = 'insert_reason_0138'
    ...
    def test_encrypted(self):
        ...
        sql_cmd4 = f'drop audit policy if exists {self.audit_policy};' \
            f'create audit policy {self.audit_policy} access prepare,' \
            f'execute, deallocate' \
        ...
        self.logger.info(text)
        sql_cmd5 = f'prepare {self.in_reason}(int,char(10)) as ' \
        ...
        self.assertIn(self.constant.PREPARE_SUCCESS_MSG, msg5, '执行失败' + text)

        text = '---step6:用户2登录数据库执行语句;expect:成功---'
        self.logger.info(text)
        sql_cmd6 = f'prepare {self.in_reason}(int,char(10)) as ' \
                   f'insert into {self.table}  values (\$1,\$2);' \
                   f'execute {self.in_reason}(5, \'avcv\'); ' \
                   f'deallocate {self.in_reason};'
        ...
        self.assertIn(self.constant.PREPARE_SUCCESS_MSG, msg6, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0138 finish----')
