...
Case Name   : 统一审计策略：审计DML操作REINDEX，过滤user
Description :
    1.系统管理员用户创建两个用户
    2.系统管理员用户创建表，并赋予用户权限
    3.系统管理员用户创建资源标签
    4.系统管理员用户创建统一审计策略，过滤用户1
    5.用户1登录数据库执行REINDEX语句
    6.用户2登录数据库执行REINDEX语句
    7.查看/var/log/postgresql日志中是否审计了用户1和用户2的REINDEX操作
Expect      :
    1.用户创建成功
    2.建表成功，赋予权限
    3.创建成功
    ...
    5.执行成功
    6.执行成功
    7.日志中审计到user001的REINDEX操作、未审计到user002的REINDEX操作
History     :
"""
import os
import re
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0139 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0139'
        self.user2 = 'u02_security_auditing_0139'
        self.table = 'table_security_auditing_0139'
        self.res_label = 'rl_security_auditing_0139'
        self.audit_policy = 'ap_security_auditing_0139'
    ...
    def test_encrypted(self):
        ...
        sql_cmd4 = f'drop audit policy if exists {self.audit_policy};' \
            f'create audit policy {self.audit_policy} access reindex' \
        ...
        text = '---step5:用户1登录数据库执行REINDEX语句;expect:成功---'
        self.logger.info(text)
        ...
        self.assertIn('REINDEX', sql_cmd5, '执行失败' + text)

        text = '---step6:用户2登录数据库执行REINDEX语句;expect:成功---'
        self.logger.info(text)
        sql_cmd6 = self.sh_primy.execut_db_sql(
            f'reindex table {self.table} ;',
            sql_type=f'-U {self.user2} -W {macro.COMMON_PASSWD} ')
        self.logger.info(sql_cmd6)
        self.assertIn('REINDEX', sql_cmd6, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0139 finish----')
