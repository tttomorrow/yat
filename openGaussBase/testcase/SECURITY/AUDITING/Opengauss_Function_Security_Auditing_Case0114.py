...
Case Name   : 审计目录下审计文件的最大数量设置-1，不支持
Description :
    1.登录数据库执行gs_guc set -N all -I all -c "audit_file_remain_threshold=-1"
Expect      :
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0114 start------')
    ...
    def test_security(self):
        sql_cmd1 = f'source {self.DB_ENV_PATH};' \
                   ...
                   f'"audit_file_remain_threshold=-1"'''
    ...
    def tearDown(self):
        self.logger.info(
            '------Opengauss_Function_Security_Auditing_Case0114 end------')
