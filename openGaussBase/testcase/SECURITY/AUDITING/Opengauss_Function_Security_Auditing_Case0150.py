...
Case Name   : 统一审计策略：相同资源标签可以绑定不同的审计行为，
            统一审计按照SQL语句匹配的策略依次打印审计信息
Description :
    1.系统管理员用户创建两个用户
    ...
    5.用户1登录数据库执行语句
    6.用户2登录数据库执行语句
    7.查看/var/log/postgresql日志中是否审计了用户1和用户2的TRUNCATE、ANALYZE操作
Expect      :
    1.用户创建成功
    2.建表成功，赋予权限
    3.创建成功
    ...
    5.执行成功
    6.执行成功
    7.日志中分别审计到用户1的TRUNCATE、ANALYZE操作、
    未审计到用户2的TRUNCATE、ANALYZE操作
History     :
"""
import os
import re
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0150 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0150'
        self.user2 = 'u02_security_auditing_0150'
        self.table = 'table_security_auditing_0128'
        self.res_label = 'rl_security_auditing_0150'
        self.audit_policy = 'ap_security_auditing_0150'
        self.audit1 = 'audit01_security_auditing_0150'
        self.audit2 = 'audit02_security_auditing_0150'
    ...
    def test_encrypted(self):
        ...
        self.logger.info(text)
        sql_cmd4 = f'drop audit policy if  exists {self.audit1} ;' \
            f' create audit policy {self.audit1} privileges analyze, drop'\
            f' on label({self.res_label}) filter on roles({self.user1});'\
            f' drop audit policy if exists {self.audit2};' \
            f' create audit policy {self.audit2} access  truncate' \
            f' on label({self.res_label}) filter on roles({self.user1});'
        msg4 = self.sh_primy.execut_db_sql(sql_cmd4)
        self.logger.info(msg4)
        self.assertIn('CREATE AUDIT POLICY', msg4, '执行失败' + text)

        text = '---step5:用户1登录数据库执行语句;expect:成功---'
        self.logger.info(text)
        sql_cmd5 = self.sh_primy.execut_db_sql(
            f'truncate table {self.table};'
            f' analyze {self.table};',
            sql_type=f'-U {self.user1} -W {macro.COMMON_PASSWD} ')
        self.logger.info(sql_cmd5)

        text = '---step6:用户2登录数据库执行语句;expect:成功---'
        self.logger.info(text)
        sql_cmd6 = self.sh_primy.execut_db_sql(
            f'truncate table {self.table};'
            f' analyze {self.table};',
        ...
        assert1 = re.search(
            ...
            msg7, re.S)
        assert2 = re.search(
            f"user name.*{self.user1}.*privilege type.*[ANALYZE].*[OK]", msg7,
            re.S)
    ...
    def tearDown(self):
        ...
        clear_cmd = f'drop audit policy if exists {self.audit_policy};' \
            f'drop audit policy if exists {self.audit1};' \
            f'drop audit policy if exists {self.audit2};' \
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0150 finish----')
