...
Case Name   : 开启具体表的DML操作（SELECT除外）审计功能，设置audit_dml_state=1
Description :
            1.设置gs_guc reload -N all -I all -c "audit_dml_state=1"
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-------Opengauss_Function_Security_Auditing_Case0073 start------')
    ...
    def test_security(self):
        excute_cmd = f'source {self.DB_ENV_PATH};' \
                     f'gs_guc reload -N all -I all -c "audit_dml_state=1"'
        ...
        self.assertTrue(msg3.find('update table003 set id=5') > -1)
        self.assertTrue(msg3.find('delete from table003 where id=5') > -1)

    def tearDown(self):
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c "audit_dml_state=0"'
        msg1 = self.userNode.sh(excute_cmd1).result()
        self.logger.info(msg1)
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0073 end-----')
