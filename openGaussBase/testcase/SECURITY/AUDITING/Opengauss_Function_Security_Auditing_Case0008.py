...
Case Name   : 只审计用户注销，audit_login_logout=4
Description :
    步骤 1.设置gs_guc reload -N all -I all -c  "audit_login_logout=4"
    步骤 2.用户user003，正确的密码，登录数据库、退出数据库；
    步骤 3.用户user003，错误的密码，登录数据库
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0008 start-----')
    ...
    def test_security(self):
        execute_cmd0 = f'source {self.DB_ENV_PATH};' \
                       f'gs_guc reload -N all -I all -c "audit_login_logout=4"'
        ...
        self.assertFalse(msg3.find('success,the current user is:user003') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg2)
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0008 end----')
