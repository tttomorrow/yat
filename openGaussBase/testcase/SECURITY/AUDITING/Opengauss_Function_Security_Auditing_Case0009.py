...
Case Name   : 表示只审计用户注销和登录成功，audit_login_logout=5
Description :
    步骤 1.设置gs_guc reload -N all -I all -c  "audit_login_logout=5"
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0009 start----')
    ...
    def test_security(self):
        execute_cmd0 = f'source {self.DB_ENV_PATH};' \
                       f'gs_guc reload -N all -I all -c "audit_login_logout=5"'
    ...
    def tearDown(self):
        ...
        self.logger.info(msg2)
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0009 end-----')
