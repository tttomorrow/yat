...
Case Name   : 开启数据库对象view的CREATE、DROP、ALTER操作审计功能，
                audit_system_object=127
Description :
    1.设置gs_guc reload -N all -I all -c  "audit_system_object=127"
    2.登录数据库，创建TRIGGER对象，CREATE TRIGGER insert_trigger
               BEFORE INSERT ON test_trigger_src_tbl
               FOR EACH ROW
               EXECUTE PROCEDURE tri_insert_func();
    3.修改TRIGGER对象，ALTER TRIGGER insert_triggerON test_trigger_src_tbl RENAME
     TO insert_trigger_renamed;
    4.删除TRIGGER，DROP TRIGGER insert_trigger_renamed ON test_trigger_src_tbl;
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '------Opengauss_Function_Security_Auditing_Case0045 start-----')
    ...
    def test_security(self):
        ...
        self.assertTrue(msg3.find('CREATE TRIGGER insert_trigger') > -1)
        self.assertTrue(
            msg3.find('ALTER TRIGGER insert_trigger ON test_trigger_src_tbl '
                      'RENAME TO') > -1)
        self.assertTrue(msg3.find('DROP TRIGGER insert_trigger_renamed ON '
    ...
    def tearDown(self):
        ...
        self.logger.info(msg1)
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0045 end----')
