...
Case Name   : 关闭数据库对象INDEX的CREATE、DROP、ALTER操作审计功能，
                audit_system_object=15
Description :
    1.设置gs_guc reload -N all -I all -c "audit_system_object=15"
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0038 start-----')
    ...
    def test_security(self):
        ...
        self.assertFalse(
            msg3.find('CREATE INDEX index001 ON table02(id)') > -1)
        self.assertFalse(
            msg3.find('ALTER INDEX  index001 RENAME TO new_index') > -1)
        self.assertFalse(msg3.find('DROP INDEX new_index') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg1)
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0038 end----')
