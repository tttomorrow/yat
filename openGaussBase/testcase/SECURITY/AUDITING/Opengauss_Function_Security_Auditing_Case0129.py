...
Case Name   : 统一审计策略：对table对象的SELECT行为的审计，过滤ip地址
Description :
    1.系统管理员用户创建表
    2.系统管理员用户创建资源标签
    3.系统管理员用户创建统一审计策略
    4.系统管理员用户-h远程接主机IP地址，连接数据库执行select语句
    5.系统管理员用户-h接127.0.0.1，连接数据库执行select语句
    6.查看/var/log/postgresql日志中是否审计了IP地址$ip的操作
Expect      :
    1.建表成功
    2.创建成功
    3.创建成功
    4.执行成功
    5.执行成功
    6.日志中审计到了审计了IP地址$ip的操作,未审计到127.0.0.1地址的操作
History     :
"""
import os
import re
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0129 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.table = 'table_security_auditing_0129'
        self.res_label = 'rl_security_auditing_0129'
        self.audit_policy = 'ap_security_auditing_0129'
    ...
    def test_encrypted(self):
        ...
        text = '---step3:系统管理员用户创建统一审计策略，过滤主机ip;expect:成功---'
        self.logger.info(text)
        ...
        text = '---step5:用户1-h接127.0.0.1连接数据库执行select语句;expect:成功---'
        self.logger.info(text)
        sql_cmd6 = f'select id from {self.table};'
        excute_cmd6 = f'source {macro.DB_ENV_PATH};' \
            f'gsql -d {self.userNode.db_name} -p {self.userNode.db_port} ' \
            f'-U {self.userNode.db_user} -W {self.userNode.db_password} ' \
            f'-h 127.0.0.1 -c "{sql_cmd6}"'
        self.logger.info(excute_cmd6)
        msg6 = self.userNode.sh(excute_cmd6).result()
        self.common.equal_sql_mdg(msg6, 'id', '6', '(1 row)', flag='1')
        
        text = '--step6:查看postgresql日志中是否审计了主机ip的操作;expect:成功--'
        self.logger.info(text)
        sleep(2)
        ...
        assert1 = re.search(
            f"user name.*{self.userNode.db_user}.*client_ip.*"
            f"{self.userNode.db_host}.*access type.*[SELECT].*result.*[OK]",
            msg7, re.S)
        self.assertTrue(assert1, '执行失败:' + text)
        self.assertNotIn('127.0.0.1', msg7, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0129 finish----')
