...
Case Name   : 开启数据库对象RESOURCE POOL的CREATE、DROP、ALTER操作审计功能
Description :
    1.设置参数"audit_system_object=1023"；
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '------Opengauss_Function_Security_Auditing_Case0053 start-----')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.userNode = Node('PrimaryDbUser')
        self.constant = Constant()
        self.common = Common()
        self.respol = 'respol_security_audit_0053'
        self.default_param = self.common.show_param('audit_system_object')
    
    def test_security(self):
        text = 'step1:设置参数audit_system_object=1023;expect:成功'
        self.logger.info(text)
        self.sh_primy.execute_gsguc('reload', self.constant.GSGUC_SUCCESS_MSG,
                                    f'audit_system_object=1023')
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0053 end-----')
