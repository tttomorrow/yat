...
Case Name   : 开启数据库对象SCHEMA的CREATE、DROP、ALTER操作审计功能，
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0035 start----')
    ...
    def test_security(self):
        excute_cmd = f'''source {self.DB_ENV_PATH};
            gs_guc reload -N all -I all -c "audit_system_object=3"'''
        ...
        self.assertTrue(msg3.find('create schema schema001') > -1)
        self.assertTrue(
            msg3.find('alter schema schema001 rename to  schema002') > -1)
        self.assertTrue(msg3.find('drop schema schema002') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0035 end----')
