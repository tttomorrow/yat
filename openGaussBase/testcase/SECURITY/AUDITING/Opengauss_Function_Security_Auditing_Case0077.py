...
Case Name   : 关闭具体表的DML操作（SELECT除外）审计功能，audit_dml_state_select=0
Description :
            1.设置gs_guc reload -N all -I all -c "audit_dml_state_select=0"
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0077 start-----')
    ...
    def test_security(self):
        excute_cmd1 = f'show audit_dml_state_select;'
        msg1 = self.sh_primy.execut_db_sql(excute_cmd1)
        self.logger.info(msg1)
        self.common.equal_sql_mdg(msg1, 'audit_dml_state_select', '0',
                                  '(1 row)', flag='1')
        self.logger.info(msg1)
        ...
        self.assertFalse(msg3.find('select *  from table003;') > -1)

    def tearDown(self):
        self.logger.info(
            '------Opengauss_Function_Security_Auditing_Case0077 end----')
