...
Case Name   : 关闭具体表的DML操作（SELECT除外）审计功能，设置audit_dml_state=1
Description :
    1.设置gs_guc reload -N all -I all -c "audit_set_parameter=0"
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '-------Opengauss_Function_Security_Auditing_Case0093 start-----')
    ...
    def test_security(self):
        excute_cmd = f'source {self.DB_ENV_PATH};' \
                     f'gs_guc reload -N all -I all -c "audit_set_parameter=0"'
        ...
        self.assertFalse(msg3.find('SET SEARCH_PATH TO ds, public') > -1)

    def tearDown(self):
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c "audit_set_parameter=1"'
        msg1 = self.userNode.sh(excute_cmd1).result()
        self.logger.info(msg1)
        self.logger.info(
            '------Opengauss_Function_Security_Auditing_Case0093 end-----')
