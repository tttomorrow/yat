...
Case Name   : 关闭数据库对象PROCEDURE/FUNCTION的CREATE、DROP、ALTER操作审计功能，
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '------Opengauss_Function_Security_Auditing_Case0046 start-----')
    ...
    def test_security(self):
        ...
        sql_cmd2 = 'CREATE FUNCTION func_add_sql(integer, integer) RETURNS ' \
                   ...
                   'IMMUTABLE ' \
                   'RETURNS NULL ON NULL INPUT;' \
                   'ALTER FUNCTION func_add_sql(integer, integer) RENAME TO ' \
                   'new_func_add_sql;' \
        ...
        self.assertFalse(msg3.find('CREATE FUNCTION func_add_sql') > -1)
        self.assertFalse(msg3.find('ALTER FUNCTION func_add_sql') > -1)
        self.assertFalse(msg3.find('DROP FUNCTION new_func_add_sql') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0046 end----')
