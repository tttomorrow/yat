...
Description :
    ...
    2.登录数据库，创建view对象，CREATE VIEW myView AS SELECT * FROM pg_tablespace
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '------Opengauss_Function_Security_Auditing_Case0043 start-----')
    ...
    def test_security(self):
        excute_cmd = f'source {self.DB_ENV_PATH};' \
                     f'gs_guc reload -N all -I all -c "audit_system_object=63"'
        ...
        self.assertTrue(
            msg3.find(
                'CREATE VIEW myView AS SELECT * FROM pg_tablespace WHERE '
                'spcname = \'pg_default\'') > -1)
        self.assertTrue(msg3.find('DROP VIEW myView') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info(msg1)
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0043 end-----')
