...
Case Name   : 统一审计策略：对table对象的copy行为审计，过滤user
Description :
    1.系统管理员用户创建两个用户
    ...
    5.用户1登录数据库执行copy语句
    6.用户2登录数据库执行copy语句
    7.查看/var/log/postgresql日志中是否审计了用户1和用户2的copy操作，
Expect      :
    1.用户创建成功
    2.建表成功，赋予权限
    3.创建成功
    ...
    5.执行成功
    6.执行成功
    7.日志中审计到了用户1的copy操作，未审计到用户2的copy操作
History     :
"""
import os
import re
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0137 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0137'
        self.user2 = 'u02_security_auditing_0137'
        self.table = 'table_security_auditing_0137'
        self.res_label = 'rl_security_auditing_0137'
        self.audit_policy = 'ap_security_auditing_0137'
        self.log_file = '/var/log/postgresql'
        self.config_path = '/etc'
        self.default_adss = self.common.show_param('audit_copy_exec')
        ...
        self.default_copy = self.common.show_param('enable_copy_server_files')
    ...
    def test_encrypted(self):
        ...
        text = '---step1.3:设置audit_copy_exec=1;expect:成功---'
        self.logger.info(text)
        result = self.sh_primy.execute_gsguc('reload',
                                             self.constant.GSGUC_SUCCESS_MSG,
                                             f'audit_copy_exec=1')
        ...
        text = '---step1.4:设置enable_copy_server_files=on;expect:成功---'
        self.logger.info(text)
        result = self.sh_primy.execute_gsguc('reload',
                                             self.constant.GSGUC_SUCCESS_MSG,
                                             f'enable_copy_server_files=on')
        self.assertEqual(True, result, '执行失败' + text)
        ...
        sql_cmd2 = f'drop table if exists {self.table} cascade;' \
            ...
            f'insert into {self.table} values(1,\'{self.user1}\');' \
        ...
        sql_cmd4 = f'drop audit policy if exists {self.audit_policy};' \
            f'create audit policy {self.audit_policy} access copy' \
        ...
        text = '---step5:用户1登录数据库执行copy语句;expect:成功---'
        self.logger.info(text)
        sql_cmd5 = f'copy {self.table} to \'$path/xq.txt\';'
        ...
        self.assertIn('COPY to or from a file', msg5, '执行失败' + text)

        text = '---step6:用户2登录数据库执行copy语句;expect:成功---'
        self.logger.info(text)
        sql_cmd6 = f'copy {self.table} to \'$path/xq.txt\';' \
                   f'delete from {self.table}  where id=2;'
        ...
        self.assertIn('COPY to or from a file', msg6, '执行失败' + text)
        ...
        assert1 = re.search(
            f"user name.*{self.user1}.*access type.*[COPY].*[OK]", msg7,
            re.S)
    ...
    def tearDown(self):
        ...
        result3 = self.sh_primy.execute_gsguc('reload',
                        self.constant.GSGUC_SUCCESS_MSG,
                        f'audit_copy_exec={self.default_adss}')
        result4 = self.sh_primy.execute_gsguc('reload',
                        self.constant.GSGUC_SUCCESS_MSG,
                        f'syslog_facility={self.default_facility}')
        result5 = self.sh_primy.execute_gsguc('reload',
                        self.constant.GSGUC_SUCCESS_MSG,
                        f'enable_copy_server_files={self.default_copy}')
        ...
        self.assertEqual(True, result5, '执行失败' + text2)
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0137 finish----')
