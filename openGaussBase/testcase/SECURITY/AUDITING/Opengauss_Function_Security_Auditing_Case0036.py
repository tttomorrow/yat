...
Case Name   : 关闭数据库对象user的CREATE、DROP、ALTER操作审计功能，
                audit_system_object=3
Description :
...
class Auditing(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '------Opengauss_Function_Security_Auditing_Case0036 start------')
    ...
    def test_security(self):
        excute_cmd = f'source {self.DB_ENV_PATH};' \
                     f'gs_guc reload -N all -I all -c "audit_system_object=3"'
        ...
        self.logger.info(msg2)
        self.assertTrue(
            'CREATE ROLE' in msg2 and 'ALTER ROLE' in msg2 and 'DROP ROLE'
            in msg2)
        time.sleep(5)
        ...
        self.assertFalse(msg3.find('create user user001 with password') > -1)
        self.assertFalse(msg3.find('alter user user001 with password') > -1)
        self.assertFalse(msg3.find('drop user user001') > -1)
    ...
    def tearDown(self):
        ...
        self.common.equal_sql_mdg(msg2, 'audit_system_object', '12295',
                                  '(1 row)', flag="1")
        self.logger.info(
            '-----Opengauss_Function_Security_Auditing_Case0036 end-----')
