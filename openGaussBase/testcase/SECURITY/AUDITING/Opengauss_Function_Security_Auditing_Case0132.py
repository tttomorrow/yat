...
Case Name   : 统一审计策略：对table对象的SELECT行为的审计，过滤user，指定DISABLE
Description :
    1.系统管理员用户创建两个用户
    2.系统管理员用户创建表，并赋予用户权限
    3.系统管理员用户创建资源标签
    4.系统管理员用户创建统一审计策略，过滤用户1
    5.用户1登录数据库执行select语句
    6.用户2登录数据库执行select语句
...
class Security(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            '---Opengauss_Function_Security_Auditing_Case0132 start---')
        self.userNode = Node(node='PrimaryDbUser')
        self.primary_root = Node(node='PrimaryRoot')
        self.sh_primy = CommonSH('PrimaryDbUser')
        self.common = Common()
        self.constant = Constant()
        self.user1 = 'u01_security_auditing_0132'
        self.user2 = 'u02_security_auditing_0132'
        self.table = 'table_security_auditing_0132'
        self.res_label = 'rl_security_auditing_0132'
        self.audit_policy = 'ap_security_auditing_0132'
    ...
    def tearDown(self):
        ...
        self.logger.info(
            '----Opengauss_Function_Security_Auditing_Case0132 finish----')
