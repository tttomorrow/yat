...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于私有模式下的临时表没有TRUNCATE权限，赋予TRUNCATE权限之后可以清空表
Description :
...
Expect      :
    1.成功
    2.成功
    3.只有未授权时用户2对临时表进行truncate时权限拒绝
    4.成功
History     :
"""
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_truncateprivilege_0008'
        self.user_name1 = 'u_security_truncateprivilege_0008_01'
        self.user_name2 = 'u_security_truncateprivilege_0008_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换用户1创建临时表并向表中插入数据' \
               ...
               '切换用户1授权truncate给用户2;' \
               '授权后切换用户2对临时表进行truncate操作 ' \
               'expect:只有未授权时用户2对临时表进行truncate时权限拒绝----'
        self.log.info(text)
        truncate_cmd = f"set role {self.user_name2} password " \
            f"'{macro.PASSWD_REPLACE}';" \
