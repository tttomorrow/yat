...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证超级用户可以更新用户私有模式下的临时表
                ((超级用户为sysadmin用户权限，访问auditadmin 用户创建的临时表)
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建不同权限用户
    3.切换auditadmin用户创建临时表并插入数据
      切换sysadmin用户对表进行update操作
    4.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_updateprivilege_0004'
        self.user_name1 = 'u_security_updateprivilege_0004_01'
        self.user_name2 = 'u_security_updateprivilege_0004_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换auditadmin用户创建临时表并插入数据;' \
               '切换sysadmin用户对表进行update操作; expect:成功----'
        self.log.info(text)
