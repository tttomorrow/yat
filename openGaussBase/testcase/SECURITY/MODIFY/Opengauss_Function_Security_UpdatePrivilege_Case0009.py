...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于global临时表没有UPDATE权限，赋予UPDATE权限之后可以更新表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.切换用户1创建全局临时表并向表中插入数据并授权
      切换用户2对全局临时表进行update操作
      切换用户1授权表给普通用户2
      授权后切换用户2对全局临时表进行update操作
    4.清理环境
Expect      :
    1.成功
    2.成功
    3.只有未授权时用户2对表进行update操作权限拒绝
    4.成功
History     :
"""
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_updataprivilege_0009'
        self.user_name1 = 'u_security_updateprivilege_0009_01'
        self.user_name2 = 'u_security_updateprivilege_0009_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换用户1创建全局临时表并向表中插入数据并授权;' \
               '切换用户2对全局临时表进行update操作;' \
               '切换用户1授权表给普通用户2;' \
               '授权后切换用户2对全局临时表进行update操作 ' \
               'expect:只有未授权时用户2对表进行update操作权限拒绝----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            create global temp table {self.user_name1}.{self.table_name}
            ...
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant update on table {self.user_name1}.{self.table_name}
            to {self.user_name2};
            set role {self.user_name2} password '{macro.PASSWD_REPLACE}';
            update {self.user_name1}.{self.table_name} set name='shanxi';''')
