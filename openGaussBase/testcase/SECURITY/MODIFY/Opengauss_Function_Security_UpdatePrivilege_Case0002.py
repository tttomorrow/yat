...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证超级用户可以UPDATE用户私有模式下的表
                (超级用户为初始用户权限，访问createrole用户创建的普通表)
Description :
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_updateprivilege_0002'
        self.user_name = 'u_security_updateprivilege_0002'
    ...
    def test_security(self):
        ...
        text = '----step4:初始用户对表进行update操作; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
