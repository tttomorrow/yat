...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于私有模式下的分区表没有UPDATE权限，赋予UPDATE权限之后可以更新表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.切换用户1创建分区表，向表中插入数据并授权给用户2
    4.切换用户2对分区表进行update操作
    5.切换用户1授权给用户2;
      授权后切换用户2对分区表进行update操作
    6.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_updateprivilege_0007'
        self.user_name1 = 'u_security_updateprivilege_0007_01'
        self.user_name2 = 'u_security_updateprivilege_0007_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换用户1创建分区表，向表中插入数据并授权给用户2 ' \
               'expect:成功----'
        self.log.info(text)
        ...
        text = '----step4:切换用户2对分区表进行update操作 ' \
        ...
        text = '----step5:切换用户1授权表给用户2;' \
               '授权后切换用户2对分区表进行update操作 expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
