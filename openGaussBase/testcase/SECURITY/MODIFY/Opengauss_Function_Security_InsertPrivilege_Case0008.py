...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证超级用户对其他用户创建的表有insert权限
                (超级用户为初始用户权限，访问普通用户创建的表)
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建普通用户
    3.切换普通用户创建表
    4.超级用户对表进行insert操作并查询表
      copyto命令导出表数据
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_insertprivilege_0008'
        self.user_name = 'u_security_insertprivilege_0008'
        self.file_path = os.path.join(macro.DB_INSTANCE_PATH,
                                      'security_insertprivilege_0008.txt')
    ...
    def test_security(self):
        ...
        text = '----step3:切换普通用户创建表 expect:成功----'
        self.log.info(text)
        ...
        text = '----step4:超级用户对表进行insert操作并查询表' \
