...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于public模式下的临时表没有TRUNCATE权限，赋予TRUNCATE权限之后可以查询表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.切换用户1创建临时表并向表中插入数据
      切换用户2对临时表进行truncate操作
      切换用户1授权表给用户2
      授权后切换用户2对临时表进行truncate操作
    4.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_truncateprivilege_0004'
        self.user_name1 = 'u_security_truncateprivilege_0004_01'
        self.user_name2 = 'u_security_truncaterivilege_0004_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换用户1创建临时表并向表中插入数据' \
               '切换用户2对临时表进行truncate操作' \
               '切换用户1授权表给用户2' \
               '授权后切换用户2对临时表进行truncate操作 ' \
               'expect:只有未授权时用户2对临时表进行truncate操作权限拒绝----'
        self.log.info(text)
        truncate_cmd = \
        ...
        sql_cmd = self.primary_sh.execut_db_sql(f'''
           ...
           {truncate_cmd}
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant truncate on table {self.table_name} to {self.user_name2};
            {truncate_cmd}''')
