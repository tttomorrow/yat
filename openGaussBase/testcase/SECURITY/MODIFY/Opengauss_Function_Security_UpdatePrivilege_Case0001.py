...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证超级用户可以UPDATE其他用户创建的表
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_updateprivilege_0001'
        self.user_name1 = 'u_security_updateprivilege_0001_01'
        self.user_name2 = 'u_security_updateprivilege_0001_02'
    ...
    def test_security(self):
        ...
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password 
        ...
        text = '----step5:切换sysadmin用户对表进行update操作; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name2} password  '{macro.PASSWD_REPLACE}';
            update public.{self.table_name} set name='shanxi' where id=1;''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SET_SUCCESS_MSG, sql_cmd,
                      "执行失败:" + text)
