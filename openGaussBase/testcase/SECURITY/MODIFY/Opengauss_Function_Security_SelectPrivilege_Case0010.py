...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于局部临时表没有select权限，赋予select权限之后可以查询表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.切换普通用户1创建局部临时表并向表中插入数据
      切换普通用户2对局部临时表进行select操作
      切换普通用户1授权给普通用户2
      授权后切换普通用户2对局部临时表进行select操作
    4.清理环境
Expect      :
    1.成功
    2.成功
    3.只有未授权时普通用户2对表进行select操作时权限拒绝
    4.成功
History     :
"""
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.user_name1 = 'u_security_selectprivilege_0010_01'
        self.user_name2 = 'u_security_selectprivilege_0010_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换普通用户1创建局部临时表并向表中插入数据; ' \
               '切换普通用户2对局部临时表进行select操作;' \
               '切换普通用户1授权表给普通用户2;' \
               '授权后切换普通用户2对局部临时表进行select操作 ' \
               'expect:只有未授权时普通用户2对表进行select操作时权限拒绝----'
        self.log.info(text)
        select_cmd = \
