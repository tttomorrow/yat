...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于public模式下的分区表没有TRUNCATE权限，赋予TRUNCATE权限之后可以清空表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.授予普通用户1在public下创建对象的权限
    4.切换普通用户1创建分区表并插入数据
    5.切换普通用户2对分区表进行truncate操作
    6.切换普通用户1对授权给普通用户2
    7.切换普通用户2对分区表进行truncate操作
    8.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_truncateprivilege_0003'
        self.user_name1 = 'u_security_truncateprivilege_0003_01'
        self.user_name2 = 'u_security_truncateprivilege_0003_02'
    ...
    def test_security(self):
        ...
        text = '----step4:切换普通用户1创建分区表并插入数据; expect:成功----'
        self.log.info(text)
        ...
        text = '----step5:切换普通用户2对分区表进行truncate操作; ' \
        ...
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant truncate on table public.{self.table_name} 
            to {self.user_name2};''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SET_SUCCESS_MSG, sql_cmd,
                      "执行失败:" + text)
        ...
        text = '----step7:切换普通用户2对分区表进行truncate操作; ' \
               'expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name2} password '{macro.PASSWD_REPLACE}';
