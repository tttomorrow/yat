...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于私有模式下的临时表的列没有insert权限，赋予insert权限之后可以插入数据
Description :
    ...
    3.切换用户1创建临时表
      切换用户2对临时表进行insert操作
      切换用户1授权给用户2
      授权后切换用户2对临时表进行insert操作
    4.清理环境
Expect      :
    1.成功
    2.成功
    3.只有授权后对临时表进行insert操作时成功
    4.成功
History     : 
"""
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_insertprivilege_0002'
        self.user_name1 = 'u_security_insertprivilege_0002_01'
        self.user_name2 = 'u_security_insertprivilege_0002_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换用户1创建临时表' \
               '切换用户2对临时表进行insert操作' \
               '切换用户1授权给用户2' \
               '授权后切换用户2对临时表进行insert操作' \
               ' expect:只有授权后对临时表进行insert操作时成功----'
        self.log.info(text)
        insert_cmd = f'''
            set role {self.user_name2} password '{macro.PASSWD_REPLACE}';
            insert into {self.table_name}(id,name) 
        ...
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            ...
            {insert_cmd}
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant insert(id,name) on table {self.table_name} 
            to {self.user_name2};
            {insert_cmd}''')
