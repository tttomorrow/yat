...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证超级用户可以查询其他用户创建的表(超级用户为sysadmin用户权限，访问普通用户创建表)
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建不同权限用户
    3.授予普通用户在public下创建对象的权限
    4.切换普通用户创建表并插入数据
    5.切换sysadmin用户对表进行select操作
    6.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_selectprivilege_0001'
        self.user_name1 = 'u_security_selectprivilege_0001_01'
        self.user_name2 = 'u_security_selectprivilege_0001_02'
    ...
    def test_security(self):
        ...
        text = '----step5:切换sysadmin超级用户对表进行select; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
