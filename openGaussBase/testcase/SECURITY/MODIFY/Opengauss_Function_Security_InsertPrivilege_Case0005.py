...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于私有模式下的分区表的列没有insert权限，赋予insert权限之后可以插入数据
Description :
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_insertprivilege_0005'
        self.user_name1 = 'u_security_insertprivilege_0005_01'
        self.user_name2 = 'u_security_insertprivilege_0005_02'
    ...
    def test_security(self):
        ...
        text = '----step5:切换用户1授权insert给用户2' \
               '授权后切换用户2对分区表进行insert操作 ' \
               'expect:授权成功,insert操作permission denied----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant insert(id,name) on table {self.user_name1}.{self.table_name} 
            to {self.user_name2};
            {insert_cmd}''')
        self.log.info(sql_cmd)
