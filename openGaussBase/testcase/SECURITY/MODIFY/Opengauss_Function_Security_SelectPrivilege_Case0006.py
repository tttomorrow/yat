...
Case Name   : 三权分立打开时验证超级用户可以查询用户私有模式下的局部临时表
                (超级用户为sysadmin权限，访问auditadmin用户创建的局部临时表）
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建不同权限用户
    3.切换auditadmin用户创建局部临时表并插入数据，切换sysadmin用户对局部临时表进行select操作
    4.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_selectprivilege_0006'
        self.user_name1 = 'u_security_selectprivilege_0006_01'
        self.user_name2 = 'u_security_selectprivilege_0006_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换auditadmin用户创建局部临时表并插入数据;' \
