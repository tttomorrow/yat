...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证非超级用户对其他用户在私有模式下创建的表没有select权限，赋权之后可以查询
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建不同权限用户
    3.切换auditadmin用户创建表并向表中插入数据并授权usage给普通用户
    4.切换普通用户对表进行select操作
    5.切换auditadmin用户授权表给普通用户2;
    6.授权后切换普通用户对表进行select操作
    7.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_selectprivilege_0015'
        self.user_name1 = 'u_security_selectprivilege_0015_01'
        self.user_name2 = 'u_security_selectprivilege_0015_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换auditadmin用户创建表并向表中插入数据' \
        ...
        text = '----step5:切换auditadmin用户对授权给普通用户2; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant select on table {self.user_name1}.{self.table_name} 
            to {self.user_name2};''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SET_SUCCESS_MSG, sql_cmd,
                      "执行失败:" + text)
        ...
        text = '----step6:切换普通用户对表进行select操作; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name2} password '{macro.PASSWD_REPLACE}';
            select * from {self.user_name1}.{self.table_name};''')
