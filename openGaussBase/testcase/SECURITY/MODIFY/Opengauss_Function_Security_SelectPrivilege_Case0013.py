...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于public模式下的临时表没有select权限，赋予select权限之后可以查询表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建不同权限用户
    3.切换createrole用户创建临时表并向表中插入数据
      切换普通用户对临时表进行select操作
      切换createrole用户授权表给普通用户
      授权后切换普通用户对临时表进行select操作
    4.清理环境
Expect      :
    1.成功
    2.成功
    3.只有未授权时普通用户对表进行select操作权限拒绝
    4.成功
History     :
"""
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_updateprivilege_0013'
        self.user_name1 = 'u_security_selectprivilege_0013_01'
        self.user_name2 = 'u_security_selectprivilege_0013_02'
    ...
    def test_security(self):
        ...
        text = '----step3:换createrole用户创建临时表并向表中插入数据' \
               '切换普通用户对临时表进行select操作' \
               '切换createrole用户授权表给普通用户' \
               '授权后切换普通用户对临时表进行select操作 ' \
               'expect:只有未授权时普通用户对临时表进行select操作权限拒绝----'
        self.log.info(text)
        select_cmd = \
            f"set role {self.user_name2} password '{macro.PASSWD_REPLACE}';" \
        ...
        sql_cmd = self.primary_sh.execut_db_sql(f'''
           ...
           {select_cmd}
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant select on table {self.table_name} to {self.user_name2};
            {select_cmd}''')
