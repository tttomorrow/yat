...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证私有模式下用户没有truncate权限时，其他权限不受影响（普通表）
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.切换用户1创建表并向表中插入数据，授权usage给用户2
    4.切换用户2对表进行select,insert,update,delete,truncate操作
    5.切换用户1授权select,insert,update,delete给用户2
      授权后切换用户2对表进行select,insert,update,delete,truncate操作
    6.清理环境
Expect      :
    1.成功
    2.成功
    ...
    5.只有truncate表permission denied
    6.成功
History     :
"""
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_truncateprivilege_0009'
        self.user_name1 = 'u_security_truncateprivilege_0009_01'
        self.user_name2 = 'u_security_truncateprivilege_0009_02'
    ...
    def test_security(self):
        ...
        text = '----step4:切换用户2对表进行select,insert,update,' \
               'delete,truncate操作  ' \
               'expect:select,insert,update,delete,truncate操作' \
               '均permission denied----'
        self.log.info(text)
        common_cmd = f'''
