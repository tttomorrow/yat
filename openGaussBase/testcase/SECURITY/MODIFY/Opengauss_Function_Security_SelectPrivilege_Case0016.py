...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于私有模式下的视图中的列没有select权限，赋予select权限之后可以查询表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.切换用户1创建表插入数据并创建视图并授权usage给用户2
    4.切换用户2对视图中的列进行select操作
    5.切换用户1授权给普通用户
    6.授权后切换用户2对视图中列进行select操作
    7.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_selectprivilege_0016'
        self.user_name1 = 'u_security_selectprivilege_0016_01'
        self.user_name2 = 'u_security_selectprivilege_0016_02'
        self.view_name = 'v_security_selectprivilege_0016'
    ...
    def test_security(self):
        ...
        text = '----step5:切换用户1授权给用户2; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant select(id) on table {self.view_name} to 
            {self.user_name2};''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SET_SUCCESS_MSG, sql_cmd,
                      "执行失败:" + text)
        ...
        text = '----step6:授权后切换用户2对视图中的列进行select操作 ' \
               'expect:成功---'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name2} password '{macro.PASSWD_REPLACE}';
