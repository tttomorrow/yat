...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于public模式下的视图没有select权限，赋予select权限之后可以查询表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建不同权限用户
    3.授予createrole用户在public下创建对象的权限
    4.切换createrole用户创建表插入数据并创建视图并授权usage给普通用户
    5.切换普通用户对视图进行select操作
    6.切换createrole用户授权给普通用户
    7.授权后切换普通用户对视图进行select操作
    8.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_selectprivilege_0014'
        self.user_name1 = 'u_security_selectprivilege_0014_01'
        self.user_name2 = 'u_security_selectprivilege_0014_02'
        self.view_name = 'v_security_selectprivilege_0014'
    ...
    def test_security(self):
        ...
        create_cmd = f"drop user if exists {self.user_name1} cascade;" \
            ...
            f"create user {self.user_name1} createrole password " \
        ...
        text = '-----step3：授予createrole用户在public下创建对象的权限 ' \
               'expect:成功-----'
        self.log.info(text)
        ...
        text = '----step4:切换createrole用户创建表插入数据并创建视图' \
               '并授权usage给普通用户 expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            create table public.{self.table_name}(id int,name varchar(100));
            insert into public.{self.table_name} 
            values(1,'beijing'),(2,'shanghai');
            create or replace view public.{self.view_name} as 
        ...
        text = '----step6:切换createrole用户授权给普通用户; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant select on table public.{self.view_name} to 
            {self.user_name2};''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SET_SUCCESS_MSG, sql_cmd,
                      "执行失败:" + text)
        ...
        text = '----step7:授权后切换普通用户对视图进行select操作 expect:成功---'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name2} password '{macro.PASSWD_REPLACE}';
            select * from public.{self.view_name};''')
