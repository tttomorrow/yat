...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证私有模式下用户没有truncate权限时，其他权限不受影响（分区表）
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.切换用户1创建分区表并向表中插入数据，授权usage给用户2
    4.切换用户2对分区表进行select,insert,update,delete,truncate操作
    5.切换用户1授权select,insert,update,delete给用户2,
      授权后切换用户2对分区表进行select,insert,update,delete,truncate操作
    6.清理环境
Expect      :
    1.成功
    2.成功
    3.成功
    4.select,insert,update,delete,truncate操作均permission denied
    5.只有truncate表时permission denied
    6.成功
History     :
"""
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_truncateprivilege_0011'
        self.user_name1 = 'u_security_truncateprivilege_0011_01'
        self.user_name2 = 'u_security_truncateprivilege_0011_02'
    ...
    def test_security(self):
        ...
        text = '----step3:切换用户1创建分区表并向表中插入数据;' \
               '授权usage给用户2 expect:成功----'
        self.log.info(text)
        ...
        text = '----step4:切换用户2对分区表进行select,insert,update,' \
               'delete,truncate操作  ' \
               'expect:select,insert,update,delete,truncate' \
               '操作均permission denied----'
        self.log.info(text)
        common_cmd = f'''
            set role {self.user_name2} password '{macro.PASSWD_REPLACE}';
            select * from {self.user_name1}.{self.table_name};
            insert into {self.user_name1}.{self.table_name} 
            values(1,'beijing'),(2,'shanghai');
            update  {self.user_name1}.{self.table_name} 
            set name='shanxi' where id=1;
            delete from {self.user_name1}.{self.table_name} where id=2;
            truncate table {self.user_name1}.{self.table_name};'''
        self.log.info(common_cmd)
        sql_cmd = self.primary_sh.execut_db_sql(f'''{common_cmd}''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SET_SUCCESS_MSG, sql_cmd,
                      "执行失败:" + text)
        self.assertTrue(sql_cmd.count(self.constant.PERMISSION_DENIED) == 5,
                        "执行失败:" + text)

        text = '----step5:切换用户1授权select,insert,update,delete给用户2;' \
               '授权后切换用户2对表进行select,insert,update,' \
               'delete,truncate操作 ' \
               'expect:只有truncate表时permission denied----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password '{macro.PASSWD_REPLACE}';
            grant select,insert,update,delete on table 
            {self.user_name1}.{self.table_name} to {self.user_name2};
            {common_cmd}''')
        self.log.info(sql_cmd)
