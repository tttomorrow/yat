...
Description :
    ...
    2.创建两个普通用户
    3.切换普通用户1创建表并向表中插入数据并授权usage给普通用户2
    4.切换普通用户2对表进行select操作
    5.切换普通用户1授权表给普通用户2;
    6.授权后切换用户2对表进行select操作
    7.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_selectprivilege_0008'
        self.user_name1 = 'u_security_selectprivilege_0008_01'
        self.user_name2 = 'u_security_selectprivilege_0008_02'
    ...
    def test_security(self):
        ...
        text = '----step4:切换普通用户2对表进行select操作; ' \
        ...
        text = '----step6:切换普通用户2对表进行select操作; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name2} password '{macro.PASSWD_REPLACE}';
            select * from {self.user_name1}.{self.table_name};''')
