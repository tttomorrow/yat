...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证超级用户对其他用户创建的表有TRUNCATE权限
                (超级用户为sysadmin用户权限，访问普通用户创建的表)
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建不同权限用户
    3.授予普通用户在public下创建对象的权限
    4.切换普通用户创建表并插入数据
    5.切换sysadmin用户对表进行truncate操作
    6.清理环境
Expect      :
    1.成功
    2.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_truncateprivilege_0001'
        self.user_name1 = 'u_security_truncateprivilege_0001_01'
        self.user_name2 = 'u_security_truncateprivilege_0001_02'
    ...
    def test_security(self):
        ...
        create_cmd = f"drop user if exists {self.user_name1} cascade;" \
            ...
            f"create user {self.user_name1} sysadmin password " \
        ...
        text = '-----step3：授予普通用户在public下创建对象的权限 ' \
               'expect:成功-----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(
            f"grant create on schema public to {self.user_name2};")
        self.log.info(sql_cmd)
        self.assertIn(self.constant.GRANT_SUCCESS_MSG, sql_cmd,
                      '执行失败:' + text)

        text = '----step4:切换普通用户创建表并插入数据; expect:成功----'
        self.log.info(text)
        ...
        text = '----step5:切换sysadmin用户对表进行truncate; expect:成功----'
        self.log.info(text)
        sql_cmd = self.primary_sh.execut_db_sql(f'''
            set role {self.user_name1} password  '{macro.PASSWD_REPLACE}';
            truncate table public.{self.table_name};''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SET_SUCCESS_MSG, sql_cmd,
                      "执行失败:" + text)
