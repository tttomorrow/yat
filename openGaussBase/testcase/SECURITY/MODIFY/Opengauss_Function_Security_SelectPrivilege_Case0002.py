...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证超级用户可以查询用户私有模式下的表(超级用户为初始用户权限，访问createrole用户创建的普通表)
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建createrole用户
    3.切换createrole用户创建表并插入数据
    4.超级用户对表进行select操作
    5.清理环境
Expect      :
    1.成功
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_selectprivilege_0002'
        self.user_name = 'u_security_selectprivilege_0002'
    ...
    def test_security(self):
        ...
        text = '----step4:超级用户对表进行select操作; expect:成功----'
        self.log.info(text)
