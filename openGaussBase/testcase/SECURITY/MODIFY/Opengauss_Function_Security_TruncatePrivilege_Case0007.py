...
"""
Case Type   : 防篡改
Case Name   : 三权分立打开时验证用户对于私有模式下的分区表没有TRUNCATE权限，赋予TRUNCATE权限之后可以清空表
Description :
    1.设置参数并重启数据库enableSeparationOfDuty=on
    2.创建两个普通用户
    3.切换用户1创建分区表并向表中插入数据
    4.切换用户2对分区表进行truncate操作
    5.切换用户1授权usage给用户2;
      授权后切换用户2对分区表进行truncate操作
...
class ModifyCase(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.table_name = 't_security_truncateprivilege_0007'
        self.user_name1 = 'u_security_truncateprivilege_0007_01'
        self.user_name2 = 'u_security_truncateprivilege_0007_02'
