-- @testpoint: 验证integer,varchar,smallint,double decision,bytea,date,boolean类型(合理)的应用

--step1:创建表   expect:成功
drop table if exists t_postgis_0005 cascade;
create table t_postgis_0005 (
...
insert into t_postgis_0005 values(2147483647,'name1','name2',32767,1,2,3,4,5,6,321.321,321.321,7,'varying',8,9,E'\\xDEADBEEF',12.12,23.23,34.34,45.45,E'\\xABCD',date '08-25-2020','varying',21.21,'info','info:text','description','smschema',true,E'\\xABCDEF');
insert into t_postgis_0005 values(1,to_char(lpad('a',5000,'x')),to_char(lpad('b',5000,'y')),32767,1,default,default,4,5,6,default,default,default,to_char(lpad('c',5000,'z')),default,9,E'\\xDEADBE',121.121,232.232,343.343,454.454,E'\\xABCD',default,to_char(lpad('d',5000,'q')),default,to_char(lpad('d',2048,'g')),'info:text','description',default,default,E'\\xABCDEF');

--step3:查看数据   expect:成功
select * from t_postgis_0005;

--step4:清理环境   expect:成功
drop table t_postgis_0005;
