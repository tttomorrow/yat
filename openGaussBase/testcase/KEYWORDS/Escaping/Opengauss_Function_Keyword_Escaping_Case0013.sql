--  @testpoint:openGauss关键字escaping(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "escaping" from "escaping" where "escaping"='hello'
union all (select "escaping" from "escaping" where "escaping"!='hello' order by "escaping" limit 2);
