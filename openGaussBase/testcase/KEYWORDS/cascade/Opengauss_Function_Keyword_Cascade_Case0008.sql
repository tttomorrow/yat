--  @testpoint:openGauss关键字cascade(非保留)，作为列名带引号并且删除时使用该列,建表成功，cascade列值是'hello'的删除成功
--创建表
...
--向表中插入数据
insert into cascade_test(c_id) values(2);

--删除表数据
delete from cascade_test where "cascade"='hello';
