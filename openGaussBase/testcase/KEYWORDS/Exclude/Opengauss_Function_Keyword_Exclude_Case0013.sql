--  @testpoint:openGauss关键字exclude(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "exclude" from "exclude" where "exclude"='hello'
union all (select "exclude" from "exclude" where "exclude"!='hello' order by "exclude" limit 2);
