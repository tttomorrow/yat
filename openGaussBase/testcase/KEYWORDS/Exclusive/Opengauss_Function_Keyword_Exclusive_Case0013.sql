--  @testpoint:openGauss关键字exclusive(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "exclusive" from "exclusive" where "exclusive"='hello'
union all (select "exclusive" from "exclusive" where "exclusive"!='hello' order by "exclusive" limit 2);
