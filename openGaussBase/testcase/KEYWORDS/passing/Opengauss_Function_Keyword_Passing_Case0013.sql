--  @testpoint:openGauss关键字passing(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "passing" from "passing" where "passing"='hello'
union all (select "passing" from "passing" where "passing"!='hello' order by "passing" limit 2);
