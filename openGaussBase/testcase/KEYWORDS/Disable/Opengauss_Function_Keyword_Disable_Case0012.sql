--  @testpoint:openGauss关键字disable(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,disable列的值按字母大小排序且只显示前2条数据
...
select "disable" from "disable" where "disable"!='hello' order by "disable" limit 2 ;
