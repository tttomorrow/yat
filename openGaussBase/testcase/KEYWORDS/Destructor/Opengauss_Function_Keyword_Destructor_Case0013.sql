--  @testpoint:openGauss关键字destructor(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "destructor" from "destructor" where "destructor"='hello'
union all (select "destructor" from "destructor" where "destructor"!='hello' order by "destructor" limit 2);
