--  @testpoint:openGauss关键字rebuild(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "rebuild" from "rebuild" where "rebuild"='hello'
union all (select "rebuild" from "rebuild" where "rebuild"!='hello' order by "rebuild" limit 2);
