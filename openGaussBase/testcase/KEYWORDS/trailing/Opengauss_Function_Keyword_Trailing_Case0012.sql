--  @testpoint:openGauss保留关键字trailing同时作为表名和列名带引号,并使用该列结合limit排序,trailing列的值按字母大小排序且只显示前2条数据
...
select "trailing" from "trailing" where "trailing"!='hello' order by "trailing" limit 2 ;
