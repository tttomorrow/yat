--  @testpoint:openGauss关键字returned_sqlstate(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "returned_sqlstate" from "returned_sqlstate" where "returned_sqlstate"='hello'
union all (select "returned_sqlstate" from "returned_sqlstate" where "returned_sqlstate"!='hello' order by "returned_sqlstate" limit 2);
