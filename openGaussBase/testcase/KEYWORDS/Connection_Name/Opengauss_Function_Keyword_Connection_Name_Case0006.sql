--  @testpoint:openGauss关键字connection_name(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table connection_name_test(
	...
	"connection_name" varchar(20) default 'gauss'
) 
...
select * from connection_name_test order by 1;
