--  @testpoint:openGauss关键字connection_name(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "connection_name" from "connection_name" where "connection_name"='hello'
union all (select "connection_name" from "connection_name" where "connection_name"!='hello' order by "connection_name" limit 2);
