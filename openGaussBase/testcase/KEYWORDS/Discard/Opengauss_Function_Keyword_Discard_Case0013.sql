--  @testpoint:openGauss关键字discard(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "discard" from "discard" where "discard"='hello'
union all (select "discard" from "discard" where "discard"!='hello' order by "discard" limit 2);
