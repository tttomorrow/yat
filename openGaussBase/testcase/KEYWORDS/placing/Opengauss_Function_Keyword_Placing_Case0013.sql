--  @testpoint:openGauss保留关键字placing同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "placing" from "placing" where "placing"='hello'
union all (select "placing" from "placing" where "placing"!='hello' order by "placing" limit 2);
