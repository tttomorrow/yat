--  @testpoint:openGauss关键字character_length(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "character_length" from "character_length" where "character_length"='hello'
union all (select "character_length" from "character_length" where "character_length"!='hello' order by "character_length" limit 2);
