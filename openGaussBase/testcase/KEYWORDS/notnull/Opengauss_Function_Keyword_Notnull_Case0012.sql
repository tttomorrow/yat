--  @testpoint:openGauss保留关键字notnull同时作为表名和列名带引号,并使用该列结合limit排序,notnull列的值按字母大小排序且只显示前2条数据
...
--查看表数据
select "notnull" from "notnull" where "notnull"!='hello' order by "notnull" limit 2 ;
