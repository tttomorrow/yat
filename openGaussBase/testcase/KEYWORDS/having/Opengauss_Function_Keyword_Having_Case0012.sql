--  @testpoint:openGauss保留关键字having同时作为表名和列名带引号,并使用该列结合limit排序,having列的值按字母大小排序且只显示前2条数据
...
--查看表数据
select "having" from "having" where "having"!='hello' order by "having" limit 2 ;
