--  @testpoint:openGauss关键字blob(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "blob" from "blob" where "blob"='hello'
union all (select "blob" from "blob" where "blob"!='hello' order by "blob" limit 2);
