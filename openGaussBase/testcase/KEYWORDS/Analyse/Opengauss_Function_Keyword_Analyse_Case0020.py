...
"""
'''
#--  @testpoint:opengauss关键字Analyse(保留)，作为目录对象名（默认只允许初始化用户创建）
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------ Opengauss_Function_Keyword_Analyse_Case0020 开始执行--------------------------")
    ...
    def test_Analyse_1(self):
        ...
        SqlMdg = commonsh.execut_db_sql('''create directory Analyse as '/tmp/';
                                                drop directory Analyse;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'Analyse';''')
        logger.info(SqlMdg)
        self.assertIn(constant.SYNTAX_ERROR_MSG, SqlMdg)
        SqlMdg = commonsh.execut_db_sql(''' create directory 'Analyse' as '/tmp/';''')
        logger.info(SqlMdg)
        ...
        def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Analyse_Case0020 执行结束--------------------------')
