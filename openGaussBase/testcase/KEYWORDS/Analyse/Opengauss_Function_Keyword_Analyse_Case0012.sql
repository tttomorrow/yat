--  @testpoint:openGauss保留关键字Analyse同时作为表名和列名带引号,并使用该列结合limit排序,Analyse列的值按字母大小排序且只显示前2条数据
...
--查看表数据
select "Analyse" from "Analyse" where "Analyse"!='hello' order by "Analyse" limit 2 ;
