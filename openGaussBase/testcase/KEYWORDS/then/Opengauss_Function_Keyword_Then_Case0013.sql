--  @testpoint:openGauss保留关键字then同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "then" from "then" where "then"='hello'
union all (select "then" from "then" where "then"!='hello' order by "then" limit 2);
