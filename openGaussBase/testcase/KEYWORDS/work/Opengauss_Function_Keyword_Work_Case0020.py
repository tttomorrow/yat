...
'''
#--  @date:2020/9/17
#--  @testpoint:opengauss关键字work(非保留)，作为目录对象名
'''
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Work_Case0020 开始执行--------------------------")

    # 关键字作为目录对象名不带双引号 - 成功
    def test_work_1(self):
        SqlMdg = commonsh.execut_db_sql('''create directory work as '/tmp/';
                                        drop directory work;''')
        logger.info(SqlMdg)
    ...
    def test_work_3(self):
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'work';''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Work_Case0020 执行结束--------------------------')