--  @testpoint:openGauss保留关键字Any同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "Any" from "Any" where "Any"='hello'
union all (select "Any" from "Any" where "Any"!='hello' order by "Any" limit 2);
