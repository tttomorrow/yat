--  @testpoint:openGauss关键字overriding(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "overriding" from "overriding" where "overriding"='hello'
union all (select "overriding" from "overriding" where "overriding"!='hello' order by "overriding" limit 2);
