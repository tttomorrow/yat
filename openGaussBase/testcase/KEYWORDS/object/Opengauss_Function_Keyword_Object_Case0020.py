...
'''
#--  @date:2020/9/17
#--  @testpoint:opengauss关键字object(非保留)，作为目录对象名
'''
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Object_Case0020 开始执行--------------------------")

    # 关键字作为目录对象名不带双引号 - 成功
    def test_object_1(self):
        SqlMdg = commonsh.execut_db_sql('''create directory object as '/tmp/';
    ...
    def test_object_3(self):
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'object';''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Object_Case0020 执行结束--------------------------')