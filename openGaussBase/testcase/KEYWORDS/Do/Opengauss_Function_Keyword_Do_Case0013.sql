--  @testpoint:openGauss保留关键字do同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "do" from "do" where "do"='hello'
union all (select "do" from "do" where "do"!='hello' order by "do" limit 2);
