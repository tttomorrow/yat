--  @testpoint:openGauss关键字returned_length(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,returned_length列的值按字母大小排序且只显示前2条数据
...
select "returned_length" from "returned_length" where "returned_length"!='hello' order by "returned_length" limit 2 ;
