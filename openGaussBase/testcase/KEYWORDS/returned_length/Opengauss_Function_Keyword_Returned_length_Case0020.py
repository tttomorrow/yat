...
"""
Case Type   : 关键字
Case Name   : opengauss关键字returned_length(非保留)，作为目录对象名
Description :
    1.关键字作为目录对象名不带引号-成功
    2.关键字作为目录对象名带双引号-成功
...
class Keyword(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Keyword_returned_length_'
                      'Case0020开始执行--')
    ...
    def test_keyword(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''create directory \
            returned_length as '/tmp/';drop directory returned_length;''')
        ...
        sql_cmd = self.commonsh.execut_db_sql('''create directory \
            "returned_length" as '/tmp/';\
            drop directory "returned_length";''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.CREATE_DIRECTORY_SUCCESS_MSG, sql_cmd)
        self.assertIn(self.constant.DROP_DIRECTORY_SUCCESS_MSG, sql_cmd)
        self.log.info(sql_cmd)
        ...
        sql_cmd = self.commonsh.execut_db_sql('drop directory '
                                             'if exists \'returned_length\';')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
        sql_cmd = self.commonsh.execut_db_sql(
            ''' create directory 'returned_length' as '/tmp/';''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
