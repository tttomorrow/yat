--  @testpoint:openGauss保留关键字inner同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "inner" from "inner" where "inner"='hello'
union all (select "inner" from "inner" where "inner"!='hello' order by "inner" limit 2);
