...
"""
'''
#--  @testpoint:opengauss关键字Case(保留)，作为目录对象名（默认只允许初始化用户创建）
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------ Opengauss_Function_Keyword_Case_Case0020 开始执行--------------------------")
    ...
    def test_Case_1(self):
        ...
        SqlMdg = commonsh.execut_db_sql('''create directory Case as '/tmp/';
                                                drop directory Case;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'Case';''')
        logger.info(SqlMdg)
        self.assertIn(constant.SYNTAX_ERROR_MSG, SqlMdg)
        SqlMdg = commonsh.execut_db_sql(''' create directory 'Case' as '/tmp/';''')
        logger.info(SqlMdg)
        ...
        def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Case_Case0020 执行结束--------------------------')
