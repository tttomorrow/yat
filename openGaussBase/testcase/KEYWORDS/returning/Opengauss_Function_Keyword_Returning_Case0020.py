...
"""
Case Type   : 关键字
Case Name   : opengauss关键字returning(保留)，作为目录对象名
Description :
...
class Keyword(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keyword_returning_Case0020开始-')
    ...
    def test_keyword(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''create directory \
            returning as '/tmp/';drop directory returning;''')
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            ''' create directory 'returning' as '/tmp/';''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
        ...
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
        sql_cmd = self.commonsh.execut_db_sql('''create directory `returning`\
            as ' / tmp / ';''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)

    def tearDown(self):
        self.log.info('-Opengauss_Function_Keyword_returning_Case0020执行-')
