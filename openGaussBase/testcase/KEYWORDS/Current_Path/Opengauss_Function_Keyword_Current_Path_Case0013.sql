--  @testpoint:openGauss关键字current_path(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "current_path" from "current_path" where "current_path"='hello'
union all (select "current_path" from "current_path" where "current_path"!='hello' order by "current_path" limit 2);
