...
'''
#--  @date:2020/9/17
#--  @testpoint:opengauss关键字role(非保留)，作为目录对象名
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Role_Case0020 开始执行--------------------------")
    def test_role_1(self):
        logger.info("------------------------关键字作为目录对象名不带双引号 - 成功--------------------------")
        SqlMdg = commonsh.execut_db_sql('''create directory role as '/tmp/';
                                            drop directory role;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'role';''')
        logger.info(SqlMdg)
        ...
        self.assertIn(constant.SYNTAX_ERROR_MSG, SqlMdg)
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Role_Case0020 执行结束--------------------------')
