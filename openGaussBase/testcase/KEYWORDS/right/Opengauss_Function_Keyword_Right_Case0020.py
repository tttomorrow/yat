...
"""
Case Type   : 关键字
Case Name   : opengauss关键字right(保留)，作为目录对象名（默认只允许初始化用户创建）
Description :
...
class Keyword(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Keyword_Right_Case0020 开始执行---')
    ...
    def test_keyword(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''create directory \
            right as '/tmp/';drop directory right;''')
        ...
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
        sql_cmd = self.commonsh.execut_db_sql('''create directory `right`\
            as ' / tmp / ';''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)

    def tearDown(self):
        self.log.info('---Opengauss_Function_Keyword_Right_Case0020 执行结束---')
