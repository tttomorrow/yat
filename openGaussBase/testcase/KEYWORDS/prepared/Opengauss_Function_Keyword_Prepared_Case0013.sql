--  @testpoint:openGauss关键字prepared(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "prepared" from "prepared" where "prepared"='hello'
union all (select "prepared" from "prepared" where "prepared"!='hello' order by "prepared" limit 2);
