--  @testpoint:openGauss保留关键字cross同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "cross" from "cross" where "cross"='hello'
union all (select "cross" from "cross" where "cross"!='hello' order by "cross" limit 2);
