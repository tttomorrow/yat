--  @testpoint:openGauss保留关键字is同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "is" from "is" where "is"='hello'
union all (select "is" from "is" where "is"!='hello' order by "is" limit 2);
