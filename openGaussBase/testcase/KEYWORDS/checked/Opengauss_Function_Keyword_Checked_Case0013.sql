--  @testpoint:openGauss关键字checked(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "checked" from "checked" where "checked"='hello'
union all (select "checked" from "checked" where "checked"!='hello' order by "checked" limit 2);
