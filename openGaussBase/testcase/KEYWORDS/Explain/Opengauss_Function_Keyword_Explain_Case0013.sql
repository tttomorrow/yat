--  @testpoint:openGauss关键字explain(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "explain" from "explain" where "explain"='hello'
union all (select "explain" from "explain" where "explain"!='hello' order by "explain" limit 2);
