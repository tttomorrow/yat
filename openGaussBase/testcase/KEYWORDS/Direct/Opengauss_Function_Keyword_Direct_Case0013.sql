--  @testpoint:openGauss关键字direct(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "direct" from "direct" where "direct"='hello'
union all (select "direct" from "direct" where "direct"!='hello' order by "direct" limit 2);
