--  @testpoint:openGauss保留关键字current_role同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "current_role" from "current_role" where "current_role"='hello'
union all (select "current_role" from "current_role" where "current_role"!='hello' order by "current_role" limit 2);
