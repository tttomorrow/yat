--  @testpoint:openGauss关键字returned_octet_length(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table returned_octet_length_test(
	...
	"returned_octet_length" varchar(20) default 'gauss'
) 
...
select * from returned_octet_length_test order by 1;
