--  @testpoint:openGauss关键字returned_octet_length(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "returned_octet_length" from "returned_octet_length" where "returned_octet_length"='hello'
union all (select "returned_octet_length" from "returned_octet_length" where "returned_octet_length"!='hello' order by "returned_octet_length" limit 2);
