...
'''
#--  @date:2020/9/28
#--  @testpoint:opengauss关键字Cast(保留)，作为目录对象名（默认只允许初始化用户创建）
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------ Opengauss_Function_Keyword_Cast_Case0020 开始执行--------------------------")
    ...
    def test_Cast_1(self):
        ...
        SqlMdg = commonsh.execut_db_sql('''create directory Cast as '/tmp/';
                                                drop directory Cast;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'Cast';''')
        logger.info(SqlMdg)
        self.assertIn(constant.SYNTAX_ERROR_MSG, SqlMdg)
        SqlMdg = commonsh.execut_db_sql(''' create directory 'Cast' as '/tmp/';''')
        logger.info(SqlMdg)
        ...
        def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Cast_Case0020 执行结束--------------------------')
