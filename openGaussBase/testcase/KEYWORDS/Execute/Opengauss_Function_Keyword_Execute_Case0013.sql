--  @testpoint:openGauss关键字execute(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "execute" from "execute" where "execute"='hello'
union all (select "execute" from "execute" where "execute"!='hello' order by "execute" limit 2);
