--  @testpoint:openGauss保留关键字current_schema同时作为表名和列名带引号,并使用该列结合limit排序,current_schema列的值按字母大小排序且只显示前2条数据
...
--查看表数据
select "current_schema" from "current_schema" where "current_schema"!='hello' order by "current_schema" limit 2 ;
