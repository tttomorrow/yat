...
'''
#--  @date:2020/9/17
#--  @testpoint:opengauss关键字database(非保留)，作为目录对象名
'''
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Database_Case0020 开始执行--------------------------")

    # 关键字作为目录对象名不带双引号 - 成功
    def test_database_1(self):
        SqlMdg = commonsh.execut_db_sql('''create directory database as '/tmp/';
    ...
    def test_database_3(self):
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'database';''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Database_Case0020 执行结束--------------------------')