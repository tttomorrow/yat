--  @testpoint:openGauss保留关键字not同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "not" from "not" where "not"='hello'
union all (select "not" from "not" where "not"!='hello' order by "not" limit 2);
