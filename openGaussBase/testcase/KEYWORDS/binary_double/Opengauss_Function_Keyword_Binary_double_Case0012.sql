--  @testpoint:openGauss关键字binary_double(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,binary_double列的值按字母大小排序且只显示前2条数据
--创建表
...
--查看表内容
select "binary_double" from "binary_double" where "binary_double"!='hello' order by "binary_double" limit 2 ;
