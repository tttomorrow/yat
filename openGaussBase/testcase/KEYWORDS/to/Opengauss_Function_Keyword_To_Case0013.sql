--  @testpoint:openGauss保留关键字to同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "to" from "to" where "to"='hello'
union all (select "to" from "to" where "to"!='hello' order by "to" limit 2);
