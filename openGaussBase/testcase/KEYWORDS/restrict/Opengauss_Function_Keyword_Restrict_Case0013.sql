--  @testpoint:openGauss关键字restrict(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "restrict" from "restrict" where "restrict"='hello'
union all (select "restrict" from "restrict" where "restrict"!='hello' order by "restrict" limit 2);
