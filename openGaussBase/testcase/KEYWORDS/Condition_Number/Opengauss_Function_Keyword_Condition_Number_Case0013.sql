--  @testpoint:openGauss关键字condition_number(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "condition_number" from "condition_number" where "condition_number"='hello'
union all (select "condition_number" from "condition_number" where "condition_number"!='hello' order by "condition_number" limit 2);
