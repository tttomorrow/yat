--  @testpoint:openGauss关键字condition_number(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table condition_number_test(
	...
	"condition_number" varchar(20) default 'gauss'
) 
...
select * from condition_number_test order by 1;
