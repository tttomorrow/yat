--  @testpoint:openGauss保留关键字when同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "when" from "when" where "when"='hello'
union all (select "when" from "when" where "when"!='hello' order by "when" limit 2);
