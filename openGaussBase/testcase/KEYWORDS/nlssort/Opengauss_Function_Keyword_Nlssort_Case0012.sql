--  @testpoint:openGauss保留关键字nlssort同时作为表名和列名带引号,并使用该列结合limit排序,nlssort列的值按字母大小排序且只显示前2条数据
...
--查看表数据
select "nlssort" from "nlssort" where "nlssort"!='hello' order by "nlssort" limit 2 ;
