--  @testpoint:openGauss关键字reassign(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "reassign" from "reassign" where "reassign"='hello'
union all (select "reassign" from "reassign" where "reassign"!='hello' order by "reassign" limit 2);
