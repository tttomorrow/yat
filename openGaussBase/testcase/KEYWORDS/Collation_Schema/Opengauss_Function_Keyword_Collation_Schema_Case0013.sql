--  @testpoint:openGauss关键字collation_schema(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "collation_schema" from "collation_schema" where "collation_schema"='hello'
union all (select "collation_schema" from "collation_schema" where "collation_schema"!='hello' order by "collation_schema" limit 2);
