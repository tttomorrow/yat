--  @testpoint:openGauss关键字collation_schema(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table collation_schema_test(
	...
	"collation_schema" varchar(20) default 'gauss'
)
...
select * from collation_schema_test order by 1;
