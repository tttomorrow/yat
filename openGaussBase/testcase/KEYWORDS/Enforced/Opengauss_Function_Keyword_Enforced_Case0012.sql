--  @testpoint:openGauss关键字enforced(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,enforced列的值按字母大小排序且只显示前2条数据
...
select "enforced" from "enforced" where "enforced"!='hello' order by "enforced" limit 2 ;
