--  @testpoint:openGauss关键字delimiters(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "delimiters" from "delimiters" where "delimiters"='hello'
union all (select "delimiters" from "delimiters" where "delimiters"!='hello' order by "delimiters" limit 2);
