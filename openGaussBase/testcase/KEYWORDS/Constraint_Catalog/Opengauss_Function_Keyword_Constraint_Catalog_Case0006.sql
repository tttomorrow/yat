--  @testpoint:openGauss关键字constraint_catalog(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table constraint_catalog_test(
	...
	"constraint_catalog" varchar(20) default 'gauss'
) 
...
select * from constraint_catalog_test order by 1;
