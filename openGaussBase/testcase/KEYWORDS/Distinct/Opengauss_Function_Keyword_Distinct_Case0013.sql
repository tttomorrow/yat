--  @testpoint:openGauss保留关键字distinct同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "distinct" from "distinct" where "distinct"='hello'
union all (select "distinct" from "distinct" where "distinct"!='hello' order by "distinct" limit 2);
