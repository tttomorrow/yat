--  @testpoint:openGauss关键字definer(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "definer" from "definer" where "definer"='hello'
union all (select "definer" from "definer" where "definer"!='hello' order by "definer" limit 2);
