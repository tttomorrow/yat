--  @testpoint:openGauss关键字Diagnostics(非保留),作为列名不带双引号，Diagnostics大小写混合，建表成功
...
select * from Diagnostics_test;
drop table Diagnostics_test;

--openGauss关键字diagnostics(非保留),作为列名不带双引号，diagnostics大小匹配，建表成功
