--  @testpoint:openGauss保留关键字Analyze同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "Analyze" from "Analyze" where "Analyze"='hello'
union all (select "Analyze" from "Analyze" where "Analyze"!='hello' order by "Analyze" limit 2);
