--  @testpoint:openGauss保留关键字deferrable同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "deferrable" from "deferrable" where "deferrable"='hello'
union all (select "deferrable" from "deferrable" where "deferrable"!='hello' order by "deferrable" limit 2);
