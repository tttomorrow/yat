--  @testpoint:openGauss保留关键字except同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "except" from "except" where "except"='hello'
union all (select "except" from "except" where "except"!='hello' order by "except" limit 2);
