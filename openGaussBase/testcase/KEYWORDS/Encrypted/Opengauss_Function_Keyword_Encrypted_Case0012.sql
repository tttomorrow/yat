--  @testpoint:openGauss关键字encrypted(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,encrypted列的值按字母大小排序且只显示前2条数据
...
select "encrypted" from "encrypted" where "encrypted"!='hello' order by "encrypted" limit 2 ;
