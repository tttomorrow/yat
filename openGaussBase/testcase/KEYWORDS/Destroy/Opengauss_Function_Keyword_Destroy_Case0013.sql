--  @testpoint:openGauss关键字destroy(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "destroy" from "destroy" where "destroy"='hello'
union all (select "destroy" from "destroy" where "destroy"!='hello' order by "destroy" limit 2);
