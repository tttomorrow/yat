--  @testpoint:openGauss关键字deferred(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "deferred" from "deferred" where "deferred"='hello'
union all (select "deferred" from "deferred" where "deferred"!='hello' order by "deferred" limit 2);
