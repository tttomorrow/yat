--  @testpoint:openGauss关键字exists(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "exists" from "exists" where "exists"='hello'
union all (select "exists" from "exists" where "exists"!='hello' order by "exists" limit 2);
