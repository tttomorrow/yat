--  @testpoint:openGauss关键字drop(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "drop" from "drop" where "drop"='hello'
union all (select "drop" from "drop" where "drop"!='hello' order by "drop" limit 2);
