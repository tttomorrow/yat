...
'''
#--  @date:2020/9/28
#--  @testpoint:opengauss关键字catalog(非保留)，作为目录对象名
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Catalog_Case0020 开始执行--------------------------")
		
    def test_catalog_1(self):
        logger.info("------------------------关键字作为目录对象名不带双引号 - 成功--------------------------")
        SqlMdg = commonsh.execut_db_sql('''create directory catalog as '/tmp/';
                                                drop directory catalog;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'catalog';''')
        logger.info(SqlMdg)
        ...
        def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Catalog_Case0020 执行结束--------------------------')
