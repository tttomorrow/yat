--  @testpoint:openGauss关键字parameter_ordinal_position(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "parameter_ordinal_position" from "parameter_ordinal_position" where "parameter_ordinal_position"='hello'
union all (select "parameter_ordinal_position" from "parameter_ordinal_position" where "parameter_ordinal_position"!='hello' order by "parameter_ordinal_position" limit 2);
