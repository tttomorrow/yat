...
"""
Case Type   : 关键字
Case Name   : opengauss关键字parameter_ordinal_position(非保留)，作为目录对象名
Description :
    1.关键字作为目录对象名不带引号-成功
    2.关键字作为目录对象名带双引号-成功
...
class Keyword(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Keyword_parameter_ordinal_'
                      'position_Case0020 开始执行---')
    ...
    def test_keyword(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''create directory \
            parameter_ordinal_position as '/tmp/';\
            drop directory parameter_ordinal_position;''')
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            '''create directory 'parameter_ordinal_position' as '/tmp/';''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
