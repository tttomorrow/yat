--  @testpoint:openGauss关键字characteristics(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
--创建表
...
create table characteristics_test(
	...
	"characteristics" varchar(20) default 'gauss'
) 
...
--查询表内容
select * from characteristics_test order by 1;
