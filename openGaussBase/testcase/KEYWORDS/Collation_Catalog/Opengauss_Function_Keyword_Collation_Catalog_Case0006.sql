--  @testpoint:openGauss关键字Collation_Catalog(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table Collation_Catalog_test(
	...
	"collation_catalog" varchar(20) default 'gauss'
) 
...
select * from Collation_Catalog_test order by 1;
