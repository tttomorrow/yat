--  @testpoint:openGauss关键字collation_catalog(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "collation_catalog" from "collation_catalog" where "collation_catalog"='hello'
union all (select "collation_catalog" from "collation_catalog" where "collation_catalog"!='hello' order by "collation_catalog" limit 2);
