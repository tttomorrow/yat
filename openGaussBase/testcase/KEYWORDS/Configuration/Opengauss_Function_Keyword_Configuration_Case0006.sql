--  @testpoint:openGauss关键字configuration(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table configuration_test(
	...
	"configuration" varchar(20) default 'gauss'
) 
...
select * from configuration_test order by 1;
