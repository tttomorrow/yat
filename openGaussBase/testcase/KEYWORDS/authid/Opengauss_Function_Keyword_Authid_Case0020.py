...
'''
#--  @date:2020/9/28
#--  @testpoint:opengauss关键字Authid(保留)，作为目录对象名（默认只允许初始化用户创建）
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------ Opengauss_Function_Keyword_Authid_Case0020 开始执行--------------------------")
    ...
    def test_Authid_1(self):
        logger.info("------------------------关键字作为目录对象名不带引号-合理报错--------------------------")
        SqlMdg = commonsh.execut_db_sql('''create directory Authid as '/tmp/';
                                                drop directory Authid;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'Authid';''')
        logger.info(SqlMdg)
        self.assertIn(constant.SYNTAX_ERROR_MSG, SqlMdg)
        SqlMdg = commonsh.execut_db_sql(''' create directory 'Authid' as '/tmp/';''')
        logger.info(SqlMdg)
        ...
        def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Authid_Case0020 执行结束--------------------------')
