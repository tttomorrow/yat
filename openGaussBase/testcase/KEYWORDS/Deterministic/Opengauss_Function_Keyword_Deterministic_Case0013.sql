--  @testpoint:openGauss关键字deterministic(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "deterministic" from "deterministic" where "deterministic"='hello'
union all (select "deterministic" from "deterministic" where "deterministic"!='hello' order by "deterministic" limit 2);
