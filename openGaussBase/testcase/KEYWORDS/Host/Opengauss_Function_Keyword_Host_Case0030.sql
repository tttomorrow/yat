-- @testpoint: opengauss关键字Host(非保留)，作为触发器名，部分测试点合理报错
...
CREATE TRIGGER Host BEFORE INSERT ON test_trigger_src_tbl FOR EACH ROW EXECUTE PROCEDURE tri_insert_func();
/
DROP TRIGGER Host ON test_trigger_src_tbl CASCADE;
