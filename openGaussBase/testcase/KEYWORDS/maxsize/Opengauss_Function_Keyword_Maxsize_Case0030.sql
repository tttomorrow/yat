-- @testpoint: opengauss关键字maxsize(非保留)，作为触发器名,部分测试点合理报错
...
create trigger maxsize before insert on test_trigger_src_tbl for each row execute procedure tri_insert_func();
/
drop trigger maxsize on test_trigger_src_tbl cascade;
