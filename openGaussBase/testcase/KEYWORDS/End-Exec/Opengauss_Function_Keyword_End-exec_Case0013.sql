--  @testpoint:openGauss关键字end-exec(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "end-exec" from "end-exec" where "end-exec"='hello'
union all (select "end-exec" from "end-exec" where "end-exec"!='hello' order by "end-exec" limit 2);
