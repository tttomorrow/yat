-- @testpoint: opengauss关键字second(非保留)，作为触发器名，部分测试点合理报错
...
create trigger second before insert on test_trigger_src_tbl for each row execute procedure tri_insert_func();
/
drop trigger second on test_trigger_src_tbl cascade;
