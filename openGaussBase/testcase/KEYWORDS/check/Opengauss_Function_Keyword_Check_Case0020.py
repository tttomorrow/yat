...
"""
'''
#--  @testpoint:opengauss关键字Check(保留)，作为目录对象名（默认只允许初始化用户创建）
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------ Opengauss_Function_Keyword_Check_Case0020 开始执行--------------------------")
    ...
    def test_Check_1(self):
        ...
        SqlMdg = commonsh.execut_db_sql('''create directory Check as '/tmp/';
                                                drop directory Check;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'Check';''')
        logger.info(SqlMdg)
        self.assertIn(constant.SYNTAX_ERROR_MSG, SqlMdg)
        SqlMdg = commonsh.execut_db_sql(''' create directory 'Check' as '/tmp/';''')
        logger.info(SqlMdg)
        ...
        def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Check_Case0020 执行结束--------------------------')
