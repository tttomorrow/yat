...
"""
Case Type   : 关键字
Case Name   : opengauss关键字primary(保留)，作为目录对象名
Description :
...
class Keyword(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keyword_primary_Case0020开始-')
    ...
    def test_keyword(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''create directory \
            primary as '/tmp/';drop directory primary;''')
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            ''' create directory 'primary' as '/tmp/';''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
    ...
    def tearDown(self):
        self.log.info('-Opengauss_Function_Keyword_primary_Case0020结束-')
