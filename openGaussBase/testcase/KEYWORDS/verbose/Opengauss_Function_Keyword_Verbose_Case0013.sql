-- @testpoint: openGauss保留关键字verbose同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "verbose" from "verbose" where "verbose"='hello'
union all (select "verbose" from "verbose" where "verbose"!='hello' order by "verbose" limit 2);
