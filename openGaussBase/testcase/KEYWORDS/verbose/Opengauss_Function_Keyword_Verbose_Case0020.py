...
"""
Case Type   : 关键字
Case Name   : opengauss关键字verbose(保留)，作为目录对象名（默认只允许初始化用户创建）
Description :
...
class Keyword(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('--Opengauss_Function_Keyword_Verbose_Case0020开始执行--')
    ...
    def test_keyword(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''create directory \
            verbose as '/tmp/';drop directory verbose;''')
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            ''' create directory 'verbose' as '/tmp/';''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
        ...
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
        sql_cmd = self.commonsh.execut_db_sql('''create directory `verbose`\
            as ' / tmp / ';''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)

    def tearDown(self):
        self.log.info('--Opengauss_Function_Keyword_Verbose_Case0020执行结束--')
