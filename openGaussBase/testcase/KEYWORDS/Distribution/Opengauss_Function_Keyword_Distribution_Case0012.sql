--  @testpoint:openGauss关键字distribution(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,distribution列的值按字母大小排序且只显示前2条数据
...
select "distribution" from "distribution" where "distribution"!='hello' order by "distribution" limit 2 ;
