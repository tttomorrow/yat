--  @testpoint:openGauss关键字Distribution(非保留),作为列名不带双引号，Distribution大小写混合，建表成功
...
select * from Distribution_test;
drop table Distribution_test;

--openGauss关键字distribution(非保留),作为列名不带双引号，distribution大小匹配，建表成功
