--  @testpoint:openGauss关键字cardinality(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,cardinality列的值按字母大小排序且只显示前2条数据
--创建表
...
--查看表内容
select "cardinality" from "cardinality" where "cardinality"!='hello' order by "cardinality" limit 2 ;
