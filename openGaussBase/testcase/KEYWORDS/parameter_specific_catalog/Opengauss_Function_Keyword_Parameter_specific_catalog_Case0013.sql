--  @testpoint:openGauss关键字parameter_specific_catalog(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "parameter_specific_catalog" from "parameter_specific_catalog" where "parameter_specific_catalog"='hello'
union all (select "parameter_specific_catalog" from "parameter_specific_catalog" where "parameter_specific_catalog"!='hello' order by "parameter_specific_catalog" limit 2);
