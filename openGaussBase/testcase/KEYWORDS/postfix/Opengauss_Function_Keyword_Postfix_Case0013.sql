--  @testpoint:openGauss关键字postfix(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "postfix" from "postfix" where "postfix"='hello'
union all (select "postfix" from "postfix" where "postfix"!='hello' order by "postfix" limit 2);
