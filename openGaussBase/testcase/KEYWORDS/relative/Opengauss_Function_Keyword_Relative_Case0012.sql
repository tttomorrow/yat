--  @testpoint:openGauss关键字relative(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,relative列的值按字母大小排序且只显示前2条数据
...
select "relative" from "relative" where "relative"!='hello' order by "relative" limit 2 ;
