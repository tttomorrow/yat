--  @testpoint:openGauss保留关键字current_user同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "current_user" from "current_user" where "current_user"='hello'
union all (select "current_user" from "current_user" where "current_user"!='hello' order by "current_user" limit 2);
