--  @testpoint:openGauss关键字real(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "real" from "real" where "real"='hello'
union all (select "real" from "real" where "real"!='hello' order by "real" limit 2);
