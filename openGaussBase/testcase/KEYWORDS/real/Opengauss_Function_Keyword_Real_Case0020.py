...
"""
'''
#--  @testpoint:opengauss关键字real(非保留)，作为目录对象名
'''
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Real_Case0020 开始执行--------------------------")

    # 关键字作为目录对象名不带双引号 - 成功
    def test_real_1(self):
        SqlMdg = commonsh.execut_db_sql('''create directory real as '/tmp/';
                                        drop directory real;''')
        logger.info(SqlMdg)
    ...
    def test_real_3(self):
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'real';''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Real_Case0020 执行结束--------------------------')