--  @testpoint:openGauss关键字recursive(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "recursive" from "recursive" where "recursive"='hello'
union all (select "recursive" from "recursive" where "recursive"!='hello' order by "recursive" limit 2);
