--  @testpoint:openGauss关键字clean(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "clean" from "clean" where "clean"='hello'
union all (select "clean" from "clean" where "clean"!='hello' order by "clean" limit 2);
