--  @testpoint:openGauss关键字backward(非保留)，作为列名带引号并且删除时使用该列,建表成功，backward列值是'hello'的删除成功
--创建表
...
--查询表内容
select * from backward_test;

--向表中插入数据
insert into backward_test(c_id) values(2);

--删除表数据
delete from backward_test where "backward"='hello';
