--  @testpoint:openGauss关键字descriptor(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,descriptor列的值按字母大小排序且只显示前2条数据
...
select "descriptor" from "descriptor" where "descriptor"!='hello' order by "descriptor" limit 2 ;
