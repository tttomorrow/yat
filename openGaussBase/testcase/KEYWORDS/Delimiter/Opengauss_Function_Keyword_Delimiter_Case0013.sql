--  @testpoint:openGauss关键字delimiter(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "delimiter" from "delimiter" where "delimiter"='hello'
union all (select "delimiter" from "delimiter" where "delimiter"!='hello' order by "delimiter" limit 2);
