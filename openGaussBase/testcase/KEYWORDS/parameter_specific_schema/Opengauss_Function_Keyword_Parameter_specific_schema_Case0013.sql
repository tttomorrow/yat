--  @testpoint:openGauss关键字parameter_specific_schema(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "parameter_specific_schema" from "parameter_specific_schema" where "parameter_specific_schema"='hello'
union all (select "parameter_specific_schema" from "parameter_specific_schema" where "parameter_specific_schema"!='hello' order by "parameter_specific_schema" limit 2);
