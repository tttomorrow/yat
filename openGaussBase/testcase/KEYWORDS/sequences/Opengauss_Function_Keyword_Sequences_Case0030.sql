-- @testpoint: opengauss关键字sequences(非保留)，作为触发器名，部分测试点合理报错
...
create trigger sequences before insert on test_trigger_src_tbl for each row execute procedure tri_insert_func();
/
drop trigger sequences on test_trigger_src_tbl cascade;
