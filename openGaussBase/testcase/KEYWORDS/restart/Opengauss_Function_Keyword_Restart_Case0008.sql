--  @testpoint:openGauss关键字restart(非保留)，作为列名带引号并且删除时使用该列,建表成功，restart列值是'hello'的删除成功
...
insert into restart_test(c_id) values(2);
delete from restart_test where "restart"='hello';
