...
"""
'''
--  @testpoint:opengauss关键字Date非保留)，作为索引名
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Date_Case0023 开始执行--------------------------")
...
#前置条件，创建一个表
    ...
    # 关键字不带引号 - 成功
    def test_do_1(self):
        SqlMdg = commonsh.execut_db_sql(''' create index public.date on explain_test(id);
                                        drop index date;''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Date_Case0023 执行结束--------------------------')