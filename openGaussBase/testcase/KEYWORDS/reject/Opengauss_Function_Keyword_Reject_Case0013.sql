--  @testpoint:openGauss保留关键字reject同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "reject" from "reject" where "reject"='hello'
union all (select "reject" from "reject" where "reject"!='hello' order by "reject" limit 2);
