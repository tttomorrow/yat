--  @testpoint:openGauss关键字collation_name(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "collation_name" from "collation_name" where "collation_name"='hello'
union all (select "collation_name" from "collation_name" where "collation_name"!='hello' order by "collation_name" limit 2);
