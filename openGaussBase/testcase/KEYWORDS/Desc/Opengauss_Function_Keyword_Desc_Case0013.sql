--  @testpoint:openGauss保留关键字desc同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "desc" from "desc" where "desc"='hello'
union all (select "desc" from "desc" where "desc"!='hello' order by "desc" limit 2);
