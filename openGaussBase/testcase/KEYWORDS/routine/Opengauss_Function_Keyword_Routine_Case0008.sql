--  @testpoint:openGauss关键字routine(非保留)，作为列名带引号并且删除时使用该列,建表成功，routine列值是'hello'的删除成功
--创建表
...
--向表中插入数据
insert into routine_test(c_id) values(2);

--删除表数据
delete from routine_test where "routine"='hello';
