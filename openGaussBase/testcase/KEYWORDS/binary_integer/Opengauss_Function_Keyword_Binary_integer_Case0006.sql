--  @testpoint:openGauss关键字binary_integer(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
--创建表
...
create table binary_integer_test(
	...
	"binary_integer" varchar(20) default 'gauss'
) 
...
--查询表内容
select * from binary_integer_test order by 1;
