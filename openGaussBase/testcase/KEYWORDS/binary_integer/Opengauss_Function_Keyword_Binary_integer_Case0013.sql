--  @testpoint:openGauss关键字binary_integer(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "binary_integer" from "binary_integer" where "binary_integer"='hello'
union all (select "binary_integer" from "binary_integer" where "binary_integer"!='hello' order by "binary_integer" limit 2);
