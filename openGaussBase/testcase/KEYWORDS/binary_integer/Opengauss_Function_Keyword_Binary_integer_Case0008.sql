--  @testpoint:openGauss关键字binary_integer(非保留)，作为列名带引号并且删除时使用该列,建表成功，binary_integer列值是'hello'的删除成功
--创建表
...
--查询表内容
select * from binary_integer_test;

--向表中插入数据
insert into binary_integer_test(c_id) values(2);

--删除表数据
delete from binary_integer_test where "binary_integer"='hello';
