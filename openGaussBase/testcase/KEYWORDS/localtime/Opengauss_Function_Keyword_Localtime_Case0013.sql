--  @testpoint:openGauss保留关键字localtime同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "localtime" from "localtime" where "localtime"='hello'
union all (select "localtime" from "localtime" where "localtime"!='hello' order by "localtime" limit 2);
