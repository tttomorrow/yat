--  @testpoint:openGauss关键字compress(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "compress" from "compress" where "compress"='hello'
union all (select "compress" from "compress" where "compress"!='hello' order by "compress" limit 2);
