--  @testpoint:openGauss关键字datetime_interval_precision(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table datetime_interval_precision_test(
	...
	"datetime_interval_precision" varchar(20) default 'gauss'
) 
...
select * from datetime_interval_precision_test order by 1;
