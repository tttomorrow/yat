--  @testpoint:openGauss关键字datetime_interval_precision(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,datetime_interval_precision列的值按字母大小排序且只显示前2条数据
...
delete from "datetime_interval_precision";
...
select "datetime_interval_precision" from "datetime_interval_precision" where "datetime_interval_precision"!='hello' order by "datetime_interval_precision" limit 2 ;
