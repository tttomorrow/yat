--  @testpoint:openGauss关键字float(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "float" from "float" where "float"='hello'
union all (select "float" from "float" where "float"!='hello' order by "float" limit 2);
