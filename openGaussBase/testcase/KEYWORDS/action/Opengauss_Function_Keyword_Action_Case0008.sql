--  @testpoint:openGauss关键字action(非保留)，作为列名带引号并且删除时使用该列,建表成功，action列值是'hello'的删除成功
--创建表
...
--查询表内容
select * from action_test;

--向表中插入数据
insert into action_test(c_id) values(2);

--删除表数据
delete from action_test where "action"='hello';
