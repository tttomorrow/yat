-- @testpoint: opengauss关键字octet_length(非保留)，作为触发器名,部分测试点合理报错
...
CREATE TRIGGER octet_length BEFORE INSERT ON test_trigger_src_tbl FOR EACH ROW EXECUTE PROCEDURE tri_insert_func();
/
drop TRIGGER octet_length on test_trigger_src_tbl cascade;
