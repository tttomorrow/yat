--  @testpoint:openGauss关键字command_function_code(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table command_function_code_test(
	...
	"command_function_code" varchar(20) default 'gauss'
) 
...
select * from command_function_code_test order by 1;
