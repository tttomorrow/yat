--  @testpoint:openGauss关键字coalesce(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "coalesce" from "coalesce" where "coalesce"='hello'
union all (select "coalesce" from "coalesce" where "coalesce"!='hello' order by "coalesce" limit 2);
