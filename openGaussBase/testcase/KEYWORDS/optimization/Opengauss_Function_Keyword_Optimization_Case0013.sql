--  @testpoint:openGauss关键字optimization(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "optimization" from "optimization" where "optimization"='hello'
union all (select "optimization" from "optimization" where "optimization"!='hello' order by "optimization" limit 2);
