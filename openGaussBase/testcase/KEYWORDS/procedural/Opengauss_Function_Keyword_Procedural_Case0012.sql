--  @testpoint:openGauss关键字procedural(非保留)同时作为表名和列名带引号,并使用该列结合limit排序,procedural列的值按字母大小排序且只显示前2条数据
...
select "procedural" from "procedural" where "procedural"!='hello' order by "procedural" limit 2 ;
