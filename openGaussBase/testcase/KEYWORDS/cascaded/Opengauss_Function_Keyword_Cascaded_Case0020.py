...
"""
Case Type   : keyword
Case Name   : opengauss关键字cascaded(非保留)，作为目录对象名
Description :
...
class Hostname(unittest.TestCase):
    def setUp(self):
        logger.info("------------------ Opengauss_Function_Keyword_Cascaded_Case0020 开始执行-----------------------")

    def test_cascaded_1(self):
        logger.info("------------------------关键字作为目录对象名不带双引号 - 成功--------------------------")
        SqlMdg = commonsh.execut_db_sql('''create directory cascaded as '/tmp/';
                                                drop directory cascaded;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'cascaded';''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
        logger.info('------------------ 无需清理环境-------------------------')
        logger.info('-------------------- Opengauss_Function_Keyword_Cascaded_Case0020 执行结束--------------------')
