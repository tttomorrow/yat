--  @testpoint:openGauss关键字cascaded(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "cascaded" from "cascaded" where "cascaded"='hello'
union all (select "cascaded" from "cascaded" where "cascaded"!='hello' order by "cascaded" limit 2);
