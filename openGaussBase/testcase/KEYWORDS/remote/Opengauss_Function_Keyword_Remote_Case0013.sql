--  @testpoint:openGauss关键字remote(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "remote" from "remote" where "remote"='hello'
union all (select "remote" from "remote" where "remote"!='hello' order by "remote" limit 2);
