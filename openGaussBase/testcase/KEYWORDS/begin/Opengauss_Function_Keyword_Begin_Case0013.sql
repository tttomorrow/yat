--  @testpoint:openGauss关键字begin(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "begin" from "begin" where "begin"='hello'
union all (select "begin" from "begin" where "begin"!='hello' order by "begin" limit 2);
