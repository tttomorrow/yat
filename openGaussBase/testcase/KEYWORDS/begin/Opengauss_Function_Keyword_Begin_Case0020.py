...
'''
#--  @date:2020/9/28
#--  @testpoint:opengauss关键字begin(非保留)，作为目录对象名
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Begin_Case0020 开始执行--------------------------")
    def test_begin_1(self):
        logger.info("------------------------关键字作为目录对象名不带双引号 - 成功--------------------------")
        SqlMdg = commonsh.execut_db_sql('''create directory begin as '/tmp/';
                                                drop directory begin;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'begin';''')
        logger.info(SqlMdg)
        ...
        self.assertIn(constant.SYNTAX_ERROR_MSG, SqlMdg)
        def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Begin_Case0020 执行结束--------------------------')
