--  @testpoint:openGauss保留关键字localtimestamp同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "localtimestamp" from "localtimestamp" where "localtimestamp"='hello'
union all (select "localtimestamp" from "localtimestamp" where "localtimestamp"!='hello' order by "localtimestamp" limit 2);
