--  @testpoint:openGauss关键字disconnect(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "disconnect" from "disconnect" where "disconnect"='hello'
union all (select "disconnect" from "disconnect" where "disconnect"!='hello' order by "disconnect" limit 2);
