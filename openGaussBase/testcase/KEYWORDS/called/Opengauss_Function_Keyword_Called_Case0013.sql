--  @testpoint:openGauss关键字called(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "called" from "called" where "called"='hello'
union all (select "called" from "called" where "called"!='hello' order by "called" limit 2);
