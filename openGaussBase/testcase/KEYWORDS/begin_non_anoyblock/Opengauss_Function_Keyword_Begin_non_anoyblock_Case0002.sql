--  @testpoint:openGauss关键字begin_non_anoyblock(非保留,作为列名带双引号，begin_non_anoyblock大小写混合，建表成功
--创建表
...
--openGauss关键字begin_non_anoyblock(非保留,作为列名带双引号，begin_non_anoyblock大小写匹配，建表成功
drop table if exists begin_non_anoyblock_test;
create table begin_non_anoyblock_test(
	c_id int, c_int int, c_integer integer, c_bool int, c_boolean int, c_bigint integer,
	c_real real, c_double real,
