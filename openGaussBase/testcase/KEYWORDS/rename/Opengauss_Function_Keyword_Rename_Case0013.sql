--  @testpoint:openGauss关键字rename(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "rename" from "rename" where "rename"='hello'
union all (select "rename" from "rename" where "rename"!='hello' order by "rename" limit 2);
