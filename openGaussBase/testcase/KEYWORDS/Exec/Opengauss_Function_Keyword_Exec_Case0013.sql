--  @testpoint:openGauss关键字exec(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "exec" from "exec" where "exec"='hello'
union all (select "exec" from "exec" where "exec"!='hello' order by "exec" limit 2);
