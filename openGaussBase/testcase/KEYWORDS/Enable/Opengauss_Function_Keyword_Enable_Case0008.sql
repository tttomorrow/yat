--  @testpoint:openGauss关键字enable(非保留)，作为列名带引号并且删除时使用该列,建表成功，enable列值是'hello'的删除成功
...
insert into enable_test(c_id) values(2);
delete from enable_test where "enable"='hello';
