...
'''
#--  @date:2020/9/17
#--  @testpoint:opengauss关键字zone(非保留)，作为目录对象名
'''
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Zone_Case0020 开始执行--------------------------")

    # 关键字作为目录对象名不带双引号 - 成功
    def test_zone_1(self):
        SqlMdg = commonsh.execut_db_sql('''create directory zone as '/tmp/';
                                        drop directory zone;''')
        logger.info(SqlMdg)
    ...
    def test_zone_3(self):
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'zone';''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Zone_Case0020 执行结束--------------------------')