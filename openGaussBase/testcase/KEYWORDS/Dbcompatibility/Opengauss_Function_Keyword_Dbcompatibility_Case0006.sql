--  @testpoint:openGauss关键字dbcompatibility(非保留),作为列名带引号并且插入时使用该列，建表成功，给该列插入数据成功
...
create table dbcompatibility_test(
	...
	"dbcompatibility" varchar(20) default 'gauss'
) 
...
select * from dbcompatibility_test order by 1;
