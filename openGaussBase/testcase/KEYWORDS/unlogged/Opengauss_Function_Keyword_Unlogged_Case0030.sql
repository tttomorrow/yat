-- @testpoint: opengauss关键字unlogged(非保留)，作为触发器名,部分测试点合理报错
...
CREATE TRIGGER unlogged BEFORE INSERT ON test_trigger_src_tbl FOR EACH ROW EXECUTE PROCEDURE tri_insert_func();
/
drop TRIGGER unlogged on test_trigger_src_tbl cascade;
