--  @testpoint:openGauss关键字depth(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "depth" from "depth" where "depth"='hello'
union all (select "depth" from "depth" where "depth"!='hello' order by "depth" limit 2);
