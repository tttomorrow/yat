--  @testpoint:openGauss保留关键字Authorization同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "Authorization" from "Authorization" where "Authorization"='hello'
union all (select "Authorization" from "Authorization" where "Authorization"!='hello' order by "Authorization" limit 2);
