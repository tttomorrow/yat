...
"""
Case Type   : 关键字
Case Name   : opengauss关键字union(保留)，作为目录对象名
Description :
...
class Keyword(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Keyword_union_Case0020开始执行---')
    ...
    def test_keyword(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''create directory \
            union as '/tmp/';drop directory union;''')
        self.log.info(sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)
        self.assertIn(self.constant.SYNTAX_ERROR_MSG, sql_cmd)

        self.log.info("---关键字作为目录对象名带双引号-成功---")
        sql_cmd = self.commonsh.execut_db_sql('create directory \\\"union\\\" '
                                             'as \'/tmp/\';'
                                             'drop directory \\\"union\\\"; ')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        self.log.info('---Opengauss_Function_Keyword_union_Case0020执行结束---')
