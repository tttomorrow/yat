--  @testpoint:openGauss关键字describe(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "describe" from "describe" where "describe"='hello'
union all (select "describe" from "describe" where "describe"!='hello' order by "describe" limit 2);
