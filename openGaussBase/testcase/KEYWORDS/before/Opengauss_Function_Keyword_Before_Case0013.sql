--  @testpoint:openGauss关键字before(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "before" from "before" where "before"='hello'
union all (select "before" from "before" where "before"!='hello' order by "before" limit 2);
