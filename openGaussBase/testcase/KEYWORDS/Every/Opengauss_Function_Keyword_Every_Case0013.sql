--  @testpoint:openGauss关键字every(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "every" from "every" where "every"='hello'
union all (select "every" from "every" where "every"!='hello' order by "every" limit 2);
