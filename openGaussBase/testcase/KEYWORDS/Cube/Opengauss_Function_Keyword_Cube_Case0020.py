...
"""
'''
#--  @testpoint:opengauss关键字cube(非保留)，作为目录对象名
'''
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Cube_Case0020 开始执行--------------------------")

    # 关键字作为目录对象名不带双引号 - 成功
    def test_cube_1(self):
        SqlMdg = commonsh.execut_db_sql('''create directory cube as '/tmp/';
                                        drop directory cube;''')
        logger.info(SqlMdg)
    ...
    def test_cube_3(self):
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'cube';''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Cube_Case0020 执行结束--------------------------')