--  @testpoint:openGauss关键字clob(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "clob" from "clob" where "clob"='hello'
union all (select "clob" from "clob" where "clob"!='hello' order by "clob" limit 2);
