--  @testpoint:openGauss保留关键字using同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "using" from "using" where "using"='hello'
union all (select "using" from "using" where "using"!='hello' order by "using" limit 2);
