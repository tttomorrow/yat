...
"""
'''
#--  @testpoint:opengauss关键字enum(非保留)，作为目录对象名
'''
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
         logger.info("------------------------ Opengauss_Function_Keyword_Enum_Case0020 开始执行--------------------------")

    # 关键字作为目录对象名不带双引号 - 成功
    def test_enum_1(self):
        SqlMdg = commonsh.execut_db_sql('''create directory enum as '/tmp/';
                                        drop directory enum;''')
        logger.info(SqlMdg)
    ...
    def test_enum_3(self):
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'enum';''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Enum_Case0020 执行结束--------------------------')