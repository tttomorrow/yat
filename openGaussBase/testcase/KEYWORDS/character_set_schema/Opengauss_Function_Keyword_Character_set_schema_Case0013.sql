--  @testpoint:openGauss关键字character_set_schema(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
--创建表
...
select "character_set_schema" from "character_set_schema" where "character_set_schema"='hello'
union all (select "character_set_schema" from "character_set_schema" where "character_set_schema"!='hello' order by "character_set_schema" limit 2);
