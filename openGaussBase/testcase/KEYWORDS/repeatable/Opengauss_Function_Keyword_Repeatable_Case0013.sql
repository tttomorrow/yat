--  @testpoint:openGauss关键字repeatable(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "repeatable" from "repeatable" where "repeatable"='hello'
union all (select "repeatable" from "repeatable" where "repeatable"!='hello' order by "repeatable" limit 2);
