--  @testpoint:openGauss保留关键字current_time同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "current_time" from "current_time" where "current_time"='hello'
union all (select "current_time" from "current_time" where "current_time"!='hello' order by "current_time" limit 2);
