--  @testpoint:openGauss关键字preferred(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "preferred" from "preferred" where "preferred"='hello'
union all (select "preferred" from "preferred" where "preferred"!='hello' order by "preferred" limit 2);
