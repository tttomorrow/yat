...
'''
#--  @date:2020/9/28
#--  @testpoint:opengauss关键字Array(保留)，作为目录对象名（默认只允许初始化用户创建）
'''
import unittest
...
class Hostname(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------ Opengauss_Function_Keyword_Array_Case0020 开始执行--------------------------")
    ...
    def test_Array_1(self):
        ...
        SqlMdg = commonsh.execut_db_sql('''create directory Array as '/tmp/';
                                                drop directory Array;''')
        logger.info(SqlMdg)
        ...
        SqlMdg = commonsh.execut_db_sql('''drop directory if exists 'Array';''')
        logger.info(SqlMdg)
        self.assertIn(constant.SYNTAX_ERROR_MSG, SqlMdg)
        SqlMdg = commonsh.execut_db_sql(''' create directory 'Array' as '/tmp/';''')
        logger.info(SqlMdg)
        ...
        def tearDown(self):
         logger.info('------------------------ Opengauss_Function_Keyword_Array_Case0020 执行结束--------------------------')
