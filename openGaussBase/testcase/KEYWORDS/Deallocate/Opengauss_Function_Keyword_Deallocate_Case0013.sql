--  @testpoint:openGauss关键字deallocate(非保留)，同时作为表名和列名带引号，与union结合查询合并两个SELECT 语句查询
...
select "deallocate" from "deallocate" where "deallocate"='hello'
union all (select "deallocate" from "deallocate" where "deallocate"!='hello' order by "deallocate" limit 2);
