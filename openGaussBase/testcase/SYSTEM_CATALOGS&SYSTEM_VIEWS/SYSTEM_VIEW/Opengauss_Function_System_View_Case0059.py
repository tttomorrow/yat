...
"""
Case Type   : 系统视图
Case Name   : 测试系统视图PG_STATIO_SYS_INDEXES字段与数据类型
Description :
    1.查看系统视图PG_STATIO_SYS_INDEXES的结构
    2.该视图字段与对应字段数据类型是否正确
Expect      :
    1.查看系统视图PG_STATIO_SYS_INDEXES的结构成功
    2.该视图字段与字段数据类型对应正确
History     :
"""
...
class SystemView(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('------Opengauss_Function_System_View_Case0059开始执行-----')
        self.com = Common()
        ...
        self.expect_result_dict = {
            ...
            'Type': ['oid', 'oid', 'name', 'name', 'name', 'bigint', 'bigint']}

    def test_index_file_damaged(self):
        LOG.info('-------------------查看表结构--------------------')
        msg = self.comsh.execut_db_sql('\d PG_STATIO_SYS_INDEXES')
        LOG.info(msg)
    ...
    def tearDown(self):
        ...
        # 无须清理环境
        LOG.info('----Opengauss_Function_System_View_Case0059执行完成------')
