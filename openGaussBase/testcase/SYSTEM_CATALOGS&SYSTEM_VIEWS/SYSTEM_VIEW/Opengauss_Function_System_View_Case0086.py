...
class SystemView(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '--------Opengauss_Function_System_View_Case0086开始执行--------')
        self.com = Common()
        self.comsh = CommonSH('dbuser')
        self.expect_result_dict = {
            'Column': ['node_name', 'nextOid', 'nextXid', 'oldestXid',
                       'xidVacLimit', 'oldestXidDB', 'lastExtendCSNLogpage',
                       'startExtendCSNLogpage', 'nextCommitSeqNo',
                       'latestCompletedXid', 'startupMaxXid'],
    ...
    def tearDown(self):
        ...
        # 无须清理环境
        LOG.info(
            '------Opengauss_Function_System_View_Case0086执行完成--------')
