...
"""
Case Type   : 系统视图
Case Name   : 测试系统视图PG_SETTINGS字段与数据类型
Description :
    1.查看系统视图PG_SETTINGS的结构
    2.该视图字段与对应字段数据类型是否正确
Expect      :
    1.查看系统视图PG_SETTINGS的结构成功
    2.该视图字段与字段数据类型对应正确
History     :
"""
...
class IndexFileDamaged(unittest.TestCase):
    def setUp(self):
        ...
        logger.info('--------------Opengauss_Function_System_View_Case0036开始执行--------------')
        self.com = Common()
        self.comsh = CommonSH('dbuser')
        self.expect_result_dict = {
            'Column': ['name', 'setting', 'unit', 'category', 'short_desc', 'extra_desc', 'context', 'vartype',
    ...
    def test_Index_file_damaged(self):
        logger.info('----------------------------查看表结构-----------------------------')
        msg = self.comsh.execut_db_sql('\d PG_SETTINGS')
        logger.info(msg)
    ...
    def tearDown(self):
        ...
        # 无须清理环境
        logger.info('-----------------------Opengauss_Function_System_View_Case0036执行完成-----------------------------')
