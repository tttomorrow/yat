...
"""
Case Type   : 基础功能
Case Name   : refcount为0 plancache为invalid 5min中内使用plan，且为invalid
Description :
    1.开启GPC
    2.重启数据库
    3.创建表
...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0051 start")
    ...
    def test_index(self):
        ...
        self.log.info("-----------9.等待3min 创建prepare------------------")
        cmd = f"select pg_sleep(180);prepare {self.pre_name} as " \
            f"select * from {self.tb_name};" \
            f"execute {self.pre_name};" \
            f"select * from dbe_perf.global_plancache_status " \
            f"where query='prepare {self.pre_name} " \
            f"as select * from {self.tb_name};' " \
            f"and valid = 't';" \
            f"ALTER TABLE {self.tb_name} DROP PARTITION p2;"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        self.assertIn(self.constant.PREPARE_SUCCESS_MSG, result)
        self.assertIn('0 rows', result)
        ...
        self.log.info("-----------9.等待2min 查询计划------------------")
        cmd = f"select * from dbe_perf.global_plancache_status; "
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        self.assertIn('(1 row)', result)
        cmd = f"select pg_sleep(120);" \
            f"select * from dbe_perf.global_plancache_status; "
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        self.assertNotIn('as select * from', result)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_GPC_case0051 end-")
