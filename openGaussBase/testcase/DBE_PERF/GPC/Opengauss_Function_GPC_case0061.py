...
"""
Case Type   : 基础功能
Case Name   : 开启pgc，不同session重复多次执行相同pbe
Description :
    1.开启GPC
    2.重启数据库
    3.创建表
    4.创建prepare语句（重复两次，不相同session）
    5.查询计划
Expect      :
History     :
"""
...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0061 start")
    ...
    def test_index(self):
        ...
        cmd = f"prepare {self.pre_name}(int) as " \
            ...
            f"select pg_sleep(2);" \
            f"begin " \
            f"for i in 1..100 loop  " \
            f"execute 'execute {self.pre_name}(199)';" \
            f"end loop; " \
            f"end;"
        for i in range(0, 8):
            session.append(ComThread(
        ...
        for i in range(0, 8):
            ...
            self.log.info(result)
            self.assertIn('ANONYMOUS BLOCK EXECUTE', result)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_GPC_case0061 end-")
