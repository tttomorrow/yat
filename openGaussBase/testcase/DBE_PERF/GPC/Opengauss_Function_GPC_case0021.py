...
"""
Case Type   : 基础功能
Case Name   : 行存表delete limit语句session间共享
Description :
    1.开启GPC
    2.重启数据库
    3.创建表
...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0021 start")
    ...
    def test_index(self):
        ...
        cmd = f"create table {self.tb_name}(i int);" \
            f"insert into {self.tb_name} values(generate_series(1,10000));" \
            f"insert into {self.tb_name} values(generate_series(1,200));"
        ...
        sql = f"prepare {self.pre_name}(int) " \
            f"as delete from {self.tb_name}  where i<\$1 limit 100;" \
            f"execute {self.pre_name}(200);" \
        ...
        for i in range(3):
            ...
            self.log.info(result)
            self.assertTrue('10100' in result or '10000' in result
                            or '9900' in result)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_GPC_case0021 end-")
