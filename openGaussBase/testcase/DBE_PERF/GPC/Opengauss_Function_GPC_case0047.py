...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0047 start")
    ...
    def test_index(self):
        ...
        self.log.info("------开启3个session-----")
        session = []
        sql = f"prepare {self.pre_name}1 " \
            f"as select count(*) from {self.tb_name};" \
            f"execute {self.pre_name}1;"  \
            f"select pg_sleep(30);"
        for i in range(3):
            session.append(
                ComThread(commonshpri.execut_db_sql, args=(sql, '')))
        for i in range(3):
            session[i].setDaemon(True)
            session[i].start()
        ...
        time.sleep(2)
        cmd = f"select * from dbe_perf.global_plancache_status " \
            f"where query='prepare {self.pre_name}1 as select " \
            f"count(*) from {self.tb_name};' " \
            f"and refcount = 3 and valid='t';"
        result = commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        self.assertIn('(1 row)', result)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_GPC_case0047 end-")
