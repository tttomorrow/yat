...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0044 start")
    ...
    def test_index(self):
        ...
        cmd = f"create table {self.tb_name}" \
            ...
            f"(section     NUMBER(6)," \
            f"salary_sum out NUMBER(8,2)," \
            f"staffs_count out INTEGER)" \
            f"IS " \
            f"BEGIN " \
            f"SELECT sum(salary), count(*) INTO salary_sum, " \
            f"staffs_count FROM {self.tb_name} " \
            f"where section_id = section;END;"
        ...
        session = []
        sql = f"CALL {self.pro_name}(2,8,6); " \
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_GPC_case0044 end-")
