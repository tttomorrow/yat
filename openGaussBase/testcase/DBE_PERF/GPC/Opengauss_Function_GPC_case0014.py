...
"""
Case Type   : 基础功能
Case Name   : 全局临时表merge into语句session间共享
Description :
    1.开启GPC
    2.重启数据库
    3.创建表
...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0014 start")
    ...
    def test_index(self):
        ...
        self.log.info("---------------3.创建表----------------------")
        cmd = f"create global temp table {self.tb_name}(i int, j int);" \
        ...
        cmd = f"select * from dbe_perf.global_plancache_status " \
            f"where query='prepare {self.pre_name}(int, int) " \
                f"as merge into {self.tb_name} t1 using {self.tb_name1} " \
            f"t2 on (t1.i=t2.i) when matched then update set t1.j=t2.j " \
            f"when not matched then insert values(\$1, \$2);' " \
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_GPC_case0014 end-")
