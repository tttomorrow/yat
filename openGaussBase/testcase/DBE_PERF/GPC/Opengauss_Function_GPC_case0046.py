...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0046 start")
    ...
    def test_index(self):
        ...
        self.log.info("---------------3.函数----------------------")
        cmd = f"CREATE FUNCTION {self.pro_name}(integer, integer) " \
            f"RETURNS integer    AS 'select \$1 + \$2;'    " \
            f"LANGUAGE SQL    " \
            f"IMMUTABLE    " \
    ...
    def tearDown(self):
        ...
        cmd = f"drop table if exists {self.tb_name} cascade;" \
            f"drop function if exists {self.pro_name};"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        ...
        self.log.info("-Opengauss_Function_GPC_case0046 end-")
