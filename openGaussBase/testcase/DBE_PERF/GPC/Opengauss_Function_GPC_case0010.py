...
"""
Case Type   : 基础功能
Case Name   : refcount为0 plancache为invalid 4min中内不使用plan
Description :
    1.开启GPC
    2.重启数据库
    3.创建表
...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0010 start")
    ...
    def test_index(self):
        ...
        self.log.info("-----------9.等待4min 查询计划------------------")
        cmd = f"select pg_sleep(240);" \
            f"select * from dbe_perf.global_plancache_status; "
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        self.assertEqual(result.count('(1 row)'), 2)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_GPC_case0010 end-")