...
"""
Case Type   : 基础功能
Case Name   : 列分区表insert语句session间共享
Description :
    1.开启GPC
    2.重启数据库
    3.创建表
...
class Gpcclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_GPC_case0049 start")
    ...
    def test_index(self):
        ...
        sql = f"prepare {self.pre_name}(int) " \
            f"as insert into {self.tb_name} values(\$1);" \
        ...
        self.assertIn('(1 row)', result1)
        cmd = f"select * from pg_catalog.plancache_status();"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_GPC_case0049 end-")
