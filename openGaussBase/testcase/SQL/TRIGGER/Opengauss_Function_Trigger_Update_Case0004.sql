-- @testpoint: 获取update触发器的定义信息不以pretty方式展示
...
--获取update触发器的定义信息，不以pretty方式显示
SELECT pg_get_triggerdef(oid,false) FROM pg_trigger WHERE oid IN (SELECT oid FROM pg_trigger where tgname='update_trigger');
