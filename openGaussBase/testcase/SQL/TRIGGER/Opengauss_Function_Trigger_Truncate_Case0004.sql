-- @testpoint: 获取truncate触发器的定义信息不以pretty方式展示
...
--获取truncate触发器的定义信息不以pretty方式展示
SELECT pg_get_triggerdef(oid,false) FROM pg_trigger WHERE oid IN (SELECT oid FROM pg_trigger where tgname='truncate_trigger');
