-- @testpoint: AFTER--DELETE在表上支持语句级触发器
--创建表
...
--创建触发器函数
CREATE OR REPLACE FUNCTION tri_delete_func01() RETURNS TRIGGER AS
$$
DECLARE
BEGIN
...
--创建触发器
CREATE TRIGGER delete_trigger01 AFTER DELETE ON test_trigger_src_tbl0 FOR EACH STATEMENT EXECUTE PROCEDURE tri_delete_func01();
/
--执行触发事件
...
DROP FUNCTION tri_delete_func01() cascade;
