-- @testpoint: 删除触发器函数获取update触发器的定义信息
...
--删除触发器函数，获取update触发器的定义信息
DROP TRIGGER update_trigger ON test_trigger_src_tbl;
SELECT pg_get_triggerdef(oid) FROM pg_trigger WHERE oid IN (SELECT oid FROM pg_trigger where tgname='update_trigger');
