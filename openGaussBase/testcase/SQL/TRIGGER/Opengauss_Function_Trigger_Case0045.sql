-- @testpoint: AFTER--TRUNCATE在表上不支持行级触发器，合理报错
--创建表
...
--创建触发器
CREATE TRIGGER truncate_trigger01 AFTER truncate ON test_trigger_src_tbl0 FOR EACH ROW EXECUTE PROCEDURE tri_truncate_func01();
/
--执行触发事件
