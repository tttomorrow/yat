...
Description :
    ...
    4.删除触发器：DROP TRIGGER insert_trigger01 ON test_trigger_src_tbl0;
Expect      :
    1.创表成功
    2.触发器函数创建成功
    3.触发器创建成功
...
class Trigger(unittest.TestCase):
    def setUp(self):
        logger.info('------Opengauss_Function_Trigger_Case0028 start------')
        self.common = Common()
    ...
    def test_trigger(self):
        ...
        sql_cmd1 = f'CREATE USER wf WITH password \'{macro.COMMON_PASSWD}\';' \
                   ...
                   f'DECLARE ' \
                   f'BEGIN ' \
                   f'INSERT INTO test_trigger_des_tbl0 VALUES(NEW.id1, NEW.id2,' \
                   f'NEW.id3);' \
                   f'RETURN NEW;' \
                   f'END ' \
                   f'\$\$ LANGUAGE PLPGSQL;' \
                   ...
                   f'tri_insert_func01();'
        sql_cmd2 = 'DROP TRIGGER insert_trigger01 ON test_trigger_src_tbl0;'
    ...
    def tearDown(self):
        ...
        msg1 = self.sh_primy.execut_db_sql(sql_cmd1)
        logger.info(msg1)
        logger.info('----Opengauss_Function_Trigger_Case0028 finish----')
