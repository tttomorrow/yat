-- @testpoint: AFTER--DELETE在表上支持行级触发器
--创建表
...
--创建触发器
CREATE TRIGGER delete_trigger01 AFTER DELETE ON test_trigger_src_tbl0 FOR EACH ROW EXECUTE PROCEDURE tri_delete_func0101();
/
--执行触发事件
