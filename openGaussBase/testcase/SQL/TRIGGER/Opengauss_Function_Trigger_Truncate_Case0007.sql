-- @testpoint: 参数为非boolean类型,获取truncate触发器的定义信息，合理报错
...
--参数为非boolean类型，获取truncate触发器的定义信息
SELECT pg_get_triggerdef(oid,'2020-12-21') FROM pg_trigger WHERE oid IN (SELECT oid FROM pg_trigger where tgname='truncate_trigger');
