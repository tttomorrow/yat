-- @testpoint: 获取delete触发器的定义信息不以pretty方式展示
...
--查看delete触发器的定义信息
SELECT pg_get_triggerdef(oid,false) FROM pg_trigger WHERE oid IN (SELECT oid FROM pg_trigger where tgname='delete_trigger');
