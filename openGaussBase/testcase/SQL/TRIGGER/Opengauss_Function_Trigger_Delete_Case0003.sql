-- @testpoint: 删除触发器函数获取delete触发器的定义信息
...
--删除触发器函数获取delete触发器的定义信息
DROP TRIGGER delete_trigger ON test_trigger_src_tbl;
SELECT pg_get_triggerdef(oid) FROM pg_trigger WHERE oid IN (SELECT oid FROM pg_trigger where tgname='delete_trigger');
