...
"""
Case Type   : 全文检索
Case Name   : 文本搜索配置token_type（zh_words）测试（解析器为pg_catalog.pound）
Description :
    1.创建数据库编码为UTF8
    2.切换至test_db_078数据库,创建文本搜索配置;创建simple字典
...
class FullTextSearch(unittest.TestCase):
    ...
    def setUp(self):
        logger.info("------------------------Opengauss_Function_Text_Search_Configuration_Case0078开始执行--------------------------")
        self.userNode = Node('dbuser')
        self.DB_ENV_PATH = macro.DB_ENV_PATH

    def test_dictionary(self):
        logger.info('-------------创建数据库----------------')
        sql_cmd1 = commonsh.execut_db_sql('''drop database if exists test_db_078;
                                           create database test_db_078 encoding 'utf8' template = template0;''')
        ...
        excute_cmd1 = f'''
                                ...
                                gsql -d test_db_078 -p {self.userNode.db_port} -c "{sql_cmd2}"
                                    '''
        ...
        sql_cmd3 = '''alter text search configuration test_pound add mapping for zh_words with pg_dict;
                      select ts_debug('pound','中文检索');'''
        excute_cmd1 = f'''
                                  source {self.DB_ENV_PATH};
                                  gsql -d test_db_078 -p {self.userNode.db_port} -c "{sql_cmd3}"
                                  '''
    ...
    def tearDown(self):
        ...
        sql_cmd4 = commonsh.execut_db_sql('''drop database test_db_078;''')
        logger.info(sql_cmd4)
        logger.info('------------------------Opengauss_Function_Text_Search_Configuration_Case0078执行结束--------------------------')
