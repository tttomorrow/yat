...
--增加文本搜索配置字串类型映射，token为numword
ALTER TEXT SEARCH CONFIGURATION english2 ADD MAPPING FOR numword WITH pg_dict;
--使用文本检索函数对所创建的词典配置english2进行测试
SELECT ts_debug('english','beta1');
SELECT ts_debug('beta1');
--删除文本搜索配置
