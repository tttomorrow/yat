...
--增加文本搜索配置字串类型映射，token为asciihword
ALTER TEXT SEARCH CONFIGURATION english2 ADD MAPPING FOR asciihword WITH pg_dict;
--使用文本检索函数对所创建的词典配置english2进行测试
SELECT ts_debug('english','up-to-date');
SELECT ts_debug('up-to-date');
--删除文本搜索配置
