...
"""
Case Type   :全文检索--解析器
Case Name   : 文本搜索配置token_type为multisymbol（解析器为pg_catalog.ngram）不支持的编码，合理报错
Description :
...
class Parser(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Text_Search_Configuration_Case0071开始执行-----------------------------')
    ...
    def test_ngram_parser(self):
       ...
       logger.info('--------------------增加文本搜索配置字串类型映射，token为multisymbol ---------')
       sql_cmd2 = commonsh.execut_db_sql('''ALTER TEXT SEARCH CONFIGURATION english2 ADD MAPPING FOR multisymbol WITH pg_dict;''')
       logger.info(sql_cmd2)
       self.assertIn(self.Constant.ALTER_TEXT_SEARCH_CONFIGURATION, sql_cmd2)
       logger.info('-----------使用文本检索函数对所创建的词典配置english2进行测试---------')
       sql_cmd3 = commonsh.execut_db_sql('''SELECT ts_debug('ngram','8+4-7*8/2');''')
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_Text_Search_Configuration_Case0071执行结束--------------------------')
