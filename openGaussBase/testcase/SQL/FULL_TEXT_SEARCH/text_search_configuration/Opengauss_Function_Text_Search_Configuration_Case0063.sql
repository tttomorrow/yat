...
--增加文本搜索配置字串类型映射，token为tag
ALTER TEXT SEARCH CONFIGURATION english2 ADD MAPPING FOR tag WITH pg_dict;
--使用文本检索函数对所创建的词典配置english2进行测试
SELECT ts_debug('english','<a href="dictionaries.html">');
SELECT ts_debug('<a href="dictionaries.html">');
--删除文本搜索配置
