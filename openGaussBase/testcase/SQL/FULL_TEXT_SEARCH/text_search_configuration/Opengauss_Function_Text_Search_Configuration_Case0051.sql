...
--增加文本搜索配置字串类型映射，token为hword_numpart
ALTER TEXT SEARCH CONFIGURATION english2 ADD MAPPING FOR hword_numpart WITH pg_dict;
--使用文本检索函数对所创建的词典配置english2进行测试
SELECT ts_debug('english','beta1 in the context postgresql-beta1');
SELECT ts_debug('beta1 in the context postgresql-beta1');
--删除文本搜索配置
