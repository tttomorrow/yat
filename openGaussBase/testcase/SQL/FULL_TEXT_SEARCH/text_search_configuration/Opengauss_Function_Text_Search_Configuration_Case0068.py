...
"""
Case Type   :全文检索--解析器
Case Name   : 文本搜索配置token_type为numeric（解析器为pg_catalog.ngram）不支持的编码，合理报错
Description :
        1.创建文本搜索配置，解析器为pg_catalog.ngram
        2.创建simple字典
        3.增加文本搜索配置字串类型映射，token为 numeric
...
class Parser(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Text_Search_Configuration_Case0068开始执行-----------------------------')
    ...
    def test_ngram_parser(self):
       ...
       logger.info('--------------------增加文本搜索配置字串类型映射，token为numeric---------')
       sql_cmd2 = commonsh.execut_db_sql('''ALTER TEXT SEARCH CONFIGURATION english2 ADD MAPPING FOR  numeric WITH pg_dict;''')
       logger.info(sql_cmd2)
       self.assertIn(self.Constant.ALTER_TEXT_SEARCH_CONFIGURATION, sql_cmd2)
       logger.info('-----------使用文本检索函数对所创建的词典配置english2进行测试---------')
       sql_cmd3 = commonsh.execut_db_sql('''SELECT ts_debug('ngram','999');''')
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_Text_Search_Configuration_Case0068执行结束--------------------------')
