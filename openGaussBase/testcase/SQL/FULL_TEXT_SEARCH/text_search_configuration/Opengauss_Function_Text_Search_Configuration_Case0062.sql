...
--增加文本搜索配置字串类型映射，token为version
ALTER TEXT SEARCH CONFIGURATION english2 ADD MAPPING FOR version WITH pg_dict;
--使用文本检索函数对所创建的词典配置english2进行测试
SELECT ts_debug('english','8.3.0');
SELECT ts_debug('8.3.0');
--删除文本搜索配置
