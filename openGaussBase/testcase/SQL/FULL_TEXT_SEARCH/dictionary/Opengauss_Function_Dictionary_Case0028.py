...
Case Name   : 创建Thesaurus词典,词典文件定义的sample words中，
              用问号（？）替代停用词
Description :
    1.创建test_data28.ths文件并定义了短语及其同义词
    2.创建词典
    3.创建文本搜索配置
    4.创建词典后，将其绑定到对应文本搜索配置中需要处理的token类型上
    5.使用词典测试,SELECT plainto_tsquery('russian','a one the two');
    SELECT to_tsvector('russian','the one a two');
    SELECT to_tsquery('russian','''the one the two''');
    SELECT to_tsvector('russian','a one a two');
    6.清理环境
Expect      :
    1.创建成功
    2.创建词典成功
    3.创建文本搜索配置成功
    4.增加映射成功
    5.显示'swsw'，词典文件? one ? two匹配了a one the two；
    显示'swsw':1；
    显示'swsw'；
    显示'swsw':1；
    6.清理环境完成
...
class Dictionary(unittest.TestCase):
    ...
    def setUp(self):
        logger.info(
            '--Opengauss_Function_Dictionary_Case0028开始执行----')
        self.DB_ENV_PATH = macro.DB_ENV_PATH
        self.DB_INSTANCE_PATH = macro.DB_INSTANCE_PATH
        self.user_node = Node('dbuser')
        self.constant = Constant()
        self.FILE_NAME = "test_data28.ths"
...
create text search configuration russian (parser =pg_catalog.default);''')
    ...
    with thesaurus_astro, english_stem;''')
        ...
        "SELECT to_tsvector('russian', 'a one a two');")
        logger.info(sql_cmd)
        self.assertIn("'swsw'", sql_cmd)
        self.assertIn("'swsw':1", sql_cmd)
    ...
    def tearDown(self):
      ...
      drop TEXT SEARCH DICTIONARY thesaurus_astro cascade;''')
        ...
        logger.info(msg1)
        logger.info(
            '----Opengauss_Function_Dictionary_Case0028执行结束---')
