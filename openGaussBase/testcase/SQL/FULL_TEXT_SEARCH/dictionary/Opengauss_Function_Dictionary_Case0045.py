...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
        logger.info("------------------------Opengauss_BaseFunc_Full_Text_Search_dictionary_0045开始执行--------------------------")
    ...
    def test_directoary_1(self):
        ...
        # 创建schema
        SqlMdg = commonsh.execut_db_sql('''drop schema if exists test_schema cascade;''')
        logger.info(SqlMdg)
        self.assertIn('DROP SCHEMA', SqlMdg)

        SqlMdg = commonsh.execut_db_sql('''create schema test_schema;''')
        logger.info(SqlMdg)
        self.assertIn('CREATE SCHEMA', SqlMdg)

       # 修改词典thesaurus_astro的模式为test_schema
        SqlMdg = commonsh.execut_db_sql('''ALTER TEXT SEARCH DICTIONARY thesaurus_astro SET SCHEMA test_schema;''')
        logger.info(SqlMdg)
    ...
    def tearDown(self):
        ...
        # 删除词典
        SqlMdg = commonsh.execut_db_sql('''drop TEXT SEARCH DICTIONARY test_schema.thesaurus_astro cascade;''')
        logger.info(SqlMdg)
        self.assertIn(constant.DROP_DICTIONARY_SUCCESS_MSG, SqlMdg)

        # 删除模式
        SqlMdg = commonsh.execut_db_sql('''drop schema test_schema cascade;''')
        logger.info(SqlMdg)
        self.assertIn('DROP SCHEMA', SqlMdg)
        logger.info('------------------------Opengauss_BaseFunc_Full_Text_Search_dictionary_0045执行结束--------------------------')
