...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
        logger.info("------------------------Opengauss_BaseFunc_Full_Text_Search_dictionary_0048开始执行--------------------------")
    ...
    def test_directoary_1(self):
       ...
       # 删除用户user1
        ...
        logger.info(SqlMdg)
        self.assertIn('NOTICE:  role "user1" does not exist', SqlMdg)

       # 修改词典所有者为user1
        SqlMdg = commonsh.execut_db_sql('''ALTER TEXT SEARCH DICTIONARY thesaurus_astro owner to user1; ''')
        logger.info(SqlMdg)
        self.assertIn('ERROR:  role "user1" does not exist', SqlMdg)
    ...
    def tearDown(self):
        ...
        logger.info(SqlMdg)
        self.assertIn(constant.DROP_DICTIONARY_SUCCESS_MSG, SqlMdg)
        logger.info('------------------------Opengauss_BaseFunc_Full_Text_Search_dictionary_0048执行结束--------------------------')
