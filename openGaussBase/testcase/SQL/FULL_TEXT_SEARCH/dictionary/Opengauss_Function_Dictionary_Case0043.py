...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
        logger.info("------------------------Opengauss_BaseFunc_Full_Text_Search_dictionary_0043开始执行--------------------------")
    ...
    def test_directoary_1(self):
        ...
        # 词典重命名
        SqlMdg = commonsh.execut_db_sql('''ALTER TEXT SEARCH DICTIONARY thesaurus_astro rename to new_thesaurus_astro;''')
        logger.info(SqlMdg)
        self.assertIn('ALTER TEXT SEARCH DICTIONARY', SqlMdg)

        # 查看词典相关信息
        SqlMdg = commonsh.execut_db_sql('''select dictname from PG_TS_DICT where dictname='new_thesaurus_astro';''')
        logger.info(SqlMdg)
        self.assertIn('new_thesaurus_astro', SqlMdg)

    # 删除词典
    def tearDown(self):
        logger.info('----------this is teardown-------')
        SqlMdg = commonsh.execut_db_sql('''drop TEXT SEARCH DICTIONARY if exists new_thesaurus_astro cascade;''')
        logger.info(SqlMdg)
        self.assertIn(constant.DROP_DICTIONARY_SUCCESS_MSG, SqlMdg)
        logger.info('------------------------Opengauss_BaseFunc_Full_Text_Search_dictionary_0043执行结束--------------------------')
