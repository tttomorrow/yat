...
"""
Case Type   : 功能测试
Case Name   : quote_ident处理带转义字符的二进制
Description :
...
class Bitstring_function(unittest.TestCase):

    def setUp(self):
        logger.info("------------Opengauss_Function_Innerfunc_Quote_Ident_Case0002开始执行---------------")
        self.commonsh = CommonSH('dbuser')

    def test_quote_ident(self):
        logger.info("-----------返回适用于SQL语句的标识符形式-----------")

        sql_cmd1 = '''SELECT quote_ident(E'O\\'hello');'''
        msg1 = self.commonsh.execut_db_sql(sql_cmd1)
        logger.info(msg1)
        self.assertTrue(msg1.splitlines()[2].strip().strip('"') == """O'hello""")

        sql_cmd2 = '''SELECT quote_ident(E'O\\nhello');'''
        ...
        sql_cmd3 = '''SELECT quote_ident(E'O\thello');'''
        msg3 = self.commonsh.execut_db_sql(sql_cmd3)
        logger.info(msg3)
        self.assertTrue(msg3.splitlines()[2].strip().strip('"') == 'O      hello')

        sql_cmd4 = '''SELECT quote_ident(E'O\\hello');'''
        msg4 = self.commonsh.execut_db_sql(sql_cmd4)
        logger.info(msg4)
        self.assertTrue(msg4.splitlines()[2].strip().strip('"') == 'Ohello')

        sql_cmd5 = '''SELECT quote_ident(E'O\\"hello');'''
        msg5 = self.commonsh.execut_db_sql(sql_cmd5)
        logger.info(msg5)
        self.assertTrue(msg5.splitlines()[2].strip().strip('"') == 'O""hello')

    def tearDown(self):
        logger.info('------------Opengauss_Function_Innerfunc_Quote_Ident_Case0002执行结束---------------')
