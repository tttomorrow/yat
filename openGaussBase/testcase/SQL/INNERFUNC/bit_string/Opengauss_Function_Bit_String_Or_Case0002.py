...
"""
Case Type   : 功能测试
Case Name   : 位串之间进行“或”操作(参数为空或者位串操作符位数不一致)
Description : 描述
    步骤 1.查看数据库状态，如果数据库没有启动则执行启动，如果已经启动则无操作
    步骤 2.清理环境删除表防止新建失败
    步骤 3.参数为空或者位串操作符位数不一致相或结果校验
Expect      : 
    步骤 1：数据库状态正常
    步骤 2：环境清理成功
    步骤 3：返回或后的结果正确
...
class Bit_String_Or_002(unittest.TestCase):

    def setUp(self):
        logger.info("------------------------Opengauss_Function_Bit_String_Or_Case0002开始执行--------------------------")
        logger.info("-----------查询数据库状态-----------")
    ...
    def test_bit_string_or_0022(self):
        Abnormal_SqlMdg1 = commonsh.execut_db_sql("SELECT B'101' | B'10111';")
        logger.info(Abnormal_SqlMdg1)
        self.assertTrue(Abnormal_SqlMdg1.find('ERROR:  cannot OR bit strings of different sizes') > -1)

    def tearDown(self):
        clear_sql = 'DROP table IF EXISTS bit_type_t1;'
        SqlMdg = commonsh.execut_db_sql(clear_sql)
        logger.info(SqlMdg)
        logger.info('-----------------------Opengauss_Function_Bit_String_Or_Case0002执行结束--------------------------')
