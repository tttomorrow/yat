...
"""
Case Type   : 功能测试
Case Name   : 位串之间进行“与”操作(参数为列或者参数为位数全为0或者1时相与)
Description : 描述
    步骤 1.查看数据库状态，如果数据库没有启动则执行启动，如果已经启动则无操作
    步骤 2.清理环境删除表防止新建失败
    步骤 3.参数为列或者参数为位数全为0或者1时相与结果校验
Expect      : 
    步骤 1：数据库状态正常
    步骤 2：环境清理成功
    步骤 3：返回与后的结果正确
...
class Bit_string_and(unittest.TestCase):

    def setUp(self):
        logger.info("-----------------------Opengauss_Function_Bit_String_And_Case0001 开始执行--------------------------")
        logger.info("-----------查询数据库状态-----------")
        commonsh.ensure_dbstatus_normal()

    def test_bit_string_and_001(self):
        ...
        aa = Normal_SqlMdg.splitlines()
        logger.info(aa)
        self.assertTrue(aa[2].strip() == '100')

    def test_bit_string_and_002(self):
        bit_dic = {'11111': (B'11111', B'11111'), '00000': (B'11111', B'00000'), '000000': (B'000000', B'000000')}
        ...
        for i in range(len(bit_dic)):
            ...
            self.assertTrue(Normal_SqlMdg.splitlines()[2].strip() == list(bit_dic.keys())[i])

    def tearDown(self):
        clear_sql = 'DROP table IF EXISTS bit_type_t1;'
        SqlMdg = commonsh.execut_db_sql(clear_sql)
        logger.info(SqlMdg)
        logger.info('-----------------------Opengauss_Function_Bit_String_And_Case0001 执行结束--------------------------')
