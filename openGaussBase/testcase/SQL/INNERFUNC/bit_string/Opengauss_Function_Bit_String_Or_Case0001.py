...
"""
Case Type   : 功能测试
Case Name   : 位串之间进行“或”操作(参数为列或者参数为位数全为0或者1时相与)
Description : 描述
    步骤 1.查看数据库状态，如果数据库没有启动则执行启动，如果已经启动则无操作
    步骤 2.清理环境删除表防止新建失败
    步骤 3.参数为列或者参数为位数全为0或者1时相或结果校验
Expect      : 
    步骤 1：数据库状态正常
    步骤 2：环境清理成功
    步骤 3：返回或后的结果正确
...
class Bit_string_or(unittest.TestCase):

    def setUp(self):
        logger.info("------------------------Opengauss_Function_Bit_String_Or_Case0001 开始执行--------------------------")
        logger.info("-----------查询数据库状态-----------")
        commonsh.ensure_dbstatus_normal()

    def test_bit_string_or_001(self):
        ...
        common.equal_sql_mdg(Normal_SqlMdg, '?column?', '101', '(1 row)', flag='1')

    def test_bit_string_or_002(self):
        bit_dic = {'11111': (B'11111', B'11111'), '11111': (B'11111', B'00000'), '000000': (B'000000', B'000000')}
    ...
    def tearDown(self):
        ...
        logger.info(SqlMdg1)
        logger.info('-----------------------Opengauss_Function_Bit_String_Or_Case0001 执行结束--------------------------')
