...
class Bit_string_non(unittest.TestCase):

    def setUp(self):
        logger.info("------------------------Opengauss_Function_Bit_String_Non_Case0001 开始执行--------------------------")
        logger.info("-----------查询数据库状态-----------")
        commonsh.ensure_dbstatus_normal()

    def test_bit_string_non_001(self):
        ...
        common.equal_sql_mdg(Normal_SqlMdg, '?column? | ?column?', '010      | 011', '(1 row)', flag='1')

    def test_bit_string_non_002(self):
        bit_dic = {'00000': B'11111', '111111': B'000000'}

        for i in range(len(bit_dic)):
            Normal_SqlMdg1 = commonsh.execut_db_sql(f"SELECT ~{list(bit_dic.values())[i]};")
    ...
    def tearDown(self):
        ...
        logger.info(SqlMdg1)
        logger.info('-----------------------Opengauss_Function_Bit_String_Non_Case0001 执行结束--------------------------')
