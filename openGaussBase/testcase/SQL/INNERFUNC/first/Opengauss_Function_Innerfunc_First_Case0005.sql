-- @testpoint: first函数返回第一个输入，若无输入，则返回一个空行：分区表
...
--输入包含null，并排序(nulls first,nulls last)
...
select s_id, first(s_score order by s_score desc  nulls last) from score group by s_id;
select c_id, first(s_score order by s_score desc  NULLS last) from score group by c_id having c_id > 102 order by c_id;
