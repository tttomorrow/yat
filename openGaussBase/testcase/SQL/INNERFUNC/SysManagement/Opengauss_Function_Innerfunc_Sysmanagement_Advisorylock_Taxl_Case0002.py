...
"""
Case Type   : 内建函数
Case Name   : pg_try_advisory_xact_lock(key1 int, key2 int)尝试获取事务级别的
             排它咨询锁
Description :
    1.创建表,插入数据
    2.会话1，更新某一条记录
...
class Innerfunc(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_advisorylock_taxl_case0002'
    ...
    def test_advisorylock(self):
        ...
        sql_cmd5 = f"start transaction;" \
                   f"update {self.tb_name} set info='abc' where id=1 " \
                   f"returning pg_try_advisory_xact_lock(id,id);"
        ...
        sql_cmd6 = f"select * from {self.tb_name} where id=1 and " \
                   f"pg_try_advisory_xact_lock(1,1);"
