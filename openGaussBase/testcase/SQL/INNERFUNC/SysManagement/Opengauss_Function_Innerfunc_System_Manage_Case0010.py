...
Case Name   : 使用函数local_single_flush_dw_stat() 显示本实例的单页面淘汰双写文件的情况
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-Opengauss_Function_Innerfunc_System_Manage_Case0010开始-')
        self.commonsh = CommonSH('dbuser')

    def test_func_sys_manage(self):
        LOG.info(f'---步骤1.使用函数local_double_write_stat()显示本实例的双写文件的情况---')
        sql_cmd = self.commonsh.execut_db_sql(f'select local_single'
                                              f'_flush_dw_stat();')
        LOG.info(sql_cmd)
    ...
    def tearDown(self):
        LOG.info('-------无需清理环境-------')
        LOG.info('-Opengauss_Function_Innerfunc_System_Manage_Case0010结束-')
