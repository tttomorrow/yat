...
class Innerfunc(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_advisorylock_axls_case0001'
    ...
    def test_advisorylock(self):
        ...
        sql_cmd1 = f"drop table if exists {self.tb_name};" \
                   ...
                   f"(id int primary key, info text);" \
                   f"insert into {self.tb_name} values (2,'test');"

        step2_txt = '---step2:会话1，使用advisory lock锁住这个ID并更新数据' \
                    ';expect:加锁成功---'
        self.log.info(step2_txt)
        sql_cmd2 = f"start transaction;" \
                   f"select pg_advisory_xact_lock_shared(id) from " \
                   f"{self.tb_name};"

        step3_txt = '---step3:会话1更新数据并查询;expect:更新成功---'
        self.log.info(step3_txt)
        sql_cmd3 = f"update {self.tb_name} set info='mmm' where id=2;" \
        ...
        step5_txt = '--step5:会话2对该id也进行加锁;expect:会话2加锁成功--'
        ...
        sql_cmd6 = f"update {self.tb_name} set info='ABC' where id=2;"

        step7_txt = '---step7:会话1提交事务;expect:提交成功;step4表更新成功-'
        self.log.info(step7_txt)
        sql_cmd7 = "commit;"

        step8_txt = '---step8:会话2提交事务;expect:提交成功---'
        ...
        self.log.info('----线程2操作事务2:step4->step5->step6->step8---')
        ...
        start_time = thread_results[1].splitlines()[-15].strip()[:-3]
        self.log.info(start_time)
        trans_to_dtime1 = datetime.datetime.strptime(start_time,
                                                     '%Y-%m-%d %H:%M:%S')
        end_time = thread_results[1].splitlines()[-9].strip()[:-3]
