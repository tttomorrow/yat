...
"""
Case Type   : 内建函数
Case Name   : pg_try_advisory_xact_lock(key bigint)尝试获取事务级别的排它咨询锁
Description :
    1.创建表,插入数据
    2.会话1，更新某一条记录
    3.会话2，读这条记录
    4.会话1提交事务
    5.会话1更新这条记录的同时，使用advisory lock锁住这个ID
...
class Innerfunc(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_advisorylock_taxl_case0001'
    ...
    def test_advisorylock(self):
        ...
        step3_txt = '---step3:会话2，读这条记录;expect:读写不冲突，' \
                    '查询成功，数据未更新--;'
        ...
        step5_txt = '--step5:会话1更新这条记录的同时，使用advisory lock' \
                    '锁住这个ID;expect:加锁成功--'
        self.log.info(step5_txt)
        sql_cmd5 = f"start transaction;" \
                   f"update {self.tb_name} set info='abc' where id=1 " \
                   f"returning pg_try_advisory_xact_lock(id);"
        ...
        sql_cmd6 = f"select * from {self.tb_name} where id=1 and " \
                   f"pg_try_advisory_xact_lock(1);"

        step7_txt = '--step7:会话1提交事务;expect:提交成功--'
        self.log.info(step7_txt)
        sql_cmd7 = "commit;"

        step8_txt = '---step8:会话2再次执行步骤6;' \
                    'expect:会话1commit后会话2查询到1行---'
        ...
        self.log.info('----线程2操作事务2:step3->step6->step8----')
        ...
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, thread_results[0],
                      '执行失败:' + step1_txt)
        self.assertTrue(thread_results[0].count('UPDATE 1') == 2,
                        '执行失败:' + step2_txt)
        ...
        self.assertTrue(thread_results[0].count('COMMIT') == 2,
                        '执行失败:' + step7_txt)
        self.assertIn('(1 row)', thread_results[-1], '执行失败:' + step8_txt)
