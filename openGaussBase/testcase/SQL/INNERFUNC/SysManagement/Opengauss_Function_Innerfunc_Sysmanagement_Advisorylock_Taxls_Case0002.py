...
"""
Case Type   : 内建函数
Case Name   : pg_try_advisory_xact_lock_shared(key1 int, key2 int)尝试获取事务
              级别的共享咨询锁
Description :
    1.创建表,插入数据
...
class Innerfunc(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = "tb_advisorylock_taxls_case0002"
        self.tb_name_01 = "tb_advisorylock_taxls_case0002_01"

    def test_advisorylock(self):
        step1_txt = '---step1:创建表,插入数据;expect:创建成功;数据插入成功---'
        self.log.info(step1_txt)
        sql_cmd = f"drop table if exists {self.tb_name};" \
                  f"create table {self.tb_name}(qq int primary key, " \
                  f"class text);" \
        ...
        self.log.info('-step2.1:会话1，更新这条记录的同时，使用advisory lock'
                      '锁住列并查询;expect:加锁成功，表1数据更新成功-')
        self.log.info(step2_txt)
        sql_cmd = f"start transaction;" \
                  f"update {self.tb_name} set class='mmm' where qq=2  " \
                  f"returning pg_try_advisory_xact_lock_shared(qq,qq);" \
                  f"select * from {self.tb_name} where qq=2;" \
        ...
        self.log.info('step3.1:会话2，读这条记录;expect:读写不冲突，'
                      '查询成功，数据未更新')
        self.log.info('--step3.2:会话2对表也进行加锁;'
        ...
        sql_cmd = f"select pg_sleep(3);" \
                  ...
                  f"start transaction;" \
                  f"select timenow();" \
                  f"update {self.tb_name} set class='eee' where qq=2 " \
                  f"returning pg_try_advisory_xact_lock_shared(qq,qq);" \
                  f"select timenow();" \
                  f"select * from {self.tb_name} where qq=2;" \
                  f"select pg_sleep(10);" \
                  f"update {self.tb_name_01} set class='mmm' where qq=2 " \
                  f"returning pg_try_advisory_xact_lock_shared(qq,qq);" \
        ...
        self.assertIn('2 | mmm',  msg_result_1, '执行失败:' + step2_txt)
        self.assertIn('2 | test', msg_result_2, '执行失败:' + step3_txt)
        ...
        trans_to_dtime1 = datetime.datetime.strptime(start_time,
                                                     '%Y-%m-%d %H:%M:%S')
        end_time = msg_result_2.splitlines()[24].strip()[:-3:]
        ...
        self.assertIn(self.constant.COMMIT_SUCCESS_MSG, msg_result_1,
                      '执行失败:' + step2_txt)
        self.assertEqual('UPDATE 1',
                         msg_result_1.splitlines()[6].strip(),
                         '执行失败:' + step3_txt)
        self.assertTrue(self.constant.COMMIT_SUCCESS_MSG in msg_result_2,
                        '执行失败:' + step3_txt)
