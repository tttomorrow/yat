...
"""
Case Type   : 内建函数
Case Name   : pg_advisory_lock(key1 int, key2 int)获取会话级别的排它咨询锁
Description :
...
class Innerfunc(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_advisorylock_al_case0002'
    ...
    def test_advisorylock(self):
        ...
        sql_cmd = f"start transaction;" \
                  ...
                  f"returning pg_advisory_lock(id,id);" \
        ...
        self.assertIn('START TRANSACTION', msg_result_1,
                      '执行失败:' + step2_txt)
        self.assertIn('UPDATE 1', msg_result_1, '执行失败:' + step4_txt)
        ...
        self.assertGreaterEqual(f'{num}', '11', '执行失败:' + step4_txt)
