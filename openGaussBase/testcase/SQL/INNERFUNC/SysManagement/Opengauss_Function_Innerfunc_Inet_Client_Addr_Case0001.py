...
"""
Case Type   : 功能测试
Case Name   : 远程连接模式下查询当前客户端的IP地址
Description :
    1.远程状态，执行命令select inet_client_addr();
Expect      :
    1.返回当前客户端的IP地址
History     :
"""
...
class Function(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--------' 
            'Opengauss_Function_Innerfunc_Inet_Client_Addr_Case0001开始------')
    ...
    def test_inet(self):
        ...
        try:
            ...
            self.log.info(self.client_ip)
            self.assertEqual(msg3.splitlines()[2].strip(), self.client_ip)
        finally:
    ...
    def tearDown(self):
        self.log.info('--------'
            'Opengauss_Function_Innerfunc_Inet_Client_Addr_Case0001结束------')