...
"""
Case Type   : 功能测试
Case Name   : 远程连接模式下返回服务器接收当前连接用的IP地址
Description :
    1.远程状态，执行命令select inet_server_addr();
Expect      :
    1.当前服务器的IP地址
History     :
"""
...
class Function(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--------' 
            'Opengauss_Function_Innerfunc_Inet_Server_Addr_Case0001开始------')
    ...
    def test_inet(self):
        ...
        try:
            ...
            self.log.info(f'msg3 = {msg3}')
            self.log.info(f'msg3.splitlines()[2].strip()'
                          f' = {msg3.splitlines()[2].strip()} ')
            self.log.info(f'self.server_ip = {self.server_ip}')
            self.assertEqual(msg3.splitlines()[2].strip(), self.server_ip)
        finally:
    ...
    def tearDown(self):
        self.log.info('--------' 
            'Opengauss_Function_Innerfunc_Inet_Server_Addr_Case0001结束------')