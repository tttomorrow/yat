...
Expect      :
    ...
    4.提交成功
    5.加锁成功
    6.在会话1未提交事务时，会话2加锁处于等待状态
    7.提交成功,会话1提交事务后，会话2中加锁成功
    8.清理环境完成
...
class Innerfunc(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_advisorylock_axl_case0001'
    ...
    def test_advisorylock(self):
        ...
        sql_cmd5 = f"start transaction;" \
                   f"update {self.tb_name} set info='abc' where id=1 " \
                   f"returning pg_advisory_xact_lock(id);"

        step6_txt = '--step6:会话2对上面记录再次执行加锁;' \
                    'expect:在会话1未提交事务时，会话2加锁处于等待状态--'
        self.log.info(step6_txt)
        sql_cmd6 = "select pg_advisory_xact_lock(1);"

        step7_txt = '---step7:会话1，commit事务;' \
                    'expect:会话1提交事务后，会话2中加锁成功---'
        ...
        self.assertTrue(thread_results[0].count('UPDATE') == 2,
                        '执行失败:' + step2_txt)
        ...
        self.log.info('分别获取step6中加锁阻塞前及加锁成功后的时间')
