...
"""
Case Type   : 功能测试
Case Name   : 远程连接模式下查询当前客户端的端口号
Description :
    1.远程状态，执行命令select inet_client_port();
Expect      :
    1.返回当前客户端的端口号
History     :
"""
...
class Function(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('--------' 
            'Opengauss_Function_Innerfunc_Inet_Client_Port_Case0001开始------')
    ...
    def test_inet(self):
        ...
        try:
            ...
            self.log.info(msg3)
            self.assertTrue(0 < int(msg3.splitlines()[2].strip()) < 999999)
        finally:
    ...
    def tearDown(self):
        self.log.info('--------' 
            'Opengauss_Function_Innerfunc_Inet_Client_Port_Case0001结束------')