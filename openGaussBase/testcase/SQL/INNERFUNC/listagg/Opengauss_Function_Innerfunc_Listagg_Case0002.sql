...
SELECT ename, listagg(eid, '、') WITHIN GROUP(ORDER BY mgrno NULLS FIRST) AS id FROM emp GROUP BY ename;
SELECT deptno, listagg(mgrno, ' & ') WITHIN GROUP(ORDER BY mgrno NULLS FIRST) AS result FROM emp GROUP BY deptno;
SELECT eid, listagg(deptno, ',') WITHIN GROUP(ORDER BY mgrno NULLS FIRST) AS deptno FROM emp GROUP BY eid;
