...
"""
Case Type   : AI特性函数
Case Name   : pg_get_triggerdef(oid, boolean) 描述：获取触发器的定义信息
Description :
    1.创建源表和触发表
    2.创建触发器函数
    3.创建触发器
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-Opengauss_Function_Innerfunc_Trigger_Func_Case0004开始-')
    ...
    def test_func_sys_manage(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'''
            ...
            insert into test_tb_trigger_001 values(1,2,3);
            ''')
        LOG.info(sql_cmd)
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'''
        select pg_get_triggerdef({num1},true) from pg_trigger;
        select pg_get_triggerdef({num1},false) from pg_trigger;
            ''')
        LOG.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'''
            ...
            ''')
        LOG.info(sql_cmd)
        LOG.info('-Opengauss_Function_Innerfunc_Trigger_Func_Case0004结束-')
