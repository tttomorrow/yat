...
"""
Case Type   : 统计信息函数
Case Name   : DBE_PERF.get_global_stat_database()描述：汇聚所有节点数据库
    统计信息，查询该函数必须具有sysadmin权限。
Description :
    1.汇聚所有节点数据库统计信息，以系统用户执行
    2.汇聚所有节点数据库统计信息，以非系统用户执行
Expect      :
    1.汇聚所有节点数据库统计信息，以系统用户执行成功
    2.汇聚所有节点数据库统计信息，以非系统用户执行合理报错
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_statistics_function_Case0024开始')
        self.dbuser = Node('dbuser')
        self.commonsh = CommonSH()

    def test_built_in_func(self):
        self.log.info('-----步骤1.汇聚所有节点数据库统计信息，以系统用户执行-----')
        sql_cmd = self.commonsh.execut_db_sql(
            f'select DBE_PERF.get_global_stat_database();')
        ...
        if number >= 3:
            ...
            if num == 20:
                self.log.info('汇聚所有节点数据库统计信息成功')
            else:
                raise Exception('函数执行异常，请检查')
        else:
            raise Exception('执行结果异常，请检查')

        self.log.info("-----步骤2.汇聚所有节点数据库统计信息，以非系统用户执行-----")
        gsql_cmd = f'source {macro.DB_ENV_PATH};' \
            ...
            f' -c "select DBE_PERF.get_global_stat_database();" '
    ...
    def tearDown(self):
        self.log.info('Opengauss_Function_statistics_function_Case0024结束')
