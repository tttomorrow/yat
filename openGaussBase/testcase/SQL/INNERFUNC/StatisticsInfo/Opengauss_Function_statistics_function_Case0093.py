...
"""
Case Type   : 统计信息函数
Case Name   : gs_wlm_readjust_user_space_with_reset_flag
            (text name, boolean isfirst)描述：修正指定用户的存储空间使用情况
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_statistics_function_Case0093开始')
    ...
    def test_built_in_func(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            f'select gs_wlm_readjust_user_space_with_'
            f'reset_flag(\'{self.dbuser.db_user}\',true);'
            f'select gs_wlm_readjust_user_space_with_'
            f'reset_flag(\'{self.dbuser.ssh_user}\',true);')
        ...
        sql_msg = f'source {macro.DB_ENV_PATH};' \
            ...
            f' -c "select gs_wlm_readjust_user_space_with_' \
            f'reset_flag(\'{self.dbuser.ssh_user}\',true);"'
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_statistics_function_Case0093结束')
