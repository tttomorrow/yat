...
"""
Case Type   : 统计信息函数
Case Name   : DBE_PERF.get_summary_stat_xact_user_tables()描述：统计所有节
    点命名空间中用户表的事务状态信息，查询该函数必须具有sysadmin权限。
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_statistics_function_Case0020开始')
        self.dbuser = Node('dbuser')
        self.commonsh = CommonSH()
        self.Constant = Constant()

    def test_built_in_func(self):
        self.log.info('-----步骤1.统计所有节点命名空间中用户表的事务状态信息，以系统用户执行-----')
        sql_cmd = self.commonsh.execut_db_sql(
            f'create table test (id int);'
            f'select DBE_PERF.get_summary_stat_xact_user_tables();')
        ...
        if num == 12:
            self.log.info('统计所有节点命名空间中用户表的事务状态信息成功')
        else:
            raise Exception('函数执行异常，请检查')

        self.log.info("-----步骤2.统计所有节点命名空间中用户表的事务状态信息，以非系统用户执行-----")
        gsql_cmd = f'source {macro.DB_ENV_PATH};' \
            ...
            f' -c "select DBE_PERF.get_summary_stat_xact_user_tables();" '
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            f'drop table test;')
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_statistics_function_Case0020结束')
