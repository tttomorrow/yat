...
"""
Case Type   : 统计信息函数
Case Name   : DBE_PERF.get_global_os_runtime()描述：显示当前操作系统运行的
            状态信息，查询该函数必须具有sysadmin权限
Description :
    1.显示当前操作系统运行的状态信息,主节点执行
    2.以非系统用户执行
    3.显示当前操作系统运行的状态信息,备节点执行
Expect      :
    1.显示当前操作系统运行的状态信息,主节点执行成功
    2.以非系统用户执行合理报错
    3.显示当前操作系统运行的状态信息,备节点执行成功
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_statistics_function_Case0006开始')
        self.dbuser = Node('dbuser')
        self.commonsh = CommonSH()
        self.commonsh1 = CommonSH('Standby1DbUser')

    def test_server_tools1(self):
        self.log.info('-----步骤1.显示当前操作系统运行的状态信息,主节点执行-----')
        sql_cmd = self.commonsh.execut_db_sql(
            f'select DBE_PERF.get_global_os_runtime() limit 3;')
        ...
        if number >= 3:
            ...
            self.log.info(list2)
            if list2 == 7:
                self.log.info('显示当前操作系统运行的状态信息成功')
            else:
    ...
    def tearDown(self):
        self.log.info('Opengauss_Function_statistics_function_Case0006结束')
