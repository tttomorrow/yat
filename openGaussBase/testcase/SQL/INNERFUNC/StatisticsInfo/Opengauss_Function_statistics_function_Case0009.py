...
"""
Case Type   : 统计信息函数
Case Name   : DBE_PERF.get_global_instance_time()描述：提供openGauss各个
            关键阶段的时间消耗，查询该函数必须具有sysadmin权限
Description :
    1.查看openGauss各个关键阶段的时间消耗，主节点执行
    2.以非系统用户执行
    3.查看openGauss各个关键阶段的时间消耗，备节点执行
Expect      :
    1.查看openGauss各个关键阶段的时间消耗，主节点执行成功
    2.以非系统用户执行合理报错
    3.查看openGauss各个关键阶段的时间消耗，备节点执行成功
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_statistics_function_Case0009开始')
        self.dbuser = Node('dbuser')
        self.commonsh = CommonSH()
        self.commonsh1 = CommonSH('Standby1DbUser')

    def test_server_tools1(self):
        self.log.info('-----步骤1.查看openGauss各个关键阶段的时间消耗，主节点执行-----')
        sql_cmd = self.commonsh.execut_db_sql(
            f'select DBE_PERF.get_global_instance_time();')
        ...
        if number >= 3:
            ...
            self.log.info(list2)
            if list2 == 4:
                self.log.info('查看openGauss各个关键阶段的时间消耗成功')
            else:
    ...
    def tearDown(self):
        self.log.info('Opengauss_Function_statistics_function_Case0009结束')
