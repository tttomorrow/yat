...
"""
Case Type   : 统计信息函数
Case Name   : DBE_PERF.get_global_statio_sys_indexes()描述：汇聚各节点的命名
    空间中所有系统表索引的IO状态信息，查询该函数必须具有sysadmin权限。
Description :
    1.汇聚各节点的命名空间中所有系统表索引的IO状态信息，以系统用户执行
    1.汇聚各节点的命名空间中所有系统表索引的IO状态信息，以非系统用户执行
Expect      :
    1.汇聚各节点的命名空间中所有系统表索引的IO状态信息，以系统用户执行
    1.汇聚各节点的命名空间中所有系统表索引的IO状态信息，以非系统用户执行
History     :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_statistics_function_Case0041开始')
        self.dbuser = Node('dbuser')
        self.commonsh = CommonSH()

    def test_built_in_func(self):
        self.log.info('---步骤1.汇聚各节点的命名空间中所有系统表索引的IO状态信息，以系统用户执行---')
        sql_cmd = self.commonsh.execut_db_sql(
            f'select DBE_PERF.get_global_statio_sys_indexes () limit 3;')
        ...
        self.log.info("---步骤2.汇聚各节点的命名空间中所有系统表索引的IO状态信息，以非系统用户执行---")
        gsql_cmd = f'source {macro.DB_ENV_PATH};' \
            ...
            f' -c "select DBE_PERF.get_global_statio_sys_indexes();" '
    ...
    def tearDown(self):
        self.log.info('Opengauss_Function_statistics_function_Case0041结束')
