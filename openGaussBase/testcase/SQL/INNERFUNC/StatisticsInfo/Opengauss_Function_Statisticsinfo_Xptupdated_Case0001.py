...
'''
Case Type： function
Case Name： pg_stat_get_xact_partition_tuples_updated(oid)表分区相关的活跃子事务中更新的tuple数。
Create at:  2020/10/9
Descption:  验证函数对分区表在事务内增删改查后updated的统计准确性
'''
import unittest
...
class Function(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------StatisticsInfo_xptupdated_001 start-----------------------------')
    ...
    def get_tuples(self):
        ...
        sql_cmd1 = '''select pg_stat_get_xact_partition_tuples_updated(a.oid) from PG_PARTITION a where a.relname = 'p1';'''
        msg1 = self.sh_primy.execut_db_sql(sql_cmd1)
        logger.info(msg1)
        msg1_list = msg1.splitlines()
        self.assertTrue(int(msg1_list[2]) == 0)
        sql_cmd2 = '''select pg_stat_get_xact_partition_tuples_updated(a.oid) from PG_PARTITION a where a.relname = 'p2';'''
        msg2 = self.sh_primy.execut_db_sql(sql_cmd2)
        logger.info(msg2)
        msg2_list = msg2.splitlines()
        self.assertTrue(int(msg2_list[2]) == 0)
        logger.info('---------------testpoint:插入数据更新不统计-------------')
        sql_cmd1 = '''INSERT INTO sales VALUES(1, 12, '2017-01-10 00:00:00', 'a', 1, 1, 1);
                    ...
                    SELECT pg_sleep(2);'''
        self.sh_primy.execut_db_sql(sql_cmd1)
        sql_cmd3 = '''select pg_stat_get_xact_partition_tuples_updated(a.oid) from PG_PARTITION a where a.relname = 'p1';'''
        msg3 = self.sh_primy.execut_db_sql(sql_cmd3)
        logger.info(msg3)
        msg3_list = msg3.splitlines()
        self.assertTrue(int(msg3_list[2]) == 0)
        sql_cmd4 = '''select pg_stat_get_xact_partition_tuples_updated(a.oid) from PG_PARTITION a where a.relname = 'p2';'''
    ...
    def tearDown(self):
        ...
        logger.info('----------------------------StatisticsInfo_xptupdated_001 finish-----------------------------')