...
"""
Case Type   : 统计信息函数
Case Name   : DBE_PERF.get_global_file_iostat()描述：汇聚所有节点
    数据文件IO的统计信息，查询该函数必须具有sysadmin权限。
Description :
    1.汇聚所有节点数据文件IO的统计信息，以系统用户执行
    2.汇聚所有节点数据文件IO的统计信息，以非系统用户执行
Expect      :
    1.汇聚所有节点数据文件IO的统计信息，以系统用户执行成功
    2.汇聚所有节点数据文件IO的统计信息，以非系统用户执行，合理报错
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_statistics_function_Case0044开始')
    ...
    def test_built_in_func(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            f'select DBE_PERF.get_global_file_iostat () limit 3;')
        ...
        if number >= 3:
            ...
            if num == 14:
                self.log.info('汇聚所有节点数据文件IO的统计信息信息成功')
            else:
        ...
        gsql_cmd = f'source {macro.DB_ENV_PATH};' \
            ...
            f' -c "select DBE_PERF.get_global_file_iostat();" '
    ...
    def tearDown(self):
        self.log.info('Opengauss_Function_statistics_function_Case0044结束')
