...
"""
Case Type   : 统计信息函数
Case Name   : DBE_PERF.get_global_config_settings()描述：汇聚各节点GUC参数
    配置信息，查询该函数必须具有sysadmin权限
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_statistics_function_Case0013开始')
        self.dbuser = Node('dbuser')
        self.commonsh = CommonSH()

    def test_built_in_func(self):
        self.log.info('-----步骤1.汇聚各节点GUC参数配置信息，以系统用户执行-----')
        sql_cmd = self.commonsh.execut_db_sql(f'select DBE_PERF.get_global_'
                                              f'config_settings() limit 3;')
        ...
        self.log.info("-----步骤2.汇聚各节点GUC参数配置信息，以非系统用户执行-------")
        gsql_cmd = f'source {macro.DB_ENV_PATH};' \
            ...
            f' -c "select DBE_PERF.get_global_config_settings();" '
    ...
    def tearDown(self):
        self.log.info('Opengauss_Function_statistics_function_Case0013结束')
