-- @testpoint: pg_stat_get_xact_blocks_hit(oid)返回当前事务中在缓存中找到的表或索引的磁盘块获取请求数。
...
-- 创建索引前查询 0
SELECT pg_sleep(1);
select pg_stat_get_xact_blocks_hit(a.oid) from PG_CLASS a where a.relname = 'sales';
-- 创建索引 0 0
create index test_index1 on sales (channel_id);
SELECT pg_sleep(1);
select pg_stat_get_xact_blocks_hit(a.oid) from PG_CLASS a where a.relname = 'sales';
select pg_stat_get_xact_blocks_hit(a.oid) from PG_CLASS a where a.relname = 'test_index1';
-- 数据插入sales  1 2
