...
Case Name   : pg_buffercache_pages函数，索引对象usage_count字段验证
Description :
    1、gsql连接数据库，创建表及索引
    2、初始查询表索引的缓存信息
    3、表中插入不同量数据
...
Expect      :
    ...
    3、表中插入不同量数据成功
    4、再次查询表的索引缓存信息，两张表查询结果usage_count值不同，数据量较大的数据count值较大；
    5、环境清理成功
History     :
"""
...
class PgBuffercachePagesCase0019(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            'Opengauss_Function_Innerfunc_Pg_Buffercache_Pages_Case0019:初始化')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.t_name1 = 't_pg_buffercache_pages_case0019_1'
        self.t_name2 = 't_pg_buffercache_pages_case0019_2'
        self.i_name1 = 'i_pg_buffercache_pages_case0019_1'
        self.i_name2 = 'i_pg_buffercache_pages_case0019_2'

    def test_main(self):
        step_txt = '----step1:创建表及索引，expect: 创建成功----'
        ...
        insert_sql = f"insert into {self.t_name1} " \
            ...
            f"values(generate_series(1, 20), 'testtext');"
        ...
        step_txt = f'----step4: 再次查询两张表的索引缓存信息，expect: ' \
        ...
        self.assertNotEqual(t1_count, t2_count, '执行失败：' + step_txt)
        self.assertGreater(int(t2_count),
                           int(t1_count), '执行失败：' + step_txt)
    ...
    def tearDown(self):
        ...
        self.assertTrue(drop_result.count(self.constant.TABLE_DROP_SUCCESS)
                        == 2, '执行失败:' + step_txt)
        self.log.info(
            'Opengauss_Function_Innerfunc_Pg_Buffercache_Pages_Case0019:执行完毕')
