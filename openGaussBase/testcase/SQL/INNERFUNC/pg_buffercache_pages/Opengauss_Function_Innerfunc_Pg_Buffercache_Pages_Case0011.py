...
Case Name   : pg_buffercache_pages函数，手工checkpoint，表对象isdirty字段验证
Description :
...
class PgBuffercachePagesCase0011(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            'Opengauss_Function_Innerfunc_Pg_Buffercache_Pages_Case0011:初始化')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.t_name1 = 't_pg_buffercache_pages_case0011_1'

    def test_main(self):
        step_txt = '----step1:修改参数值，expect: 修改成功----'
        ...
        self.change_pram_list = ['on', 'all', '0', 'off', 'on']
        ...
        self.log.info(step_txt)
        select_sql = f"select isdirty from pg_buffercache_pages() where " \
            f"relfilenode in (select relfilenode from pg_class " \
            f"where relname='{self.t_name1}');"
        self.log.info(select_sql)
        select_result = self.pri_sh.execut_db_sql(select_sql)
        ...
        step_txt = f'----step6: 手工执行checkpoint，再次查询表的缓存信息' \
        ...
        for i in range(0, 5):
            self.assertEqual([j for j in self.param_dict.values()][i],
                    ...
                    f"参数{[j for j in self.param_dict.keys()][i]}还原失败！"
                             + step_txt)
        self.log.info(
            'Opengauss_Function_Innerfunc_Pg_Buffercache_Pages_Case0011:执行完毕')
