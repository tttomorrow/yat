...
Case Name   : pg_buffercache_pages函数，自动checkpoint，索引对象isdirty字段验证
Description :
    ...
    4、表中插入一定量数据
    5、再次查询表的索引缓存信息
    6、等待checkpoint_timeout时间，再次查询表的索引缓存信息
    7、环境清理
Expect      :
    1、参数值修改成功
    ...
    6、等待checkpoint_timeout时间，再次查询缓存信息，isdirty字段为f
    7、环境清理成功
History     :
      增加log_min_duration_statement参数值修改，增加log_checkpoints参数值修改，
      增加log_statement参数值修改，同时增加重启后的等待时间，方便定位问题
"""
...
class PgBuffercachePagesCase0021(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            'Opengauss_Function_Innerfunc_Pg_Buffercache_Pages_Case0021:初始化')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.t_name1 = 't_pg_buffercache_pages_case0021_1'
        self.i_name1 = 'i_pg_buffercache_pages_case0021_1'
    ...
    def test_main(self):
        ...
        checkpoint_sql = f'select pg_sleep(32);'
        ...
        for i in range(0, 6):
            self.assertEqual([j for j in self.param_dict.values()][i],
                ...
                f"参数{[j for j in self.param_dict.keys()][i]}还原失败！"
                             + step_txt)
        self.log.info(
            'Opengauss_Function_Innerfunc_Pg_Buffercache_Pages_Case0021:执行完毕')
