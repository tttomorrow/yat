...
"""
Case Type   : 功能测试
Case Name   : floor函数入参给正负浮点型，取不大于参数的最大整数
...
class Floor_002(unittest.TestCase):

    def setUp(self):
        logger.info("------Opengauss_Function_Innerfunc_Function_Floor_Case0002开始执行------------")
        logger.info("-----------查询数据库状态-----------")
    ...
    def test_floor_002(self):
        ...
        SqlMdg = commonsh.execut_db_sql('insert into data_01 values (21.25,-1.99);')
        logger.info(SqlMdg)
        self.assertTrue(SqlMdg.find('INSERT 0 1') > -1)
        SqlMdg = commonsh.execut_db_sql('select  floor(clo1) , floor(clo2) from data_01;')
        common.equal_sql_mdg(SqlMdg, 'floor | floor', '-------+-------', '21 |    -2', '(1 row)')
    ...
    def tearDown(self):
        ...
        logger.info(SqlMdg)
        self.assertTrue(SqlMdg.find(constant.TABLE_DROP_SUCCESS) > -1)
        logger.info('------Opengauss_Function_Innerfunc_Function_Floor_Case0002执行结束-----------')
