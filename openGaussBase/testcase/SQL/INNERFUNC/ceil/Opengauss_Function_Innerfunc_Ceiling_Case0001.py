...
class Ceiling_001(unittest.TestCase):

    def setUp(self):
        logger.info("-------Opengauss_Function_Innerfunc_Function_Ceiling_Case0001开始执行--------------")
        logger.info("-----------查询数据库状态-----------")
        commonsh.ensure_dbstatus_normal()

    def test_ceiling_001(self):
        ...
        common.equal_sql_mdg(SqlMdg, 'ceiling | ceiling', '---------+---------', '21 |      -1', '(1 row)')
    ...
    def tearDown(self):
        ...
        logger.info(SqlMdg)
        self.assertTrue(SqlMdg.find(constant.TABLE_DROP_SUCCESS) > -1)
        logger.info("-------Opengauss_Function_Innerfunc_Function_Ceiling_Case0001执行结束--------------")
