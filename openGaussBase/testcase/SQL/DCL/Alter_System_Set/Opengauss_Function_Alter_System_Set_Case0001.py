...
class Altertestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info('==Opengauss_Function_Alter_System_Set_Case0001开始执行==')
    ...
    def test_alter(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'''drop schema \
            if exists system_user_001 cascade;\
            DROP USER IF EXISTS system_user_001;\
            CREATE USER system_user_001 PASSWORD '{macro.COMMON_PASSWD}';\
            ''')
        LOGGER.info(sql_cmd)
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'''SET SESSION AUTHORIZATION \
            system_user_001 PASSWORD '{macro.COMMON_PASSWD}';
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(f"DROP USER system_user_001")
        LOGGER.info(sql_cmd)
        LOGGER.info('==Opengauss_Function_Alter_System_Set_Case0001执行结束==--')
