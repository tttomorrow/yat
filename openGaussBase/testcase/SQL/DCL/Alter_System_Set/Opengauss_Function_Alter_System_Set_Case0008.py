...
"""
Case Type   : 功能测试
Case Name   : 有管理员权限:修改userset类型:合理报错
Description :
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Alter_System_Set_Case0008开始执行-----------------------------')
    ...
    def test_common_user_permission(self):
        logger.info('-----------------------------------------管理员用户登录，show如下参数 期望:显示成功-----------------------------------------')
        ...
        sql_cmd = self.commonsh.execut_db_sql('''
                                            ALTER SYSTEM SET trace_sort to on;
                                            ALTER SYSTEM SET  convert_string_to_digit to off;
                                            ''')
        logger.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_Alter_System_Set_Case0008执行结束--------------------------')