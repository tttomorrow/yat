...
"""
Case Type   : 功能测试
Case Name   : 将用户在指定表上的权限授权给其他用户
Description :
        1.创建用户1，创建表；
        2.查看用户1是否有表的相关操作权限
        3.给用户1赋予指定表的select、insert权限；
        4.退出当前数据库，用新建用户1连接数据库；
        5.创建用户2，将用户1的权限授权给用户2；
        6.用新建用户2连接数据库，对指定表进行select、insert操作；
        7.清理环境；
Expect      :
        1.创建用户1成功，创建表成功；
        2.无相关权限；
...
class Grant(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_DCL_Grant_Case0029.py start-------------------')
    ...
    def test_grant(self):
        ...
        self.user1 = 'p_user09_1'
        self.user2 = 'p_user09_2'
        self.table = 'p_table'
        sql_cmd1 =f'''                    
        ...
        logger.info('------------退出当前数据库，用新建用户2连接数据库，对表进行已赋权操作--------------')
    ...
    def tearDown(self):
        ...
        logger.info(msg5)
        logger.info('------------------Opengauss_Function_DCL_Grant_Case0029.py finish------------------')
