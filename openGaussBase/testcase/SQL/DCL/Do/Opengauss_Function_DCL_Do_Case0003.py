...
'''
--- Case Type： Do
--- Case Name： 执行匿名代码块，指定解析代码的程序语言为internal，合理报错
'''
...
class DCL_Do_test(unittest.TestCase):
    ...
    def test_dcl_do(self):
        logger.info("------------------Opengauss_Function_DCL_Do_Case0003开始执行-----------------")
        logger.info("======前置条件，创建表，并查看指定语言是否支持内联块======")
        sql_cmd1 = f'''
                    drop table if exists do_test;
                    create table do_test(id int,name varchar(10));
                    --为0代表不支持
                    select laninline from pg_language where lanname='internal';
                    '''
        ...
        logger.info("======指定解析代码的程序语言为internal，利用do执行代码块======")
        sql_cmd2 = f'''
                    do language internal
                    \$\$
                    declare
                    sql_str varchar(100);
    ...
    def tearDown(self):
        ...
        logger.info(msg3)
        logger.info('---------------Opengauss_Function_DCL_Do_Case0003执行结束---------------')