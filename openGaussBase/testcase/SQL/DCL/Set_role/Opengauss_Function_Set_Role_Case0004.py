...
Description :
    ...
    3.使用role4_001连接gsql，执行set role语句，密码错误:设置失败，查询CURRENT_USER为role4_001
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Set_Role_Case0004开始执行-----------------------------')
    ...
    def test_common_user_permission(self):
        ...
        logger.info('----------使用role4_001连接gsql，执行set role语句，密码错误:设置失败，查询CURRENT_USER为role4_001---------')
        sql_cmd = (f'''
                    SET session ROLE role4_002 password '{macro.COMMON_PASSWD}.upper()';
        ...
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        self.assertIn("Invalid username/password,set role denied.", msg)
        self.assertIn("role4_001", msg)
        self.assertNotIn("role4_002", msg)
    ...
    def tearDown(self):
        ...
        sql_cmd = commonsh.execut_db_sql('''
                                        ...
                                        ''')
        logger.info(sql_cmd)
        logger.info('-------------------------Opengauss_Function_Set_Role_Case0004执行结束---------------------------')