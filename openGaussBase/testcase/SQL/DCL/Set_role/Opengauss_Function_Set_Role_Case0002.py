...
Description :
    ...
    3.使用role2_001连接gsql，执行set role语句，加lcoal期望:设置成功，查询CURRENT_USER为role2_001
    4.退出会话后，使用role2_001重新连接gsql，查看当前会话用户，当前用户。期望:SESSION_USER, CURRENT_USER均为role2_001
    5.使用role2_001连接gsql，执行set role语句，在事务中设置local 期望:设置成功，查询表table_set_role2_001的属主为role2_002，CURRENT_USER为role2_001
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Set_Role_Case0002开始执行-----------------------------')
    ...
    def test_common_user_permission(self):
        ...
        logger.info('----------使用role2_001连接gsql，执行set role语句，加lcoal期望:设置成功，查询CURRENT_USER为role2_001---------')
        sql_cmd = (f'''
                    SET local ROLE role2_002 password '{macro.COMMON_PASSWD}';
        ...
        logger.info('----------使用role2_001连接gsql，执行set role语句，在事务中设置local 期望:设置成功，查询表table_set_role2_001的属主为role2_002，CURRENT_USER为role2_001--------')
        sql_cmd = (f'''
                    START TRANSACTION;
                        SET local ROLE role2_002 password '{macro.COMMON_PASSWD}';
    ...
    def tearDown(self):
        ...
        sql_cmd = commonsh.execut_db_sql('''drop table if exists table_set_role2_001 cascade;drop role if exists role2_001;drop role if exists role2_002;''')
        logger.info(sql_cmd)
        logger.info('-------------------------Opengauss_Function_Set_Role_Case0002执行结束---------------------------')