...
Description :
    ...
    3.使用role5_001连接gsql，执行set role语句设置为role5_002 期望:设置成功，查询CURRENT_USER为role5_002
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Set_Role_Case0005开始执行-----------------------------')
    ...
    def test_common_user_permission(self):
        ...
        logger.info('----------使用role5_001连接gsql，执行set role语句设置为role5_002 期望:设置成功，查询CURRENT_USER为role5_002---------')
        sql_cmd = (f'''
                    SET session ROLE role5_002 password '{macro.COMMON_PASSWD}';
    ...
    def tearDown(self):
        ...
        sql_cmd = commonsh.execut_db_sql('''drop role if exists role5_001;drop role if exists role5_002;''')
        logger.info(sql_cmd)
        logger.info('-------------------------Opengauss_Function_Set_Role_Case0005执行结束---------------------------')