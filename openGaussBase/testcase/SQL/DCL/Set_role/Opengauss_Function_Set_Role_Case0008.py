...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Set_Role_Case0008开始执行-----------------------------')
    ...
    def test_common_user_permission(self):
        ...
        logger.info('----------使用role8_001连接gsql，执行set role语句设置为当前用户 期望:设置成功，查询CURRENT_USER为role8_001---------')
        sql_cmd = (f'''SET session ROLE role8_001 password '{macro.COMMON_PASSWD}';SELECT SESSION_USER, CURRENT_USER;''')
        excute_cmd = f'''
                        source {self.DB_ENV_PATH};
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -U role8_001 -W {macro.COMMON_PASSWD} -c "{sql_cmd}"
                    '''
        ...
        sql_cmd = (f'''
                    SET session ROLE role8_002 password '{macro.COMMON_PASSWD}';
    ...
    def tearDown(self):
        ...
        sql_cmd = commonsh.execut_db_sql('''
                                    drop role if exists role8_001;drop role if exists role8_002;
                                  ''')
        logger.info(sql_cmd)
        logger.info('-------------------------Opengauss_Function_Set_Role_Case0008执行结束---------------------------')
