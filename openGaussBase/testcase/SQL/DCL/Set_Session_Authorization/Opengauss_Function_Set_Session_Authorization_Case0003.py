...
Description :
    ...
    3.使用role3_001连接gsql，执行SET SESSION AUTHORIZATION 为初始用户:设置失败，查询SESSION_USER和CURRENT_USER为gs1026
    SET SESSION AUTHORIZATION gs1026 password "{macro.COMMON_PASSWD}";
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Set_Session_Authorization_Case0003开始执行-----------------------------')
    ...
    def test_common_user_permission(self):
        ...
        logger.info('---------使用role3_001连接gsql，执行SET SESSION AUTHORIZATION 为初始用户:设置失败，查询SESSION_USER和CURRENT_USER为gs1026----------')
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''
                                            ...
                                            ''')
        logger.info(sql_cmd)
        logger.info('-------------------------Opengauss_Function_Set_Session_Authorization_Case0003执行结束---------------------------')
