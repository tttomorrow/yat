...
Description :
    ...
    3.使用role13_001连接gsql，执行set role语句，加session 期望:设置成功，查询SESSION_USER和CURRENT_USER为"RORE13_002"，二次设置成功，查询SESSION_USER和CURRENT_USER为"RORE13_002"
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Set_Session_Authorization_Case0013开始执行-----------------------------')
    ...
    def test_common_user_permission(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'''
                                            ...
                                            drop role if exists \\"RORE13_002\\";
                                            create role role13_001 password '{macro.COMMON_PASSWD}';
                                            create role \\"RORE13_002\\" password '{macro.COMMON_PASSWD}';
                                            alter role role13_001 with login;
                                            alter role \\"RORE13_002\\" with login;
                                            grant all privileges to  role13_001;
                                            grant all privileges to  \\"RORE13_002\\";
                                            ''')
        logger.info(sql_cmd)
        ...
        logger.info('----------使用role13_001连接gsql，执行set role语句，加session 期望:设置成功，查询SESSION_USER和CURRENT_USER为"RORE13_002"，二次设置成功，查询SESSION_USER和CURRENT_USER为"RORE13_002"---------')
        sql_cmd = (f'''
                    SET SESSION SESSION AUTHORIZATION \\"RORE13_002\\" password '{macro.COMMON_PASSWD}';
                    SELECT SESSION_USER, CURRENT_USER;
                    SET SESSION SESSION AUTHORIZATION role13_001 password '{macro.COMMON_PASSWD}';
                    SELECT SESSION_USER, CURRENT_USER;
                    SET SESSION SESSION AUTHORIZATION \\"RORE13_002\\" password '{macro.COMMON_PASSWD}';
                    SELECT SESSION_USER, CURRENT_USER;
                    SET SESSION SESSION AUTHORIZATION role13_001 password '{macro.COMMON_PASSWD}';
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('''
                                            ...
                                            ''')
        logger.info(sql_cmd)
        logger.info('-------------------------Opengauss_Function_Set_Session_Authorization_Case0013执行结束---------------------------')
