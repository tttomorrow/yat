...
Description :
    ...
    4.使用role6_001连接gsql，执行set role语句设置为sysadmin:role6_002 期望:设置失败，查询SESSION_USER和CURRENT_USER为role6_001
    5.使用role6_001连接gsql，执行set role语句设置为非组内group:role6_003 期望:设置失败，查询SESSION_USER和CURRENT_USER为role6_001
    6.使用role6_001连接gsql，执行set role语句设置为初始用户gs1026 期望:设置失败，查询SESSION_USER和CURRENT_USER为role6_001
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_Set_Session_Authorization_Case0006开始执行-----------------------------')
    ...
    def test_common_user_permission(self):
        ...
        logger.info('----------使用role6_001连接gsql，执行SET SESSION AUTHORIZATION语句设置为sysadmin:role6_002 期望:设置失败，查询SESSION_USER和CURRENT_USER为role6_001---------')
        sql_cmd = (f'''
                    SET SESSION AUTHORIZATION role6_002 password '{macro.COMMON_PASSWD}';
                    SELECT SESSION_USER, CURRENT_USER;
                    ''')
        excute_cmd = f'''
                        source {self.DB_ENV_PATH};
        ...
        logger.info('----------使用role6_001连接gsql，执行set role语句设置为非组内group:role6_003 期望:设置失败，查询SESSION_USER和CURRENT_USER为role6_001---------')
        sql_cmd = (f'''
                    SET SESSION AUTHORIZATION role6_003 password '{macro.COMMON_PASSWD}';
                    SELECT SESSION_USER, CURRENT_USER;
                    ''')
        excute_cmd = f'''
                        source {self.DB_ENV_PATH};
        ...
        logger.info('---------使用role6_001连接gsql，执行set role语句设置为初始用户gs1026 期望:设置失败，查询SESSION_USER和CURRENT_USER为role6_001----------')
        sql_cmd = (f'''SET SESSION AUTHORIZATION \"{self.username}\" password '{self.password}';SELECT SESSION_USER, CURRENT_USER;''')
        excute_cmd = f'''
                        source {self.DB_ENV_PATH};
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -U role6_001 -W {macro.COMMON_PASSWD} -c "{sql_cmd}"
                        '''
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql("drop role "
            "if exists role6_001, role6_002, role6_003;"
            "drop group if exists group6_001, group6_002;")
        logger.info(sql_cmd)
        logger.info('-------------------------Opengauss_Function_Set_Session_Authorization_Case0006执行结束---------------------------')