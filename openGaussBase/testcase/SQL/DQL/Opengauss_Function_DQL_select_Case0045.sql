...
select lead(b.c2, 2,null) over(partition by b.c1 order by b.id),MIN(b.c2) over(partition by b.c1 order by b.id),listagg(a.sal,'|')WITHIN GROUP (order by a.id) over(partition by a.deptno ),MAX(b.c2) over(partition by b.c1 order by b.id) from t_agg_join_003 a left join t_agg_join_004 b on a.id2=b.id and a.sal=b.id order by 1,2;
