-- @testpoint: DQL语法，结合select...where多层嵌套
...
select sa.sname, sc.sno from sc sc,(select st.sname, s.* from stu st, (select sno, cno from sc where cno in ('c001')) s where st.sno = s.sno) sa
 where sc.sno = sa.sno and sc.cno = 'c002' order by 1,2;
