-- @testpoint: DQL语法，结合SELECT ...right..join右连接
...
select cname, scs.cno from course co right join (select cno from (select count(*) num from (select distinct sno from sc)) cs1,(select cno, count(cno) cns from sc group by cno) cs2 where cs2.cns = cs1.num) scs on (co.cno = scs.cno);
