-- @testpoint: DQL语法，结合SELECT ...left...join左连接
...
select co.cname, cns.cnum from course co left join (select cno, count(cno) cnum from sc group by cno) cns on (co.cno = cns.cno) order by cnum;
