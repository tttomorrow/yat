-- @testpoint: DQL语法，结合分析函数lag,LAST_VALUE
...
select lag(b.c2, 2,null) over(partition by b.c1 order by b.id),LAST_VALUE(a.sal) over(partition by a.deptno order by a.id)  from t_agg_join_003 a left join t_agg_join_004 b on a.id2=b.id and a.sal=b.id order by 1,2;
