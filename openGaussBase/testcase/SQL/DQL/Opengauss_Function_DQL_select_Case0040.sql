...
select count(b.c2) over(partition by b.c1 order by b.id),count(a.sal) over(partition by a.deptno order by a.id)  from t_agg_join_003 a right join t_agg_join_004 b on a.id2=b.id order by 1,2 limit 10;
select count(b.c2) over(partition by b.c1 order by b.id),count(a.sal) over(partition by a.deptno order by a.id)  from t_agg_join_003 a left join t_agg_join_004 b on a.id2=b.id order by 1,2 limit 10;
