...
Case Name   : revoke用户目标表的update权限，执行merge_into操作
Description :
    1.创建用户
    2.建表并插入数据
    3.revoke用户u_mergeinto_05对目标表的update权限
    4.u_mergeinto_05用户对表执行merge into操作
    5.清理环境
Expect      :
    1.建表成功且数据插入成功
    2.用户创建成功
...
class MergeInto(unittest.TestCase):
    def setUp(self):
        ...
        self.logger.info('Opengauss_Function_DML_Merge_Into_Case0005'
                         '开始执行')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.tb_name1 = 't_mergeinto_05_01'
        self.tb_name2 = 't_mergeinto_05_02'
        self.user = 'u_mergeinto_05'
    ...
    def test_mergeinto_permission(self):
        ...
        text3 = '--step3:revoke用户对目标表的update权限;expect:成功--'
        self.logger.info(text3)
        sql_cmd3 = self.commonsh.execut_db_sql(f'''
        revoke update on {self.tb_name1} from {self.user};
        ''', sql_type=f' -U {self.user} -W {macro.COMMON_PASSWD}')
        ...
        self.assertIn('ERROR:  permission denied for relation ' \
                      't_mergeinto_05_01', sql_cmd5, '执行失败:' + text4)
