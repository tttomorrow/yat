...
Case Name   : revoke用户源表select权限，执行merge_into操作
Description :
    1.创建用户
    2.建表并插入数据
    3.revoke用户u_mergeinto_04对源表select权限
    4.u_mergeinto_04用户对表执行merge into操作
    5.清理环境
Expect      :
    1.建表成功且数据插入成功
    2.用户创建成功
...
class MergeInto(unittest.TestCase):
    def setUp(self):
        ...
        self.logger.info('Opengauss_Function_DML_Merge_Into_Case0004'
                         '开始执行')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.tb_name1 = 't_mergeinto_04_01'
        self.tb_name2 = 't_mergeinto_04_02'
        self.user = 'u_mergeinto_04'
    ...
    def test_mergeinto_permission(self):
        ...
        text3 = '--step3:revoke用户对源表select权限;expect:成功--'
        ...
        self.assertIn('ERROR:  permission denied for relation ' \
                      't_mergeinto_04_02', sql_cmd5, '执行失败:' + text4)
