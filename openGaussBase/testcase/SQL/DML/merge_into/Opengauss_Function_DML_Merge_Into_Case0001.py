...
Case Name   : 用户同时拥有目标表的update和insert权限以及源表select权限，执行merge_into操作
Description :
    1.创建用户
    2.建表并插入数据
    3.赋予用户u_mergeinto_01对目标表的update和insert权限以及源表select权限
    4.u_mergeinto_01用户对表执行merge into操作
    5.清理环境
Expect      :
    1.用户创建成功
    2.建表成功且数据插入成功
...
class MergeInto(unittest.TestCase):
    def setUp(self):
        ...
        self.logger.info('Opengauss_Function_DML_Merge_Into_Case0001'
                         '开始执行')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.tb_name1 = 't_mergeinto_01_01'
        self.tb_name2 = 't_mergeinto_01_02'
        self.user = 'u_mergeinto_01'
    ...
    def test_mergeinto_permission(self):
        ...
        insert into {self.tb_name1} values (1502, 'olympus is50', 
                                                        'electrncs');
        insert into {self.tb_name1} values (1600, 'play gym', 
                                                            'toys');
        ...
        text3 = '--step3:赋予用户对目标表的update和insert权限以及源表select权限;expect:成功--'
        self.logger.info(text3)
        sql_cmd3 = self.commonsh.execut_db_sql(f'''
        grant update,insert on {self.tb_name1} to {self.user};
        grant select on {self.tb_name2} to {self.user};''',
                        sql_type=f' -U {self.user} -W {macro.COMMON_PASSWD}')
        self.logger.info(sql_cmd3)
        self.assertIn(self.constant.GRANT_SUCCESS_MSG, sql_cmd3,
                      '执行失败:' + text3)

        text4 = '--step4:进行merge into操作;expect:成功--'
        ...
        self.assertEqual('MERGE 4', sql_cmd5, '执行失败:' + text4)
