...
"""
'''
--  @testpoint:使用analyze收集普通表的统计信息,限定列
'''
import sys
import unittest
...
class SYS_Operation(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Set_Case0095开始执行-----------------------------')
...
insert into customer_info values(generate_series(1,200),'a');''')
        ...
        # 使用ANALYZE语句；查询系统表PG_STATS，只显示WR_RETURNED_DATE_SK列的数据
        sql_cmd2 = commonsh.execut_db_sql('''ANALYZE customer_info(WR_RETURNED_DATE_SK);
        ANALYZE verbose customer_info(WR_RETURNED_DATE_SK);
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DML_Set_Case0095执行结束--------------------------')
