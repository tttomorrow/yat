...
"""
'''
--  @testpoint:索引表损坏,使用analyze分析索引，合理报错
'''
import sys
import unittest
...
class SYS_Operation(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Set_Case0106开始执行-----------------------------')
    ...
    def test_analyze(self):
       ...
       CREATE TABLE customer_info(WR_RETURNED_DATE_SK INTEGER ,WR_RETURNED_TIME_name varchar(200) );''')
        ...
        drop index WR_RETURNED_DATE_SK_uni;
        analyze VERIFY fast WR_RETURNED_DATE_SK_uni;
       ''')
        self.assertIn(constant.CREATE_INDEX_SUCCESS_MSG, sql_cmd2)
        self.assertIn('ERROR:  relation "wr_returned_date_sk_uni" does not exist', sql_cmd2)
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DML_Set_Case0106执行结束--------------------------')
