...
'''
--  @date:2020/11/9
--  @testpoint:vacuum full在事务内执行，合理报错
'''
import sys
import unittest
...
class SYS_Operation(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Set_Case0074开始执行-----------------------------')
    ...
    def test_vacuum(self):
      ...
      insert into customer_info values (generate_series(1,2000),'a');''')
        ...
        # 修改表数据
        sql_cmd2 = commonsh.execut_db_sql('''select n_tup_ins,n_tup_upd,n_tup_del,n_live_tup,n_dead_tup,last_vacuum,last_data_changed from pg_stat_all_tables where relname='customer_info';
      update customer_info set WR_RETURNED_TIME_name='c' where WR_RETURNED_DATE_SK >1000 and WR_RETURNED_DATE_SK<2000;''')
        ...
        self.assertIn(constant.VACUUM_TRANSACTION_FAIL_MSG, sql_cmd3)
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DML_Set_Case0074执行结束--------------------------')
