...
'''
--  @date:2020/11/9
--  @testpoint:对于列存表执行VACUUM操作，指定了VERBOSE选项
'''
import sys
import unittest
...
class SYS_Operation(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Set_Case0075开始执行-----------------------------')
    ...
    def test_vacuum(self):
      ...
      update customer_info set WR_RETURNED_TIME_name='c' where WR_RETURNED_DATE_SK >1000 and WR_RETURNED_DATE_SK<2000;''')
        ...
        # 执行vacuum命令，显示列存表相关表信息输出，无customer_info表信息
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DML_Set_Case0075执行结束--------------------------')
