...
'''
--  @date:2020/11/9
--  @testpoint:修改参数vacuum_defer_cleanup_age，执行vacuum命令
'''
import sys
import unittest
...
class SYS_Operation(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Set_Case0120开始执行-----------------------------')

    def test_index(self):
        # 查看参数默认值并修改参数值
        sql_cmd1 = commonsh.execut_db_sql('''show vacuum_defer_cleanup_age;
        ALTER SYSTEM SET vacuum_defer_cleanup_age TO 1000000;''')
        logger.info(sql_cmd1)
        self.assertIn('0', sql_cmd1)
        self.assertIn(constant.ALTER_SYSTEM_SUCCESS_MSG, sql_cmd1)
        # 建表并插入数据；对表数据进行update操作,死行数为999
    ...
    def tearDown(self):
        ...
        # 恢复参数默认值并删除表
        sql_cmd3 = commonsh.execut_db_sql('''ALTER SYSTEM SET vacuum_defer_cleanup_age TO 0;
        drop table customer_info;''')
        logger.info(sql_cmd3)
        logger.info('------------------------Opengauss_Function_DML_Set_Case0120执行结束--------------------------')
