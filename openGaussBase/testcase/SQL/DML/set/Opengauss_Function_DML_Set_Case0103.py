...
'''
--  @date:2020/11/9
--  @testpoint:analyze命令，对索引表index使用CASCADE操作,合理报错
'''
import sys
import unittest
...
class SYS_Operation(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Set_Case0103开始执行-----------------------------')
    ...
    def test_analyze(self):
       ...
       CREATE TABLE customer_info(WR_RETURNED_DATE_SK INTEGER ,WR_RETURNED_TIME_name varchar(200) );''')
        ...
        # 创建索引并对索引表执行analyze ...cascade，合理报错
        ...
        ANALYZE VERIFY fast WR_RETURNED_DATE_SK_uni CASCADE;
        ANALYZE VERIFY COMPLETE WR_RETURNED_DATE_SK_uni CASCADE;''')
        self.assertIn(constant.CREATE_INDEX_SUCCESS_MSG, sql_cmd2)
        self.assertIn(constant.ANALYZE_CASCADE_FAIL_MSG, sql_cmd2)
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DML_Set_Case0103执行结束--------------------------')
