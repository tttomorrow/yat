...
"""
'''
--  @testpoint:使用analyze收集分区表的统计信息，添加 PARTITION参数
'''
import sys
import unittest
...
class SYS_Operation(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Set_Case0097开始执行-----------------------------')
    ...
    def test_analyze(self):
       ...
       insert into customer_par values(generate_series(2000000,3000000),generate_series(2000000,3000000),generate_series(2000000,3000000),generate_series(2000000,3000000));''')
        ...
        # 手动分析p1分区并查看上一次手动ANALYZE的时间
        sql_cmd2 = commonsh.execut_db_sql('''ANALYZE customer_par PARTITION(p1);
        select relname,last_autovacuum,last_analyze from pg_stat_user_tables where relname='customer_par';
        ANALYZE verbose customer_par PARTITION(p1);
        ''')
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DML_Set_Case0097执行结束--------------------------')
