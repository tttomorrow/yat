...
'''
--  @date:2020/11/9
--  @testpoint:行存表，执行VACUUM DELTAMERGE命令，合理报错
'''
import sys
import unittest
...
class SYS_Operation(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Set_Case0114开始执行-----------------------------')
    ...
    def test_analyze(self):
       ...
       CREATE TABLE customer_info(WR_RETURNED_DATE_SK INTEGER ,WR_RETURNED_TIME_name varchar(200));''')
        ...
        self.assertIn(constant.VACUUM_FAIL_MSG, sql_cmd3)
    ...
    def tearDown(self):
        ...
        logger.info(sql_cmd3)
        logger.info('------------------------Opengauss_Function_DML_Set_Case0114执行结束--------------------------')
