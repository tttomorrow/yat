...
"""
Case Type   : DML
Case Name   : share锁与share row exclusive锁并发
Description :
    1.创建测试表并插入数据
    2.开启session1，对表加share锁，不做提交
    3.不关闭session1，开启session2，对表加share row exclusive锁
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.开启session1，对表加share锁,不做提交操作成功
...
class LockTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DML_Lock_Case0043 开始执行')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.tb_name = 't_lock_0043'
    ...
    def test_dml_lock(self):
        ...
        text2 = '-step2: 开启session1，对表加share锁，不做提交;' \
                ' expect: 加锁成功-'
        self.log.info(text2)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.DROP_TABLE_SUCCESS, sql_cmd,
                      '执行失败:' + text)
        self.log.info('Opengauss_Function_DML_Lock_Case0043 执行结束')
