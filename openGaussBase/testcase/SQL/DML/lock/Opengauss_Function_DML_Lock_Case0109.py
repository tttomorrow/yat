...
"""
Case Type   : 锁定表
Case Name   : 对表进行drop时是否产生RowExclusiveLock锁
Description :
    1.创建测试表并插入数据
    2.统计锁信息
    3.开启事务，对测试表进行删除，不做提交,统计锁信息
    4.进行校验
Expect      :
    1.创建测试表并插入数据成功
    2.统计锁信息
...
class LockFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Lock_Case0109开始执行-----------------------------')
    ...
    def tearDown(self):
        ...
        logger.info(msg)
        self.assertIn(self.Constant.TABLE_DROP_SUCCESS, msg)
        logger.info('-----------------------Opengauss_Function_DML_Lock_Case0109执行完成-----------------------------')
