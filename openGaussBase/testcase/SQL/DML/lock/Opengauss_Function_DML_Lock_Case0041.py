...
"""
Case Type   : DML
Case Name   : share update exclusive锁与exclusive锁并发
Description :
    1.创建测试表并插入数据
...
class LockTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DML_Lock_Case0041 开始执行')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.tb_name = 't_lock_0041'
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.DROP_TABLE_SUCCESS, sql_cmd,
                      '执行失败:' + text)
        self.log.info('Opengauss_Function_DML_Lock_Case0041 执行结束')
