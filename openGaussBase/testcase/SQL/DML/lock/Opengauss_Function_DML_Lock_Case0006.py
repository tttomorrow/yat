...
"""
Case Type   : DML
Case Name   : exclusive是否允许对目标表进行并发查询
Description :
    1.创建测试表并插入数据
    2.开启事务，对表加EXCLUSIVE锁，然后进行select操作，不做提交
    3.开启新的事务，对表进行select操作，不做提交
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.开启事务，对表加exclusive锁，然后进行select操作，不做提交操作成功
    3.开启新的事务，对表进行select操作，不做提交成功，exclusive支持并发查询
    4.清理环境成功
History     :
"""
import time
...
class DmlTestCase(unittest.TestCase):
    ...
    def test_dml_lock(self):
        ...
        text3 = '-----step3: 开启新的事务，对表进行select操作，' \
                '不做提交; expect: exclusive支持并发查询-----'
        self.log.info(text3)
        sql = f"start transaction;" \
              f"select * from {self.tb_name} ;"
        ...
        self.assertTrue(assert_eq_1, '执行失败:' + text2)
        self.assertIn('  1 | sk1              | tt   |  3332', msg_result_1,
                      '执行失败:' + text2)
        assert_eq_2 = msg_result_2.count('START TRANSACTION') is 1
        self.assertTrue(assert_eq_2, '执行失败:' + text3)
        self.assertIn('  1 | sk1              | tt   |  3332', msg_result_2,
                      '执行失败:' + text3)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0006结束')
