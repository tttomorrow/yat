...
"""
Case Type   : DML
Case Name   : exclusive锁与access exclusive锁并发
Description :
    1.创建测试表并插入数据
    2.开启session1，对表加exclusive锁，不做提交
...
class LockTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DML_Lock_Case0050 开始执行')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.tb_name = 't_lock_0050'
    ...
    def test_dml_lock(self):
        ...
        text2 = '-step2: 开启session1，对表加exclusive锁，不做提交;' \
                ' expect: 加锁成功-'
        self.log.info(text2)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.DROP_TABLE_SUCCESS, sql_cmd,
                      '执行失败:' + text)
        self.log.info('Opengauss_Function_DML_Lock_Case0050 执行结束')
