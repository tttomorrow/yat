...
"""
Case Type   : DML
Case Name   : access share锁与share锁并发
Description :
    1.创建测试表并插入数据
...
class DmlTestCase(unittest.TestCase):
    ...
    def test_dml_lock(self):
        ...
        text3 = '-step3: 开启新的事务，对表加share锁操作成功; ' \
                'expect: 加锁成功，事务未阻塞-'
        self.log.info(text3)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0021结束')
