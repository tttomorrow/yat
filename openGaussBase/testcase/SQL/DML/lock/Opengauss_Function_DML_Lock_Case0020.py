...
"""
Case Type   : DML
Case Name   : access share锁与share update exclusive锁并发
Description :
    1.创建测试表并插入数据
...
class DmlTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0020结束')
