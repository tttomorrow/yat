...
class LockFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Lock_Case0106开始执行-----------------------------')
    ...
    def test_lock_file(self):
        ...
        sql_cmd = '''select count(*) from PG_LOCKS where mode = 'ShareRowExclusiveLock'; '''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
        ...
        sql_cmd = '''
                    ...
                    commit;
                    select count(*) from PG_LOCKS where mode = 'ShareRowExclusiveLock';
                    '''
        excute_cmd = f'''
                    source {self.DB_ENV_PATH} ;
                    gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_DML_Lock_Case0106执行完成-----------------------------')
