...
"""
Case Type   : 锁定表
Case Name   : 对表进行查询时是否产生AccessShareLock锁
Description :
    1.创建测试表并插入数据
    2.查看视图PG_LOCKS，统计锁信息
    3.开启事务，对数据库表加锁并统计锁信息
    4.进行校验
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.查看视图PG_LOCKS，统计锁信息成功
    3.查看视图PG_THREAD_WAIT_STATUS与PG_LOCKS，显示产生AccessShareLock锁
    4.校验成功
    5.清理和检察官
"""
...
class LockFile(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Lock_Case0107开始执行------------------------')
    ...
    def test_lock_file(self):
        ...
        sql_cmd = '''select count(*) from PG_LOCKS where mode = 'AccessShareLock'; '''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
        ...
        logger.info('-----------------开启事务，对数据库表加锁并统计锁信息----------------')
        sql_cmd = '''
                    start transaction;
                    select * from testzl;
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_DML_Lock_Case0107执行完成-----------------------------')
