...
"""
Case Type   : DML
Case Name   : row exclusive锁发生时进行更新
Description :
    1. 创建测试表并插入数据
    2. 开启事务，对表加row exclusive锁,不做提交
    3. 开启新的事务，进行更新操作
    4. 清理环境
Expect      :
    1. 创建测试表并插入数据成功
    2. 开启事务，对表加row share锁,不做提交，进行查询操作成功
    3. 开启新的事务，进行更新操作成功，更新成功
    4. 清理环境
History     :
"""
import unittest
...
class DmlTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DML_Lock_Case0012开始')
        self.constant = Constant()
        self.commonsh1 = CommonSH('PrimaryDbUser')
        self.commonsh2 = CommonSH('PrimaryDbUser')
        self.tb_name = 't_lock_0012'
        
    def test_dml_lock(self):
        text1 = '-----step1: 创建测试表并插入数据; expect: 创建表并插入数据成功-----'
        self.log.info(text1)
        ...
        text2 = '-step2: 开启事务，对表加row exclusive锁,不做提交; expect: 执行成功-'
        self.log.info(text2)
        ...
        text3 = '-----step3: 开启新的事务，进行更新操作; expect: 更新成功-----'
        self.log.info(text3)
        ...
        self.assertIn('UPDATE 1', msg_result_2, '执行失败:' + text3)
        self.assertIn('1 | sk5              | tt   |  3332',
                      msg_result_2, '执行失败:' + text3)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0012结束')
