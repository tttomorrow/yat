...
"""
Case Type   : DML
Case Name   : 对表上row share锁后，进行更新操作
Description :
    1. 创建测试表并插入数据
    2. 开启事务，对表加row share锁,不做提交，进行查询操作
    3. 开启新的事务，进行更新操作
    4. 清理环境
Expect      :
    1. 创建测试表并插入数据成功
...
class DmlTestCase(unittest.TestCase):
    ...
    def test_dml_lock(self):
        ...
        text2 = '-step2: 开启事务，对表加row share锁,不做提交，进行查询操作; expect: 查询成功-'
        self.log.info(text2)
        ...
        text3 = '-----step3: 开启新的事务，进行update操作; expect: 执行成功-----'
        self.log.info(text3)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0010结束')
