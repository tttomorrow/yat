...
"""
Case Type   : DML
Case Name   : row exclusive锁与share锁并发
Description :
    1.创建测试表并插入数据
...
Expect      :
    ...
    3.不关闭session1，开启session2，，对表加share锁操作失败，session2堵塞，
    session1结束后，加锁成功
    4.清理环境成功
History     :
"""
...
class LockTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DML_Lock_Case0034 开始执行')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.tb_name = 't_lock_0034'
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.DROP_TABLE_SUCCESS, sql_cmd,
                      '执行失败:' + text)
        self.log.info('Opengauss_Function_DML_Lock_Case0034 执行结束')
