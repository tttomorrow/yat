...
Description :
    1.创建测试表并插入数据
    2.开启事务，对表加share锁，不做提交
    3.开启新的事务，对表加share row exclusive锁
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.开启事务，对表加share锁，不做提交成功
    3.开启新的事务，对表加share row exclusive锁操作失败，事务堵塞
    4.清理环境成功
History     :
"""
import datetime
...
class DmlTestCase(unittest.TestCase):
    ...
    def test_dml_lock(self):
        ...
        text2 = '-step2: 开启事务，对表加share锁; expect: 加锁成功-'
        self.log.info(text2)
        ...
        text3 = '-step3: 开启新的事务，对表加share row exclusive锁; ' \
                'expect: 事务阻塞，加锁失败，事务阻塞,待session1结束后，session2加锁成功-'
        self.log.info(text3)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0023结束')
