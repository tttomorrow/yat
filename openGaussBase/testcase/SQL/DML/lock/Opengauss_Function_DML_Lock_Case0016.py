...
"""
Case Type   : DML
Case Name   : 数据库表加access exclusive锁后进行更新
Description :
    1.创建测试表并插入数据
    2.开启事务，对表加access exclusive锁,不做提交
    3. 开启新的事务，对表进行更新,不做提交
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.开启事务，对表加access exclusive锁,不做提交操作成功
    3.开启新的事务，对表进行更新失败，事务堵塞
    4.清理环境成功
History     :
"""
import unittest
...
class DmlTestCase(unittest.TestCase):
    ...
    def test_dml_lock(self):
        ...
        text2 = '-----step2: 开启事务，对表加access exclusive锁，' \
        ...
        self.assertTrue(
            msg_result_1.count('1 | sk1              | tt   |  3332') is 2,
            '执行失败:' + text2)
        self.assertIn(self.constant.START_TRANSACTION_SUCCESS_MSG,
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0016结束')
