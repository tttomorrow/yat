...
"""
Case Type   : DML
Case Name   : access share锁与access exclusive锁并发
Description :
    1.创建测试表并插入数据
    2.开启事务，对表加access share锁，不做提交
    3.开启新的事务，对表加access exclusive锁
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.开启事务，对表加access share锁，不做提交成功
    3.开启新的事务，对表加access exclusive锁失败，事务阻塞
    4.清理环境成功
History     :
"""
import datetime
...
class DmlTestCase(unittest.TestCase):
    ...
    def test_dml_lock(self):
        ...
        text2 = '-step2: 开启事务，对表加access share锁，不做提交; expect: 加锁成功-'
        self.log.info(text2)
        ...
        text3 = '-step3: 开启新的事务，对表加access exclusive锁; ' \
                'expect: 加锁失败，事务阻塞,待session1结束后，session2加锁成功-'
        self.log.info(text3)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0024结束')
