...
"""
Case Type   : DML
Case Name   : access share与row exclusive并发
Description :
    1.创建测试表并插入数据
...
Expect      :
    ...
    3.开启新的事务，对表加row exclusive锁成功，事务未堵塞
    4.清理环境成功
History     :
"""
import unittest
...
class DmlTestCase(unittest.TestCase):
    ...
    def test_dml_lock(self):
        ...
        text3 = '-step3: 开启新的事务，对表加row exclusive锁; expect: 加锁成功，事务未阻塞-'
        self.log.info(text3)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0019结束')
