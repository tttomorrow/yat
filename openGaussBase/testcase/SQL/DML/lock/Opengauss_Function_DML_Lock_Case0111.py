...
"""
Case Type   : 锁定表
Case Name   : 对表进行delete时是否产生RowExclusiveLock锁
Description :
    1. 创建测试表并插入数据
    2.统计锁信息
    3.开启事务，对测试表进行delete，不做提交,统计锁信息
    4.进行校验
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.统计锁信息成功
    3.开启事务，对测试表进行delete，不做提交操作成功
    4.校验成功，显示产生RowExclusiveLock锁
    5.清理环境成功
"""
...
class LockFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Lock_Case0111开始执行-----------------------------')
    ...
    def test_lock_file(self):
        ...
        sql_cmd = '''
                    start transaction;
                    delete from testzl where sk = 001;
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_DML_Lock_Case0111执行完成-----------------------------')
