...
"""
Case Type   : 锁定表
Case Name   : 对表进行SELECT FOR UPDATE是否会对测试表请求ROW SHARE锁
Description :
    1.创建测试表并插入数据
    2.统计锁信息
    3.开启事务，对测试表进行SELECT FOR UPDATE，不做提交,统计锁信息
    4.进行校验
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.统计锁信息成功
    3.开启事务，对测试表进行SELECT FOR UPDATE，不做提交,统计锁信息成功
    4.进行校验成功
    5.清理环境成功
"""
...
class LockFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Lock_Case0110开始执行-----------------------------')
    ...
    def test_lock_file(self):
        ...
        sql_cmd = '''select count(*) from PG_LOCKS where mode = 'RowShareLock'; '''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
        ...
        logger.info('-----------------开启事务，对数据库表执行forupdate并统计锁信息----------------')
        sql_cmd = '''
                    start transaction;
                    select * from testzl where sk = 1 for update;
                    select count(*) from PG_LOCKS where mode = 'RowShareLock';
                    '''
        excute_cmd = f'''
                    source {self.DB_ENV_PATH} ;
                    gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_DML_Lock_Case0110执行完成-----------------------------')
