...
"""
Case Type   : DML
Case Name   : share update exclusive锁与share update exclusive锁并发
Description :
    1.创建测试表并插入数据
    2.开启session1，对表加share update exclusive锁，不做提交
    3.不关闭session1，开启session2，对表加share update exclusive锁
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.开启session1，对表加share update exclusive锁,不做提交操作成功
    3.不关闭session1，开启session2，，对表加share update exclusive锁操作失败，session2阻塞，
    session1结束后，加锁成功
    4.清理环境成功
History     :
"""
...
class LockTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DML_Lock_Case0038 开始执行')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.tb_name = 't_lock_0038'
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.DROP_TABLE_SUCCESS, sql_cmd,
                      '执行失败:' + text)
        self.log.info('Opengauss_Function_DML_Lock_Case0038 执行结束')
