...
"""
Case Type   : DML
Case Name   : 数据库表加access share锁后进行并发查询
Description :
    1.创建测试表并插入数据
...
class DmlTestCase(unittest.TestCase):
    ...
    def test_dml_lock(self):
        ...
        text2 = '-step2: 开启事务，对表加access share锁,不做提交，进行查询操作; expect: 加锁成功-'
        self.log.info(text2)
        ...
        text3 = '--step3: 开启新的事务，对表进行查询; expect: 事务未堵塞，查询成功--'
        self.log.info(text3)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh1.execut_db_sql(
            f"drop table if exists {self.tb_name};")
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DML_Lock_Case0015结束')
