...
"""
Case Type   : 锁定表
Case Name   : 对表进行ALTER时是否产生AccessExclusiveLock锁
Description :
    1. 创建测试表并插入数据
    2.查看视图PG_LOCKS，统计锁信息
    3.开启事务，对测试表进行ALTER，不做提交,统计锁信息
    4.进行校验
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
...
class LockFile(unittest.TestCase):
    def setUp(self):
        logger.info('-------------------------Opengauss_Function_DML_Lock_Case0112开始执行---------------------------')
    ...
    def test_lock_file(self):
        ...
        sql_cmd = '''
                    start transaction;
                    alter table testzl rename to test1;
                    select count(*) from PG_LOCKS where mode = 'AccessExclusiveLock' and locktype = 'relation';
                    '''
        excute_cmd = f'''
                    source {self.DB_ENV_PATH} ;
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_DML_Lock_Case0112执行完成-----------------------------')
