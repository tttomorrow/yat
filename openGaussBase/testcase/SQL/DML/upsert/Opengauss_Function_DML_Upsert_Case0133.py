...
Case Name   : upsert使用子查询（子查询表为普通表）支持GPC验证；
Description :
...
class UpsertCase133(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_DML_Upsert_Case0133:初始化----')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.t1 = 't_dml_upsert0133'
        self.t2 = 't_dml_upsert_sub0133'
    ...
    def test_main(self):
        ...
        self.log.info(step_txt)
        prepare_sql = f'prepare gpc_tb_pre1(int, int) as ' \
        ...
        self.log.info(step_txt)
        prepare_sql = f'prepare gpc_tb_pre2(int, int) as ' \
            f'insert into {self.t1} values(\$1,\$1) on duplicate key ' \
            f'update j = (select j from {self.t2} where i =\$1 );' \
        ...
        step_txt = '----step8: prepare执行，子查询中不包含参数，update不存在冲突 expect:执行成功----'
        self.log.info(step_txt)
        prepare_sql = f'prepare gpc_tb_pre3(int, int) as ' \
            f'insert into {self.t1} values(\$1,\$1) on duplicate key ' \
            f'update j = (select j from {self.t2} where i =1 );' \
        ...
        step_txt = '----step10: prepare执行，子查询中不包含参数，update存在冲突 expect:执行成功----'
        self.log.info(step_txt)
        prepare_sql = f'prepare gpc_tb_pre4(int, int) as ' \
            f'insert into {self.t1} values(\$1,\$1) on duplicate key ' \
    ...
    def tearDown(self):
        ...
        self.log.info('----Opengauss_Function_DML_Upsert_Case0133:用例执行完毕----')
