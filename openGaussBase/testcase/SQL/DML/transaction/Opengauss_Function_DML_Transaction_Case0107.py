...
"""
Case Type   : 事务控制
Case Name   : 设置session会话的事务隔离级别为REPEATABLE READ，对表进行插入操作后不提交，新起session设置事务隔离级别为REPEATABLE READ，查询数据
Description :
    1.新建测试表
    2.开启事务并执行事务，执行ROLLBACK TRANSACTION命令对事务进行回滚
    3.查看数据是否回滚
    4.清理环境
Expect      :
    1.新建测试表成功
    2.开启事务并执行事务，执行ROLLBACK TRANSACTION命令对事务进行回滚成功
    3.查看数据回滚成功
    4.清理环境成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0107开始执行--------------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0107执行完成--------------------')
