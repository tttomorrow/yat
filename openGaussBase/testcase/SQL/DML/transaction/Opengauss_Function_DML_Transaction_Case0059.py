...
"""
Case Type   : 事务控制
Case Name   : start transaction与end配合使用，执行事务后查看备机数据
Description :
    1.创建测试表
    2.开启事务
    3.使用END进行提交
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0059开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            logger.info('--------------开启事务并执行事务后使用end进行提交-------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0059执行完成--------------------')
