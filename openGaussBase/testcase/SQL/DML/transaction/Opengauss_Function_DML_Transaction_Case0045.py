...
"""
Case Type   : 事务控制
Case Name   : 开启事务时添加参数LOCAL，指定事务隔离级别为READ COMMITTED，访问模式为READ WRITE,执行事务后查看备机数据
Description :
    1.新建测试表
    2.开启事务,
    3.设置事务的隔离级别为READ COMMITTED，访问模式为READ WRITE,并指定LOCAL声明该命令只在当前事务中有效
    4.在事务中对数据库表插入一条数据后执行COMMIT命令进行提交
    5.查看备机数据是否同步
Expect      :
    1.创建测试表成功
    2.开启事务成功
    3.设置事务的隔离级别为READ COMMITTED，访问模式为READ WRITE,并指定LOCAL声明该命令只在当前事务中有效成功
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('------------------Opengauss_Function_DML_Transaction_Case0045开始执行----------------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0045执行完成-----------------------')
