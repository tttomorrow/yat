...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0025开始执行------------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            logger.info('-------------在匿名块中写入货币类型的变量，对数据库表执行插入操作-------------')
            sql_cmd = f'''  declare 
                              a money;
                            begin
    ...
    def tearDown(self):
        ...
        logger.info('---------------------Opengauss_Function_DML_Transaction_Case0025执行完成---------------------')
