...
"""
Case Type   : 事务控制
Case Name   : 开启事务时不添加参数LOCAL，指定事务隔离级别为READ COMMITTED，访问模式为READ WRITE,执行事务后查看备机数据
Description :
    1.新建测试表
    2.开启事务
    3.设置事务的隔离级别为READ COMMITTED，访问模式为READ WRITE
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0049开始执行-----------------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0049执行完成--------------------')
