...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0065开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            logger.info('----------------------使用BEGIN开启事务并执行事务后使用end与斜杠提交--------------------------')
            vars = '/'
            sql_cmd = f'''begin;
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0065执行完成--------------------')
