...
"""
Case Type   : 事务控制
Case Name   : 匿名块中是否支持数组函数
Description :
    1.创建测试表
    2.将数组函数处理后的数据插入到数据库表中
    3.查看备机数据同步
    4.清理环境
Expect      :
    1.创建测试表成功
    2.将数组函数处理后的数据插入到数据库表中成功
    3.查看备机数据同步成功
    4.清理环境成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('-------------------------Opengauss_Function_DML_Transaction_Case0096开始执行-------------------------')
    ...
    def test_transaction_file(self):
        ...
        logger.info('--------------将数组函数处理后的数据插入到数据库表中------------')
        sql_cmd = '''begin
                        insert into testzl values (array_append(ARRAY[1,2], 3));
                      end;
                      '''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
    ...
    def tearDown(self):
        ...
        logger.info('-------------------------Opengauss_Function_DML_Transaction_Case0096执行完成--------------------')
