...
"""
Case Type   : 事务控制
Case Name   : 输入字符类型的变量，匿名块正常执行,匿名块中事务是否同步到主机
Description :
    1.新建测试表
    2.在匿名块中写入字符类型的变量，对数据库表执行插入操作
    3.查看主机事务数据
    4.查看备机事务数据
Expect      :
    1.创建测试表成功
    2.在匿名块中写入字符类型的变量，对数据库表执行插入操作成功
    3.主机事务数据同步成功
    4.备机事务数据同步成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info(
            '--------------------Opengauss_Function_DML_Transaction_Case0027开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            sql_cmd = '''drop table if exists testzl;
                         create table testzl(sk char,sk1 varchar,sk2 varchar2,sk3 nvarchar2,sk4 clob,sk5 text); '''
            excute_cmd = f'''
            ...
            logger.info('-------------在匿名块中写入字符类型的变量，对数据库表执行插入操作-------------')
            sql_cmd = f'''  declare 
                              a CHAR;
                              b VARCHAR;
                              c VARCHAR2;
    ...
    def tearDown(self):
        ...
        logger.info('---------------------Opengauss_Function_DML_Transaction_Case0027执行完成------------------------')
