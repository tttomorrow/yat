...
"""
Case Type   : 事务控制
Case Name   : 匿名块变量中是否支持SEQUENCE函数
Description :
    1.创建测试表
    2.将SEQUENCE函数处理后的数据插入到数据库表中
    3.查看备机数据同步
    4.清理环境
Expect      :
    1.创建测试表成功
    2.将SEQUENCE函数处理后的数据插入到数据库表中成功
    3.查看备机数据同步成功
    4.清理环境成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('-----------------------Opengauss_Function_DML_Transaction_Case0079开始执行--------------------')
        self.PrimaryNode = Node('PrimaryDbUser')
        self.DB_ENV_PATH = macro.DB_ENV_PATH
        self.Constant = Constant()

    def test_transaction_file(self):
        logger.info('------------------------新建测试表和SEQUENCE-------------------------')
        ...
        logger.info('--------------将SEQUENCE函数处理后的数据插入到数据库表中-------------')
        sql_cmd = '''declare 
                        i varchar;
                      begin
                        i := 'serial';
                        insert into testzl values (nextval(i));
                      end;
                      '''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
    ...
    def tearDown(self):
        ...
        msg = self.PrimaryNode.sh(excute_cmd).result()
        logger.info(msg)
        logger.info('-----------------------Opengauss_Function_DML_Transaction_Case0079执行完成---------------------')
