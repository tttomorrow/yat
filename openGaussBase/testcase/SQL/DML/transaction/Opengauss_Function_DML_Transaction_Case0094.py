...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0094开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        sql_cmd = '''
                      begin
                        insert into testzl values (array_to_json('{{{{1,5}},{{99,100}}}}'::int[]));
                      end;
                      '''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_DML_Transaction_Case0094执行完成-----------------------')
