...
"""
Case Type   : 事务控制
Case Name   : 开启事务执行insert语句后是否可以再次设置事务隔离级别
Description :
    1.创建测试表
    2.以默认方式开启事务后执行insert语句
    3.重新开启事务隔离级别为REPEATABLE READ的事务隔离级别
Expect      :
    1.创建测试表成功
    2.以默认方式开启事务后执行insert语句成功
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('----Opengauss_Function_DML_Transaction_Case0019开始执行----')
    ...
    def test_transaction_file(self):
        ...
        logger.info('以默认方式开启数据库进行insert后重新开启事务隔离级别为REPEATABLE READ的事务隔离级别')
    ...
    def tearDown(self):
        ...
        logger.info('----Opengauss_Function_DML_Transaction_Case0019执行完成----')
