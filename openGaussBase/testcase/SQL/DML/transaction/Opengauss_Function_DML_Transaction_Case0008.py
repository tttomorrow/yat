...
"""
Case Type   : 事务控制
Case Name   : 在隔离级别设为READ COMMITTED的事务中是否可以查询到隔离级别设为
              REPEATABLE READ的事务提交的事务数据，备机数据是否同步
Description :
    1.创建测试表
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_DML_Transaction_Case0008开始执行----')
        self.Standby_SH = CommonSH('Standby1DbUser')
        self.constant = Constant()
        self.t_name = 't_dml_transaction_case0008'
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_DML_Transaction_Case0008执行完成----')
