...
"""
Case Type   : 事务控制
Case Name   : begin与end配合使用，执行事务后查看备机数据
Description :
    1.创建测试表
    2.使用BEGIN开启事务
    3.使用END进行提交
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('----Opengauss_Function_DML_Transaction_Case0060开始执行-----')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            logger.info('------使用BEGIN开启事务后使用end进行提交------')
            sql_cmd = f'''BEGIN;
    ...
    def tearDown(self):
        ...
        logger.info('----Opengauss_Function_DML_Transaction_Case0060执行完成----')
