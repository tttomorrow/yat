...
"""
Case Type   : 事务控制
Case Name   : begin与commit配合使用，执行事务后查看备机数据
Description :
    1.创建测试表
    2.使用BEGIN开启事务
    3.使用COMMIT进行提交
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0061开始执行--------------------')
    ...
    def tearDown(self):
        ...
        msg = self.PrimaryNode.sh(excute_cmd).result()
        logger.info(msg)
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0061执行完成--------------------')
