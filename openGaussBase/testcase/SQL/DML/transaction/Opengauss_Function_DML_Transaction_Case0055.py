...
"""
Case Type   : 事务控制
Case Name   : 开启事务时不添加参数ISOLATION_LEVEL，执行事务后查看备机数据
Description :
    1.新建测试表
    2.开启事务不指定任何事务隔离级别与访问模式，执行事务后进行提交
    3.查看备机数据是否同步
Expect      :
    1.创建测试表成功
    2.开启事务不指定任何事务隔离级别与访问模式，执行事务后进行提交成功
    3.查看备机事务数据同步成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('---------------------Opengauss_Function_DML_Transaction_Case0055开始执行----------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            logger.info('--------------开启事务不指定任何事务隔离级别与访问模式，执行事务后进行提交-------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0055执行完成-----------------------')
