...
"""
Case Type   : 事务控制
Case Name   : 开启事务执行update语句后是否可以再次设置事务隔离级别
Description :
    1.创建测试表
    2.以默认方式开启事务后
    3.执行update语句
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0021开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        logger.info('--------以默认方式开启数据库进行delete后重新开启事务隔离级别为REPEATABLE READ的事务隔离级别----------------------')
        sql_cmd = f'''start transaction;
                      update testzl set sk = 2 where SQ_FT = 3332;
    ...
    def tearDown(self):
        ...
        logger.info('-----------------------Opengauss_Function_DML_Transaction_Case0021执行完成--------------------')
