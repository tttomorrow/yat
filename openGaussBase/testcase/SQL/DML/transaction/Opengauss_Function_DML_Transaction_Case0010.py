...
"""
Case Type   : 事务控制
Case Name   : 两个事务的隔离级别设为REPEATABLE READ，一个事务提交后在新的事务中
...
Description :
    ...
    2.开启事务并指定事务隔离级别为REPEATABLE READ
...
Expect      :
    1.创建测试表成功
    2.开启事务并指定事务隔离级别为REPEATABLE READ成功
    3.在事务中对数据库表插入一条数据成功
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_DML_Transaction_Case0010开始执行----')
        self.Standby_SH = CommonSH('Standby1DbUser')
        self.constant = Constant()
        self.t_name = 't_dml_transaction_case0010'
    ...
    def test_transaction_file(self):
        ...
        step2_txt = '----step2：开启事务并指定事务隔离级别为REPEATABLE READ ' \
                    'expect:开启设置事务成功----'
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_DML_Transaction_Case0010执行完成----')
