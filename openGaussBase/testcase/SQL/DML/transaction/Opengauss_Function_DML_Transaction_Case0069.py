...
"""
Case Type   : 事务控制
Case Name   : 执行ROLLBACK  TRANSACTION命令事务是否回滚
Description :
    1.创建测试表
    2.开启事务并执行事务
    3.执行rollback transaction命令对事务进行回滚
    4.查看事务数据回滚
Expect      :
    1.创建测试表成功
    2.开启事务并执行事务成功
    3.执行rollback transaction命令对事务进行回滚成功
    4.查看事务数据回滚成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0069开始执行--------------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0069执行完成--------------------')
