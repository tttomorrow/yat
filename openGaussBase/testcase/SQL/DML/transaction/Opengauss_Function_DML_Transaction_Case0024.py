...
"""
Case Type   : 事务控制
Case Name   : 输入数值类型的变量，匿名块正常执行,匿名块中事务是否同步到主机
Description :
    1.创建测试表
    2.在匿名块中写入数值类型的变量，对数据库表执行插入操作
    3.主机事务数据同步
    4.备机事务数据同步
    5.清理环境
Expect      :
    1.创建测试表成功
    2.在匿名块中写入数值类型的变量，对数据库表执行插入操作成功
    3.主机事务数据同步成功
    4.备机事务数据同步成功
    5.清理环境成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('---------------------Opengauss_Function_DML_Transaction_Case0024开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            sql_cmd = '''drop table if exists testzl;
                         create table testzl(sk integer,id tinyint,name smallint,sq_ft bigint); '''
            excute_cmd = f'''
            ...
            logger.info('-------------在匿名块中写入数值类型的变量，对数据库表执行插入操作-------------')
            sql_cmd = f'''  declare 
                              a integer;
                              b TINYINT;
                              c SMALLINT;
                            ...
                            begin
                              ...
                              b := 255;
                              c := 3000;
                              insert into testzl values (a,b,c,d);
                            end;
                          '''
            excute_cmd = f'''
                            source {self.DB_ENV_PATH} ;
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0024执行完成-------------------------')
