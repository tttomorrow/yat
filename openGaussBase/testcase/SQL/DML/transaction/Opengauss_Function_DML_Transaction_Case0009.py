...
"""
Case Type   : 事务控制
Case Name   : 在隔离级别设为REPEATABLE READ的事务中是否可以查询到隔离级别设为
              READ COMMITTED的事务提交的事务数据，备机数据是否同步
Description :
    1.创建测试表
    ...
    3.在事务中对数据库表插入一条数据
    INSERT INTO TEST_TRAN VALUES (002,'SK1','TT',3332);
    4.开启新的事务，并指定指定事务隔离级别为REPEATABLE READ
    ...
    COMMIT;
    7.在第二个事务中查询第一个事务数据
    SELECT * FROM TEST_TRAN WHERE SK = 2;
    8.查看备机数据是否同步
    SELECT * FROM TEST_TRAN;
    9.清理环境
    DROP TABLE IF EXISTS TEST_TRAN;
Expect      :
    ...
    3.在事务中对数据库表插入一条数据成功
    4.开启新的事务，并指定指定事务隔离级别为REPEATABLE READ成功
    5.在第二个事务中进行事务查询失败
    6.提交第一个事务成功
    7.在第二个事务中查询第一个事务数据失败
    8.备机数据同步成功
    9.清理环境成功
History     :
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_DML_Transaction_Case0009开始执行----')
        self.Standby_SH = CommonSH('Standby1DbUser')
        self.constant = Constant()
        self.t_name = 't_dml_transaction_case0009'
    ...
    def test_transaction_file(self):
        ...
        sql_cmd3 = f"INSERT INTO {self.t_name} VALUES (002,'SK1','TT',3332);"
        step4_txt = '----step4：开启新的事务，并指定指定事务隔离级别' \
                    '为REPEATABLE READ expect:开启设置事务成功----'
        self.log.info(step4_txt)
        sql_cmd4 = 'START TRANSACTION ISOLATION LEVEL ' \
                   'REPEATABLE READ READ WRITE;'
        step5_txt = '----step5：在第二个事务中进行查询(获取快照) expect:查询失败----'
        self.log.info(step5_txt)
        sql_cmd5 = f'SELECT * FROM {self.t_name};'
        step6_txt = '----step6：提交第一个事务 expect:提交成功----'
        self.log.info(step6_txt)
        sql_cmd6 = 'COMMIT;'
        step7_txt = '----step7：在第二个事务中查询第一个事务数据 expect:查询失败----'
        self.log.info(step7_txt)
        sql_cmd7 = f"SELECT * FROM {self.t_name} WHERE SK = 2;"

        threads = []
        self.log.info('----线程1操作: step1-step2-step3-step6----')
        ...
        self.log.info('----线程2操作: step4-step5-step7----')
        tras_sql2 = f'SELECT PG_SLEEP(11);' \
            f'{sql_cmd4}' \
            f'{sql_cmd5}' \
            f'SELECT PG_SLEEP(15);' \
            f'{sql_cmd7}'
        ...
        ses2_res5 = thread_results[1].splitlines()[-10].strip()
        self.assertIn('0 row', ses2_res5, '执行失败:' + step5_txt)
        self.assertIn(self.constant.COMMIT_SUCCESS_MSG, thread_results[0],
                      '执行失败:' + step6_txt)
        ses2_res7 = thread_results[1].splitlines()[-1].strip()
        self.assertIn('0 row', ses2_res7, '执行失败:' + step7_txt)

        step8_txt = '----step8：查看备机数据是否同步 expect:同步成功----'
        self.log.info(step8_txt)
        sql_cmd8 = f'SELECT * FROM {self.t_name};'
        msg_primary = Primary_SH.execut_db_sql(sql_cmd8)
        self.log.info(msg_primary)
        msg_standby = self.Standby_SH.execut_db_sql(sql_cmd8)
        self.log.info(msg_standby)
        self.assertEqual(msg_primary, msg_standby, '执行失败:' + step8_txt)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_DML_Transaction_Case0009执行完成----')
