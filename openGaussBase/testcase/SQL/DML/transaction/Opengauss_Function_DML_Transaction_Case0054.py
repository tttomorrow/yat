...
"""
Case Type   : 事务控制
Case Name   : 开启事务时指定参数ISOLATION_LEVEL为REPEATABLE READ，执行事务后查看备机数据
Description :
    1.新建测试表
    2.开启事务并指定事务隔离级别为REPEATABLE READ，执行事务后进行提交
    3.查看备机数据是否同步
Expect      :
    1.创建测试表成功
    2.开启事务并指定事务隔离级别为REPEATABLE READ，执行事务后进行提交成功
    3.查看备机事务数据同步成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('---------------------Opengauss_Function_DML_Transaction_Case0054开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            logger.info('--------------开启事务并指定事务隔离级别为REPEATABLE READ，执行事务后进行提交-------------')
            sql_cmd = f'''START TRANSACTION ISOLATION LEVEL REPEATABLE READ;
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0054执行完成------------------------')
