...
"""
Case Type   : 事务控制
Case Name   : 事务可选关键字设为TRANSACTION，事务隔离级别为READ COMMITTED，访问模式为READ ONLY,执行事务
Description :
    1.新建测试表
    2.开启事务
    3.在事务中对数据库表插入数据
Expect      :
    1.创建测试表成功
    2.开启TRANSACTION事务并指定隔离级别为READ COMMITTED，访问模式为READ ONLY成功
    3.在事务中对数据库表插入一条数据失败
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0042开始执行------------------------')
    ...
    def test_transaction_file(self):
        ...
        logger.info('--------------开启TRANSACTION事务并指定隔离级别为READ COMMITTED，访问模式为READ ONLY-------------')
        sql_cmd = f'''BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED READ ONLY;
    ...
    def tearDown(self):
        ...
        logger.info('-------------------Opengauss_Function_DML_Transaction_Case0042执行完成------------------------')
