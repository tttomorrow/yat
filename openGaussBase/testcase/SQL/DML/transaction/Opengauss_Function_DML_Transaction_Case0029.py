...
"""
Case Type   : 事务控制
Case Name   :  输入日期/时间类型的变量，匿名块正常执行,匿名块中事务是否同步到主机
Description : 1.新建测试表 2.在匿名块中写入日期/时间类型的变量，对数据库表执行插入操作 3.查看主机事务数据 4.查看备机事务数据
Expect      : 
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('---------------------Opengauss_Function_DML_Transaction_Case0029开始执行------------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            sql_cmd = '''drop table if exists testzl;
                         create table testzl(sk date,sk1 time ,sk2 timestamp,sk3 smalldatetime,sk4 interval ,sk5 reltime); '''
            excute_cmd = f'''
            ...
            logger.info('-------------在匿名块中写入日期/时间类型的变量，对数据库表执行插入操作-------------')
            sql_cmd = f'''  declare 
                              a DATE;
                              b TIME ;
                            ...
                            begin
                              ...
                              e := INTERVAL '3' DAY;
                              f := '60';
                              insert into testzl values (a,b,c,d,e,f);
                            end;
                         '''
            excute_cmd = f'''
                            source {self.DB_ENV_PATH} ;
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0029执行完成------------------------')
