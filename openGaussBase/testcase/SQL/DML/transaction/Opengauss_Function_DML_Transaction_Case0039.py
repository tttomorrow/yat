...
"""
Case Type   : 事务控制
Case Name   : 事务可选关键字设为WORK，事务隔离级别为REPEATABLE READ，访问模式为READ WRITE,执行事务后备机数据是否同步
Description :
    1.新建测试表
    2.开启事务
...
Expect      :
    1.创建测试表成功
    2.开启WORK事务并指定隔离级别为REPEATABLE READ，访问模式为READ WRITE成功
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Transaction_Case0039开始执行-----------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            logger.info('--------------开启WORK事务并指定隔离级别为REPEATABLE READ，访问模式为READ WRITE-------------')
            sql_cmd = f'''BEGIN WORK ISOLATION LEVEL READ COMMITTED READ WRITE;
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DML_Transaction_Case0039执行完成------------------------')
