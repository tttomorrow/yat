...
"""
Case Type   : 事务控制
Case Name   : start transaction与commit配合使用，执行事务后查看备机数据
Description : 1.新建测试表 2.开启事务并执行事务 3.查看备机数据是否同步
Expect      : 
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('-------------------Opengauss_Function_DML_Transaction_Case0058开始执行----------------------')
    ...
    def tearDown(self):
        ...
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0058执行完成---------------------')
