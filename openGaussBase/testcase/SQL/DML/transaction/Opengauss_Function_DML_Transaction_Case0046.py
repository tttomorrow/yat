...
"""
Case Type   : 事务控制
Case Name   : 开启事务时添加参数LOCAL，指定事务隔离级别为READ COMMITTED，访问模式为READ ONLY,执行事务
Description :
    1.新建测试表
    2.开启事务后
    3.设置事务的隔离级别为READ COMMITTED，访问模式为READ ONLY,并指定LOCAL声明该命令只在当前事务中有效
    4.在事务中对数据库表插入数据
Expect      :
    1.创建测试表成功
    2.开启事务成功
    3.设置事务的隔离级别为READ COMMITTED，访问模式为READ ONLY,并指定LOCAL声明该命令只在当前事务中有效成功
    4.在事务中对数据库表插入一条数据失败
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('-----------------------Opengauss_Function_DML_Transaction_Case0046开始执行----------------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0046执行完成-----------------------')
