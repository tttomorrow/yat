...
"""
Case Type   : 事务控制
Case Name   : 匿名块中是否支持文本检索函数
Description :
    1.创建测试表
    2.在匿名块中写入文本检索类型的变量，对数据库表执行插入操作
    3.主机事务数据同步
    4.备机事务数据同步
    5.清理环境
Expect      :
    1.创建测试表成功
    2.在匿名块中写入文本检索类型的变量，对数据库表执行插入操作成功
    3.主机事务数据同步成功
    4.备机事务数据同步成功
    5.清理环境成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0103开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        sql_cmd = '''begin
                        insert into testzl values (to_tsquery('Fat:ab & Cats'));
                      end;
                      '''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0103执行完成--------------------')
