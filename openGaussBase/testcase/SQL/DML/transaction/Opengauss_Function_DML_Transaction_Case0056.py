...
"""
Case Type   : 事务控制
Case Name   : 开启事务时添加参数READ WRITE ，执行事务后查看备机数据
Description :
    1.创建测试表
    2.开启事务指定访问模式为READ WRITE，执行事务后进行提交
    3.查看备机事务数据同步
    4.清理环境
Expect      :
    1.创建测试表成功
    2.开启事务指定访问模式为READ WRITE，执行事务后进行提交成功
    3.查看备机事务数据同步成功
    4.清理环境成功
"""
...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0056开始执行-------------------------')
    ...
    def test_transaction_file(self):
        ...
        if 'Standby' not in msg:
            ...
            logger.info('--------------开启事务指定访问模式为READ WRITE，执行事务后进行提交-------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0056执行完成-----------------------')
