...
class TransactionFile(unittest.TestCase):
    def setUp(self):
        logger.info('--------------------Opengauss_Function_DML_Transaction_Case0086开始执行--------------------')
    ...
    def test_transaction_file(self):
        ...
        logger.info('--------------将统计信息函数处理后的数据插入到数据库表中-------------')
        sql_cmd = '''declare 
                        i varchar;
                      begin
                        i := pg_backend_pid();
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Transaction_Case0086执行完成--------------------')
