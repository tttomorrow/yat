-- @testpoint: insert组合数据类型，float,real,smallint类型
...
--创建唯一索引
drop index if exists idx_tbl_hash_4;
create unique index idx_tbl_hash_4 on tbl_hash(col_float,col_real,col_smallint);
--插入数据，结合union
insert into tbl_hash(col_float,col_real,col_smallint)
select 1.11111111,1.2222222,32767 from sys_dummy
union all select 1.11111112,1.2222222,32767 from sys_dummy
union all select 1.11111113,1.2222222,32767 from sys_dummy
union all select 1.11111114,1.2222222,32767 from sys_dummy
union all select null,1.2222225,32767 from sys_dummy
union all select null,1.2222226,32767 from sys_dummy order by 1,2,3 nulls first;
--查询
