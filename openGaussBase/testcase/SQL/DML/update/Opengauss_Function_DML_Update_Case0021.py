...
Case Name   : 用户没有expression和condition的select权限，执行update语句，合理报错
Description :
    1.建表并插入数据
    2.创建用户
    3.回收用户test_update的表select权限
...
class Update(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DML_Update_Case0021开始执行-----------------------------')
    ...
    def test_update_permission(self):
       ...
       insert into t_update02 values (1,'小明');''')
        ...
        sql_cmd2 = commonsh.execut_db_sql(f'''drop user if exists test_update001 cascade;
       create user test_update001 password  '{macro.COMMON_PASSWD}';''')
        logger.info(sql_cmd2)
        self.assertIn(constant.CREATE_ROLE_SUCCESS_MSG, sql_cmd2)
        sql_cmd3 = commonsh.execut_db_sql('''revoke select on t_update02 from test_update001;''')
        logger.info(sql_cmd3)
        self.assertIn(constant.REVOKE_SUCCESS_MSG, sql_cmd3)
        sql_cmd4 = '''update t_update02 set id = id +1 where name = '小明';'''
        excute_cmd1 = f'''
                    source {self.DB_ENV_PATH};
                    gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -U test_update001 -W '{macro.COMMON_PASSWD}' -c "{sql_cmd4}"
                    '''
    ...
    def tearDown(self):
        ...
        drop user if exists test_update001 cascade;''')
        logger.info(sql_cmd5)
        logger.info('------------------------Opengauss_Function_DML_Update_Case0021执行结束--------------------------')
