...
"""
Case Type   : explain
Case Name   : explain结合delete语句使用后rollback回滚
Description :
    1.建表
    2.向表中插入数据
    3.开启事务,explain结合delete语句使用,打印执行信息,
      查询表中score=55的对应数据,rollback回滚,查询表中score=55的对应数据
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
    3.开启事务成功,成功打印执行信息,返回数据为空,回滚成功,
      返回score=55对应的数据
    4.清理环境成功
History     :
"""
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_delete_rollback_0024'
    ...
    def test_explain(self):
        ...
        step = 'step3:开启事务,explain结合delete语句使用,打印执行信息,' \
               '查询表中score=55的对应数据,rollback回滚,' \
               '查询表中score=55的对应数据' \
               'expect:开启事务成功,成功打印执行信息,' \
               '返回数据为空,回滚成功，返回score=55的对应数据'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''start transaction;
            explain performance delete from {self.table} where score = 55;
            select * from {self.table} where score = 55;
            rollback;
            select * from {self.table} where score = 55;
            ''')
        ...
        self.assertTrue(' 语文 | 02     |    55' not in data_spl[0]
                        and '语文' in name_spl,
                        "explain结合delete语句使用后rollback回滚失败" + step)
