...
"""
Case Type   : explain
Case Name   : explain结合表的cross join连接方式使用
Description :
    1.建表1
    2.建表2
    3.向表1中插入数据
    4.向表2中插入数据
    5.explain结合select语句，使用表的cross join连接方式，打印执行信息
    6.清理环境
Expect      :
    1.建表1成功
    2.建表2成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table1 = 'explain_table_crossjoin1_0013'
        self.table2 = 'explain_table_crossjoin2_0013'
        self.Constant = Constant()

    def test_explain(self):
        self.logger.info('Opengauss_Function_DML_Explain_Case0013 start')
        ...
        step = 'step5:explain结合select语句，使用表的cross join连接方式，' \
               '打印执行信息   expect:打印的执行信息中含表的连接方式'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain performance select * from {self.table1} cross
            join {self.table2} where name = sname;''')
    ...
    def tearDown(self):
        ...
        self.assertTrue(self.Constant.DROP_TABLE_SUCCESS in de_table
                        ...
                        "执行失败" + step)
        self.logger.info('Opengauss_Function_DML_Explain_Case0013 finish')
