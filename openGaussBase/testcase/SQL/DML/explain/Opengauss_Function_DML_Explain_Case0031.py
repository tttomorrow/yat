...
"""
Case Type   : explain
Case Name   : explain分析临时表
Description :
    1.建表
    2.向表中插入数据
    3.建临时表,explain结合select语句使用,打印执行信息
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table1 = 'explain_temp_tab1_0031'
        self.table2 = 'explain_temp_tab2_0031'
    ...
    def test_explain(self):
        ...
        create_table = self.primary_sh.execut_db_sql(f'''
            ...
            create table {self.table1} (col1 int,col2 int);''')
        ...
        insert_data = self.primary_sh.execut_db_sql(f'''
            insert into {self.table1} values(1,1),(2,2);''')
        self.logger.info(insert_data)
        self.assertIn(self.Constant.INSERT_SUCCESS_MSG, insert_data,
                      "向表中插入数据失败" + step)

        step = 'step3:建临时表,explain结合select语句使用,打印执行信息' \
               ' expect:建临时表成功,打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            create temp table {self.table2} as select * from {self.table1};
            explain performance select * from {self.table2};''')
