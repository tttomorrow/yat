...
"""
Case Type   : explain
Case Name   : explain使用参数cpu,参数默认值为True
Description :
    1.建表
    2.向表中插入数据
    3.explain使用参数cpu,参数默认值为True
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_param_cpu_0052'
    ...
    def test_explain(self):
        ...
        step = 'step3:explain使用参数cpu,参数默认值为True expect:打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain (analyze,cpu True) select * from {self.table};''')
        self.logger.info(explain)
        self.assertIn('CPU: ', explain, "打印信息不正确" + step)
