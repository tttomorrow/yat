...
"""
Case Type   : explain
Case Name   : explain结合分区表的global索引使用
Description :
    1.建分区表
    2.向表中插入数据
    3.创建global索引
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_partition_tab_0002'
        self.index = 'explain_partition_index_0002'

    def test_explain(self):
        self.logger.info('Opengauss_Function_DML_Explain_Case0002 start')
        ...
        step = 'step3:创建global索引 expect:创建global索引成功'
        self.logger.info(step)
        create_index = self.primary_sh.execut_db_sql(f'''
            drop index if exists {self.index};
            create index {self.index} on {self.table}(ca_num)  global;''')
    ...
    def tearDown(self):
        ...
        self.assertIn(self.Constant.DROP_TABLE_SUCCESS, de_index_table,
                      "执行失败" + step)
        self.logger.info('Opengauss_Function_DML_Explain_Case0002 finish')
