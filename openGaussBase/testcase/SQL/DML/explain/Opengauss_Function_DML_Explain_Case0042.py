...
"""
Case Type   : explain
Case Name   : explain使用参数num_nodes,参数缺省值
Description :
    1.建表
    2.向表中插入数据
    3.explain使用参数num_nodes,参数缺省值
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_param_num_nodes_0042'
    ...
    def test_explain(self):
        ...
        step = 'step3:explain使用参数num_nodes,参数缺省值 ' \
               'expect:报错提示参数无效'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain (num_nodes) select * from  {self.table};''')
