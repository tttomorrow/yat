...
"""
Case Type   : explain
Case Name   : explain结合update语句使用后rollback回滚
Description :
    1.建表
    2.向表中插入数据
    3.开启事务,explain结合update语句使用，打印执行信息,
      查询表中score=55对应的name值,rollback 回滚,
      查询表中score=55对应的name值
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
    3.开启事务成功,成功打印执行信息,返回表中score=55对应的修改后的name值,
      回滚成功,返回表中score=55对应的修改前的name值
    4.清理环境成功
History     :
"""
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_update_rollback_0022'
    ...
    def test_explain(self):
        ...
        step = 'step3:开启事务,explain结合update语句使用，打印执行信息,' \
               '查询表中score=55对应的name值,rollback 回滚,' \
               '查询表中score=55对应的name值 ' \
               'expect:开启事务成功,打印执行信息,' \
               '返回表中score=55对应的修改后的name值,回滚成功,' \
               '返回表中score=55对应的修改前的name值'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''start transaction;
            explain performance update {self.table} set name = '英语' \
            where score = 55;
            select name from {self.table} where score = 55;
            rollback;
            select name from {self.table} where score = 55;
            ''')
        self.logger.info(explain)
        name_spl = str(explain).splitlines()
        self.logger.info(name_spl)
        name_index = name_spl.index(' 英语')
        name_index1 = name_spl.index(' 语文')
        self.assertTrue('英语' in name_spl[name_index]
                        and '语文' in name_spl[name_index1],
