...
"""
Case Type   : explain
Case Name   : explain结合count()函数使用
Description :
    1.建表
    2.向表中插入数据
    3.explain结合count()函数使用，打印执行信息
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_table_func1_0018'
    ...
    def test_explain(self):
        ...
        step = 'step3:explain结合count()函数使用,打印执行信息' \
               'expect:打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain performance select count(*) from {self.table};''')
        self.logger.info(explain)
        self.assertIn('Output: count(*)', explain,
                      "explain结合count()函数使用打印信息错误" + step)
