...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_column_tab_0008'
        self.index = 'explain_column_index_0008'
        self.Constant = Constant()

    def test_explain(self):
        self.logger.info('Opengauss_Function_DML_Explain_Case0008 start')
        ...
        insert_data = self.primary_sh.execut_db_sql(f'''
            ...
            md5(random()::text) from (select * from generate_series(1,100000) 
            as id) as x;''')
        ...
        create_index = self.primary_sh.execut_db_sql(f'''
            ...
            on {self.table} using gin(to_tsvector('english',first_name));''')
        ...
        explain = self.primary_sh.execut_db_sql(f'''
            ...
            where to_tsvector('english', first_name) @@  to_tsquery
            ('english', 'test') order by id, first_name, last_name; ''')
        self.logger.info(explain)
        self.assertIn('CStore Index Heap Scan', explain,
                      "打印信息错误" + step)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.Constant.DROP_TABLE_SUCCESS, de_index_table,
                      "执行失败" + step)
        self.logger.info('Opengauss_Function_DML_Explain_Case0008 finish')
