...
"""
Case Type   : explain
Case Name   : explain使用参数plan,参数默认值为off
Description :
    1.建表
    2.向表中插入数据
    3.explain使用参数plan,参数默认值为off
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_param_plan_0069'
    ...
    def test_explain(self):
        ...
        step = 'step3:explain使用参数plan,参数默认值为off' \
               'expect:打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain (plan off) select * from {self.table};''')
        self.logger.info(explain)
        self.assertIn(f'Seq Scan on {self.table}', explain,
                      "执行结果不正确" + step)
