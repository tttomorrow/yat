...
"""
Case Type   : explain
Case Name   : explain结合not exists子查询使用
Description :
    1.建表1
    2.建表2
    3.向表1中插入数据
    4.向表2中插入数据
    5.explain结合not exists子查询使用,打印执行信息
    6.清理环境
Expect      :
    1.建表1成功
    2.建表2成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table1 = 'explain_tab_not_exists1_0071'
        self.table2 = 'explain_tab_not_exists2_0071'
    ...
    def test_explain(self):
        ...
        create_table1 = self.primary_sh.execut_db_sql(f'''
            ...
            create table {self.table1} (name varchar(10),stu_id 
        ...
        self.logger.info(step)
        create_table2 = self.primary_sh.execut_db_sql(
        ...
        self.logger.info(step)
        insert_data1 = self.primary_sh.execut_db_sql(
            f'''insert into {self.table2} values('张三',1,50),
            ('李四',2,55),('王五',3,30);''')
        ...
        insert_data2 = self.primary_sh.execut_db_sql(
            f'''insert into {self.table2} values('数学','01',50,'张三'),
        ...
        step = 'step5:explain结合not exists子查询使用,打印执行信息' \
               'expect:打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(
            f'''explain performance select * from {self.table1} 
            where not exists (select sname from {self.table2} 
            where num >= 50);''')
        self.logger.info(explain)
        self.assertIn(f"Output: {self.table1}.name, {self.table1}.stu_id, "
                      f"{self.table1}.score", explain,
                      "explain不能结合not exists子查询使用" + step)

    def tearDown(self):
        step = 'step6:清理环境 expect:清理环境成功'
        de_table = self.primary_sh.execut_db_sql(
            f'''drop table {self.table1};
            drop table {self.table2};''')
        self.logger.info(de_table)
        self.assertTrue(de_table.count(self.Constant.DROP_TABLE_SUCCESS) == 2,
                        "执行失败" + step)
