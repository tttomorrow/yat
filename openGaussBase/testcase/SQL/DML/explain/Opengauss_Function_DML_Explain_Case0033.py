...
"""
Case Type   : explain
Case Name   : explain使用多个参数analyze,cpu,buffers
Description :
    1.建表
    2.向表中插入数据
    3.explain结合select语句使用多个参数analyze,cpu,buffers,打印执行信息
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_params_0033'
    ...
    def test_explain(self):
        ...
        step = 'step3:explain结合select语句使用多个参数analyze,cpu,buffers,' \
               '打印执行信息 expect:打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain (analyze,cpu,buffers) select * from {self.table};''')
        self.logger.info(explain)
        self.assertTrue('actual time' in explain and 'Buffers:' in explain
                        and 'CPU:' in explain, "打印信息不正确" + step)
