...
"""
Case Type   : explain
Case Name   : explain使用参数format,指定输出格式为xml
Description :
    1.建表
    2.向表中插入数据
    3.explain使用参数format,指定输出格式为xml
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_param_format_0046'
    ...
    def test_explain(self):
        ...
        step = 'step3:explain使用参数format,指定输出格式为xml' \
               'expect:打印执行信息以xml格式显示'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            set explain_perf_mode = normal;
            explain (format xml) select * from {self.table};''')
        self.logger.info(explain)
        explain_spl = str(explain).split(' <Query>')
        self.logger.info(explain_spl)
        self.assertTrue('<explain '
                        'xmlns="http://www.postgresql.org/2009/explain">' in
                        explain_spl[0] and '</explain>' in explain_spl[1],
                        "执行失败" + step)
