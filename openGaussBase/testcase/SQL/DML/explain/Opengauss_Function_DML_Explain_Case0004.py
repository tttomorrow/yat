...
"""
Case Type   : explain
Case Name   : explain结合行存表hash索引使用
Description :
    1.建行存表
    2.向表中插入数据
    3.创建hash索引
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_row_tab_0004'
        self.index = 'explain_row_index_0004'

    def test_explain(self):
        self.logger.info('Opengauss_Function_DML_Explain_Case0004 start')
        ...
        step = 'step3:创建hash索引 expect:创建hash索引成功'
        self.logger.info(step)
        create_index = self.primary_sh.execut_db_sql(f'''
            drop index if exists {self.index};
            create index {self.index} on {self.table} using hash (num);''')
    ...
    def tearDown(self):
        ...
        self.assertIn(self.Constant.DROP_TABLE_SUCCESS, de_index_table,
                      "执行失败" + step)
        self.logger.info('Opengauss_Function_DML_Explain_Case0004 finish')
