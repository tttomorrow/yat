...
"""
Case Type   : explain
Case Name   : explain在参数plan的状态为on时与其他参数组合使用
Description :
    1.建表
    2.向表中插入数据
    3.eexplain在参数plan的状态为on时与其他参数组合使用
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
    3.报错提示plan状态为on时不能与其他参数一起使用
    4.清理环境成功
History     :
"""
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_param_plan_verbose_0070'
    ...
    def test_explain(self):
        ...
        step = 'step3:xplain在参数plan的状态为on时与其他参数组合使用' \
               'expect:报错提示plan状态为on时不能与其他参数一起使用'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain (plan,verbose) select * from {self.table};''')
        self.logger.info(explain)
        self.assertIn("ERROR:  EXPLAIN option 'PLAN' "
                      "can not work with other options.", explain,
                      "执行结果错误" + step)
