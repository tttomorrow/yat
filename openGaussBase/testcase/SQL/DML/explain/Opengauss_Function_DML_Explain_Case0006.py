...
"""
Case Type   : explain
Case Name   : explain结合列存表psort索引使用
Description :
    1.建列存表
    2.向表中插入数据
    3.创建psort索引
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_column_tab_0006'
        self.index = 'explain_column_index_0006'
        self.Constant = Constant()

    def test_explain(self):
        self.logger.info('Opengauss_Function_DML_Explain_Case0006 start')
        ...
        step = 'step3:创建psort索引 expect:创建psort索引成功'
        self.logger.info(step)
        create_index = self.primary_sh.execut_db_sql(f'''
            drop index if exists {self.index};
            create index  {self.index} on {self.table} using  psort(a);''')
    ...
    def tearDown(self):
        ...
        self.assertIn(self.Constant.DROP_TABLE_SUCCESS, de_index_table,
                      "执行失败" + step)
        self.logger.info('Opengauss_Function_DML_Explain_Case0006 finish')
