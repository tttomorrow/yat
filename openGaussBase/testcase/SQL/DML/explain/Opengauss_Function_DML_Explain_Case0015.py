...
"""
Case Type   : explain
Case Name   : explain结合not in子查询使用
Description :
    1.建表1
    2.建表2
    3.向表1中插入数据
    4.向表2中插入数据
    5.explain结合not in子查询使用，打印执行信息
    6.清理环境
Expect      :
    1.建表1成功
    2.建表2成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table1 = 'explain_table_notin1_0015'
        self.table2 = 'explain_table_notin2_0015'
    ...
    def test_explain(self):
        ...
        step = 'step5:explain结合not in子查询使用，打印执行信息' \
               'expect:打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain performance select * from {self.table1} 
            where name not in (select sname from {self.table2} where num >= 50)
            ;''')
        self.logger.info(explain)
        self.assertIn('Nested Loop Anti Join', explain,
                      "explain无法结合not in子查询使用" + step)
