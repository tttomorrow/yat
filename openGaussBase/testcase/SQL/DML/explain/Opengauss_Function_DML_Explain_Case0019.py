...
"""
Case Type   : explain
Case Name   : explain结合char_length()函数使用
Description :
    1.建表
    2.向表中插入数据
    3.explain结合char_length()函数使用，打印执行信息
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_table_func1_0019'
    ...
    def test_explain(self):
        ...
        create_table = self.primary_sh.execut_db_sql(f'''
            ...
            create table {self.table}(name varchar(10),stu_id integer 
            not null,score int );''')
        ...
        step = 'step3:explain结合char_length()函数使用,打印执行信息' \
               'expect:打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain performance select char_length
            ((select name from {self.table} where score = 30));''')
        self.logger.info(explain)
        self.assertIn('Output: char_length(($0)::text)', explain,
                      "explain结合char_length()函数使用打印信息错误" + step)
