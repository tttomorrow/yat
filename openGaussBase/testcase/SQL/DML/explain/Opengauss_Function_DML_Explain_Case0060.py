...
"""
Case Type   : explain
Case Name   : explain使用参数analyse,参数默认值为False
Description :
    1.建表
    2.向表中插入数据
    3.explain使用参数analyse,参数默认值为False
    4.清理环境
Expect      :
    1.建表成功
    2.向表中插入数据成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'explain_param_analyse_0060'
    ...
    def test_explain(self):
        ...
        step = 'step3:explain使用参数analyse,参数默认值为False' \
               'expect:不打印执行信息'
        self.logger.info(step)
        explain = self.primary_sh.execut_db_sql(f'''
            explain (analyse False) select * from {self.table};''')
