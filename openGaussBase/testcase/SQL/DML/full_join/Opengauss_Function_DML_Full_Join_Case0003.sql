-- @testpoint: right join/left join/inner join /full join混合使用查询(15张表)，并且不带索引
...
full outer join all_datatype_table t8 on t1.id=t8.id
full join all_datatype_table t9 on t1.id=t9.id
full outer join all_datatype_table t10 on t1.id=t10.id
right join all_datatype_table t11 on t1.id=t11.id
full outer join all_datatype_table t12 on t1.id=t12.id
full outer join all_datatype_table t13 on t1.id=t13.id
right join all_datatype_table t14 on t1.id=t14.id
