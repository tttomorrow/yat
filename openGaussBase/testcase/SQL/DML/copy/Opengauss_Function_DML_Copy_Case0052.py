...
"""
Case Type   : 拷贝数据
Case Name   : 对insert命令的结果进行copy to
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.对insert命令的结果进行copy to
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0052开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_52'
        self.file_name = 'testcopy52.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:对insert命令的结果进行copy to Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy (insert into {self.tb_name} "
            f"values(003,'sk3','tt3',3333)) to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}';")
        self.log.info(sql_cmd)
        self.assertIn(self.Constant.COPY_SYNTAX_ERROR_MSG, sql_cmd,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0052执行完成')
