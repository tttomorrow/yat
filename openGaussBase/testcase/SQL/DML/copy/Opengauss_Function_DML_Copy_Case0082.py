...
"""
Case Type   : 拷贝数据
Case Name   : text/binary格式数据的指定header进行copy from/to
Description :
    1.创建测试表并插入数据
    2.构造数据文件
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0082开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_82'
        self.file_name = 'testcopy82.csv'
    ...
    def test_copy_file(self):
        ...
        excute_cmd = f'''mkdir {self.copy_dir_path};
            touch {macro.DB_INSTANCE_PATH}/pg_copydir/{self.file_name};'''
        ...
        text = 'step3:binary格式指定header进行copy from Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'binary',header 'on');")
        self.log.info(sql_cmd)
        self.assertIn('COPY HEADER available only in CSV mode', sql_cmd,
                      '执行失败:' + text)

        text = 'step4:binary格式指定header进行copy to Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
        ...
        text = 'step5:text格式指定header进行copy from Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'text',header 'on');")
        self.log.info(sql_cmd)
        self.assertIn('COPY HEADER available only in CSV mode', sql_cmd,
                      '执行失败:' + text)

        text = 'step6:text格式指定header进行copy to Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0082执行完成')
