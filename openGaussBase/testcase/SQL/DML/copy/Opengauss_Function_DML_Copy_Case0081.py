...
"""
Case Type   : 拷贝数据
Case Name   : csv格式数据的header为off/on进行copy from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv格式数据的header为off进行copy from
    4.csv格式数据的header为on进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
    3.copy成功,数据文件中第一行被识别为数据
    4.copy成功,数据文本第一行会被识别为标题行，被忽略
    5.清理环境成功
History     :
"""
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0081开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_81'
        self.file_name = 'testcopy81.csv'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:csv格式数据的header为off进行copy from' \
        ...
        text = 'step4:csv格式数据的header为on进行copy from' \
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0081执行完成')
