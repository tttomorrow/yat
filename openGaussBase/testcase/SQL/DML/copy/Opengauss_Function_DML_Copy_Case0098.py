...
"""
Case Type   : 拷贝数据
Case Name   : csv copy from模式下指定force_not_null参数为存在/不存在的字段
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv copy from模式下指定参数force_not_null(age)
    4.csv copy from模式下指定参数force_not_null(sq_ft)
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_98'
        self.file_name = 'testcopy98.csv'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:csv copy from模式下指定参数force_not_null(age)' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'csv',force_not_null (age));"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        ...
        text = '-----step4:csv copy from模式下指定参数force_not_null(sq_ft)' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'csv',force_not_null (sq_ft));"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY 3', sql_res, '执行失败:' + text)
