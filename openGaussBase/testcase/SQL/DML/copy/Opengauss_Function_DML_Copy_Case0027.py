...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0027开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        logger.info('----------------------------构造含有异常数据类型的数据文件-----------------------------')
        var = r'abcd'
        excute_cmd = f'''
                        sed -i 's/3332/{var}/g' {self.DB_INSTANCE_PATH}/pg_copydir/testzl.dat;
        ...
        sql_cmd = f'''
                      copy testzl from '{self.DB_INSTANCE_PATH}/pg_copydir/testzl.dat' WITH(COMPATIBLE_ILLEGAL_CHARS 'true');
                      select * from testzl;'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
    ...
    def tearDown(self):
        ...
        logger.info(msg)
        logger.info('------------------Opengauss_Function_DML_Copy_Case0027执行完成-----------------------------')
