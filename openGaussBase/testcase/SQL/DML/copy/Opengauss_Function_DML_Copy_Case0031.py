...
"""
Case Type   : 拷贝数据
Case Name   : csv文件中的内容导入数据库表默认分隔符为英文逗号
Description :
    1.创建测试表
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0031开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        excute_cmd = f'''mkdir {self.DB_INSTANCE_PATH}/pg_copydir;
                        touch {self.DB_INSTANCE_PATH}/pg_copydir/copydata.csv;'''
        logger.info(excute_cmd)
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        ...
        excute_cmd = f'''
                        echo "001,sk1,tt,3332" >>  {self.DB_INSTANCE_PATH}/pg_copydir/copydata.csv;
                        echo "001,sk1,tt,3332" >>  {self.DB_INSTANCE_PATH}/pg_copydir/copydata.csv;
                        echo "001,sk1,tt,3332" >>  {self.DB_INSTANCE_PATH}/pg_copydir/copydata.csv;
                        '''
        logger.info(excute_cmd)
        ...
        logger.info('--------------------------进行数据导入--------------------------')
        sql_cmd = f'''copy testzl from '{self.DB_INSTANCE_PATH}/pg_copydir/copydata.csv'  with (format csv) ;'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
    ...
    def tearDown(self):
        ...
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0031执行完成-----------------------------')
