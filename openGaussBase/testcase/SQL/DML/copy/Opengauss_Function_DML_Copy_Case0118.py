...
"""
Case Type   : 拷贝数据
Case Name   : copy_option的参数null值和delimiter参数相同/不同进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.null值和delimiter参数不同进行copy to
    4.null值和delimiter参数不同进行copy from
    5.null值和delimiter参数相同进行copy to
    6.null值和delimiter参数相同进行copy from
    7.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_118'
        self.file_name = 'testcopy118.csv'
    ...
    def test_copy_file(self):
        ...
        text = '-----step5:null值和delimiter参数相同进行copy to ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
        ...
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('delimiter must not appear in the NULL specification',
                      sql_res, '执行失败:' + text)

        text = '-----step6:null值和delimiter参数相同进行copy from ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
