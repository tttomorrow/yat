...
"""
Case Type   : 拷贝数据
Case Name   : csv模式下指定copy_option参数escape为单字节进行copy from/to
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv格式数据指定escape为单字节字符进行copy to
    4.csv格式数据指定escape为单字节字符进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.tb_name = 't_copy_137'
        self.file_name = 'testcopy137.csv'
