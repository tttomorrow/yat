...
"""
Case Type   : 拷贝数据
Case Name   : header选项为on指定/不指定fileheader进行copy to
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.header选项为on指定fileheader进行copy to
    4.header选项为on不指定fileheader进行copy to
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
    3.copy成功,导出数据文件将testcopy01.csv文件第一行数据识别为标题行导出
    4.copy成功,表头被识别导出
    5.清理环境成功
History     :
"""
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0085开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_85'
        self.file_name = 'testcopy85.csv'
        self.file_name1 = 'testcopy01.csv'
    ...
    def test_copy_file(self):
        ...
        excute_cmd = f'''mkdir {self.copy_dir_path};
            ...
            touch {os.path.join(self.copy_dir_path, self.file_name1)};'''
        self.log.info(excute_cmd)
        msg = self.userNode.sh(excute_cmd).result()
        self.log.info(msg)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name1)}';")
        self.assertNotIn(self.Constant.SQL_WRONG_MSG[1], msg)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)

        text = 'step3:header选项为on指定fileheader进行copy to' \
               'Expect:copy成功,导出数据文件将testcopy01.csv文件' \
               '第一行数据识别为标题行导出'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'csv',header 'on',fileheader '"
            f"{os.path.join(self.copy_dir_path, self.file_name1)}');")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd, '执行失败:' + text)

        text = 'step4:header选项为on不指定fileheader进行copy to' \
               'Expect:copy成功,表头被识别导出'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'csv',header 'on');")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0085执行完成')
