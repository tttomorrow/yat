...
"""
Case Type   : 拷贝数据
Case Name   : column(offset,length)offset取值为-1/0/1,length取值为2进行copy from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.offset取值为-1，length取值为2进行copy from
    4.offset取值为0，length取值为2进行copy from
    5.offset取值为1，length取值为2进行copy from
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0068开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_68'
        self.file_name = 'testcopy68.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:offset取值为-1，length取值为2进行copy from' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" fixed formatter(id(-1,2));")
        self.log.info(sql_cmd)
        self.assertIn(self.Constant.COPY_SYNTAX_ERROR_MSG,
                      sql_cmd, '执行失败:' + text)

        text = 'step4:offset取值为0，length取值为2进行copy from' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}' "
            f"fixed formatter(id(0,2));")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)

        text = 'step5:offset取值为1，length取值为2进行copy from' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0068执行完成')
