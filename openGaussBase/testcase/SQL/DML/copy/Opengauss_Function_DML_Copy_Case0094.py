...
"""
Case Type   : 拷贝数据
Case Name   : text格式数据指定eol为./0/a进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.text格式数据指定eol为.进行copy to
    4.text格式数据指定eol为.进行copy from
    5.text格式数据指定eol为a进行copy to
    6.text格式数据指定eol为a进行copy from
    7.text格式数据指定eol为0进行copy to
    8.text格式数据指定eol为0进行copy from
    9.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    ...
    def test_copy_file(self):
        ...
        msg = self.pri_user.sh(excute_cmd).result()
        self.log.info(msg)
        self.assertEqual(len(msg), 0, '执行失败:' + text)

        text = '-----step3:text格式数据指定eol为.进行copy to' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'text',eol '.');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('cannot contain any characters', sql_res,
                      '执行失败:' + text)

        text = '-----step4:text格式数据指定eol为.进行copy from' \
               'Expect:copy失败-----'
        self.log.info(text)
        ...
        text = '-----step5:text格式数据指定eol为a进行copy to' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'text',eol 'a');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('cannot contain any characters', sql_res,
        ...
        text = '-----step6:text格式数据指定eol为a进行copy from' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
        ...
        text = '-----step7:text格式数据指定eol为0进行copy to' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'text',eol '0');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('cannot contain any characters', sql_res,
        ...
        text = '-----step8:text格式数据指定eol为0进行copy from' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
