...
"""
Case Type   : 拷贝数据
Case Name   : copy_option指定quote进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_129'
        self.file_name = 'testcopy129.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:csv格式数据指定quote进行copy to' \
               'Expect:copy成功-----'
        self.log.info(text)
        ...
        text = '-----step4:csv格式数据指定quote进行copy from' \
               'Expect:copy成功-----'
        self.log.info(text)
