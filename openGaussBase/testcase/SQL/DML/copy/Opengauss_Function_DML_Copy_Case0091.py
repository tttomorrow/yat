...
"""
Case Type   : 拷贝数据
Case Name   : csv格式数据指定escape为双字节字符进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv格式数据指定escape为双字节字符进行copy to
    4.csv格式数据指定escape为双字节字符进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_91'
        self.file_name = 'testcopy91.csv'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:csv格式数据指定escape为双字节字符进行copy to'\
               ' Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'csv',escape '/t');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY escape must be a single one-byte character',
                      sql_res, '执行失败:' + text)

        text = '-----step4:csv格式数据指定escape为双字节字符进行copy from '\
               'Expect:copy失败-----'
        self.log.info(text)
