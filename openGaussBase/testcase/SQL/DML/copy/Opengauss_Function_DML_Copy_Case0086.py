...
"""
Case Type   : 拷贝数据
Case Name   : header选项为off进行copy to
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.header选项为off进行copy to
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0086开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_86'
        self.file_name = 'testcopy86.csv'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:header选项为off进行copy to' \
               'Expect:copy成功,导出数据不包含表头'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0086执行完成')
