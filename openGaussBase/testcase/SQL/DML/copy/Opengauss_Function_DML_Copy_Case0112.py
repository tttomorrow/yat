...
"""
Case Type   : 拷贝数据
Case Name   : 使用sysadmin权限的用户指定log errors data
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.给文件中插入数据
    4.创建sysadmin权限的用户
    5.使用sysadmin权限的用户对指定log errors data 进行copy from
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.tb_name = 't_copy_112'
        self.file_name = 'testcopy112.dat'
        self.user = 'u_copy_01'
    ...
    def test_copy_file(self):
        ...
        text = '-----step5:使用sysadmin权限的用户对指定log errors data ' \
               '进行copy from Expect:使用sysadmin权限的用户copy失败-----'
        self.log.info(text)
        ...
        self.log.info(sql_cmd)
        self.assertIn(' COPY to or from a file is prohibited', sql_cmd,
                      '执行失败:' + text)
