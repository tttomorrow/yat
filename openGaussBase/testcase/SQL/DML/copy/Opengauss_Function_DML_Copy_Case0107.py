...
"""
Case Type   : 拷贝数据
Case Name   : copy from模式下指定transform参数
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.copy from模式下指定transform参数为简单表达式
    4.copy from模式下指定transform参数为条件表达式
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_107'
        self.file_name = 'testcopy107.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:copy from模式下指定transform参数为简单表达式' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}' " \
            f"transform (name AS name='tt1');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY 3', sql_res, '执行失败:' + text)

        text = '-----step4:copy from模式下指定transform参数为条件表达式' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}' " \
            f"transform (name AS coalesce(not null,'true'));"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY 3', sql_res, '执行失败:' + text)
