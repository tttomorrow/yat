...
"""
Case Type   : 拷贝数据
Case Name   : without escaping在text格式中进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.without escaping在text格式中进行copy to
    4.without escaping在text格式中进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0061开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_61'
        self.file_name = 'testcopy61.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:escaping在text格式中进行copy to Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy ((select sk from {self.tb_name})) to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f"without escaping with (format 'text');")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)

        text = 'step4:escaping在text格式中进行copy from Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy (select sk from {self.tb_name}) to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0061执行完成')
