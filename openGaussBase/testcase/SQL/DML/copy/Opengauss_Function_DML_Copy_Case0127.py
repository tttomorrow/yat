...
"""
Case Type   : 拷贝数据
Case Name   : copy from模式下copy_option指定/不指定ignore_extra_data参数
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.给文件中插入数据
    4.copy from模式下不指定ignore_extra_data
    5.copy from模式下指定ignore_extra_data
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_127'
        self.file_name = 'testcopy127.dat'
    ...
    def test_copy_file(self):
        ...
        excute_cmd = f'''echo "001	sk1	tt	3332	123" >>  {os.path.join
            ...
            echo "001	sk1	tt	3332	456" >>  {os.path.join
            (self.copy_dir_path, self.file_name)};
            echo "001	sk1	tt	3332	567" >>  {os.path.join
            (self.copy_dir_path, self.file_name)};'''
        self.log.info(excute_cmd)
        msg = self.common.get_sh_result(self.pri_user, excute_cmd)
        self.log.info(msg)
        ...
        text = '-----step4:copy from模式下不指定ignore_extra_data参数' \
               'Expect:copy失败-----'
        self.log.info(text)
        ...
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('extra data after last expected column', sql_res,
                      '执行失败:' + text)

        text = '-----step5:copy from模式下指定ignore_extra_data参数' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
