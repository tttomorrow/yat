...
"""
Case Type   : 拷贝数据
Case Name   : 创建任意格式的文件，使用Tab分隔开，将文件中的数据导入数据库表中(指定分隔符)
Description :
    1.创建测试表
    2.创建数据文件并写入文件内容（数据之间用Tab隔开）
    3.执行copy命令
    4.进行校验
    5.清理环境
Expect      :
    1.创建测试表成功
    2.创建数据文件并写入文件内容成功
    3.将copydata文件中的数据拷贝到testzl表中成功
    4.校验成功
    5.清理环境成功
"""
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0035开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        excute_cmd = f'''mkdir {self.DB_INSTANCE_PATH}/pg_copydir;
                        touch {self.DB_INSTANCE_PATH}/pg_copydir/copydata;'''
        logger.info(excute_cmd)
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        ...
        excute_cmd = f'''
                        echo "001,sk1,tt,3332" >>  {self.DB_INSTANCE_PATH}/pg_copydir/copydata;
                        echo "001,sk1,tt,3332" >>  {self.DB_INSTANCE_PATH}/pg_copydir/copydata;
                        echo "001,sk1,tt,3332" >>  {self.DB_INSTANCE_PATH}/pg_copydir/copydata;
                        '''
        logger.info(excute_cmd)
        ...
        logger.info('--------------------------进行数据导入--------------------------')
        sql_cmd = f'''copy testzl from '{self.DB_INSTANCE_PATH}/pg_copydir/copydata' DELIMITERS ',';'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
    ...
    def tearDown(self):
        ...
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0035执行完成-----------------------------')
