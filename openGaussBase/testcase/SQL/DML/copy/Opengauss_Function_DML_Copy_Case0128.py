...
"""
Case Type   : 拷贝数据
Case Name   : copy from模式下copy_option指定/不指定fill_missing_fields参数
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.给文件中插入数据
    4.copy from模式下不指定fill_missing_fields
    5.copy from模式下指定fill_missing_fields
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.tb_name = 't_copy_128'
        self.file_name = 'testcopy128.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step4:copy from模式下不指定fill_missing_fields' \
               'Expect:copy失败-----'
        self.log.info(text)
        ...
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('missing data for column ',  sql_res,
                      '执行失败:' + text)

        text = '-----step5:copy from模式下指定fill_missing_fields' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" fill_missing_fields;"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
