...
"""
Case Type   : 拷贝数据
Case Name   : 反语法测试：copy file from table语法是否有合理报错
Description :
    1. 创建测试表并插入数据
    2.构造数据文件
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0003开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        logger.info('-----------------------使用反语法copy file from table进行copy--------------------------')
        sql_cmd = f'''copy '{self.DB_INSTANCE_PATH}/pg_copydir/testzl.dat' from testzl;'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port}  -c "{sql_cmd}"
                        '''
    ...
    def tearDown(self):
        ...
        logger.info(msg)
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0003执行完成-----------------------------')
