...
"""
Case Type   : 拷贝数据
Case Name   : 声明字段列表进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.声明字段列表进行copy to
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0057开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_57'
        self.file_name = 'testcopy57.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:声明字段列表进行copy to Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name}(id) to "
            f"'{os.path.join(self.copy_dir_path, self.file_name)}';")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)

        text = 'step4:声明字段列表进行copy from Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name}(id) from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}';")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)

        text = 'step5:不声明字段列表进行copy to Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
        ...
        text = 'step6:不声明字段列表进行copy from Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0057执行完成')
