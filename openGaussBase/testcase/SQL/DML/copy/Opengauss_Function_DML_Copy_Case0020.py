...
"""
Case Type   : 拷贝数据
Case Name   : 执行copy from 测试FORCE_NOT_NULL参数
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.从文件中拷贝数据到表
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('---------------------------Opengauss_Function_DML_Copy_Case0020开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        sql_cmd = '''drop table if exists testzl;
                    ...
                    insert into testzl values (001,' ','tt',3332);
                    insert into testzl values (001,' ','tt',3332);
                    insert into testzl values (001,' ','tt',3332);
                    '''
        excute_cmd = f'''
        ...
        sql_cmd = f'''
                      copy testzl from '{self.DB_INSTANCE_PATH}/pg_copydir/testzl.dat' WITH(COMPATIBLE_ILLEGAL_CHARS(ID));
                      select * from testzl;'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
        ...
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        self.assertIn(self.Constant.COPY_REQUIRES_ERROR_MSG, msg)
    ...
    def tearDown(self):
        ...
        logger.info('------------------Opengauss_Function_DML_Copy_Case0020执行完成-----------------------------')
