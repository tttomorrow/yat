...
"""
Case Type   : 拷贝数据
Case Name   : 从表中copy数据到一个与表无重复数据的文件中，文件中数据是否被覆盖
Description :
    1.创建测试表并插入数据成功
    2.构造初始数据文件成功
    3.构造数据文件成功
    4.从文件中拷贝数据到空表成功
    5.进行校验成功，最后一次的数据将之前的数据覆盖
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造初始数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0024开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        sql_cmd = '''drop table if exists testzl;
                    ...
                    CREATE TABLE testcopy(a INTEGER,b CHAR(16),c VARCHAR(20),d INTEGER);
                    insert into testcopy values (002,'sk2','xx',2224);
                    insert into testcopy values (002,'sk2','xx',2224);
                    insert into testcopy values (002,'sk2','xx',2224);
                    CREATE TABLE testcheck(a INTEGER,b CHAR(16),c VARCHAR(20),d INTEGER);
                    '''
        excute_cmd = f'''
        ...
        logger.info('--------------------------从文件中拷贝数据到空表---------------------------')
        sql_cmd = f'''copy testcheck from '{self.DB_INSTANCE_PATH}/pg_copydir/testzl.dat';'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
        ...
        logger.info('---------------------------进行校验---------------------------')
        sql_cmd = f'''select count(*) from testcheck;'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
    ...
    def tearDown(self):
        ...
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0024执行完成-----------------------------')
