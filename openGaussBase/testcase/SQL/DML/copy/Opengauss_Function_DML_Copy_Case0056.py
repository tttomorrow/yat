...
"""
Case Type   : 拷贝数据
Case Name   : 无模式修饰表名进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.无模式修饰表名进行copy to
    4.无模式修饰表名进行copy from
    5.无模式修饰不存在的表名进行copy to
    6.无模式修饰不存在的表名进行copy from
    7.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0056开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 'tpcds.t_copy_56'
        self.tb_name1 = 'tpcds.t_copy1_56'
        self.file_name = 'testcopy56.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:无模式修饰表名进行copy to Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
        ...
        text = 'step4:无模式修饰表名进行copy from Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
        ...
        text = 'step5:无模式修饰不存在的表名进行copy to Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name1} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}';")
        self.log.info(sql_cmd)
        self.assertIn(self.Constant.NOT_EXIST, sql_cmd,
                      '执行失败:' + text)

        text = 'step6:无模式修饰不存在的表名进行copy from Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name1} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}';")
        self.log.info(sql_cmd)
        self.assertIn(self.Constant.NOT_EXIST, sql_cmd,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0056执行完成')
