...
"""
Case Type   : 拷贝数据
Case Name   : format为fixed格式进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.format为fixed格式中进行copy to
    4.format为fixed格式中进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0066开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_66'
        self.file_name = 'testcopy66.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:format为fixed格式中进行copy to Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f"fixed formatter(sk(0,1),id(2,3),name(5,3),sq_ft(8,4));")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)

        text = 'step4:format为fixed格式中进行copy from Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0066执行完成')
