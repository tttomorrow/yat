...
"""
Case Type   : 拷贝数据
Case Name   : 复制导出数据文件后将该新文件copy进测试表
Description : 
    1.创建测试表并插入数据
    2.构造数据文件并
    3.对该数据文件做复制，形成新的数据文件
    4.执行COPY FROM命令
    5，进行校验
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件并成功
    3.对该数据文件做复制，形成新的数据文件成功
    4.执行COPY FROM命令成功
    5，进行校验成功
"""
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('---------------------Opengauss_Function_DML_Copy_Case0009开始执行--------------------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------Opengauss_Function_DML_Copy_Case0009执行完成-----------------------------')
