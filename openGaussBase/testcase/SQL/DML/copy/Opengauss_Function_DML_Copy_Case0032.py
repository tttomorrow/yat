...
"""
Case Type   : 拷贝数据
Case Name   : fixed文件中的内容是否可以导入数据库表中（指定分隔符）
Description :
    1.创建测试表
    2.创建数据文件并写入文件内容（数据之间用英文逗号隔开）
    3.指定分隔符执行copy命令
    4.进行校验
    5.清理环境
Expect      :
    1.创建测试表成功
    2.创建数据文件并写入文件内容（数据之间用英文逗号隔开）成功
    3.指定分隔符执行copy命令成功
    4.进行校验成功
    5.清理环境成功
"""
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0032开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        logger.info('--------------------------进行数据导入--------------------------')
        sql_cmd = f'''copy testzl from '{self.DB_INSTANCE_PATH}/pg_copydir/copydata.fixed'  DELIMITER ',' ;'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
    ...
    def tearDown(self):
        ...
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0032执行完成-----------------------------')
