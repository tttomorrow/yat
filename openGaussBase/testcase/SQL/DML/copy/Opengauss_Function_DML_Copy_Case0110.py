...
"""
Case Type   : 拷贝数据
Case Name   : 指定log errors data是否会填充容错表的rawrecord字段
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.给文件中插入数据
    4.指定log errors data 进行copy from
    5.查看容错表的rawrecord字段是否被填充
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_110'
        self.file_name = 'testcopy110.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step4:指定log errors data 进行copy from' \
               'Expect:copy成功-----'
        self.log.info(text)
        ...
        text = '-----step5:查看容错表的rawrecord字段是否被填充' \
               'Expect:rawrecord字段被填充-----'
        self.log.info(text)
        sql_cmd = f"select * from pgxc_copy_error_log;"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('nnn2', sql_res, '执行失败:' + text)
