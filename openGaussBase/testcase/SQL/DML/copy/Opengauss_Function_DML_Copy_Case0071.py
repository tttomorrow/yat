...
"""
Case Type   : 拷贝数据
Case Name   : 分隔符和null参数相同进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.分隔符和null参数相同进行copy to
    4.分隔符和null参数相同进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0071开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_71'
        self.file_name = 'testcopy71.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:分隔符和null参数相同进行copy to Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
        ...
        text = 'step4:分隔符和null参数相同进行copy from Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0071执行完成')
