...
"""
Case Type   : 拷贝数据
Case Name   : 对values命令的结果进行copy to
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.对values命令的结果进行copy to(用花括弧包围的values命令)
    4.对values命令的结果进行copy to(用多个圆括弧包围的values命令)
    5.对values命令的结果进行copy to(用圆括弧包围的values命令)
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0051开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_51'
        self.file_name = 'testcopy51.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:对values命令的结果进行copy(用花括弧包围的values命令)' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = f"copy {{values (001,'sk1','tt1',3331)}} to " \
                  f"'{os.path.join(self.copy_dir_path, self.file_name)}';"
        self.log.info(sql_cmd)
        with self.assertRaises(KeyError):
            self.commonsh.execut_db_sql(sql_cmd)

        text = 'step4:对values命令的结果进行copy(用多个圆括弧包围的' \
               'values命令 Expect:copy成功)'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy ((values (001,'sk1','tt1',3331))) to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}';")
        self.log.info(sql_cmd)
        self.assertIn('COPY 1', sql_cmd,
                      '执行失败:' + text)

        text = 'step5:对values命令的结果进行copy(用圆括弧包围的values命令)' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy (values (001,'sk1','tt1',3331)) to '"
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0051执行完成')
