...
"""
Case Type   : 拷贝数据
Case Name   : text/binary/csv/fixed格式数据指定eol进行copy from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.binary格式数据指定eol进行copy from
    4.csv格式数据指定eol进行copy from
    5.fixed格式数据指定eol进行copy from
    6.text格式数据指定eol进行copy from
    7.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_92'
        self.file_name = 'testcopy92.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:binary格式数据指定eol进行copy from ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'binary',eol '\\n');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('EOL specification can not be used with non-text format',
                      sql_res, '执行失败:' + text)

        text = '-----step4:csv格式数据指定eol进行copy from ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'csv',eol '\\n');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('EOL specification can not be used with non-text format',
        ...
        text = '-----step5:fixed格式数据指定eol进行copy from ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" fixed formatter(name(1,3)) eol '\\n';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('EOL specification can not be used with non-text format',
        ...
        text = '-----step6:text格式数据指定eol进行copy from ' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
