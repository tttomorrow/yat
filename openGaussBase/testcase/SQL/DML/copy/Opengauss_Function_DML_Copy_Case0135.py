...
"""
Case Type   : 拷贝数据
Case Name   : csv copy from模式下copy_option指定参数force not null
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv copy from模式下指定参数force not null为age
    4.csv copy from模式下指定参数force not null为sq_ft
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_135'
        self.file_name = 'testcopy135.csv'
    ...
    def test_copy_file(self):
        ...
        sql_cmd = self.pri_sh.execut_db_sql(
            ...
            f"{os.path.join(self.copy_dir_path, self.file_name)}' csv;")
        self.assertIn('COPY 3', sql_cmd, '执行失败:' + text)

        text = '-----step3:csv copy from模式下指定参数force not null为age' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}' " \
            f"csv force not null age;"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn(self.Constant.NOT_EXIST, sql_res, '执行失败:' + text)

        text = '-----step4:csv copy from模式下指定参数force not null为sq_ft' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"csv force not null sq_ft;"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
