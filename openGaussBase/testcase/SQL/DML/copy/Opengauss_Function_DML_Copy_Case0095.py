...
"""
Case Type   : 拷贝数据
Case Name   : eol参数和delimiter、null参数相同/不同进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.eol参数和delimiter参数相同进行copy to
    4.eol参数和delimiter参数相同进行copy from
    5.eol参数和null参数相同进行copy to
    6.eol参数和null参数相同进行copy from
    7.eol参数和delimiter、null参数不同进行copy to
    8.eol参数和delimiter、null参数不同进行copy from
    9.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_94'
        self.file_name = 'testcopy94.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:eol参数和delimiter参数相同进行copy to' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'text',delimiter '//',eol '//',null '/');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('cannot contain user-define EOL string', sql_res,
                      '执行失败:' + text)

        text = '-----step4:eol参数和delimiter参数相同进行copy from' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
        ...
        text = '-----step5:eol参数和null参数相同进行copy to' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
        ...
        text = '-----step6:eol参数和null参数相同进行copy from' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
        ...
        text = '-----step7:eol参数和delimiter、null参数不同进行copy to' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'text',delimiter '//',eol '\\r',null ',');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY 3', sql_res, '执行失败:' + text)

        text = '-----step8:eol参数和delimiter、null参数不同进行copy from' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
