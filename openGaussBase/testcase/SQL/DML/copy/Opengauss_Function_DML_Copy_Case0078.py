...
"""
Case Type   : 拷贝数据
Case Name   : null值字符长度不同进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0078开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_78'
        self.file_name = 'testcopy78.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:null值字符长度为101进行copy to Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
        ...
        self.log.info(sql_cmd)
        self.assertIn('null value string is too long', sql_cmd,
                      '执行失败:' + text)

        text = 'step4:null值字符长度为101进行copy from Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
        ...
        text = 'step5:null值字符长度为100进行copy to Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
        ...
        text = 'step6:null值字符长度为100进行copy from Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
        ...
        text = 'step7:null值字符长度为1进行copy to Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
        ...
        text = 'step8:null值字符长度为1进行copy from Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0078执行完成')
