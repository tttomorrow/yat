...
"""
Case Type   : 拷贝数据
Case Name   : copy from模式下指定timestamp_format参数
Description :
    1.创建测试表并插入数据
    2.构造数据文件
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_104'
        self.file_name = 'testcopy104.dat'
    ...
    def test_copy_file(self):
        ...
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}' " \
            f"with(format 'text',timestamp_format 'YYYY-M-D HH:MI:SS');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        ...
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"with(format 'binary',timestamp_format 'YYYY-MM-DD HH:MI:SS');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
