...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0022开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        logger.info('-----------------------对视图copy to,失败--------------------------')
        sql_cmd = f'''copy testzl_view to '{self.DB_INSTANCE_PATH}/pg_copydir/testzl.dat';'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port}  -c "{sql_cmd}"
                        '''
        ...
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        self.assertIn(self.Constant.COPY_FROM_VIEW_FAIL_MSG, msg)
    ...
    def tearDown(self):
        ...
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0022执行完成-----------------------------')
