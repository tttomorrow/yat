...
"""
Case Type   : 拷贝数据
Case Name   : 从文件中copy数据到一张与文件数据不重复的表中，表中数据是否被覆盖
Description :
    1.创建测试表并插入数据
    2.构造数据文件（与步骤1中表数据不重复）
    3. 从文件中拷贝数据到表
    4.进行校验
    5,清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件（与步骤1中表数据不重复）成功
    3. 从文件中拷贝数据到表成功
    4.进行校验表中数据未被覆盖
    5,清理环境
"""
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0011开始执行-----------------------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0011执行完成-----------------------------')
