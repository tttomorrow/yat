...
"""
Case Type   : 拷贝数据
Case Name   : 使用系统用户copy是否成功
Description : 
    1.创建测试表并插入数据
    2.构造数据文件
    3.使用系统用户进行copy，将表中数据copy到数据文件
    4.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0001开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        logger.info('--------------------------使用系统用户进行copy,将表中数据copy到数据文件---------------------------')
    ...
    def tearDown(self):
        ...
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0001执行完成-----------------------------')
