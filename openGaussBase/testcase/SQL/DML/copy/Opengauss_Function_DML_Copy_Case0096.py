...
"""
Case Type   : 拷贝数据
Case Name   : eol参数长度不同进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.eol度为11进行copy to
    4.eol长度为11进行copy from
    5.eol长度为10进行copy to
    6.eol长度为10进行copy from
    7.eol长度为1进行copy to
    8.eol长度为1进行copy from
    9.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_96'
        self.file_name = 'testcopy96.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:eol长度为11进行copy to Expect:copy失败-----'
        self.log.info(text)
        ...
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('EOL string must not exceed the maximum length',
                      sql_res, '执行失败:' + text)

        text = '-----step4:eol长度为11进行copy from Expect:copy失败-----'
        self.log.info(text)
        ...
        text = '-----step5:eol长度为10进行copy to Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"with(eol '##########');"
        ...
        text = '-----step6:eol长度为10进行copy from Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"with(eol '##########');"
        ...
        text = '-----step7:eol长度为1进行copy to Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"with(eol '#');"
        ...
        text = '-----step8:eol长度为1进行copy from Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"with(eol '#');"
