...
"""
Case Type   : 拷贝数据
Case Name   : delimiter分隔符长度不同进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.delimiters分隔符长度为11进行copy to
    4.delimiters分隔符长度为11进行copy from
    5.delimiters分隔符长度为10进行copy to
    6.delimiters分隔符长度为10进行copy from
    7.delimiters分隔符长度为5进行copy to
    8.delimiters分隔符长度为5进行copy from
    9.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0059开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_59'
        self.file_name = 'testcopy59.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:delimiters分隔符长度为11进行copy to' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            ...
            f"delimiters '///////////';")
        self.log.info(sql_cmd)
        self.assertIn('ERROR:  COPY delimiter must be less than 10 bytes',
                      sql_cmd, '执行失败:' + text)

        text = 'step4:delimiters分隔符长度为11进行copy from' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
        ...
        text = 'step5:delimiters分隔符长度为10进行copy to' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
        ...
        text = 'step6:delimiters分隔符长度为10进行copy from' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
        ...
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)
        text = 'step7:delimiters分隔符长度为5进行copy to' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            ...
            f"delimiters '/////';")
        self.log.info(sql_cmd)
        self.assertIn('COPY 6', sql_cmd,
                      '执行失败:' + text)

        text = 'step8:delimiters分隔符长度为5进行copy from' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0059执行完成')
