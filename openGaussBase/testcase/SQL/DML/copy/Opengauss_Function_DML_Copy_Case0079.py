...
"""
Case Type   : 拷贝数据
Case Name   : csv格式数据的null值和quote参数相同进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv格式数据的null值和quote参数相同进行copy to
    4.csv格式数据的null值和quote参数相同进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0079开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_79'
        self.file_name = 'testcopy79.csv'
    ...
    def test_copy_file(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            ...
            f"insert into {self.tb_name} values (001,'sk1','tt1','');"
        ...
        text = 'step3:csv格式数据的null值和quote参数相同进行copy to' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'csv',null '/',quote '/');")
        self.log.info(sql_cmd)
        self.assertIn('CSV quote character must not appear in the NULL '
                      'specification', sql_cmd, '执行失败:' + text)

        text = 'step4:csv格式数据的null值和quote参数相同进行copy from' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0079执行完成')
