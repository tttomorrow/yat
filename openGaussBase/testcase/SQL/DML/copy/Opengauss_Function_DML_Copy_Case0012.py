...
"""
Case Type   : 拷贝数据
Case Name   : 从文件中copy数据到一张与文件数据重复的表中，表中数据是否被覆盖
Description :
    1.创建测试表并插入数据
    2.构造数据文件（与步骤1中表数据重复）
    3. 从文件中拷贝数据到表
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0012开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        sql_cmd = '''drop table if exists testzl;
                    ...
                    insert into testcopy values (001,'sk1','tt',3332);
                    insert into testcopy values (001,'sk1','tt',3332);
                    insert into testcopy values (001,'sk1','tt',3332);
                    '''
        excute_cmd = f'''
    ...
    def tearDown(self):
        ...
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0012执行完成-----------------------------')
