...
"""
Case Type   : 拷贝数据
Case Name   : csv格式和fixed格式不指定/指定eol参数进行copy to
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv格式数据不指定eol进行copy to
    4.csv格式数据指定eol进行copy to
    5.fixed格式数据不指定eol进行copy to
    6.fixed格式数据指定eol进行copy to
    7.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_93'
        self.file_name = 'testcopy93.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:csv格式数据不指定eol进行copy to Expect:copy失败-----'
        self.log.info(text)
        ...
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('except 0x0D0A and 0x0A', sql_res,
                      '执行失败:' + text)

        text = '-----step4:csv格式数据指定eol进行copy to Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'csv',eol '\\n');"
        ...
        text = '-----step5:fixed格式数据不指定eol进行copy to Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" fixed formatter(sk(0,1)," \
            f"id(2,3),name(5,3),sq_ft(8,4)) eol ' ';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('except 0x0D0A and 0x0A', sql_res,
        ...
        text = '-----step6:fixed格式数据指定eol进行copy to Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" fixed formatter(sk(0,1)," \
