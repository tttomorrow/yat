...
"""
Case Type   : 拷贝数据
Case Name   : csv格式数据的分隔符和quote参数不同进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv格式数据的分隔符和quote参数不同进行copy to
    4.csv格式数据的分隔符和quote参数不同进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0074开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_74'
        self.file_name = 'testcopy74.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:csv格式数据的分隔符和quote参数不同进行copy to' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'csv',delimiter '/',quote ',');")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)

        text = 'step4:csv格式数据的分隔符和quote参数不同进行copy from' \
               'Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0074执行完成')
