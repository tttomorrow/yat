...
"""
Case Type   : 拷贝数据
Case Name   : copy from模式下copy_option指定timestamp_format参数
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.copy from模式下指定timestamp_format不合法的timestamp格式
    4.copy from模式,binary格式下指定timestamp_format
    5.copy from模式下指定timestamp_format合法的timestamp格式
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.tb_name = 't_copy_121'
        self.file_name = 'testcopy121.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:copy from模式下指定timestamp_format' \
               '不合法的timestamp格式 Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}' " \
            f"timestamp_format 'YYYY-M-D HH:MI:SS';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        ...
        text = '-----step4:copy from模式,binary格式下指定timestamp_format' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"binary timestamp_format 'YYYY-MM-DD HH:MI:SS';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        ...
        text = '-----step5:copy from模式下指定timestamp_format合法的' \
               'timestamp格式 Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"timestamp_format 'YYYY-MM-DD HH:MI:SS';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
