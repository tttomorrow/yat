...
"""
Case Type   : 拷贝数据
Case Name   : COPY FROM不能够识别none,会将其默认为字符串
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.执行copy命令
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
    3.构造含有\n数据文件,将第二行字段ID数据改为none成功
    4.进行copy from成功，表数据第二行ID字段显示为none
"""
import unittest
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0114开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        logger.info('--------------------------构造含有none的文件---------------------------')
        excute_cmd = f'''
                        sed -i 's/sk1/none/g' {self.DB_INSTANCE_PATH}/pg_copydir/testzl.dat;
        ...
        logger.info('---------------------------进行校验---------------------------')
        sql_cmd = f'''select * from testzl;
                      select count(*) from testzl where id = 'none';'''
        excute_cmd = f'''
                       source {self.DB_ENV_PATH} ;
                       gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                       '''
    ...
    def tearDown(self):
        ...
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0114执行完成-----------------------------')
