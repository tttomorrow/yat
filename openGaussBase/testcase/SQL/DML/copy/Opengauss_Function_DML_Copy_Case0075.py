...
"""
Case Type   : 拷贝数据
Case Name   : text格式的分隔符包含小写字母、数字和特殊字符.\进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.text格式数据的分隔符包含.\进行copy to
    4.text格式数据的分隔符包含.\进行copy from
    5.text格式数据的分隔符包含a进行copy to
    6.text格式数据的分隔符包含a进行copy from
    7.text格式数据的分隔符包含1进行copy to
    8.text格式数据的分隔符包含1进行copy from
    9.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0075开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_75'
        self.file_name = 'testcopy75.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step5:text格式数据的分隔符包含a进行copy to' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'text',delimiter 'a');")
        self.log.info(sql_cmd)
        self.assertIn('cannot contain any characters', sql_cmd,
                      '执行失败:' + text)

        text = 'step6:text格式数据的分隔符包含a进行copy from' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
        ...
        text = 'step7:text格式数据的分隔符包含1进行copy to' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'text',delimiter '1');")
        self.log.info(sql_cmd)
        self.assertIn('cannot contain any characters', sql_cmd,
                      '执行失败:' + text)

        text = 'step8:text格式数据的分隔符包含1进行copy from' \
               'Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0075执行完成')
