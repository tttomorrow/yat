...
"""
Case Type   : 拷贝数据
Case Name   : csv文件中的内容是否可以导入数据库表中（指定分隔符）
Description :
    1.创建测试表
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0030开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        logger.info('--------------------------进行数据导入--------------------------')
        sql_cmd = f'''copy testzl from '{self.DB_INSTANCE_PATH}/pg_copydir/copydata.csv'  DELIMITER '+' ;'''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
                        '''
    ...
    def tearDown(self):
        ...
        msg = self.userNode.sh(excute_cmd).result()
        logger.info(msg)
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0030执行完成-----------------------------')
