...
"""
Case Type   : 拷贝数据
Case Name   : copy from模式下copy_option指定date_format参数，不带时区
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.copy from模式下指定date_format不合法的date格式
    4.copy from模式,binary格式下指定date_format
    5.copy from模式下指定date_format合法的date格式
    6.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_120'
        self.file_name = 'testcopy120.dat'
    ...
    def test_copy_file(self):
        ...
        sql_cmd = f"drop table if exists {self.tb_name};" \
            ...
            f"name varchar(20),create_date date);" \
        ...
        text = '-----step3:copy from模式下指定date_format不合法的date格式' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}' " \
            f"date_format 'YYYY-M-D HH:MI:SS';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        ...
        text = '-----step4:copy from模式,binary格式下指定date_format' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"binary date_format 'YYYY-MM-DD HH:MI:SS';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        ...
        text = '-----step5:copy from模式下指定date_format合法的date格式' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"date_format 'YYYY-MM-DD HH:MI:SS';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
