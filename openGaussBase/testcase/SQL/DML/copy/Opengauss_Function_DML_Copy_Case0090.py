...
"""
Case Type   : 拷贝数据
Case Name   : text/binary/fixed格式数据指定escape进行copy from/to
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.text格式数据指定escape进行copy to
    4.text格式数据指定escape进行copy from
    5.binary格式数据指定escape进行copy to
    6.binary格式数据指定escape进行copy from
    7.fixed格式数据指定escape进行copy to
    8.fixed格式数据指定escape进行copy from
    9.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_90'
        self.file_name = 'testcopy90.csv'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:text格式数据指定escape进行copy to ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = self.pri_sh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'text',escape '\\');")
        self.log.info(sql_cmd)
        self.assertIn('COPY escape available only in CSV mode', sql_cmd,
                      '执行失败:' + text)

        text = '-----step4:text格式数据指定escape进行copy from ' \
               'Expect:copy失败-----'
        self.log.info(text)
        ...
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY escape available only in CSV mode', sql_res,
                      '执行失败:' + text)

        text = '-----step5:binary格式数据指定escape进行copy to ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'binary',escape '\\');"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY escape available only in CSV mode', sql_res,
        ...
        text = '-----step6:binary格式数据指定escape进行copy from ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
        ...
        text = '-----step7:fixed格式数据指定escape进行copy to ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" fixed formatter(name(1,3)) escape '\\';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY escape available only in CSV mode', sql_res,
        ...
        text = '-----step8:fixed格式数据指定escape进行copy from ' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
