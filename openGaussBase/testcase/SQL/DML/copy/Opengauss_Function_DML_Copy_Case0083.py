...
"""
Case Type   : 拷贝数据
Case Name   : csv格式数据的header为0/1或true/false进行copy from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv格式数据的header为0进行copy from
    4.csv格式数据的header为1进行copy from
    5.csv格式数据的header为false进行copy from
    6.csv格式数据的header为true进行copy from
    7.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
    3.copy失败
    4.copy失败
    5.copy成功,数据文件中第一行被识别为数据
    6.copy成功,数据文本第一行会被识别为标题行，被忽略
    7.清理环境成功
History     :
"""
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0083开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_83'
        self.file_name = 'testcopy83.csv'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:csv格式数据的header为0进行copy from Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'csv',header '0');")
        self.log.info(sql_cmd)
        self.assertIn('header requires a Boolean value', sql_cmd,
                      '执行失败:' + text)

        text = 'step4:csv格式数据的header为1进行copy from Expect:copy失败'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} to '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'csv',header '1');")
        self.log.info(sql_cmd)
        self.assertIn('header requires a Boolean value', sql_cmd,
                      '执行失败:' + text)

        text = 'step5:csv格式数据的header为false进行copy from ' \
               'Expect:copy成功，数据文件中第一行被识别为数据'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
            f"copy {self.tb_name} from '"
            f"{os.path.join(self.copy_dir_path, self.file_name)}'"
            f" with(format 'csv',header 'false');")
        self.log.info(sql_cmd)
        self.assertIn('COPY 3', sql_cmd,
                      '执行失败:' + text)

        text = 'step6:csv格式数据的header为true进行copy from ' \
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0083执行完成')
