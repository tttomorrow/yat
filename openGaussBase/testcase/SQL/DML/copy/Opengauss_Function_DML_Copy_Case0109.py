...
"""
Case Type   : 拷贝数据
Case Name   : reject取不同值进行copy from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.给文件中插入数据
    4.reject取值为-2进行copy from
    5.reject取值为0进行copy from
    6.reject取值为2.5进行copy from
    7.reject取值为1进行copy from
    8.reject取值为unlimited进行copy from
    9.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_109'
        self.file_name = 'testcopy109.dat'
    ...
    def test_copy_file(self):
        ...
        text = '-----step4:reject取值为-2进行copy from' \
               'Expect:copy失败-----'
        self.log.info(text)
        ...
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('PER NODE REJECT LIMIT', sql_res,
                      '执行失败:' + text)

        text = '-----step5:reject取值为0进行copy from' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"log errors reject limit '0';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('PER NODE REJECT LIMIT', sql_res,
        ...
        text = '-----step6:reject取值为2.5进行copy from' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"log errors reject limit '2.5';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('invalid input syntax for integer', sql_res,
                      '执行失败:' + text)

        text = '-----step7:reject取值为1进行copy from' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"log errors reject limit '1';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY 2', sql_res, '执行失败:' + text)

        text = '-----step8:reject取值为 unlimited进行copy from' \
               'Expect:copy成功-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} from '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f"log errors reject limit 'unlimited';"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn('COPY 2', sql_res, '执行失败:' + text)
