...
"""
Case Type   : 拷贝数据
Case Name   : 执行copy from 测试/0非法字符容错 COMPATIBLE_ILLEGAL_CHARS参数为true
Description :
    1.创建测试表并插入数据
    2.构造数据文件成功
    3.构造含有/0的数据文件
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_DML_Copy_Case0016开始执行-----------------------------')
    ...
    def test_copy_file(self):
        ...
        logger.info('---------------------------进行校验---------------------------')
        var2 = r' '
        sql_cmd = f'''select count(*) from testzl where ID = '{var2}';
                             '''
        excute_cmd = f'''
                        source {self.DB_ENV_PATH} ;
                        gsql -d {self.userNode.db_name} -p {self.userNode.db_port} -c "{sql_cmd}"
    ...
    def tearDown(self):
        ...
        logger.info('------------------Opengauss_Function_DML_Copy_Case0016执行完成-----------------------------')
