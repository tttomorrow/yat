...
"""
Case Type   : 拷贝数据
Case Name   : csv copy to模式下指定force_quote参数为存在/不存在的字段
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.csv copy to模式下指定参数force_quote(age)
    4.csv copy to模式下指定参数force_quote(sq_ft)
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
    3.copy失败
    4.copy成功，声明的字段周围对所有非null值都使用引号包围
    5.清理环境成功
History     :
"""
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 't_copy_97'
        self.file_name = 'testcopy97.csv'
    ...
    def test_copy_file(self):
        ...
        text = '-----step3:csv copy to模式下指定参数force_quote(age)' \
               'Expect:copy失败-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'csv',force_quote (age));"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
        self.assertIn(self.Constant.NOT_EXIST, sql_res,
                      '执行失败:' + text)

        text = '-----step4:csv copy to模式下指定参数force_quote(sq_ft)' \
               'Expect:copy成功，声明的字段周围对所有非null值都使用引号包围-----'
        self.log.info(text)
        sql_cmd = f"copy {self.tb_name} to '" \
            f"{os.path.join(self.copy_dir_path, self.file_name)}'" \
            f" with(format 'csv',force_quote (sq_ft));"
        self.log.info(sql_cmd)
        sql_res = self.pri_sh.execut_db_sql(sql_cmd)
        self.log.info(sql_res)
