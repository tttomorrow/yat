...
"""
Case Type   : 拷贝数据
Case Name   : text格式的分隔符不包含小写字母、数字和特殊字符.\进行copy to/from
Description :
    1.创建测试表并插入数据
    2.构造数据文件
    3.text格式数据的分隔符不包含小写字母、数字和特殊字符.\进行copy to
    4.text格式数据的分隔符不包含小写字母、数字和特殊字符.\进行copy from
    5.清理环境
Expect      :
    1.创建测试表并插入数据成功
    2.构造数据文件成功
...
class CopyFile(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Dml_Copy_Case0076开始执行')
        self.commonsh = CommonSH('PrimaryDbUser')
        self.userNode = Node(node='PrimaryDbUser')
        self.Constant = Constant()
        self.tb_name = 't_copy_76'
        self.file_name = 'testcopy76.dat'
    ...
    def test_copy_file(self):
        ...
        text = 'step3:text格式数据的分隔符不包含小写字母、数字和' \
               '特殊字符.\进行copy to Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
        ...
        text = 'step4:text格式数据的分隔符不包含小写字母、数字和' \
               '特殊字符.\进行copy from Expect:copy成功'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Dml_Copy_Case0076执行完成')
