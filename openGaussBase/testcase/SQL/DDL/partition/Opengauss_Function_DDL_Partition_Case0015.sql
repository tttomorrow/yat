-- @testpoint: 范围分区全部数据类型，测试多个单列索引insert数据的analyze功能

drop table if exists PARTITION_RANGE018;
create table PARTITION_RANGE018(
field1   integer,
field2   bigint,
field3   real,
field4   decimal(10,2),
...
create index index_PARTITION_RANGE0181 on PARTITION_RANGE018(field1) local;
create index index_PARTITION_RANGE0182 on PARTITION_RANGE018(field7) local;
create index index_PARTITION_RANGE0184 on PARTITION_RANGE018(field12) local;
create index index_PARTITION_RANGE0185 on PARTITION_RANGE018(field17) local;
create index index_PARTITION_RANGE0186 on PARTITION_RANGE018(field18) local;
create index index_PARTITION_RANGE0187 on PARTITION_RANGE018(field19) local;
declare
i int:=0;
begin
  loop
