-- @testpoint: 范围分区表，truncate分区，测试analyze收集统计信息功能(由于影响cbo执行计划，不清除统计信息)

drop table if exists partition_range00023;
create table partition_range00023(
field1   integer,
field2   bigint,
field3   real,
field4   decimal(10,2),
...
partition part12 values less than (TO_DATE('2019-09-1 00:00:00','yyyy-mm-dd hh24:mi:ss'))
 );                  
...
begin
  loop
    i:=i+1;
insert into partition_range00023  values(256,10000000,123.3212,123456.123,12345633333333333333333333333333333333,'b','957',
'简自豪',lpad('345abc',50,'abc'),'135156151',null,'2010-09-11 00:00:00',
...
alter table partition_range00023 truncate partition part01; 
analyze verbose  partition_range00023 ;
drop table if exists partition_range00023;
