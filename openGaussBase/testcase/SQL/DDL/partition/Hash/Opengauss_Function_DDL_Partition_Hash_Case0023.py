...
"""
Case Type   : Hash分区表
Case Name   : 创建global索引并向分区表中插入数据，通过索引检索表中数据
Description :
    1、创建Hash分区表
    2、向分区表中插入数据
    3、创建global索引
...
class HashPartitionCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_DDL_Partition_Hash_Case0023开始执行---")
    ...
    def test_query_dop(self):
        ...
        step3_text = "---step3:创建global索引;expect:创建成功---"
        self.logger.info(step3_text)
        sql_cmd3 = f'''drop index if exists {self.index_name};
        create index {self.index_name} on {self.tab_name}(p_id) global;'''
    ...
    def tearDown(self):
        ...
        self.logger.info(
            "---Opengauss_Function_DDL_Partition_Hash_Case0023执行结束---")
