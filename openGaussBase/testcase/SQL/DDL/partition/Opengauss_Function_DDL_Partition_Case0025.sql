-- @testpoint: 范围分区全部数据类型，测试字符单列索引空表的analyze功能

drop table if exists PARTITION_RANGE006;
create table PARTITION_RANGE006(
field1   integer,
field2   bigint,
field3   real,
field4   decimal(10,2),
...
create index index_partition_range0032 on PARTITION_RANGE006(field7) local;
analyze verbose PARTITION_RANGE006;  --12月份只支持此命令
drop table if exists PARTITION_RANGE006;
