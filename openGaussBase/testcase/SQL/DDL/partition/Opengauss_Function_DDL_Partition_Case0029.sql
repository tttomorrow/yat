-- @testpoint: 范围分区全部数据类型，测试日期单列索引空表的analyze功能

drop table if exists PARTITION_RANGE010;
create table PARTITION_RANGE010(
field1   integer,
field2   bigint,
field3   real,
field4   decimal(10,2),
...
create index index_PARTITION_RANGE0104 on PARTITION_RANGE010(field12) local;
declare
i int:=0;
begin
  loop
    i:=i+1;
insert into PARTITION_RANGE010  values(256,10000000,123.3212,123456.123,123456,'dnf','957',
'简自豪',lpad('345abc',50,'abc'),'12121212121211',null,'2008-09-11 00:00:00',
...
begin
  loop
    i:=i+1;
insert into PARTITION_RANGE010  values(256,10000000,123.3212,123456.123,123456,'dnf','957',
'简自豪',lpad('345abc',50,'abc'),'123115151',null,'2008-08-11 00:00:00',
'2012-11-11 00:00:00',interval '2' day,'2016-12-11 00:00:00','2011-12-11 00:00:00','true',null,null);
exit when i= 1000;
  end loop;
  raise info'111';
...
begin
  loop
    i:=i+1;
insert into PARTITION_RANGE010  values(256,10000000,123.3212,123456.123,123456,'dnf','957',
'简自豪',lpad('345abc',50,'abc'),'32111212121',null,'2008-07-11 00:00:00',
'2012-11-11 00:00:00',interval '2' day,'2016-12-11 00:00:00','2011-12-11 00:00:00','true',null,null);
exit when i= 1000;
  end loop;
  raise info'111';
...
analyze verbose  PARTITION_RANGE010 ;
drop table if exists PARTITION_RANGE010;
