-- @testpoint: 范围分区表，增加分区，测试analyze收集统计信息功能

drop table if exists partition_range00014;
create table partition_range00014(
field1   integer,
field2   bigint,
field3   real,
field4   decimal(10,2),
...
begin
  loop
    i:=i+1;
insert into partition_range00014  values(256,10000000,123.3212,123456.123,12345633333333333333333333333333333333,'b','957',
'简自豪',lpad('345abc',50,'abc'),'1523151515',null,'2008-09-11 00:00:00',
...
ALTER TABLE partition_range00014 add PARTITION part15 values less than (maxvalue);
analyze verbose  partition_range00014 ;
drop table if exists partition_range00014;
