-- @testpoint: 范围分区表，分区键为数值(real)，测试analyze收集统计信息功能

drop table if exists partition_range00007;
create table partition_range00007(
field1   integer,
field2   bigint,
field3   real,
field4   decimal(10,2),
...
create index index_PARTITION_RANGE008 on partition_range00007(field19,field18,field11) local; 
create index index_PARTITION_RANGE009 on partition_range00007(field1,field7,field11,field12,field17) global;
 declare
i int:=0;
begin
  loop
    i:=i+1;
insert into partition_range00007  values(256,10000000,123.3212,123456.123,123456,'dnf','957',
'简自豪',lpad('345abc',50,'abc'),'123123121212',null,'2008-09-11 00:00:00',
...
analyze verbose  partition_range00007 ;
drop table if exists partition_range00007;