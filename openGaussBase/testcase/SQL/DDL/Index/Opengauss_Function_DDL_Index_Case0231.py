...
'''
Case Type： 功能
Case Name： 创建索引过程中在事务内对无关表进行update操作并对原表进行插入操作
Descption:
1.创建表，并插入大数据
2.使用concurrently创建索引
3.在创建索引的同时开启事务对无关表进行 update操作，且对该表插入数据
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0231'
        self.tblname_no_rel = 'ddl_index_case0231_no_rel'
        self.idxname = 'ddl_index_case0231_idx'
        self.log.info(
            '------------------------------------Opengauss_Function_DDL_Index_Case0231.py start------------------------------------')
...
update {self.tblname_no_rel} set first_name=\'测试查询不阻塞\', last_name=\'测试%%不阻塞\' where id = 712;')
        ...
        self.log.info(
            '-----------------start transaction & update no relevance table & insert inyo relevance table-----------------------------')
        before_time = self.dbPrimaryDbUser.sh("date +'%s'").result()
        result = self.primary_sh.execut_db_sql(
            f'START TRANSACTION;update {self.tblname_no_rel} set first_name=\'测试 test 查询不阻塞\', last_name=\'测试--不阻塞\' where id < 7172;INSERT INTO {self.tblname} SELECT id, md5(random()::text), md5(random()::text) FROM (SELECT * FROM generate_series(1,80000) AS id) AS x;')
        ...
        result = self.primary_sh.execut_db_sql(
            f'SET ENABLE_SEQSCAN=off;SELECT * FROM {self.tblname}  where id = 712;')
        self.log.info(result)
