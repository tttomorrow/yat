...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'column_tab37'
        self.index = 'column_index37'
        logger.info("======SetUp:检查数据库状态是否正常======")
        status = primary_sh.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)

    def test_column_unique_index(self):
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0037开始执行")
        ...
        logger.info("======步骤3:反复清除插入数据，数据唯一且非空======")
    ...
    def tearDown(self):
        ...
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0037执行结束")
