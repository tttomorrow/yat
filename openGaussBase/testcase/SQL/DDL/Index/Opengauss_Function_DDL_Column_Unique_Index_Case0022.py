...
Case Name   : 开启delta表功能，创建列存普通表，指定阈值为1000，插入小于1000数据量
Description :
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'column_tab22'
        self.index = 'column_index22'
    ...
    def test_column_unique_index(self):
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0022开始执行")
        ...
        logger.info("======步骤7:插入小于1000数据，数据不唯一且存在空值======")
        ...
        logger.info("======步骤8:插入小于1000合理数据，更新表中数据为已存在的数据======")
    ...
    def tearDown(self):
        ...
        if 'off' != recov_res.splitlines()[-2].strip():
            ...
            result = primary_sh.get_db_cluster_status()
            self.assertTrue("Degraded" in result or "Normal" in result)
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0022执行结束")
