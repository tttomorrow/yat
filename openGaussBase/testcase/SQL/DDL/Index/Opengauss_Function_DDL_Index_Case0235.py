...
'''
Case Type： 功能
Case Name： 不同表并行创建concurrently索引和普通索引
Descption:
1.创建表a，并插入数据
2.创建表b，并插入数据
3.同时创建concurrently索引和普通索引
'''
import unittest
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0235'
        self.tblname_no_relevance = 'ddl_index_case0235_no_rel'
        self.idxname = 'ddl_index_case0235_idx'
        self.idxname_no_relevance = 'ddl_index_case0235_idx_no_rel'
        self.log.info(
            '------------------------------------Opengauss_Function_DDL_Index_Case0235.py start------------------------------------')
...
update {self.tblname_no_relevance} set first_name=\'测试查询不阻塞\', last_name=\'测试%%不阻塞\' where id = 712;')
        ...
        result = self.primary_sh.execut_db_sql(
            f'create index  {self.idxname_no_relevance} on {self.tblname_no_relevance} using btree(id);')
