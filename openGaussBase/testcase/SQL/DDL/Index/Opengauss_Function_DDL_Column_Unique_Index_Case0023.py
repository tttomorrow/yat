...
Case Name   : 开启delta表功能，创建列存本地临时表，指定阈值为1000，插入小于1000数据量
Description :
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'column_tab23'
        self.index = 'column_index23'
    ...
    def test_column_unique_index(self):
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0023开始执行")
        ...
        logger.info(sql_res7_2)
        sql_res8_1 = sql_res.splitlines()[-5].strip()
        logger.info("sql_res8_1")
        logger.info(sql_res8_1)
        sql_res8_2 = sql_res.splitlines()[-4].strip()
        ...
        self.assertTrue(self.constant.INSERT_SUCCESS_MSG in sql_res8_1
                        and self.constant.UPDATE_SUCCESS_MSG in sql_res8_2)
        logger.info("======步骤8校验通过======")
    ...
    def tearDown(self):
        ...
        if 'off' != recov_res.splitlines()[-2].strip():
            ...
            result = primary_sh.get_db_cluster_status()
            self.assertTrue("Degraded" in result or "Normal" in result)
        logger.info("Opengauss_Function_Column_Unique_Index_Case0023执行结束")
