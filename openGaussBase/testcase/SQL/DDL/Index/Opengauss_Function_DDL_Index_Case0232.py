...
'''
Case Type： 功能
Case Name： 创建索引过程中在事务内对无关表进行delete操作并对原表进行delete操作
Descption:
1.创建表，并插入大数据
2.使用concurrently创建索引
3.在创建索引的同时开启事务对无关表进行 delete操作并且删除相关表数据
4.查询索引是否存在
5. 查询删除数据
'''
import unittest
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0232'
        self.tblname_no_rel = 'ddl_index_case0232_no_rel'
        self.idxname = 'ddl_index_case0232_idx'
        self.log.info(
            '------------------------------------Opengauss_Function_DDL_Index_Case0232.py start------------------------------------')

    def test_in_select(self):
        self.log.info('-------------------create table--------------------------------')
        result = self.primary_sh.execut_db_sql(f'DROP TABLE IF EXISTS {self.tblname};CREATE TABLE {self.tblname}(id INT, c_point point);')
...
update {self.tblname} set c_point=point\'(1,1)\' where id = 712;')
        ...
        result = self.primary_sh.execut_db_sql(
            f'START TRANSACTION;delete  FROM {self.tblname_no_rel} where first_name=\'测试查询不阻塞\'; delete from {self.tblname} where c_point  <^ point(1,2);')
        ...
        result = self.primary_sh.execut_db_sql(f'SET ENABLE_SEQSCAN=off;explain select * from {self.tblname} where c_point  <^ point(1,2);')
        self.log.info(result)
