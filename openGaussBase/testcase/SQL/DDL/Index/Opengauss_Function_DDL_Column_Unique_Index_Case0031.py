...
Case Name   : 开启delta表功能，指定阈值为1000，执行vacuum后，再次插入数据
Description :
    ...
    5、普通列存表执行vacuum，列存分区表执行vacuum partition;
...
Expect      :
    ...
    4、两个表插入数据均成功;
    5、执行vacuum成功，执行vacuum partition成功;
    6、再次插入数据成功;
    7、清理环境成功;
History     :
"""
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'column_tab31'
        self.part_table = 'column_part_tab31'
    ...
    def test_column_unique_index(self):
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0031开始执行")
        ...
        logger.info("======步骤5:执行vacuum && vacuum partition======")
    ...
    def tearDown(self):
        ...
        if 'off' != recov_res.splitlines()[-2].strip():
            ...
            result = primary_sh.get_db_cluster_status()
            self.assertTrue("Degraded" in result or "Normal" in result)
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0031执行结束")
