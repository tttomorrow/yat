...
Case Name   : 执行vacuum full后，再次插入数据
Description :
    1、创建普通列存表，指定主键约束;
    2、创建列存分区表，指定唯一约束
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'column_tab39'
        self.part_table = 'column_part_tab39'
        logger.info("======SetUp:检查数据库状态是否正常======")
        status = primary_sh.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)

    def test_column_unique_index(self):
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0039开始执行")
        ...
        logger.info("======步骤4:执行vacuum full && vacuum full partition======")
    ...
    def tearDown(self):
        ...
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0039执行结束")
