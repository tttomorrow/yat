...
Case Name   : 开启delta表功能，创建列存范围分区表，指定阈值为1000，插入小于1000数据量
Description :
    1、开启列存delta表功能，重启数据库;
       gs_guc set -N all -D {dn1} -c "enable_delta_store = on"
    2、创建列存范围分区表，指定deltarow_threshold阈值为1000;
    3、创建唯一索引;
    4、插入小于1000数据，数据唯一且非空值;
    5、清空表数据，插入小于1000数据，数据唯一且存在空值;
    6、清空表数据，插入小于1000数据，数据不唯一且非空值;
    7、插入小于1000数据，数据不唯一且存在空值;
    8、插入小于1000合理数据，更新表中数据为已存在的数据;
    9、清空表数据，插入小于1000合理数据，更新表中数据为不存在的数据;
    10、清理环境;
Expect      :
    1、设置参数成功，重启数据库成功;
    2、创建列存范围分区表、指定阈值为10000成功;
    3、创建唯一索引成功;
    4、插入数据成功;
    5、插入数据成功;
    6、插入数据失败;
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'column_tab24'
        self.index = 'column_index24'
    ...
    def test_column_unique_index(self):
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0024开始执行")
        logger.info("======步骤2:创建列存范围分区表，指定deltarow_threshold阈值为1000======")
        ...
        sql_cmd1 = f'''drop table if exists {self.table};
            ...
            (partition part_1 values less than(500),
             partition part_2 values less than(1000),
        ...
        logger.info("======步骤5:清空表数据，插入小于1000数据，数据唯一且存在空值======")
        sql_cmd3 = f'''truncate table {self.table};
            insert into {self.table} values(generate_series(1,996));
        ...
        logger.info("======步骤6:清空表数据，插入小于1000数据，数据不唯一且非空值======")
        sql_cmd4 = f'''truncate table {self.table};
            insert into {self.table} values(generate_series(1,499));
            insert into {self.table} values(generate_series(1,499));'''
        ...
        logger.info("======步骤7:清空表数据,插入小于1000数据，数据不唯一且存在空值======")
        ...
        logger.info("======步骤8:清空表数据，插入小于1000合理数据，更新表中数据为已存在的数据======")
        ...
        self.assertTrue(self.constant.INSERT_SUCCESS_MSG in sql_res6
                        and self.constant.UPDATE_SUCCESS_MSG in sql_res6)

        logger.info("======步骤9:清空表数据，插入小于1000合理数据，更新表中数据为不存在的数据======")
        sql_cmd7 = f'''truncate table {self.table};
            insert into {self.table} values(generate_series(1,999));
            update {self.table} set id=20210513 where id=666;'''
    ...
    def tearDown(self):
        ...
        if 'off' != recov_res.splitlines()[-2].strip():
            ...
            result = primary_sh.get_db_cluster_status()
            self.assertTrue("Degraded" in result or "Normal" in result)
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0024执行结束")
