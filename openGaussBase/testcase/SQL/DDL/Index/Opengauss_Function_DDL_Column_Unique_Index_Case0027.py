...
Case Name   : 开启delta表功能，创建列存范围分区表，指定阈值为1000，插入大于1000数据量
Description :
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.table = 'column_tab27'
        self.index = 'column_index27'
    ...
    def test_column_unique_index(self):
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0027开始执行")
    ...
    def tearDown(self):
        ...
        if 'off' != recov_res.splitlines()[-2].strip():
            ...
            result = primary_sh.get_db_cluster_status()
            self.assertTrue("Degraded" in result or "Normal" in result)
        logger.info("Opengauss_Function_DDL_Column_Unique_Index_Case0027执行结束")
