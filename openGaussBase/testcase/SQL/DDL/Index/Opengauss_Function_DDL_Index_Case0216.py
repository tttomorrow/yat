...
'''
Case Type： 功能
Case Name： 创建索引过程中在事务内对无关表进行delete操作
Descption:
1.创建表，并插入大数据
2.使用concurrently创建索引
3.在创建索引的同时开启事务对无关表进行delete操作
4.查询索引是否存在
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0216'
        self.tblname_no_rel = 'ddl_index_case0216_no_rel'
        self.idxname = 'ddl_index_case0216_idx'
        self.log.info(
            '------------------------------------Opengauss_Function_DDL_Index_Case0216.py start------------------------------------')
...
update {self.tblname} set c_point=point\'(1,1)\' where id = 712;')
        ...
        result = self.primary_sh.execut_db_sql(
            f'START TRANSACTION;delete  FROM {self.tblname_no_rel} where first_name=\'测试查询不阻塞\'; END;')
