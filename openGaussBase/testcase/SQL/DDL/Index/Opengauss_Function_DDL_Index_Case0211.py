...
'''
Case Type： 功能
Case Name： 创建索引过程中在事务外对相关表进行delete操作
Descption:
1.创建表，并插入大数据
2.使用concurrently创建索引
3.在创建索引的同时对表进行 update操作
4.查询索引是否存在
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0211'
        self.idxname = 'ddl_index_case0211_idx'
        self.log.info(
            '------------------------------------Opengauss_Function_ddl_index_case0211.py start------------------------------------')
...
update {self.tblname} set first_name=\'测试查询不阻塞\', last_name=\'测试%%不阻塞\' where id = 712;')
        ...
        result = self.primary_sh.execut_db_sql(f'delete  FROM {self.tblname} where id=712; ')
        ...
        result = self.primary_sh.execut_db_sql(f'select * from {self.tblname} where id = 712;')
        self.log.info(result)
