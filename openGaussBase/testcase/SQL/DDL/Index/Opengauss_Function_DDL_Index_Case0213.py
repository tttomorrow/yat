...
'''
Case Type： 功能
Case Name： 创建索引过程中在事务内对无关表进行select操作
Descption:
1.创建表，并插入大数据
2.使用concurrently创建索引
3.在创建索引的同时开启事务对无关表进行 select操作
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0213'
        self.tblname_no_rel = 'ddl_index_case0213_no_rel'
        self.idxname = 'ddl_index_case0213_idx'
        self.log.info(
            '------------------------------------Opengauss_Function_DDL_Index_Case0213.py start------------------------------------')
...
update {self.tblname_no_rel} set first_name=\'test hung^%\', last_name=\'123 test num \\\' where id = 712;')
        self.log.info(result)
        self.assertIn(self.Constant.INSERT_SUCCESS_MSG, result)
        self.assertIn(self.Constant.UPDATE_SUCCESS_MSG, result)
        ...
        self.log.info('-----------------start transaction & select table-----------------------------')
        before_time = self.dbPrimaryDbUser.sh("date +'%s'").result()
        result = self.primary_sh.execut_db_sql(
            f'START TRANSACTION;SELECT * FROM {self.tblname_no_rel} where id = 712; END;')
    ...
    def tearDown(self):
        ...
        self.log.info(result)
        result = self.primary_sh.execut_db_sql(f'RESET ENABLE_SEQSCAN;drop table if exists {self.tblname_no_rel};')
        self.log.info(result)
