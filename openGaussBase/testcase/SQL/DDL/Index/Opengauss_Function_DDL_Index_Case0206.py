...
'''
Case Type： 功能
Case Name： 创建索引过程中在事务内对相关表进行select操作
Descption:
1.创建表，并插入大数据
2.使用concurrently创建索引
3.在创建索引的同时开启事务对表进行 select操作
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0206'
        self.idxname = 'ddl_index_case0206_idx'
        self.log.info(
            '------------------------------------Opengauss_Function_DDL_Index_Case0206.py start------------------------------------')
...
update {self.tblname} set first_name=\'test hung^%\', last_name=\'123 test num \\\' where id = 712;')
        ...
        result = self.primary_sh.execut_db_sql(
            f'START TRANSACTION;SELECT * FROM {self.tblname} where to_tsvector(\'english\',first_name)@@ to_tsquery(\'english\', \'test\'); END;')
        ...
        result = self.primary_sh.execut_db_sql(
            f'SET ENABLE_SEQSCAN=off;explain SELECT * FROM {self.tblname} where to_tsvector(\'english\',first_name)@@ to_tsquery(\'english\', \'test\');')
        self.log.info(result)
