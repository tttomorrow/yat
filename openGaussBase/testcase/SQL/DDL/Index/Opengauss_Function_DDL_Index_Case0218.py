...
'''
Case Type： 功能
Case Name： 创建索引过程中在事务外对无关表进行update操作
Descption:
1.创建表，并插入大数据
2.使用concurrently创建索引
3.在创建索引的同时对无关表进行 update操作
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0218'
        self.idxname = 'ddl_index_case0218_idx'
        self.tblname_no_rel = 'ddl_index_case0218_no_rel'
        self.log.info('----------------------Opengauss_Function_ddl_index_case0218.py start---------------------------')
    ...
    def test_in_select(self):
        ...
        result = self.primary_sh.execut_db_sql(
            f'update {self.tblname_no_rel} set first_name=\'测试 test 查询不阻塞\', last_name=\'测试--不阻塞\' where id = 712;')
        ...
        result = self.primary_sh.execut_db_sql(
            f'SET ENABLE_SEQSCAN=off;SELECT * FROM {self.tblname_no_rel} where id=712;')
        self.log.info(result)
