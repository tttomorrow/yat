...
class DDLTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.table = 't_columns_unique_index_0094'
        self.index = 'i_columns_unique_index_0094'
        self.const = 'const_94'
    ...
    def test_column_unique_index(self):
        self.log.info('---Opengauss_Function_DDL_Column_Unique_Index_Case0094'
                      '开始执行---')
        ...
        sql_cmd1 = f'''drop table if exists {self.table};
            ...
            alter table {self.table} add primary key(age);
            alter table {self.table} add constraint {self.const} unique(age);
            create unique index {self.index} on {self.table} 
            using btree(age) local;
