...
Case Name   : 创建列存分区表,gs_dump导出表后，添加唯一约束等，导入表
Description :
    1、创建列存分区表,插入数据
...
class DDLTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.table = 't_columns_unique_index_0092'
        self.index = 'i_columns_unique_index_0092'
    ...
    def test_column_unique_index(self):
        self.log.info('---Opengauss_Function_DDL_Column_Unique_Index_Case0092'
                      '开始执行---')
        text = '-----step1:创建列存分区表，插入数据   expect:成功-----'
        self.log.info(text)
        ...
        shell2 = f'''sed -i "{lines} ialter table {self.table} add primary\
            key(age);alter table {self.table} add constraint cons_91 \
            unique(age);create unique index {self.index} on {self.table} \
            using btree(age) local;" {self.dump_path}'''
        ...
        text = '------step5:查看数据，再次插入已存在数据   expect:失败------'
        self.log.info(text)
