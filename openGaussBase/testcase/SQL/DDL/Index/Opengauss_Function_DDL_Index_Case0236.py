...
'''
Case Type： 功能
Case Name： 相同表并行创建concurrently索引
Descption:
1.创建表a，并插入数据
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0236'
        self.idxname = 'ddl_index_case0236_idx'
        self.idxname1 = 'ddl_index_case0236_idx1'
        self.log.info(
            '------------------------------------Opengauss_Function_DDL_Index_Case0236.py start------------------------------------')
...
update {self.tblname} set first_name=\'测试查询不阻塞\', last_name=\'测试%%不阻塞\' where id = 712;')
        ...
        result = self.primary_sh.execut_db_sql(
            f'select current_time;create index concurrently {self.idxname1} on {self.tblname} using btree(id);')
        self.log.info(result)
        ...
        self.assertTrue(('deadlock detected' in result) or ('deadlock detected' in create_idx_result))
        self.assertTrue(
