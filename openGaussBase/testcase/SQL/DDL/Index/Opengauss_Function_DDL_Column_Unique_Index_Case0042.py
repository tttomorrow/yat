...
"""
Case Type   : DDL
Case Name   : 开启多个事务，session1中创建索引，插入数据，session2不开启事务，插入相同数据
Description :
    1.创建列存表
    2.开启两个会话，session1中创建唯一索引，插入数据，未提交时，session2中插入相同数据，session2阻塞
    3.校验数据是否插入成功
    4.清理环境
Expect      :
    1.创建列存表成功
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DDL_Column_Unique_Index_Case0042开始')
        self.constant = Constant()
        self.commonsh1 = CommonSH('PrimaryDbUser')
        self.commonsh2 = CommonSH('PrimaryDbUser')
        self.tb_name = 't_column_tab_0042'
        self.index_name = 'i_column_index_0042'
    ...
    def test_unique_index(self):
        ...
        text2 = '-----step2.1 session1开启事务插入数据，先不提交，待session2开启后' \
                '再提交session1; expect:session1提交成功-----'
        self.log.info(text2)
        sql_01 = f"start transaction;" \
            f"create unique index {self.index_name} " \
            f"on {self.tb_name} using btree(id2);" \
        ...
        text3 = '-----step2.2 session2不开启事务插入数据; expect:session2阻塞，' \
        ...
        expect_msg = f'ERROR:  duplicate key value violates ' \
            f'unique constraint \"t_column_tab_0042_pkey\"'
        result = msg_result_1 + msg_result_2_2
        self.assertEqual(result.count(expect_msg), 1, '执行失败:' + text3)
        self.assertEqual(result.count('START TRANSACTION'), 1,
                         '执行失败:' + text2 + text3)
        ...
        text4 = '---step3.校验数据是否插入成功; expect:插入1000条数据成功---'
        self.log.info(text4)
        sql_cmd = self.commonsh1.execut_db_sql(
            f'select count(*) from {self.tb_name};')
        self.log.info(sql_cmd)
        self.assertIn('1000', sql_cmd, '执行失败:' + text4)

    def tearDown(self):
        text5 = '--step4.清理环境; 清理环境成功--'
        self.log.info(text5)
        sql_cmd = self.commonsh1.execut_db_sql(f'drop table {self.tb_name};')
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_DDL_Column_Unique_Index_Case0042结束')
