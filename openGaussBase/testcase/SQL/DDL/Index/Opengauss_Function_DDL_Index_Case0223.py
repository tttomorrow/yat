...
'''
Case Type： 功能
Case Name：使用concurrently创建部分索引，部分索引为已有数据范围
Descption:
1.创建表，并插入数据
3.创建部分在线索引
4.创建索引的同时对表插入数据
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0223'
        self.idxname = 'ddl_index_case0223_idx'
        self.log.info('----------------------Opengauss_Function_ddl_index_case0223.py start---------------------------')
    ...
    def test_in_select(self):
        ...
        sql = f'select current_time;create index concurrently {self.idxname} on {self.tblname}(id) where id < 8000000;select current_time;'
