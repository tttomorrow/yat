'''
Case Type： 功能
Case Name： 创建索引过程中在事务内对无关表进行insert操作并对原表进行update操作
Descption:
1.创建表，并插入大数据
2.使用concurrently创建索引
3.在创建索引的同时开启事务对无关表进行insert操作对相关表update操作
...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tblname = 'ddl_index_case0233'
        self.tblname_no_relevance = 'ddl_index_case0233_no_rel'
        self.idxname = 'ddl_index_case0233_idx'
        self.log.info(
            '------------------------------------Opengauss_Function_DDL_Index_Case0233.py start------------------------------------')
...
update {self.tblname_no_relevance} set first_name=\'测试查询不阻塞\', last_name=\'测试%%不阻塞\' where id = 712;')
        ...
        self.log.info('-----------------start transaction & insert no relevance table & update relevance table-----------------------------')
        before_time = self.dbPrimaryDbUser.sh("date +'%s'").result()
        result = self.primary_sh.execut_db_sql(
            f'START TRANSACTION;insert into {self.tblname_no_relevance} values(200000001,\'^c test 号\',\'\\test--\');update {self.tblname_no_relevance} set first_name=\'test\', last_name=\'测试%%不阻塞\' where id = 712; ')
