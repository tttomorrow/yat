...
class Concurrently(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.value = 1000000
        self.log.info(
            '------------------------------------Opengauss_Function_ddl_index_case0212.py start------------------------------------')
    ...
    def test_in_select(self):
        ...
        result = self.primary_sh.execut_db_sql(
            f"INSERT INTO {self.tblname} SELECT id, md5(random()::text), md5(random()::text) FROM (SELECT * FROM generate_series(1,{self.value}) AS id) AS x;\
update {self.tblname} set first_name='测试查询不阻塞', last_name='测试%%不阻塞' where id = 712;")
        ...
        result = self.primary_sh.execut_db_sql(
            f'insert into {self.tblname} values(200000001,\'^c test 号\',\'\\test--\'); ')
