--  @testpoint: create gin index:在同一列上创建部分索引 和普通索引，再进行部分索引设置范围内的查询
...
CREATE INDEX  test_gin_row2_first_name_idx2  ON test_gin_row2 USING GIN(to_tsvector('english', first_name)) where(to_tsvector('english', first_name) @@ to_tsquery('english', 'formed'));
...
INSERT INTO test_gin_row2 VALUES(5001, 'Mexico, officially the United Mexican States, is a federal republic in the southern part of North America.', 'Mexico');
...
SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'Mexico');
explain SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'Mexico');
...
--reindex 
reindex index test_gin_row2_first_name_idx;
SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'formed');
explain SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'formed');
SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'Mexico');
explain SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'Mexico');
...
--alter
--fail
alter index test_gin_row2_first_name_idx rename to test_gin_row2_first_name_idx2;
--successfully
alter index test_gin_row2_first_name_idx rename to test_gin_row2_first_name_idx1;
SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'formed');
explain SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'formed');
SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'Mexico');
explain SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'Mexico');
