...
--query & check
explain SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'test') ORDER BY num, data1, data2;SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'test') ORDER BY num, data1, data2;
...
SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'test') ORDER BY num, data1, data2;
...
--query
SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'formed');
explain SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'formed');
SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'America');
explain SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'America');
--update data
update test_gin_student_column set data1='America is a rock' WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'America');
SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'formed');
explain SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'formed');
update test_gin_student_column set data2='test' WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'America');
SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'America');
explain SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'America');
--delete data
delete from test_gin_student_column where to_tsvector('english', data2) @@ to_tsquery('english', 'America');
SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'rock');
explain SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'rock');
SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'America');
explain SELECT * FROM test_gin_student_column WHERE to_tsvector('english', data2) @@ to_tsquery('english', 'America');
