--  @testpoint: create gin index: FASTUPDATE=on
...
--create gin index
--successfully(COLUMN)
CREATE INDEX  test_gin_2_first_name_idx  ON test_gin_2 USING GIN(to_tsvector('english', first_name)) with (FASTUPDATE=on);
--successfully(row)
CREATE INDEX  test_gin_row1_idx ON test_gin_row1 USING GIN(info) with (FASTUPDATE=on);
--successfully(row)
CREATE INDEX  test_gin_row2_first_name_idx  ON test_gin_row2 USING GIN(to_tsvector('english', first_name)) with (FASTUPDATE=on);
