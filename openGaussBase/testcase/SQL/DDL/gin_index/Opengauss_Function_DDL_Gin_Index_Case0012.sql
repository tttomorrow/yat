--  @testpoint: create gin index:FASTUPDATE=on,GIN_PENDING_LIST_LIMIT
...
CREATE INDEX test_gin_student_index_column1 ON test_gin_student_column USING GIN(to_tsvector('english', data1)) LOCAL with (FASTUPDATE=on,GIN_PENDING_LIST_LIMIT=63) ;
...
--didn't refer to special partition 
CREATE INDEX test_gin_student_index_row1 ON test_gin_student_row USING GIN(to_tsvector('english', data1)) LOCAL with (FASTUPDATE=on,GIN_PENDING_LIST_LIMIT=63) ;
--refer to special partition
...
--didn't refer to special partition 
CREATE INDEX test_gin_student_index_row1 ON test_gin_student_row USING GIN(to_tsvector('english', data1)) LOCAL with (FASTUPDATE=on,GIN_PENDING_LIST_LIMIT=64) ;
--refer to special partition
CREATE INDEX test_gin_student_index_row2 ON test_gin_student_row USING GIN(to_tsvector('english', data2)) LOCAL
(
    PARTITION data2_index_1,
