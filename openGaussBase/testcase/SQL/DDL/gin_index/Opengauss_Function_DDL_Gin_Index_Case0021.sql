...
CREATE INDEX test_gin_student_index_row2 ON test_gin_student_row USING GIN(to_tsvector('english', data1)) LOCAL
(
    PARTITION data2_index_1,
    PARTITION data2_index_2,
    PARTITION data2_index_3 
) ;
CREATE INDEX test_gin_student_index_row3 ON test_gin_student_row USING GIN(to_tsvector('simple', data1)) LOCAL;


--query
explain SELECT * FROM test_gin_student_row WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'Mexico') ORDER BY num, data1, data2;
explain SELECT * FROM test_gin_student_row WHERE to_tsvector('english', data2) @@ to_tsquery('simple', 'Mexico') ORDER BY num, data1, data2;
--reindex
reindex index test_gin_student_index_row1;
explain SELECT * FROM test_gin_student_row WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'Mexico') ORDER BY num, data1, data2;
explain SELECT * FROM test_gin_student_row WHERE to_tsvector('english', data2) @@ to_tsquery('simple', 'Mexico') ORDER BY num, data1, data2;

--drop index
drop index test_gin_student_index_row1;
explain SELECT * FROM test_gin_student_row WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'Mexico') ORDER BY num, data1, data2;
explain SELECT * FROM test_gin_student_row WHERE to_tsvector('english', data2) @@ to_tsquery('simple', 'Mexico') ORDER BY num, data1, data2;

--alter index
alter index test_gin_student_index_row2 RENAME PARTITION data2_index_2 to data2_index_x;
explain SELECT * FROM test_gin_student_row WHERE to_tsvector('english', data1) @@ to_tsquery('english', 'Mexico') ORDER BY num, data1, data2;
explain SELECT * FROM test_gin_student_row WHERE to_tsvector('english', data2) @@ to_tsquery('simple', 'Mexico') ORDER BY num, data1, data2;
--query & check result
