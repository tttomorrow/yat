...
update test_gin_row2 set first_name='America is a rock' WHERE to_tsvector('english', last_name) @@ to_tsquery('english', 'America');
SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'formed');
explain SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'formed');
delete from test_gin_row2 where to_tsvector('english', last_name) @@ to_tsquery('english', 'America');
SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'America');
explain SELECT * FROM test_gin_row2 WHERE to_tsvector('english', first_name) @@ to_tsquery('english', 'America');
