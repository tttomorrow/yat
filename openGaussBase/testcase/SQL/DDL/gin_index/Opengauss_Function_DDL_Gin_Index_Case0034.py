...
"""
Case Type   : 功能
Case Name   : 分区表创建索引前dump，创建索引后 恢复数据-c
Description :
...
class set_search_limit(unittest.TestCase):
    ...
    dump_file_name = 'Opengauss_Gin_Index_0034.tar'
    dump_file = os.path.join(macro.DB_INSTANCE_PATH, dump_file_name)

    def setUp(self):
        logger.info("-----------this is setup-----------")
        self.Constant = Constant()
        logger.info("-----------Opengauss_Gin_Index_0034 start-----------")
    ...
    def tearDown(self):
        ...
        for i in range(int(self.node_num)):
            ...
            logger.info(result)
        time.sleep(10)
        logger.info("-----------Opengauss_Gin_Index_0034 end-----------")
