...
class set_search_limit(unittest.TestCase):
    ...
    dump_file_name = 'Opengauss_Gin_Index_0030.tar'
    dump_file = os.path.join(macro.DB_INSTANCE_PATH, dump_file_name)

    def setUp(self):
        logger.info("-----------this is setup-----------")
        logger.info("-----------Opengauss_Gin_Index_0030 start-----------")
    ...
    def test_set_search_limit(self):
        ...
        msg = self.commsh.execut_db_sql(sql, dbname=self.dbname)
        logger.info(msg)
        self.assertTrue(msg.find(self.Constant.DROP_TABLE_SUCCESS)>-1)
    ...
    def tearDown(self):
        ...
        logger.info(result)
        logger.info("-----------Opengauss_Gin_Index_0030 end-----------")
