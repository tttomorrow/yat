...
"""
Case Type   : 功能
Case Name   : 创建索引前dump，创建索引后 恢复数据
Description :
...
Expect      :
    ...
    2.dump成功
    3.创建索引成功
    4.恢复数据成功但是恢复过程中会报错，因为导入的表已存在无法重建
    5.索引存在
    6.正常使用索引且查询结果正确
History     :
"""
...
class set_search_limit(unittest.TestCase):
    ...
    dump_file_name = 'Opengauss_Gin_Index_0027.tar'

    def setUp(self):
        logger.info("-----------this is setup-----------")
        self.Constant = Constant()
        logger.info("-----------Opengauss_Gin_Index_0027 start-----------")
    ...
    def tearDown(self):
        ...
        logger.info(result)
        logger.info("-----------Opengauss_Gin_Index_0027 end-----------")
