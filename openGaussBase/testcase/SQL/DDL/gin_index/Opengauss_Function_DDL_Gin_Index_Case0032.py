...
"""
Case Type   : 功能
Case Name   : 创建索引后dump，alter索引， 恢复数据
Description :
...
class set_search_limit(unittest.TestCase):
    ...
    dump_file_name = 'Opengauss_Gin_Index_0032.tar'
    dump_file = os.path.join(macro.DB_INSTANCE_PATH, dump_file_name)

    def setUp(self):
        logger.info("-----------this is setup-----------")
        logger.info("-----------Opengauss_Gin_Index_0032 start-----------")
    ...
    def tearDown(self):
        ...
        logger.info(result)
        logger.info("-----------Opengauss_Gin_Index_0032 end-----------")
