-- @testpoint: 创建表使用表结构里的列，与union all+order by结合
...
create table ct_as as (select c_char c1,c_varchar2 c2 from ct_tbl_as) union all (select c_char c3,c_varchar2 c4 from ct_tbl_as) order by 1;
