-- @testpoint: 临时表与视图结合,先用查询视图，创建成功后，应该不带not null约束
...
create temporary table ct_as as select * from ct_tbl_as_v union all (select c_char c3,c_varchar2 c4 from ct_tbl_as);
drop view if exists ct_tbl_as_v;
drop table if exists ct_as;
drop table if exists ct_tbl_as;