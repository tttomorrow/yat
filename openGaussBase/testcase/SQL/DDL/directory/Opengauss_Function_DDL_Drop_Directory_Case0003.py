...
Case Name   : enable_access_server_directory=off时，
              有sysadmin权限的非初始用户删除目录对象失败，合理报错
Description :
    1.创建目录
    2.初始用户创建目录对象，创建sysadmin权限用户
    3.enable_access_server_directory=off时，有sysadmin权限的非初始用户删除目录对象
...
class DropDirectory(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.dir_name = 'dir_object_drop_directory_0003'
        self.dir_path = os.path.join('/tmp', 'dir_drop_directory_0003')
        self.user_name = 'u_object_drop_directory_0003'
    ...
    def test_drop_directory(self):
        text = "-----step2:初始用户创建目录对象，创建sysadmin权限用户，" \
               "expect:创建成功-----"
        self.logger.info(text)
        ...
        self.assertIn(self.dir_name, sql_msg, '执行失败' + text)

        text = "-----step3:enable_access_server_directory=off时，有sysadmin权限" \
               "的非初始用户删除目录对象，expect:删除失败，合理报错-----"
        self.logger.info(text)
