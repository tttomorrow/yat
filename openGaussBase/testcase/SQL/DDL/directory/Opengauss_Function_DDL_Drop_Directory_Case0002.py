...
Case Name   : enable_access_server_directory=on时，有sysadmin权限的非初始用户删除目录对象
Description :
    1.创建目录
    2.修改guc参数enable_access_server_directory=on
    3.初始用户创建目录对象，创建sysadmin权限用户
    4.有sysadmin权限的非初始用户删除目录对象
    5.删除用户，删除目录
    6.恢复参数
Expect      :
    1.创建目录成功
...
class DropDirectory(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.dir_name = 'dir_object_drop_directory_0002'
        self.dir_path = os.path.join('/tmp', 'dir_drop_directory_0002')
        self.guc_result = ''
        self.user_name = 'u_object_drop_directory_0002'
    ...
    def test_drop_directory(self):
        text = "-----step3:初始用户创建目录对象，创建sysadmin权限用户，" \
               "expect:创建成功-----"
        self.logger.info(text)
        ...
        text = "-----step4:有sysadmin权限的非初始用户删除目录对象，" \
               "expect:删除成功-----"
        self.logger.info(text)
