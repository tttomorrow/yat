...
Case Name   : enable_access_server_directory=on，无SYSADMIN权限的非初始用户创建directory
Description :
    1.创建目录
    2.修改guc参数enable_access_server_directory=on
    3.创建普通用户
    4.无SYSADMIN权限的非初始用户创建directory
    5.通过系统表查看目录信息
    6.删除目录对象
    7.恢复参数
Expect      :
...
class CreateDirectory(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.user_name = 'u_create_directory_0008'
        self.dir_name = 'dir_object_create_directory_0008'
        self.dir_path = os.path.join('/tmp', 'dir_create_directory_0008')
        self.guc_result = ''
    ...
    def test_create_directory(self):
        ...
        text = "-----step4:无SYSADMIN权限的非初始用户创建directory;" \
               "expect:创建失败，合理报错-----"
        self.logger.info(text)
        ...
        self.assertIn(self.constant.PERMISSION_DENIED and
                      "must be sysadmin or a member of the "
                      "gs_role_directory_create role to create a directory",
                      create_msg, '执行失败' + text)
