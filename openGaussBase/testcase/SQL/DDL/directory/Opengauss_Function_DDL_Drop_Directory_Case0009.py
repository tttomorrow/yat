...
Case Name   : enable_access_server_directory=off时，directory属主删除目录对象失败，合理报错
Description :
    1.创建目录
    2.修改guc参数enable_access_server_directory=on
    3.创建sysadmin权限用户,sysadmin用户创建目录对象
    4.修改guc参数enable_access_server_directory=off
    5.sysadmin用户删除目录对象
    6.删除目录对象，删除用户，删除目录
Expect      :
    1.创建目录成功
...
class DropDirectory(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.dir_name = 'dir_object_drop_directory_0009'
        self.dir_path = os.path.join('/tmp', 'dir_drop_directory_0009')
        self.guc_result = ''
        self.check_default = ''
        self.user_name = 'u_object_drop_directory_0009'
    ...
    def test_drop_directory(self):
        text = "-----step3:创建sysadmin权限用户，sysadmin用户创建目录对象" \
               "expect:创建成功-----"
        self.logger.info(text)
        ...
        text = '-----step4:修改guc参数enable_access_server_directory=off;' \
               'expect:修改guc参数成功-----'
        self.logger.info(text)
        ...
        text = "-----step5:directory属主删除目录对象，expect:删除失败，合理报错-----"
        self.logger.info(text)
    ...
    def tearDown(self):
        text = "-----step5:删除目录对象，删除用户，删除目录，expect:删除成功-----"
        self.logger.info(text)
        drop_cmd = f"drop directory {self.dir_name};" \
