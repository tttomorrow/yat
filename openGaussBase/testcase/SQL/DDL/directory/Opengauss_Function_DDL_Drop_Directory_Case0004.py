...
Case Name   : enable_access_server_directory=on时，无sysadmin权限的非初始用户
              删除目录对象失败，合理报错
Description :
    1.创建目录
    2.修改guc参数enable_access_server_directory=on
    3.初始用户创建目录对象，创建普通用户
    4.无sysadmin权限的非初始用户删除目录对象
    5.初始用户删除目录对象，删除用户，删除目录
    6.恢复参数
Expect      :
    1.创建目录成功
    2.修改guc参数成功
...
class DropDirectory(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.dir_name = 'dir_object_drop_directory_0004'
        self.dir_path = os.path.join('/tmp', 'dir_drop_directory_0004')
        self.guc_result = ''
        self.user_name = 'u_object_drop_directory_0004'
    ...
    def test_drop_directory(self):
        text = "-----step3:初始用户创建目录对象，创建普通用户，expect:创建成功-----"
        self.logger.info(text)
        ...
        text = "-----step4:无sysadmin权限的非初始用户删除目录对象，" \
               "expect:删除失败，合理报错---"
        self.logger.info(text)
    ...
    def tearDown(self):
        text_5 = "-----step5:初始用户删除目录对象，删除用户，删除目录，expect:删除成功-----"
