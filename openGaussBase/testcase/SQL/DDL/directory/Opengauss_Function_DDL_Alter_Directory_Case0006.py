...
Case Name   : enable_access_server_directory=off时，有sysadmin权限的用户修改属主失败，合理报错
Description :
    1.创建目录,修改参数enable_access_server_directory=off
    2.初始用户创建目录对象，创建用户
    3.有sysadmin权限的用户将属主修改为自己
    4.有sysadmin权限的用户将属主修改为其它sysadmin用户
    5.有sysadmin权限的用户将属主修改为其它普通用户
    6.删除目录，删除用户
    7.恢复参数
Expect      :
    1.创建目录成功,修改参数成功
...
class AlterDirectory(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.user_name_1 = 'u_alter_directory_0006_01'
        self.user_name_2 = 'u_alter_directory_0006_02'
        self.user_name_3 = 'u_alter_directory_0006_03'
        self.dir_name = 'dir_alter_directory_0006'
        self.dir_path = os.path.join('/tmp', 'dir_alter_directory_0006')
    ...
    def test_alter_directory(self):
        text = '-----step1:创建目录,修改参数enable_access_server_directory=off;' \
        ...
        create_cmd = f"create or replace directory {self.dir_name} " \
            ...
            f"{self.user_name_3};" \
            f"create user {self.user_name_1} sysadmin " \
        ...
        text = "-----step3:有sysadmin权限的用户将属主修改为自己;" \
               "expect:修改失败，合理报错-----"
        self.logger.info(text)
        ...
        text = "-----step4:有sysadmin权限的用户将属主修改为其它sysadmin用户; " \
               "expect:修改失败，合理报错-----"
        self.logger.info(text)
        alter_cmd = f"alter directory {self.dir_name} " \
        ...
        text = "-----step5:有sysadmin权限的用户将属主修改为其它普通用户;" \
               " expect:修改失败，合理报错-----"
        self.logger.info(text)
        alter_cmd = f"alter directory {self.dir_name} " \
    ...
    def tearDown(self):
        text = "-----step6:删除目录，删除用户;expect:删除成功-----"
        self.logger.info(text)
