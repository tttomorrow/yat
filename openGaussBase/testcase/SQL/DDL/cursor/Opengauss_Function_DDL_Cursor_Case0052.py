...
'''
--  Case Type:Cursor
--  testpoint:declare声明游标，使用value子句，子句类型为字符类型；
--  date:2020-11-03
'''
...
class CURSOR_TEST(unittest.TestCase):
    ...
    def test_cursor_declare(self):
        logger.info("------------------Opengauss_Function_DDL_Cursor_Case0052开始执行-----------------")
        logger.info("======结合value子句，子句类型为字符类型======")
        sql_cmd = f'''
                    start transaction;
                    declare cursor52 cursor for values(char 'char',varchar(10) 'cursor test',cast('type clob' as clob),text 'type text');
                    fetch from cursor52;
                    close cursor52;
                    end;
                    '''
    ...
    def tearDown(self):
        ...
        logger.info('---------------Opengauss_Function_DDL_Cursor_Case0052执行结束---------------')
