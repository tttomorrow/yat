...
'''
--  Case Type:Cursor
--  testpoint:declare声明游标，使用select限定条件(group by)查询，指定游标返回的行；
--  date:2020-11-03
'''
...
class CURSOR_TEST(unittest.TestCase):
    ...
    def test_cursor_declare(self):
        logger.info("------------------Opengauss_Function_DDL_Cursor_Case0045开始执行-----------------")
        logger.info("======前置条件，创建表======")
        sql_cmd1 = f'''
                    drop table if exists cur_test_45;
                    create table cur_test_45(c_id int,c_num int,c_name varchar(10),c_city varchar(10),c_add varchar(20));
                    insert into cur_test_45 values(1,18,'Allen','Beijing','AAAAABAAAAA'),(21,368,'Bob','Shanghai','AAAAACAAAAA'),
        ...
        logger.info("======使用select限定条件(group by)查询，指定游标返回的行======")
        sql_cmd2 = f'''
                    start transaction;
                    declare cursor45 cursor for select count(c_name),c_add from cur_test_45 group by c_add;
                    fetch from cursor45;
                    close cursor45;
                    end;
                    '''
        ...
        self.assertIn('AAAAAEAAAAA', msg2)
    ...
    def tearDown(self):
        logger.info('======清理环境======')
        sql_cmd3 = f'''
                    drop table cur_test_45 cascade;
                   '''
        msg3 = self.sh_primysh.execut_db_sql(sql_cmd3)
        logger.info(msg3)
        logger.info('---------------Opengauss_Function_DDL_Cursor_Case0045执行结束---------------')
