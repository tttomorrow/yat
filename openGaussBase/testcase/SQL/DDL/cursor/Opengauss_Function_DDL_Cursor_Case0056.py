...
'''
--  Case Type:Cursor
--  testpoint:declare声明游标，使用value子句，子句类型为网络地址类型；
--  date:2020-11-03
'''
...
class CURSOR_TEST(unittest.TestCase):
    ...
    def test_cursor_declare(self):
        logger.info("------------------Opengauss_Function_DDL_Cursor_Case0056开始执行-----------------")
        logger.info("======结合value子句，子句类型为网络地址类型======")
        sql_cmd = f'''
                    start transaction;
                    declare cursor56 cursor for values(broadcast('192.168.1.5/24'),abbrev(inet '10.1.0.0/16'));
                    fetch from cursor56;
                    close cursor56;
                    end;
                    '''
        ...
        self.assertIn('192.168.1.255/24', msg)
    ...
    def tearDown(self):
        ...
        logger.info('---------------Opengauss_Function_DDL_Cursor_Case0056执行结束---------------')
