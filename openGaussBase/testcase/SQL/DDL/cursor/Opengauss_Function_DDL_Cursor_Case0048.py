...
'''
--  Case Type:Cursor
--  testpoint:declare声明游标，使用value子句，子句长度为多行；
--  date:2020-11-03
'''
...
class CURSOR_TEST(unittest.TestCase):
    ...
    def test_cursor_declare(self):
        logger.info("------------------Opengauss_Function_DDL_Cursor_Case0048开始执行-----------------")
        logger.info("======结合value子句，子句长度为多行======")
        sql_cmd = f'''
                    start transaction;
                    declare cursor48 cursor for values(0,1),(1,2),(2,3),(3,4),(4,5) ;
                    fetch from cursor48;
                    close cursor48;
                    end;
                    '''
    ...
    def tearDown(self):
        ...
        logger.info('---------------Opengauss_Function_DDL_Cursor_Case0048执行结束---------------')
