...
'''
--  Case Type:Cursor
--  testpoint:declare声明游标，使用value子句，子句类型为常用数据混合数据类型；
--  date:2020-11-03
'''
...
class CURSOR_TEST(unittest.TestCase):
    ...
    def test_cursor_declare(self):
        logger.info("------------------Opengauss_Function_DDL_Cursor_Case0059开始执行-----------------")
        logger.info("======结合value子句，子句类型为常用数据混合数据类型======")
        sql_cmd = f'''
                    start transaction;
                    declare cursor59 cursor for values(102,char 'cursor_test',text 'cursor',TRUE,FALSE,'12.34'::float8::numeric::money);
                    fetch from cursor59;
                    close cursor59;
                    end;
                    '''
    ...
    def tearDown(self):
        ...
        logger.info('---------------Opengauss_Function_DDL_Cursor_Case0059执行结束---------------')
