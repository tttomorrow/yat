...
'''
--  Case Type:Cursor
--  testpoint:declare声明游标，以默认文本格式返回数据；
--  date:2020-11-03
'''
...
class CURSOR_TEST(unittest.TestCase):
    ...
    def test_cursor_declare(self):
        logger.info("------------------Opengauss_Function_DDL_Cursor_Case0034开始执行-----------------")
        logger.info("======前置条件，创建表======")
        sql_cmd1 = f'''
                    drop table if exists cur_test_34;
                    create table cur_test_34(c_id int,c_num int,c_name varchar(10),c_city varchar(10),c_add varchar(20));
                    insert into cur_test_34 values(1,18,'Allen','Beijing','AAAAABAAAAA'),(2,368,'Bob','Shanghai','AAAAACAAAAA'),
        ...
        logger.info("======以默认文本格式返回数据======")
        sql_cmd2 = f'''
                    start transaction;
                    declare cursor34 cursor for select * from cur_test_34 order by 1;
                    fetch from cursor34;
                    close cursor34;
                    end;
                    '''
    ...
    def tearDown(self):
        logger.info('======清理环境======')
        sql_cmd3 = f'''
                    drop table cur_test_34 cascade;
                   '''
        msg3 = self.sh_primysh.execut_db_sql(sql_cmd3)
        logger.info(msg3)
        logger.info('---------------Opengauss_Function_DDL_Cursor_Case0034执行结束---------------')
