...
"""
'''
#--  @testpoint:cursor声明游标，使用value子句，子句类型为货币类型
'''
...
class Hostname(unittest.TestCase):
    ...
    def setUp(self):
        logger.info("------------------------Opengauss_BaseFunc_DDL_Cursor_0019开始执行--------------------------")

# value子句，类型为货币类型
    def test_pascal(self):
        self.sql_cmd = f'''
                        start transaction;
                        cursor cursor19 for values('12.34'::float8::numeric::money);
                        fetch from cursor19;
                        close cursor19;
                        end;
                        '''
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_BaseFunc_DDL_Cursor_0019执行结束--------------------------')
