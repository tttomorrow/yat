...
'''
--  Case Type:Cursor
--  testpoint:declare声明游标，使用value子句，子句类型为布尔类型；
--  date:2020-11-03
'''
...
class CURSOR_TEST(unittest.TestCase):
    ...
    def test_cursor_declare(self):
        logger.info("------------------Opengauss_Function_DDL_Cursor_Case0051开始执行-----------------")
        logger.info("======结合value子句，子句类型为布尔类型======")
        sql_cmd = f'''
                    start transaction;
                    declare cursor51 cursor for values(TRUE,true),(FALSE,false);
                    fetch forward 2 from cursor51;
                    close cursor51;
                    end;
                    '''
    ...
    def tearDown(self):
        ...
        logger.info('---------------Opengauss_Function_DDL_Cursor_Case0051执行结束---------------')
