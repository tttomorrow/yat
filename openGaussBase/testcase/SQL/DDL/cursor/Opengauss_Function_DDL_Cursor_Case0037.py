...
'''
--  Case Type:Cursor
--  testpoint:declare声明游标，不以默认方式检索数据行，参数设为no scroll，不以倒序方式提取数据；
--  date:2020-11-03
'''
...
class CURSOR_TEST(unittest.TestCase):
    ...
    def test_cursor_declare(self):
        logger.info("------------------Opengauss_Function_DDL_Cursor_Case0037开始执行-----------------")
        logger.info("======前置条件，创建表======")
        sql_cmd1 = f'''
                    drop table if exists cur_test_37;
                    create table cur_test_37(c_id int,c_num int,c_name varchar(10),c_city varchar(10),c_add varchar(20));
                    insert into cur_test_37 values(1,18,'Allen','Beijing','AAAAABAAAAA'),(2,368,'Bob','Shanghai','AAAAACAAAAA'),
        ...
        sql_cmd2 = f'''
                    start transaction;
                    declare cursor37 no scroll cursor for select * from cur_test_37 order by 1;
                    fetch from cursor37;
                    close cursor37;
                    end;
                    '''
    ...
    def tearDown(self):
        logger.info('======清理环境======')
        sql_cmd3 = f'''
                    drop table cur_test_37 cascade;
                   '''
        msg3 = self.sh_primysh.execut_db_sql(sql_cmd3)
        logger.info(msg3)
        logger.info('---------------Opengauss_Function_DDL_Cursor_Case0037执行结束---------------')
