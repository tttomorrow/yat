...
"""
Case Type   : 功能
Case Name   :  shared_buffers小；maintenance_work_mem值较大时，分区表建数据量较
               大的索引
Description :
    1.修改参数shared_buffers为128MB；maintenance_work_mem为1GB
    2.重启数据库
    3.创建分区表，创建哈希索引并插入50W数据
    4.使用索引
    5.清理环境
Expect      :
    1.修改成功
    2.重启数据库成功
    3.创建分区表，创建哈希索引并插入50W数据成功
    4.索引数据存在且查询计划走索引扫描
    5.清理环境完成
History     :
"""
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0024start-')
        self.constant = Constant()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.tb_name = "t_hash_index_0024"
        self.id_name = "i_hash_index_0024"
    ...
    def test_hash_index(self):
        ...
        text = '---step3:创建分区表并插入数据；创建哈希索引;expect:创建成功---'
        self.log.info(text)
        sql_cmd = self.pri_sh.execut_db_sql(f'''drop table if exists \
            {self.tb_name};
           create table {self.tb_name}(id int,name varchar(20)) partition by \
           range(id)
           (
            partition p01 values less than (500),
            partition p02 values less than (1000),
            partition p03 values less than (1500)
            );
            insert into {self.tb_name} select random()*10, 'XXX'  from \
        ...
        self.assertIn(' Partitioned Bitmap Index Scan',
                      sql_cmd, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0024finish--')
