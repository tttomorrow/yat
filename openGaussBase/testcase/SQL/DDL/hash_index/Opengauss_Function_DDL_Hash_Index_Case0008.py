...
"""
Case Type   : 功能
Case Name   : 创建索引后dump，alter索引后， 恢复数据
Description :
    1.创建测试库和表并创建hash索引
    2.dump数据
    3.修改索引名
...
class DDL(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0008start-')
        self.constant = Constant()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.tb_name = "t_hash_index_0008"
        self.id_name = "i_hash_index_0008"
        self.id_name_01 = "i_hash_index_0008_01"
        self.db_name = "db_hash_index_0008"
    ...
    def test_hash_index(self):
        ...
        text = '---step3:修改索引名;expect:修改成功-------'
        self.log.info(text)
        sql_cmd = f'''alter index {self.id_name} rename to \
            {self.id_name_01};'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn(self.constant.ALTER_INDEX_SUCCESS_MSG, sql_result,
                      '执行失败:' + text)
        ...
        text = '--step5:查看hash索引;expect:两个索引均存在--'
        self.log.info(text)
        ...
        self.assertTrue(sql_result.find(f'{self.id_name}') > -1,
                        '执行失败:' + text)
        self.assertTrue(sql_result.find(f'{self.id_name_01}') > -1,
                        '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(msg)
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0008finish--')
