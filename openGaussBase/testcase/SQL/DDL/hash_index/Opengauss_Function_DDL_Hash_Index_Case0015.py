...
"""
Case Type   : 功能
Case Name   : 在事务中删除hash索引，未提交之前以及提交后观察事务隔离性是否正确
Description :
    1.创建表并创建索引
    2.session1开启事务，删除hash索引，不提交并查询
    3.session1未提交前在session2中查询索引;待session1提交后，session2查询
    4.清理环境
Expect      :
    1.创建成功
    2.开启事务成功,删除索引成功，未提交session1查询索引已删除
    3.索引存在;索引不存在
    4.清理环境完成
History     :
"""
...
class DmlTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DML_Lock_Case0015开始')
        self.constant = Constant()
        self.commonsh1 = CommonSH('PrimaryDbUser')
        self.commonsh2 = CommonSH('PrimaryDbUser')
        self.tb_name = "t_hash_index_0015"
        self.id_name = "i_hash_index_0015"
        self.db_name = "db_hash_index_0015"
    ...
    def test_dml_lock(self):
        ...
        text = 'step2:session1开启事务，删除hash索引，不提交并查询' \
               'expect:开启事务成功,删除索引成功，未提交session1查询索引已删除'
        self.log.info(text)
        sql = f'''begin;
            drop index {self.id_name};
        ...
        text = '-----step3:session1未提交前在session2中查询索引;' \
               '待session1提交后，session2查询;expect:索引未删除;索引已删除---'
        self.log.info(text)
        ...
        self.assertIn('DROP INDEX', msg_result_1, '执行失败:' + text)
        self.assertEqual('(0 rows)', msg_result_1.splitlines()[4].strip(),
                         '执行失败:' + text)
        ...
        self.assertEqual(f'{self.id_name}',
                         ...
                         '执行失败:' + text)
        self.assertEqual('(0 rows)', msg_result_2.splitlines()[-1].strip(),
                         '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '-Opengauss_Function_DDL_Hash_Index_Case0015finish--')
