...
"""
Case Type   : 功能
Case Name   :  shared_buffers和maintenance_work_mem值较大时，临时表建数据量较
               小的索引
Description :
    1.修改参数shared_buffers，maintenance_work_mem为为1GB
    2.重启数据库
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0029start-')
        self.constant = Constant()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.tb_name = "t_hash_index_0029"
        self.id_name = "id_hash_index_0029"
    ...
    def test_hash_index(self):
        ...
        sql_cmd = self.pri_sh.execut_db_sql(f'''drop table if exists \
            {self.tb_name};
            create temp table {self.tb_name} (id int, num int, sex varchar 
            default 'male');
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0029finish--')
