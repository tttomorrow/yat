...
"""
Case Type   : 功能
Case Name   :  shared_buffers和maintenance_work_mem值较小时，临时表建数据量较
               大的索引
Description :
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0027start-')
        self.constant = Constant()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.tb_name = "t_hash_index_0027"
        self.id_name = "i_hash_index_0027"
    ...
    def test_hash_index(self):
        ...
        text = '---step3:创建表并插入数据；创建哈希索引;未离开会话查询索' \
               '引数据;expect:创建成功；索引数据存在且查询计划走索引扫描---'
        self.log.info(text)
        sql_cmd = self.pri_sh.execut_db_sql(f'''drop table if exists \
            {self.tb_name};
            create global temp table {self.tb_name} (id int, num int, \
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0027finish--')
