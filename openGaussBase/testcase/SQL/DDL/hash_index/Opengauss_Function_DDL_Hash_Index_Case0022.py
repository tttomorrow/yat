...
"""
Case Type   : 功能
Case Name   :  shared_buffers大；maintenance_work_mem值较小时，分区表建数据量较
               大的索引
Description :
    1.修改参数shared_buffers为1GB；maintenance_work_mem为128MB
    2.重启数据库
...
class DdlTestCase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0022start-')
        self.constant = Constant()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.tb_name = "t_hash_index_0022"
        self.id_name = "i_hash_index_0022"
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info('-Opengauss_Function_DDL_Hash_Index_Case0022finish--')
