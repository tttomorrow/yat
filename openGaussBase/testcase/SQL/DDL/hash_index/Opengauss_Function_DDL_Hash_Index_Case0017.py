...
"""
Case Type   : 功能
Case Name   : 在事务中执行修改操作，未提交之前以及提交后观察事务隔离性是否正确
Description :
    1.创建表并创建索引，插入数据
    2.session1开启事务，修改数据，不提交并查询
    3.session1未提交前在session2中查询;待session1提交后，session2查询
    4.清理环境
Expect      :
    1.创建成功
    2.开启事务成功,修改数据成功，未提交session1查询数据已修改且查询计划走
    索引扫描
    3.数据未修改;数据已修改,和session1修改数据查询一致
    4.清理环境完成
History     :
"""
import time
...
class DmlTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = "t_hash_index_0017"
        self.id_name = "i_hash_index_0017"
        self.db_name = "db_hash_index_0017"
    ...
    def test_dml_lock(self):
        ...
        text = 'step2:session1开启事务，修改数据，不提交并查询' \
               'expect:开启事务成功,修改数据成功，未提交session1查询数据已修改'
        self.log.info(text)
        sql = f'''begin;
            update {self.tb_name} set id =id*5 where id =10;
            select count(*) from {self.tb_name} where id=50;
            explain select count(*) from {self.tb_name} where id=50;
            select pg_sleep(10);
            commit;'''
        ...
        text = '-----step3:session1未提交前在session2中查询;' \
               '待session1提交后，session2查询;expect:数据未修改;' \
               'session1提交后，session2查询数据已修改---'
        self.log.info(text)
        sql = f'''select count(*) from {self.tb_name} where id=50;
            explain select count(*) from {self.tb_name} where id=50;
            select pg_sleep(15);
            select count(*) from {self.tb_name} where id=50;'''
        ...
        self.assertIn('UPDATE', msg_result_1, '执行失败:' + text)
        ...
        self.assertEqual('0', msg_result_2.splitlines()[2].strip(),
                         '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '-Opengauss_Function_DDL_Hash_Index_Case0017finish--')
