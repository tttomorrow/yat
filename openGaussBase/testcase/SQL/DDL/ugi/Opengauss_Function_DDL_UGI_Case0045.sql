-- @testpoint: 验证【间隔】分区表【truncate partition】时，update global index关键字对【btree】类型【普通索引】的重建作用
...
--收集统计信息
analyse test_ugi_045;

--禁用seq scan，原因是初始数据量较小，因此禁用seq scan，验证是否会走到index scan
set enable_seqscan = off;

--确认清空分区可以破坏全局索引，并使用alter index xxx rebuild重建索引global_index_id_045
--查看分区状态
select t1.relname, partstrategy, boundaries from pg_partition t1, pg_class t2
where t1.parentid = t2.oid and t2.relname = 'test_ugi_045' and t1.parttype = 'p' order by relname;
--查看执行计划，确认走index scan
explain analyse select * from test_ugi_045 where c_date = '2021-06-01 00:00:00';
--清空分区p2，破坏全局索引
alter table test_ugi_045 truncate partition p2;
--分区p2已被清空
select t1.relname, partstrategy, boundaries from pg_partition t1, pg_class t2
where t1.parentid = t2.oid and t2.relname = 'test_ugi_045' and t1.parttype = 'p' order by relname;
--查看索引可用情况，索引已不可用
select c.relname, i.indisusable from pg_index i join pg_class c on i.indexrelid = c.oid 
where i.indrelid = 'test_ugi_045'::regclass order by c.relname;
--查看执行计划，走seq scan
explain analyse select * from test_ugi_045 where c_date = '2021-06-01 00:00:00';
--使用alter index xxx rebuild重建索引
alter index global_index_id_045 rebuild;
--查看索引可用情况，global_index_id_045可用，global_index_info_045不可用，可用索引查看执行计划确走index scan
select c.relname, i.indisusable from pg_index i join pg_class c on i.indexrelid = c.oid 
where i.indrelid = 'test_ugi_045'::regclass order by c.relname;
explain analyse select * from test_ugi_045 where c_date = '2021-06-03 00:00:00';
vacuum analyse;
...
select t1.relname, partstrategy, boundaries from pg_partition t1, pg_class t2
where t1.parentid = t2.oid and t2.relname = 'test_ugi_045' and t1.parttype = 'p' order by relname;
--此时，global_index_id_045可用，global_index_info_045不可用
select c.relname, i.indisusable from pg_index i join pg_class c on i.indexrelid = c.oid 
where i.indrelid = 'test_ugi_045'::regclass order by c.relname;
--查看执行计划，c_date走index scan，c_info走seq scan
explain analyse select * from test_ugi_045 where c_date = '2021-06-06 00:00:00';
explain analyse select * from test_ugi_045 where c_info = '1-4';

--表test_ugi_045应当包含"wait_clean_gpi=y"
select a.relname,a.parttype,a.reloptions from pg_partition a, pg_class b 
where a.parentid = b.oid and b.relname = 'test_ugi_045' and a.reloptions[3] like '%wait_clean_gpi=y%' order by 1,2,3;
--执行清理
vacuum analyse;
--表test_ugi_045不再包含"wait_clean_gpi=y"
select a.relname,a.parttype,a.reloptions from pg_partition a, pg_class b 
where a.parentid = b.oid and b.relname = 'test_ugi_045' and a.reloptions[3] like '%wait_clean_gpi=y%' order by 1,2,3;

--执行insert、update、delete等操作
insert into test_ugi_045(c_id, c_date, c_info) values(11, '2021-06-09 00:00:00', '1-5');
update test_ugi_045 set c_info = '4-1' where c_date = '2021-06-06 00:00:00';
delete from test_ugi_045 where c_date = '2021-06-09 00:00:00';
--查看执行计划，c_date走index scan
explain analyse select * from test_ugi_045 where c_date = '2021-06-06 00:00:00';

--确认test_ugi_045表中数据量正确，6行
select count(*) from test_ugi_045;
--清理表
drop table test_ugi_045;
--启用seq_scan
