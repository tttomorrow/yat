...
"""
Case Type   : 行访问控制策略
Case Name   : 行存表定义行访问控制策略，测试当前行访问控制影响的SQL操作默认为all
Description :
        1.创建行存表all_data
        2.创建用户
        3.插入数据
...
Expect      :
        ...
        4.赋予权限成功
        5. 创建行访问控制策略成功
        6.只查询到role为alice的这一行数据
        7.只查询到role为bob的这一行数据
        8.清理环境完成
History     :
"""
import sys
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0006开始执行-----------------------------')
    ...
    def test_sysadmin_user_permission(self):
        ...
        # 将表all_data的读取权限赋予alice和bob用户并打开行访问控制策略开关
        ...
        # 切换至alice用户执行SELECT操作，行访问控制策略生效，只查询到role为alice的这一行数据
        ...
        # 切换至bob用户执行SELECT操作，行访问控制策略生效，只查询到role为bob的这一行数据
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0006执行结束--------------------------')
