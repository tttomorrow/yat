...
"""
Case Type   : 行访问控制策略
Case Name   : 定义访问控制策略,行访问控制影响的SQL操作为select（unlogged表）
Description :
        1.创建unlogged表
        2.创建用户
        3.插入数据
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0023开始执行-----------------------------')
    ...
    def test_user_permission(self):
        ...
        sql_cmd4 = commonsh.execut_db_sql('''grant select on test_policy5 to test_user_001,test_user_002,test_user_003;
        ...
        self.assertIn(constant.CREATE_ROW_LEVEL_SECURITY_POLICY_SUCCESS_MSG, sql_cmd5)
        ...
        sql_cmd6 = ('''select * from test_policy5;
                    update test_policy5 set id = id + 50 where usr = 'test_user_001';
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0023执行结束--------------------------')
