...
"""
Case Type   : 行访问控制策略
Case Name   : 定义访问控制策略,行访问控制影响的SQL操作为update（行存分区表）
Description :
        1.创建分区表
        2.创建用户
        3.插入数据
        ...
        7.修改表信息c_usr = 'test_user_001'的c_id
        8.删除表信息
        9.清理环境
Expect      :
        1.创建成功
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0018开始执行-----------------------------')
    ...
    def test_user_permission(self):
        ...
        sql_cmd4 = commonsh.execut_db_sql('''grant select,update on test_policy1 to test_user_001,test_user_002,test_user_003;
        ...
        sql_cmd5 = commonsh.execut_db_sql('''drop POLICY if exists pol1 ON test_policy1;
                                      CREATE POLICY pol1 ON test_policy1 FOR update TO PUBLIC USING (c_usr = current_user);''')
        logger.info(sql_cmd5)
        self.assertIn(self.Constant.CREATE_ROW_LEVEL_SECURITY_POLICY_SUCCESS_MSG, sql_cmd5)
        # 以test_user_001登录数据库，测试,查询表的所有信息
        # 修改表信息c_usr = 'test_user_001'的c_id,成功修改1条数据
        # 删除表信息，合理报错
        ...
        self.assertIn('ERROR:  permission denied for relation test_policy1', msg1)
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0018执行结束--------------------------')
