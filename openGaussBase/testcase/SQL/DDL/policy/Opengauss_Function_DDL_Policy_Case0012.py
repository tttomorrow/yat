...
"""
Case Type   : 行访问控制策略
Case Name   : 创建行访问控制策略，行访问控制影响的SQL操作为delete（行存表）
Description :
        1.创建行存表all_data
        2.创建用户
        3.插入数据
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0012开始执行-----------------------------')
    ...
    def test_user_permission(self):
        # 创建行存表passwd
        sql_cmd1 = commonsh.execut_db_sql('''drop table if exists passwd;
        CREATE TABLE passwd (user_name text UNIQUE NOT NULL,pwhash text,uid int PRIMARY KEY,gid int NOT NULL,real_name text NOT NULL,home_phone text,extra_info text,home_dir text NOT NULL);''')
        ...
        # 向表插入数据
        sql_cmd3 = commonsh.execut_db_sql('''INSERT INTO passwd VALUES('u_usr1','xxx',0,0,'Admin','111-222-3333',null,'/root');
                                      INSERT INTO passwd VALUES('u_usr2','xxx',1,1,'Bob','123-456-7890',null,'/home/bob');
                                      INSERT INTO passwd VALUES('u_usr3','xxx',2,1,'Alice','098-765-4321',null,'/home/alice');''')
        logger.info(sql_cmd3)
        self.assertIn(self.Constant.INSERT_SUCCESS_MSG, sql_cmd3)
        # 授予用户表的select和delete权限并打开行级安全检查
        sql_cmd4 = commonsh.execut_db_sql('''grant select,delete on passwd to u_usr1,u_usr2,u_usr3;
        ...
        sql_cmd5 = commonsh.execut_db_sql('''drop POLICY if exists pol2 ON passwd;
                                      CREATE POLICY pol2 ON passwd FOR delete TO PUBLIC USING (user_name = current_user);''')
        ...
        sql_cmd7 = ('''delete from passwd where user_name = 'u_usr2';
                 delete from passwd where user_name = 'u_usr1';''')
        excute_cmd2 = f'''
                                source {self.DB_ENV_PATH};
    ...
    def tearDown(self):
        ...
        # 删除行访问控制策略
        sql_cmd8 = commonsh.execut_db_sql('''drop POLICY if exists pol2 ON passwd;''')
        logger.info(sql_cmd8)
        # 删除表
        sql_cmd9 = commonsh.execut_db_sql('''drop table if exists passwd;''')
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0012执行结束--------------------------')
