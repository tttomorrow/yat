...
"""
Case Type   : 行访问控制策略
Case Name   : 定义访问控制策略,行访问控制影响的SQL操作为update（列存分区表）
Description :
        1.创建分区表
        2.创建用户
        3.插入数据
        ...
        5.创建行访问控制策略
        6.以test_test_user_001登录数据库
        7.修改表信息c_usr = 'test_test_user_001'的c_id
        8.删除表信息
        9.清理环境
Expect      :
        1.创建成功
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0021开始执行-----------------------------')
    ...
    def test_user_permission(self):
       ...
       (
        ...
        # 向表插入数据
        sql_cmd3 = commonsh.execut_db_sql('''insert into test_policy3 values(60,'test_user_001'),(150,'test_user_002'),(40,'test_user_003');''')
        logger.info(sql_cmd3)
        self.assertIn(self.Constant.INSERT_SUCCESS_MSG, sql_cmd3)
        # 授予用户表的select和update权限并打开行级安全检查
        sql_cmd4 = commonsh.execut_db_sql('''grant select,update on test_policy3 to test_user_001,test_user_002,test_user_003;
        ...
        sql_cmd5 = commonsh.execut_db_sql('''drop POLICY if exists t_pol1 ON test_policy3;
                                      CREATE POLICY t_pol1 ON test_policy3 FOR update TO PUBLIC USING (c_usr = current_user);''')
        logger.info(sql_cmd5)
        self.assertIn(self.Constant.CREATE_ROW_LEVEL_SECURITY_POLICY_SUCCESS_MSG, sql_cmd5)
        # 以test_test_user_001登录数据库，测试,查询表的所有信息
        # 修改表信息c_usr = 'test_test_user_001'的c_id，修改1条数据成功
        # 删除表信息，合理报错
        sql_cmd6 = ('''select * from test_policy3;
                    update test_policy3 set c_id = c_id + 50 where c_usr = 'test_user_001';
                    delete from test_policy3 where c_usr = 'test_user_001';''')
        excute_cmd1 = f'''
    ...
    def tearDown(self):
        ...
        # 删除行访问控制策略
        sql_cmd7 = commonsh.execut_db_sql('''drop POLICY if exists t_pol1 ON test_policy3;''')
        logger.info(sql_cmd7)
        # 删除表
        sql_cmd8 = commonsh.execut_db_sql('''drop table test_policy3;''')
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0021执行结束--------------------------')
