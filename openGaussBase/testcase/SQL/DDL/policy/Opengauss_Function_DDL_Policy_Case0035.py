...
"""
Case Type   : 行访问控制策略
Case Name   : 创建行访问控制策略，行访问控制影响的SQL操作为update...returning
Description :
        1.创建行存表test_policy
        2.创建用户
        3.插入数据
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0035开始执行-----------------------------')
    ...
    def test_user_permission(self):
        ...
        sql_cmd4 = commonsh.execut_db_sql('''grant select,update on test_policy to test_user_001,test_user_002,test_user_003;
        ...
        sql_cmd6 = ('''select * from test_policy;
                    update test_policy set id = id + 1 where usr = 'test_user_001' returning id;
                    delete from test_policy where usr = 'test_user_001';''')
        excute_cmd1 = f'''
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0035执行结束--------------------------')
