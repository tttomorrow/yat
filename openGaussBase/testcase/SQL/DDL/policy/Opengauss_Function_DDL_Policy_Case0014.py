...
"""
Case Type   : 行访问控制策略
Case Name   : 定义访问控制策略,行访问控制影响的SQL操作为select（列存表）
Description :
        1.创建行存表all_data
        2.创建用户
        3.插入数据
        ...
        5.创建行访问控制策略
        6.以t_user1登录数据库，测试,查询表信息;修改表信息;删除表信息
        7.清理环境
Expect      :
        1.创建成功
        2.创建成功
        ...
        4.赋予权限成功
        5.创建行访问控制策略成功
        6.只有usr字段值为t_user1才对用户t_user1可见；合理报错；合理报错
        7.清理环境完成
History     :
"""
import sys
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0014开始执行-----------------------------')
    ...
    def test_user_permission(self):
        ...
        sql_cmd4 = commonsh.execut_db_sql('''grant select on test_policy to t_user1,t_user2,t_user3;
        ...
        # 以t_user1登录数据库，测试,查询表信息，只有usr字段值为t_user1才对用户t_user1可见,故SELECT类操作受行访问控制的影响，只能查看到满足条件(using_expression返回值为TRUE)的元组数据
        # 修改表信息,合理报错
        # 删除表信息，合理报错
        sql_cmd6 = ('''select * from test_policy;
                    update test_policy set id = id + 1 where usr = 't_user1';
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0014执行结束--------------------------')
