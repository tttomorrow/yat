...
"""
Case Type   : 行访问控制策略
Case Name   : 定义访问控制策略,行访问控制影响的SQL操作为select（列存分区表）
Description :
        1.创建分区表
        2.创建用户
        3.插入数据
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0020开始执行-----------------------------')
    ...
    def test_user_permission(self):
       ...
       (
        ...
        sql_cmd4 = commonsh.execut_db_sql('''grant select on test_policy3 to test_user_001,test_user_002,test_user_003;
        ...
        sql_cmd5 = commonsh.execut_db_sql('''drop POLICY if exists t_pol1 ON test_policy3;
                                      CREATE POLICY t_pol1 ON test_policy3 FOR SELECT TO PUBLIC USING (c_usr = current_user);''')
    ...
    def tearDown(self):
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0020执行结束--------------------------')
