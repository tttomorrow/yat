...
"""
Case Type   : 行访问控制策略
Case Name   : 定义访问控制策略,行访问控制影响的SQL操作为delete（行存分区表）
Description :
        1.创建分区表
        2.创建用户,其中u_usr1为系统管理员,u_usr2和u_usr3为普通用户
        3.插入数据
...
class Policy(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0019开始执行-----------------------------')
    ...
    def test_user_permission(self):
        ...
        # 向表插入数据
        sql_cmd3 = commonsh.execut_db_sql('''insert into test_policy1 values(60,'u_usr1'),(150,'u_usr2'),(40,'u_usr3');''')
        logger.info(sql_cmd3)
        self.assertIn(self.Constant.INSERT_SUCCESS_MSG, sql_cmd3)
        # 授予用户表的select和delete权限并打开行级安全检查
        sql_cmd4 = commonsh.execut_db_sql('''grant select,delete on test_policy1 to u_usr1,u_usr2,u_usr3;
        ...
        sql_cmd5 = commonsh.execut_db_sql('''drop POLICY if exists pol1 ON test_policy1;
                                      CREATE POLICY pol1 ON test_policy1 FOR delete TO PUBLIC USING (c_usr = current_user);''')
        ...
        sql_cmd7 = ('''update test_policy1 set c_usr = 'u_usr2new' where c_id = 150;
        delete from test_policy1 where c_usr = 'u_usr2';
        delete from test_policy1 where c_usr = 'u_usr1';''')
        excute_cmd1 = f'''
                                   source {self.DB_ENV_PATH};
    ...
    def tearDown(self):
        ...
        # 删除行访问控制策略
        sql_cmd8 = commonsh.execut_db_sql('''drop POLICY if exists pol1 ON test_policy1;''')
        logger.info(sql_cmd8)
        # 删除表
        sql_cmd9 = commonsh.execut_db_sql('''drop table test_policy1;''')
        ...
        logger.info('------------------------Opengauss_Function_DDL_Policy_Case0019执行结束--------------------------')
