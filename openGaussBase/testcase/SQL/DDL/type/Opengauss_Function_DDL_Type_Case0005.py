...
"""
'''
--  @testpoint:创建复合类型，复合类型的列的现有数据类型为日期时间类型
'''
import sys
import unittest
...
class Privategrant(unittest.TestCase):
    def setUp(self):
        logger.info('------------------------Opengauss_Function_DDL_Type_Case0005开始执行-----------------------------')

    def test_common_user_permission(self):
        # 创建一种复合类型
        sql_cmd1 = commonsh.execut_db_sql('''drop type if exists t_type5 cascade;
                                      CREATE TYPE t_type5 AS (f1 date, f2 time without time zone ,f3 timestamp with time zone);''')
        logger.info(sql_cmd1)
        self.assertIn(constant.CREATE_TYPE_SUCCESS_MSG, sql_cmd1)
        # 建表1
        sql_cmd2 = commonsh.execut_db_sql('''drop table if exists t1_test;
                                      CREATE TABLE t1_test(a int, b t_type5);''')
        logger.info(sql_cmd2)
        self.assertIn(constant.TABLE_CREATE_SUCCESS, sql_cmd2)
        # 建表2
        sql_cmd3 = commonsh.execut_db_sql('''drop table if exists t2_test;
                                      CREATE TABLE t2_test(a int, b t_type5);''')
        logger.info(sql_cmd3)
        self.assertIn(constant.TABLE_CREATE_SUCCESS, sql_cmd3)
        # 表1插入数据
        sql_cmd4 = commonsh.execut_db_sql('''INSERT INTO t1_test values(1,('12-10-2010','21:21:21','2010-12-12'));''')
    ...
    def tearDown(self):
        ...
        # 删除类型
        sql_cmd9 = commonsh.execut_db_sql('''drop type t_type5 cascade;''')
        logger.info(sql_cmd9)
        logger.info('------------------------Opengauss_Function_DDL_Type_Case0005执行结束--------------------------')
