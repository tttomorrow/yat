...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0016_01'
        self.table2 = 't_rule_0016_02'
        self.rule = 'r_test_0016'
        self.view = 'v_rule_0016'
    ...
    def test_rule(self):
        ...
        create_table = self.pri_sh.execut_db_sql(
            ...
            create table {self.table2}(c1_int int);''')
        self.log.info(create_table)
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, create_table,
                      '执行失败' + text)
        self.assertIn(self.constant.TABLE_CREATE_SUCCESS, create_table,
        ...
        create_rule = self.pri_sh.execut_db_sql(
            ...
            do instead update {self.table2} set c1_int = 200 where c1_int < 10 
            returning *; ''')
        self.log.info(create_rule)
        self.assertIn('CREATE RULE', create_rule, '执行失败' + text)

        text = '----step6:查询表2数据 expect:返回返回原始数据----'
        self.log.info(text)
        ...
        text = '----step7:通过视图修改表1数据,触发rule ' \
               'expect:修改表2数据成功,不返回触发rule后修改的表2数据----'
        self.log.info(text)
        update_data = self.pri_sh.execut_db_sql(
            f'''update {self.view} set c_int = 100;''')
        self.log.info(update_data)
        self.assertIn('UPDATE 9', update_data, '执行失败' + text)

        text = '----step8:查询表1数据 expect:返回表1原始数据----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
        ...
        text = '----step9:查询表2数据 expect:返回表2修改后的数据----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select * from {self.table2};''')
        self.log.info(select_data)
        self.assertEqual(select_data.count('200'), 9, '执行失败' + text)
