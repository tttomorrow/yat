...
"""
Case Type   : rule
Case Name   : 为视图创建select类型的无条件instead规则,规则使用returning语句,
              合理报错
Description :
    1.建表1
    2.建表2
    3.向表1,表2中插入数据
    4.为表1创建视图
    5.为视图创建select类型的无条件instead规则,使用returning语句
    6.清理环境
Expect      :
    1.建表1成功
    2.建表2成功
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0020_01'
        self.table2 = 't_rule_0020_02'
        self.rule = 'r_test_0020'
        self.view = 'v_rule_0020'
    ...
    def test_rule(self):
        ...
        text = '----step5:为视图创建select类型的无条件instead规则,' \
               '使用returning语句 expect:创建失败,合理报错-----'
        self.log.info(text)
        create_rule = self.pri_sh.execut_db_sql(
            f'''create or replace rule {self.rule} as on select to {self.view} 
            do instead select * from {self.table2} returning *;''')
        self.log.info(create_rule)
        self.assertIn('ERROR:  syntax error at or near "returning"',
