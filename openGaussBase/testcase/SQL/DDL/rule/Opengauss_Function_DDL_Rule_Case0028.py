...
"""
Case Type   : rule
Case Name   : 创建rule,do指定instead,不执行初始命令,只执行rule中sql语句
Description :
    1.建表1
    2.建表2
    3.向表1中插入数据
    4.为表1创建rule,do指明instead
    5.查看表1数据量
    6.查看表2数据量
    7.向表1中插入数据
    8.查询表1数据量
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0028_01'
        self.table2 = 't_rule_0028_02'
        self.rule = 'r_test_0028'
    ...
    def test_rule(self):
        ...
        text = '----step4:为表1创建rule,do指明instead ' \
               'expect:创建成功----'
        self.log.info(text)
        ...
        text = '----step5:查看表1数据量 expect:返回表1原始数据量----'
        self.log.info(text)
        ...
        text = '----step6:查看表2数据量 expect:返回表2原始数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
        ...
        text = '----step7:向表1中插入数据,触发rule expect:向表1中插入数据成功'
        self.log.info(text)
        insert_data = self.pri_sh.execut_db_sql(f'''insert into {self.table1} 
            values(generate_series(1,10000));''')
        self.log.info(insert_data)
        ...
        text = '----step8:查询表1数据量 expect:返回表1新增后数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table1};''')
        self.log.info(select_data)
        self.com.equal_sql_mdg(select_data, 'count', '10001', '(1 row)',
                               flag="1")

        text = '----step9:再次查询表2数据量 ' \
