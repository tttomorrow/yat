...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0006_01'
        self.table2 = 't_rule_0006_02'
        self.rule = 'r_test_0006'
    ...
    def test_rule(self):
        ...
        create_table = self.pri_sh.execut_db_sql(
            ...
            create table {self.table1}(c_int int) with (segment = on);''',
            dbname='postgres')
        ...
        create_table = self.pri_sh.execut_db_sql(
            ...
            create table {self.table2}(c_int int) with (segment = on);''',
            dbname='postgres')
        self.log.info(create_table)
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, create_table,
                      '执行失败' + text)
        ...
        create_rule = self.pri_sh.execut_db_sql(
            ...
            values(generate_series(1,10000));''', dbname='postgres')
        ...
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table2};''', dbname='postgres')
        ...
        text = '----step6:再次查看表2数据量 expect:返回通过触发rule新增后的' \
               '数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table2};''', dbname='postgres')
    ...
    def tearDown(self):
        ...
        clean_environment = self.pri_sh.execut_db_sql(
            ...
            drop table {self.table2};''', dbname='postgres')
