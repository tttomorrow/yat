...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0009_01'
        self.table2 = 't_rule_0009_02'
        self.rule = 'r_test_0009'
    ...
    def test_rule(self):
        ...
        create_table = self.pri_sh.execut_db_sql(
            ...
            create table {self.table1}(c1 int, c2 int) 
        ...
        create_table = self.pri_sh.execut_db_sql(
            ...
            create table {self.table2}(c1 int, c2 int) 
            partition by range (c2) (
                partition p1 start(100) end(1000) every(200) ,
                partition p2 end(3000),
                partition p3 start(3000) end(3500),
                partition p4 start(3500),
                partition p5 start(4000) end(5000) every(1000)
            )
            enable row movement;''')
        self.log.info(create_table)
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, create_table,
        ...
        create_rule = self.pri_sh.execut_db_sql(
            ...
            {self.table1} do also insert into {self.table2} values
            (generate_series(1,4999),generate_series(1,4999));;''')
        ...
        insert_data = self.pri_sh.execut_db_sql(f'''insert into {self.table1} 
            values(generate_series(1,3000),generate_series(1,2000));''')
        ...
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table2};''')
        self.log.info(select_data)
        self.com.equal_sql_mdg(select_data, 'count', '4999', '(1 row)',
                               flag="1")
