...
"""
Case Type   : rule
Case Name   : 为视图创建insert类型的有条件instead规则,规则使用returning语句,
              合理报错
Description :
    1.建表1
    2.建表2
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0011_01'
        self.table2 = 't_rule_0011_02'
        self.rule = 'r_test_0011'
        self.view = 'v_rule_0011'
    ...
    def test_rule(self):
        ...
        text = '----step4:为视图创建rule,使用returning语句 ' \
               'expect:创建失败,合理报错'
        self.log.info(text)
        create_rule = self.pri_sh.execut_db_sql(
            f'''create or replace rule {self.rule} as on insert to {self.view} 
            where new.c_int > 5000 do instead insert into {self.table2} 
            values(generate_series(1,300)) returning *;''')
        self.log.info(create_rule)
        er = 'ERROR:  RETURNING lists are not supported in conditional rules'
        self.assertIn(er, create_rule, '执行失败' + text)
