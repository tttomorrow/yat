...
"""
Case Type   : rule
Case Name   : 为视图创建delete类型的有条件instead规则,规则使用returning语句,
              合理报错
Description :
    1.建表1
    2.建表2
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0018_01'
        self.table2 = 't_rule_0018_02'
        self.rule = 'r_test_0018'
        self.view = 'v_rule_0018'
    ...
    def test_rule(self):
        ...
        text = '----step4:为视图创建delete类型的有条件instead规则 ' \
               'expect:创建失败,合理报错'
        self.log.info(text)
        create_rule = self.pri_sh.execut_db_sql(
            f'''create or replace rule {self.rule} as on delete to 
            {self.view} where old.c_int < 5000  do instead delete from 
            {self.table2}  where c_int < 500 returning *;''')
        self.log.info(create_rule)
        str1 = 'ERROR:  RETURNING lists are not supported in conditional rules'
