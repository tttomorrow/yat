...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0017_01'
        self.table2 = 't_rule_0017_02'
        self.rule = 'r_test_0017'
        self.view = 'v_rule_0017'
    ...
    def test_rule(self):
        ...
        insert_data = self.pri_sh.execut_db_sql(
            f'''insert into {self.table1} values(generate_series(600,800));
            insert into {self.table2} values(generate_series(1,1000));''')
        ...
        text = '----step6:查询表2的数据量 expect:返回表2原始数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table2};''')
        self.log.info(select_data)
        self.com.equal_sql_mdg(select_data, 'count', '1000', '(1 row)',
                               flag="1")

        text = '----step7:通过视图删除表1数据,触发rule ' \
               'expect:返回通过触发rule删除的表2数据----'
        self.log.info(text)
        delete_data = self.pri_sh.execut_db_sql(
            f'''delete from {self.view} returning *;''')
        self.log.info(delete_data)
        self.assertIn(self.constant.DELETE_SUCCESS_MSG, delete_data,
                      '执行失败' + text)
        for i in range(0, 500):
            self.assertIn(str(i), delete_data, '执行失败' + text)

        text = '----step8:查询表1数据量 expect:返回表1原始数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table1};''')
        self.log.info(select_data)
        self.com.equal_sql_mdg(select_data, 'count', '201', '(1 row)',
                               flag="1")

        text = '----step9:再次查询表2的数据量 expect:返回表2通过触发rule删除后' \
               '的数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table2};''')
        self.log.info(select_data)
        self.com.equal_sql_mdg(select_data, 'count', '501', '(1 row)',
                               flag="1")
