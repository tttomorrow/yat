...
"""
Case Type   : rule
Case Name   : 创建rule,do指定also,初始命令和rule中sql语句都执行
Description :
    1.建表1
    2.建表2
    3.向表1中插入数据
    4.为表1创建rule,do指定also
    5.查看表1数据量
    6.表2数据量
    7.向表1中插入数据
    8.查询表1数据量
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0027_01'
        self.table2 = 't_rule_0027_02'
        self.rule = 'r_test_0027'
    ...
    def test_rule(self):
        ...
        text = '----step7:向表1中插入数据,触发rule xpect:向表1中插入数据成功'
        self.log.info(text)
        insert_data = self.pri_sh.execut_db_sql(f'''insert into {self.table1} 
            values(generate_series(1,10000));''')
        self.log.info(insert_data)
        ...
        text = '----step8:查询表1数据量 expect:返回表1新增后的数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table1};''')
        self.log.info(select_data)
