...
"""
Case Type   : rule
Case Name   : 创建select类型rule,do指定also,合理报错
Description :
    1.建表1
    2.建表2
    3.为表1创建select类型rule,do指定also
    4.清理环境
Expect      :
    1.建表1成功
    2.建表2成功
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0033_01'
        self.table2 = 't_rule_0033_02'
        self.rule = "_RETURN"
    ...
    def test_rule(self):
        ...
        text = '----step3:为表1创建select类型rule,do指定also' \
               '串 expect:创建失败,合理报错----'
        self.log.info(text)
        create_rule = self.pri_sh.execut_db_sql(
            f'''create or replace rule \\"{self.rule}\\" as on select to 
            {self.table1} do also select * from {self.table2} where 
            c_int<100;''')
        self.log.info(create_rule)
        s = 'ERROR:  rules on SELECT must have action INSTEAD SELECT'
