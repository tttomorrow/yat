...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0003_01'
        self.table2 = 't_rule_0003_02'
        self.rule = 'r_test_0003'
        self.view = 'v_rule_0003'
    ...
    def test_rule(self):
        ...
        text = '----step5:为视图创建rule expect:为视图创建rule成功'
        self.log.info(text)
        ...
        text = '----step6:查看表2中b<200的数据 expect:返回1-199----'
        self.log.info(text)
        ...
        text = '----step7:修改视图数据,触发rule expect:触发rule,修改表2数据' \
               '成功----'
        self.log.info(text)
        update_data = self.pri_sh.execut_db_sql(
            f'''update {self.view} set c_int = generate_series(1,50) 
            where c_int=50;''')
        self.log.info(update_data)
        self.assertIn(self.constant.UPDATE_SUCCESS_MSG, update_data,
                      '执行失败' + text)

        text = '----step8:再次查看表2中b<200的数据 expect:返回数据均为20----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select * from {self.table2} where c_int < 200;''')
        self.log.info(select_data)
        self.assertEqual(select_data.count('20'), 199, '执行失败' + text)
