...
"""
Case Type   : rule
Case Name   : 为视图创建update类型的有条件instead规则,规则使用returning语句,
              合理报错
Description :
    1.建表1
    2.建表2
    3.为表1创建视图
    4.为视图创建update类型的有条件instead规则
    5.清理环境
Expect      :
    1.建表1成功
    2.建表2成功
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0015_01'
        self.table2 = 't_rule_0015_02'
        self.rule = 'r_test_0015'
        self.view = 'v_rule_0015'
    ...
    def test_rule(self):
        ...
        text = '----step4:为视图创建update类型的有条件instead规则 ' \
               'expect:创建失败,合理报错'
        self.log.info(text)
        create_rule = self.pri_sh.execut_db_sql(
            f'''create or replace rule {self.rule} as on update to 
            {self.view} where new.c_int <> old.c_int do instead update 
            {self.table2} set c_int = 10 where c_int < 500 returning *;''')
        self.log.info(create_rule)
        s = 'ERROR:  RETURNING lists are not supported in conditional rules'
