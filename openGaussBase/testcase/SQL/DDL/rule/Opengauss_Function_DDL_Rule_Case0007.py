...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0007_01'
        self.table2 = 't_rule_0007_02'
        self.rule = 'r_test_0007'
    ...
    def test_rule(self):
        ...
        create_table = self.pri_sh.execut_db_sql(
            ...
            create table {self.table2}(col1 int, col2 int)
            partition by hash(col1)
            (
              partition p1,
              partition p2,
        ...
        text = '----step6:再次查看表2数据量 expect:返回通过触发rule新增后' \
               '的数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table2};''')
        self.log.info(select_data)
        self.com.equal_sql_mdg(select_data, 'count', '2', '(1 row)', flag="1")
