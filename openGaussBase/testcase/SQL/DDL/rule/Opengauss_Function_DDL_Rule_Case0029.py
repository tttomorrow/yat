...
"""
Case Type   : rule
Case Name   : 创建带条件的rule,do不指明also/instead,also做缺省值
Description :
    1.建表1
    2.建表2
    3.为表1创建rule,do不指明also/instead
    4.查看表1数据量
    5.查看表2数据量
    6.向表1中插入数据
    7.再次查看表1数据量
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0029_01'
        self.table2 = 't_rule_0029_02'
        self.rule = 'r_test_0029'
    ...
    def test_rule(self):
        ...
        text = '----step5:查看表2数据量 expect:返回表2原始数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
        ...
        text = '----step6:向表1中插入数据,触发rule xpect:向表1中插入数据成功'
        self.log.info(text)
        ...
        text = '----step7:再次查询表1数据量 expect:返回表1新增后的数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table1};''')
        self.log.info(select_data)
        ...
        text = '----step8:再次查询表表2数据量 expect:返回表2通过触发rule新增后' \
               '的数据量----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select count(*) from {self.table2};''')
