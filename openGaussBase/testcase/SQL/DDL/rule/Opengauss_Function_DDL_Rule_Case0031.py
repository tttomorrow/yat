...
"""
Case Type   : rule
Case Name   : 创建select类型rule,do指定instead
Description :
    1.建表1
    2.建表2
    3.向表1,表2中插入数据
    4.为表1创建select类型rule,do指定instead
    5.查看表1数据,触发rule
    6.清理环境
Expect      :
    1.建表1成功
...
class RULE(unittest.TestCase):
    def setUp(self):
        ...
        self.log = Logger()
        self.table1 = 't_rule_0031_01'
        self.table2 = 't_rule_0031_02'
        self.rule = "_RETURN"
    ...
    def test_rule(self):
        ...
        text = '----step4:为表1创建select类型rule,do指定instead ' \
               'expect:创建成功----'
        self.log.info(text)
        ...
        text = '----step5:查看表1数据,触发rule ' \
               'expect:返回创建的rule中select语句的返回结果-----'
        self.log.info(text)
        select_data = self.pri_sh.execut_db_sql(
            f'''select * from {self.table1};''')
        self.log.info(select_data)
        self.assertIn('199', select_data, '执行失败' + text)
