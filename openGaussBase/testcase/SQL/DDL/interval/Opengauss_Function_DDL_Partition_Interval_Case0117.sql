-- @testpoint: interval分区,MERGE PARTITIONS合并分区未按照边界升序排列,合理报错
...
-- 插入数据
insert into pttb values (1,'aaa',1,'2020-04-23',true,'aaa',1.1);  --pttb_p1
insert into pttb values (2,'bbb',2,'2020-03-23',false,'bbb',2.2); --pttb_p2
insert into pttb values (3,'ccc',3,'2020-02-23',true,'ccc',3.3);  --pttb_p3
insert into pttb values (4,'ddd',4,'2020-07-23',false,'ddd',4.4); --sys_p1
insert into pttb values (5,'eee',5,'2020-06-23',true,'eee',5.5);  --sys_p2
insert into pttb values (6,'fff',6,'2020-05-23',false,'fff',6.6); --sys_p3

-- 查看分区信息
select relname, parttype, partstrategy, boundaries,reltablespace from pg_partition
where parentid = (select oid from pg_class where relname = 'pttb') order by relname;

-- merge partitions,分区未按照边界升序排列
alter table pttb merge partitions pttb_p3, pttb_p1 into partition pttb_p4;
alter table pttb merge partitions sys_p1, sys_p3 into partition sys_p4;
