...
"""
Case Type   : 功能测试
Case Name   : interval分区,VACUUM TABLE
Description :
    1. 创建表,建索引,插入数据
    2. 删除索引后vacuum table
    3. 删除数据后vacuum table
...
class Function(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info('Opengauss_Function_DDL_Partition_Interval_Case0079开始')
    ...
    def test_copy(self):
        ...
        self.log.info('--步骤2 删除索引后vacuum table--')
        cmd1 = f'''drop index idx9; vacuum (verbose, analyze) test9;'''
        ...
        self.assertTrue(msg1.count(
            'DETAIL:  0 dead row versions cannot be removed yet.') == 4)

        self.log.info('--步骤3 删除数据后vacuum table--')
        cmd2 = f'''delete from test9; vacuum (verbose, analyze) test9;'''
        ...
        self.assertTrue(msg1.count(
            'DETAIL:  1 dead row versions cannot be removed yet.') > -1)

    def tearDown(self):
        self.commonsh.execut_db_sql('drop table if exists test9;'
                                    ' drop index if exists idx9;')
        self.log.info('Opengauss_Function_DDL_Partition_Interval_Case0079结束')