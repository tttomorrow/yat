-- @testpoint: interval分区,ALTER TABLE MOVE PARTITION将分区从STORE IN列表移至其它表空间
drop table if exists my_ptb1;
drop tablespace if exists mytsp1;
drop tablespace if exists mytsp2;
drop tablespace if exists mytsp3;

create tablespace mytsp1 relative location 'partition_table_space/mytsp1' maxsize '10m';
create tablespace mytsp2 relative location 'partition_table_space/mytsp2' maxsize '10m';
...
partition by range (col_4) interval ('1 month') store in(mytsp1,mytsp2)
(partition my_ptb1_p1 values less than ('2020-03-01') tablespace mytsp1,
