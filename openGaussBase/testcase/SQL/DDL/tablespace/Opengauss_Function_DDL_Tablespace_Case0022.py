...
"""
Case Type   : 功能测试-表空间
Case Name   : 表空间创建指定随机读取page的开销seq_page_cost值
Description :
    1、创建表空间seq_page_cost设定值为负数
    2、创建表空间seq_page_cost设定值为字符串
    3、创建表空间seq_page_cost缺省设置
    4、创建表空间seq_page_cost设定值为正整数
    5、创建表空间seq_page_cost设定值为正浮点数
Expect      :
    1、创建表空间seq_page_cost设定值为负数，创建失败
    2、创建表空间seq_page_cost设定值为字符串，创建失败
    3、创建表空间seq_page_cost缺省设置，创建成功
    4、创建表空间seq_page_cost设定值为正整数，创建成功
    5、创建表空间seq_page_cost设定值为正浮点数，创建成功
History     :
"""
import os
import unittest
...
class Tablespace(unittest.TestCase):
    def setUp(self):
        ...
        self.tbspc_name1 = 'tsp_tbspc0022_1'
        self.tbspc_location1 = 'tbspc0022_1'
        self.tbspc_name2 = 'tsp_tbspc0022_2'
        self.tbspc_location2 = 'tbspc0022_2'
        self.tbspc_name3 = 'tsp_tbspc0022_3'
        self.tbspc_location3 = 'tbspc0022_3'
        self.pwd = macro.PASSWD_INITIAL
        self.sys_user = 'u_tbspc0022'
    ...
    def test_main(self):
        ...
        step_txt = '----step4:创建表空间seq_page_cost设定值为正整数; expect:创建成功----'
        self.log.info(step_txt)
        ...
        spcoptions = self.pri_sh.execut_db_sql(check_sql).splitlines()[-2]
        self.log.info(spcoptions)
        self.assertIn('seq_page_cost=6', spcoptions)

        step_txt = '----step5:创建表空间seq_page_cost设定值为正浮点数; expect:创建成功----'
        self.log.info(step_txt)
        create_sql = f"create tablespace {self.tbspc_name3} " \
            f"relative location '{self.tbspc_location3}' " \
            f"with (seq_page_cost=9.333);"
        create_result = self.pri_sh.execut_db_sql(create_sql,
                                                  sql_type=self.connect_info)
        self.log.info(create_result)
        self.assertIn(self.constant.TABLESPCE_CREATE_SUCCESS, create_result)
        ...
        spcoptions = self.pri_sh.execut_db_sql(check_sql).splitlines()[-2]
        self.log.info(spcoptions)
        self.assertIn('seq_page_cost=9.333', spcoptions)
