...
class Tablespace(unittest.TestCase):
    def setUp(self):
        ...
        self.tbspc_name1 = 'tsp_tbspc0039_1'
        self.tbspc_location1 = 'tbspc0039_1'
        self.tbspc_name2 = 'tsp_tbspc0039_2'
        self.tbspc_location2 = 'tbspc0039_2'
        self.table_name = 't_tbspc0039'
        self.db_name = 'db_tbspc0039'
        self.index_name = 'idx_tbspc0039'
    ...
    def test_main(self):
        ...
        step_txt = '----step3:数据库创建表及索引;expect:创建成功----'
        self.log.info(step_txt)
        ...
        step_txt = '----step4:变更数据库的表空间tablespace2;expect:变更成功----'
        self.log.info(step_txt)
        alter_sql = f"alter database {self.db_name} " \
            f"set tablespace {self.tbspc_name2}"
        alter_result = self.sh.execut_db_sql(alter_sql)
        self.log.info(alter_result)

        step_txt = '----step5:查询表对应的表空间及表文件物理位置; expect:表空间变更----'
        self.log.info(step_txt)
        ...
        tb_tbspc_oid = tmp_result.splitlines()[-2].split('|')[1].strip()
        self.assertEqual(tb_tbspc_oid, '0', "执行失败" + step_txt)
        ...
        step_txt = '----step6:查询索引对应的表空间及索引文件物理位置; expect:表空间变更----'
        self.log.info(step_txt)
        ...
        tmp_result = self.sh.execut_db_sql(index_info_sql,
                                           dbname=f'{self.db_name}')
        self.log.info(tmp_result)
        idx_tbspc_oid = tmp_result.splitlines()[-2].split('|')[1].strip()
        self.assertEqual(idx_tbspc_oid, '0', "执行失败" + step_txt)
        ...
        step_txt = '----step7:在tablespace1上创建的表插入数据; expect:插入成功----'
        self.log.info(step_txt)
        ...
        step_txt = '----step8:查询数据; expect:正常使用索引----'
        self.log.info(step_txt)
    ...
    def check_ob_tbspc(self, object_name):
        ...
        location_sql = f"select pg_relation_filepath(" \
            ...
            f"::regclass);"
        t_link = self.sh.execut_db_sql(location_sql,
                                       dbname=f'{self.db_name}').splitlines()[
            -2].strip()
