...
"""
Case Type   : 功能测试-表空间
Case Name   : 设置表空间属性（set）seq_page_cost顺序读取page的开销
Description :
    1、修改表空间seq_page_cost设定值为负数
    2、修改表空间seq_page_cost设定值为字符串
    3、修改表空间seq_page_cost不设置值
    4、修改表空间seq_page_cost设定值为正整数
    5、修改表空间seq_page_cost设定值为正浮点数
    6、同时修改表空间seq_page_cost、random_page_cost设定值为正浮点数
Expect      :
    1、修改表空间seq_page_cost设定值为负数，修改失败
    2、修改表空间seq_page_cost设定值为字符串，修改失败
    3、修改表空间seq_page_cost不设置值，修改成功
    4、修改表空间seq_page_cost设定值为正整数，修改成功
    5、修改表空间seq_page_cost设定值为正浮点数，修改成功
    6、同时修改表空间seq_page_cost、random_page_cost设定值为正浮点数，修改成功
History     :
"""
...
class Tablespace(unittest.TestCase):
    def setUp(self):
        ...
        self.tbspc_name1 = 'tsp_tbspc0027'
        self.tbspc_location1 = 'tbspc0027'
        self.pwd = macro.PASSWD_INITIAL
        self.sys_user = 'u_tbspc0027_sys'
        self.connect_sys = f'-U {self.sys_user} -W {self.pwd}'
        self.err_flag1 = 'out of bounds for option "seq_page_cost"'
    ...
    def test_main(self):
        ...
        step_txt = '----step0:缺省seq_page_cost创建表空间; expect:创建成功----'
        self.log.info(step_txt)
        ...
        step_txt = '----step1:修改表空间seq_page_cost设定值为负数; expect:修改失败----'
        self.log.info(step_txt)
        alter_sql = f"alter  tablespace {self.tbspc_name1} " \
            f"set (seq_page_cost='-6');"
        ...
        step_txt = '----step2:修改表空间seq_page_cost设定值为字符串; expect:修改失败----'
        self.log.info(step_txt)
        alter_sql = f"alter  tablespace {self.tbspc_name1} " \
            f"set (seq_page_cost='abc');"
        alter_result = self.pri_sh.execut_db_sql(alter_sql,
                                                 sql_type=self.connect_sys)
        self.log.info(alter_result)
        self.assertIn(self.err_flag2, alter_result, "执行失败" + step_txt)

        step_txt = '----step3:修改表空间seq_page_cost不设置值; expect:修改失败----'
        self.log.info(step_txt)
        alter_sql = f"alter tablespace {self.tbspc_name1} " \
            f"set (seq_page_cost);"
        alter_result = self.pri_sh.execut_db_sql(alter_sql,
        ...
        step_txt = '----step4:修改表空间seq_page_cost设定值为正整数; expect:修改成功----'
        self.log.info(step_txt)
        alter_sql = f"alter tablespace {self.tbspc_name1} " \
            f"set (seq_page_cost='6');"
        alter_result = self.pri_sh.execut_db_sql(alter_sql,
        ...
        spcoptions = self.pri_sh.execut_db_sql(check_sql).splitlines()[-2]
        self.log.info(spcoptions)
        self.assertIn('seq_page_cost=6', spcoptions, "执行失败" + step_txt)

        step_txt = '----step5:修改表空间seq_page_cost设定值为正浮点数; expect:修改成功----'
        self.log.info(step_txt)
        alter_sql = f"alter  tablespace {self.tbspc_name1} " \
            f"set (seq_page_cost=9.333);"
        alter_result = self.pri_sh.execut_db_sql(alter_sql,
                                                 sql_type=self.connect_sys)
        self.log.info(alter_result)
        self.assertIn(self.constant.TABLESPCE_ALTER_SUCCESS, alter_result,
        ...
        step_txt = '----step6:同时修改表空间seq_page_cost、random_page_cost设定值' \
                   '为正浮点数; expect:修改成功----'
        self.log.info(step_txt)
        alter_sql = f"alter  tablespace {self.tbspc_name1} " \
