...
"""
Case Type   : 功能测试
Case Name   : Jsonb额外支持的操作符:?|数组字符串中的任何一个是否做为顶层键存在,右侧操作数为text[]
Description :
    1.数组字符串中的任何一个做为顶层键存在
    2.数组字符串中的任何一个做为顶层键不存在
Expect      :
    1.数组字符串中的任何一个做为顶层键存在,执行结果为t
    2.数组字符串中的任何一个做为顶层键不存在,执行结果为f
History     :
"""
...
class Jsonb(unittest.TestCase):
    ...
    def test_copy(self):
        self.log.info('---Opengauss_Function_Datatype_Jsonb_Case0037开始---')
        self.log.info('------数组字符串中的任何一个做为顶层键存在,执行结果为t------')
        sql_cmd1 = r'''
            ...
            select '{{\"a\":1,\"b\": [1,2,3],\"c\":{{\"b\":\"d\"}}}}'
            ::jsonb ?| array['b','c'];
            select '[null, false, 123,{{\"a\":true}},\"test\"]'::jsonb ?| 
            array['test','123'];
            '''
        ...
        self.log.info('--------数组字符串中的任何一个做为顶层键不存在,执行结果为f-------')
        sql_cmd2 = r'''
            select 'null'::jsonb ?| array['null'];
            ...
            select '[{{\"a\":true}}, null] ':: jsonb ?| array['b'];
            '''
    ...
    def tearDown(self):
        ...
        self.log.info('---Opengauss_Function_Datatype_Jsonb_Case0037结束---')