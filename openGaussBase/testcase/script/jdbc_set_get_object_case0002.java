...
/**
 **/
public class jdbc_set_get_object_case0002 {
    ...
    public static void main(String[] args) {
        ...
        if(conn==null){
            ...
            try{
                String sql = "drop table if exists jdbc_set_get_object_case0001;" +
                        "create table jdbc_set_get_object_case0001(t_date date);";
                PreparedStatement ps_create=conn.prepareStatement(sql);
                ps_create.execute();
                ps_create.close();

                sql = "insert into jdbc_set_get_object_case0001 values(?)";
                ...
                ps_insert.execute();
                ps_insert.setObject(1, LocalDateTime.ofEpochSecond(0, 0, ZoneOffset.of("+00:00")));
                ps_insert.execute();
                ps_insert.setObject(1, LocalDateTime.of(2021, 1, 29, 21,59));
                ps_insert.execute();
                ...
                sql = "select * from jdbc_set_get_object_case0001;";
                PreparedStatement ps_select=conn.prepareStatement(sql);
                ResultSet rs=ps_select.executeQuery();
                int idx=1;
                while(rs.next()){
                    LocalDateTime result1 = (LocalDateTime)rs.getObject(1, LocalDateTime.class);
