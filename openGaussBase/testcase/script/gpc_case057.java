...
public class gpc_case057 {
    ...
    public static String genURLFromHostsPortsDBname(String[] hostnames, String[] ports, String dbname) {
        ...
        if (hostnames.length == 1) {
            sourceURL = "jdbc:postgresql://" + hostnames[0] + ":" + ports[0] + "/" + dbname + "?prepareThreshold=1";
        } else {
    ...
    public static void main(String[] args) {
        ...
        if(conn==null){
            ...
            try{
                String sql = "drop table if exists gpc_case056;" +
                        "create table gpc_case056(i int);";
                PreparedStatement ps_create=conn.prepareStatement(sql);
                ps_create.execute();
                ps_create.close();

                sql = "insert into gpc_case056 values(?)";
                PreparedStatement ps_insert=conn.prepareStatement(sql);
                ps_insert.setObject(1,"1");
                ps_insert.execute();
                ps_insert.setObject(1,"1");
                ps_insert.execute();
                ps_insert.setObject(1,"1");
