...
public class jdbc_PrepareStatement_Case0011_15 {
    ...
    public static void main(String[] args) {
        ...
        //  更新预编译
        PreparedStatement stmt_updt = null;
        try {
            stmt_updt = conn.prepareStatement("UPDATE "+tb_name+" SET c_customer_name = ? WHERE c_customer_sk = ?");
            for (int i=0;i<1000;i++)
            {
            	stmt_updt.setInt(2, i);
                stmt_updt.setString(1, "modified "+i);
                stmt_updt.executeUpdate();
                if (i == 0 || i == 999)
                {
                	System.out.println("update succeed!"+i);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
