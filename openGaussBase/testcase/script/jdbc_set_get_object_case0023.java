...
/**
 **/
public class jdbc_set_get_object_case0023 {
    ...
    public static void main(String[] args) {
        ...
        if(conn==null){
            ...
            try{
                String sql = "insert into jdbc_set_get_object_case0023_time values(?)";
                PreparedStatement ps_insert=conn.prepareStatement(sql);
                ps_insert.setObject(1, LocalTime.now());
                ps_insert.execute();
                ps_insert.close();

                sql = "insert into jdbc_set_get_object_case0023_date values(?)";
                ps_insert=conn.prepareStatement(sql);
                ps_insert.setObject(1, LocalDate.now());
                ps_insert.execute();
                ps_insert.close();

                sql = "insert into jdbc_set_get_object_case0023_stamptz values(?)";
                ps_insert=conn.prepareStatement(sql);
                ps_insert.setObject(1, OffsetDateTime.now());
                ps_insert.execute();
                ps_insert.close();

                sql = "insert into jdbc_set_get_object_case0023_stamp values(?)";
                ps_insert=conn.prepareStatement(sql);
