...
/**
 **/
public class jdbc_set_get_object_case0011 {
    ...
    public static void main(String[] args) {
        ...
        if(conn==null){
            ...
            try{
                String sql = "drop table if exists jdbc_set_get_object_case0011;" +
                        "create table jdbc_set_get_object_case0011(t_TIMESTAMP TIMESTAMP with time zone);";
                PreparedStatement ps_create=conn.prepareStatement(sql);
                ps_create.execute();
                ps_create.close();

                sql = "insert into jdbc_set_get_object_case0011 values(?)";
                PreparedStatement ps_insert=conn.prepareStatement(sql);
                ps_insert.setObject(1, OffsetDateTime.MAX);
                ps_insert.execute();
                ps_insert.setObject(1, OffsetDateTime.MIN);
                ps_insert.execute();
                ps_insert.setObject(1, null, Types.TIMESTAMP_WITH_TIMEZONE);
                ps_insert.execute();
                ps_insert.setObject(1, OffsetDateTime.now());
                ps_insert.execute();
                ps_insert.setObject(1, OffsetDateTime.now(ZoneOffset.of("+08:00")));
                ps_insert.execute();
                ps_insert.setObject(1, OffsetDateTime.parse("2020-02-29T23:59:59-09:00"));
                ps_insert.execute();
                ps_insert.setObject(1, OffsetDateTime.of(2020, 02, 28, 23,
                        59, 59, 0, ZoneOffset.of("-09:00")));
                ps_insert.execute();
                ...
                sql = "select * from jdbc_set_get_object_case0011  ;";
                PreparedStatement ps_select=conn.prepareStatement(sql);
                ResultSet rs=ps_select.executeQuery();
                int idx=1;
                while(rs.next()){
                    OffsetDateTime result1 = rs.getObject(1, OffsetDateTime.class);
