...
public class jdbc_PrepareStatement_Case0001_05 {
    ...
    public static void main(String[] args) {
		...
		// 插入预编译
        PreparedStatement stmt_inst = null;
        try {
            stmt_inst = conn.prepareStatement("insert into "+tb_name+" (c_customer_sk, c_customer_name,c_customer_bak) values (?,?,?)");
            for (int i = 50000; i <= 100000; i++) {
                stmt_inst.setInt(1, i);
                stmt_inst.setString(2, "data" + i);
                stmt_inst.setString(3, "bak" + i);
                stmt_inst.executeUpdate();
                if (i == 50000 || i == 100000)
                {
                	System.out.println("insert succeed!"+i);
                	Date d = new Date();
