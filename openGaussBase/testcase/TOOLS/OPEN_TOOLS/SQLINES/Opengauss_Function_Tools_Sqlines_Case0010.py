...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具，不指定-in参数，合理报错
Description :
    1、sqlines安装
    2、创建测试脚本
    3、sqlines -s=oracle -t=opengauss -out={self.out_file} -log={self.log_file}
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0010.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0010_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0010.log'
    ...
    def test_main(self):
        ...
        step_txt = '----step3:执行sqlines命令不指定-in参数，expect: 合理报错----'
        self.log.info(step_txt)
        cmd = f"{self.source_path};" \
            f"sqlines -s=oracle -t=opengauss -out={self.out_file} " \
            f"-log={self.log_file}"
        self.log.info(cmd)
        res = self.root.sh(cmd).result()
        self.log.info(res)
        check_point = '-in option not set'
