...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具-s参数值指定为无效值（非oracle|mysql|postgresql）
Description :
    1、sqlines安装
    2、创建测试脚本
    3、sqlines -s=adkfjkadf -t=opengauss -in={self.in_file}
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0004.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0004_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0004.log'
    ...
    def test_main(self):
        ...
        step_txt = '----step3:执行sqlines命令，expect: 不转换不报错----'
        self.log.info(step_txt)
        cmd1 = f"{self.source_path};" \
            f"sqlines -s=adkfjkadf -t=opengauss -in={self.in_file}" \
            f" -out={self.out_file} -log={self.log_file}"
        self.log.info(cmd1)
        res = self.root.sh(cmd1).result()
        self.log.info(res)
        self.assertNotIn('error', res, '执行失败：')
        cmd2 = f'diff {self.in_file} {self.out_file}'
        self.log.info(cmd2)
        res = self.root.sh(cmd2).result()
        self.log.info(res)
