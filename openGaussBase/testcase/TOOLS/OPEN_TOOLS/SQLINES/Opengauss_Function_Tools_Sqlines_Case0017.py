...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具，-out参数值使用绝对路径
Description :
    1、sqlines安装
    2、创建测试脚本
    3、sqlines -s=oracle -t=opengauss -in={self.in_file}
    -out=/root/sqlines_test/{self.out_file} -log={self.log_file}
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0017.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0017_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0017.log'
        self.out = os.path.join('/root', 'sqlines_test', f'{self.out_file}')
    ...
    def test_main(self):
        ...
        cmd = f"{self.source_path};" \
            ...
            f"-out={self.out} -log={self.log_file}"
        self.log.info(cmd)
        res = self.root.sh(cmd).result()
        self.log.info(res)
    ...
    def tearDown(self):
        ...
        cmd = f"rm -rf {self.file_name} {self.unzip_dir} sqlines.log " \
            f"{self.in_file} {self.out} {self.log_file}"
        self.log.info(cmd)
        self.root.sh(cmd)
