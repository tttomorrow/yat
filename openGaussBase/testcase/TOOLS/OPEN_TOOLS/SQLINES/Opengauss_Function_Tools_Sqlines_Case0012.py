...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具，-in参数值使用绝对路径
Description :
    1、sqlines安装
    2、创建测试脚本
    3、sqlines -s=oracle -t=opengauss -in=/root/sqlines_test/{self.in_file}
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0012.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0012_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0012.log'
        self.ind = os.path.join('/root', 'sqlines_test', self.in_file)
    ...
    def test_main(self):
        ...
        cmd3 = f'mkdir sqlines_test;mv {self.in_file} sqlines_test/'
        self.log.info(cmd3)
        self.root.sh(cmd3)

        step_txt = '----step3:执行sqlines命令，expect: 执行成功----'
        self.log.info(step_txt)
        cmd = f"{self.source_path};" \
            f"sqlines -s=oracle -t=opengauss " \
            f"-in={self.ind} -out={self.out_file} -log={self.log_file}"
        self.log.info(cmd)
        res = self.root.sh(cmd).result()
        self.log.info(res)
    ...
    def tearDown(self):
        ...
        cmd = f"rm -rf {self.file_name} {self.unzip_dir} sqlines.log " \
            f"{self.ind} {self.out_file} {self.log_file} sqlines_test"
        self.log.info(cmd)
        self.root.sh(cmd)
