...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具，-log参数值使用绝对路径
Description :
    1、sqlines安装
    2、创建测试脚本
    3、sqlines -s=postgresql -t=opengauss -in={self.in_file}
    -out={self.out_file} -log=/root/{self.log_file}
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0020.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0020_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0020.log'
        self.log_out = os.path.join('/root', self.log_file)
    ...
    def test_main(self):
        ...
        cmd = f"{self.source_path};" \
            ...
            f"-out={self.out_file} -log={self.log_out}"
        self.log.info(cmd)
        res = self.root.sh(cmd).result()
        self.log.info(res)
        self.assertNotIn('error', res, '执行失败：')
        cmd2 = f'ls {self.log_out}'
        self.log.info(cmd2)
        res = self.root.sh(cmd2).result()
        self.log.info(res)
        self.assertEqual(res, f'{self.log_out}', '执行失败' + step_txt)
