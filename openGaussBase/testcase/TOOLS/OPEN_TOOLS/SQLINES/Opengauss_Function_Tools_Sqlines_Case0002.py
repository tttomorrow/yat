...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具，不指定-s参数，合理报错
Description :
    1、安装sqlines
    2、创建测试脚本
    3、sqlines -t=opengauss -in={self.in_file} -out={self.out_file}
    -log={self.log_file}
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0002.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0002_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0002.log'
    ...
    def test_main(self):
        ...
        step_txt = '----step3:执行sqlines命令不指定-s参数，expect: 合理报错----'
        self.log.info(step_txt)
        cmd = f"{self.source_path};" \
            f"sqlines -t=opengauss -in={self.in_file}" \
            f" -out={self.out_file} -log={self.log_file}"
        self.log.info(cmd)
        res = self.root.sh(cmd).result()
        self.log.info(res)
        check_point = '-s option not set'
