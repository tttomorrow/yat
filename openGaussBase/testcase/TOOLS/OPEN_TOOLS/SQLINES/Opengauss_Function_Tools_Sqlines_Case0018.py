...
Expect      :
    1、sqlines安装成功
    2、脚本创建成功
    3、转换成功，日志存放到当前工作目录下，文件名为：sqlines.log
    4、环境清理成功
History     :
"""
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0018.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0018_out.sql'
    ...
    def test_main(self):
        ...
        res = self.root.sh(cmd2).result()
        self.log.info(res)
        self.assertEqual(res, 'sqlines.log', '执行失败' + step_txt)
