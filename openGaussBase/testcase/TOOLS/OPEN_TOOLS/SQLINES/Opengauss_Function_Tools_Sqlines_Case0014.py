...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具，-in参数值使用特殊字符
Description :
    1、sqlines安装
    2、创建文件名含有特殊符号的脚本文件
    3、sqlines -s=oracle -t=opengauss -in=.~@#$%^[] -out={self.out_file}
    -log={self.log_file}
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0014_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0014.log'
    ...
    def test_main(self):
        ...
        step_txt = '----step2:创建脚本文件，expect: 脚本创建成功----'
        self.log.info(step_txt)
        cmd1 = "echo 'create table test(id int);' > .~@#$%^[]"
        self.log.info(cmd1)
        self.root.sh(cmd1)
        cmd2 = 'ls .~@#$%^[]'
        self.log.info(cmd2)
        res = self.root.sh(cmd2).result()
        self.log.info(res)
        self.assertEqual(res, '.~@#$%^[]', '执行失败' + step_txt)

        step_txt = '----step3:执行sqlines命令，expect: 执行成功,未报错----'
        self.log.info(step_txt)
        cmd = f"{self.source_path};" \
            f"sqlines -s=oracle -t=opengauss -in=.~@#$%^[] " \
