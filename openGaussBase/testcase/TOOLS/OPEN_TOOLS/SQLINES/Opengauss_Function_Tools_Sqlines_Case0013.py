...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具，-in参数值使用通配符
Description :
    1、sqlines安装
    2、创建测试脚本
    3、sqlines -s=oracle -t=opengauss -in=*.sql -out={self.out_file}
    -log={self.log_file}
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0013.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0013_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0013.log'
    ...
    def test_main(self):
        ...
        cmd = f"{self.source_path};" \
            f"sqlines -s=oracle -t=opengauss -in=*.sql -out={self.out_file}" \
            f" -log={self.log_file}"
        self.log.info(cmd)
        res = self.root.sh(cmd).result()
        self.log.info(res)
