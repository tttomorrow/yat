...
"""
Case Type   : sqlines工具
Case Name   : sqlines工具，不指定-out参数
Description :
    1、sqlines安装
    2、创建测试脚本
    3、sqlines -s=oracle -t=opengauss -in={self.in_file} -log={self.log_file}
    4、环境清理：卸载sqlines、删除对应文件
Expect      :
    1、sqlines安装成功
    2、脚本创建成功
    3、转换成功，文件和输入文件存放在相同目录，且文件名为输入文件名+'_out'
    4、环境清理成功
History     :
"""
...
class SqlinesTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.in_file = 'Opengauss_Function_Tools_Sqlines_Case0015.sql'
        self.out_file = 'Opengauss_Function_Tools_Sqlines_Case0015_out.sql'
        self.log_file = 'Opengauss_Function_Tools_Sqlines_Case0015.log'
    ...
    def test_main(self):
        ...
        step_txt = '----step3:执行sqlines命令不指定-out参数，expect: 转换成功，' \
                   '文件和输入文件存放在相同目录，且文件名为输入文件名+_out----'
        self.log.info(step_txt)
        ...
        cmd2 = f'ls {self.out_file}'
        self.log.info(cmd2)
        res = self.root.sh(cmd2).result()
        self.log.info(res)
        self.assertEqual(res, f'{self.out_file}', '执行失败' + step_txt)
