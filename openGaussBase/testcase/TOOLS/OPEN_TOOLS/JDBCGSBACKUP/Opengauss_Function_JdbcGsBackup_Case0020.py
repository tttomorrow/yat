...
Case Name   :  JdbcGsBackup -m dump,导出/导入指定数据库下的物化视图和视图
Description :
        1.创建数据库
        2.指定数据库下建表，建物化视图和视图，建用户
        3.修改信任方式为sha256
        4.导出指定数据库
        5.删表后执行导入
        6.查看视图
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0020start---')
    ...
    def test_tools_backup(self):
        ...
        self.log.info('---在指定数据库下创建测试用户,表，视图---')
        sql_cmd = f'''drop table if exists {self.tb_name} cascade;
            ...
            create materialized view t_mv (id, name) as select * from \
            {self.tb_name};
            drop view if exists test_v20;
            create view test_v20 as select * from {self.tb_name};
        ...
        self.log.info('---查看视图，验证导入成功---')
        sql_cmd = f'''select * from t_mv;
            select * from test_v20'''
        ...
        self.log.info(msg)
        result = msg.splitlines()[-2].strip()
        self.log.info(result)
        self.assertTrue(result.find('1 | aaa') > -1)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0020finish---')
