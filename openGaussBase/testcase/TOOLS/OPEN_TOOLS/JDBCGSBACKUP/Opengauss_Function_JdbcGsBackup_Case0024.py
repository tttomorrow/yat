...
Case Name   :  JdbcGsBackup -m dump导出指定数据库,指定-f选项(目录非空)，
              导出合理报错
Description :
        1.创建数据库
        2.指定数据库下建表，建用户
...
Expect      :
        ...
        2.创建成功
        3.修改成功
        4.导出报错backup failed: destination file is not empty
        5.清理环境完成
History     :
"""
import os
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0024start---')
    ...
    def test_tools_backup(self):
        ...
        self.log.info(msg)
        self.assertIn('backup failed: destination file is not empty', msg)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0024finish---')
