...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        text = '----Opengauss_Function_JdbcGsBackup_Case0072开始执行----'
        self.log.info(text)
        ...
        self.db_name = "db_jdbcgsbackup_0072"
        self.user = "u_jdbcgsbackup_0072"
        self.tb_name = "t_jdbcgsbackup_0072"
    ...
    def test_tools_backup(self):
        ...
        text = '----step8:修改文件权限后导入;expect:导入报错----'
        self.log.info(text)
        sql_cmd = f'''chmod 000 {self.backup}/openGauss_backup.zip;
            cd {self.backup};
            java -jar openGauss-tools-backup.jar \
            -m restore \
            -d {self.db_name} \
        ...
        msg = self.Primary_Node.sh(sql_cmd).result()
        self.log.info(msg)
        self.assertIn('Permission denied', msg, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0072finish---')
