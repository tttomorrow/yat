...
Case Name   :  JdbcGsBackup -m dump,指定-f选项(用户没有该目录的读写权限)，
              导出合理报错
Description :
        1.创建数据库
        2.指定数据库下建表，建用户
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.pri_sh = CommonSH('dbuser')
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0023start---')
    ...
    def test_tools_backup(self):
        ...
        self.log.info('---创建导出文件并修改权限---')
        mkdir_cmd = f"touch {self.package}/dump_023;" \
                    f"chmod 000 {self.package}/dump_023;"
        self.log.info(mkdir_cmd)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0023finish---')
