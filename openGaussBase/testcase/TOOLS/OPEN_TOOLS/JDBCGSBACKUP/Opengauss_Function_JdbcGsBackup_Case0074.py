...
Case Name   : jdbcpgbackup -m xxx, 省略-s选项，导出/导入全部模式
Description :
        1.创建工具所在目录
        2.获取工具安装包并解压
        3.创建数据库
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0074start---')
        ...
        self.db_name = "db_jdbcgsbackup_0074"
        self.user = "u_jdbcgsbackup_0074"
        self.tb_name = "t_jdbcgsbackup_0074"
    ...
    def test_tools_backup(self):
        ...
        text = '---step7:删除表后执行导入;expect:导入报错---'
        self.log.info(text)
        sql_cmd = f'''drop table if exists {self.tb_name};'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0074finish---')
