...
Case Name   :  JdbcGsBackup -m dump导出指定数据库,数据库不存在，导出报错
Description :
        1.创建用户
        2.修改信任方式为sha256
        3.导出，数据库不存在
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0013start---')
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''drop database if exists {self.db_name} cascade;
        ...
        self.log.info(msg)
        self.assertIn('FATAL: database "gs_db03" does not exist', msg)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_result)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0013finish---')
