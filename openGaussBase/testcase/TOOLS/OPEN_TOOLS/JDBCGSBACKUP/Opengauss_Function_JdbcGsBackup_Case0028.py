...
Case Name   :  JdbcGsBackup -m 导出及导入，-s选项测试（自定义模式）
Description :
        1.创建数据库
        2.指定数据库下建表，建用户
        3.修改表的schema为自定义schema
        4.修改信任方式为sha256
        4.导出指定数据库,-s指定1个模式
        5.删除schema1下的表后执行导入
        6.查看导入表
        7.导出，-s指定多个模式
        8.删除schema2和schema3下的表执行导入
        9.查看导入表
        10.清理环境
Expect      :
        1.创建成功
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0028start---')
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''drop table if  exists schema1.pgbackup_t1;'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        ...
        self.assertNotIn(self.constant.jdbcgsbackup_failed[0] and
                         self.constant.jdbcgsbackup_failed[1] and
                         self.constant.jdbcgsbackup_failed[2], msg)
        self.log.info('---查询导入的表不指定模式，报错---')
        sql_cmd = f'''select * from pgbackup_t1;'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn('ERROR:  relation "pgbackup_t1" does not exist ',
                      sql_result)
        self.log.info('---查询导入的表指定模式---')
        sql_cmd = f'''select * from schema1.pgbackup_t1;'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn('3 rows', sql_result)
        self.log.info('---导出多个模式,多个模式用逗号分隔--')
        sql_cmd = f'''cd {self.package}/openGauss-tools-backup;\
            java -jar openGauss-tools-backup.jar \
            -m dump \
            -d {self.db_name} \
            ...
            -P {macro.COMMON_PASSWD} \
            -s schema2,schema3 \
            -f {macro.DB_INSTANCE_PATH}/dump028 '''
        self.log.info(sql_cmd)
        msg = self.Primary_Node.sh(sql_cmd).result()
        self.log.info(msg)
        self.assertIn(self.constant.jdbcgsbackup_success, msg)
        self.assertNotIn(self.constant.jdbcgsbackup_failed[0] and
                         self.constant.jdbcgsbackup_failed[1] and
                         self.constant.jdbcgsbackup_failed[2], msg)
        self.log.info('---删除schema2和schema3下的表后执行导入---')
        sql_cmd = f'''drop table if  exists schema2.pgbackup_t2;
            drop table if  exists schema3.pgbackup_t3;'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, sql_result)
        ...
        self.assertNotIn(self.constant.jdbcgsbackup_failed[0] and
                         self.constant.jdbcgsbackup_failed[1] and
                         self.constant.jdbcgsbackup_failed[2], msg)
        self.log.info('---查询导入的表---')
        sql_cmd = f'''select * from schema2.pgbackup_t2;'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn('3 rows', sql_result)
        sql_cmd = f'''select * from schema3.pgbackup_t3;'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn('3 rows', sql_result)
    ...
    def tearDown(self):
        ...
        self.log.info(result)
        sql_cmd = f'''rm -rf {macro.DB_INSTANCE_PATH}/dump028;'''
        self.log.info(sql_cmd)
        result = self.Primary_Node.sh(sql_cmd).result()
        self.log.info(result)
        cmd = f"rm -rf " \
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0028finish---')
