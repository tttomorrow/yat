...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        text = '----Opengauss_Function_JdbcGsBackup_Case0056开始执行----'
        self.log.info(text)
        ...
        self.file_path = os.path.join(self.backup, 'gs_backup', 'schemas',
                                      'public', 'tables.sql')
        self.db_name = "db_jdbcgsbackup_0056"
        self.user = "u_jdbcgsbackup_0056"
        self.tb_name = "t_jdbcgsbackup_0056"
        self.tb_name_01 = "t_jdbcgsbackup_0056_01"
    ...
    def test_tools_backup(self):
        ...
        text = '----step7:解压导出文件并修改导出文件内容 ' \
               'expect:修改导出文件成功----'
        self.log.info(text)
        sql_cmd = f'''cd {self.backup};
            unzip openGauss_backup.zip;
            echo 'CREATE TABLE {self.tb_name_01} (id int4,name varchar(20))\
            WITH (orientation=row,compression=no);'>{self.file_path}'''
        msg = self.Primary_Node.sh(sql_cmd).result()
        self.log.info(msg)
        self.assertIn('gs_backup', sql_cmd, '执行失败:' + text)

        text = '----step8:重新压缩gs_backup文件后导入 ' \
               'expect:导入重新压缩后的文件成功----'
        self.log.info(text)
        sql_cmd = f'''cd {self.backup};
            zip -r gs_backup.zip gs_backup;'''
        msg = self.Primary_Node.sh(sql_cmd).result()
        self.log.info(msg)
        sql_cmd = f'''cd {self.backup};\
            java -jar openGauss-tools-backup.jar \
        ...
        text = '----step9:查询导入的表 expect:新表导入成功，无数据----'
        self.log.info(text)
        sql_cmd = f'''select count(*) from {self.tb_name_01};'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertEqual('0', sql_result.splitlines()[-2].strip(),
                         '执行失败:' + text)

        text = '----step10:查询已存在的表 expect:导入成功----'
        self.log.info(text)
        self.log.info('---查询已存在的表---')
        sql_cmd = f'''select count(*) from {self.tb_name};'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertEqual('200', sql_result.splitlines()[-2].strip(),
                         '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0056finish---')
