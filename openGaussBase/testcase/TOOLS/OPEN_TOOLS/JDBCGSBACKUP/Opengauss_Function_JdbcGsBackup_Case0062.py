...
Case Name   : failover后，新主节点执行导出/导入，-h指定新主节点
Description :
        1.执行failover
        2.新主节点创建工具所在目录
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0062start---')
        ...
        self.db_name = "db_jdbcgsbackup_0062"
        self.user = "u_jdbcgsbackup_0062"
        self.tb_name = "t_jdbcgsbackup_0062"
    ...
    def test_tools_backup(self):
        ...
        text = '----step6:备份新主节点pg_hba.conf文件并修改新主节点认证方式;' \
               'expect:修改成功----'
        self.log.info(text)
        ...
        text = '----step9:导入,-h指定新主节点; expect:导入成功----'
        self.log.info(text)
        sql_cmd = f'''cd {self.package}/openGauss-tools-backup;\
            java -jar openGauss-tools-backup.jar \
            -m restore \
        ...
        text = 'step10:---查看表数据---;expect:表导入成功，数据正确---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in result or "Normal" in result,
                        '执行失败:' + text)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0062finish---')
