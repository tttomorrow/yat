...
Case Name   : jdbcgsbackup -m xxx,session1导出时，session2其他用户查询表
Description :
        1.创建工具目录
        2.获取工具安装包并解压
        3.创建数据库
        4.指定数据库下建用户，创建表并修改表的模式
        5.修改信任方式为sha256
        6.session1导出时，session2查询表
        7.获取2个session结果
        8.删除表后执行导入
        9.查看导入表数据
        10.清理环境
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0055start---')
        ...
        self.db_name = "db_jdbcgsbackup0055"
        self.user = "s_jdbcgsbackup0055"
        self.tb_name = "t_jdbcgsbackup0055"
    ...
    def test_tools_backup(self):
        ...
        text = 'step6.1:---session2初始用户查询表;expect:查询成功---'
        self.log.info(text)
        sql_cmd = f'''select count(*) from {self.tb_name};'''
        self.log.info(sql_cmd)
        connect_thread2 = ComThread(self.pri_sh.execut_db_sql, args=(
            sql_cmd,
            f'-U {self.Primary_Node.ssh_user} '
            f'-W {self.Primary_Node.ssh_password}',
            f'{self.db_name}'))
        connect_thread2.setDaemon(True)
        connect_thread2.start()

        text = 'step7:---获取2个session结果;expect获取成功---'
        self.log.info(text)
        ...
        self.assertEqual('1000000', thread2_result.splitlines()[2].strip(),
                         '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0055finish---')
