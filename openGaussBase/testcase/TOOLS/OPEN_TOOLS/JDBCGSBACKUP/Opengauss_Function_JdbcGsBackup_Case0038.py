...
Case Name   :  创建非约束触发器后，使用jdbcpgbackup -m dump导出指定数据库
Description :
        1.创建数据库
        2.指定数据库下建表，建用户，触发器函数
        3.创建触发器并执行INSERT触发事件
        4.修改信任方式为sha256
        5.导出指定数据库
        6.删表后执行导入
        7.查看导入表
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0038start---')
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''drop user if exists {self.user};
            ...
            drop table if exists {self.tb_name_1} cascade;
            create table {self.tb_name}(id1 int, id2 int, id3 int);
            create table  {self.tb_name_1}(id1 int, id2 int, id3 int);'''
        self.log.info(sql_cmd)
        ...
        self.log.info('---创建触发器函数---')
        sql_cmd = f'''create or replace function tri_insert_func01() returns \
        trigger as
            \$\$
            declare
            begin
                insert into {self.tb_name_1} values(new.id1, new.id2, \
                new.id3);
                return new;
            end
            \$\$ language plpgsql;
            create trigger insert_jdbc01 before insert on {self.tb_name} \
            for each row execute procedure tri_insert_func01();
        ...
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn(self.constant.TRIGGER_CREATE_SUCCESS_MSG, sql_result)
        ...
        sql_cmd = f'''drop table if  exists {self.tb_name};
            drop table if  exists {self.tb_name_1};'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.log.info('---导入---')
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0038finish---')
