...
Case Name   :  JdbcGsBackup -m dump,指定-f选项(文件存在且有权限)
Description :
        1.创建数据库
        2.指定数据库下创建测试用户和表
        3.修改信任方式为sha256
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0021start---')
    ...
    def test_tools_backup(self):
        ...
        self.log.info('---创建导出文件---')
        sql_cmd = f'touch {self.package}/dump_021.zip'
        self.log.info(sql_cmd)
        mk_cmd = self.Root_Node.sh(sql_cmd).result()
        self.log.info(mk_cmd)
        self.log.info('---导出至指定文件---')
    ...
    def tearDown(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0021finish---')
