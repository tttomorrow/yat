...
Case Name   : 系统管理员使用JdbcGsBackup -m dump导出/导入指定数据库,-h为主机名
Description :
        1.创建工具所在目录
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0003start---')
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''hostname'''
        self.log.info(sql_cmd)
        hostname_msg = self.Primary_Node.sh(sql_cmd).result()
        self.log.info(hostname_msg)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0003finish---')
