...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0058start---')
        ...
        self.db_name = "db_jdbcgsbackup_0058"
        self.user = "u_jdbcgsbackup_0058"
        self.tb_name = "t_jdbcgsbackup_0058"
    ...
    def test_tools_backup(self):
        ...
        text = '----step6:导出指定数据库5次 expect:导出成功----'
        self.log.info(text)
        for i in range(1, 6):
            file_path = f"{macro.DB_INSTANCE_PATH}/dump_zh_{i}.zip"
    ...
    def tearDown(self):
        ...
        sql_cmd = f"rm -rf {self.package};" \
                  f"rm -rf {macro.DB_INSTANCE_PATH}/dump_zh_*.zip"
        self.log.info(sql_cmd)
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0058finish---')
