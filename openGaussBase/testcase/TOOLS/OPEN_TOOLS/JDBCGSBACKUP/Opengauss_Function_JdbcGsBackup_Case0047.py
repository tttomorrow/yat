...
Case Name   : 使用jdbcpgbackup -m dump导出指定数据库下的全局临时表
Description :
        1.创建工具目录
        2.获取工具安装包并解压
        3.创建数据库
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0047start---')
        ...
        self.db_name = "db_jdbcgsbackup_0047"
        self.user = "u_jdbcgsbackup_0047"
        self.tb_name = "t_jdbcgsbackup_0047"
    ...
    def test_tools_backup(self):
        ...
        text = '---step4:创建测试用户;创建全局临时表;expect:创建成功---'
        self.log.info(text)
        sql_cmd = f'''drop user if exists {self.user};
            create user {self.user} with sysadmin \
            password '{macro.COMMON_PASSWD}';
            drop table if exists {self.tb_name};
            create global temporary table {self.tb_name}
            (id integer not null,name char(16) not null,address varchar(50),\
            postcode char(6)) on  commit preserve rows;
            insert into {self.tb_name} values(5,'数据库','西安','55822');'''
        self.log.info(sql_cmd)
        ...
        text = '---step8:查询表;expect:导入成功，表无数据---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0047finish---')
