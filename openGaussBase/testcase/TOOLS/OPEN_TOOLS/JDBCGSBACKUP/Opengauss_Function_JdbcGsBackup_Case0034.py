...
Case Name   :  JdbcGsBackup -m dump导出指定数据库下的MOT表
Description :
        1.创建数据库
        2.指定数据库下建MOT表，建用户
        3.修改信任方式为sha256
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0034start---')
    ...
    def test_tools_backup(self):
        ...
        self.log.info('-----关闭参数enable_incremental_checkpoint----')
        sql_cmd = f'''alter system set enable_incremental_checkpoint to off;'''
        self.log.info(sql_cmd)
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn('ALTER SYSTEM SET', sql_result)
        msg = self.pri_sh.restart_db_cluster()
        self.log.info(msg)
        status = self.pri_sh.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('---在指定数据库下创建测试用户和表---')
        sql_cmd = f'''drop foreign table if exists {self.tb_name};
            create foreign table {self.tb_name}(a int);
            insert into {self.tb_name} values(generate_series(1,2000));
        ...
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn(self.constant.CREATE_FOREIGN_SUCCESS_MSG, sql_result)
        ...
        self.assertNotIn(self.constant.jdbcgsbackup_failed[0] and
                         self.constant.jdbcgsbackup_failed[1] and
                         self.constant.jdbcgsbackup_failed[2], msg)
        self.log.info('---查询导入的表，MOT表未导入---')
        sql_cmd = f'''select * from {self.tb_name};'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn('ERROR:  relation "t_03" does not exist', sql_result)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0034finish---')
