...
Case Name   :  JdbcGsBackup -m dump只导出及导入模式（自定义模式），不导数据
Description :
        1.创建数据库
        2.指定数据库下建表，建用户
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0025start---')
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''drop table if  exists  schema1.pgbackup_t1;
            drop table if  exists  schema2.pgbackup_t2;
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0025finish---')
