...
Case Name   :  JdbcGsBackup -m xx，-s指定模式（自定义模式）,模式错误，导出报错
               其他符号分隔
Description :
        1.创建数据库
        2.指定数据库下建表，建用户
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0031start---')
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''cd {self.package}/openGauss-tools-backup;\
            ...
            -s schema1,schema2,schema3_new '''
        self.log.info(sql_cmd)
        msg = self.Primary_Node.sh(sql_cmd).result()
        self.log.info(msg)
        self.assertIn('backup failed: schema schema3_new not found in '
                      'database', msg)
        ...
        sql_cmd = f'''cd {self.package}/openGauss-tools-backup;\
            ...
            -P {macro.COMMON_PASSWD} \
            -s schema1,schema2,schema3_new \
            -n schema1,schema2,schema3_new'''
        self.log.info(sql_cmd)
        msg = self.Primary_Node.sh(sql_cmd).result()
        self.log.info(msg)
        self.assertIn(self.constant.jdbcgsbackup_failed[0] and
                      self.constant.jdbcgsbackup_failed[1], msg)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0031finish---')
