...
Case Name   : 使用jdbcpgbackup -m dump导出指定数据库,对象名称包含双引号
Description :
        1.创建数据库
        2.指定数据库下建表，建用户
        3.修改信任方式为sha256
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0039start---')
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''drop table if  exists "PGBACKUP_T1";
            ...
            insert into "PGBACKUP_T1" values (2);
        ...
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertEqual('2', sql_result.splitlines()[-2].strip())
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0039finish---')
