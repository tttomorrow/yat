...
Case Name   :  JdbcGsBackup -m dump,导出/导入指定数据库，
              不指定-U（默认postgres），指定-P选项
Description :
        1.创建数据库
        2.指定数据库下建表，建用户，用户名为postgres
        3.修改信任方式为sha256
        4.导出
        5.删表后执行导入
        6.查看导入表
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0019start---')
    ...
    def test_tools_backup(self):
        ...
        self.assertEqual('  1 | aa', msg_list[2])
        self.assertEqual('  2 | bb', msg_list[3])
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0019finish---')
