...
Case Name   :  JdbcGsBackup -m dump导出,添加-t选项，显示导出每项数据花费的时间
Description :
        1.创建数据库
        2.指定数据库下建表，建用户
        3.修改信任方式为sha256
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0015start---')
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0015finish---')
