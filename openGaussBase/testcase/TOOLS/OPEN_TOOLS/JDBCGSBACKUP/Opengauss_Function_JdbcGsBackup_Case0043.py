...
Case Name   : 使用jdbcpgbackup -m dump导出指定数据库，数据库下包含存储过程
Description :
        1.创建数据库
        2.指定数据库下建用户，建表并创建存储过程
        3.修改信任方式为sha256
        4.导出指定数据库
        5.删除表和存储过程后执行导入
        6.查看导入表
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0043start---')
        ...
        self.db_name = "db_jdbcgsbackup_0043"
        self.user = "u_jdbcgsbackup_0043"
        self.tb_name = "t_jdbcgsbackup_0043"
        self.pr_name = "pr_jdbcgsbackup_0043"
    ...
    def test_tools_backup(self):
        ...
        text = '---step4:创建测试用户;创建表和存储过程;expect:创建成功---'
        self.log.info(text)
        sql_cmd = f'''drop table if exists {self.tb_name};
            create table {self.tb_name}(id int,name varchar(100),ctime date);
            insert into {self.tb_name} values \
            (1,'数据库','2020-11-24 16:10:28');
            create or replace procedure {self.pr_name}(str boolean) is
            begin
            insert into {self.tb_name} values(2,'测试','2020-09-17 16:10:28');
            end;
        ...
        text = '---step7:删除表和存储过程后执行导入;expect:导入成功---'
        self.log.info(text)
        sql_cmd = f'''drop table if exists {self.tb_name};
            drop procedure {self.pr_name}(str boolean);'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        sql_cmd = f'''cd {self.package}/openGauss-tools-backup;\
        ...
        text = '---step8:查询表是否导;expect:表导入成功---'
        self.log.info(text)
        ...
        text = '---step9:调用存储过程;expect:报错---'
        self.log.info(text)
        sql_cmd = f'''call {self.pr_name}(true);'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0043finish---')
