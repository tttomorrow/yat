...
Case Name   :  JdbcGsBackup -m xx，-s导出public模式
               其他符号分隔
Description :
        1.创建数据库
        2.指定数据库下建表，建用户并修改其中一个表的schema
        3.修改信任方式为sha256
        4.导出指定数据库,添加-s选项
        5.删表后执行导入
        6.查看导入表
...
Expect      :
        ...
        2.创建成功
        3.修改成功
        4.public模式导出成功，schema1未导出
        5.public模式导入成功，schema1未导入
        6.schema1下的表未导入成功，public模式下的表导入成功
        7.清理环境完成
"""
import os
import unittest
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0030start---')
    ...
    def test_tools_backup(self):
        ...
        self.log.info('--创建测试用户和表;修改其中一个表的schema---')
        sql_cmd = f'''drop table if  exists pgbackup_t1;
        ...
        sql_cmd = f''' drop table schema1.pgbackup_t1;
            drop table pgbackup_t2;
            drop table pgbackup_t3;'''
        self.log.info(sql_cmd)
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        ...
        self.log.info(msg)
        self.assertIn(self.constant.jdbcgsbackup_success, msg)
        self.log.info('---查询pgbackup_t1表，报错，未导入---')
        ...
        self.log.info('---查询pgbackup_t2和pgbackup_t3表---')
        sql_cmd = f'''select * from pgbackup_t2;'''
        self.log.info(sql_cmd)
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn('3 rows', sql_result)
        sql_cmd = f'''select * from pgbackup_t3;'''
        self.log.info(sql_cmd)
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0030finish---')
