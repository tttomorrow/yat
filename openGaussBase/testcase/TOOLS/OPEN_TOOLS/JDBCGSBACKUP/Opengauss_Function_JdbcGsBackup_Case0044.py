...
Case Name   : 使用jdbcpgbackup -m dump导出/导入大数据（4千万）
Description :
        1.创建数据库
        2.指定数据库下建用户，建表并插入4千万数据
        3.修改信任方式为sha256
        4.导出指定数据库
        5.删除表后执行导入
        6.查看导入表
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0044start---')
        ...
        self.db_name = "db_jdbcgsbackup_0044"
        self.user = "u_jdbcgsbackup_0044"
        self.tb_name = "t_jdbcgsbackup_0044"
        self.values = '10000000'
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''drop user if exists {self.user};
            ...
            create table {self.tb_name}(i int);'''
        self.log.info(sql_cmd)
        ...
        text = '---step8:删除表执行导入;expect:导入成功---'
        self.log.info(text)
        ...
        text = '---step9:查询表是否导入;expect:导入成功,数据正确---'
        self.log.info(text)
        sql_cmd = f'''select count(*) from {self.tb_name};'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        self.log.info(sql_result)
        self.assertIn('40000000', sql_result.splitlines()[2].strip(),
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0044finish---')
