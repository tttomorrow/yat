...
Case Name   : 导出的数据库客户端编码与导入数据库客户端编码不一致
Description :
        1.创建工具目录
        2.获取工具安装包并解压
        3.创建数据库
...
Expect      :
        ...
        5.修改成功
        6.导出成功
        7.导入报错 ERROR: character with byte sequence 0xe6 0xb5 0x8b in
        encoding "UTF8" has no equivalent in encoding "LATIN1"
        8.清理环境完成
History     :
"""
import os
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0057start---')
        ...
        self.db_name = "db_jdbcgsbackup_0057"
        self.db_name_01 = "db_jdbcgsbackup_0057_01"
        self.user = "u_jdbcgsbackup_0057"
        self.tb_name = "t_jdbcgsbackup_0057"
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = self.pri_sh.execut_db_sql(f'''drop database if exists \
            ...
            create database {self.db_name_01} encoding 'latin1';''')
        self.log.info(sql_cmd)
        ...
        sql_cmd = f'''drop user if exists {self.user};
            ...
            create table {self.tb_name} (name varchar(20));
            insert into {self.tb_name} values ('测试数据库');'''
        self.log.info(sql_cmd)
        ...
        text = '----step7:导入至latin1编码的数据库 expect:导入报错----'
        self.log.info(text)
        sql_cmd = f'''cd {self.package}/openGauss-tools-backup;\
            java -jar openGauss-tools-backup.jar \
            -m restore \
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0057finish---')
