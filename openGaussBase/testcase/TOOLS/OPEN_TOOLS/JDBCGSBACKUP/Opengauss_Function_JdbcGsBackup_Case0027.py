...
Case Name   :  JdbcGsBackup -m dump导出指定文件格式为其他格式（非zip）
Description :
        1.创建数据库
        2.指定数据库下建表，建用户
        3.修改信任方式为sha256
        4.导出指定数据库,添加-f选项,导出文件为txt文件；导出文件无后缀
        5.删表后执行导入
        6.查看导入表
        7.清理环境
Expect      :
        ...
        2.创建成功
        3.修改成功
        4.导出成功（JdbcGsBackup程序用Java中ZipOutPutStream类来处理文件，
        该类是创建zip文件）
        5.导入成功
        6.表导入成功，数据正确
        7.清理环境完成
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0027start---')
    ...
    def test_tools_backup(self):
        ...
        self.log.info('---导出，指定-f选项，导出为txt格式-')
        sql_cmd = f'''cd {self.package}/openGauss-tools-backup;\
            ...
            -s public \
            -f {macro.DB_INSTANCE_PATH}/dump27.txt;'''
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0027finish---')
