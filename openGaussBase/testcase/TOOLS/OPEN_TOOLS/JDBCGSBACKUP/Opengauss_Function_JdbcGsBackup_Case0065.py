...
Case Name   : 备节点导出过程中停止主节点，-h主节点
Description :
        1.创建工具所在目录
        2.获取工具安装包并解压
        3.创建数据库
        4.指定数据库下建用户，建表
        5.修改主节点pg_hba.conf文件信任方式为sha256
        6.备节点导出过程中停止主节点，-h主节点
        7.重启集群并检查数据库状态
        8.清理环境
Expect      :
        1.创建成功
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0065start---')
        ...
        self.db_name = "db_jdbcgsbackup_0065"
        self.user = "u_jdbcgsbackup_0065"
        self.tb_name = "t_jdbcgsbackup_0065"
    ...
    def test_tools_backup(self):
        ...
        text = '---step5:修改主节点pg_hba.conf文件;expect:修改成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0065finish---')
