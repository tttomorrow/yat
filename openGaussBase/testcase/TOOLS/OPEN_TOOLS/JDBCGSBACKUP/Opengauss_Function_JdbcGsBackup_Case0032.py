...
Case Name   :  JdbcGsBackup -m xx，导出/导入指定数据库下的行存表
              （覆盖基本常用数据类型）
               其他符号分隔
Description :
        1.创建数据库
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0032start---')
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''drop user if exists {self.user};
            ...
            drop index if exists jdbcpgbackup_index1;
            create unique index jdbcpgbackup_index1 on {self.tb_name}\
            (col_tinyint);
            drop view if exists jdbcpgbackup_v032;
            create view jdbcpgbackup_v032 as select * from {self.tb_name};'''
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0032finish---')
