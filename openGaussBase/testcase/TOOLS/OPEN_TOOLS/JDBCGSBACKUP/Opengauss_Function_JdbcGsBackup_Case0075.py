...
Case Name   : 导出指定数据库下的对象，对象名包含特殊字符
Description :
        1.创建工具所在目录
        2.获取工具安装包并解压
        3.创建数据库
        4.指定数据库下建用户，建表，表名含特殊字符
        5.修改信任方式为sha256
        6.导出指定数据库
        7.删除表执行导入
        8.查看导入表
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0075start---')
        ...
        self.db_name = "db_jdbcgsbackup_0075"
        self.user = "u_jdbcgsbackup_0075"
        self.tb_name = '\\"t_jdbcgsbackup_0075*%$\\"'
    ...
    def test_tools_backup(self):
        ...
        sql_cmd = f'''drop table if exists {self.tb_name};
            ...
            insert into {self.tb_name} values (100);
        ...
        text = '---step7:删除表后执行导入;expect:导入成功---'
        self.log.info(text)
        sql_cmd = f'''drop table if exists {self.tb_name};'''
        sql_result = self.pri_sh.execut_db_sql(sql=sql_cmd,
                                               dbname=f'{self.db_name}')
        ...
        text = '---step8:查询表是否导入;expect:表导入成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0075finish---')
