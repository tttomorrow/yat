...
Case Name   : 使用jdbcgsbackup -m dump导出指定数据库，指定-U和-P为普通用户
Description :
        1.创建工具所在目录
...
Expect      :
        ...
        4.在创建的库下建表,建用户成功
        5.修改成功
        6.有报错，但是导出文件有表的DDL；表数据文件为空
        7.导入,报错ERROR: permission denied for schema public
        8.表不存在，未导入成功
        9.清理环境完成
...
class ToolsBackup(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0007start---')
    ...
    def test_tools_backup(self):
        ...
        self.log.info(msg)
        self.assertIn('ERROR: permission denied', msg)
        ...
        msg = self.Root_Node.sh(sql_cmd).result()
        self.log.info(msg)
        self.assertIn('ERROR: permission denied for schema public', msg)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_JdbcGsBackup_Case0007finish---')
