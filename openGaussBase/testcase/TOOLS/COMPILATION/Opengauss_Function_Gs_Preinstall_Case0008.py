...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall命令，不设置参数--skip-hostname-set
Description :
...
Expect      :
    ...
    2.配置成功
    3.成功
    4.预安装成功,xml配置文件中主机名与IP的映射关系未写入“/etc/hosts”文件中
    5.清理成功
History     :
"""
import os
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.u_name = 'u_gs_preinstall_0008'
        path = macro.DB_INSTANCE_PATH
        for i in range(len(path.split('/')) - 2):
            path = os.path.join(path, '..')
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0008')
        ...
        self.host_bk = os.path.join(path, 'hosts_gs_preinstall_bk')
        ...
        msg = self.common.get_sh_result(self.root_node, hosts_cmd)
        self.assertEqual('', msg, '执行失败:' + test)
    ...
    def test_standby(self):
        ...
        test = '-----查看主机名-----'
        self.log.info(test)
        node_name = self.common.get_sh_result(self.root_node,
                                              "uname -n")
        ...
        result = self.common.get_sh_result(self.root_node, xml_cmd)
        ...
        test = '-----step4:在script目录下，执行gs_preinstall命令 ' \
               'expect:预安装成功,xml配置文件中主机名与IP的映射关系未写入“/etc/hosts”文件中-----'
        self.log.info(test)
        preinstall_cmd = f"cd {script_path};" \
            ...
            f"cat {self.host_file}"
        self.log.info(preinstall_cmd)
        msg = self.common.get_sh_result(self.root_node,
                                        preinstall_cmd)
        self.assertIn(self.constant.preinstall_success_msg, msg,
                      '执行失败:' + test)
        self.assertIn(self.constant.preinstall_setos_success_msg, msg,
                      '执行失败:' + test)
        self.assertNotIn(f'{self.node.db_host}  {node_name}',
                         msg, '执行失败:' + test)
    ...
    def tearDown(self):
        ...
        hosts_msg = self.common.get_sh_result(self.root_node, hosts_cmd)
        ...
        self.assertEqual('', hosts_msg, '执行失败:' + text_1)
        self.assertEqual('', udel_msg, '执行失败:' + text_1)
        self.assertEqual('not exists', del_msg, '执行失败:' + text_3)
