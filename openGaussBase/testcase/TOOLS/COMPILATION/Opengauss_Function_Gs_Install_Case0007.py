...
Description :
    ...
    5.gs_install安装数据库：
    gs_install -X [xml配置文件路径] --gsinit-parameter= --xlogdir=[事务日志存储目录]
    6.清理环境
    postuninstall清理数据库：
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Install_Case0007开始执行----')
        self.primary_root_node = Node('PrimaryRoot')
        self.primary_node = Node('PrimaryDbUser')
        self.common = Common()
        path = os.path.dirname(os.path.dirname(macro.DB_INSTANCE_PATH))
        self.openGauss_path = os.path.join(path, 'dir_gs_install_0007')
    ...
    def test_standby(self):
        ...
        self.log.info(text)
        xlogdir = os.path.join(self.openGauss_path, 'xlogdir')
        install_cmd = f"gs_install -X {self.xml_path} " \
            f"--gsinit-parameter= --xlogdir={xlogdir}"
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Install_Case0007执行完成----')
