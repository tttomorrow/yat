...
"""
Case Type   : tools
Case Name   : 初始化数据库gs_initdb:本地用户通过Unix域套接字连接数据库时的认证方法为trust
Description :
    1.执行命令：gs_initdb -D $data/ --nodename=single -w $password --auth=sha256
     --auth-local=trust
Expect      :
    1.在data目录下生成初始化后的文件,pg_hba.conf文件中，local认证方法为trust
History     :
"""
import os
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0040 start----')
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0040 finish---')
