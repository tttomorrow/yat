...
"""
Case Type   : tools
Case Name   : 初始化数据库gs_initdb：本地用户通过TCP/IP连接数据库时的认证方法为reject
Description :
    1.执行命令：gs_initdb -D $data/ --nodename=single -w $password
    --auth-host=reject
Expect      :
    1.在data目录下生成初始化后的文件,pg_hba.conf文件中，host行认证方法为reject
History     :
"""
import os
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0038 start----')
    ...
    def test_systools(self):
        ...
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      ...
                      f' {self.userNode.ssh_password} --auth-host=reject'
        ...
        self.logger.info('--------查看认host证方法为reject--------')
        ...
        self.assertTrue(msg4.count('reject') >= 2)
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0038 finish---')
