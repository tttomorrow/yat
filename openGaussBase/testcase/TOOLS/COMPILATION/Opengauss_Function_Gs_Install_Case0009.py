...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_install命令安装数据库，不指定-l，且不在XML文件中配置
              gaussdbLogPath
Description :
    1.下载解压数据库安装包
...
Expect      :
    ...
    3.成功
    4.预安装成功
    5.安装成功,生成日志/var/log/gaussdb/用户名/om/gs_install-YYYY-MM-DD_hhmmss.log
    6.清理成功
History     :
"""
import os
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Install_Case0009开始执行----')
        self.primary_root_node = Node('PrimaryRoot')
        self.primary_node = Node('PrimaryDbUser')
        self.common = Common()
        path = os.path.dirname(os.path.dirname(macro.DB_INSTANCE_PATH))
        self.openGauss_path = os.path.join(path, 'dir_gs_install_0009')
    ...
    def test_standby(self):
        ...
        text = '----step2:配置xml文件,不配置参数gaussdbLogPath expect:配置成功----'
        self.log.info(text)
        ...
        text = '----step5:执行gs_install命令 expect:安装成功,xml指定路径下生成log文件----'
        self.log.info(text)
        gaussdb_log = os.path.join('/', 'var', 'log', 'gaussdb',
                                   self.primary_node.ssh_user, 'om')
        ...
        self.assertIn(Constant.install_success_msg, exec_msg,
                      '执行失败:' + text)
        ls_cmd = f"ls {gaussdb_log}"
        exec_msg = self.primary_root_node.sh(ls_cmd).result()
        assert1 = re.search(
            r"gs_install-[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{6}.log", exec_msg)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Install_Case0009执行完成----')
