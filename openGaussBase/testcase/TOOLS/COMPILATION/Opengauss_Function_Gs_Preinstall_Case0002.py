...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall命令，用户为数字开头+字母
Description :
    1.拷贝script文件,创建数据库相关文件夹
    2.配置xml文件
    3.创建初始用户0u_gs_preinstall_0002并修改文件属组
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.u_name = '0u_gs_preinstall_0002'
        path = macro.DB_INSTANCE_PATH
        for i in range(len(path.split('/')) - 2):
            path = os.path.join(path, '..')
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0002')
    ...
    def test_standby(self):
        ...
        text = '-----step4:在script目录下，执行gs_preinstall命令 expect:预安装成功-----'
        self.log.info(text)
