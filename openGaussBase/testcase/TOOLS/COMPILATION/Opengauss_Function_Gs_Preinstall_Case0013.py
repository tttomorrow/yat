...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall -V查看版本
Description :
    1.拷贝script文件,创建数据库相关文件夹
    2.在script目录下，执行gs_preinstall命令：./gs_preinstall -V
    3.清理环境
Expect      :
    1.成功
    2.返回版本信息
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0013')
    ...
    def test_standby(self):
        ...
        step = '----step2:在script目录下，执行gs_preinstall命令 expect:返回版本信息----'
        self.log.info(step)
        ...
        self.log.info(msg)
        self.assertTrue(msg.find('compiled at') > -1, '执行失败:' + step)
