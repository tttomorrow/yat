...
"""
Case Type   : tools
Case Name   : 执行初始化数据库命令gs_initdb：指定参数--pwpasswd，初始化中无需手动输入密码
Description :
    1.执行命令：gs_initdb -D $data/ --nodename=single  --pwpasswd=*****
Expect      :
    1.初始化成功，初始化过程中无需手动输入密码
History     :
"""
import os
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0026 start----')
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0026 finish---')
