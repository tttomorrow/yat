...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行初始化数据库命令gs_initdb：本地用户连接数据库时的认证方法为md5
Description :
    1.执行命令：
    gs_initdb -D [初始化数据库目录] --nodename=single -W --auth=md5，需输入admin用户密码
    2.清理环境
    删除[初始化数据库目录]：rm -rf [初始化数据库目录]
Expect      :
    1.在[初始化数据库目录]下生成初始化后的文件,pg_hba.conf文件中，认证方法为md5
    2.清理成功
History     :
"""
import os
...
class Tools(unittest.TestCase):
    ...
    def test_standby(self):
        ...
        step_txt = '----step1:执行gs_initdb命令 expect:在[初始化数据库目录] ' \
                   '下生成初始化后的文件,pg_hba.conf文件中，认证方法为md5----'
        ...
        self.assertTrue(pg_msg.count('md5') >= 2, '执行失败:' + step_txt)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Initdb_Case0007执行完成----')
