...
"""
Case Type   : tools
Case Name   : 执行初始化数据库命令gs_initdb：系统默认区域的编码格式和用--encoding参数
                指定的编码格式匹配
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0010 start----')
    ...
    def test_systools(self):
        ...
        self.assertTrue(msg1.find('Success. You can now start the database '
                                  'server of single node') > -1)
        ...
        self.logger.info('--------查看到编码格式--------')
        excute_cmd3 = f'locale|grep LC_CTYP'
        self.logger.info(excute_cmd3)
        msg3 = self.userNode.sh(excute_cmd3).result()
        self.logger.info(msg3)
        self.assertTrue(msg3.find('en_US.UTF-8') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0010 finish---')
