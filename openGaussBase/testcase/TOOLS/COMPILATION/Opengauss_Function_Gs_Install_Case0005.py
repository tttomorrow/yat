...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_install命令安装数据库，指定参数--gsinit-parameter
              =--username=autotest
Description :
    1.下载解压数据库安装包
    2.配置xml文件
    ...
    5.gs_install安装数据库：
    gs_install -X [xml配置文件路径] --gsinit-parameter=--username=[初始用户]
    6.清理环境
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Install_Case0005开始执行----')
        self.primary_root_node = Node('PrimaryRoot')
        self.primary_node = Node('PrimaryDbUser')
        self.common = Common()
        path = os.path.dirname(os.path.dirname(macro.DB_INSTANCE_PATH))
        self.openGauss_path = os.path.join(path, 'dir_gs_install_0005')
    ...
    def test_standby(self):
        ...
        text = '----step5:执行gs_install命令 expect:安装失败----'
        self.log.info(text)
        install_cmd = f"gs_install -X {self.xml_path} " \
            f"--gsinit-parameter=--username={self.primary_node.ssh_user}"
        exec_msg = self.common.do_install(self.primary_node, self.env_path,
                                          install_cmd)
        self.assertIn('GAUSS-50000', exec_msg, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        uninstall_cmd = f"source {self.env_path};" \
            f"{self.script_path}/gs_uninstall --delete-data"
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Install_Case0005执行完成----')
