...
"""
Case Type   : tools
Case Name   : 初始化数据库gs_initdb:本地用户通过Unix域套接字连接数据库的认证方法为sha256
Description :
    1.执行命令：gs_initdb -D $data/ --nodename=single -w $password
    --auth-local=sha256
Expect      :
    1.在data目录下生成初始化后的文件,pg_hba.conf文件中，local认证方法为sha256
History     :
"""
import os
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0041 start----')
    ...
    def test_systools(self):
        ...
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      ...
                      f' {self.userNode.ssh_password} --auth-local=sha256'
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0041 finish---')
