...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall命令，使用环境变量分离的版本，指定参数--sep-env-file=ENVFILE
Description :
...
Expect      :
    ...
    2.配置成功
    3.成功
    4.预安装成功,在[env文件路径]生成环境变量文件env,且内容无残缺
    5.清理成功
History     :
"""
import os
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.u_name = 'u_gs_preinstall_0020'
        path = macro.DB_INSTANCE_PATH
        for i in range(len(path.split('/')) - 2):
            path = os.path.join(path, '..')
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0020')
        ...
        self.env_path = os.path.join(self.conf_path, '0020_env')
    ...
    def test_standby(self):
        ...
        text = '-----step4:在script目录下，执行gs_preinstall命令 ' \
               'expect:预安装成功,指定路径生成环境变量文件,且内容完整-----'
        self.log.info(text)
        ...
        self.assertIn(self.constant.preinstall_success_msg, msg,
                      '执行失败:' + text)
        self.assertIn('0020_env', msg, '执行失败:' + text)
        msg_list = ['MPPDB_ENV_SEPARATE_PATH', 'GPHOME',
                    'PATH=$GPHOME/script/gspylib/pssh/bin',
                    'LD_LIBRARY_PATH=$GPHOME/lib:$LD_LIBRARY_PATH',
                    'PYTHONPATH=$GPHOME/lib', 'GAUSSHOME',
                    'PATH=$GAUSSHOME/bin:$PATH',
                    'LD_LIBRARY_PATH=$GAUSSHOME/lib:$LD_LIBRARY_PATH',
                    'S3_CLIENT_CRT_FILE=$GAUSSHOME/lib/client.crt',
                    'GAUSS_VERSION', 'PGHOST', 'GAUSSLOG', 'umask 077',
                    'GAUSS_ENV=1']
        for content in msg_list:
            self.assertTrue(msg.find(content) > -1, '执行失败:' + text)
