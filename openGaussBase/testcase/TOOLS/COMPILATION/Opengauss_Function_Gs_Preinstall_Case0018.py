...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall命令，不明确指定-l，在XML文件中配置了gaussdbLogPath
Description :
...
Expect      :
    ...
    2.配置成功
    3.创建成功
    4.预安装成功,在xml配置的在XML文件中配置了gaussdbLogPath路径生成以"gs_preinstall"+时间戳+".log"命名的日志文件
    5.清理成功
History     :
"""
import os
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.u_name = 'u_gs_preinstall_0018'
        path = macro.DB_INSTANCE_PATH
        for i in range(len(path.split('/')) - 2):
            path = os.path.join(path, '..')
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0018')
    ...
    def test_standby(self):
        ...
        text = '-----step3:配置xml文件 expect:配置成功-----'
        self.log.info(text)
        ...
        self.log.info(text)
        gaussdb_log = os.path.join(self.data_path,
                                   'gaussdb_log', self.u_name, 'om')
