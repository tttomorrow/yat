...
Case Name   : 执行初始化数据库命令gs_initdb：指定参数--pwfile，
                指定的文件第一行为正确的系统管理员密码
Description :
    1.在指定路径下创建一个文件，命名为wf.txt(自定义)，文件第一行为数据库初始用户密码，
    执行命令：gs_initdb -D $data1/ --nodename=single  --pwfile=$wf.txt
Expect      :
    1.初始化成功，且在初始化过程中无需输入密码，系统会自动读取文件中的密码
History     :
"""
import os
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0021 start----')
    ...
    def test_systools(self):
        ...
        excute_cmd0 = f'touch {self.file};' \
                      f'echo "{self.userNode.ssh_password}" >> {self.file};'
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0021 finish---')
