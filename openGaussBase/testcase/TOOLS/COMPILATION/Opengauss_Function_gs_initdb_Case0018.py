...
"""
Case Type   : tools
Case Name   : 执行初始化数据库命令gs_initdb：指定参数--lc-ctype
Description :
    1.执行命令：gs_initdb -D $data/ --nodename=single -w $passwd
    --lc-ctype=yo_NG.utf8
Expect      :
    1.初始化成功，打印CTYPE:    yo_NG.utf8信息,在data目录下生成初始化后的文件
History     :
"""
import os
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0018 start----')
    ...
    def test_systools(self):
        ...
        excute_cmd1 = f'source {self.DB_ENV_PATH};gs_initdb -D ' \
                      ...
                      f'{self.userNode.ssh_password} --lc-ctype=yo_NG.utf8'
        self.logger.info(excute_cmd1)
        msg1 = self.userNode.sh(excute_cmd1).result()
        self.logger.info(msg1)
        self.assertTrue(
            msg1.find('Success. You can now start the database server') > -1)
        self.assertTrue(msg1.find('CTYPE:    yo_NG.utf8') > -1)
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0018 finish---')
