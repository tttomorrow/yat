...
"""
Case Type   : tools
Case Name   : 初始化数据库gs_initdb：本地用户通过Unix域套接字连接数据库时的认证方法为md5
Description :
    1.执行命令：gs_initdb -D $data/ --nodename=single -w $password
    --auth-local=md5
Expect      :
    1.在data目录下生成初始化后的文件,pg_hba.conf文件中，local认证方法为md5
History     :
"""
import os
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0042 start----')
    ...
    def test_systools(self):
        ...
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      ...
                      f' {self.userNode.ssh_password} --auth-local=md5'
        ...
        self.logger.info('--------查看认local证方法为md5--------')
        pg_file = os.path.join(self.dir_path, 'pg_hba.conf')
        excute_cmd4 = f'cat {pg_file} |grep "local"'
        self.logger.info(excute_cmd4)
        msg4 = self.userNode.sh(excute_cmd4).result()
        self.logger.info(msg4)
        self.assertTrue(msg4.count('md5') >= 1)
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0042 finish---')
