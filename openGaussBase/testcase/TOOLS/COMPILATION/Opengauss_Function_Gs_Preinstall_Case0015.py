...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall命令，指定日志文件名及路径
Description :
    ...
    --sep-env-file=[env文件路径]  --non-interactive  -l [指定日志路径]/[文件名]
    --skip-hostname-set
    5.清理环境
    删除数据库相关目录：
    rm -rf [数据库相关目录]
Expect      :
    ...
    2.配置成功
    3.成功
    4.预安装成功,生成以"[指定文件名]"+时间戳+".log"命名的日志文件
    5.清理成功
History     :
"""
import os
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.u_name = 'u_gs_preinstall_0015'
        path = macro.DB_INSTANCE_PATH
        for i in range(len(path.split('/')) - 2):
            path = os.path.join(path, '..')
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0015')
    ...
    def test_standby(self):
        ...
        self.assertIn(self.constant.preinstall_success_msg, msg,
                      '执行失败:' + text)
        assert1 = re.search(
            r"preinstall_0015-[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{6}.log", msg)
