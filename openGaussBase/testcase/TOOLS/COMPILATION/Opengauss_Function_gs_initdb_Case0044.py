...
"""
Case Type   : tools
Case Name   : 初始化数据库gs_initdb:指定参数---auth-loca，会覆盖指定参数--auth生成的值
Description :
    1.执行命令：gs_initdb -D $data/ --nodename=single -w $password --auth=sha256
    --auth-local=reject
Expect      :
    1.在data目录下生成初始化后的文件,pg_hba.conf文件中，local认证方法为reject
History     :
"""
import os
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('----Opengauss_Function_gs_initdb_Case0043 start----')
    ...
    def test_systools(self):
        ...
        self.logger.info('--------查看认host证方法为sha256--------')
        ...
        self.assertTrue(msg4.count('sha256') >= 2)
        self.logger.info('--------查看认local证方法为reject--------')
        pg_file = os.path.join(self.dir_path, 'pg_hba.conf')
        excute_cmd4 = f'cat {pg_file} |grep "local"'
        self.logger.info(excute_cmd4)
        msg4 = self.userNode.sh(excute_cmd4).result()
        self.logger.info(msg4)
        self.assertTrue(msg4.count('reject') >= 1)
    ...
    def tearDown(self):
        ...
        self.logger.info('---Opengauss_Function_gs_initdb_Case0043 finish---')
