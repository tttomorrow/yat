...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_install命令安装数据库，不指定-l，在XML文件中配置
              gaussdbLogPath
Description :
    1.下载解压数据库安装包
    2.配置xml文件,配置gaussdbLogPath
...
Expect      :
    ...
    3.成功
    4.预安装成功
    5.安装成功,在xml配置的在XML文件中配置了gaussdbLogPath路径生成以"gs_install"+时间戳+".log"命名的日志文件
    6.清理成功
History     :
"""
import os
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Install_Case0008开始执行----')
        self.primary_root_node = Node('PrimaryRoot')
        self.primary_node = Node('PrimaryDbUser')
        self.common = Common()
        path = os.path.dirname(os.path.dirname(macro.DB_INSTANCE_PATH))
        self.openGauss_path = os.path.join(path, 'dir_gs_install_0008')
    ...
    def test_standby(self):
        ...
        self.log.info(text)
        gaussdb_log = os.path.join(gaussdb_log_path,
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Install_Case0008执行完成----')
