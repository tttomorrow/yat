...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_install命令安装数据库，指定告警上报组件的绝对路径，
              指定参数--alarm-component=alarm_component
Description :
    1.下载解压数据库安装包
    2.配置xml文件
    ...
    5.gs_install安装数据库：
    gs_install -X [xml配置文件路径] --alarm-component=[自定义路径]
    6.清理环境
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Install_Case0002开始执行----')
        self.primary_root_node = Node('PrimaryRoot')
        self.primary_node = Node('PrimaryDbUser')
        self.common = Common()
        path = os.path.dirname(os.path.dirname(macro.DB_INSTANCE_PATH))
        self.openGauss_path = os.path.join(path, 'dir_gs_install_0002')
    ...
    def test_standby(self):
        ...
        self.log.info(text)
        alarm_path = os.path.join(self.openGauss_path, 'alarm')
        install_cmd = f"gs_install -X {self.xml_path} " \
            f"--alarm-component={alarm_path}"
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Install_Case0002执行完成----')
