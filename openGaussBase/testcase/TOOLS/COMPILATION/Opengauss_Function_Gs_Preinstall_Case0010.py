...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall命令，设置参数--non-interactive
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.node = Node('PrimaryDbUser')
        self.common = Common()
        self.u_name = 'u_gs_preinstall_0010'
        path = macro.DB_INSTANCE_PATH
        for i in range(len(path.split('/')) - 2):
            path = os.path.join(path, '..')
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0010')
    ...
    def test_standby(self):
        ...
        xml_cmd = f"sed -i 's#v_nodeNames#{pri_node_name}#g' {self.xml_path};" \
