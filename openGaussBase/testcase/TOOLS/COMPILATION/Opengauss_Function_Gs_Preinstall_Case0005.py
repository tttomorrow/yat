...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall命令，用户为特殊字符
Description :
    1.拷贝script文件,创建数据库相关文件夹
    2.配置xml文件
    3.创建初始用户_u_gs_preinstall_0005并修改文件属组
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.u_name = '_u_gs_preinstall_0005'
        path = macro.DB_INSTANCE_PATH
        for i in range(len(path.split('/')) - 2):
            path = os.path.join(path, '..')
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0005')
