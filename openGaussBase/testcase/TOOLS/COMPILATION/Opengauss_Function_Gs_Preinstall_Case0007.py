...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_preinstall命令，不设置操作系统参数--skip-os-set
Description :
...
Expect      :
    ...
    2.配置成功
    3.成功
    4.预安装成功,打印内容包含Setting OS parameters
    5.清理成功
History     :
"""
import os
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.u_name = 'u_gs_preinstall_0007'
        path = macro.DB_INSTANCE_PATH
        for i in range(len(path.split('/')) - 2):
            path = os.path.join(path, '..')
        self.openGauss_path = os.path.join(path, 'dir_gs_preinstall_0007')
    ...
    def test_standby(self):
        ...
        text = '-----step4:在script目录下，执行gs_preinstall命令 ' \
               'expect:预安装成功,打印内容包含Setting OS parameters-----'
        self.log.info(text)
