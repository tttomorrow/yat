...
"""
Case Type   : 服务端工具
Case Name   : 导出包含模板库的数据并将函数体前的$去掉
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.导出包含模板库的数据并将函数体前的$去掉
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0097start----')
    ...
    def test_server_tools(self):
        ...
        self.assertIn("AS '", dumpall_msg)

    def tearDown(self):
        LOG.info('---清理环境---')
        sql_cmd2 = 'drop function  abort;'
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0097finish----')
