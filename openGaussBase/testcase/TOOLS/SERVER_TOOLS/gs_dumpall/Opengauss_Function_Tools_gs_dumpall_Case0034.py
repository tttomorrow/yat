...
"""
Case Type   : 服务端工具
Case Name   : 转储数据需用AES128进行加密，指定密钥不符合规则
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.转储数据需用AES128进行加密，指定密钥不符合规则
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0034start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-----转储数据需用AES128进行加密，指定密钥不符合规则-----')
        excute_cmd2 = f'''source {macro.DB_ENV_PATH} ;
                gs_dumpall  -p {self.dbuser_node.db_port}\
                --with-encryption=AES128 --with-key=126@数据库wsq;
                              '''
        LOG.info(excute_cmd2)
        msg2 = self.dbuser_node.sh(excute_cmd2).result()
        LOG.info(msg2)
        self.assertIn('The key is illegal, must be letters or numbers', msg2)
    ...
    def tearDown(self):
        ...
        LOG.info(msg3)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0034finish----')
