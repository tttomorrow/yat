...
"""
Case Type   : 服务端工具
Case Name   : 导出数据需用AES128进行加密，指定密钥不符合规则
Description :
    1.创建数据
    2.导出数据需用AES128进行加密，指定密钥不符合规则
    3.清理环境
Expect      :
    1.创建数据成功
    2.导出失败
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools(self):
        ...
        text = '----step2:导出数据需用AES128进行加密，指定密钥;expect:导出失败----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, clear_msg,
                      '执行失败:' + text)
        self.log.info(
            '----Opengauss_Function_Tools_gs_dumpall_Case0034_finish----')
