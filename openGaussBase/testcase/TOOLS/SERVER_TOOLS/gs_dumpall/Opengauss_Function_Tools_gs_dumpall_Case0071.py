...
"""
Case Type   : 服务端工具
Case Name   : 将角色和表空间相关数据导出到指定文件
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.将角色和表空间相关数据导出到指定文件
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0071start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info(cat_cmd)
        msg3 = self.dbuser_node.sh(cat_cmd).result()
        LOG.info(msg3)
        self.assertIn('CREATE ROLE manager', msg3)
        self.assertIn('CREATE TABLESPACE tbspace', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0071finish----')
