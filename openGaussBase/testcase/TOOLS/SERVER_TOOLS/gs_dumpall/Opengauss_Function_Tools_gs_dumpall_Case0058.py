...
"""
Case Type   : 服务端工具
Case Name   : 指定备份进程并发数（取值不在正常范围内）
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.指定备份进程并发数（取值不在正常范围内）
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0058start----')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.gs_dumpall_error_msg = 'gs_dumpall: options --parallel-jobs ' \
    ...
    def test_server_tools(self):
        ...
        LOG.info('------指定备份进程并发数（取值不在正常范围内）-----')
        dumpall_cmd = f'''source {macro.DB_ENV_PATH} ;
            gs_dumpall -p {self.dbuser_node.db_port} --parallel-jobs 10008\
            -f {macro.DB_INSTANCE_PATH}/dumpall_qm;
            '''
        LOG.info(dumpall_cmd)
        dumpall_msg = self.dbuser_node.sh(dumpall_cmd).result()
        LOG.info(dumpall_msg)
        self.assertIn(self.gs_dumpall_error_msg, dumpall_msg)


    def tearDown(self):
        LOG.info('---清理环境---')
        clear_msg = '''drop table if exists t1; 
            drop table if exists test1; 
            '''
        clear_cmd = f'''source {macro.DB_ENV_PATH} ;
            gsql -d {self.dbuser_node.db_name}\
            -p {self.dbuser_node.db_port} -c "{clear_msg}";
            rm -rf {macro.DB_INSTANCE_PATH}/dumpall_qm;
            '''
        LOG.info(clear_cmd)
        clear_msg = self.dbuser_node.sh(clear_cmd).result()
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0058finish----')
