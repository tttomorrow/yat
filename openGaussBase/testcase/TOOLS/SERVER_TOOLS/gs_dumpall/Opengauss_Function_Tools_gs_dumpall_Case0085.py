...
"""
Case Type   : 服务端工具
Case Name   : 指定转储使用的角色名并将结果输出到指定的文件
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.指定转储使用的角色名并将结果输出到指定的文件
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0085start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--指定转储使用的角色名并将结果输出到指定的文件--')
    ...
    def tearDown(self):
        LOG.info('---清理环境---')
        sql_cmd2 = f'drop table if exists test1;' \
            f'drop user if exists user1;' \
            f'drop user if exists user2;'
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0085finish----')
