...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0048start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--在导出的表空间指令前添加清理指令--')
        dumpall_cmd = f'''source {macro.DB_ENV_PATH} ;
            gs_dumpall -p {self.dbuser_node.db_port}  -t -c;
            '''
        ...
        self.assertIn('DROP TABLESPACE IF EXISTS tbspace;', dumpall_msg)
        self.assertIn('CREATE TABLESPACE tbspace', dumpall_msg)
    ...
    def tearDown(self):
        ...
        clear_cmd = f'''source {macro.DB_ENV_PATH} ;
            ...
            '''
        LOG.info(clear_cmd)
        sql_msg1 = self.dbuser_node.sh(clear_cmd).result()
        LOG.info(sql_msg1)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0048finish----')
