...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0055start----')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.gs_dumpall_assert_msg = "dump database " \
                                     "dbname='template1' successfully"
    ...
    def test_server_tools(self):
        ...
        self.assertIn(self.gs_dumpall_assert_msg, dumpall_msg)
        self.assertIn('COPY t1 (id, name)', dumpall_msg)


    def tearDown(self):
        LOG.info('---清理环境---')
        clear_msg = '''drop function if exists abort;
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0055finish----')
