...
"""
Case Type   : 服务端工具
Case Name   : 在转储过程中使用的系统管理员名称导出数据
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.在转储过程中使用的系统管理员名称导出数据
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0016start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------在转储过程中使用的系统管理员名称导出数据-----')
        excute_cmd2 = f'''
                source {macro.DB_ENV_PATH} ;
                gs_dumpall -p {self.dbuser_node.db_port} \
                -S {self.dbuser_node.ssh_user};
                          '''
        LOG.info(excute_cmd2)
        msg2 = self.dbuser_node.sh(excute_cmd2).result()
        LOG.info(msg2)
        self.assertIn(self.constant.gs_dumpall_success_msg, msg2)
        self.assertIn('CREATE TABLE test1', msg2)
        self.assertIn('CREATE SCHEMA schema1', msg2)
        self.assertIn('CREATE DATABASE db_test', msg2)
        self.assertIn('CREATE TABLESPACE tbspace', msg2)
    ...
    def tearDown(self):
        ...
        LOG.info(msg3)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0016finish----')
