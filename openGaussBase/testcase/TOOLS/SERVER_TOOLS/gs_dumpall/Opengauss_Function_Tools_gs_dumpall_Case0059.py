...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0059start----')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.gs_dumpall_msg = "gs_dumpall: unrecognized option '--format=c'"
    ...
    def test_server_tools(self):
        ...
        dumpall_cmd = f'''source {macro.DB_ENV_PATH} ;
            gs_dumpall -p {self.dbuser_node.db_port}  --format=c\
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0059finish----')
