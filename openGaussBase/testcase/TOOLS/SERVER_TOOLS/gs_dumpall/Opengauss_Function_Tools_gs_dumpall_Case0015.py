...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0015start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------只转储对象定义（模式），而非数据-----')
        excute_cmd2 = f'''
                source {macro.DB_ENV_PATH} ;
                gs_dumpall -p {self.dbuser_node.db_port} -s;
                          '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg3)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0015finish----')
