...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0025start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------参数-r和-t不可同时使用-----')
        excute_cmd2 = f'''
                source {macro.DB_ENV_PATH} ;
                gs_dumpall -p {self.dbuser_node.db_port}  -r -t;
                          '''
        LOG.info(excute_cmd2)
        msg2 = self.dbuser_node.sh(excute_cmd2).result()
        LOG.info(msg2)
        self.assertIn('gs_dumpall: options -r/--roles-only and \
-t/--tablespaces-only cannot be used together', msg2)
    ...
    def tearDown(self):
        ...
        LOG.info(msg3)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0025finish----')
