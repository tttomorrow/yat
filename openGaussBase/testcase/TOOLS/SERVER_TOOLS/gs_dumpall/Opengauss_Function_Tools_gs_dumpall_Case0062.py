...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0062start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--使用指定的用户名连接（有密码提示）--')
        dumpall_cmd = f'''source {macro.DB_ENV_PATH} ;
            expect <<EOF
            set timeout -1
            spawn gs_dumpall -p {self.dbuser_node.db_port} -U\
            {self.dbuser_node.ssh_user};
            expect "Password:"
            send "{self.dbuser_node.ssh_password}\r"
            expect eof \n''' + "EOF"
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0062finish----')
