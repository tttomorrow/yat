...
"""
Case Type   : 服务端工具
Case Name   : --binary-upgrade-usermap单独使用
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.--binary-upgrade-usermap单独使用
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0039start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--------binary-upgrade-usermap单独使用-----')
        excute_cmd2 = f'''
        ...
        LOG.info(msg2)
        self.assertIn('gs_dumpall: options --binary-upgrade-usermap should \
be used with --binary-upgrade option', msg2)
    ...
    def tearDown(self):
        ...
        LOG.info(msg3)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0039finish----')
