...
"""
Case Type   : 服务端工具
Case Name   : 只转储全局对象（角色和表空间）的定义
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.只转储全局对象（角色和表空间）的定义
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0046start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--只转储全局对象（角色和表空间）的定义--')
        dumpall_cmd = f'''source {macro.DB_ENV_PATH} ;
            gs_dumpall -p {self.dbuser_node.db_port}  -g -s;
            '''
        ...
        self.assertIn('CREATE ROLE manager', dumpall_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0046finish----')
