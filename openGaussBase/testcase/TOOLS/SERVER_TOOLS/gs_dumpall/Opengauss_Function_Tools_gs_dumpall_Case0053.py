...
"""
Case Type   : 服务端工具
Case Name   : 导出的数据去掉函数体前面的$符,标识符加引号
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.导出的数据去掉函数体前面的$符,标识符加引号
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0053start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------导出的数据去掉函数体前面的$符,标识符加引号-----')
        dumpall_cmd = f'''source {macro.DB_ENV_PATH} ;
            gs_dumpall -p {self.dbuser_node.db_port} \
            --disable-dollar-quoting --quote-all-identifiers;
            '''
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0053finish----')
