...
"""
Case Type   : 服务端工具
Case Name   : 只导相关转储对象的定义,不以INSERT命令带列名方式导出数据,最终将结果输出到指定的文件
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.只导相关转储对象的定义,不以INSERT命令带列名方式导出数据,最终将结果输出到指定的文件
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0086start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--只导相关转储对象的定义,不以INSERT命令带列名方式导出数据,最终将结果输出到指定的文件--')
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0086finish----')
