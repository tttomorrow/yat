...
"""
Case Type   : 服务端工具
Case Name   : 备份进程并发数在正常取值范围内
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.备份进程并发数在正常取值范围内是否可以导出数据
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0042start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------备份进程并发数在正常取值范围内：500-----')
        dumpall_cmd = f'''source {macro.DB_ENV_PATH} ;
            gs_dumpall -p {self.dbuser_node.db_port} --parallel-jobs 1\
        ...
        cat_cmd = f"cat {macro.DB_INSTANCE_PATH}" \
            f"/dumpall_qm_{self.dbuser_node.db_name};"
        cat_msg = self.dbuser_node.sh(cat_cmd).result()
        LOG.info(cat_msg)
        self.assertIn('CREATE TABLE t1', cat_msg)
        self.assertIn('COPY t1', cat_msg)
    ...
    def tearDown(self):
        ...
        clear_cmd = f'''source {macro.DB_ENV_PATH} ;
            ...
            rm -rf {macro.DB_INSTANCE_PATH}/dumpall*
            '''
        LOG.info(clear_cmd)
        clear_msg = self.dbuser_node.sh(clear_cmd).result()
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0042finish----')
