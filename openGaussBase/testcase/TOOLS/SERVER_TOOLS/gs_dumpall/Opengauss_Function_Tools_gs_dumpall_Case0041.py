...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0041start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------parallel-jobs单独使用-----')
        excute_cmd2 = f'''
        source {macro.DB_ENV_PATH} ;
        gs_dumpall -p {self.dbuser_node.db_port} --parallel-jobs 3;
                          '''
        LOG.info(excute_cmd2)
        msg2 = self.dbuser_node.sh(excute_cmd2).result()
        LOG.info(msg2)
        self.assertIn('gs_dumpall: options --parallel-jobs should be \
used with -f/--file option', msg2)
    ...
    def tearDown(self):
        ...
        LOG.info(msg3)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0041finish----')
