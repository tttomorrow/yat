...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0095start----')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.dumpall_msg1 = "dump database dbname='template1' successfully"
        self.dumpall_msg2 = "dump database dbname='dbtest' successfully"
        self.dumpall_msg3 = "CREATE TABLESPACE tbspace"
        self.dumpall_msg4 = "CREATE ROLE manager"
        self.dumpall_msg5 = "CREATE TABLE test1"
        self.dumpall_msg6 = "COPY test1 (id, name) FROM stdin"

    def test_server_tools(self):
        LOG.info('------------创建数据-------------')
        sql_cmd1 = '''drop database if exists dbtest; 
        ...
        sql_cmd1 = '''drop table if exists test1; 
            ...
            '''
        gsql_cmd1 = f'''source {macro.DB_ENV_PATH} ;
            gsql -d dbtest -p {self.dbuser_node.db_port} -c "{sql_cmd1}"
            '''
        LOG.info(gsql_cmd1)
        sql_msg1 = self.dbuser_node.sh(gsql_cmd1).result()
        LOG.info(sql_msg1)
        ...
        self.assertIn(self.dumpall_msg1, dumpall_msg)
        self.assertIn(self.dumpall_msg2, dumpall_msg)
        self.assertIn(self.dumpall_msg3, dumpall_msg)
        self.assertIn(self.dumpall_msg4, dumpall_msg)
        self.assertIn(self.dumpall_msg5, dumpall_msg)
        self.assertIn(self.dumpall_msg6, dumpall_msg)

    def tearDown(self):
        LOG.info('---清理环境---')
        sql_cmd2 = f'drop database  dbtest;' \
            f'drop tablespace  tbspace;'
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0095finish----')
