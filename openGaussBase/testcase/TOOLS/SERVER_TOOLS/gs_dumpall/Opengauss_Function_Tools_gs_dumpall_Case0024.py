...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0024start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------参数-r和-a不可同时使用-----')
        excute_cmd2 = f'''
                source {macro.DB_ENV_PATH} ;
                gs_dumpall -p {self.dbuser_node.db_port}  -r -a;
                          '''
...
-a/--data-only cannot be used together', msg2)
    ...
    def tearDown(self):
        ...
        LOG.info(msg3)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0024finish----')
