...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0082start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--对导出的数据进行加密设置并将结果输出到指定的文件--')
        ...
        du_cmd = f'du -h {macro.DB_INSTANCE_PATH}/dumpall.sql;'
        LOG.info(du_cmd)
        du_msg = self.dbuser_node.sh(du_cmd).result()
        LOG.info(du_msg)
        du_msg_list = du_msg.split()[0]
        self.assertTrue(float(du_msg_list[:-1]) > 0)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0082finish----')
