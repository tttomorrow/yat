...
"""
Case Type   : 服务端工具
Case Name   : 以INSERT命令带列名方式导出数据
Description :
    1.连接数据库：
    2.创建表并插入数据
    3.退出数据库
    4.source环境变量
    5.以INSERT命令带列名方式导出数据
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建表并插入数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0051start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------以INSERT命令带列名方式导出数据-----')
        dumpall_cmd = f'''source {macro.DB_ENV_PATH} ;
            gs_dumpall -p {self.dbuser_node.db_port} -a --column-inserts;
            '''
        ...
        self.assertIn("INSERT INTO test1 (id, name) VALUES", dumpall_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0051finish----')
