...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0094start----')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.dumpall_msg1 = "dump database dbname='template1' successfully"
        self.dumpall_msg2 = "DROP TABLESPACE IF EXISTS tbspace"
        self.dumpall_msg3 = "DROP ROLE IF EXISTS manager"
    ...
    def test_server_tools(self):
        ...
        LOG.info(cat_msg)
        self.assertIn(self.dumpall_msg2, cat_msg)
        self.assertIn(self.dumpall_msg3, cat_msg)
        self.assertIn(self.dumpall_msg4, cat_msg)
        self.assertIn(self.dumpall_msg5, cat_msg)
        self.assertIn(self.dumpall_msg6, cat_msg)

    def tearDown(self):
        LOG.info('---清理环境---')
        sql_cmd2 = f'drop table  test1;' \
            f'drop role if exists manager; ' \
            f'drop tablespace if exists tbspace;'
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0094finish----')
