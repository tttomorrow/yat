...
"""
Case Type   : 服务端工具
Case Name   : 导出指定的数据库并将结果输出到指定的文件
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.导出指定的数据库并将结果输出到指定的文件
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0083start----')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()

    def test_server_tools(self):
        LOG.info('------------创建数据库-------------')
        sql_cmd = '''drop database if exists dbtest; 
            create database  dbtest;
            '''
        gsql_cmd = f'''source {macro.DB_ENV_PATH} ;
            gsql -d {self.dbuser_node.db_name} -p\
            {self.dbuser_node.db_port} -c "{sql_cmd}"
            '''
        LOG.info(gsql_cmd)
        sql_msg = self.dbuser_node.sh(gsql_cmd).result()
        LOG.info(sql_msg)
        self.assertIn(self.constant.CREATE_DATABASE_SUCCESS, sql_msg)
        LOG.info('------------创建数据-------------')
        ...
        LOG.info('--导出指定的数据库并将结果输出到指定的文件--')
        self.dumpall_msg = "dump database dbname='dbtest' successfully"
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0083finish----')
