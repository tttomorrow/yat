...
"""
Case Type   : 服务端工具
Case Name   : 添加参数--binary-upgrade是否可以导出数据
Description :
    1.连接数据库：
    2.创建数据
    3.退出数据库
    4.source环境变量
    5.添加参数--binary-upgrade是否可以导出数据
    6.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0038start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--添加参数--binary-upgrade是否可以导出数据--')
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('----Opengauss_Function_Tools_gs_dumpall_Case0038finish----')
