...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数--vector，--key-base64加密明文字符串
Description :
    1.生成以base64方式编码的密钥；
      base64.b64encode([用户输入的口令].encode(encoding='utf-8'))
    2.执行命令，组合使用参数--vector，--key-base64加密明文字符串
      gs_encrypt --vector=[用户输入的盐值] --key-base64=
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令组合使用参数--vector，--key-base64加密明文字符串" \
               "except:执行成功，输出加密的密文"
        self.logger.info(step)
        key_spl = str(key).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --vector={vec} --key-base64={key_spl}\
            {str1}'''
