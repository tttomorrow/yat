...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数--vector，--file-prefix,--key-base64
            加密明文字符串
Description :
    1.生成以base64方式编码的密钥；
    ...
    3.执行命令，组合使用参数--vector，--file-prefix,--key-base64加密明文字符串
      gs_encrypt --vector=[用户输入的盐值] --file-prefix=
      [用户指定的加密文件前缀名]  --key-base64=[用户输入口令编码后的密钥]
      [待加密明文字符]
    4.清理环境
Expect      :
    1.成功生成密钥；
    2.成功生成文件；
    3.执行失败，报错提示不能同时使用密钥，盐值和加密文件进行加密
    4.清理环境成功
History     :
"""
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1:生成以base64方式编码的密钥   except:成功生成密钥"
        self.logger.info(step)
        encrypt = '12gg##GG'
        vec = '5555555gggggGGGG'
        ...
        step = "step3:执行命令组合使用参数--vector，--file-prefix," \
               "--key-base64加密明文字符串 except:step3执行失败，" \
               "报错提示不能同时使用密钥盐值和加密文件进行加密"
        self.logger.info(step)
        self.key_spl = str(key).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --vector={vec} --file-prefix={self.prefix} \
            --key-base64={self.key_spl} {self.prefix}'''
        self.logger.info(check_cmd1)
