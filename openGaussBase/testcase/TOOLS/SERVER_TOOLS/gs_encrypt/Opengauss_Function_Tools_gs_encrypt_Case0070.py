...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数--key，--key-base64加密明文字符串
Description :
    1.生成以base64方式编码的密钥；
      base64.b64encode([用户输入的口令].encode(encoding='utf-8'))
    2.执行命令，组合使用参数--key，--key-base64加密明文字符串
      gs_encrypt --key=[用户输入的口令] --key-base64=[用户输入口令编码后的密钥]
      [待加密明文字符串]
Expect      :
    1.成功生成密钥；
    2.执行失败，报错提示密钥和盐值要同时指定
History     :
"""
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令，组合使用参数--key，--key-base64加密明文字符串" \
               "except:执行失败，报错提示密钥和盐值要同时指定"
        self.logger.info(step)
        key_spl = str(key).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --key={encrypt} --key-base64={key_spl} \
            {str1}'''
