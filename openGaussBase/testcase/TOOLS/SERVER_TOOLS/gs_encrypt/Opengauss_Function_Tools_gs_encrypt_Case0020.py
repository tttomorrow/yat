...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数-v，-f,-B加密明文字符串
Description :
    1.生成以base64方式编码的密钥；
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0020 start')
    ...
    def test_gs_encrypt(self):
        ...
        step = "step3:执行命令，组合使用参数-v，-f，-B加密明文字符串 except:" \
               "step3执行失败，报错提示不能同时使用密钥盐值和加密文件进行加密"
        self.logger.info(step)
        key_spl = str(key).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt -v {vec} -f {self.prefix} -B \
            {key_spl} {self.prefix}'''
        self.logger.info(check_cmd1)
        msg1 = self.dbuserNode.sh(check_cmd1).result()
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0020 finish')
