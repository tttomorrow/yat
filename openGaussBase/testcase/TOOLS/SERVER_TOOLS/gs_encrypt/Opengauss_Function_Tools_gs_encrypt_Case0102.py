...
Case Name   : gs_encrypt工具使用参数--key-base64，--vector-base64加明文字符串，
              --key-base64后输入加密口令长度为7
Description :
    1.生成长度为7的口令以base64方式编码后的密钥；
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1-2:生成长度为7的口令以base64方式编码的密钥,生成以base64方" \
        ...
        step = "step3:执行命令，使用--key-base64，--vector-base64参数" \
               "加密明文字符串 except:执行失败，报错提示口令长度必须为8-16位"
        self.logger.info(step)
