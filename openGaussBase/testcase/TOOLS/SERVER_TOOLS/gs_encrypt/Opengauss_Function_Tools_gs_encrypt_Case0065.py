...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具单独使用参数--vector加密明文字符串
Description :
    1.执行命令:
      gs_encrypt --vector=[用户输入的盐值]  [待加密的明文字符串]
      单独使用参数--vector加密明文字符串
Expect      :
    1.执行失败，报错提示--vector参数不能单独使用
History     :
"""
...
class GsEncrypt62(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1:执行命令,单独使用--vector参数加密明文字符串" \
               "except:执行失败，报错提示--vector参数不能单独使用"
        self.logger.info(step)
        str1 = 'test'
        vec = 'kj#HH452kkkkkkkk'
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --vector={vec} {str1}'''
