...
Case Name   : gs_encrypt工具使用参数--key-base64，--vector-base64加明文字符串，
              --key-base64后输入加密口令长度为15
Description :
    1.生成长度为15的口令以base64方式编码后的密钥；
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1-2:生成长度为15的口令以base64方式编码的密钥," \
               "生成以base64方式编码的16位字符串密文 " \
               "except:成功生成密钥，成功生成密文"
        self.logger.info(step)
        str1 = 'test'
        encrypt = '11##ddDGggggggg'
        self.rand = '123456789123456l'
        enc = encrypt.encode(encoding='utf-8')
        enc1 = self.rand.encode(encoding='utf-8')
