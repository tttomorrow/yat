...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数-k，-f加密明文字符串
Description :
    1.执行命令:
    ...
    2.执行命令:
      gs_encrypt -k [用户输入的口令] -f [用户指定的加密文件前缀名]
      [待加密明文字符串]   组合使用参数-k,-f加密明文字符串
    3.清理环境
Expect      :
    1.成功生成文件；
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0011 start')
        self.dbuserNode = Node('PrimaryDbUser')
        self.Constant = Constant()
        self.prefix = 'test'
        self.encrypt = 'Mppdb@123'
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令，组合使用参数-k,-f加密明文字符串 " \
               "except:执行失败，报错提示口令和盐值要同时指定"
        self.logger.info(step)
        check_cmd1 = f''' source {macro.DB_ENV_PATH};
            gs_encrypt -k {self.prefix} -f {self.prefix}  {self.prefix}'''
        self.logger.info(check_cmd1)
        msg1 = self.dbuserNode.sh(check_cmd1).result()
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0011 finish')
