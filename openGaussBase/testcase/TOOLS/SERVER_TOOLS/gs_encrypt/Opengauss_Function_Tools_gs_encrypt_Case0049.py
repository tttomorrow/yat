...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具使用参数-B，-D加明文字符串，-B后输入只包含大写字母的
              加密口令
Description :
    1.生成8位纯大写字母口令以base64方式编码的密钥；
...
class Tools(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1-2:生成8位纯大写字母口令以base64方式编码的密钥,生成以" \
               "base64方式编码的16位字符串密文  except:成功生成密钥，成功生成" \
               "密文"
        ...
        ran = rand.encode(encoding='utf-8')
        key = base64.b64encode(enc)
        vec = base64.b64encode(ran)
        self.assertEqual(type(key), bytes, step)

        step = "step3:执行命令，使用-B，-D参数加密明文字符串 except:执行" \
               "失败，报错提示口令至少包含3种类型字符"
        self.logger.info(step)
