...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数--file-prefix，--key-base64,
...
Description :
    ...
    4.执行命令
      gs_encrypt --file-prefix=[用户指定的加密文件前缀名]
      --key-base64=[用户输入口令编码后的密钥]
      --vector-base64=[用户输入字符串编码的密文]  [待加密明文字符串]
      组合使用参数--file-prefix，--key-base64,--vector-base64加密明文字符串
    5.清理环境
Expect      :
    1.成功生成密钥；
    2.成功生成密文；
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1-2:生成以base64方式编码的密钥,生成16位字符串以" \
               "base64方式编码后的密文    except:成功生成密钥,成功生成密文"
        self.logger.info(step)
        encrypt = '12gg##GG'
        rand = '11ll111111111111'
        ...
        step = "step4:执行命令，组合使用参数--file-prefix，--key-base64," \
               "--vector-base64加密明文字符串 " \
               "except:执行失败，报错提示不能同时使用密钥盐值和加密文件进行" \
               "加密"
        self.logger.info(step)
        self.key_spl = str(key).split("'")[1]
        self.vec_spl = str(vec).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --file-prefix={self.prefix} --key-base64={self.key_spl} \
