...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数-v，-f组合使用，加密明文字符串
Description :
    1.执行命令:
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0014 start')
        self.dbuserNode = Node('PrimaryDbUser')
        self.Constant = Constant()
        self.prefix = 'test'
        self.encrypt = 'ss##11GG'
        self.vec = '1234567891234567'
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令,组合使用参数-v,-f加密明文字符串 " \
               "except:step2执行失败，报错提示口令和盐值要同时指定"
        self.logger.info(step)
        check_cmd1 = f''' source {macro.DB_ENV_PATH};
            gs_encrypt -v {self.vec} -f {self.prefix}  {self.prefix}'''
        self.logger.info(check_cmd1)
        msg1 = self.dbuserNode.sh(check_cmd1).result()
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0014 finish')
