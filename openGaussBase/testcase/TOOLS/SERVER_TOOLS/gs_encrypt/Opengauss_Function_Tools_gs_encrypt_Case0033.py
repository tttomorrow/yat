...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数-k，-v加密明文字符串,-k后输入8位纯小写字
              母的口令
Description :
    1.执行命令:
      gs_encrypt -k [用户输入的口令] -v [用户输入的盐值]  [待加密明文字符串]
      组合使用参数-k，-v加密明文字符串,-k后输入8位纯小写字母的口令
Expect      :
...
class Tools(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1:执行命令,组合使用参数-k，-v加密明文字符串,-k后输入8" \
               "位纯小写字母口令 except:执行失败，报错提示口令要至少包含3种类" \
               "型字符"
