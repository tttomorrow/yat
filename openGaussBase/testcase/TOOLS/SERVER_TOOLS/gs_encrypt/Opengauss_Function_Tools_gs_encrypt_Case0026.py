...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具加密数字，字符混合字符串
Description :
    1.执行命令:
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0026 start')
        self.dbuserNode = Node('PrimaryDbUser')

    def test_gs_encrypt(self):
        self.logger.info('------进行加密操作信息------')
        step = "step1:执行命令，加密数字，字符混合的字符串  except：执行成功," \
               "输出加密后的密文"
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0026 finish')
