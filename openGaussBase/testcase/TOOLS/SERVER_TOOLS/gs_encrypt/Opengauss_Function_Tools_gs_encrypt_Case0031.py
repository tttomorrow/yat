...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0031 start')
        self.dbuserNode = Node('PrimaryDbUser')

    def test_gs_encrypt(self):
        self.logger.info('------------进行加密操作-------------')
        step = "step1:执行命令,组合使用参数-k，-v加密明文字符串,-k后输入口令" \
               "长度为10   except:执行成功,输出加密后的密文"
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0031 finish')
