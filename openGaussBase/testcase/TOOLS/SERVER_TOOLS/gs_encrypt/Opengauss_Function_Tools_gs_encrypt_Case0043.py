...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数-k，-v加密明文字符串，-v后输入盐值长度为
              17
Description :
    1.执行命令：
      gs_encrypt -k [用户输入的口令] -v [用户输入的盐值]  [待加密明文字符串]
      组合使用参数-k，-v加密明文字符串，-v后输入盐值长度为17
Expect      :
    1.执行失败，报错提示盐值的长度必须为16
History     :
"""
...
class Tools(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1:执行命令,组合使用参数-k，-v加密字符串，-v后输入盐值长度" \
               "为17  except:执行失败，报错提示盐值的长度必须为16"
        self.logger.info(step)
        str1 = 'test'
        encrypt = '1234##FF'
        vec = '12345678912356555'
        ...
        self.assertIn('gs_encrypt: the length of -v must equal to 16',
                      msg1, "执行失败" + step)
