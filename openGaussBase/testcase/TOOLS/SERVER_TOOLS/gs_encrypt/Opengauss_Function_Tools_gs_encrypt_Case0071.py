...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数--key，--vector-base64加密明文字符串
Description :
    1.生成16位字符串以base64方式编码后的密文；
      base64.b64encode([用户输入的16位字符串].encode(encoding='utf-8'))
    2.执行命令:
      gs_encrypt --key=[用户输入的口令] --vector-base64=
      [用户输入16位字符串编码后的密文]   [待加密明文字符串]
      组合使用参数--key,--vector-base64加密明文字符串
Expect      :
    1.成功生成密文；
    2.执行成功，输出加密后的密文
History     :
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令,组合使用参数--key，--vector-base64加密明文" \
               "字符串 except:执行成功,输出加密后的密文"
        self.logger.info(step)
        vec_spl = str(vec).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --key={encrypt} --vector-base64={vec_spl}\
            {str1}'''
