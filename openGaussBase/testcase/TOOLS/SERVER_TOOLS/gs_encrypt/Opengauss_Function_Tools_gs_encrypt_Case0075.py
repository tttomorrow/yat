...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数--key，--vector，--file-prefix对明文
              字符串加密
Description :
    ...
    2.执行命令:
      gs_encrypt --key=[用户输入的口令] --vector=[用户输入的盐值] --file-prefix
      =[用户指定的加密文件前缀名]    [待加密明文字符串]
      组合使用参数--key，--vector，--file-prefix加密明文字符串
    3.清理环境
Expect      :
    1.成功生成文件；
    2.执行失败，报错提示不能同时使用密钥，盐值和加密文件进行加密
    3.清理环境成功
History     :
"""
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令，组合使用参数--key，--vector，--file-prefix" \
               "加密明文字符串 except:step2执行失败，报错提示不能同时使用密钥，" \
               "盐值和加密文件进行加密"
        self.logger.info(step)
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --key={encrypt} --vector={vec} \
            --file-prefix={self.prefix} {self.prefix}'''
        self.logger.info(check_cmd1)
