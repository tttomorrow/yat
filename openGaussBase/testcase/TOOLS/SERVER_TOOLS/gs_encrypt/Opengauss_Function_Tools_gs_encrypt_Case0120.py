...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具单独使用参数--file-prefix加密明文字符串
Description :
    1.执行命令:
    ...
    2.执行命令:
      gs_encrypt --file-prefix=[用户指定的加密文件前缀名]  [待加密明文字符串]
      单独使用参数--file-prefix加密明文字符串
    3.清理环境
Expect      :
    1.成功生成文件；
    2.执行成功，输出加密后的密文
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令,单独使用参数--file-prefix加密明文字符串 " \
