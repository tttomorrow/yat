...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具使用参数--key-base64，--vector-base64
              加明文字符串，--key-base64后输入只包含小写字母特殊字符的口令
Description :
    1.生成8位只包含特殊字符和数字的口令以base64方式编码后的密钥；
      base64.b16encode([用户输入的口令].encode(encoding='utf-8'))
    ...
    3.执行命令:
      ...
      使用--key-base64，--vector-base64参数加密明文字符串
Expect      :
    1.成功生成密钥；
    2.成功生成密文；
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1-2:生成8位含小写字母特殊字符的口令以base64方式编码的密钥," \
               "生成以base64方式编码的16位字符串密文  except:成功生成密钥，成" \
               "功生成密文"
        ...
        step = "step3:执行命令，使用--key-base64，--vector-base64" \
               "参数加密明文字符串 " \
               "except:执行失败，报错提示口令至少包含3种类型字符"
        self.logger.info(step)
