...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数-k，-v，-B加密明文字符串
Description :
    1.生成输入口令以base64方式编码的密钥:
      base64.b64encode([用户输入的口令].encode(encoding='utf-8'))
    2.执行命令:
      gs_encrypt -k [用户输入的口令] -v [用户输入的盐值] -B
      [用户输入口令编码后的密钥]    [待加密明文字符串]
      组合使用参数-k，-v，-B加密明文字符串
Expect      :
    1.成功生成密钥；
    2.执行成功，输出加密后的密文
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0018 start')
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令，组合使用参数-k，-v，-B加密明文字符串 except:" \
               "执行成功，输出加密后的密文"
        self.key_spl = str(self.key).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt -k {self.encrypt} -v {self.vec} -B {self.key_spl} \
            {self.str}'''
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0018 finish')
