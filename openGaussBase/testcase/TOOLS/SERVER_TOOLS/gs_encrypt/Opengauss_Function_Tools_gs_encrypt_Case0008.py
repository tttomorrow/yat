...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具单独使用-B参数加密明文字符串
Description :
    1.生成输入口令以base64方式编码的密钥:
      base64.b64encode([用户输入的口令].encode(encoding='utf-8'))
    2.执行命令:
      gs_encrypt -B  [用户输入口令编码后的密钥]   [待加密明文字符串]
      单独使用-B参数加密明文字符串
Expect      :
    1.成功生成密钥；
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0008 start')
        self.dbuserNode = Node('PrimaryDbUser')
        self.Constant = Constant()
        self.str = 'test'
        self.encrypt = '123@;HHl'
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令,单独使用参数-B加密明文字符串 except:执行失败，" \
               "报错提示参数-B不能单独使用"
        self.logger.info(step)
        self.key_spl = str(self.key).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt -B {self.key_spl} {self.str}'''
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0008 finish')
