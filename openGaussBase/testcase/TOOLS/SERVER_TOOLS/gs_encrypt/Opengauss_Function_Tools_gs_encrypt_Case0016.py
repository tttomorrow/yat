...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数-v，-D加密明文字符串
Description :
    1.生成输入16位字符串以base64方式编码的密文:
      base64.b64encode([用户输入的16位字符串].encode(encoding='utf-8'))
    2.执行命令:
      gs_encrypt -v [用户输入的盐值]  -D [用户输入16位字符串编码后的密文]
      [待加密明文字符串]     组合使用参数-v，-D加密明文字符串
Expect      :
    1.成功生成密文；
    2.执行失败，报错提示口令要和盐值一起指定
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0016 start')
        self.dbuserNode = Node('PrimaryDbUser')
        self.Constant = Constant()
        self.str = 'test'
        self.rand = '123@hh5555555555'

    def test_gs_encrypt(self):
        self.logger.info('-----进行加密操作-----')
        step = "step1:生成输入16位字符串以base64方式编码后的密文 " \
               "except:成功生成密文"
        ...
        step = "step2:执行命令,组合使用参数-v，-D加密明文字符串 except:执行失" \
               "败，报错提示口令要和盐值一起指定"
        self.logger.info(step)
        self.vec_spl = str(self.vec).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt -v {self.rand} -D {self.vec_spl} {self.str}'''
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0016 finish')
