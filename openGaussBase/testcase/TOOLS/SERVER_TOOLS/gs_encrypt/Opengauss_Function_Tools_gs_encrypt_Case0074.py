...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数--vector,--vector-base64加密明文字符串
Description :
    1.生成16位字符串以base64方式编码后的密文；
      base64.b64encode([用户输入的16位字符串].encode(encoding='utf-8'))
    2.执行命令，组合使用参数--vector,--vector-base64加密明文字符串
      gs_encrypt --vector=[用户输入的盐值] --vector-base64=
      [用户输入16位字符串编码后的密文] 待加密明文字符串]
Expect      :
    1.成功生成密文；
    2.执行失败，报错提示密钥要和盐值一起指定
History     :
"""
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1:生成16位字符串以base64方式编码后的密文 except:成功生成" \
               "密文"
        self.logger.info(step)
        str1 = 'test'
        rand = '123@hh5555555555'
        enc = rand.encode(encoding='utf-8')
        vec = base64.b64encode(enc)
        self.assertEqual(type(vec), bytes, step)

        step = "step2:执行命令使用参数--vector,--vector-base64加密明文字符串 " \
               "except:执行失败，报错提示密钥要和盐值一起指定"
        self.logger.info(step)
        vec_spl = str(vec).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --vector={rand} --vector-base64={vec_spl}\
            {str1}'''
