...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具使用参数-B，-D加明文字符串，-D后输入长度为10的字符串
Description :
    1.生成以base64方式编码的密钥；
      base64.b64encode([用户输入的口令].encode(encoding='utf-8'))
    2.生成10位字符串以base64方式编码后的密文；
      base64.b64encode([用户输入的10位字符串].encode(encoding='utf-8'))
    3.执行命令:
...
class GsEncrypt57(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
    ...
    def test_gs_encrypt(self):
        ...
        step = "step1-2:生成以base64方式编码的密钥,生成以base64方式编码的10位" \
