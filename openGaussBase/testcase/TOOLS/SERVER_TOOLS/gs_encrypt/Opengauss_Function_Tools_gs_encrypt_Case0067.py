...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具单独使用参数--vector-base64加密明文字符串
Description :
    1.生成输入16位字符串以base64方式编码的密文:
      base64.b64encode([用户输入的16位字符串].encode(encoding='utf-8'))
    2.执行命令:
      gs_encrypt --vector-base64=[用户输入16位字符串编码后的密文]
       [待加密明文字符串]
      单独使用参数--vector-base64加密明文字符串
Expect      :
    1.成功生成密文；
    2.执行失败，报错提示--vector-base64参数不能单独使用
History     :
"""
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        str1 = 'test'
        rand = 'kkkkkkkkkkkkkkkk'
        enc = rand.encode(encoding='utf-8')
        vec = base64.b64encode(enc)
        self.assertEqual(type(vec), bytes, "生成失败" + step)

        step = "step2:执行命令,单独使用参数--vector-base64加密明文字符串 " \
               "except:执行失败，报错提示--vector-base64参数不能单独使用"
        self.logger.info(step)
        vec_spl = str(vec).split("'")[1]
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_encrypt --vector-base64={vec_spl} {str1}'''
