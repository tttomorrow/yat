...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数--vector，--file-prefix组合使用，
              加密明文字符串
Description :
    1.执行命令
    ...
    2.执行命令，组合使用参数--vector，--file-prefix加密明文字符串
      gs_encrypt --vector=[用户输入的盐值] --file-prefix=
      [用户指定的加密文件前缀名]  [待加密明文字符串]
    3.清理环境
Expect      :
    1.成功生成文件到$GAUSSHOME/bin目录下；
...
class GsEncrypt60(unittest.TestCase):
    ...
    def test_gs_encrypt(self):
        ...
        step = "step2:执行命令使用参数--vector，--file-prefix加密明文字符串 " \
               "except:step2执行失败，报错提示密钥和盐值要同时指定"
        self.logger.info(step)
        check_cmd1 = f''' source {macro.DB_ENV_PATH};
