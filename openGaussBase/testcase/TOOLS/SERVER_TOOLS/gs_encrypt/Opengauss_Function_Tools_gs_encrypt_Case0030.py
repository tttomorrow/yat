...
"""
Case Type   : 服务端工具
Case Name   : gs_encrypt工具组合使用参数-k，-v加密明文字符串,-k后输入口令长度为
              17
Description :
    1.执行命令:
      gs_encrypt -k [用户输入的口令] -v [用户输入的盐值]  [待加密明文字符串]
      组合使用参数-k，-v加密明文字符串,-k后输入口令长度为17
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0030 start')
        self.dbuserNode = Node('PrimaryDbUser')

    def test_gs_encrypt(self):
        self.logger.info('-----进行加密操作-----')
        step = "step1:执行命令,组合使用参数-k，-v加密字符串,-k后输入口令长度" \
               "为17  except:执行失败，报错提示输入口令长度必须在8-16位"
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_gs_encrypt_Case0030 finish')
