...
"""
Case Type   : 服务端工具
Case Name   : 通过时间筛选gaussdb进程的core文件
Description :
        1.创建收集内容的配置文件，文件内容是json 格式
        2.通过时间筛选gaussdb进程的core文件
        3.删除创建收集内容的配置文件
Expect      :
        1.写入完成
        2.收集成功
...
class Tools(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-----Opengauss_Function_Tools_gs_collector_Case0027start-----')
    ...
    def test_server_tools1(self):
        ...
        test_str = r'''{{"Collect":[{{"TypeName": "CoreDump", "Content":
            "DataReceiver,DataSender","Interval":"3", "Count":"2"}}]}}'''
        ...
        self.log.info('-----步骤2：通过时间筛选gaussdb进程的core文件-----')
        ...
        check_cmd1 = f'hostname'
        self.log.info(check_cmd1)
        hostname1 = self.dbusernode.sh(check_cmd1).result()
        ...
        self.assertIn(f'{hostname1}.tar.gz', tar_msg)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-----Opengauss_Function_Tools_gs_collector_Case0027finish-----')