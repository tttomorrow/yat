...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-c和-C的使用
Description :
    1.创建数据
    2.导出数据
    3.导入时先清理数据，创建数据库，然后导入
    4.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0091开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-d db_test {macro.DB_INSTANCE_PATH}/test.sql  -c -C"
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0091执行结束-")
