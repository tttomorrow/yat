...
"""
Case Type   : 服务端工具
Case Name   : -s/--schema-only 和 -a/--data-only不能同时使用
Description :
    1.创建数据
    2.导出数据
    3.导入数据
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0048开始执行--")
        self.info = "gs_restore: options -s/--schema-only and " \
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"/home/test_restore/test2.tar -s -a "
    ...
    def tearDown(self):
        ...
        self.log.info(msg)
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0048执行结束--")
