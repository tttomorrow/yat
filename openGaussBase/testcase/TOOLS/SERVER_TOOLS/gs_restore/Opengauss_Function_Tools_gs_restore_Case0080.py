...
"""
Case Type   : 服务端工具
Case Name   : 连接用户不是系统管理员，使用参数-w，不出现输入密码提示
Description :
    1.创建数据
    2.导出数据
    3.导入数据
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0080开始执行--")
        self.log.info("----------------------创建数据-----------------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-d {self.db_name} " \
            f"-U manager -w  {macro.DB_INSTANCE_PATH}/test.tar -c"
        self.log.info(restore_cmd)
        restore_msg = self.dbuser_node.sh(restore_cmd).result()
        self.log.info(restore_msg)
        self.assertIn('Invalid username/password,login denied', restore_msg)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0080执行结束-")
