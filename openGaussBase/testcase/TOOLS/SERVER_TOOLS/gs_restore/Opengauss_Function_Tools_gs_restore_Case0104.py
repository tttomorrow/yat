...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-C和-t,-n,-I,-s的使用
Description :
    1.创建数据
    2.导出数据
    3.导入时先创建数据库，然后导入指定表、模式中的对象、索引，模式
    4.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0104开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            f"gs_restore -p {self.dbuser_node.db_port} -d test  " \
            f"{macro.DB_INSTANCE_PATH}/test.tar -C " \
            f"-n schema1 -n schema2  " \
            f"-t test1 -t test4 -s -I index1 -I index2;"
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0104执行结束-")
