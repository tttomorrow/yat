...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0092开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-d {self.db_name} {macro.DB_INSTANCE_PATH}/test.tar -s -c"
        ...
        self.log.info("--------查看导入数据，只导入定义，不导入表数据----")
        check_cmd = f'''source {macro.DB_ENV_PATH};
            gsql -p {self.dbuser_node.db_port} -d {self.db_name}  \
            -c "select * from test;"
            '''
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0095执行结束-")
