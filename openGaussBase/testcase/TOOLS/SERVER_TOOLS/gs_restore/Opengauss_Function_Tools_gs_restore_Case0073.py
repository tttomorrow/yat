...
"""
Case Type   : 服务端工具
Case Name   : 连接用户不是系统管理员,没有-W参数,导入到数据库会提示用户输入密码(用户没有权限)
Description :
    1.创建数据
    1)创建数据库
    2)创建表并插入数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Tools_gs_restore_Case0073start---')
        ...
        self.db_name1 = "db_restore0073_01"
        self.db_name2 = "db_restore0073_02"
        self.tb_name = "t_restore0073"
        self.u_name = "u_restore0073"
    ...
    def test_tools_restore(self):
        ...
        text = '-----step3:导入之前导出的数据,提示输入密码;expect:密码输入正确,导入报错-----'
        self.log.info(text)
        ...
        self.assertIn('permission denied', restore_result, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '------Opengauss_Function_Tools_gs_restore_Case0073finish------')