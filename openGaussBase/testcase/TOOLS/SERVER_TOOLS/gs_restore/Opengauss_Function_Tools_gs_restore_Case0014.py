...
"""
Case Type   : 服务端工具
Case Name   : gs_dump导出tar归档格式的数据，使用gs_restore方式导入到脚本文件
Description :
    1.创建数据
    2.导出数据
    3.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0014开始执行--")
    ...
    def test_server_tools1(self):
        ...
        /home/test_restore/test2.tar -f /home/test_restore/test2.tar -c'''
        Log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        Log.info(msg)
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('---Opengauss_Function_Tools_gs_restore_Case0014执行结束---')
