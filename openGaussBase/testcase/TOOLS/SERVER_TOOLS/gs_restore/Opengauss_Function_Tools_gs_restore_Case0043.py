...
"""
Case Type   : 服务端工具
Case Name   : 参数-S, --sysadmin=NAME（不添加name）
Description :
    1.创建数据
    2.导出数据
    3.导入数据
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0043开始执行--")
        self.log.info("----------------------创建数据-----------------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f" -d test /home/test_restore/test2.tar -S"
        self.log.info(restore_cmd)
        msg = self.dbuser_node.sh(restore_cmd).result()
        self.log.info(msg)
        self.assertIn("gs_restore: option requires an argument -- 'S'", msg)
    ...
    def tearDown(self):
        ...
        self.log.info(msg)
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0043执行结束--")
