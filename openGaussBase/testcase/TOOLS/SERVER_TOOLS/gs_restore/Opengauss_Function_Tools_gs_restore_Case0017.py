...
"""
Case Type   : 服务端工具
Case Name   : 用户同时输入dbname和-l选项（不允许）
Description :
    1.创建数据
    2.导出数据
    3.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0016开始执行--")
...
need to be specified for gs_restore", msg)
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('---Opengauss_Function_Tools_gs_restore_Case0016执行结束---')
