...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-C和-t的使用
Description :
    1.创建数据
    2.导出数据
    3.导入时先创建数据库，然后倒入指定表数据
    4.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
    3.指定表test1,test2导入成功
    4.清理环境成功
History     :
"""
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0101开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f'''source {macro.DB_ENV_PATH};
            gs_restore -p {self.dbuser_node.db_port} -d test \
            {macro.DB_INSTANCE_PATH}/test.tar  -t test1 -t test4 -C;
            gsql -p {self.dbuser_node.db_port} -d test -c \
            "select * from test1;
            select * from test4;
            select * from test3;
            "'''
        ...
        self.assertIn('3 | hehe', restore_msg)
        self.assertIn('783 | cai', restore_msg)
        self.assertIn('relation "test3" does not exist ', restore_msg)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0101执行结束-")
