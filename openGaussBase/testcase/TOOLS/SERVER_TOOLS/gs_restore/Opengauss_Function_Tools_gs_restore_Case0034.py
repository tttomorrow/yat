...
"""
Case Type   : 服务端工具
Case Name   : 只导入已列举的模式中的对象(-n, --schema=NAME),不添加对象名称到脚本文件
Description :
        1.创建表并插入数据
        2.创建模式
        3.导出数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0034开始执行--")
    ...
    def test_server_tools1(self):
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-n -f /home/test_restore/ -n"
        Log.info(restore_cmd)
        msg = self.dbuser_node.sh(restore_cmd).result()
        Log.info(msg)
        self.assertIn("gs_restore: option requires an argument -- 'n'", msg)
    ...
    def tearDown(self):
        ...
        Log.info(msg)
        Log.info("---Opengauss_Function_Tools_gs_restore_Case0034执行结束---")
