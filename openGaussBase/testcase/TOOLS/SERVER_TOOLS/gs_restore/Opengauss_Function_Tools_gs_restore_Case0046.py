...
"""
Case Type   : 服务端工具
Case Name   : 只导入数据定义不导入数据（表内容）到脚本文件（-s）
Description :
    1.创建数据
    2.导出数据
    3.导入数据
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0046开始执行--")
        self.log.info("----------------------创建数据-----------------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-s -f /home/test_restore/test2.tar"
    ...
    def tearDown(self):
        ...
        self.log.info(msg)
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0046执行结束--")
