...
"""
Case Type   : 服务端工具
Case Name   : 导入PUBLIC模式下的test4和schema2模式下test2
Description :
    1.创建数据
    2.导出数据
    3.导入数据
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0051开始执行--")
        self.log.info("----------------------创建数据-----------------------")
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'''select * from test4;
            select * from schema2.test2;
            ''')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0051执行结束--")
