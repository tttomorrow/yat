...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-C和-i的使用
Description :
    1.创建数据
    2.导出数据
    3.导入时先创建数据库，只导入指定的索引
    5.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0100开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        check_cmd = f"source {macro.DB_ENV_PATH}; " \
            ...
            f"--index=index1 --index=index2 --index=index3 -C"
        self.log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        self.log.info(msg)
        self.assertIn('could not execute query: ERROR:  '
                      'relation "test3" does not exist', msg)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0100执行结束-")