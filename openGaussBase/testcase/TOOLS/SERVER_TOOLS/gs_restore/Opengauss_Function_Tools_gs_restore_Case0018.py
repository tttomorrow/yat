...
"""
Case Type   : 服务端工具
Case Name   : 导入到数据库时不指定连接密码（未并行导入）
Description :
    1.创建数据
    2.导出数据
    3.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0018开始执行--")
    ...
    def test_server_tools1(self):
       ...
       gs_dump -p {self.dbuser_node.db_port} {self.dbuser_node.db_name} \
        ...
        Log.info("--------------导入之前导出的数据----------------")
        check_cmd = f'''source {macro.DB_ENV_PATH}
        gs_restore /home/test_restore/test2 \
        -p {self.dbuser_node.db_port} -d {self.dbuser_node.db_name} -c
      '''
        Log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        Log.info(msg)
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('---Opengauss_Function_Tools_gs_restore_Case0018执行结束---')
