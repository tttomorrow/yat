...
"""
Case Type   : 服务端工具
Case Name   : 只导入已列举的模式中的对象(-n, --schema=NAME)并添加对象名称到数据库
Description :
        1.创建表并插入数据
        2.创建索引
        3.导出数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0033开始执行--")
    ...
    def test_server_tools1(self):
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
             ...
             f"-n schema1 -n schema2 -n schema4"
        Log.info(restore_cmd)
        msg = self.dbuser_node.sh(restore_cmd).result()
        Log.info(msg)
        self.assertIn("table test1 complete data imported !", msg)
        self.assertIn("table test2 complete data imported !", msg)
    ...
    def tearDown(self):
        ...
        Log.info(msg)
        Log.info("---Opengauss_Function_Tools_gs_restore_Case0033执行结束---")
