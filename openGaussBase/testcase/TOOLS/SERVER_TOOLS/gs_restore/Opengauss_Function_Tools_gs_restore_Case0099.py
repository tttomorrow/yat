...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0099开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-d test  {macro.DB_INSTANCE_PATH}/test.sql"
        ...
        self.log.info("--------------再次导入之前导出的数据----------------")
        check_cmd = f"source {macro.DB_ENV_PATH};" \
            f" gs_restore -p {self.dbuser_node.db_port} " \
            f"-d test {macro.DB_INSTANCE_PATH}/test.sql -e -C"
        self.log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        self.log.info(msg)
        self.assertIn(f'ERROR:  database "{self.db_name}" '
            f'already exists', msg)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0099执行结束-")
