...
"""
Case Type   : 服务端工具
Case Name   : gs_dump导出自定义归档格式的数据，使用gs_restore方式导入到数据库
Description :
    1.创建数据
    2.导出数据
    3.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("---Opengauss_Function_Tools_gs_restore_Case0009开始执行---")
...
-W {self.dbuser_node.db_password}   /home/test_restore/test2 -c'''
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('--Opengauss_Function_Tools_gs_restore_Case0009执行结束--')
