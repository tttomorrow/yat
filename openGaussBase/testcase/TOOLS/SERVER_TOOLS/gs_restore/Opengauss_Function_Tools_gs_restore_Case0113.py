...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-s和-t的使用
Description :
    1.创建数据
    2.导出数据
    3.导入时只导入已列举的模式中的对象和表数据
    4.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0113开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH} ;" \
            ...
            f"-d  {self.db_name} " \
            f"{macro.DB_INSTANCE_PATH}/test.tar -t " \
            f"schema1.table1 -t public.table2 " \
            f"-t table4 -s "
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0113执行结束-")
