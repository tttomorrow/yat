...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-e和-i 的使用
Description :
    1.创建数据
    2.导出数据
    3.导入时只导入索引定义，若执行sql时报错，会退出，提示报错信息
    4.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0105开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        dump_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"{self.db_name} -f {macro.DB_INSTANCE_PATH}/test.tar " \
            f"-F t; gsql -p {self.dbuser_node.db_port} -d " \
            f"{self.db_name} -c " \
            f"'drop index index1; " \
            f"drop index index2;" \
            f"drop index index3;" \
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH} ;" \
            ...
            f" --index=index1 --index=index2 --index=index3 -e ;" \
        ...
        self.log.info(msg)
        self.assertIn('ERROR:  index "index4" does not exist', msg)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0105执行结束-")
