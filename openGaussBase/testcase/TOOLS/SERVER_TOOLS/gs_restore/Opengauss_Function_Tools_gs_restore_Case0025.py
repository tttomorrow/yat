...
"""
Case Type   : 服务端工具
Case Name   : -c/--clean 和 -a/--data-only不能同时使用
Description :
    1.创建数据
    2.导出数据
    3.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0025开始执行--")
    ...
    /home/test_restore/test2.sql -a -c
    '''
        Log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        Log.info(msg)
        self.assertIn('gs_restore: options -c/--clean and \
-a/--data-only cannot be used together', msg)
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('---Opengauss_Function_Tools_gs_restore_Case0025执行结束---')
