...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0093开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        self.log.info("------------------清理环境------------------")
        sql_cmd = self.commonsh.execut_db_sql(f'''drop index index1;           
            drop index index2; 
            drop index index3; 
            ''', dbname=self.db_name)
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0093执行结束-")
