...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-e和-n的使用
Description :
    1.创建数据
    2.导出数据
    3.只导入指定模式中的对象，发送SQL语句到数据库时如果出现错误，退出
    4.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
    3.导入失败（提示schema1已存在，然后退出）
    4.清理环境成功
History     :
"""
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0108开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH} ; " \
            ...
            f"-d {self.db_name}  " \
            f"{macro.DB_INSTANCE_PATH}/test.tar -n  " \
            f"schema1 -n schema2 -n schema4 -e"
        self.log.info(restore_cmd)
        msg = self.dbuser_node.sh(restore_cmd).result()
        self.log.info(msg)
        self.assertIn('ERROR:  schema "schema1" already exists', msg)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0108执行结束-")
