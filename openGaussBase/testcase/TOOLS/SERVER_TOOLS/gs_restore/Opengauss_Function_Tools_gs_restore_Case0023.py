...
"""
Case Type   : 服务端工具
Case Name   : 全量导出后，只导入数据，不导入定义到脚本文件(-a)
Description :
    1.创建表并插入数据
    2.导出数据
    3.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0023开始执行--")
    ...
    def test_server_tools1(self):
        ...
        -l /home/test_restore/test2.sql -f /home/test_restore/test2.sql  -a'''
        Log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        Log.info(msg)
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('---Opengauss_Function_Tools_gs_restore_Case0023执行结束---')
