...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0088开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-d {self.db_name} " \
            f"{macro.DB_INSTANCE_PATH}/test.sql -a -t test1 -t test2;"
        ...
        self.log.info("------------------查看数据是否被导入------------------")
        sql_cmd = self.commonsh.execut_db_sql(f'''select * from test1;
            select * from test2;''', dbname=self.db_name)
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0088执行结束-")
