...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0083开始执行--")
        self.log.info("----------------------创建数据-----------------------")
        ...
        sql = f'''create table test (id  int,name char(20));
            ...
            create user manager identified by '{macro.PASSWD_REPLACE}';
            create role  role1  identified by '{macro.PASSWD_REPLACE}';
        ...
        self.log.info("-------------清理数据-------------")
        sql_cmd = self.commonsh.execut_db_sql(f"drop table test;",
                                              dbname=self.db_name)
        self.log.info(sql_cmd)
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH}; " \
            ...
            f"-d {self.db_name} -U manager " \
            f"-W {macro.PASSWD_REPLACE} {macro.DB_INSTANCE_PATH}/test.tar  " \
            f"--role=role1 --rolepassword={macro.PASSWD_REPLACE}"
        ...
        sql_cmd = self.commonsh.execut_db_sql(f"select * from test;",
                                              dbname=self.db_name)
        self.log.info(sql_cmd)
        self.assertIn('ERROR:  relation "test" does not exist', sql_cmd)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0083执行结束-")
