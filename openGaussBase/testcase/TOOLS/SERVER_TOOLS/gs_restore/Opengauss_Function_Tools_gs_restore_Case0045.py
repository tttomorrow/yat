...
"""
Case Type   : 服务端工具
Case Name   : 多次输入-n schema-name -P 'function-name(args)'同时导入多个指定模式下的函数到脚本文件
Description :
    1.创建数据
    2.导出数据
    3.导入数据
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0045开始执行--")
        self.log.info("----------------------创建模式-----------------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-P schema1.func1 -n schema2 -P shema2.func3  " \
            f"-f /home/test_restore/tt.sql"
    ...
    def tearDown(self):
        ...
        self.log.info(msg)
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0045执行结束--")
