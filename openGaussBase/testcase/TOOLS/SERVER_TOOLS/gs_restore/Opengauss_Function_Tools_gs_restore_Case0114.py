...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-n和-s和-t的使用
Description :
    1.创建数据
    2.导出数据
    3.导入时只导入已列举的模式中的对象、模式、和表
    4.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
    3.导入失败(提示：表test1已经存在)
    4.清理环境成功
History     :
"""
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0114开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH} ;" \
            ...
            f"-t test1 -t test2 -t test4 -s "
        self.log.info(restore_cmd)
        msg = self.dbuser_node.sh(restore_cmd).result()
        self.log.info(msg)
        self.assertIn('ERROR:  relation "test1" already exists', msg)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0114执行结束-")
