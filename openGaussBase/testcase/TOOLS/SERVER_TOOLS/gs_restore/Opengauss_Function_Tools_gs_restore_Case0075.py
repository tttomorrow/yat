...
"""
Case Type   : 服务端工具
Case Name   : 连接用户不是系统管理员,没有-W参数,导入到数据库会提示用户输入密码(用户赋予权限，但输入的密码不正确)
Description :
    1.创建数据
    1)创建数据库
    2)创建表并插入数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Tools_gs_restore_Case0075start---')
        ...
        self.u_passwd = macro.COMMON_PASSWD
        self.db_name1 = "db_restore0075_01"
        self.db_name2 = "db_restore0075_02"
        self.tb_name = "t_restore0075"
        self.u_name = "u_restore0075"
    ...
    def test_tools_restore(self):
        ...
        text = '-----step3:导入之前导出的数据,提示输入密码;expect:密码输入不正确,导入报错-----'
        self.log.info(text)
        restore_cmd = f'''source {macro.DB_ENV_PATH};
            ...
            expect "Password:"
            send "{macro.COMMON_PASSWD} + '123'\r"
            expect eof\n''' + '''EOF'''
        self.log.info(restore_cmd)
        restore_result = self.Primary_Node.sh(restore_cmd).result()
        self.log.info(restore_result)
        self.assertIn('Invalid username/password,login denied',
                      restore_result, '执行失败:' + text)

        text = '---在导入的数据库中查询表和数据，表1不存在---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '------Opengauss_Function_Tools_gs_restore_Case0075finish------')
