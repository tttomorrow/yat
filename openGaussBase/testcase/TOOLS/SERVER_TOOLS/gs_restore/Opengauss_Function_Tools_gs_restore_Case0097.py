...
"""
Case Type   : 服务端工具
Case Name   : 导入时参数-c和-n和-t的使用
Description :
    1.创建数据
    2.导出数据
    3.导入时只导入已列举的模式对象和表数据
    4.清理环境
Expect      :
    1.创建数据成功
    2.导出数据成功
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0097开始执行--")
        self.log.info("-----------------创建数据--------------")
        ...
        check_cmd = f'''source {macro.DB_ENV_PATH};
            gsql -p {self.dbuser_node.db_port} -d \
            {self.db_name} -c "
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0097执行结束-")
