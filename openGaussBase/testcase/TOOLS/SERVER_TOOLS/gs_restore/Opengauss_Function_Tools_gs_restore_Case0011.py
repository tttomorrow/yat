...
"""
Case Type   : 服务端工具
Case Name   : gs_dump导出tar归档格式的数据，使用gs_restore方式导入到数据库
Description :
    1.创建数据
    2.导出数据
    3.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0011开始执行--")
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('---Opengauss_Function_Tools_gs_restore_Case0011执行结束---')
