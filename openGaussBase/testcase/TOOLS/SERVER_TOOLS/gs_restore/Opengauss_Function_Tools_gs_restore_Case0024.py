...
"""
Case Type   : 服务端工具
Case Name   : 在导入数据库之前先清理已存在于将要还原的数据库中的数据库对象（-c）
Description :
    1.创建数据
    2.导出数据
    3.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0024开始执行--")
    ...
    def test_server_tools1(self):
       ...
       gs_dump -p {self.dbuser_node.db_port} {self.dbuser_node.db_name} \
        ...
        Log.info("--------------导入之前导出的数据----------------")
        check_cmd = f'''source {macro.DB_ENV_PATH}
    gs_restore  -p {self.dbuser_node.db_port} -l /home/test_restore/test2.sql \
    -f /home/test_restore/test2.sql  -c
    '''
        Log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        Log.info(msg)
        self.assertIn('restore operation successful', msg)
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('---Opengauss_Function_Tools_gs_restore_Case0024执行结束---')
