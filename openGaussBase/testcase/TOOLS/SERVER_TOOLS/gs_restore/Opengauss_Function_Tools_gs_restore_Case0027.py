...
"""
Case Type   : 服务端工具
Case Name   : 导入到指定的脚本文件之前先创建数据库（-C）
Description :
    1.创建数据
    3.导出数据
    4.导入之前导出的数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0027开始执行--")
    ...
    def test_server_tools1(self):
        ...
        -l /home/test_restore/test2.sql -C -f /home/test_restore/t30
        '''
        Log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        Log.info(msg)
        ...
        Log.info("-----------查看导入的数据是否正确------------")
        check_cmd = f'''source {macro.DB_ENV_PATH}
                        cat /home/test_restore/t30
                        '''
        Log.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        Log.info(msg)
        self.assertIn("FUNCTION public func_increment_sql(integer)", msg)
    ...
    def tearDown(self):
        ...
        msg = self.root_user.sh(check_cmd).result()
        Log.info(msg)
        Log.info('---Opengauss_Function_Tools_gs_restore_Case0027执行结束---')
