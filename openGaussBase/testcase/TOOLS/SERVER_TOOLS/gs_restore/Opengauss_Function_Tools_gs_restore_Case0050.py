...
"""
Case Type   : 服务端工具
Case Name   : 选项-t与-n选项同时使用时，用来指定某个模式下的表对象。导入schema1模式下的test1和schema2模式下test2
Description :
    1.创建数据
    2.导出数据
    3.导入数据
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0050开始执行--")
        self.log.info("----------------------创建数据-----------------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"/home/test_restore/test2.tar -n schema1 " \
            f"-t test1  -n schema2 -t test2"
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'''select * from schema1.test1;
            select * from schema2.test2;
            ''')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.log.info(msg)
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0050执行结束--")
