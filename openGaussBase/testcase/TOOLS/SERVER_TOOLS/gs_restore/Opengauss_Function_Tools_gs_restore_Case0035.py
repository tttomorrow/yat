...
"""
Case Type   : 服务端工具
Case Name   : 只导入已列举的模式中的对象(-n, --schema=NAME),添加对象名称到脚本文件
Description :
        1.创建表并插入数据
        2.创建模式
        3.导出数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info("--Opengauss_Function_Tools_gs_restore_Case0035开始执行--")
    ...
    def test_server_tools1(self):
        ...
        Log.info("----------------导出tar格式文件-----------------")
        mkdir_cmd = \
            f"mkdir /home/{self.primary_dbuser.dbuser}/test_restore/ ;"
        Log.info(mkdir_cmd)
        ...
        restore_cmd = \
            ...
            f"-l /home/test_restore/test2.tar  " \
            f"-n schema1 -n schema2 -n schema3"
    ...
    def tearDown(self):
        Log.info("--------------------清理环境---------------------")
        sql_cmd = f'''
            drop table schema1.test1  cascade;
        ...
        Log.info(msg)
        Log.info("---Opengauss_Function_Tools_gs_restore_Case0035执行结束---")
