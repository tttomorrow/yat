...
"""
Case Type   : 服务端工具
Case Name   : 指定连接的用户名正确和密码正确(如果主机的认证策略是trust，连接用户不是系统管理员)
Description :
    1.创建数据
    2.导出数据
    3.导入数据
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools1(self):
        self.log.info("--Opengauss_Function_Tools_gs_restore_Case0079开始执行--")
        self.log.info("----------------------创建数据-----------------------")
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-W {macro.PASSWD_REPLACE} {macro.DB_INSTANCE_PATH}/test.tar -c"
        self.log.info(restore_cmd)
        msg = self.dbuser_node.sh(restore_cmd).result()
        self.log.info(msg)
        self.assertIn('table test complete data imported', msg)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_restore_Case0079执行结束-")
