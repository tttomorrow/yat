...
"""
Case Type   : 系统内部工具
Case Name   : 使用guc方式修改备节点端口，查询数据库状态，校验节点信息
Description :
    1、guc方式修改集群备节点port,重启数据库;
       gs_guc set -D {dn1} -c "port=new_port"
...
Expect      :
    ...
    4、显示指定节点状态信息，指定节点为备机，存在port字段，备机端口号为修改后port;
...
class Tools(unittest.TestCase):
    ...
    def setUp(self):
        logger.info("======Opengauss_Function_Tools_gs_om_Case0109开始执行======")
    ...
    def test_server_tools(self):
        logger.info("======步骤1:guc方式修改备节点port，重启数据库，校验修改成功======")
        ...
        show_cmd = f'''source {macro.DB_ENV_PATH};
            ...
            -c "show port"'''
        show_res = self.standby_node.sh(show_cmd).result()
    ...
    def tearDown(self):
        ...
        if self.node_port not in status_msg.splitlines()[11]:
            ...
            logger.info("======端口号正常======")
        logger.info("======Opengauss_Function_Tools_gs_om_Case0109执行结束======")
