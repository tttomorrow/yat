...
"""
Case Type   : 服务端工具
Case Name   : 数据库启动时，指定服务器名称值
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.关闭数据库：gs_om -t stop;
    3.启动时指定-h的值有误是否可以启动成功
    4.启动时指定-h的值正确是否可以启动成功
    5.查看数据库状态：gs_om -t status --detail
    6.启动时指定-h后跟备机是否可以启动成功
...
Expect      :
    ...
    3.启动失败
    4.启动成功
    5.主机ctupopenga00011启动成功，备机ctupopenga00006不成功
    6.启动成功
    7.状态正常
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_om_Case0018start---')
    ...
    def test_server_tools1(self):
        ...
        if 'Standby' not in status_msg:
            ...
            self.log.info('----------启动时指定-h值有误是否可以启动成功---------')
            stop_cmd2 = f'source {macro.DB_ENV_PATH}; ' \
                f'gs_om -t start -h ctupopenga1111;'
            self.log.info(stop_cmd2)
            start_msg = self.dbuser_node.sh(stop_cmd2).result()
            self.log.info(start_msg)
            self.assertIn('Can not start the database', start_msg)

            self.log.info('----------启动时指定-h的值正确是否可以启动成功---------')
            ...
            status_msg = self.dbuser_node.sh(status_cmd).result()
            self.log.info(status_msg)
            self.assertTrue("Degraded" in status_msg or "Normal"
                in status_msg or "stopped" in status_msg)

            self.log.info('----------启动时指定-h后跟备机名是否可以启动成功---------')
            self.log.info('-----查看备机名称-----')
            check_cmd = f'hostname'
            self.log.info(check_cmd)
            hostname = self.standby_node.sh(check_cmd).result()
            self.log.info(hostname)
    ...
    def tearDown(self):
        ...
        self.log.info(start_cmd)
        self.log.info('--Opengauss_Function_Tools_gs_om_Case0018finish--')
