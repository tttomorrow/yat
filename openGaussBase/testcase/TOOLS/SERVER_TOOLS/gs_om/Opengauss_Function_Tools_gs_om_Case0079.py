...
"""
Case Type   : 服务端工具
Case Name   : 查询指定服务器状态的所有信息的同时生成日志文件
Description :
    1.查询指定服务器状态的所有信息的同时生成日志文件
    2.查看日志文件是否生成
    3.删除日志文件
Expect      :
    1.查询成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info('--Opengauss_Function_Tools_gs_om_Case0079start--')
        self.dbusernode = Node('dbuser')
        self.constant = Constant()
        self.commonsh = CommonSH()

    def test_server_tools1(self):
        Log.info('------查询指定服务器状态的所有信息的同时生成日志文件-------')
        Log.info('-----------查看主机名称------------')
    ...
    def tearDown(self):
        ...
        Log.info(clear_msg)
        Log.info('--Opengauss_Function_Tools_gs_om_Case0079finish--')
