...
"""
Case Type   : 服务端工具
Case Name   : 数据库启动时，不设置--time-out超时时间或者不指定--time-out参数
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.停止数据库：gs_om -t stop;
    3.启动时，不设置超时时间:gs_om -t start --time-out;
    4.启动时，不指定--time-out参数:gs_om -t start
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('-----Opengauss_Function_Tools_gs_om_Case0024start------')
    ...
    def test_server_tools1(self):
        ...
        self.logger.info('-------启动时，不设置超时时间--------')
        start_cmd1 = f'source {macro.DB_ENV_PATH}; gs_om -t start --time-out;'
        self.logger.info(start_cmd1)
        start_msg1 = self.dbuser_node.sh(start_cmd1).result()
        self.logger.info(start_msg1)
        self.assertIn('option --time-out requires argument', start_msg1)
        self.logger.info('--------启动时，不指定--time-out参数---------')
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0024finish--')
