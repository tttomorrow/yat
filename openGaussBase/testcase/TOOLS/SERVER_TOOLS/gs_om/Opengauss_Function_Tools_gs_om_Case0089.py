...
"""
Case Type   : 服务端工具
Case Name   : 开启集群内安装kerberos客户端认证（安装服务端）
Description :
    1.开启集群内安装kerberos服务端认证:gs_om -t kerberos -m install -U omm用户  --krb-server
    2.开启集群内安装kerberos客户端认证:gs_om -t kerberos -m install -U omm用户  --krb-client
    3.关闭集群内安装kerberos认证：gs_om -t kerberos -m uninstall -U omm用户
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0089start--')
    ...
    def test_server_tools1(self):
        ...
        self.logger.info('-------开启集群内安装kerberos客户端认证--------')
        check_cmd2 = f'''
        source {macro.DB_ENV_PATH}; 
        gs_om -t kerberos -m install -U {self.dbuser_node.ssh_user}\
         --krb-client'''
        ...
        self.assertIn("Successfully install Kerberos", msg2)

        self.logger.info('-----------关闭集群内安装kerberos认证-----------')
        check_cmd3 = f'''
        source {macro.DB_ENV_PATH};
        ...
        self.assertIn('Successfully uninstall Kerberos', msg3)
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0089finish--')
