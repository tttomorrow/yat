...
"""
Case Type   : 服务端工具
Case Name   : 查询指定服务器数据库状态时，并将数据库状态信息输出到指定的output文件
Description :
    1.查询指定服务器数据库状态时，并将数据库状态信息输出到指定的output文件
    2.查看日志文件是否生成
    3.删除输出文件
Expect      :
    1.查询成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0064start--')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.commonsh = CommonSH()

    def test_server_tools1(self):
        self.logger.info('----查询主机名称----')
        check_cmd = f'hostname'
        self.logger.info(check_cmd)
        hostname = self.dbuser_node.sh(check_cmd).result()
        self.logger.info(hostname)
        self.logger.info('----步骤1：查询指定服务器数据库状态时，并将数据库状态信息输出到指定的output文件----')
        ...
        self.logger.info('------步骤2：查询是否结果输出到指定文件-------')
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0064finish--')
