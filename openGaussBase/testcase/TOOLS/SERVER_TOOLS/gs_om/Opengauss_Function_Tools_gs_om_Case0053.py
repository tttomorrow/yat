...
"""
Case Type   : 服务端工具
Case Name   : 关闭数据库，设置关闭模式和日志路径
Description :
    1.查看数据库状态
    2.关闭数据库时指定-l,并设置关闭模式为fast是否可以关闭成功
    3.启动数据库
    4.查看数据库状态
    5.关闭数据库时指定-l,并设置关闭模式为immediate是否可以关闭成功
    6.启动数据库
    7.查看数据库状态
Expect      :
    1.状态正常
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_om_Case0053start---')
    ...
    def test_server_tools1(self):
        ...
        self.log.info('---步骤2.1：关闭数据库,设置关闭模式为fast,并将日志放在指定路径下---')
        ...
        self.log.info(status_msg1)
        self.assertTrue("Degraded" in status_msg1 or "Normal" in status_msg1)
        self.log.info('---步骤5.1：关闭数据库,设置关闭模式为immediate,并将日志放在指定路径下---')
    ...
    def tearDown(self):
        ...
        self.log.info('--Opengauss_Function_Tools_gs_om_Case0053finish--')
