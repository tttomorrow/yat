...
"""
Case Type   : 服务端工具
Case Name   : 查询数据库状态时，结果输出到指定文件
Description :
    1.查询数据库状态时，结果输出到指定文件
    2.查看输出文件是否生成
    3.删除输出文件
Expect      :
    1.查询成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0060start--')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.commonsh = CommonSH()

    def test_server_tools1(self):
        self.logger.info('----步骤1：查询数据库状态时，结果输出到指定文件----')
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0060finish--')
