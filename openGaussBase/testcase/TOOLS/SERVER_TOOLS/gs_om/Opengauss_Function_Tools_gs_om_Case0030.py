...
"""
Case Type   : 服务端工具
Case Name   : 数据库启动时，指定服务器名称和设置超时时间
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.关闭数据库：gs_om -t stop;
    3.启动时，指定服务器名称并设置超时时间;
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_om_Case0030start---')
    ...
    def test_server_tools1(self):
        ...
        self.log.info('---数据库启动时，指定服务器名称和设置超时时间---')
        start_cmd = f'source {macro.DB_ENV_PATH}; ' \
            f'gs_om -t start -h {hostname} --time-out=268;'
    ...
    def tearDown(self):
        ...
        self.log.info(start_cmd)
        self.log.info('--Opengauss_Function_Tools_gs_om_Case0030finish--')
