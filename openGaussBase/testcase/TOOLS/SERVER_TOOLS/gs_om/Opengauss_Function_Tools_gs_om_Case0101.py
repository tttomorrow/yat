...
"""
Case Type   : 服务端工具
Case Name   : 级联备主备切换--switchover将级联备升成备机后,再次failover将备机升成主机,查询数据库状态及节点信息
Description :
    1.级联备切换为备机
    2.停掉主机，将备机切换为主机
    3.结合--detail参数，查看数据库状态及节点详细信息，校验是否有port字段且端口正确
    4.结合--all参数，查看数据库状态及所有节点信息，校验是否有port字段且端口正确
    5.结合-h,-o参数，查看指定数据库节点状态及信息，校验是否有port字段且端口正确
    6.query查询数据状态及相关信息，校验是否有port字段且端口正确
    7.恢复原有集群状态
    8.清理环境，删除文件信息
Expect      :
    1.级联备机切换为备机成功
    2.备机切换为主机成功
    3.显示状态详细信息，存在port字段且端口号正确
    4.显示数据库所有节点信息，存在port字段且端口号正确
    5.显示指定节点状态信息，存在port字段且端口号正确
    6.显示数据库状态信息，存在port字段且端口号正确
    7.恢复集群原有状态成功
    8.清理环境成功
History     :
"""
...
class Tools(unittest.TestCase):
    ...
    def test_tools_om(self):
        if self.node_num > 2:
            ...
            text = '-----step2:停掉主机,将备机切换为主机;except:主备机切换成功-----'
            self.log.info(text)
            stop_result = Primary_SH.execute_gsctl(
                'stop', self.constant.GS_CTL_STOP_SUCCESS_MSG, '-M primary')
            self.assertTrue(stop_result, '执行失败' + text)
            failover_result = self.comshsta[0].execute_gsctl(
                'failover', self.constant.FAILOVER_SUCCESS_MSG)
            self.assertTrue(failover_result, '执行失败' + text)
            result = self.comshsta[0].exec_refresh_conf()
            self.assertTrue(result, '执行失败' + text)
            start_result = Primary_SH.execute_gsctl(
                'start', self.constant.REBUILD_SUCCESS_MSG, '-M standby')
