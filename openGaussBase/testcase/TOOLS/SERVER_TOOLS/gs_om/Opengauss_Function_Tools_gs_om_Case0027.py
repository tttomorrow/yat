...
"""
Case Type   : 服务端工具
Case Name   : 数据库启动时，指定不以安全模式启动数据库
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.停止数据库：gs_om -t stop;
    3.启动时，不以安全模式启动:gs_om -t start --security-mode=off;
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0027start--')
    ...
    def test_server_tools1(self):
        ...
        self.logger.info('----------启动时，以安全模式启动-----------')
        start_cmd = f'''source {macro.DB_ENV_PATH};
            gs_om -t start --security-mode=off;
            '''
    ...
    def tearDown(self):
        ...
        self.logger.info(start_cmd)
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0027finish--')
