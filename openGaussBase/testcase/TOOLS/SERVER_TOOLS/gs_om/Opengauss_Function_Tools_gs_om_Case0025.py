...
"""
Case Type   : 服务端工具
Case Name   : 数据库启动时，设置--time-out=SECS超时时间
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.停止数据库：gs_om -t stop;
    3.启动时，设置超时时间:gs_om -t start --time-out=-100.23;
    4.启动时，设置超时时间:gs_om -t start --time-out=302;
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0025start--')
    ...
    def test_server_tools1(self):
        ...
        self.logger.info('----------启动时，设置超时时间异常----------')
        self.error_msg = "The parameter '--time-out' type should be" \
                         " a nonnegative integer"
        start_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_om -t start --time-out=-100.23;
            '''
        self.logger.info(start_cmd1)
        start_msg1 = self.dbuser_node.sh(start_cmd1).result()
        self.logger.info(start_msg1)
        self.assertIn(self.error_msg, start_msg1)
        self.logger.info('------------启动时，设置超时时间正常-------------')
        start_cmd2 = f'''source {macro.DB_ENV_PATH};
            gs_om -t start --time-out=100;
            '''
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0025finish--')
