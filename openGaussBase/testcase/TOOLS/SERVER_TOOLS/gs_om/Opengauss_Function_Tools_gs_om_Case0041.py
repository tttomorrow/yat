...
"""
Case Type   : 服务端工具
Case Name   : 数据库关闭时，不指定日志存放路径
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.关闭数据库时，不指定日志文件存放路径：gs_om -t stop;
    3.在默认路径下，查看日志
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_om_Case0041start---')
    ...
    def test_server_tools1(self):
        ...
        self.log.info('----------数据库关闭时，不指定日志存放路径----------')
    ...
    def tearDown(self):
        ...
        self.log.info(start_cmd)
        self.log.info('--Opengauss_Function_Tools_gs_om_Case0041finish--')
