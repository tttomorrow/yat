...
"""
Case Type   : 服务端工具
Case Name   : 数据库关闭时，指定服务器名称和设置超时时间
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.关闭数据库指定-h参数的值并设置超时时间是否可以关闭成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_om_Case0050start---')
    ...
    def test_server_tools1(self):
        ...
        self.log.info('----------关闭数据库指定-h参数的值并设置超时时间是否可以关闭成功----------')
    ...
    def tearDown(self):
        ...
        self.log.info(start_cmd)
        self.log.info('--Opengauss_Function_Tools_gs_om_Case0050finish--')