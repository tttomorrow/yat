...
"""
Case Type   : 服务端工具
Case Name   : 数据库启动时，指定服务器名称和日志存放路径和dn路径和设置超时时间和是否以安全模式启动数据库
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.关闭数据库：gs_om -t stop;
    3.启动时，指定服务器名称，dn路径，超时时间，日志文件存放路径并以安全模式启动
    4.在指定路径下，查看日志
    5.查看数据库状态：gs_om -t status --detail;
    6.关闭数据库：gs_om -t stop;
    7.启动时，指定服务器名称，dn路径，超时时间，日志文件存放路径并且不以安全模式启动
    8.在指定路径下，查看日志
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_om_Case0037start---')
    ...
    def test_server_tools1(self):
        ...
        self.log.info('---启动时，指定服务器名称，dn路径，超时时间，日志文件存放路径并以安全模式启动---')
        start_cmd = f'source {macro.DB_ENV_PATH};' \
            f'gs_om -t start -h {hostname} ' \
            f'-l {macro.DB_INSTANCE_PATH}/omlog.log ' \
        ...
        self.log.info('---启动时，指定服务器名称，dn路径，超时时间，日志文件存放路径并且不以安全模式启动---')
        start_cmd = f'source {macro.DB_ENV_PATH};' \
            f'gs_om -t start -h {hostname} ' \
            f'-l {macro.DB_INSTANCE_PATH}/omlog.log ' \
            f'-D {macro.DB_INSTANCE_PATH} ' \
    ...
    def tearDown(self):
        ...
        self.log.info(start_cmd)
        self.log.info('--Opengauss_Function_Tools_gs_om_Case0037finish--')
