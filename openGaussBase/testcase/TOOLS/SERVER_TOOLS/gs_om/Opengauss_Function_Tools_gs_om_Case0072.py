...
"""
Case Type   : 服务端工具
Case Name   : 查询数据库所有节点的信息,并生成日志文件到指定路径
Description :
    1.查询数据库所有节点的信息,并生成日志文件到指定路径
    2.查看日志文件是否生成
    3.删除日志文件
Expect      :
    1.查询成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        Log.info('--Opengauss_Function_Tools_gs_om_Case0072start--')
        self.dbusernode = Node('dbuser')
        self.constant = Constant()
        self.commonsh = CommonSH()

    def test_server_tools1(self):
        Log.info('------查询数据库所有节点的信息,并生成日志文件到指定路径-------')
    ...
    def tearDown(self):
        ...
        Log.info(clear_msg)
        Log.info('--Opengauss_Function_Tools_gs_om_Case0072finish--')