...
"""
Case Type   : 服务端工具
Case Name   : 查询指定服务器状态的详细信息的同时生成日志文件并将结果输出到指定文件
Description :
    1.查询指定服务器状态的详细信息的同时生成日志文件并将结果输出到指定文件
    2.查看日志文件是否生成
    3.查看输出文件是否生成
    4.删除日志文件
    5.删除输出文件
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0081start--')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.commonsh = CommonSH()

    def test_server_tools1(self):
        self.logger.info('------查询指定服务器状态的详细信息的同时生成日志文件并将结果输出到指定文件-------')
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0081finish--')
