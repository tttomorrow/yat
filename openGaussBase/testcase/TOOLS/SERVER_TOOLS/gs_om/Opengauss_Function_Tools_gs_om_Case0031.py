...
"""
Case Type   : 服务端工具
Case Name   : 数据库启动时，指定服务器名称和是否以安全模式启动数据库
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.关闭数据库：gs_om -t stop;
    3.启动时，指定服务器名称以安全模式启动
    4.查看数据库状态：gs_om -t status --detail;
    5.关闭数据库：gs_om -t stop;
    6.启动时，指定服务器名称不以安全模式启动
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_om_Case0031start---')
    ...
    def test_server_tools1(self):
        ...
        self.log.info('---启动时，指定服务器名称以安全模式启动---')
        start_cmd = f'source {macro.DB_ENV_PATH}; ' \
            f'gs_om -t start -h {hostname} --security-mode=on;'
        ...
        self.log.info('---启动时，指定服务器名称以非安全模式启动---')
        start_cmd = f'source {macro.DB_ENV_PATH}; ' \
            f'gs_om -t start -h {hostname} --security-mode=off;'
        self.log.info(start_cmd)
        start_msg = self.dbuser_node.sh(start_cmd).result()
        self.log.info(start_msg)
        self.assertIn(self.constant.GS_OM_START_SUCCESS_MSG, start_msg)
    ...
    def tearDown(self):
        ...
        self.log.info(start_cmd)
        self.log.info('--Opengauss_Function_Tools_gs_om_Case0031finish--')
