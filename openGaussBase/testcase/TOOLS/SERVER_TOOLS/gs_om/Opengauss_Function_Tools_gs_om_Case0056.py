...
"""
Case Type   : 服务端工具
Case Name   : 数据库关闭时，指定--mode和--time-out
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.关闭数据库时设置超时时间并设置关闭模式为fast是否可以关闭成功
    3.查看数据库状态：gs_om -t status --detail
    4.启动数据库：gs_om -t start
    5.查看数据库状态：gs_om -t status --detail
    6.关闭数据库时设置超时时间并设置关闭模式为immediate是否可以关闭成功
    7.查看数据库状态：gs_om -t status --detail
    8.启动数据库：gs_om -t start
    9.查看数据库状态：gs_om -t status --detail
Expect      :
    1.状态正常
    2.关闭数据库
    3.状态不可用
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0056start--')
    ...
    def test_server_tools1(self):
        ...
        self.logger.info('--------关闭数据库时设置超时时间并设置关闭模式为fast是否可以关闭成功---------')
        ...
        stop_cmd1 = f'''source {macro.DB_ENV_PATH}; 
            gs_om -t stop --time-out=306 -m fast;
            '''
        self.logger.info(stop_cmd1)
        stop_msg1 = self.dbuser_node.sh(stop_cmd1).result()
        self.logger.info(stop_msg1)
        self.logger.info('------------------启动数据库------------------')
        start_cmd1 = self.commonsh.start_db_cluster()
        self.logger.info(start_cmd1)
        self.assertTrue(start_cmd1)

        self.logger.info('------------------查询数据库状态------------------')
        status_cmd3 = self.commonsh.get_db_cluster_status()
        self.logger.info(status_cmd3)
        self.assertTrue("Degraded" in status_cmd3 or "Normal" in status_cmd3)

        self.logger.info('------关闭数据库时设置超时时间并设置关闭模式为immediate是否可以关闭成功------')
        certificate_path = os.path.join(macro.DB_INSTANCE_PATH, 'server.*')
        ls_cmd = f"ls -l {certificate_path}"
        self.logger.info(ls_cmd)
        stop_cmd2 = f'''source {macro.DB_ENV_PATH}; 
            gs_om -t stop --time-out=300 --mode=immediate;
            '''
    ...
    def tearDown(self):
        ...
        self.logger.info('--Opengauss_Function_Tools_gs_om_Case0056finish--')
