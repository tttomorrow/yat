...
"""
Case Type   : 服务端工具
Case Name   : 数据库启动时，指定服务器名称和日志存放路径
Description :
    1.查看数据库状态：gs_om -t status --detail
    2.关闭数据库：gs_om -t stop;
    3.启动时，指定服务器名称和日志文件存放路径
    4.在指定路径下，查看日志
    5.查看数据库状态
    6.删除日志文件
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_om_Case0028start---')
    ...
    def test_server_tools1(self):
        ...
        self.log.info('---启动时，指定服务器名称和日志文件存放路径---')
        ...
        start_cmd = f'source {macro.DB_ENV_PATH}; ' \
            ...
            f'{macro.DB_INSTANCE_PATH}/omlog.log;'
    ...
    def tearDown(self):
        ...
        self.log.info(start_cmd)
        self.log.info('--Opengauss_Function_Tools_gs_om_Case0028finish--')
