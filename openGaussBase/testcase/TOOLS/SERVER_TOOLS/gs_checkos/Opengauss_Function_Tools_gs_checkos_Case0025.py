...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_checkos_Case0025start-------------------')
    ...
    def test_server_tools(self):
        ...
        if len(str_1) > 2:
            ...
            self.assertEqual(str_list[0].strip(), 'B1. [ Set system control parameters ]', '结果验证失败3')
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_checkos_Case0025finish------------------')