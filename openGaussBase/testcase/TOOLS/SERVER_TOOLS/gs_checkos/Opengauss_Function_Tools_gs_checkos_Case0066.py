...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_checkos_Case0066start-------------------')
    ...
    def test_server_tools(self):
        ...
        checkos_cmd = f'''
                            ...
                            gs_checkos -i  A1,A2,A3,A4,A5,A6,A7,A8
                            '''
        ...
        self.assertEqual(len(num_list), 8)
        self.assertIn(num_list[0],self.Constant.GS_CHECKOS_SUCCESS_MSG1)
        self.assertEqual(len(status_list),8)
        for i in status_list:
            self.assertTrue(i in ['Normal', 'Warning'], f'状态验证失败，当前状态：{i}')
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_checkos_Case0066finish------------------')
