...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_checkos_Case0012start-------------------')
    ...
    def test_server_tools(self):
        ...
        if len(str_1) > 2:
            ...
            self.assertEqual(str_list[0].strip(), 'A2. [ Kernel version status ]', '结果验证失败3')
            i = str_list[1].strip()
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_checkos_Case0012finish------------------')