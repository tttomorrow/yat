...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_checkos_Case0013start-------------------')
        self.rootNode = Node('default')
        self.Constant = Constant()

    def test_server_tools(self):
        logger.info('------------------root用户检查Unicode状态------------------')
        checkos_cmd = f'''
        ...
        if len(str_1) > 2:
            ...
            self.assertEqual(str_list[0].strip(), 'A3. [ Unicode status ]', '结果验证失败3')
            i = str_list[1].strip()
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_checkos_Case0013finish------------------')