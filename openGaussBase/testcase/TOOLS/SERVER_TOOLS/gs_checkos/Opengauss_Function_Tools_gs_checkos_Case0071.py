...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('---Opengauss_Function_Tools_gs_checkos_Case0071start---')
        self.rootNode = Node('default')
        self.Constant = Constant()

    def test_server_tools(self):
        logger.info('---root用户检查操作系统版本，内核版本,Unicode状态,时区\
状态,交换内存状态,系统控制参数,文件系统配置状态,磁盘配置状态,预读块大小状态,\
IO调度状态,网卡配置状态,时间一致性,防火墙状态---')
        checkos_cmd = f'''
            source {macro.DB_ENV_PATH}
            gs_checkos -i  A1,A2,A3,A4,A5,A6,A7,A8,A9,A10,A11,A12,A13
                            '''
        ...
        self.assertEqual(len(num_list), 13)
        self.assertIn(num_list[0], self.Constant.GS_CHECKOS_SUCCESS_MSG1)
        self.assertEqual(len(status_list), 13)
...
失败，当前状态：{i}')
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('--Opengauss_Function_Tools_gs_checkos_Case0071finish--')
