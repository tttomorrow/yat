...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0023start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('---------只导出数据，不输出模式(数据定义)----------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test --format=p -a ; '''
        ...
        self.assertIn('2', msg3)
        self.assertIn('aa', msg3)
        self.assertIn('bb', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0023finish------')
