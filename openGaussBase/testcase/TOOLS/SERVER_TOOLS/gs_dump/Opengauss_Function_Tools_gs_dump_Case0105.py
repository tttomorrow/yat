...
"""
Case Type   : 服务端工具
Case Name   : 修改dump导出的数据文件后,再导入(自定义归档格式:在导出的文件末尾添加任意字符串)
Description :
    1.创建数据；
    2.执行导出操作：
    ...
    3.在导出的文件末尾添加任意字符串
    4.导入修改后的文件
    gs_restore -p port -d databasename -f dump1.txt
    5.清理环境：
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_Tools_gs_dump_Case0105start---')
        ...
        self.dump_path2 = os.path.join(
            os.path.dirname(macro.DB_INSTANCE_PATH), 'dump2.txt')
        self.db_name1 = "db_dump0105_01"
        self.db_name2 = "db_dump0105_02"
        self.tb_name = "t_dump0105"
    ...
    def test_tools_dump(self):
        ...
        text = '-----step3:在导出的文件末尾添加任意字符串(不影响原来文件内容);expect:修改成功-----'
        self.log.info(text)
        sed_cmd = f"sed '$a qweweewrrcxcbgvhzjggsdght1436t5yfraca2agcsss" \
            f"case modified success' {self.dump_path1} > {self.dump_path2};"
        self.log.info(sed_cmd)
        sed_result = self.Primary_Node.sh(sed_cmd).result()
        self.log.info(sed_result)

        text = '---step4:导入修改后的文件(不影响原来文件内容);expect:导入成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '------Opengauss_Function_Tools_gs_dump_Case0105finish------')
