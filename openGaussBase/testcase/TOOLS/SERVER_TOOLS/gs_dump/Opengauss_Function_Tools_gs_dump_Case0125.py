...
"""
Case Type   : 服务端工具
Case Name   : 创建兼容的数据库类型为C的数据库,然后导出数据,是否可以正常导出
Description :
    1.连接数据库，创建兼容C格式的数据库
    create database databasename dbcompatibility 'C';
    2.在创建好的数据库中,创建数据
    3.连接创建好的数据库,导出数据
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Tools_gs_dump_Case0125start---')
        ...
        self.db_name1 = "db_dump0125_01"
        self.db_name2 = "db_dump0125_02"
        self.tb_name = "t_dump0125"

    def test_tools_dump(self):
        text = '---step1:创建兼容的数据库类型为C的数据库;expect:创建成功---'
        self.log.info(text)
        sql_cmd = self.pri_sh.execut_db_sql(f'''
            drop database if exists {self.db_name1};
            drop database if exists {self.db_name2};
            create database {self.db_name1} dbcompatibility 'C';
        ...
        self.assertIn('CREATE TABLE', gsql_result, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
        '------Opengauss_Function_Tools_gs_dump_Case0125finish------')