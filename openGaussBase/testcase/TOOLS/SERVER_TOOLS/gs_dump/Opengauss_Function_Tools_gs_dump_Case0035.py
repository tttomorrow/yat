...
"""
Case Type   : 服务端工具
Case Name   : 导出的文件中没有与-N后的模式相匹配的模式（有-N没有-n）
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
    ...
    7.退出数据库
    8.source环境变量
    9.导出的文件中没有与-N后的模式相匹配的模式（有-N没有-n）
    10.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
    ...
    7.退出数据库
    8.source环境变量
    9.导出schema1,schema3本身和所有它包含的对象成功
    10.清理环境成功
History     :
"""
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0035start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--导出的文件中没有与-N后的模式相匹配的模式（有-N没有-n）--')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p -N schema2;
                      '''
        LOG.info(excute_cmd3)
        msg3 = self.dbuser_node.sh(excute_cmd3).result()
        LOG.info(msg3)
        self.assertIn('CREATE SCHEMA schema1', msg3)
        self.assertIn('CREATE SCHEMA schema3', msg3)
        self.assertIn('CREATE TABLE test1 ', msg3)
        self.assertIn('CREATE TABLE test3 ', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0035finish---')
