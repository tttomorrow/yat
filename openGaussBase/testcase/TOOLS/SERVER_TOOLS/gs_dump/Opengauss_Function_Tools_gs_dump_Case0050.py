...
Description :
    ...
    5.退出数据库
    6.source环境变
    7.以INSERT命令带列名（INSERT INTO表（列、…）值…）方式导出数据
    8.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_dump_Case0050start---')
    ...
    def test_server_tools(self):
        ...
        LOG.info('------以INSERT命令带列名（INSERT INTO表\
        （列、…）值…）方式导出数据--------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p --attribute-inserts;
                      '''
        ...
        self.assertIn('INSERT INTO t1 (i) VALUES (1);', msg3)
        self.assertIn('INSERT INTO t2 (id) VALUES (8);', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0050finish-----')
