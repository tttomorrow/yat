...
"""
Case Type   : 服务端工具
Case Name   : 指定转储数据需用AES128进行加密，密钥长度大于16字节
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
    ...
    5.退出数据库
    6.source环境变量
    7.指定转储数据需用AES128进行加密，密钥长度大于16字节
    8.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0092start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('---指定转储数据需用AES128进行加密，密钥长度大于16字节---')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p\
          --with-encryption=AES128 --with-key=789qaz@wsx123!qwer;
                      '''
        LOG.info(excute_cmd3)
        msg3 = self.dbuser_node.sh(excute_cmd3).result()
        LOG.info(msg3)
        self.assertIn('The key is illegal,the length must be 16', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----opengauss_function_tools_gs_dump_case0092finish----')
