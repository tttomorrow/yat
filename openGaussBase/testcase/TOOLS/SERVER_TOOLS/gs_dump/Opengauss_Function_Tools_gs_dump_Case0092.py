...
"""
Case Type   : 服务端工具
Case Name   : 导出数据需用AES128进行加密，密钥长度大于16字节
Description :
    1.创建测试数据
    2.导出数据需用AES128进行加密，密钥长度大于16字节
    3.清理环境
Expect      :
    1.创建测试数据成功
    2.导出失败
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----opengauss_function_tools_gs_dump_case0092_start------')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()
        self.assert_msg = f'The input key must be 8~16 bytes and contain ' \
            f'at least three kinds of characters!'
        self.key = f'789qaz@wsx123qwer！'
        self.db_name = 'db_dump_0092'
        self.t_name = 't_dump_0092'
    ...
    def test_server_tools(self):
        ...
        text = '----step2:导出数据需用AES128进行加密，密钥长度大于16字节;expect:导出失败----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.DROP_DATABASE_SUCCESS, msg4,
                      '执行失败:' + text)
        self.log.info(
            '----opengauss_function_tools_gs_dump_case0092_finish----')
