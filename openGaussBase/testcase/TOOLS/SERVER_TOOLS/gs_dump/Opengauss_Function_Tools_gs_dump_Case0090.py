...
"""
Case Type   : 服务端工具
Case Name   : 以INSERT命令带列名的方式导出指定的表
Description :
    1.连接数据库：
    2.创建数据库test：
    3.切换到数据库test
    ...
    5.退出数据库
    6.source环境变量
    7.以INSERT命令带列名（INSERT INTO表（列、…）值…）方式导出指定的表
    8.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
    ...
    5.退出数据库
    6.source环境变量
    7.以INSERT命令带列名（INSERT INTO表（列、…）值…）方式导出指定的表成功
    8.清理环境成功
History     :
"""
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0090start-----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('---------以INSERT命令带列名的方式导出指定的表----------')
        ...
        self.assertIn('INSERT INTO t1 (i, d) VALUES (1, 2);', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0090finish----')
