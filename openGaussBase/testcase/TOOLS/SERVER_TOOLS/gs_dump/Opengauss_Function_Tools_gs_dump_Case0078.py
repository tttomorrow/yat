...
"""
Case Type   : 服务端工具
Case Name   : 导出数据时单独使用--binary-upgrade-usermap参数
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
    ...
    5.退出数据库
    6.source环境变
    7.导出数据时单独使用--binary-upgrade-usermap参数
    8.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0078start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-----导出数据时单独使用--binary-upgrade-usermap参数----')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p \
        --binary-upgrade-usermap="u1=u2";
                    '''
        LOG.info(excute_cmd3)
        msg3 = self.dbuser_node.sh(excute_cmd3).result()
        LOG.info(msg3)
        self.assertIn('gs_dump: options --binary-upgrade-usermap should be \
used with --binary-upgrade option', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----opengauss_function_tools_gs_dump_case0078finish----')
