...
"""
Case Type   : 服务端工具
Case Name   : 指定转储数据需用AES128进行加密，密钥长度小于16字节
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
    ...
    5.退出数据库
    6.source环境变量
    7.指定转储数据需用AES128进行加密，密钥长度小于16字节
    8.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0091start------')
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----opengauss_function_tools_gs_dump_case0091finish----')
