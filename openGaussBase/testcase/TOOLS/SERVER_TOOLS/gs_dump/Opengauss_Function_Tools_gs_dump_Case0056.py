...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0056start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('----在创建好的数据库中创建表插入数据,删除列----')
        sql_cmd2 = ''' 
                drop table  if exists t1;
                create table t1 (i int,d int );
                insert into t1 values(1,2),(2,3),(3,4);
                alter table t1 drop column if exists  i;
                '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('---opengauss_function_tools_gs_dump_case0056finish---')
