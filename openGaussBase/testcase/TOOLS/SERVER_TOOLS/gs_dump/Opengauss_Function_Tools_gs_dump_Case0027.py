...
"""
Case Type   : 服务端工具
Case Name   : 导出创建数据库对象的指令时不指定/指定清理指令
Description :
    1.连接数据库：
    2.创建数据库test：
    3.切换到数据库test
    ...
    5.退出数据库
    6.source环境变量
    7.导出创建数据库对象的指令时不指定清理指令
    8.导出创建数据库对象的指令时指定清理指令
    9.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0027start----')
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0027finish----')
