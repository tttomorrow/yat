...
"""
Case Type   : 服务端工具
Case Name   : 备份/还原操作是否影响导出(gs_backup)
Description :
    1.会话1创建大量数据
    2.创建备份文件保存路径
    ...
    gs_backup -t backup --backup-dir=/data/backup;
    5.再次在会话1执行导出操作
    gs_dump database -p port -f /data/dump2;
    6.在会话1执行导出操作的同时在会话2执行还原操作
    gs_backup -t restore --backup-dir=/data/backup;
    7.清理环境
    drop database dump;
    rm -rf /data/dump1;
    rm -rf /data/dump2;
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-------Opengauss_Function_Tools_gs_dump_Case0111start------')
        ...
        self.gs_backup_path = os.path.join(
            os.path.dirname(macro.DB_INSTANCE_PATH), 'gs_backup')
        self.db_name = "db_dump0111"
        self.tb_name = "t_dump0111"
    ...
    def test_tools_dump(self):
        ...
        self.log.info(text)
        is_dir_exists_cmd = f'''if [ ! -d "{self.gs_backup_path}" ]
                                               then
                                                   mkdir {self.gs_backup_path}
                                               fi'''
        ...
        gs_backup_cmd = f'source {macro.DB_ENV_PATH};' \
            f'gs_backup -t backup ' \
            f'--backup-dir={self.gs_backup_path};'
        self.log.info(gs_backup_cmd)
        connect_thread2 = ComThread(
            self.com.get_sh_result, args=(self.Primary_Node2, gs_backup_cmd))
        ...
        self.assertIn(self.constant.gs_backup_success,
                      thread2_result,
                      '执行失败:' + text)

        text = '---step5:再次在会话1执行导出操作;expect:导出成功---'
        self.log.info(text)
        dump_cmd = f'''source {macro.DB_ENV_PATH};\
                    gs_dump {self.db_name} \
                    -p {self.Primary_Node1.db_port} \
        ...
        text = '-----step6:在会话1执行导出操作的同时在会话2执行备份还原操作;expect:还原成功-----'
        self.log.info(text)
        gs_backup_cmd = f'source {macro.DB_ENV_PATH};' \
            f'gs_backup -t restore ' \
            f'--backup-dir={self.gs_backup_path};'
        self.log.info(gs_backup_cmd)
        connect_thread2 = ComThread(
            self.com.get_sh_result, args=(self.Primary_Node2, gs_backup_cmd))
        ...
        thread2_result = connect_thread2.get_result()
        self.log.info(thread2_result)
        self.assertIn(self.constant.gs_backup_restore_success,
                      thread2_result,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '------Opengauss_Function_Tools_gs_dump_Case0111finish------')
