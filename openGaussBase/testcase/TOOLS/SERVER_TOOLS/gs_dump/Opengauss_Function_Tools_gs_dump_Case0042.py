...
Expect      :
    ...
    7.退出数据库
    8.source环境变量
    9.导出表t1成功（当使用-t时，-N没有任何效应，由-t选择的表的转储不受影响）
    10.清理环境成功
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0042start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-------导出指定的表时指定不要导出的模式--------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
         gs_dump -p {self.dbuser_node.db_port} test -F p -t schema1.t1\
          -t schema2.t2 -N schema3;
                      '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0042finish-----')
