...
"""
Case Type   : 服务端工具
Case Name   : 输出符合SQL标准的SET SESSION AUTHORIZATION命令来确定对象所有权
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
    ...
    5.退出数据库
    6.source环境变
    7.输出符合SQL标准的SET SESSION AUTHORIZATION命令来确定对象所有权
    8.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0062start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-------------输出符合SQL标准的SET SESSION AUTHORIZATION命令\
        来确定对象所有权-------------')
    ...
    def teardown(self):
        ...
        LOG.info(msg4)
        LOG.info('----opengauss_function_tools_gs_dump_case0062finish----')
