...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('------Opengauss_Function_Tools_gs_dump_Case0053start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('---在创建好的数据库中创建表，插入数据并设置所属表空间---')
        sql_cmd2 = '''
                    drop tablespace  if exists tbspace; 
                    create tablespace tbspace relative \
                    location 'tbspace_qm/tbspace';
                    ...
                    create table t3 (i int ) tablespace tbspace;
                    insert into t3 values(3),(6),(9);
                '''
        ...
        LOG.info('----------------不输出选择表空间的命令---------------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
         gs_dump -p {self.dbuser_node.db_port} test -F p --no-tablespaces;
                      '''
        ...
        self.assertIn('-- Name: t3; Type: TABLE; Schema: public; \
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0053finish----')
