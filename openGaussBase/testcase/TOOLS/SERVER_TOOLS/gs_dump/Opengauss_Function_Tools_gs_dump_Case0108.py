...
"""
Case Type   : 服务端工具
Case Name   : 修改dump导出的数据文件后，再导入(目录归档格式:*.dat.gz文件)
Description :
    1.创建数据
    2.执行导出操作
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(
            '---Opengauss_Function_Tools_gs_dump_Case0108start---')
        ...
        self.db_name1 = "db_dump0108_01"
        self.db_name2 = "db_dump0108_02"
        self.tb_name = "t_dump0108"
    ...
    def test_tools_dump(self):
        ...
        sed_cmd = f"sed -i '$a qweweewrrcxcbgvhzjggsdggcsss" \
            f"case modified success' {self.dump_dir1}/*.dat.gz ;"
        ...
        self.log.info(text)
        sed_cmd = f"sed -i '3,$d' {self.dump_dir2}/*.dat.gz;"
        self.log.info(sed_cmd)
        sed_result = self.Primary_Node.sh(sed_cmd).result()
        self.log.info(sed_result)

        text = '---step4.2:导入修改后的文件(影响原来文件内容);expect:导入成功---'
        self.log.info(text)
        gsql_cmd = f"source {macro.DB_ENV_PATH};" \
            f"gs_restore -d {self.db_name2} " \
            f"-p {self.Primary_Node.db_port} " \
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '------Opengauss_Function_Tools_gs_dump_Case0108finish------')
