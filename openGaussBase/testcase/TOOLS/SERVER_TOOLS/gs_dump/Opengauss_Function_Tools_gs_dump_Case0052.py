...
"""
Case Type   : 服务端工具
Case Name   : 以INSERT命令（而非COPY命令）转储数据
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('------Opengauss_Function_Tools_gs_dump_Case0052start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-------以INSERT命令（而非COPY命令）转储数据----------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p --inserts;
                      '''
        ...
        self.assertIn('INSERT INTO t1 VALUES (1);', msg3)
        self.assertIn('INSERT INTO t2 VALUES (8);', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0052finish-----')
