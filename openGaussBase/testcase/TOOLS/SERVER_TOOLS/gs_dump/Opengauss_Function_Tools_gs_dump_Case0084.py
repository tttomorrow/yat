...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0084start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-----使用指定的用户名连接（出现密码提示）-----')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH};
            expect <<EOF
            set timeout -1
            spawn gs_dump -p {self.dbuser_node.db_port} test -F p -U qaz
            expect "Password:"
            send "qwer@123\r"
            expect EOF
            '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----opengauss_function_tools_gs_dump_case0084finish----')
