...
"""
Case Type   : 服务端工具
Case Name   : 修改dump导出的数据文件后,再导入(tar归档格式:在导出的文件末尾添加任意字符串)
Description :
    1.创建数据；
    2.执行导出操作：
    gs_dump databasename -p port  -F t -f dump1.tar
    3.修改导出文件的部分内容
    4.导入修改后的文件
    gs_restore -p port -d databasename -c dump1.tar
    5.清理环境：
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Tools_gs_dump_Case0110start---')
        ...
        self.db_name1 = "db_dump0110_01"
        self.db_name2 = "db_dump0110_02"
        self.tb_name = "t_dump0110"
    ...
    def test_tools_dump(self):
        ...
        sed_cmd = f"sed -i '$a qweweewrrcxcbgvhzjggsdggcsss" \
            f"case modified success' {self.dump_path};"
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '------Opengauss_Function_Tools_gs_dump_Case0110finish------')
