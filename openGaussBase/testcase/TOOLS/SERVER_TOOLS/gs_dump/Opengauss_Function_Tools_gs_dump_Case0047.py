...
"""
Case Type   : 服务端工具
Case Name   : 导出数据时，指定需要dump的表文件
Description :
    1.创建数据库testdump
    2.创建模式，创建表并插入数据，设置表的所属模式
    3.创建表文件
    4.导出数据时，指定需要dump的表文件
    5.连接数据库，清理环境
Expect      :
    1.创建数据库testdump成功
    2.创建模式成功,创建表并插入数据成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0047start-----')
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0047finish----')
