...
"""
Case Type   : 服务端工具
Case Name   : 指定已转储的名称区段为post-data，是否可以导出数据
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
    ...
    5.退出数据库
    6.source环境变量
    7.指定已转储的名称区段为post-data，是否可以导出数据
    8.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0060start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-------指定已转储的名称区段为post-data，是否可以导出数据---------')
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('---Opengauss_Function_Tools_gs_dump_Case0060finish---')
