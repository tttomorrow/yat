...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0040start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-----在创建好的数据库中创建模式，创建表并插入数据-----')
        sql_cmd2 = ''' 
        ...
        LOG.info('----------使用通配符导出指定的多个表----------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port}   test  -F p  -t 't*' ;
                      '''
        ...
        gs_dump -p {self.dbuser_node.db_port} test -F p -t t1 -t t2 -t t3 ;
                              '''
        ...
        self.assertIn('CREATE TABLE t2', msg4)
        self.assertIn('COPY t2', msg4)
        self.assertIn('CREATE TABLE t3', msg4)
        self.assertIn('COPY t3', msg4)
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0040finish----')
