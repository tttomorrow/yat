...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0020start-----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-------导出一个纯文本格式文件时设置压缩比级别--------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
         gs_dump -p {self.dbuser_node.db_port} test --format=p\
         -f {macro.DB_INSTANCE_PATH}/dump_qm -Z 9;
                      '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0020finis------')
