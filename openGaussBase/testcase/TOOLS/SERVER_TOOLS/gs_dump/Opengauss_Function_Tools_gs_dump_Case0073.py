...
"""
Case Type   : 服务端工具
Case Name   : 备份文件已存在，导出目录格式的文件是否导出成功
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
    ...
    6.source环境变量
    7.以目录格式导出（文件不存在）
    8.备份文件已存在，导出目录格式的文件是否导出成功（文件已存在）
    9.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0073start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-----以目录格式导出（文件不存在）-----')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F d\
        ...
        LOG.info('-----备份文件已存在，导出目录格式的文件是否导出成功-----')
        ...
        LOG.info(msg4)
        self.assertIn('File exists', msg4)
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('----opengauss_function_tools_gs_dump_case0073finish----')
