...
"""
Case Type   : 服务端工具
Case Name   : 导出数据需用AES128进行加密，指定密钥不合规则
Description :
    1.创建测试数据
    2.导出数据需用AES128进行加密，指定密钥不合规则
    3.清理环境
Expect      :
    1.创建数据成功
    2.导出失败
...
class Tools(unittest.TestCase):
    ...
    def test_server_tools(self):
        ...
        text = '----step2:导出数据需用AES128进行加密，指定密钥不合规则;expect:导出失败----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.DROP_DATABASE_SUCCESS, msg4,
                      '执行失败:' + text)
        self.log.info(
            '----opengauss_function_tools_gs_dump_case0065_finish----')
