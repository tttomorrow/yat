...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0097start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-----以文本格式导出指定表以外的数据-----')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p -a -T test1  ;
                      '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('---opengauss_function_tools_gs_dump_case0097finish---')
