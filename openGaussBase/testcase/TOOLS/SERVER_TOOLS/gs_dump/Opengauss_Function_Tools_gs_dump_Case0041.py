...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('------Opengauss_Function_Tools_gs_dump_Case0041start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('---------导出指定的表时指定要导出的模式---------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p -t\
         schema1.t1 -n schema2;
                      '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0041finish-----')
