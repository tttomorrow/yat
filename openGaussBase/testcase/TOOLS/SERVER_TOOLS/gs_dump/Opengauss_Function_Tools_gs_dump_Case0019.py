...
"""
Case Type   : 服务端工具
Case Name   : 导出一个tar格式的文件时设置压缩比级别
Description :
    1.连接数据库：
    2.创建数据库
    3.切换到数据库test
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info(
            '-------Opengauss_Function_Tools_gs_dump_Case0019start-------')
    ...
    def test_server_tools(self):
        ...
        LOG.info(msg3)
        self.assertIn('compression is not supported by tar archive format',
                      msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0019finish-----')
