...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0007start-----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--------指定导出的数据库时数据库名称前添加-d---------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
                        gs_dump -p {self.dbuser_node.db_port} -d test  -v
                                        '''
        LOG.info(excute_cmd3)
        msg3 = self.dbuser_node.sh(excute_cmd3).result()
        LOG.info(msg3)
        self.assertIn("invalid option -- 'd'", msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case007finish-----')
