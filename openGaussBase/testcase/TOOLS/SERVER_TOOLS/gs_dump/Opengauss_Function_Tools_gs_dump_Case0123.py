...
"""
Case Type   : 服务端工具
Case Name   : 创建兼容的数据库类型为A的数据库,然后导出数据,是否可以正常导出
Description :
    1.连接数据库,创建兼容A格式的数据库
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Tools_gs_dump_Case0123start---')
        ...
        self.db_name1 = "db_dump0123_01"
        self.db_name2 = "db_dump0123_02"
        self.tb_name = "t_dump0123"

    def test_tools_dump(self):
        text = '---step1:创建兼容的数据库类型为A的数据库;expect:创建成功---'
        self.log.info(text)
        sql_cmd = self.pri_sh.execut_db_sql(f'''
            drop database if exists {self.db_name1};
            drop database if exists {self.db_name2};
            create database {self.db_name1} dbcompatibility 'A';
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '------Opengauss_Function_Tools_gs_dump_Case0123finish------')
