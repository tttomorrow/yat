...
"""
Case Type   : 服务端工具
Case Name   : 以INSERT命令转储每个表的对象标识符
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0076start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-----以INSERT命令转储每个表的对象标识符-----')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p -o --inserts;
                      '''
        LOG.info(excute_cmd3)
        msg3 = self.dbuser_node.sh(excute_cmd3).result()
        LOG.info(msg3)
        self.assertIn('options --inserts/--column-inserts and -o/--oids \
cannot be used together', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----opengauss_function_tools_gs_dump_case0076finish----')
