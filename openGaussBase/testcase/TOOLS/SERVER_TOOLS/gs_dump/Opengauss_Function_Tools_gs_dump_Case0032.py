...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_dump_Case0032start---')
    ...
    def test_server_tools(self):
        ...
        LOG.info('-----------------导出与模式名称匹配的模式----------------')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
         gs_dump -p {self.dbuser_node.db_port} test --format=p -n schema1;
                      '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0032finish-----')
