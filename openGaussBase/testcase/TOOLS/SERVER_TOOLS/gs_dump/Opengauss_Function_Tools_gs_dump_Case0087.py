...
Expect      :
    ...
    5.创建表成功
    6.退出数据库
    7.使用指定的用户名连接：（出现密码提示：输入正确的用户密码并导出数据）
    8.清理环境成功
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----opengauss_function_tools_gs_dump_case0087start------')
    ...
    def tearDown(self):
        LOG.info('-----------------清理环境：删除数据库-----------------')
        sql_cmd4 = '''drop user role1; drop database if exists test; '''
        ...
        LOG.info(msg4)
        LOG.info('----opengauss_function_tools_gs_dump_case0087finish----')
