...
"""
Case Type   : 服务端工具
Case Name   : 导出数据库时在指定输出路径下输出一个目录格式文件
Description :
    1.连接数据库：
    2.创建数据库
    3.切换到数据库test
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0013start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('---导出数据库时在指定输出路径下输出一个目录格式文件---')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
                          gs_dump -p {self.dbuser_node.db_port}\
                          test --format=d -f {macro.DB_INSTANCE_PATH}/dump_qm;
                                                    '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0013finish-----')
