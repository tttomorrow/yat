...
"""
Case Type   : 服务端工具
Case Name   : 导出纯文本格式内容的数据库全量信息时指定用户和密码
Description :
    1.连接数据库：
    2.创建数据库
    3.切换到数据库test
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0024start-----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('---导出纯文本格式内容的数据库全量信息时指定用户和密码---')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port}   test  -F p\
        -U {self.dbuser_node.ssh_user}  -W {self.dbuser_node.ssh_password};
                            '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg4)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0024finish----')
