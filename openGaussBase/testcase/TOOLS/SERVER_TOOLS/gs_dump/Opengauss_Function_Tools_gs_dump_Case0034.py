...
"""
Case Type   : 服务端工具
Case Name   : 当同时输入-n和-N时不转储与-N模式名称匹配的模式
Description :
    1.连接数据库
    2.创建数据库test
    3.切换到数据库test
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0034start----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('----当同时输入-n和-N时不转储与-N模式名称匹配的模式----')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
        gs_dump -p {self.dbuser_node.db_port} test -F p -n schema1 -N schema2;
                      '''
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0034finish----')
