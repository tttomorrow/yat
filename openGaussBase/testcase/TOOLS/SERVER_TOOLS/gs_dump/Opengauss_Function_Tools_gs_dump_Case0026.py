...
"""
Case Type   : 服务端工具
Case Name   : 导出纯文本格式内容的数据库全量信息时指定数据库用户
Description :
    1.连接数据库：
    2.创建数据库
    3.切换到数据库test
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_dump_Case0026start---')
        self.dbuser_node = Node('dbuser')
        self.constant = Constant()

    def test_server_tools(self):
        LOG.info('------------------连接数据库,创建数据库-----------------')
        ...
        LOG.info('----导出纯文本格式内容的数据库全量信息时指定数据库用户---')
        ...
        LOG.info(msg3)
        self.assertIn('not permitted to login', msg3)

    def tearDown(self):
        LOG.info('-----------------清理环境：删除数据库-----------------')
        sql_cmd4 = '''drop role if exists qm;
        ...
        LOG.info(msg4)
        LOG.info('----Opengauss_Function_Tools_gs_dump_Case0026finish----')
