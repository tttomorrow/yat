...
"""
Case Type   : 服务端工具
Case Name   : 修改dump导出的数据文件后,再导入(tar归档格式:删除部分字段)
Description :
    1.创建数据；
    2.执行导出操作：
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Tools_gs_dump_Case0109start---')
        ...
        self.db_name1 = "db_dump0109_01"
        self.db_name2 = "db_dump0109_02"
        self.tb_name = "t_dump0109"
    ...
    def test_tools_dump(self):
        ...
        self.log.info(text)
        sed_cmd = f"sed -i '5,$d' {self.dump_path};"
        ...
        self.assertIn('unexpected end of file', gsql_result, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '------Opengauss_Function_Tools_gs_dump_Case0109finish------')
