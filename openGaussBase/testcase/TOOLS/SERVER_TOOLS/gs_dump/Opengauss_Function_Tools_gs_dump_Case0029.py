...
"""
Case Type   : 服务端工具
Case Name   : 导出文本格式的文件以创建数据库和连接到创建的数据库的命令开始
Description :
    1.连接数据库：
    2.创建数据库
    3.切换到数据库test
    ...
    5.退出数据库
    6.source环境变量
    7.导出文本格式的文件以创建数据库和连接到创建的数据库的命令开始
    8.连接数据库，清理环境
Expect      :
    1.数据库连接成功
    2.创建数据库test成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_dump_Case0029start------')
    ...
    def test_server_tools(self):
        ...
        LOG.info('---导出文本格式的文件以创建数据库和\
                 连接到创建的数据库的命令开始---')
        excute_cmd3 = f'''source {macro.DB_ENV_PATH} ;
         gs_dump -p {self.dbuser_node.db_port} test --format=p  -C;
                      '''
        LOG.info(excute_cmd3)
        msg3 = self.dbuser_node.sh(excute_cmd3).result()
        LOG.info(msg3)
        self.assertIn('CREATE DATABASE', msg3)
        self.assertIn('\connect test', msg3)
    ...
    def tearDown(self):
        ...
        LOG.info(msg5)
        LOG.info('---Opengauss_Function_Tools_gs_dump_Case0029finish----')
