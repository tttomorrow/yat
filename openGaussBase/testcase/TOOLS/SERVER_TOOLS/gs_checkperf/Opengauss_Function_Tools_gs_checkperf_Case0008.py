...
"""
Case Type   : 服务端工具
Case Name   : opengauss安装用户检查openGauss性能
Description :
    1.opengauss安装安装用户检查openGauss性能
    gs_checkperf -U 运行openGauss的用户名称 -i  PMK
Expect      :
    1.检查指标无丢失无遗漏
History     : 
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-----Opengauss_Function_Tools_gs_checkperf_Case0008_开始-----')
        self.dbuser_node = Node('dbuser')

    def test_tools_checkperf(self):
        text = '----step1:opengauss安装用户检查openGauss性能;expect:检查指标无丢失无遗漏----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-----Opengauss_Function_Tools_gs_checkperf_Case0008_结束-----')
