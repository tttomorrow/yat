...
"""
Case Type   : 服务端工具
Case Name   : opengauss安装用户检查openGauss性能(不使用-i指定检查项)
Description :
    1.opengauss安装用户检查openGauss性能(不使用-i指定检查项)
    gs_checkperf -U 运行openGauss的用户名称
Expect      :
    1.正常显示检查结果
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-----Opengauss_Function_Tools_gs_checkperf_Case0018_开始-----')
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-----Opengauss_Function_Tools_gs_checkperf_Case0018_结束-----')
