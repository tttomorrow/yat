...
"""
Case Type   : 服务端工具
Case Name   : 执行gs_cgroup -S {classname}-E "elapsedtime={0-4294967295}" --a
              设置参数elapsedtime，且设置异常操作为abort
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -S {classname}-E "elapsedtime={0-4294967295}" --a
    设置异常参数elapsedtime为uint型 = [0-4294967295] 指定异常操作--a
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0030开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0030_class'
        self.Grp_Name = 'Cgroup_Case0030_grp'
    ...
    def test_gs_cgroup(self):
        ...
        execute_cmd = f'source {macro.DB_ENV_PATH};' \
                      ...
                      f'-E "elapsedtime={percent}" --a;' \
                      f'gs_cgroup -p'
        self.log.info(execute_cmd)
        msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(msg)
        msg1 = msg.split('ABORT:')[1]
        self.log.info(msg1)
        if percent == 0:
            self.assertNotIn(f'ElapsedTime={percent}', msg1, '执行失败' + text)
        else:
            self.assertIn(f'ElapsedTime={percent}', msg1, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0030执行完成----')
