...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0019开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0019_class'
        self.Grp_Name = 'Cgroup_Case0019_grp'
    ...
    def test_gs_cgroup(self):
        ...
        for percent in percent_lis:
            ...
            self.log.info(msg)
            error_msg = f'ERROR: invalid value for workload group ' \
                        f'dynamic percentage. its range should be 1 ~ 99!'
            self.assertIn(error_msg, msg, '执行失败' + text)

    def tearDown(self):
        self.log.info('----No Need To Clean----')
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0019执行完成----')
