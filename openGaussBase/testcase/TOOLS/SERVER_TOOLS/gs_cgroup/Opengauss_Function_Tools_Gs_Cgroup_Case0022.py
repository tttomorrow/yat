...
"""
Case Type   : 服务端工具
Case Name   : 执行gs_cgroup -d -S {classname} -G {grpname}
              删除{classname}组下的{grpname}控制组
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1} -g n 创建控制组
    2.omm用户下执行gs_cgroup -d -S {classname} -G {grpname}删除控制组
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0022开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0022_class'
        self.Grp_Name = 'Cgroup_Case0022_grp'
    ...
    def test_gs_cgroup(self):
        ...
        text = '----step2:删除class控制组下的wokload控制组 expect:删除成功----'
        self.log.info(text)
        execute_cmd = f'source {macro.DB_ENV_PATH};' \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0022执行完成----')
