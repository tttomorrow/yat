...
"""
Case Type   : 服务端工具
Case Name   : 修改Backend组资源占用为!=[1,99]
Description :
    1.omm用户下执行gs_cgroup -u -T Backend -t n 修改Backend组资源占用为!=[1,99]
Expect      :
    1.修改失败
History     :
"""
...
class Tools(unittest.TestCase):
    ...
    def test_gs_cgroup(self):
        text = '----step1:修改Backend组资源占用为!=[1,99] expect:修改失败----'
        self.log.info(text)
        ...
        for percent in percent_lis:
            execute_cmd = f'source {macro.DB_ENV_PATH};' \
                          f'gs_cgroup --revert;' \
                          f'gs_cgroup -u -T Backend -t {percent};' \
                          f'gs_cgroup -p'
