...
Case Name   : 执行gs_cgroup -S {classname} -E "spillsize = [0-4294967295]"
              --penalty设置异常参数spillsize为uint型0-4294967295，
              指定异常操作--penalty
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -S {classname}
    -E "spillsize={0-4294967295}" --penalty
    设置异常参数spillsize为uint型 = [0-4294967295] 指定异常操作--penalty
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0037开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0037_class'
        self.Grp_Name = 'Cgroup_Case0037_grp'
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0037执行完成----')
