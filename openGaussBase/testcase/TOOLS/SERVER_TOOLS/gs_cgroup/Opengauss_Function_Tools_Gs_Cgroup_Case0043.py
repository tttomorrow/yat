...
Case Name   : 执行gs_cgroup -S {classname} -E"broadcastsize !={0-4294967295}"
              --a设置参数broadcastsize，且设置异常操作为abort
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户执行gs_cgroup -S {classname}-E "broadcastsize !={0-4294967295}" --a
    设置异常参数broadcastsize为 != [0-4294967295]的uint型 指定异常操作--a
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0043开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0043_class'
        self.Grp_Name = 'Cgroup_Case0043_grp'
    ...
    def test_gs_cgroup(self):
        ...
        text = '----step2:设置异常参数broadcastsize !=[0-4294967295]的uint型，' \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0043执行完成----')
