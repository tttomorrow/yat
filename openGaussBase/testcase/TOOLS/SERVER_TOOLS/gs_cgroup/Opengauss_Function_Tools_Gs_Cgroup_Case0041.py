...
"""
Case Type   : 服务端工具
Case Name   : 执行gs_cgroup -S {classname} -E "broadcastsize={0-4294967295}"
              --penalty设置异常参数broadcastsize为uint型0-4294967295，
              异常操作为--penalty
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -S {classname}
    -E "broadcastsize={0-4294967295}" --penalty
    设置异常参数broadcastsize为uint型0-4294967295，异常操作为--penalty
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0041开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0041_class'
        self.Grp_Name = 'Cgroup_Case0041_grp'
    ...
    def test_gs_cgroup(self):
        ...
        text = '----step2:设置异常参数broadcastsize为uint型0-4294967295，' \
               '异常操作为--penalty  expect:修改失败----'
        self.log.info(text)
        ...
        for percent in percent_lis:
            execute_cmd = f'source {macro.DB_ENV_PATH};' \
                          ...
                          f'-E "broadcastsize={percent}" --penalty;' \
                          f'gs_cgroup -p'
            self.log.info(execute_cmd)
            msg = self.primary_node.sh(execute_cmd).result()
            self.log.info(msg)
            error_msg = "ERROR: threshold 'BroadcastSize', " \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0041执行完成----')
