...
Case Name   : 执行gs_cgroup -S {classname} -E "allcputime=[0,4294967295]"
              设置异常参数allcputime为uint型0-4294967295，不指定异常操作
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0036开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0036_class'
        self.Grp_Name = 'Cgroup_Case0036_grp'
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0036执行完成----')
