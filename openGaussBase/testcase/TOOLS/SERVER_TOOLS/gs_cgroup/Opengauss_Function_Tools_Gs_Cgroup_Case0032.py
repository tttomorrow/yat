...
"""
Case Type   : 服务端工具
Case Name   : 执行gs_cgroup -S {classname}-E "elapsedtime ={0-4294967295}"
              设置异常参数elapsedtime为uint型0-4294967295，不指定异常操作
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -S {classname}-E "elapsedtime ={0-4294967295}"
    设置异常参数elapsedtime为uint型0-4294967295，不指定异常操作
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0032开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0032_class'
        self.Grp_Name = 'Cgroup_Case0032_grp'
    ...
    def test_gs_cgroup(self):
        ...
        text = '----step2:设置异常参数elapsedtime =[0-4294967295]的uint型，' \
               '不指定异常操作  expect:修改失败----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0032执行完成----')
