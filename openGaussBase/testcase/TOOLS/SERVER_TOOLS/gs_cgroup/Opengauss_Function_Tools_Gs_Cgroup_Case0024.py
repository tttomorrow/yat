...
"""
Case Type   : 服务端工具
Case Name   : 执行gs_cgroup -u -S {classname} -G {grpname} -g n
              修改class控制组下的workload控制组资源占用为 n
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -u -S {classname} -G {grpname1} -g n
    修改class控制种族资源占用为 n
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0024开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0024_class'
        self.Grp_Name = 'Cgroup_Case0024_grp'
    ...
    def test_gs_cgroup(self):
        ...
        text = '----step2:修改class控制组下的workload控制组资源占用  ' \
               'expect:修改成功----'
        self.log.info(text)
        while True:
        ...
        execute_cmd = f'source {macro.DB_ENV_PATH};' \
                      f'gs_cgroup -u -S {self.Class_Name} ' \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0024执行完成----')
