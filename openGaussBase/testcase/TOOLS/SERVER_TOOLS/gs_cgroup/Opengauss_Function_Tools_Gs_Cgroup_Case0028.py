...
"""
Case Type   : 服务端工具
Case Name   : 执行gs_cgroup -S {classname} -E "blocktime = [0-4294967295]"
              设置异常参数blocktime为uint型0-4294967295，不指定异常操作
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -S {classname} -E "blocktime={0-4294967295}"
    设置异常参数blocktime为uint型 = [0-4294967295]
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0028开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0028_class'
        self.Grp_Name = 'Cgroup_Case0028_grp'
    ...
    def test_gs_cgroup(self):
        ...
        execute_cmd = f'source {macro.DB_ENV_PATH};' \
                      ...
                      f'-E "blocktime={percent}";' \
                      f'gs_cgroup -p'
        self.log.info(execute_cmd)
        msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(msg)
        error_msg = 'ERROR: threshold \'BlockTime\' for "penalty" is invalid!'
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0028执行完成----')
