...
"""
Case Type   : 服务端工具
Case Name   : 执行gs_cgroup -u -S {classname} -s n 修改class组资源分配值 n取值1-99
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -u -S {classname} -s n 修改class控制种族资源占用为n
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0023开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0023_class'
        self.Grp_Name = 'Cgroup_Case0023_grp'
    ...
    def test_gs_cgroup(self):
        ...
        text = '----step2:修改class控制组资源占用  expect:修改成功----'
        self.log.info(text)
        while True:
        ...
        execute_cmd = f'source {macro.DB_ENV_PATH};' \
                      ...
                      f'gs_cgroup -u -S {self.Class_Name} -s {percent};' \
                      f'gs_cgroup -p'
        self.log.info(execute_cmd)
        msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(msg)
        msg1 = msg.split(f'{self.Grp_Name}')[1]. \
            split('Timeshare Group information')[0]
        self.log.info(msg2)
        msg2 = msg.split(f'{self.Class_Name}')[1]. \
            split('Workload Group information')[0]
        self.log.info(msg1)
        assert1 = re.findall('\([0-9]{2}\)', msg1)
        self.log.info(assert1)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0023执行完成----')
