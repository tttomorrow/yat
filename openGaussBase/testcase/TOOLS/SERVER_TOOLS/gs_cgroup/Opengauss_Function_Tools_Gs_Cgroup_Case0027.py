...
"""
Case Type   : 服务端工具
Case Name   : 执行gs_cgroup -S {classname} -E "blocktime != [0-4294967295]" --a
              设置异常参数blocktime != [0-4294967295]的uint型，异常操作为--a
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -S {classname} -E "blocktime={0-4294967295}" --a
    设置异常参数blocktime为uint型 != [0-4294967295]，异常操作为--a
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0027开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0027_class'
        self.Grp_Name = 'Cgroup_Case0027_grp'
    ...
    def test_gs_cgroup(self):
        ...
        text = '----step2:设置异常参数blocktime为uint != [0-4294967295]，' \
               '异常操作为--a  expect:修改失败----'
        self.log.info(text)
        percent1 = random.randint(4294967296, 8294967296)
        percent2 = random.randint(-4294967295, 0)
        percent_lis = [percent1, percent2]
        for percent in percent_lis:
            execute_cmd = f'source {macro.DB_ENV_PATH};' \
                          f'gs_cgroup -S {self.Class_Name} ' \
                          f'-E "blocktime={percent}" --a;' \
                          f'gs_cgroup -p'
            self.log.info(execute_cmd)
            msg = self.primary_node.sh(execute_cmd).result()
            error_msg = 'ERROR: threshold \'BlockTime\', ' \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0027执行完成----')
