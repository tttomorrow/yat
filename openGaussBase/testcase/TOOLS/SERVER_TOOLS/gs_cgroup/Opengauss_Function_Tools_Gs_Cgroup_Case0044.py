...
Case Name   : 执行gs_cgroup -S {classname} -E "broadcastsize = [0-4294967295]"
              设置异常参数broadcastsize为uint型0-4294967295，不指定异常操作
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -S {classname} -E "broadcastsize={0-4294967295}"
    设置异常参数broadcastsize为uint型 = [0-4294967295]
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0044开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0044_class'
        self.Grp_Name = 'Cgroup_Case0044_grp'
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0044执行完成----')
