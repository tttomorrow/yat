...
Case Name   : 执行gs_cgroup -S {classname} -E "allcputime!=[0,4294967295]"
              设置异常参数allcputime为 !=[0,4294967295]uint型，不指定异常操作
Description :
    1.omm用户下执行gs_cgroup -c -S {classname} -G {grpname1}
    创建class及workload控制组
    2.omm用户下执行gs_cgroup -S {classname} -E "allcputime!=[0,4294967295]"
    设置异常参数allcputime为 !=[0,4294967295]uint型，不指定异常操作
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('----Opengauss_Function_Gs_Cgroup_Case0035开始执行----')
        self.primary_node = Node('PrimaryDbUser')
        self.Class_Name = 'Cgroup_Case0035_class'
        self.Grp_Name = 'Cgroup_Case0035_grp'
    ...
    def test_gs_cgroup(self):
        ...
        for percent in percent_lis:
            execute_cmd = f'source {macro.DB_ENV_PATH};' \
                          ...
                          f'-E "allcputime={percent}";' \
                          f'gs_cgroup -p'
            self.log.info(execute_cmd)
            msg = self.primary_node.sh(execute_cmd).result()
            self.log.info(msg)
            error_msg = 'ERROR: threshold \'AllCpuTime\', ' \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Gs_Cgroup_Case0035执行完成----')
