...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_guc_Case0019开始执行-----')
    ...
    def test_server_tools1(self):
        ...
        LOG.info('---------步骤2.使用gs_guc reload进行参数设定，并指定-N为主节点---------')
        check_cmd = f'''hostname'''
        LOG.info(check_cmd)
        hostname = self.dbuser.sh(check_cmd).result()
        ...
        check_cmd = f'source {macro.DB_ENV_PATH};' \
            ...
            f'krb_caseins_users; gs_guc check -N {hostname} ' \
        ...
        msg = self.dbuser.sh(check_cmd).result()
        LOG.info(msg)
        self.assertIn(f'krb_caseins_users=off', msg)

    def tearDown(self):
        LOG.info('---------步骤6.使用gs_guc reload恢复默认值---------')
        check_cmd = f'source {macro.DB_ENV_PATH};' \
            f'gs_guc reload -N all -I all -c  krb_caseins_users=off'
        LOG.info(check_cmd)
        msg = self.dbuser.sh(check_cmd).result()
        LOG.info(msg)
        LOG.info('-----Opengauss_Function_Tools_gs_guc_Case0019执行结束-----')
