...
"""
Case Type   : 服务端工具
Case Name   : 检查配置文件中参数，指定参数名称-c
Description :
    1.检查指定主机的指定实例路径不指定具体参数
    2.检查所有主机的所有实例路径下的参数track_activities
Expect      :
    1.执行失败
    2.检查成功
History     : 
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_guc_Case0009开始执行-----')
    ...
    def test_server_tools1(self):
        ...
        LOG.info(msg1)
        self.assertIn("gs_guc: option requires an argument -- 'c'", msg1)
    ...
    def tearDown(self):
        LOG.info('----------------无需清理环境-----------------------')
        LOG.info('-----Opengauss_Function_Tools_gs_guc_Case0009执行结束-----')
