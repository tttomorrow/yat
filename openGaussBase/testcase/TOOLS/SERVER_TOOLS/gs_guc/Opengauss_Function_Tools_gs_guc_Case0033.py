...
"""
Case Type   : 服务端工具
Case Name   : 设置-c参数时，参数都可以省略双引号
Description :
    1.查看krb_caseins_users默认值
    2.设置krb_caseins_users
    3.重启数据库
    4.查看krb_caseins_users设置是否成功
    5.恢复设置
    6.检查是否恢复成功
Expect      :
    1.显示成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('----Opengauss_Function_Tools_gs_guc_Case0033开始执行----')
    ...
    def test_server_tools(self):
        ...
        LOG.info('--------------步骤2.设置krb_caseins_users---------------')
        check_cmd = f'source {macro.DB_ENV_PATH};' \
            f'gs_guc set -N all -I all -c krb_caseins_users=on;'
        LOG.info(check_cmd)
        ...
        LOG.info('-----步骤4.查看krb_caseins_users是否设置成功-----')
        check_cmd = f'source {macro.DB_ENV_PATH};' \
        ...
        LOG.info('-----步骤6.查看krb_caseins_users是否设置成功-----')
        check_cmd = f'source {macro.DB_ENV_PATH};' \
            f'gs_guc check -N all -D {macro.DB_INSTANCE_PATH} ' \
            f'-c  krb_caseins_users;'
        LOG.info(check_cmd)
    ...
    def tearDown(self):
        LOG.info('------------------步骤7.恢复设置------------------')
        check_cmd = f'source {macro.DB_ENV_PATH};' \
            f'gs_guc set -N all -I all -c krb_caseins_users=off;'
        LOG.info(check_cmd)
        msg = self.dbuser_node.sh(check_cmd).result()
        LOG.info(msg)
        check_cmd = f'source {macro.DB_ENV_PATH};' \
        ...
        msg = self.dbuser_node.sh(check_cmd).result()
        LOG.info(msg)
        LOG.info('----Opengauss_Function_Tools_gs_guc_Case0033执行结束----')
