...
"""
Case Type   : 服务端工具
Case Name   : omm用户检查comm_max_datanode参数值范围小于数据库节点个数
Description :
    1.由root用户切换到omm用户下
    2.在非本地模式下检查： gs_check -i CheckMaxDatanode
    3.在本地模式下检查： gs_check -i CheckMaxDatanode -L
Expect      :
    1.切换到omm用户下
    2.非本地模式下检查完成
    3.本地模式下执行检查完成
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0168start-------------------')
        self.dbuserNode = Node('dbuser')
        self.Constant = Constant()

    def test_server_tools(self):
        logger.info('------------------omm用户在非本地模式下检查comm_max_datanode参数值范围小于数据库节点个数------------------')
        check_cmd1 = f'''
                            source {macro.DB_ENV_PATH}
                            gs_check -i CheckMaxDatanode
                            '''
        ...
        logger.info('------------------omm用户在本地模式下检查comm_max_datanode参数值范围小于数据库节点个数------------------')
        check_cmd2 = f'''
                            source {macro.DB_ENV_PATH}
                            gs_check -i CheckMaxDatanode -L
                            '''
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0168finish------------------')
