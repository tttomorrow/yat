...
Description :
     1.跳过文件句柄检查，检查CPU：gs_check -i CheckFilehandle,CheckCPU   --skip-root-items
     2.先检查CPU再跳过文件句柄检查：gs_check -i CheckCPU,CheckFilehandle   --skip-root-items
     3.直接跳过文件句柄检查（单项检查）：gs_check -i CheckFilehandle   --skip-root-items
Expect      :
    1.检查完成
    2.检查完成
    3.检查失败
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0320start-------------------')
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0320finish------------------')
