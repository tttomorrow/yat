...
Description :
    1.在非本地模式下检查： gs_check -i CheckNICModel并交互输入root权限的用户及其密码
    2.在本地模式下检查： gs_check -i CheckNICModel -L并交互输入root权限的用户及其密码
Expect      :
    1.输入密码完成，检查完成
    2.输入密码完成，检查完成
History     :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-----Opengauss_Function_Tools_gs_check_Case0199_start-----')
    ...
    def test_server_tools(self):
        text = '-----step1:omm用户在非本地模式下检查网卡型号和驱动版本一致性;expect:检查完成-----'
        self.log.info(text)
        ...
        text = '-----step2:omm用户在本地模式下检查网卡型号和驱动版本一致性;expect:检查完成-----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_gs_check_Case0199_finish----')
