...
"""
Case Type   : 服务端工具
Case Name   : 使用--skip-root-items跳过指定的检查项（不含root权限检查项）
Description :
     1.CPU检查，数据库连接检查（多项检查）：gs_check -i CheckCPU,CheckDBConnection    --skip-root-items
     2.CPU检查（单项检查）：gs_check -i CheckCPU    --skip-root-items
Expect      :
     1.检查完成
     2.检查完成
History     :
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0330start-------------------')
        self.dbuserNode = Node('dbuser')
        self.rootNode = Node('default')
        self.Constant = Constant()

    def test_server_tools1(self):
        logger.info('------------------CPU检查，数据库连接检查（多项检查）------------------')
        check_cmd1 = f'''
                                source {macro.DB_ENV_PATH}
                                gs_check -i CheckCPU,CheckDBConnection    --skip-root-items 
                                '''
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0330finish------------------')
