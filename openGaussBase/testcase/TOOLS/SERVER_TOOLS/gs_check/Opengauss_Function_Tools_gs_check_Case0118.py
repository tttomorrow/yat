...
"""
Case Type   : 服务端工具
Case Name   : omm用户检查数据库节点端口是否占用
Description :
    1.由root用户切换到omm用户下
    2.在非本地模式下检查： gs_check -i CheckPortConflict并交互输入root权限的用户及其密码
    3.在本地模式下检查： gs_check -i CheckPortConflict -L并交互输入root权限的用户及其密码
Expect      :
    1.切换到omm用户下
    2.输入密码完成，检查完成
    3.输入密码完成，检查完成
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0118start-------------------')
    ...
    def test_server_tools(self):
        logger.info('------------------omm用户在非本地模式下检查数据库节点端口是否占用------------------')
        check_cmd1 = f'''
                                source {macro.DB_ENV_PATH}
                                expect <<EOF
                                set timeout -1
                                spawn gs_check -i CheckPortConflict
                                expect "*]:"
        ...
        logger.info('------------------omm用户在本地模式下检查数据库节点端口是否占用------------------')
        check_cmd2 = f'''
                                source {macro.DB_ENV_PATH}
                                expect <<EOF
                                set timeout -1
                                spawn gs_check -i CheckPortConflict -L
                                expect "*]:"
                                send "{self.rootNode.ssh_user}\r"
                                expect "*]:"
                                send "{self.rootNode.ssh_password}\r"
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0118finish------------------')
