...
"""
Case Type   : 服务端工具
Case Name   : root用户检测环境变量分离文件改动
Description :
    1.root用户进入安装目录下的scriprt目录
    2.在非本地模式下检查： gs_check -i CheckMpprcFile
    3.在本地模式下检查： gs_check -i CheckMpprcFile -L
Expect      :
    1.root用户进入安装目录下的scriprt目录
    2.非本地模式下检查失败
    3.本地模式下执行检查完成
...
class Tools(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('----Opengauss_Function_Tools_gs_check_Case0063finish----')