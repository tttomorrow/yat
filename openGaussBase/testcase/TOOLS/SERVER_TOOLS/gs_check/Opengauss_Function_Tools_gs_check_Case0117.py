...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-----Opengauss_Function_Tools_gs_check_Case0117_start-----')
    ...
    def test_server_tools(self):
        text = '-----step1:omm用户在非本地模式下检查omm用户是否存在;expect:检查完成-----'
        self.log.info(text)
        ...
        text = '-----step2:omm用户在本地模式下检查omm用户是否存在;expect:检查完成-----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_gs_check_Case0117_finish----')
