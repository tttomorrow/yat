...
"""
Case Type   : 服务端工具
Case Name   : root用户检查XML差异(合理报错，这个参数目前暂不支持)
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_check_Case0047start-----')
        self.rootNode = Node('default')
        self.constant = Constant()
        self.SCRIPT_PATH = f'{macro.DB_INSTANCE_PATH}/../tool/script'
        self.error_msg = f'ERROR: There were errors when parsing these' \
            f' items: CheckConfigFileDiff. maybe items name is incorrect'

    def test_server_tools(self):
        LOG.info('-----root用户在script目录下进行非本地模式下检查XML差异-----')
        check_cmd1 = f'cd {self.SCRIPT_PATH};' \
            f' ./gs_check -i CheckConfigFileDiff;'
        LOG.info(check_cmd1)
        msg1 = self.rootNode.sh(check_cmd1).result()
        LOG.info(msg1)
        self.assertIn(self.constant.GS_CHECK_ERROR_MSG1, msg1)
        LOG.info('-----root用户在script目录下进行本地模式下检查XML差异----')
        check_cmd2 = f'cd {self.SCRIPT_PATH};' \
            f' ./gs_check -i CheckConfigFileDiff -L;'
        LOG.info(check_cmd2)
        msg2 = self.rootNode.sh(check_cmd2).result()
        LOG.info(msg2)
        self.assertTrue(self.error_msg, msg2)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('----Opengauss_Function_Tools_gs_check_Case0047finish----')