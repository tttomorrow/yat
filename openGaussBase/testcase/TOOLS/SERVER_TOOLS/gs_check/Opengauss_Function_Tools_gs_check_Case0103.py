...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0103start-------------------')
        self.dbuserNode = Node('dbuser')
        self.Constant = Constant()

    def test_server_tools(self):
        logger.info('------------------omm用户在非本地模式下检查CPU使用率-------------------')
        check_cmd1 = f'''
        ...
        logger.info('------------------omm用户在本地模式下检查CPU使用率-------------------')
        check_cmd2 = f'''
                        source {macro.DB_ENV_PATH};
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0103finish------------------')
