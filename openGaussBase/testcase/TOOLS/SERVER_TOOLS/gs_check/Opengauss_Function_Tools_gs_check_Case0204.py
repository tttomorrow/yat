...
"""
Case Type   : 服务端工具
Case Name   : 用户检查防火墙状态（参数i和参数U组合测试）
Description :
    运行openGauss的用户检查指定检查项： gs_check -i CheckFirewall -U omm并交互输入root权限的用户及其密码
Expect      :
    检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0204start-------------------')
        self.dbuserNode = Node('dbuser')
        self.rootNode = Node('default')
        self.Constant = Constant()

    def test_server_tools(self):
        logger.info('------------------运行openGauss的用户检查防火墙状态------------------')
        check_cmd1 = f'''
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0204finish------------------')
