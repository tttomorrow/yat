...
"""
Case Type   : 服务端工具
Case Name   : 就地升级前巡检场景下执行检查修复支持设置的Abnormal项
Description :
     就地升级前巡检场景下执行检查修复支持设置的Abnormal项：gs_check -e binary_upgrade --set
Expect      :
     检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0374 start---')
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0374 finish---')
