...
Description :
    运行openGauss的用户检查指定检查项： gs_check -i CheckIOConfigure -U omm并交互输入root权限的用户及其密码
Expect      :
    密码输入正确；检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0238start-------------------')
        self.dbuserNode = Node('dbuser')
        self.rootNode = Node('default')
        self.Constant = Constant()

    def test_server_tools(self):
        logger.info('------------------运行openGauss的用户检查IO配置------------------')
        check_cmd1 = f'''
                        source {macro.DB_ENV_PATH}
                        expect <<EOF
                        set timeout -1
                        spawn gs_check -i CheckIOConfigure -U {self.dbuserNode.ssh_user}
                        expect "*]:"
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0238finish------------------')
