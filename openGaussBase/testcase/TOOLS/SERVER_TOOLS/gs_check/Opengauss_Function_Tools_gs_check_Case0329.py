...
"""
Case Type   : 服务端工具
Case Name   : 使用--skip-root-items跳过需要root权限执行的检查项（CheckNICModel）
Description :
     1.跳过网卡多队列检查，检查CPU：gs_check -i CheckNICModel,CheckCPU   --skip-root-items
     2.先检查CPU再跳过网卡多队列检查：gs_check -i CheckCPU,CheckNICModel   --skip-root-items
     3.直接跳过网卡多队列检查（单项检查）：gs_check -i CheckNICModel   --skip-root-items
Expect      :
    1.检查完成
    2.检查完成
    3.检查失败
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0329start-------------------')
    ...
    def test_server_tools1(self):
        ...
        check_cmd1 = f'''
                                ...
                                gs_check -i CheckNICModel,CheckCPU   --skip-root-items 
                                '''
        ...
        check_cmd2 = f'''
                                source {macro.DB_ENV_PATH}
                                gs_check -i CheckCPU,CheckNICModel   --skip-root-items 
                                 '''
        ...
        check_cmd3 = f'''
                                        source {macro.DB_ENV_PATH}
                                        gs_check -i CheckNICModel   --skip-root-items 
                                         '''
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0329finish------------------')
