...
"""
Case Type   : 服务端工具
Case Name   : omm用户检查omm用户检查数据目录大文件
Description :
    1.由root用户切换到omm用户下
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0160start-------------------')
        self.dbuserNode = Node('dbuser')
        self.Constant = Constant()


    def test_server_tools(self):
        logger.info('------------------omm用户在非本地模式下检查数据目录大文件------------------')
        check_cmd1 = f'''
                            source {macro.DB_ENV_PATH}
                            gs_check -i CheckLargeFile
                            '''
        ...
        logger.info('------------------omm用户在本地模式下检查数据目录大文件------------------')
        check_cmd2 = f'''
                            source {macro.DB_ENV_PATH}
                            gs_check -i CheckLargeFile -L
                            '''
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0160finish------------------')
