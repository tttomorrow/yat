...
"""
Case Type   : 服务端工具
Case Name   : 安装场景下执行检查修复支持设置的Abnormal项
Description :
     安装场景下执行检查修复支持设置的Abnormal项：gs_check -e install --set
Expect      :
     检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_check_Case0376_开始---')
    ...
    def test_server_tools1(self):
        text = '------step1:安装场景下执行检查修复支持设置的Abnormal项;expect:检查完成-------'
        self.log.info(text)
        check_cmd = f'''su - {self.dbuser_node.ssh_user} -c "
                    source {macro.DB_ENV_PATH};
                    expect -c \\\"set timeout -1
                    spawn gs_check -e install --set
                    expect *):
                    send {self.send_var}\\n
                    expect *]:
    ...
    def tearDown(self):
        ...
        self.log.info('---Opengauss_Function_Tools_gs_check_Case0376_结束---')
