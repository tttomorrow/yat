...
"""
Case Type   : 服务端工具
Case Name   : 就地升级前巡检场景下执行，设置超时时间为1600S
Description :
     就地升级前巡检场景下执行，设置超时时间为1600S：gs_check -e binary_upgrade --time-out=1600
Expect      :
     检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0369 start---')
    ...
    def test_server_tools1(self):
        ...
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_check -e binary_upgrade --time-out=1600;
            '''
        LOG.info(check_cmd1)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0369 finish---')
