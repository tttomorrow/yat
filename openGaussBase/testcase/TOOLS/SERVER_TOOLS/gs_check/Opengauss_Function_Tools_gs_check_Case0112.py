...
"""
Case Type   : 服务端工具
Case Name   : omm用户检查crontab是否有残留Gauss相关信息
Description :
    1.由root用户切换到omm用户下
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0112start-------------------')
        self.dbuserNode = Node('dbuser')
        self.Constant = Constant()


    def test_server_tools(self):
        logger.info('------------------omm用户在非本地模式下检查crontab是否有残留Gauss相关信息------------------')
        check_cmd1 = f'''
                            source {macro.DB_ENV_PATH}
                            gs_check -i CheckCrontabLeft
                            '''
        ...
        logger.info('------------------omm用户在本地模式下检查crontab是否有残留Gauss相关信息------------------')
        check_cmd2 = f'''
                            source {macro.DB_ENV_PATH}
                            gs_check -i CheckCrontabLeft -L
                            '''
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0112finish------------------')
