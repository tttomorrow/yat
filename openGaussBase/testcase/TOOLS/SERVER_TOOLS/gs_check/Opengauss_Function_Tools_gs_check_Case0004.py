...
"""
Case Type   : 服务端工具
Case Name   : root用户检查防火墙状态
Description :
    1.root用户进入安装目录下的scriprt目录
    2.在非本地模式下检查： gs_check -i CheckFirewall
    3.在本地模式下检查： gs_check -i CheckFirewall -L
Expect      :
    1.进入安装目录下的scriprt目录
    2.非本地模式下检查失败
    3.本地模式下检查完成
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_check_Case0004start-----')
        self.rootNode = Node('default')
        self.constant = Constant()
        self.SCRIPT_PATH = f'{macro.DB_INSTANCE_PATH}/../tool/script'

    def test_server_tools(self):
        LOG.info('-----root用户在scriprt目录下进行非本地模式下检查防火墙状态-----')
        check_cmd1 = f'cd {self.SCRIPT_PATH}; ./gs_check -i CheckFirewall;'
        LOG.info(check_cmd1)
        msg1 = self.rootNode.sh(check_cmd1).result()
        LOG.info(msg1)
        self.assertIn(self.constant.GS_CHECK_ERROR_MSG1, msg1)
        LOG.info('-----root用户在scriprt目录下进行本地模式下检查防火墙状态----')
        check_cmd2 = f'cd {self.SCRIPT_PATH}; ' \
            f'./gs_check -i CheckFirewall -L;'
        LOG.info(check_cmd2)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('--产生的文件清理完成--')
        LOG.info('----Opengauss_Function_Tools_gs_check_Case0004finish----')
