...
"""
Case Type   : 服务端工具
Case Name   : omm用户检查网卡RXTX值
Description :
    1.在非本地模式下检查： gs_check -i CheckRXTX并交互输入root权限的用户及其密码
    2.在本地模式下检查： gs_check -i CheckRXTX -L并交互输入root权限的用户及其密码
Expect      :
    1.输入密码完成，检查完成
    2.输入密码完成，检查完成
History     :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-----Opengauss_Function_Tools_gs_check_Case0193_start-----')
    ...
    def test_server_tools(self):
        text = '-----step1:omm用户在非本地模式下检查网卡RXTX值;expect:检查完成-----'
        self.log.info(text)
        check_cmd1 = f'''su - {self.dbuserNode.ssh_user} -c "
                    source {macro.DB_ENV_PATH};
                    expect -c \\\"set timeout -1
                    spawn gs_check -i CheckRXTX 
                    expect *]:
        ...
        text = '-----step2:omm用户在本地模式下检查网卡RXTX值;expect:检查完成-----'
        self.log.info(text)
        check_cmd2 = f'''su - {self.dbuserNode.ssh_user} -c "
                   source {macro.DB_ENV_PATH};
                   expect -c \\\"set timeout -1
                   spawn gs_check -i CheckRXTX -L
                   expect *]:
                   send {self.rootNode.ssh_user}\\n
                   expect *]:
                   send {self.rootNode.ssh_password}\\n
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_gs_check_Case0193_finish----')
