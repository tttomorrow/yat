...
"""
Case Type   : 服务端工具
Case Name   : 在本地进行执行升级巡检场景检查（upgrade）
Description :
     1.在本地执行升级巡检场景检查：gs_check -e upgrade -L
Expect      :
     1.在本地执行升级巡检场景检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Tools_gs_check_Case0354_开始---')
    ...
    def test_server_tools1(self):
        text = '---------step1:在本地执行升级巡检场景检查;expect:检查完成-----------'
        self.log.info(text)
        check_cmd = f'''su - {self.dbuser_node.ssh_user} -c "
                       source {macro.DB_ENV_PATH};
                       expect -c \\\"set timeout -1
                       spawn gs_check -e upgrade -L
                       expect *]:
    ...
    def tearDown(self):
        ...
        self.log.info('--Opengauss_Function_Tools_gs_check_Case0354_结束--')
