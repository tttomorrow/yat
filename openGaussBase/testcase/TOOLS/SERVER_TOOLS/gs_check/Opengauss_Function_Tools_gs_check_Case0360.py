...
"""
Case Type   : 服务端工具
Case Name   : omm用户进行就地升级前巡检场景检查（binary_upgrade）
Description :
     就地升级前巡检场景检查：gs_check -e binary_upgrade -U omm
Expect      :
     检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    ...
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0360start---')
    ...
    def test_server_tools1(self):
        ...
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_check -e binary_upgrade -U {self.dbuserNode.ssh_user};
            '''
        LOG.info(check_cmd1)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0360finish---')
