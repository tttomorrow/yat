...
"""
Case Type   : 服务端工具
Case Name   : root用户检查max_process_memory参数设置是否合理
Description :
    1.root用户进入安装目录下的scriprt目录
    2.在非本地模式下检查： gs_check -i CheckMaxProcMemory
    3.在本地模式下检查： gs_check -i CheckMaxProcMemory -L
Expect      :
    1.root用户进入安装目录下的scriprt目录
    2.非本地模式下检查失败
    3.本地模式下执行检查完成
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_check_Case0033start-----')
    ...
    def test_server_tools(self):
        ...
        check_cmd1 = f'cd {self.SCRIPT_PATH};' \
            f' ./gs_check -i CheckMaxProcMemory;'
        LOG.info(check_cmd1)
        ...
        check_cmd2 = f'cd {self.SCRIPT_PATH};' \
            f' ./gs_check -i CheckMaxProcMemory -L;'
        LOG.info(check_cmd2)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('----Opengauss_Function_Tools_gs_check_Case0033finish----')