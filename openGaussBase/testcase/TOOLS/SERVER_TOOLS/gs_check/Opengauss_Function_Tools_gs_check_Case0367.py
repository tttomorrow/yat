...
"""
Case Type   : 服务端工具
Case Name   : 例行巡检场景下检查，跳过root权限检查项
Description :
     例行巡检场景下检查，跳过root权限检查项：gs_check -e inspect  --skip-root-items
Expect      :
     检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0367 start---')
    ...
    def test_server_tools1(self):
        ...
        check_cmd1 = f'''source {macro.DB_ENV_PATH};
            gs_check -e inspect --skip-root-items;
            '''
        LOG.info(check_cmd1)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0367 finish---')
