...
"""
Case Type   : 服务端工具
Case Name   : root用户检查CPU使用率
Description :
    1.root用户进入安装目录下的scriprt目录
    2.在非本地模式下检查： gs_check -i CheckCPU
    3.在本地模式下检查： gs_check -i CheckCPU -L
Expect      :
    1.进入安装目录下的scriprt目录
    2.非本地模式下检查失败
    3.本地模式下检查完成
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_check_Case0003 start-----')
        self.rootNode = Node('default')
        self.constant = Constant()
        self.SCRIPT_PATH = f'{macro.DB_INSTANCE_PATH}/../tool/script'

    def test_server_tools(self):
        LOG.info('-----root用户在scriprt目录下进行非本地模式下的CPU检查-----')
        ...
        LOG.info('-----root用户在scriprt目录下进行本地模式下的CPU检查----')
        check_cmd2 = f'cd {self.SCRIPT_PATH};' \
            f'./gs_check -i CheckCPU -L;'
        LOG.info(check_cmd2)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('--产生的文件清理完成--')
        LOG.info('----Opengauss_Function_Tools_gs_check_Case0003 finish----')
