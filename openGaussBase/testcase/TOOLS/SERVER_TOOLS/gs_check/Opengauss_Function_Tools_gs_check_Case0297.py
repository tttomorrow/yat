...
Description :
    1.运行openGauss的用户检查指定检查项： gs_check -i CheckMultiQueue -U
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-----Opengauss_Function_Tools_gs_check_Case0297_start-----')
    ...
    def test_server_tools(self):
        text = '-----step1:运行openGauss的用户检查指定的检查项:网卡多队列;' \
               'expect:检查完成-----'
        self.log.info(text)
        check_cmd = f'''su - {self.dbuserNode.ssh_user} -c "
                    source {macro.DB_ENV_PATH};
                    expect -c \\\"set timeout -1
                    spawn gs_check -i CheckMultiQueue -U \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_gs_check_Case0297_finish----')
