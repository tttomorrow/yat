...
"""
Case Type   : 服务端工具
Case Name   : 就地升级前巡检场景检查跳过指定检查项
Description :
     就地升级前巡检场景检查跳过指定检查项：
     gs_check -e binary_upgrade --skip-items CheckXid
Expect      :
     检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0379start---')
        self.dbuserNode = Node('dbuser')
        self.rootNode = Node('default')
        self.Constant = Constant()

    def test_server_tools1(self):
        LOG.info('-------就地升级前巡检场景检查跳过指定检查项------')
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('---Opengauss_Function_Tools_gs_check_Case0379finish---')
