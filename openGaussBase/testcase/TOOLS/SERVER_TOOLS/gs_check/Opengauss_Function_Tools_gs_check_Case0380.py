...
"""
Case Type   : 服务端工具
Case Name   : 升级前巡检场景检查跳过指定多个检查项
Description :
     升级前巡检场景检查跳过指定多个检查项：
     gs_check -e upgrade --skip-items CheckCreateView,
     CheckMpprcFile,CheckBootItems,CheckDilateSysTab
Expect      :
     检查完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Tools_gs_check_Case0380_开始----')
        ...
        self.skip_options = f'CheckCreateView,CheckMpprcFile,' \
            f'CheckBootItems,CheckDilateSysTab'

    def test_server_tools1(self):
        text = '-------step1:升级前巡检场景检查跳过指定多个检查项;expect:检查完成--------'
        self.log.info(text)
        check_cmd = f'''su - {self.dbuser_node.ssh_user} -c "
                    source {macro.DB_ENV_PATH};
                    expect -c \\\"set timeout -1
                    spawn gs_check -e upgrade --skip-items {self.skip_options}
                    expect *]:
    ...
    def tearDown(self):
        ...
        self.log.info(
            '------Opengauss_Function_Tools_gs_check_Case0380_结束------')
