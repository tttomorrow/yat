...
"""
Case Type   : 服务端工具
Case Name   : root用户检查TD模式数据库下的orc表中是否包含date类型的列
Description :
    1.root用户进入安装目录下的scriprt目录
    2.在非本地模式下检查： gs_check -i CheckTDDate
    3.在本地模式下检查： gs_check -i CheckTDDate -L
Expect      :
    1.root用户进入安装目录下的scriprt目录
    2.非本地模式下检查失败
    3.本地模式下执行检查完成
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_check_Case0089start-----')
    ...
    def test_server_tools(self):
        LOG.info('-----root用户在script目录下进行非本地模式下'
                 '检查TD模式数据库下的orc表中是否包含date类型的列-----')
        check_cmd1 = f'cd {self.SCRIPT_PATH};' \
            f' ./gs_check -i CheckTDDate;'
        LOG.info(check_cmd1)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('----Opengauss_Function_Tools_gs_check_Case0089finish----')