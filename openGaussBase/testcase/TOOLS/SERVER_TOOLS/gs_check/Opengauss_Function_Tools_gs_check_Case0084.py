...
"""
Case Type   : 服务端工具
Case Name   : root用户检查Default表达式中是否包含nextval(sequence)
Description :
    1.root用户进入安装目录下的scriprt目录
    2.在非本地模式下检查： gs_check -i CheckNextvalInDefault
    3.在本地模式下检查： gs_check -i CheckNextvalInDefault -L
Expect      :
    1.root用户进入安装目录下的scriprt目录
    2.非本地模式下检查失败
    3.本地模式下执行检查完成
...
class Tools(unittest.TestCase):
    def setUp(self):
        LOG.info('-----Opengauss_Function_Tools_gs_check_Case0084start-----')
        self.rootNode = Node('default')
        self.constant = Constant()
        self.SCRIPT_PATH = f'{macro.DB_INSTANCE_PATH}/../tool/script'

    def test_server_tools(self):
        LOG.info('-----root用户在script目录下进行非本地模式下'
                 '检查Default表达式中是否包含nextval(sequence)-----')
        check_cmd1 = f'cd {self.SCRIPT_PATH};' \
            f' ./gs_check -i CheckNextvalInDefault;'
        LOG.info(check_cmd1)
        msg1 = self.rootNode.sh(check_cmd1).result()
        LOG.info(msg1)
        self.assertIn(self.constant.GS_CHECK_ERROR_MSG1, msg1)
        LOG.info('-----root用户在script目录下进行本地模式下检'
                 '查Default表达式中是否包含nextval(sequence)----')
        check_cmd2 = f'cd {self.SCRIPT_PATH};' \
            f' ./gs_check -i CheckNextvalInDefault -L;'
        LOG.info(check_cmd2)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-----------------产生的文件清理完成------------------')
        LOG.info('----Opengauss_Function_Tools_gs_check_Case0084finish----')
