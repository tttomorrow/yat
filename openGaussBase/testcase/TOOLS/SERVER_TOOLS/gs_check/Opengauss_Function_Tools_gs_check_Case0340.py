...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0340start-------------------')
    ...
    def test_server_tools1(self):
        ...
        for cmd in cmd_list:
            ...
            logger.info(msg1)
            logger.info('-------------截取Example:到General options部分的语法-------------------')
            start_index1 = msg1.find('Example:')+len('Example:')
            ...
            grammar = [['gs_check -i ITEM [...] [-U USER] [-L] [-l LOGFILE] [-o OUTPUTDIR]',
            ...
            options_list3 = list(set(options_list2))
            logger.info(options_list3)
            parameter = ['-o', '-i', '--cid', '-V,', '-U', '--skip-root-items', '-L', '-l', '--disk-threshold',
                         '--skip-items', '--set', '-?,', '--routing', '--time-out', '--version', '-e', '--format',
                         '--help']
            if len(options_list3) == len(parameter):
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0340finish------------------')