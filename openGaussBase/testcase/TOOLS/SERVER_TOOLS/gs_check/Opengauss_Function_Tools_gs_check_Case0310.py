...
"""
Case Type   : 服务端工具
Case Name   : 使用--skip-root-items跳过需要root权限执行的检查项（CheckFirewall）
Description :
     1.跳过防火墙检查，检查CPU：gs_check -i CheckFirewall,CheckCPU   --skip-root-items
     2.先检查CPU再跳过防火墙检查：gs_check -i CheckCPU,CheckFirewall   --skip-root-items
     3.直接跳过防火墙检查（单项检查）：gs_check -i CheckFirewall   --skip-root-items
Expect      :
    1.检查完成
    2.检查完成
    3.检查失败
...
class Tools(unittest.TestCase):
    def setUp(self):
        logger.info('--------------Opengauss_Function_Tools_gs_check_Case0310start-------------------')
        self.dbuserNode = Node('dbuser')
        self.rootNode = Node('default')
        self.Constant = Constant()

    def test_server_tools1(self):
        logger.info('------------------跳过防火墙检查，检查CPU------------------')
        check_cmd1 = f'''
                                source {macro.DB_ENV_PATH}
                                gs_check -i CheckFirewall,CheckCPU   --skip-root-items 
                                '''
        ...
        logger.info('------------------先检查CPU再跳过防火墙检查------------------')
        check_cmd2 = f'''
                                source {macro.DB_ENV_PATH}
                                gs_check -i CheckCPU,CheckFirewall   --skip-root-items 
                                 '''
        ...
        self.assertTrue(flag)
        logger.info('------------------直接跳过防火墙检查（单项检查）------------------')
        check_cmd3 = f'''
                                        source {macro.DB_ENV_PATH}
                                        gs_check -i CheckFirewall   --skip-root-items 
                                         '''
    ...
    def tearDown(self):
        logger.info('--------------无需清理环境-------------------')
        logger.info('------------------Opengauss_Function_Tools_gs_check_Case0310finish------------------')
