...
Description :
    ...
    4.安装目录权限改为777
    5.安装脚本权限改为700
    6.omm用户进入解压后目录下的simpleInstall.执行安装脚本
      sh install.sh  -p xxx -w xxx
    7.清理环境：删除安装目录，删除omm用户
Expect      :
    1.用户和用户组创建成功
    2.目录创建成功
...
class Tools(unittest.TestCase):
    ...
    def test_tools_simpleinstall(self):
        ...
        text = '----step5:root用户修改安装脚本权限为700;expect:修改权限成功----'
        self.log.info(text)
        chm_cmd = f'chmod -R 700 {self.simpleInstall_path};'
        self.log.info(chm_cmd)
        chm_result = self.common.get_sh_result(self.root_node, chm_cmd)
        self.log.info(chm_result)

        text = '----step6:omm用户进入安装目录下执行安装;expect:提示权限不足安装失败----'
        self.log.info(text)
