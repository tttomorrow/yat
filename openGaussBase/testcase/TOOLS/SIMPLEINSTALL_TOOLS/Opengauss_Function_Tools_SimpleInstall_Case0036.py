...
"""
Case Type   : 简化安装
Case Name   : 极简安装--安装路径权限为700
Description :
    1.创建omm用户和用户组
    2.root用户创建安装目录
    3.下载并解压openGauss安装包到安装目录
    4.安装目录权限改为700
    5.omm用户进入解压后目录下的simpleInstall.执行安装脚本
...
class Tools(unittest.TestCase):
    ...
    def test_tools_simpleinstall(self):
        ...
        text = '----step4:root用户修改安装路径权限为700;expect:修改权限成功----'
        self.log.info(text)
        chm_cmd = f'chmod -R 700 {self.install_pkg_path};'
        self.log.info(chm_cmd)
        chm_result = self.common.get_sh_result(self.root_node, chm_cmd)
        self.log.info(chm_result)

        text = '----step5:omm用户进入安装目录下执行安装;expect:提示权限不足安装失败----'
        self.log.info(text)
