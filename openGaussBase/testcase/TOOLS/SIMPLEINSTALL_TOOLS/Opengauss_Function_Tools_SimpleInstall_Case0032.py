...
"""
Case Type   : 简化安装
Case Name   : 极简安装--使用root用户执行安装脚本
Description :
    1.创建omm用户和用户组
    2.root用户创建安装目录
    3.下载并解压openGauss安装包到安装目录
    4.安装目录权限改为777
    5.root用户进入解压后目录下的simpleInstall.执行安装脚本
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.bz_pkg_path = os.path.join(self.install_pkg_path,
                                        '*.tar.bz2')
        self.log.info(self.bz_pkg_path)
        self.simpleInstall_path = os.path.join(self.install_pkg_path,
                                               'simpleInstall')
        self.u_name = 'u_sim_0032'

    def test_tools_simpleinstall(self):

        text = '----查询系统未使用端口----'
        self.log.info(text)
        port = self.common.get_not_used_port(self.root_node)
        self.log.info(port)
        ...
        text = '----step4:root用户修改安装路径权限为777;expect:修改权限成功----'
        self.log.info(text)
        chm_cmd = f'chmod -R 777 {self.install_pkg_path};'
        self.log.info(chm_cmd)
        chm_result = self.common.get_sh_result(self.root_node, chm_cmd)
        self.log.info(chm_result)

        text = '----step5:root用户进入安装目录下执行安装;expect:安装失败----'
        self.log.info(text)
        install_cmd = f'''cd {self.simpleInstall_path};
                          sh install.sh  -p {port}  -w {macro.COMMON_PASSWD} '''
        self.log.info(install_cmd)
        install_msg = self.common.get_sh_result(self.root_node, install_cmd)
        self.log.info(install_msg)
        self.assertIn('can not install', install_msg, '执行失败' + text)
