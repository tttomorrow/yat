...
"""
Case Type   : tools
Case Name   : 启动gaussdb进程时，使用-d参数设置其值为6是否可以成功（超出取值范围）
Description :
    1.关闭正在运行的数据库
    gs_ctl stop -D /opt/openGauss_zl/cluster/dn1
    2.查看进程，确定关闭成功
    ps -ef|grep zl
    3.使用gaussdb工具后台运行进程，分别设置-d参数的值为6
    gaussdb -D /opt/openGauss_zl/cluster/dn1 -p 19701 -d 6 -M primary &
    4.恢复-重启数据库
    gs_ctl restart -D /opt/openGauss_zl/cluster/dn1 -M primary
Expect      :
    1.关闭正在运行的数据库成功
    2.查看进程，数据库连接进程不存在，数据库关闭成功
    3.使用gaussdb工具后台启动数据库进程失败
    4.重启数据库成功
History     :
"""
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_Gaussdb_Case0012 start--')
    ...
    def test_systools(self):
        ...
        self.logger.info('---使用gaussdb工具后台运行进程，设置-d参数的值为6----')
        excute_cmd3 = f'source {self.DB_ENV_PATH};' \
                      f'gaussdb -D {self.DB_INSTANCE_PATH} -p' \
                      f' {self.userNode.db_port} -d 6 -M primary'
    ...
    def tearDown(self):
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0012 finish-')
