...
Expect      :
    1.关闭正在运行的数据库成功
    2.使用gaussdb工具指定--single作为第三个参数启动数据库失败
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0056 start-')
    ...
    def test_systools(self):
        ...
        if 'Standby' in msg0:
            ...
            excute_cmd7 = f'source {self.DB_ENV_PATH};gs_ctl start -D ' \
    ...
    def tearDown(self):
        self.logger.info('Opengauss_Function_Tools_Gaussdb_Case0056 finish')
