...
"""
Case Type   : tools
Case Name   : gaussdb工具的-S参数单位是否为KB
Description :
    1.关闭正在运行的数据库
    gs_ctl stop -D /opt/openGauss_zl/cluster/dn1
    2.查看work_mem参数的值
    show work_mem;
    3.使用gaussdb工具设置-S参数的值为65，启动数据库
    gaussdb -D /opt/openGauss_zl/cluster/dn1 -p 19701 -S 1500 -M primary &
    4.查看work_mem参数的值是否为-S指定值，且单位为KB
    show work_mem;
Expect      :
    1.关闭正在运行的数据库成功
    2.查看work_mem参数的值成功，显示当前值为64MB
    3.使用gaussdb工具设置-S参数的值为1500，启动数据库成功
    4.查看work_mem参数的值成功，为-S指定值，且单位为KB，显示1500KB
History     :
"""
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_Gaussdb_Case0039 start--')
...
-S 1500 -M primary'''
        ...
        self.common.equal_sql_mdg(msg6, 'work_mem', '1500kB', '(1 row)',
                                flag='1')
    ...
    def tearDown(self):
        ...
        self.common.equal_sql_mdg(msg2, 'work_mem', '64MB', '(1 row)',
                                flag='1')
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0039 finish-')
