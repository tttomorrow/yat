...
"""
Case Type   : tools
Case Name   : 启动gaussdb进程时，使用-C参数显示参数值是否可以成功（文件中未注释参数）
Description :
    1.关闭正在运行的数据库
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_Gaussdb_Case0009 start--')
    ...
    def test_systools(self):
        ...
        excute_cmd3 = f'source {self.DB_ENV_PATH};' \
                      ...
                      f'{self.userNode.db_port} -c session_timeout -M primary'
    ...
    def tearDown(self):
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0009 finish-')
