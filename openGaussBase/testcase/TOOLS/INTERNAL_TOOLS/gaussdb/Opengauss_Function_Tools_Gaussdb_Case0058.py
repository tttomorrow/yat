...
Description :
    ...
    2.使用gaussdb工具指定当前数据库名称启动数据库(若使用&，输入命令后会退出该页面，后台运行)
    gaussdb --single -D /opt/openGauss_luz/cluster/dn1 -p 19703 postgres
    3.查看集群状态是否正常
...
Expect      :
    1.关闭正在运行的数据库成功
    2.使用gaussdb工具指定当前数据库名称启动成功
    3.查看数据库状态成功，集群状态为stop（该方式会启动一个独立使用的界面，
    执行命令时不加&就可以看到）
    4.恢复-重启数据库成功
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0058 start-')
    ...
    def tearDown(self):
        self.logger.info('Opengauss_Function_Tools_Gaussdb_Case0058 finish')
