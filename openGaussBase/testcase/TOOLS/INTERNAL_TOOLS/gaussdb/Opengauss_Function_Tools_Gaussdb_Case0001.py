...
"""
Case Type   : tools
Case Name   : 使用gaussdb 工具指定&后台启动数据库
Description :
    1.关闭正在运行的数据库
    ...
    4.查看进程，确定gaussdb进程运行成功
    ps -ef|grep zl
Expect      :
    1.关闭正在运行的数据库成功
    2.查看进程，数据库连接进程不存在，数据库关闭成功
    3.使用gaussdb工具后台启动数据库成功
    4.查看进程，zl用户下有gaussdb进程正在运行，gaussdb进程运行成功
History     :
"""
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_Gaussdb_Case0001 start--')
    ...
    def tearDown(self):
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0001 finish-')
