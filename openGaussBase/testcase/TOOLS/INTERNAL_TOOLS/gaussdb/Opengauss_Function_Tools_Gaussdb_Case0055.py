...
"""
Case Type   : tools
Case Name   : 在集群备机上使用gaussdb指定--single参数启动是否成功
Description :
    1.关闭正在运行的数据库（备机）
    gs_ctl stop -D /opt/openGauss_zl/cluster/dn1
    2.使用gaussdb工具指定--single启动数据库（备机）(不使用后台运行，直接运行)
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0055 start-')
    ...
    def test_systools(self):
        ...
        if 'Standby' not in msg0:
            ...
            self.standbyNode2 = Node(node='Standby1DbUser')
            ...
            excute_cmd3 = f'source {self.DB_ENV_PATH};gaussdb --single -D ' \
                          ...
                          f'{self.standbyNode.db_port} ' \
                          f'{self.standbyNode.db_name}'
