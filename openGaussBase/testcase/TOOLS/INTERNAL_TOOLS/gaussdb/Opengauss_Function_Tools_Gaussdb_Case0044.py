...
"""
Case Type   : tools
Case Name   : 启动gaussdb进程时，使用-S参数设置值小于最小值，启动是否成功
Description :
    1.关闭正在运行的数据库
    gs_ctl stop -D /opt/openGauss_zl/cluster/dn1
    2.使用gaussdb工具设置-S参数的值小于最小值，启动数据库
    gaussdb -D /opt/openGauss_zl/cluster/dn1 -p 19701 -S 63 -M primary &
Expect      :
    1.关闭正在运行的数据库成功
    2.使用gaussdb工具设置-S参数的值大小于最小值，启动数据库失败
History     :
"""
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_Gaussdb_Case0044 start--')
    ...
    def test_systools(self):
        ...
        excute_cmd3 = f'source {self.DB_ENV_PATH};' \
                      ...
                      f'{self.userNode.db_port} -S 63 -M primary'
    ...
    def tearDown(self):
        ...
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0044 finish-')
