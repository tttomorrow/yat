...
"""
Case Type   : tools
Case Name   : 使用gaussdb开启自启动模式,参数-r的指定文件为不存在的文件是否成功
Description :
    1.关闭正在运行的数据库
    gs_ctl stop -D /opt/openGauss_zl/cluster/dn1
    2.使用gaussdb工具-r指定已存在的文件启动
    gaussdb --boot -D /opt/openGauss_zl/cluster/dn1  -d 5 -r /data/zhanglu/
    testzl.log
    3.查看指定文件是否生成，文件中是否有记录
    cat /data/zhanglu/testzl.log
    4.恢复-重启数据库
    gs_ctl restart -D /opt/openGauss_zl/cluster/dn1 -M primary
Expect      :
    1.关闭正在运行的数据库成功
    2.使用gaussdb工具-r指定已存在的文件启动成功
    3.文件生成成功，文件中有记录
    4.启动数据库成功
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0070 start-')
    ...
    def test_systools(self):
        ...
        self.logger.info('------------查看指定文件中是否有记录-----------')
        excute_cmd4 = f'cat {self.log_path}'
        msg5 = self.userNode.sh(excute_cmd4).result()
        self.assertTrue(len(msg5) > 0)
    ...
    def tearDown(self):
        ...
        self.logger.info('Opengauss_Function_Tools_Gaussdb_Case0070 finish')
