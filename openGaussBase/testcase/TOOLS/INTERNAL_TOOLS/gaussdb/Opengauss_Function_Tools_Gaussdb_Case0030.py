...
"""
Case Type   : tools
Case Name   : 启动gaussdb进程时，使用-N参数设置服务器接受的客户端连接的最大数为字符串，
                启动是否成功
Description :
    1.查看当前最大连接数
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_Gaussdb_Case0030 start--')
    ...
    def test_systools(self):
        ...
        self.logger.info("----使用gaussdb工具指定指定-N参数设置最大连接数为字符串---")
...
-N abc -M primary
    ...
    def tearDown(self):
        ...
        self.common.equal_sql_mdg(msg2, 'max_connections', '5000', '(1 row)',
                                flag='1')
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0030 finish-')
