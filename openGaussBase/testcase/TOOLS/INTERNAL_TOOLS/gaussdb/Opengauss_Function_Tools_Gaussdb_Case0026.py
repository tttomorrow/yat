...
Case Name   : 启动gaussdb进程时，使用-N参数设置服务器接受的客户端连接的最大数启动是否成功，
                执行后数据库中max_connections参数的值是否为设置值
Description :
    1.查看当前最大连接数
    ...
    gaussdb -D /opt/openGauss_zl/cluster/dn1 -p 19701 -N 100 -M primary &
    5.查看当前最大连接数，是否为设置值
    show max_connections;
Expect      :
    1.查看当前最大连接数成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Tools_Gaussdb_Case0026 start--')
    ...
    def test_systools(self):
        ...
        excute_cmd3 = f'source {self.DB_ENV_PATH};' \
                      ...
                      f'{self.userNode.db_port} -N 100 -M primary'
        ...
        self.common.equal_sql_mdg(msg3, 'max_connections', '100', '(1 row)',
                                flag='1')
    ...
    def tearDown(self):
        ...
        self.common.equal_sql_mdg(msg2, 'max_connections', '5000', '(1 row)',
                                flag='1')
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0026 finish-')
