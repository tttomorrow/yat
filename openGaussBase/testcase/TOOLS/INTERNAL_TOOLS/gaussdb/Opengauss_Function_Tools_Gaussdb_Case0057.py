...
"""
Case Type   : tools
Case Name   : 在单机数据库上使用gaussdb指定--single参数作为第一个参数启动是否成功
Description :
    1.关闭正在运行的数据库（单机节点）
    gs_ctl stop -D /opt/openGauss_zl/cluster/dn1
    2.使用gaussdb工具指定--single作为第三个参数启动数据库(若使用&，输入命令后会退出该页面)
    gaussdb --single -D /openGauss_luz/cluster/dn1 -p 19703 --single postgres
    3.查看集群状态是否正常
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('-Opengauss_Function_Tools_Gaussdb_Case0057 start-')
    ...
    def test_systools(self):
        ...
        if 'Standby' in msg0:
            ...
            excute_cmd3 = f'source {self.DB_ENV_PATH};gaussdb -D ' \
                          ...
                          f' {self.userNode.db_port} --single' \
                          f' {self.userNode.db_name}'
    ...
    def tearDown(self):
        self.logger.info('Opengauss_Function_Tools_Gaussdb_Case0057 finish')
