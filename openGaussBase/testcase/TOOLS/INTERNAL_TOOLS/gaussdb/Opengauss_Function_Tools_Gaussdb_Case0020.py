...
"""
Case Type   : tools
Case Name   : 启动gaussdb进程时，主机使用-h参数指定备机host启动是否成功
Description :
    1.关闭正在运行的数据库（备机）
    ...
    ps -ef|grep zl
    3.使用gaussdb工具后台运行进程
    gaussdb -D /opt/openGauss_zl/cluster/dn1 -p 19701 -h 10.183.187.233 -M
    standby &
    4.恢复-重启数据库
...
Expect      :
    ...
    3.使用gaussdb工具指定备机host启动数据库进程失败，提示信息为：
    FATAL:  could not create listen socket for "10.183.187.233:19701"
    4.重启数据库成功
History     :
"""
import unittest
    ...
    def test_systools(self):
        ...
        if 'Standby' not in msg:
            ...
            excute_cmd3 = f'source {self.DB_ENV_PATH};' \
                          ...
                          f' {self.userNode.db_port} -h {host_msg} -M primary'
