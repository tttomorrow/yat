...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--cppflags参数打印用于编译数据库的CPPFLAGS变量的
              数值是否成功
Description :
    1.执行命令打印用于编译数据库的CPPFLAGS变量的数值
Expect      :
    1.执行命令打印用于编译数据库的CPPFLAGS变量的数值成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_pg_config_Case0016开始执行---')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('-----执行命令打印用于编译数据库的CPPFLAGS变量的数值----')
        print_cmd = f'''source {macro.DB_ENV_PATH};
            pg_config --cppflags;'''
        LOG.info(print_cmd)
        print_msg = self.PrimaryNode.sh(print_cmd).result()
        LOG.info(print_msg)
        path = '-D_GNU_SOURCE -I' + self.cluster_path + '/app/include'
        self.assertIn(path, print_msg)

    def tearDown(self):
        LOG.info('-----------------this is tearDown-------------------')
        # 无需清理环境
        LOG.info('---Opengauss_Function_Tools_pg_config_Case0016执行完成---')
