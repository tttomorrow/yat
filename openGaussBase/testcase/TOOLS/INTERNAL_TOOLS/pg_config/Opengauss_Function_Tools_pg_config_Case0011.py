...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--sharedir参数打印体系无关的支持文件的位置是否成功
Description :
    1.执行命令打印体系无关的支持文件的位置
Expect      :
    1.执行命令打印体系无关的支持文件的位置成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_pg_config_Case0011开始执行---')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('----------打印体系无关的支持文件的位置-----------')
        print_cmd = f'''source {macro.DB_ENV_PATH};
            pg_config --sharedir;'''
        LOG.info(print_cmd)
        print_msg = self.PrimaryNode.sh(print_cmd).result()
        LOG.info(print_msg)
        path = self.cluster_path + '/app/share/postgresql'
        self.assertIn(path, print_msg)

    def tearDown(self):
        LOG.info('-----------------this is tearDown-------------------')
        # 无需清理环境
        LOG.info('---Opengauss_Function_Tools_pg_config_Case0011执行完成---')
