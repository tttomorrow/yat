...
Case Name   : 检查当前环境SELinux状态,使用gs_sshexkey创建互信
Description :
    1、检查当前环境SELinux状态
    2、创建备份目录，备份互信文件
    3、创建hostfile,文件路径{dn1/hostfile},将环境IP写入文件hostfile
    4、普通用户下，执行创建互信命令
       gs_sshexkey -f {dn1/hostfile}
    5、清理环境、删除互信文件
Expect      :
    1、SELinux状态为Disabled
    2、创建文件成功，备份互信文件成功
    3、创建文件成功，添加IP成功
    4、执行gs_sshexkey成功，互信建立成功
    5、清理环境成功
History     :
"""
...
class ToolsTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        logger.info("======步驟5：清理环境，删除hostfile文件&恢复环境原有互信关系======")
        logger.info("======备节点清理环境======")
        for i in self.standby_ip_list:
