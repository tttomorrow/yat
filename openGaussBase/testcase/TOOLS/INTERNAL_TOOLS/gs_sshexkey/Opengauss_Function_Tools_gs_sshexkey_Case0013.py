...
Case Name   : root用户下，执行gs_sshexkey --version 命令
Description :
    1、root用户下，执行gs_sshexkey --version
    2、检查选项和用法
Expect      :
    1、命令执行成功
    2、版本信息描述正确
...
class ToolsTestCase(unittest.TestCase):
    ...
    def test_gs_sshexkey(self):
        logger.info("===Opengauss_Function_Tools_gs_sshexkey_Case0013开始执行===")
        logger.info("======root用户下执行gs_sshexkey --version命令======")
    ...
    def tearDown(self):
        ...
        logger.info("===Opengauss_Function_Tools_gs_sshexkey_Case0013执行结束===")
