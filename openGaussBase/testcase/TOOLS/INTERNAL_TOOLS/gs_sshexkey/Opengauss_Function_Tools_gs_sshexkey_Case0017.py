...
Description :
    ...
    3、root用户下，执行创建互信命令,添加-l参数指定日志路径
       gs_sshexkey -f {dn1/hostfile} -l {dn1/logfile}
    4、清理环境、删除互信文件
Expect      :
    1、创建文件成功，备份互信文件成功
    2、创建文件成功，添加IP成功
    3、执行gs_sshexkey命令成功，输出写入日志中
    4、清理环境成功
History     :
"""
...
class ToolsTestCase(unittest.TestCase):
    ...
    def test_gs_sshexkey(self):
        logger.info("Opengauss_Function_Tools_gs_sshexkey_Case0017开始执行")
        ...
        logger.info("======步驟3：执行gs_sshexkey命令,指定日志路径======")
    ...
    def tearDown(self):
        ...
        logger.info(rm_res6)
        logger.info("===Opengauss_Function_Tools_gs_sshexkey_Case0017执行结束===")
