...
Case Name   : 普通用户下，使用gs_sshexkey创建互信,ip文件包含其他信息
Description :
    1、创建hostfile,文件路径{dn1/hostfile},将环境IP和其他信息写入文件hostfile
    2、普通用户下，执行创建互信命令,gs_sshexkey -f {dn1/hostfile} -W {db_password}
    3、清理环境、删除互信文件
Expect      :
    1、创建文件成功，添加IP成功
    2、执行gs_sshexkey命令失败
...
class ToolsTestCase(unittest.TestCase):
    ...
    def test_gs_sshexkey(self):
        logger.info("===Opengauss_Function_Tools_gs_sshexkey_Case0007开始执行===")
        logger.info("======步驟1：普通用户下创建hostfile文件，添加主备IP&其他信息======")
        add_cmd = f'''touch {self.ssh_file}
            echo -e 'IP1:{self.IP1}\nIP2:{self.IP2}' > {self.ssh_file}
    ...
    def tearDown(self):
        ...
        logger.info("===Opengauss_Function_Tools_gs_sshexkey_Case0007执行结束===")
