...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup set-config命令指定--log-level-console=info、
              --log-level-file=info、--log-filename=pg_probackup-1.log、
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0203')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0203 start----')
    ...
    def test_server_tools(self):
        ...
        backup_cmd = f"source {macro.DB_ENV_PATH};" \
                     ...
                     f"--log-level-file=info " \
                     f"--log-filename=pg_probackup-1.log " \
                     f"--error-log-filename='gs_probackup_error.log' " \
                     f"--log-directory=203log"
        ...
        exec_msg = self.Primary_User_Node.sh(add_cmd).result()
        self.log.info(exec_msg)
        self.assertIn('log-directory = 203log', exec_msg, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0203 end----')
