...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--external-mapping=OLDDIR=NEWDIR 选项，执行restore
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0075开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        self.assertIn(f'Restore of backup {self.backupid} completed',
                      restore_msg)
        LOG.info('-----------step5.2 查看外部目录是否重定向--------------')
        cat_cmd = f"ls {self.cluster_path}/bbb;"
        LOG.info(cat_cmd)
        cat_msg = self.PrimaryNode.sh(cat_cmd).result()
        LOG.info(cat_msg)
        self.assertIn('a.dat', cat_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0075执行完成-')
