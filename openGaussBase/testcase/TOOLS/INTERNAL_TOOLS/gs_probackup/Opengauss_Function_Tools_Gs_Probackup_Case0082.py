...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用--tablespace-mapping但不使用指定--external-mapping，
              执行restore
Description :
    1.新建目录
    2.创建表空间及表
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0082开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-i {self.backupid} --tablespace-mapping " \
            f"{self.cluster_path}/aaa={self.cluster_path}/bbb ;"
        LOG.info(restore_cmd)
        restore_msg = self.PrimaryNode.sh(restore_cmd).result()
        LOG.info(restore_msg)
        self.assertIn('If specify --tablespace-mapping option, you must '
                      'specify --external-mapping option together',
                      restore_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0082执行完成-')
