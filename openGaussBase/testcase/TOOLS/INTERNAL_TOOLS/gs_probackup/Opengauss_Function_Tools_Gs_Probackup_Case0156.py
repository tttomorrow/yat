...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup set-config命令添加远程模式相关参数，
              --remote-proto错误
Description :
    1.创建备份目录
    2.进行初始化
    3.添加远程实例
    4.将远程连接添加到pg_probackup.conf配置文件
    5.查看pg_probackup.conf配置文件中是否有远程连接信息
    6.恢复环境
Expect      :
    1.创建备份目录成功
    2.初始化成功
    3.添加远程实例成功
    4.添加完成,此处不校验参数信息
    6.pg_probackup.conf配置文件中有远程连接信息
    6.恢复环境完成
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0156')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0156 start----')
    ...
    def test_server_tools(self):
        ...
        self.assertIn("'test_0156' " + self.constant.init_success, exec_msg,
                      '执行失败:' + text)
        ...
        text = '---step5:查看pg_probackup.conf配置文件中是否有远程连接信息;' \
               'expect:远程连接信息添加成功---'
        self.log.info(text)
        cat_cmd = f"cat {self.gs_probackup_path}/backups/test_0156/" \
                  f"pg_probackup.conf"
        self.log.info(cat_cmd)
        cat_msg = self.Primary_User_Node.sh(cat_cmd).result()
        self.log.info(cat_msg)
        self.assertIn('remote-proto = tcp' and
                      f'remote-host = {self.Standby_User_Node.db_host}',
                      cat_msg, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0156 end----')
