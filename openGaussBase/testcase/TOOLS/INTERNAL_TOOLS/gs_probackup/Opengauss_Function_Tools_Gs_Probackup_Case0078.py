...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定后表空间从OLDDIR重新定位到NEWDIR目录（-T），执行restore
Description :
    1.新建目录
    2.创建表空间及表
    3.进行初始化
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0078开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        check_cmd = f"ls {self.cluster_path}/bbb ;"
        LOG.info(check_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0078执行完成-')
