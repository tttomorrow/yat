...
Case Name   : 远程模式下，执行全备份,通过SSH连接对远程系统进行全备份，
              ...
              -U指定普通用户,指定--retention-window、--retention-redundancy选项
Description :
    1.创建备份目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(
            ...
            'gs_probackup_testdir0169')
        self.pg_hba = os.path.join(macro.DB_INSTANCE_PATH,
                                   macro.PG_HBA_FILE_NAME)
        self.us_name = "us_gs_probackup_0169"
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0169 start----')
    ...
    def test_server_tools(self):
        ...
        init_msg = self.Primary_User_Node.sh(init_cmd).result()
        self.log.info(init_msg)
        self.assertIn("'test_0169' " + self.constant.init_success, init_msg,
                      '执行失败:' + text)
        ...
        backup_cmd = f"source {macro.DB_ENV_PATH}; " \
                     ...
                     f" --retention-window=7 " \
                     f"--retention-redundancy=3"
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0169 end----')
