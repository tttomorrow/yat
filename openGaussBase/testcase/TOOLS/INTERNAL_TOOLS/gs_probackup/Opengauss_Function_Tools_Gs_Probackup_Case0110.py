...
"""
Case Type   : 系统内部使用工具
Case Name   : 不指定-i，检查是否可以从备份副本恢复到指定的lsn
            （lsn不是Stop LSN）
Description :
    1.新建目录
    2.进行初始化
...
Expect      :
    ...
    4.进行备份成功
    5.查看备份信息成功
    6.执行validate失败，报错Backup satisfying target options is not found
    7.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0110开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        self.assertIn('Backup satisfying target options is not found',
                      validate_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0110执行完成-')
