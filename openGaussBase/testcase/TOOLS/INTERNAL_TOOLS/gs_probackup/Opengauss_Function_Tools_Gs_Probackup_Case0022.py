...
"""
Case Type   : 系统内部使用工具
Case Name   : 只指定-B、--instance、-i选项，执行gs_probackup set-backup
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
    ...
    5.执行全量备份
    6.查看备份的外部文件
    7.只指定-B、--instance、-i选项执行set-backup
    8.删除新建目录
Expect      :
    1.新建目录成功
    2.进行初始化成功
    ...
    5.执行全量备份成功
    6.查看备份的外部文件，该文件备份成功
    7.只指定-B、--instance、-i选项执行set-backup失败
    8.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0022开始执行-')
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0022执行完成-')
