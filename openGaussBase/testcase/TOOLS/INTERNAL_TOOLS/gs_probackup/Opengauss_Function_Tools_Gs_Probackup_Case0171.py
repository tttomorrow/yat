...
Case Name   : 远程模式下，执行增量备份,通过SSH连接对远程系统进行增量备份，
             ...
             --remote-port、--remote-path<非远程安装目录>-d、-p、-U选项，
             -U指定普通用户，合理报错
Description :
...
class Probackup(unittest.TestCase):
    def setUp(self):
        ...
        self.us_name = "us_gs_probackup_0171"
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0171 start----')
    ...
    def test_server_tools(self):
        ...
        init_cmd = f"source {macro.DB_ENV_PATH};" \
                   ...
                   f" --instance=test_slave_backup0171 " \
                   f"-D {macro.DB_INSTANCE_PATH}"
        self.log.info(init_cmd)
        init_msg = self.Primary_User_Node.sh(init_cmd).result()
        self.log.info(init_msg)
        self.assertIn("'test_slave_backup0171' " + self.constant.init_success,
                      init_msg, '执行失败:' + text)
        ...
        text = '--step9:进行增量远程备份;expect:增量备份失败----'
        self.log.info(text)
        backup_cmd = f"source {macro.DB_ENV_PATH};" \
                     f"gs_probackup backup " \
                     f"-B {self.gs_probackup_path} " \
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0171 end----')
