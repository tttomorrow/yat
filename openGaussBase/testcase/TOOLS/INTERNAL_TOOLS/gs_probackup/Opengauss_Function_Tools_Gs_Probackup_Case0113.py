...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--recovery-target-lsn=lsn、
              --recovery-target-inclusive=false，检查备份文件
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
    4.进行备份
    5.查看备份XID（recovery-xid）
    6.执行validate
    7.删除新建目录
Expect      :
    1.新建目录成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0113开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        validate_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f" --instance=pro1 --recovery-target-inclusive=false " \
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0113执行完成-')
