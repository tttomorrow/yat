...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定-i、--recovery-target-time=time，目标恢复时间和backup_id
              时间不匹配，执行validate验证备份文件
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
    4.进行备份
    5.查看备份信息
    6.执行validate（目标恢复时间和backup_id 时间不匹配）
    7.删除新建目录
Expect      :
    1.新建目录成功
    2.进行初始化成功
    ...
    4.进行备份成功
    5.查看备份信息成功
    6.执行validate失败，报错Requested backup does not satisfy restore options
    7.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0104开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        self.row_msg = self.show_msg.splitlines()[-1].strip()
        LOG.info(self.row_msg)
        self.time_msg1 = self.row_msg.split()[3]
        LOG.info(self.time_msg1)
        self.time_msg2 = self.row_msg.split()[4]
        LOG.info(self.time_msg2)

        LOG.info('-----------step6 执行validate--------------')
        recovery_msg = f'{self.time_msg1} '+f'{self.time_msg2}'
        ...
        validate_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"--recovery-target-time='{self.time_msg1}';"
        LOG.info(validate_cmd)
        validate_msg = self.PrimaryNode.sh(validate_cmd).result()
        LOG.info(validate_msg)
        self.assertIn('does not satisfy restore options', validate_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0104执行完成-')
