...
Case Name   : 使用gs_probackup set-config命令指定--log-level-console=warning、
              ...
              --log-directory=<绝对路径>，省略--log-level-file参数
Description :
    1.创建备份目录和日志文件的目录
    2.进行初始化
    3.添加远程实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0201')
        self.gs_probackup_log = os.path.join(macro.DB_INSTANCE_PATH,
                                             'gs_probackup_log0201')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0201 start----')
    ...
    def test_server_tools(self):
        ...
        exec_msg = self.Primary_User_Node.sh(add_cmd).result()
        self.log.info(exec_msg)
        self.assertIn('log-level-console = WARNING', exec_msg,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0201 end----')
