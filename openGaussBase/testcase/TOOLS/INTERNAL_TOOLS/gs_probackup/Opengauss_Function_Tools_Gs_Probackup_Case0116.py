...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--recovery-target-name参数为其他值，检查是否有合理报错
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0116开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        validate_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f" --instance=pro1 -i {self.backupid} " \
            f"--recovery-target-name={self.name_msg}N;"
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0116执行完成-')
