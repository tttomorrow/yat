...
"""
Case Type   : 系统内部使用工具
Case Name   : 并发执行、使用临时物理复制槽、将指定的多个目录包含到备份中，
              指定-j、--temp-slot选项，执行备份
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0050开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        back_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"{self.PrimaryNode.db_port} --temp-slot ;"
        LOG.info(back_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0050执行完成-')
