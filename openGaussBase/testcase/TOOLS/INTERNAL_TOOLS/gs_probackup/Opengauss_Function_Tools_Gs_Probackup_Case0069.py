...
"""
Case Type   : 系统内部使用工具
Case Name   : 不指定--no-validate，执行restore
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
Expect      :
    ...
    4.进行备份成功
    5.restore成功，有验证信息INFO: Validating backup QM53JP
    INFO: Backup QM53JP data files are valid
    6.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0069开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info(mkdir_msg)
        self.assertIn('testdir', mkdir_msg)
        self.assertIn('prodir', mkdir_msg)
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-i {self.backupid} --incremental-mode=checksum ;"
        ...
        self.assertIn(f"Restore of backup {self.backupid} completed",
                      restore_msg)
        self.assertIn('data files are valid', restore_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0069执行完成-')
