...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--wal-depth为浮点型，执行delete
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0095开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        delete_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f" --instance=pro1 -i {self.fullbackupid} --wal-depth=1.1;"
        LOG.info(delete_cmd)
        delete_msg = self.PrimaryNode.sh(delete_cmd).result()
        LOG.info(delete_msg)
        self.assertIn('Option --wal-depth should be a 32bit unsigned integer',
                      delete_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0095执行完成-')
