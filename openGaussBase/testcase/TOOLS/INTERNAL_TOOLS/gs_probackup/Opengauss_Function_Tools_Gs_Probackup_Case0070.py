...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--no-validate，执行restore
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
Expect      :
    ...
    4.进行备份成功
    5.restore成功，没有验证信息INFO: Validating backup QM53JP
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0070开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        self.assertIn(f"Restore of backup {self.backupid} completed",
                      restore_msg)
        self.assertNotIn('data files are valid', restore_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0070执行完成-')
