...
"""
Case Type   : 系统内部使用工具
Case Name   : 显示pg_probackup.conf配置文件的内容,指定--format=json
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
    4.向pg_probackup.conf配置文件中添加设置信息
    5.查看pg_probackup.conf配置文件的内容，是否为纯文本格式
    6.删除新建目录
Expect      :
    1.新建目录成功
    2.进行初始化成功
    ...
    5.查看pg_probackup.conf配置文件的内容成功，为json格式
    6.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0034开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('---step5 查看pg_probackup.conf配置文件是否为JSON格式---')
        cat_cmd = f"source {macro.DB_ENV_PATH};" \
            f"gs_probackup show-config -B " \
            f"{macro.DB_BACKUP_PATH}/testdir --instance=pro1 --format=json"
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0034执行完成-')
