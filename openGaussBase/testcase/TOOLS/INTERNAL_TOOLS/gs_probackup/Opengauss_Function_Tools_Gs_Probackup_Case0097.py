...
"""
Case Type   : 系统内部使用工具
Case Name   : 同时指定--delete-expired、--merge-expired、
              --retention-redundancy选项
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0097开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        fullback_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"gs_probackup backup -B {self.cluster_path}/testdir " \
            f" --instance=pro1 -b full  -d {self.PrimaryNode.db_name} -p " \
            f"{self.PrimaryNode.db_port} --delete-expired --merge-expired " \
        ...
        LOG.info(show_msg)
        self.assertTrue(show_msg.count('pro1') == 3)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0097执行完成-')
