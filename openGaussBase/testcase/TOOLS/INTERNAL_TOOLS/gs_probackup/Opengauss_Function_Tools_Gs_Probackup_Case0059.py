...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0059开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        init_cmd = f"mkdir {self.cluster_path}/testdir;" \
            f"mkdir {self.cluster_path}/extdir1;" \
            f"mkdir {self.cluster_path}/extdir2;" \
            f"touch {self.cluster_path}/extdir1/aaa;" \
            f"touch {self.cluster_path}/extdir2/bbb;" \
            f"echo 'gs_probackup test'>>{self.cluster_path}/extdir1/aaa;" \
            f"echo 'gs_probackup test'>>{self.cluster_path}/extdir2/bbb;" \
            f"ls {self.cluster_path}/extdir1;" \
            f"ls {self.cluster_path}/extdir2;"
        LOG.info(init_cmd)
        init_msg = self.PrimaryNode.sh(init_cmd).result()
        LOG.info(init_msg)
        self.assertIn('aaa', init_msg)
        self.assertIn('bbb', init_msg)
        ...
        back_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"{self.PrimaryNode.db_port} --compress-algorithm=pglz -E " \
            f"{self.cluster_path}/extdir1:{self.cluster_path}/extdir2 ;"
        LOG.info(back_cmd)
        ...
        check_cmd1 = f"ls " \
            ...
            f"/external_directories/externaldir1;"
        LOG.info(check_cmd1)
        check_msg1 = self.PrimaryNode.sh(check_cmd1).result()
        LOG.info(check_msg1)
        self.assertIn('aaa', check_msg1)
        check_cmd2 = f"ls " \
            f"{self.cluster_path}/testdir/backups/pro1/{self.backupid}" \
            f"/external_directories/externaldir2;"
        LOG.info(check_cmd2)
        check_msg2 = self.PrimaryNode.sh(check_cmd2).result()
        LOG.info(check_msg2)
        self.assertIn('bbb', check_msg2)

    def tearDown(self):
        LOG.info('------------------this is tearDown--------------------')
        LOG.info('------------------step5 删除新建目录------------------')
        clear_cmd = f"rm -rf {self.cluster_path}/testdir;" \
            f"rm -rf {self.cluster_path}/extdir1;" \
            f"rm -rf {self.cluster_path}/extdir2;"
        LOG.info(clear_cmd)
        clear_msg = self.PrimaryNode.sh(clear_cmd).result()
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0059执行完成-')
