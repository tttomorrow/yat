...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--expire-time选项从恢复时间开始计算，备份要固定的时间量
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
Expect      :
    ...
    4.备份成功
    5.指定从恢复时间开始计算，备份要固定的时间量成功，显示备份的过期
      时间为指定时间
    6.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0025开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        set_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"--expire-time='2021-01-01 00:00:00+03'; "
        LOG.info(set_cmd)
        set_msg = self.PrimaryNode.sh(set_cmd).result()
        LOG.info(set_msg)
        self.assertIn(f'2021-01-01 05:00:00+08', set_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0025执行完成-')
