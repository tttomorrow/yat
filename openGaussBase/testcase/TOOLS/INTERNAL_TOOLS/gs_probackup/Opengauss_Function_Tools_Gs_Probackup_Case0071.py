...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定特定备份id，并发执行、跳过块级验证、强制恢复，执行restore
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0071开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-i {self.backupid} --no-validate --skip-block-validation " \
            f"-j 4 --force ;"
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0071执行完成-')
