...
"""
Case Type   : 系统内部使用工具
Case Name   : 只指定--restore-command=cmdline,执行 validate
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
    4.进行备份
    5.查看保存点名称（recovery-name）
    6.执行validate
    7.删除新建目录
Expect      :
    1.新建目录成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0117开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        self.row_msg = self.show_msg.splitlines()[-7].strip()
        LOG.info(self.row_msg)
        self.name_msg = self.row_msg.split('=')[1].strip()
        LOG.info(self.name_msg)

        LOG.info('-----------step6 执行validate--------------')
        self.vars = 'cp /srv/archivedir/%f "%p"'
        validate_cmd = f"source {macro.DB_ENV_PATH};" \
            f"gs_probackup validate -B {self.cluster_path}/testdir " \
            f" --instance=pro1 -i {self.backupid} " \
            f"--restore-command='{self.vars}';"
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0117执行完成-')
