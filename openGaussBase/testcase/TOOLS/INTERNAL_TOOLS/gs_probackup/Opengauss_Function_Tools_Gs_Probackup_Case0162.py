...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup set-config命令添加--ssh-options选项
Description :
    1.创建备份目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0162')
        self.remote_lib = os.path.join(os.path.dirname(macro.DB_INSTANCE_PATH),
                                       'app', 'lib')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0162 start----')
    ...
    def test_server_tools(self):
        ...
        init_msg = self.Primary_User_Node.sh(init_cmd).result()
        self.log.info(init_msg)
        self.assertIn("'test_0162' " + self.constant.init_success, init_msg,
                      '执行失败:' + text)
        ...
        text = '--step5:查看pg_probackup.conf配置文件;' \
               'expect:显示ssh-options信息--'
        self.log.info(text)
        cat_cmd = f"cat {self.gs_probackup_path}/backups/test_0162/" \
                  f"pg_probackup.conf"
        self.log.info(cat_cmd)
        cat_msg = self.Primary_User_Node.sh(cat_cmd).result()
        self.log.info(cat_msg)
        self.assertIn("ssh-options = '-o ServerAliveCountMax=5"
                      " -o ServerAliveInterval=60'", cat_msg)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0162 end----')
