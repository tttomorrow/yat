...
Expect      :
    ...
    4.进行备份成功
    5.执行validate失败，报错required parameter not specified:
    BACKUP_PATH (-B, --backup-path)
    6.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0099开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        show_cmd = f"source {macro.DB_ENV_PATH};" \
            f"gs_probackup validate;"
        LOG.info(show_cmd)
        show_msg = self.PrimaryNode.sh(show_cmd).result()
        LOG.info(show_msg)
        self.assertIn('required parameter not specified', show_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0099执行完成-')
