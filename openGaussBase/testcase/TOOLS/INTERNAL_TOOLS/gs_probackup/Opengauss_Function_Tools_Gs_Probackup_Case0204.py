...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0204')
        self.us_name = "us_gs_probackup_0204"
        self.db_name = "db_gs_probackup_0204"
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0204 start----')
    ...
    def test_server_tools(self):
        ...
        add_cmd = f"source {macro.DB_ENV_PATH};" \
                  ...
                  f"--instance=test_0204 "
        self.log.info(add_cmd)
        exec_msg = self.Primary_User_Node.sh(add_cmd).result()
        self.log.info(exec_msg)
        self.assertIn("'test_0204' " + self.constant.init_success, exec_msg,
                      '执行失败:' + text)

        text = '--step4:进行全量备份;expect:备份成功----'
        self.log.info(text)
        backup_cmd = f"source {macro.DB_ENV_PATH};" \
                     ...
                     f"-W {self.Primary_User_Node.ssh_password} " \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0204 end----')
