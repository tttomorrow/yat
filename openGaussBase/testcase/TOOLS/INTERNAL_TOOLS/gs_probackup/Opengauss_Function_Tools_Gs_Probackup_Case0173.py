...
Case Name   : 远程模式下，执行增量备份,通过SSH连接对远程系统进行增量备份指定备
              份要固定的时间量，指定--ttl选项
Description :
...
class Probackup(unittest.TestCase):
    def setUp(self):
        ...
        self.us_name = "us_gs_probackup_0173"
    ...
    def test_server_tools(self):
        ...
        init_cmd = f"source {macro.DB_ENV_PATH};" \
                   ...
                   f" --instance=test_slave_backup0173 " \
                   f"-D {macro.DB_INSTANCE_PATH}"
        self.log.info(init_cmd)
        init_msg = self.Primary_User_Node.sh(init_cmd).result()
        self.log.info(init_msg)
        self.assertIn("'test_slave_backup0173' " + self.constant.init_success,
                      init_msg, '执行失败:' + text)

        text = '--step5:创建测试用户;expect:创建用户成功---'
        self.log.info(text)
        create_cmd = Primary_SH.execut_db_sql(f"drop user if exists "
                                              f"{self.us_name};"
        ...
        self.assertIn(self.constant.CREATE_ROLE_SUCCESS_MSG, create_cmd,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        rm_result = self.Primary_User_Node.sh(rm_cmd).result()
        self.log.info(rm_result)
        drop_cmd = Primary_SH.execut_db_sql(f"drop user {self.us_name};")
        self.log.info(drop_cmd)
        sed_cmd1 = f"sed -i '/enable_cbm_tracking/d' {self.conf_file}"
        self.log.info(sed_cmd1)
        sed_result1 = self.Primary_User_Node.sh(sed_cmd1).result()
        ...
        guc_result = self.Primary_User_Node.sh(guc_cmd).result()
        self.log.info(guc_result)
        sed_cmd2 = f"sed -i '/{self.us_name}/d' {self.pg_hba}"
        self.log.info(sed_cmd2)
        sed_result2 = self.Primary_User_Node.sh(sed_cmd2).result()
