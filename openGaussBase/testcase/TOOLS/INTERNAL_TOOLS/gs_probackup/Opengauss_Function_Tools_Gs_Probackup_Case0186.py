...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup backup命令指定-j、--compress-algorithm=pglz选项，
              合理报错
Description :
    1.新建备份目录
    2.进行初始化
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0186start--')
    ...
    def test_system_internal_tools(self):
        ...
        init_msg = self.Primary_Node.sh(init_cmd).result()
        LOG.info(init_msg)
        self.assertIn("'test_186' " + self.constant.init_success, init_msg)
        LOG.info('---step4查看pg_probackup.conf配置文件---')
        cat_cmd = f"cat {self.gs_probackup_bak_path}/backups/" \
                  f"test_186/pg_probackup.conf"
        LOG.info(cat_cmd)
        ...
        backup_cmd = f"source {macro.DB_ENV_PATH};" \
                     ...
                     f"-b FULL  " \
                     f"-j 6 " \
                     f"--compress-algorithm=pglz " \
                     f"-d postgres " \
                     f"-p {self.Primary_Node.db_port}"
        LOG.info(backup_cmd)
        exec_msg = self.Primary_Node.sh(backup_cmd).result()
        LOG.info(exec_msg)
        self.assertIn('ERROR: Multithread backup does not support '
                      'pglz compression', exec_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0186finish-')
