...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用help显示gs_probackup validate命令的摘要信息
Description :
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0138开始执行-')
        ...
        self.except_msg = ['gs_probackup validate -B backup-path',
                           ...
                           '',
                           '-B, --backup-path=backup-path    location of '
                           'the backup storage area',
                           ...
                           '--progress                   '
                           'show progress',
                           '--skip-block-validation      set to '
                           'validate only file-level checksum',
                           '', 'Recovery options:',
                           '--recovery-target-time=time  time stamp up '
                           'to which recovery will proceed',
                           '--recovery-target-xid=xid    transaction ID up '
                           'to which recovery will proceed',
    ...
    def tearDown(self):
        ...
        # 无须清理环境
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0138执行完成--')
