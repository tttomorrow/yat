...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_probackup add-instance备份多个外部文件
Description :
    1.新建备份目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例，并且指定多个需要备份的其他目录
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0013开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        init_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-D {macro.DB_INSTANCE_PATH} -E " \
            f"{macro.DB_BACKUP_PATH}/dir1:{macro.DB_BACKUP_PATH}/dir2 " \
            f"--instance=pro1;"
        LOG.info(init_cmd)
        init_msg = self.PrimaryNode.sh(init_cmd).result()
        LOG.info(init_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0013执行完成-')
