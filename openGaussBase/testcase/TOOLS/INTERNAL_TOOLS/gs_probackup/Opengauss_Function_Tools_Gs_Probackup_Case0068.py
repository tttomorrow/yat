...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--no-sync进行restore,有提示信息不将备份文件同步到磁盘
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
Expect      :
    ...
    4.进行备份成功
    5.restore成功，有提示信息WARNING: Restored files are not synced to disk
    6.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0068开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        self.assertIn(f"Restore of backup {self.backupid} completed",
                      restore_msg)
        self.assertIn('Restored files are not synced to disk', restore_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0068执行完成-')
