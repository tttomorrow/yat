...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0160')
        self.remote_lib = os.path.join(os.path.dirname(macro.DB_INSTANCE_PATH),
                                       'app', 'lib')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0160start----')
    ...
    def test_server_tools(self):
        ...
        self.assertIn("'test_0160' " + self.constant.init_success, exec_msg,
                      '执行失败:' + text)
        ...
        text = '---step5:查看pg_probackup.conf配置文件中是否有远程连接信息;' \
               'expect:远程连接信息添加成功---'
        self.log.info(text)
        cat_cmd = f"cat {self.gs_probackup_path}/backups/test_0160/" \
                  f"pg_probackup.conf"
        self.log.info(cat_cmd)
        cat_msg = self.Primary_User_Node.sh(cat_cmd).result()
        self.log.info(cat_msg)
        self.assertIn(f'remote-host = {self.Standby_User_Node.db_host}',
                      cat_msg, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0160end----')
