...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup add-instance命令添加--ssh-options选项
Description :
    1.创建备份目录
    2.进行初始化
    3.添加远程实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0154')
        self.remote_lib = os.path.join(os.path.dirname(macro.DB_INSTANCE_PATH),
                                       'app', 'lib')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0154 start----')
    ...
    def test_server_tools(self):
        ...
        self.assertIn("'test_0154' " + self.constant.init_success, exec_msg,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0154 end----')
