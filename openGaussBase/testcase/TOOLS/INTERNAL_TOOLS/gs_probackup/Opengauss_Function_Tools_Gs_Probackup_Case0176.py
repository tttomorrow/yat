...
Case Name   : 使用gs_probackup backup命令同时指定--remote-proto=ssh 、
              ...
              --remote-user=<user>选项，-U为初始用户，远端主机以sha256方式，
              合理报错
Description :
...
Expect      :
    ...
    7.配置成功
    5.创建成功
    8.全量远程备份失败FATAL:  Forbid remote connection with initial user
    9.增量远程备份失败ERROR: backup destination is not empty
    10.备份成功
    11.清理环境成功
History     :
"""
...
class Probackup(unittest.TestCase):
    def setUp(self):
        ...
        self.us_name = "us_gs_probackup_0176"
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0176 start----')
    ...
    def test_server_tools(self):
        ...
        init_cmd = f"source {macro.DB_ENV_PATH};" \
                   ...
                   f"--instance=test_slave_backup0176 " \
                   f"-D {macro.DB_INSTANCE_PATH}"
        self.log.info(init_cmd)
        init_msg = self.Primary_User_Node.sh(init_cmd).result()
        self.log.info(init_msg)
        self.assertIn("'test_slave_backup0176' " + self.constant.init_success,
                      init_msg, '执行失败:' + text)
        ...
        text = '--step7:进行全量远程备份;expect:远端备份失败----'
        self.log.info(text)
        ...
        text = '--step8:进行增量远程备份;expect:远端备份失败----'
        self.log.info(text)
        backup_cmd = f"source {macro.DB_ENV_PATH};" \
                     f"gs_probackup backup " \
                     f"-B {self.gs_probackup_path} " \
        ...
        exec_msg = self.Primary_User_Node.sh(backup_cmd).result()
        self.log.info(exec_msg)
        self.assertIn('ERROR', exec_msg, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0176 end----')
