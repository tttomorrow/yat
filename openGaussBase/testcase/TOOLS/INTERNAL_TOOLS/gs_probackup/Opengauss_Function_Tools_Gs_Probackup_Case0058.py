...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0058开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        init_cmd = f"mkdir {self.cluster_path}/testdir;" \
            f"mkdir {self.cluster_path}/extdir;" \
            f"touch {self.cluster_path}/extdir/aaa;" \
            f"echo 'gs_probackup test '>>{self.cluster_path}/extdir/aaa;" \
            f"ls {self.cluster_path}/extdir;"
        LOG.info(init_cmd)
        ...
        LOG.info(check_msg)
        self.assertIn('aaa', check_msg)

    def tearDown(self):
        LOG.info('------------------this is tearDown--------------------')
        LOG.info('------------------step5 删除新建目录------------------')
        clear_cmd = f"rm -rf {self.cluster_path}/testdir;" \
            f"rm -rf {self.cluster_path}/extdir;"
        LOG.info(clear_cmd)
        clear_msg = self.PrimaryNode.sh(clear_cmd).result()
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0058执行完成-')
