...
Expect      :
    ...
    gs_probackup设置
    5.备份成功，备份目录下生成备份内容，pid以及数据库信息；压缩比最大，
    处理速度最慢
    6.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0182start--')
    ...
    def test_system_internal_tools(self):
        ...
        init_msg = self.Primary_Node.sh(init_cmd).result()
        LOG.info(init_msg)
        self.assertIn("'test_182' " + self.constant.init_success, init_msg)
        LOG.info('---step4查看pg_probackup.conf配置文件---')
        cat_cmd = f"cat {self.gs_probackup_bak_path}/backups/" \
                  f"test_182/pg_probackup.conf"
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0182finish-')
