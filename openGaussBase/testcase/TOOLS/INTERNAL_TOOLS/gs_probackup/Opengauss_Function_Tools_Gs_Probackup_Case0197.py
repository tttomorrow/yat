...
"""
Case Type   : 系统内部使用工具
Case Name   :使用gs_probackup set-config命令指定--log-level-console=verbose、
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0197')
        self.gs_probackup_log = os.path.join(macro.DB_INSTANCE_PATH,
                                             'gs_probackup_log0197')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0197 start----')
    ...
    def test_server_tools(self):
        ...
        backup_cmd = f"source {macro.DB_ENV_PATH};" \
                     ...
                     f"-B {self.gs_probackup_path} " \
                     f"--instance=test_0197 " \
                     f"--log-level-console=verbose " \
                     f"--log-level-file=warning " \
        ...
        exec_msg = self.Primary_User_Node.sh(add_cmd).result()
        self.log.info(exec_msg)
        self.assertIn('log-level-console = VERBOSE' and
                      f'log-directory = {self.gs_probackup_log}', exec_msg,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0197 end----')
