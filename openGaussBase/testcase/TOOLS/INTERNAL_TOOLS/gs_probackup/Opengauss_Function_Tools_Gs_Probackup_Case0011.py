...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_probackup add-instance后是否生成pg_probackup.conf配置文件，
              并且记录了指定数据目录data_dir的gs_probackup设置
Description :
    1.新建备份目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
    4.查看是否生成pg_probackup.conf配置文件(并且记录了
    指定数据目录data_dir的gs_probackup设置)
    5.删除新建目录
Expect      :
    1.新建备份目录成功
    2.进行初始化成功
    3.初始化新的备份实例成功
    4.查看pg_probackup.conf文件成功，文件中记录了指定数据
    目录data_dir的gs_probackup设置
    5.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0011开始执行--')
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0011执行完成-')
