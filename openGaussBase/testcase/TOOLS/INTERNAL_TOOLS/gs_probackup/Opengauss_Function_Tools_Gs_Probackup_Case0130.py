...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用help显示gs_probackup set-config命令的摘要信息
Description :
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0130开始执行-')
        self.constant = Constant()
        self.PrimaryNode = Node('PrimaryDbUser')
        self.except_msg = ['gs_probackup set-config -B backup-path '
                           ...
                           'valid backups with ability to perform',
                           'the point in time recovery;  disables; '
                           '(default: 0)',
                           '', 'Compression options:',
                           '--compress-algorithm=compress-algorithm',
                           ...
                           '', 'Connection options:',
                           '-U, --pguser=username            user name to '
                           'connect as (default: current local user)',
                           '-d, --pgdatabase=dbname          '
                           'database to connect (default: username)',
                           "-h, --pghost=hostname            database server"
                           " host or socket directory(default: "
                           "'local socket')",
                           '-p, --pgport=port                '
    ...
    def tearDown(self):
        ...
        # 无须清理环境
        LOG.info(
            '---Opengauss_Function_Tools_Gs_Probackup_Case0130执行完成---')
