...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup set-config命令指定--log-level-console=warning、
              --log-level-file=info、
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(
            ...
            'gs_probackup_testdir0194')
        self.gs_probackup_log = os.path.join(
            os.path.dirname(macro.DB_INSTANCE_PATH),
            'gs_probackup_log0194')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0194 start----')
    ...
    def test_server_tools(self):
        ...
        exec_msg = self.Primary_User_Node.sh(add_cmd).result()
        self.log.info(exec_msg)
        self.assertIn(f'log-directory = {self.gs_probackup_log}', exec_msg,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0194 end----')
