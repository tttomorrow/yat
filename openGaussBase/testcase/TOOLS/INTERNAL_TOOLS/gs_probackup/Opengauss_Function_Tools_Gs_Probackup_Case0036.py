...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--format=plain，使用show命令显示备份目录的内容，
              显示是否为纯文本格式
Description :
    1.新建目录
    2.进行初始化
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0036开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        cat_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"--format=plain -i {self.backupid}"
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0036执行完成-')
