...
Case Name   : 远程模式下，执行增量备份,通过SSH连接对远程系统进行增量备份指定
              备份要固定的时间量，指定--expire-time选项
Description :
...
class Probackup(unittest.TestCase):
    def setUp(self):
        ...
        self.us_name = "us_gs_probackup_0174"
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0174 start----')
    ...
    def test_server_tools(self):
        ...
        init_cmd = f"source {macro.DB_ENV_PATH};" \
                   ...
                   f"--instance=test_slave_backup0174 " \
                   f"-D {macro.DB_INSTANCE_PATH}"
        self.log.info(init_cmd)
        init_msg = self.Primary_User_Node.sh(init_cmd).result()
        self.log.info(init_msg)
        self.assertIn("'test_slave_backup0174' " + self.constant.init_success,
                      init_msg, '执行失败:' + text)
        ...
        backup_cmd = f"source {macro.DB_ENV_PATH};" \
                     ...
                     f"--expire-time='2021-12-30 00:00:00+03'"
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0174 end----')
