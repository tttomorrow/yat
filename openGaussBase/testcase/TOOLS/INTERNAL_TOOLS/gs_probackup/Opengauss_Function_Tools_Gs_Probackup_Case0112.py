...
"""
Case Type   : 系统内部使用工具
Case Name   : 只指定--recovery-target-inclusive=boolean,检查备份文件
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
Expect      :
    ...
    4.进行备份成功
    5.执行validate失败，报错 The '--recovery-target-inclusive' option can be
     applied only when either of '--recovery-target-time',
     '--recovery-target-xid' or '--recovery-target-lsn' options is specified
    6.删除新建目录成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0112开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        self.row_msg = self.show_msg.splitlines()[-9].strip()
        LOG.info(self.row_msg)
        self.xid_msg = self.row_msg.split('=')[1].strip()
        LOG.info(self.xid_msg)

        LOG.info('-----------step6 执行validate--------------')
        validate_cmd = f"source {macro.DB_ENV_PATH};" \
            f"gs_probackup validate -B {self.cluster_path}/testdir " \
            f" --instance=pro1 --recovery-target-inclusive=true " \
            f" --recovery-target-xid='{self.xid_msg}';"
        LOG.info(validate_cmd)
        validate_msg = self.PrimaryNode.sh(validate_cmd).result()
        LOG.info(validate_msg)
        self.assertIn(f'Backup {self.backupid} is valid', validate_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0112执行完成-')
