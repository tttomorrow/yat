...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0135开始执行-')
        ...
        self.except_msg = ['gs_probackup restore -B backup-path '
                           ...
                           '--force                      ignore invalid '
                           'status of the restored backup',
                           '--no-sync                    do not sync '
                           'restored files to disk',
                           '--no-validate                disable backup '
                           'validation during restore',
                           '--skip-block-validation      set to validate '
                           'only file-level checksum',
                           '--external-mapping=OLDDIR=NEWDIR',
                           ...
                           '',
                           'Recovery options:',
                           '--recovery-target-time=time  time stamp up '
                           'to which recovery will proceed',
                           '--recovery-target-xid=xid    transaction ID '
                           'up to which recovery will proceed',
                           '--recovery-target-lsn=lsn    LSN of the '
                           'write-ahead log location up to which recovery '
                           'will proceed',
                           '--recovery-target-name=target-name',
                           ...
                           '--remote-host=destination    remote host '
                           'address or hostname',
                           '--remote-port=port           remote host port '
                           '(default: 22)',
    ...
    def tearDown(self):
        ...
        # 无须清理环境
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0135执行完成--')
