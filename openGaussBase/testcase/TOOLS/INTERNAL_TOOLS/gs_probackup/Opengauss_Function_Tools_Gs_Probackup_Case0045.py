...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定-D、 -C 、--backup-pg-log、-j 、--no-validate、
              --no-sync进行备份
Description :
    1.新建目录
    2.进行初始化
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0045开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        back_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"{self.PrimaryNode.db_port} -j 4  --no-validate  -C " \
            f"--backup-pg-log  --no-sync ;"
        LOG.info(back_cmd)
        back_msg = self.PrimaryNode.sh(back_cmd).result()
        LOG.info(back_msg)
        self.assertIn('Backup files are not synced to disk', back_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0045执行完成-')
