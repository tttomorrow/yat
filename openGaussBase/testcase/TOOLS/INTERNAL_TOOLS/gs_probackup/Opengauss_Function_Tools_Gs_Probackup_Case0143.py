...
Case Name   : 使用gs_probackup add-instance命令同时指定--remote-proto=ssh 、
              ...
              --remote-user=<安装user>、--remote-path<远程安装目录>
Description :
    1.创建备份目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.remote_lib = os.path.join(os.path.dirname(macro.DB_INSTANCE_PATH),
                                       'app', 'lib')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0143 start----')
    ...
    def test_server_tools(self):
        ...
        init_msg = self.Primary_User_Node.sh(init_cmd).result()
        self.log.info(init_msg)
        self.assertIn("'test_0143' " + self.constant.init_success, init_msg,
                      '执行失败:' + text)

        text = 'step4:查看pg_probackup.conf配置文件;expect:数据目录添加成功---'
        self.log.info(text)
        cat_cmd = f"cat {self.gs_probackup_path}/backups/" \
                  f"test_0143/pg_probackup.conf"
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0143 end----')
