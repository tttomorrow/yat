...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定用于压缩数据文件的算法、压缩级别，指定
              --compress-algorithm=pglz、--compress-level=2，执行备份
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
Expect      :
    ...
    4.执行备份成功，有提示信息compress-algorithm: pglz,compress-level: 2
    5.删除新建目录
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0056开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        back_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"{self.PrimaryNode.db_port} --compress-algorithm=pglz " \
            f"--compress-level=2 ;"
        LOG.info(back_cmd)
        back_msg = self.PrimaryNode.sh(back_cmd).result()
        LOG.info(back_msg)
        self.assertIn('compress-algorithm: pglz', back_msg)
        self.assertIn('compress-level: 2', back_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0056执行完成-')
