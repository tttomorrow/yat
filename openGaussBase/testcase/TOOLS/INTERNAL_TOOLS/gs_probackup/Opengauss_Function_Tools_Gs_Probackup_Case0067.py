...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--progress执行restore,恢复信息有详细进度展示
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0067开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        stop_cmd = self.sh_primary.stop_db_cluster()
        LOG.info(stop_cmd)
        ...
        self.assertIn(f"Restore of backup {self.backupid} completed",
                      restore_msg)
        self.assertIn('Progress:', restore_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        start_cmd = self.sh_primary.start_db_cluster()
        LOG.info(start_cmd)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0067执行完成-')
