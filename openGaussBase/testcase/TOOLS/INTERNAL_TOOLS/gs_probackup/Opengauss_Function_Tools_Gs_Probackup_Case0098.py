...
"""
Case Type   : 系统内部使用工具
Case Name   : 按保留策略评估备份，但是不会实际执行删除操作，
              指定--dry-run选项，执行备份
Description :
    1.新建目录
    2.进行初始化
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0098开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info(show_msg)
        self.assertTrue(show_msg.count('pro1') == 4)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0098执行完成-')
