...
"""
Case Type   : 系统内部使用工具
Case Name   : pg_probackup.conf中未配置连接选项，命令中也不指定连接参数，
              连接选项使用默认值，执行备份
Description :
    1.新建目录
    2.进行初始化
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0041开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info(back_msg)
        self.assertIn('connect to server failed', back_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0041执行完成-')
