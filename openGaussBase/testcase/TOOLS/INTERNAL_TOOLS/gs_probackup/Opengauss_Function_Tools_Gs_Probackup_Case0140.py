...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0140开始执行-')
        self.constant = Constant()
        self.dbuserNode = Node('dbuser')
        self.sh_dbuser = CommonSH('PrimaryDbUser')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info(init_cmd)
        init_msg = self.dbuserNode.sh(init_cmd).result()
        LOG.info(init_msg)
        ...
        back_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f" --instance=pro1 -b full  -d {self.dbuserNode.db_name} -p " \
            f"{self.dbuserNode.db_port} ;"
        LOG.info(back_cmd)
        back_msg = self.dbuserNode.sh(back_cmd).result()
        LOG.info(back_msg)
        ...
        stop_cmd = self.sh_dbuser.stop_db_cluster()
        LOG.info(stop_cmd)
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            f"gs_probackup restore -B {self.cluster_path}/testdir " \
            f" --instance=pro1  -i {self.backupid} -D " \
            f"{self.cluster_path}/prodir;"
        LOG.info(restore_cmd)
        restore_msg = self.dbuserNode.sh(restore_cmd).result()
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        start_cmd = self.sh_dbuser.start_db_cluster()
        LOG.info(start_cmd)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0140执行完成-')
