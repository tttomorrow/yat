...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0053开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        back_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"{self.PrimaryNode.db_port} -D /a/c/b/s/d/e ;"
        LOG.info(back_cmd)
        back_msg = self.PrimaryNode.sh(back_cmd).result()
        LOG.info(back_msg)
        self.assertIn('No such file or directory', back_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0053执行完成-')
