...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定-j、--progress参数执行delete
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
    4.进行全量备份
    5.执行delete（使用增量备份的back-id）
    6.查看备份情况
    7.删除新建目录
Expect      :
    1.新建目录成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0085开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        delete_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f" --instance=pro1 -i {self.fullbackupid} -j 4 --progress;"
        LOG.info(delete_cmd)
        delete_msg = self.PrimaryNode.sh(delete_cmd).result()
        LOG.info(delete_msg)
        self.assertIn(f'delete of backup {self.fullbackupid}', delete_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0085执行完成-')
