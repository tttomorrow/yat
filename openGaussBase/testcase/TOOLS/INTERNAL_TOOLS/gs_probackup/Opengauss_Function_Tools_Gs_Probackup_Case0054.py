...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定删除过期备份、要保留的备份副本数、能够完成恢复的
              最早时间点，指定--delete-expired、--retention-redundancy、
              --retention-window选项,执行备份
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0054开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        back_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"{self.PrimaryNode.db_port} --delete-expired " \
            f"--retention-redundancy=10 --retention-window=7 ;"
        LOG.info(back_cmd)
        back_msg = self.PrimaryNode.sh(back_cmd).result()
        LOG.info(back_msg)
        self.assertIn('REDUNDANCY=10', back_msg)
        self.assertIn('WINDOW=7', back_msg)
        self.assertIn('to delete by retention policy', back_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0054执行完成-')
