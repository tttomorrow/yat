...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup backup命令指定--compress-algorithm=pglz和
              --compress-level=-1，合理报错
Description :
    1.新建备份目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0184start--')
    ...
    def test_system_internal_tools(self):
        ...
        init_msg = self.Primary_Node.sh(init_cmd).result()
        LOG.info(init_msg)
        self.assertIn("'test_184' " + self.constant.init_success, init_msg)
        LOG.info('---step4查看pg_probackup.conf配置文件---')
        cat_cmd = f"cat {self.gs_probackup_bak_path}/backups/" \
                  f"test_184/pg_probackup.conf"
        LOG.info(cat_cmd)
        ...
        LOG.info(exec_msg)
        self.assertIn("ERROR: Option --compress-level should be a "
                      "32bit unsigned integer: '-1'", exec_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0184finish-')
