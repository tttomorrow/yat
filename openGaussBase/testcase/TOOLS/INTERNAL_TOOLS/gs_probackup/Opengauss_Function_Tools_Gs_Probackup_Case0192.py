...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup backup命令添加连接参数选项-d（其他数据库）、
              -h（本机ip）、-p、-U（普通用户）、-W选项
Description :
    1.创建备份目录
    2.进行初始化
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(
            ...
            'gs_probackup_testdir0192')
        self.us_name = "us_gs_probackup_0192"
        self.db_name = "db_gs_probackup_0192"
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0192 start----')
    ...
    def test_server_tools(self):
        ...
        add_cmd = f"source {macro.DB_ENV_PATH};" \
                  ...
                  f"--instance=test_0192 "
        self.log.info(add_cmd)
        exec_msg = self.Primary_User_Node.sh(add_cmd).result()
        self.log.info(exec_msg)
        self.assertIn("'test_0192' " + self.constant.init_success, exec_msg,
                      '执行失败:' + text)

        text = '--step6:进行全量备份;expect:备份成功----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0192 end----')
