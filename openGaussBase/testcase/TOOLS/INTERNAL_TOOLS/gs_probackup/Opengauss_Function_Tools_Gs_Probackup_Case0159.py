...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup set-config命令同时指定--remote-host=<主机名>、
...
Description :
    ...
    3.获取远端主机名
    4.添加远程实例
    5.将远程连接添加到pg_probackup.conf配置文件
    6.查看pg_probackup.conf配置文件中是否有远程连接信息
    7.恢复环境
Expect      :
    1.创建备份目录成功
    2.初始化成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0159')
        self.remote_lib = os.path.join(os.path.dirname(macro.DB_INSTANCE_PATH),
                                       'app', 'lib')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0159 start----')
    ...
    def test_server_tools(self):
        ...
        self.assertIn("'test_0159' " + self.constant.init_success, exec_msg,
                      '执行失败:' + text)

        text = '--step5:将远程连接添加到pg_probackup.conf配置文件;' \
               'expect:添加完成----'
        self.log.info(text)
        ...
        text = '---step6:查看pg_probackup.conf配置文件中是否有远程连接信息;' \
               'expect:远程连接信息添加成功---'
        self.log.info(text)
        cat_cmd = f"cat {self.gs_probackup_path}/backups/test_0159/" \
                  f"pg_probackup.conf"
        self.log.info(cat_cmd)
        cat_msg = self.Primary_User_Node.sh(cat_cmd).result()
        self.log.info(cat_msg)
        self.assertIn('remote-proto = ssh' and
                      f'remote-host = {s_hostname}',
                      cat_msg, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0159 end----')
