...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定的--tablespace-mapping=OLDDIR=NEWDIR 选项为相对路径，
              执行restore
Description :
    1.新建目录
    2.创建表空间及表
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0080开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        back_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"={self.cluster_path}/aaa:{self.cluster_path}/bbb;"
        LOG.info(back_cmd)
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-i {self.backupid} --external-mapping={self.cluster_path}" \
            f"/aaa={self.cluster_path}/bbb --tablespace-mapping=aaa=bbb ;"
        LOG.info(restore_cmd)
        restore_msg = self.PrimaryNode.sh(restore_cmd).result()
        LOG.info(restore_msg)
        self.assertIn(
            f'old directory is not an absolute path in tablespace mapping',
            restore_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0080执行完成-')
