...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup set-config命令指定--log-level-console=log、
              --log-level-file=log、
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        self.gs_probackup_path = os.path.join(macro.DB_INSTANCE_PATH,
                                              'gs_probackup_testdir0199')
        self.gs_probackup_log = os.path.join(macro.DB_INSTANCE_PATH,
                                             'gs_probackup_log0199')
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0199 start----')
    ...
    def test_server_tools(self):
        ...
        backup_cmd = f"source {macro.DB_ENV_PATH};" \
                     ...
                     f"-B {self.gs_probackup_path} " \
                     f"--instance=test_0199 " \
                     f"--log-level-console=log " \
                     f"--log-level-file=log " \
        ...
        exec_msg = self.Primary_User_Node.sh(add_cmd).result()
        self.log.info(exec_msg)
        self.assertIn('log-level-console = LOG', exec_msg, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_Tools_Gs_Probackup_Case0199 end----')
