...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定--skip-external-dirs、--skip-block-validation、
              --no-validate执行restore
Description :
    1.新建目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0083开始执行-')
    ...
    def test_system_internal_tools(self):
        ...
        restore_cmd = f"source {macro.DB_ENV_PATH};" \
            ...
            f"-i {self.backupid}  --skip-external-dirs " \
            f"--skip-block-validation --no-validate;"
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0083执行完成-')
