...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用gs_probackup backup命令指定压缩选项--compress
            （等价于 --compress-algorithm=zlib and --compress-level=1）
Description :
    1.新建备份目录
    2.进行初始化
    3.在备份路径内初始化一个新的备份实例
    4.查看是否生成pg_probackup.conf配置文件
    5.创建指定实例的备份，添加--compress
    6.删除新建目录
Expect      :
    1.新建备份目录成功
    2.进行初始化成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        LOG.info('--Opengauss_Function_Tools_Gs_Probackup_Case0185start--')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info(exec_msg)
        self.assertIn('pg_stop backup() successfully executed', exec_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('-Opengauss_Function_Tools_Gs_Probackup_Case0185finish-')
