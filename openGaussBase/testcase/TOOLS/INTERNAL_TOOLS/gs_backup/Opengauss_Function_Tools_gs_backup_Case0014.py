...
"""
Case Type   : 系统内部使用工具
Case Name   : 主机执行gs_backup -l指定不存在文件进行备份时，日志存放文
              件是否会自动产生并将信息写入
Description :
    1.创建备份目录
    2.指定参数-l执行gs_backup
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0014开始执行---')
    ...
    def tearDown(self):
        ...
        if 'Standby' not in query_msg:
            ...
            LOG.info(clear_msg2)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0014执行完成---')
