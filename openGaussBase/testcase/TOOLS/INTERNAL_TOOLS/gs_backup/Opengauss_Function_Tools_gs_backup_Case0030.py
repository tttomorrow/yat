...
"""
Case Type   : 系统内部使用工具
Case Name   : 主机执行gs_backup -l指定不存在文件进行恢复时，日志存放文件是否
             会自动产生并将信息写入
Description :
    1.新建备份目录
    2.执行gs_backup进行备份
    3.使用gs_backup脚本恢复数据库主机
...
Expect      :
    ...
    2.备份成功
    3.恢复成功
    4.主机生成restore_zh.log与gs_local开头的日志
    5.备机生成gs_local开头的日志
    6.删除备份文件成功
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0030start---')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('---步骤3:主机执行gs_backup进行恢复---')
        gs_backup_cmd = f'''source {macro.DB_ENV_PATH};
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0030finish---')
