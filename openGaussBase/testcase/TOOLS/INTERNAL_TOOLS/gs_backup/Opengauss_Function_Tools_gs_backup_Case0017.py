...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用root用户执行gs_backup进行恢复
Description :
    1.使用root用户执行gs_backup进行恢复
Expect      :
    1.使用root用户执行gs_backup进行恢复成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0017开始执行---')
    ...
    def test_system_internal_tools(self):
            ...
            LOG.info(backup_msg)
            self.assertIn(self.constant.root_exec_fail, backup_msg)
    ...
    def tearDown(self):
        ...
        if 'Standby' not in query_msg:
            ...
            LOG.info(clear_msg2)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0017执行完成---')
