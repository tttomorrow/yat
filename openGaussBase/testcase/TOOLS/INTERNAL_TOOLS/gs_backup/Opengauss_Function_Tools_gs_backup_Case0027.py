...
"""
Case Type   : 基础功能
Case Name   : 关闭备数据库，指定参数--all执行gs_backup进行恢复是否会恢复参数文件与二进制文件
Description :
...
class Backupclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_Tools_gs_backup_Case0027 start")
    ...
    def test_backup0026(self):
        ...
        text = '--step6: 查看主机与备机postgresql.conf文件，' \
               '参数值是否恢复,查看二进制文件是否恢复（不做重启） ' \
        ...
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.assertIn("postgresql.conf.sample", result, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_backup_Case0027 end-")
