...
"""
Case Type   : 系统内部使用工具
Case Name   : 关闭数据库集群，执行gs_backup进行恢复
Description :
    1.新建备份目录
    ...
    4.确认该参数是否已更改成功
    5.关闭数据库集群
    6.执行gs_backup对参数文件进行单节点恢复
    7.查看postgresql.conf文件，参数值是否恢复
Expect      :
    1.切换用户成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0024开始执行---')
    ...
    def tearDown(self):
        ...
        if 'Standby' not in query_msg:
            ...
            LOG.info(clear_msg2)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0024执行完成---')
