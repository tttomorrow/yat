...
"""
Case Type   : 基础功能
Case Name   : 关闭备数据库，指定参数--binary执行gs_backup进行恢复是否只恢复二进制文件
Description :
...
class Backupclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_Tools_gs_backup_Case0026 start")
    ...
    def test_backup0026(self):
        ...
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.assertNotIn(self.session_timeout, result, '执行失败:' + text)
        cmd = f"ls " \
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_gs_backup_Case0026 end-")
