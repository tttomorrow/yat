...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定错误的数据库hostname进行gs_backup备份
Description :
    1.创建备份目录
    2.指定错误的hostname执行gs_backup
Expect      :
    1.创建备份目录并去除权限成功
    2.备份失败
History     :
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0007开始执行---')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info(backup_msg)
        self.assertIn('Failed to obtain the node configuration', backup_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0007执行完成---')
