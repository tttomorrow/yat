...
"""
Case Type   : 系统内部使用工具
Case Name   : 主机执行gs_backup -l指定已存在文件进行备份时，日志存放
              文件是否会将信息写入
Description :
    1.创建备份目录
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0015开始执行---')
    ...
    def tearDown(self):
        ...
        if 'Standby' not in query_msg:
            ...
            LOG.info(clear_msg2)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0015执行完成---')
