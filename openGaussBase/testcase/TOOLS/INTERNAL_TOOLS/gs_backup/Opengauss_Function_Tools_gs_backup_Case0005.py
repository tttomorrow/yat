...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定备数据库hostname执行gs_backup进行备份
Description :
    1.创建备份目录
    2.指定备机hostname执行gs_backup
Expect      :
    1.创建备份目录并去除权限成功
    2.指定备机hostname执行gs_backup成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0005开始执行---')
    ...
    def tearDown(self):
        ...
        LOG.info(clear_msg)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0005执行完成---')
