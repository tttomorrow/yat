...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定错误的hostname执行gs_backup恢复备机
Description :
    1.新建备份目录
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0023开始执行---')
        ...
        if 'Standby' not in query_msg:
            ...
            sed_cmd = f'''sed -i \
                ...
                {macro.DB_INSTANCE_PATH}/postgresql.conf
                                    '''
            LOG.info(sed_cmd)
            sed_msg = self.StandbyNode.sh(sed_cmd).result()
            LOG.info(sed_msg)
            LOG.info('------------------校验文件参数值---------------')
            check_cmd = f"cat {macro.DB_INSTANCE_PATH}/postgresql.conf|" \
                f"grep session_timeout;"
            LOG.info(check_cmd)
            check_msg = self.StandbyNode.sh(check_cmd).result()
            LOG.info(check_msg)
            ...
            backup_cmd = f'''source {macro.DB_ENV_PATH};
                gs_backup -t restore \
                --backup-dir={macro.DB_BACKUP_PATH}/tesdir -h 123;
                '''
            LOG.info(backup_cmd)
            backup_msg = self.StandbyNode.sh(backup_cmd).result()
            LOG.info(backup_msg)
            self.assertIn(self.constant.hostname_error, backup_msg)
    ...
    def tearDown(self):
        ...
        if 'Standby' not in query_msg:
            ...
            LOG.info('----------------恢复文件参数值----------------')
            recover_cmd = f"sed -i " \
                f"'s/{self.pn} = {self.nv}/{self.pn} = {self.pv}/' " \
            ...
            LOG.info(clear_msg2)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0023执行完成---')
