...
"""
Case Type   : 系统内部使用工具
Case Name   : 关闭备数据库，指定参数--parameter执行gs_backup进行备份，
                是否只备份参数文件（此参数为默认值）
Description :
    1.创建备份目录
    2.关闭数据库备机
    3.指定参数--parameter执行gs_backup
    4.查看主机与备机对应目录，该目录是否只存放参数文件
    5.开启备机
Expect      :
    1.创建备份目录成功
    2.关闭备机成功
    3.指定参数--parameter执行gs_backup成功
    4.查看主机与备机对应目录成功，该目录只存放参数文件
    5.开启备机成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0009开始执行---')
    ...
    def test_system_internal_tools(self):
        ...
        if 'Standby' not in query_msg:
            ...
            backup_cmd = f'''source {macro.DB_ENV_PATH};
                gs_backup -t backup \
                --backup-dir={macro.DB_BACKUP_PATH}/tesdir --parameter;
                '''
            LOG.info(backup_cmd)
    ...
    def tearDown(self):
        ...
        if 'Standby' not in query_msg:
            ...
            LOG.info(clear_msg2)
        LOG.info('---Opengauss_Function_Tools_gs_backup_Case0009执行完成---')
