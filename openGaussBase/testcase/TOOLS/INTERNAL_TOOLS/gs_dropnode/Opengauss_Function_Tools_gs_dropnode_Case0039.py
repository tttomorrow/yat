...
Case Name   : 3参，1主2备减容2备成功后根据减容提示输入否不进行重启 期望：减容操作成功
Description :
        1.1主2备，执行减容2 提示出现后输入yes 是否重启输入no
        2.重启后查询
Expect      :
        1.期望:操作成功，减容成功
        2.减容成功
...
class Gstoolstestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Tools_gs_dropnode_Case0039 start==")
    ...
    def test_tool(self):
        LOGGER.info("步骤1：1主2备，执行减容 根据减容提示输入yes,重启提示输入no")
    ...
    def tearDown(self):
        ...
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Tools_gs_dropnode_Case0039 finish=")
