...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_dropnode的之后，删除非减容备节点的postgresql.conf文件
Description :
    1.下载解压数据库安装包
    2.配置xml文件
    3.创建用户并修改文件属性
    ...
    6.一主两备在减容备1过程删除备2节点postgresql.conf文件
    7.清理环境
...
class Tools(unittest.TestCase):
    ...
    def test_gs_dropnode(self):
        ...
        text = '----step6:在对备1减容时删除备2中postgresql.conf文件 expect:失败----'
        self.log.info(text)
        ...
        self.log.info('在备1减容同时删除备2文件')
        post_path = os.path.join(f'{self.instance_path}', 'postgresql.conf')
        del_cmd = f'rm -rf {post_path};'
        session_2 = ComThread(self.common.get_sh_result, args=(
            self.standby2_root_node, del_cmd,))
