...
"""
Case Type   : 系统内部使用工具
Case Name   : 1+2备在被减容的备库执行gs_uninstall --delete-data
Description :
    1.下载解压数据库安装包
    2.配置xml文件
    3.创建用户并修改文件属性
...
class Tools(unittest.TestCase):
    ...
    def test_gs_dropnode(self):
        ...
        execute_cmd = f'su - {self.u_name} <<EOF\n' \
                      ...
                      f'EOF'
        self.log.info(execute_cmd)
        uninstall_msg = self.standby2_root_node.sh(execute_cmd).result()
        ...
        check_res = self.primary_root_node.sh(check_cmd).result()
        self.log.info(check_res)
        err_msg = f"[GAUSS-50300] : User installation path of designated " \
                  f"user {self.u_name} does not exist"
        self.assertIn(err_msg, check_res, '数据库卸载失败')

    def tearDown(self):
        text = "----step8:清理用户及文件 expect:成功----"
        self.log.info(text)
