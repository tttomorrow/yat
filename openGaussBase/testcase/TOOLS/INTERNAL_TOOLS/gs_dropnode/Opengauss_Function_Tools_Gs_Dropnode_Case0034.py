...
"""
Case Type   : 系统内部使用工具
Case Name   : 在减容开始之后删除非待减容备机与主机的用户互信
Description :
    1.下载解压数据库安装包
    2.配置xml文件
    3.创建用户并修改文件属性
    ...
    6.一主两备在减容备1过程中删除备2与主机用户互信
    7.清理环境，清理用户下进程，删除数据库相关目录并删除用户
Expect      :
    1.下载解压成功
    2.配置成功
    3.用户创建成功且文件属性修改成功
...
class Tools(unittest.TestCase):
    ...
    def test_gs_dropnode(self):
        ...
        text = '----step6:在对备1减容时删除备2与主机用户互信 expect:减容失败----'
        self.log.info(text)
        ...
        self.log.info('在备1减容同时删除备2与主机用户互信')
        ...
        check_msg = f"[FAILURE] {standby2_node_name}"
