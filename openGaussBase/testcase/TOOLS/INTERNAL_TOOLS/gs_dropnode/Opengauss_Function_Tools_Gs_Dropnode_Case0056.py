...
Description :
    ...
    7.在减容备机上执行gs_uninstall --delete-data卸载数据库
    8../gs_expansion -U user -G group -X {xml} -h 备机ip [-L]
    扩容被减容备机
    9.执行gs_dropnode进行减容：
    ./gs_dropnode -U {usrname} -G {grp_name} -h {hostname}
    10.清理环境
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        wget_result = self.primary_node.sh(f"wget {macro.FTP_PATH} "
                                           f"-c -t2 -T30").result()
        self.host_tuple = (self.pri_host, self.sta1_host, self.sta2_host)
    ...
    def test_gs_dropnode(self):
        ...
        for name in [primary_node_name, standby1_node_name, standby2_node_name]:
            self.assertIsNotNone(name, '执行失败:' + text)
        text = '----查询系统未使用端口----'
        self.log.info(text)
        ...
        text = '----step6:执行gs_dropnode进行备机减容 expect:成功'
        self.log.info(text)
        ...
        self.assertNotIn(self.sta1_host, check_res, '数据库状态检查失败')

        text = "----step7:在减容备机上执行卸载数据库 expect:成功----"
        self.log.info(text)
        ...
        execute_cmd = f'su - {self.u_name} <<EOF\n' \
                      ...
                      f'EOF'
        self.log.info(execute_cmd)
        uninstall_msg = self.standby1_root_node.sh(execute_cmd).result()
        self.log.info(uninstall_msg)
        self.assertIn(Constant.uninstall_success_msg, uninstall_msg, '卸载失败')

        text = '----step8:执行gs_expansion进行备机扩容 expect:成功'
        self.log.info(text)
        ...
        expansion_cmd = f'./gs_expansion -U {self.u_name} -G {self.u_name} ' \
                        f'-X {self.xml_path} -h {self.sta1_host}'
        ...
        check_cmd = f"su - {self.u_name} <<EOF\n" \
                    ...
                    f"EOF"
        check_res = self.primary_root_node.sh(check_cmd).result()
        for check in [self.pri_host, self.sta1_host, self.sta2_host]:
            self.assertIn(check, check_res, '扩容后检查失败')

        text = '----step9:再次执行gs_dropnode进行备机减容 expect:成功'
        self.log.info(text)
        drop_cmd = f'''expect <<EOF
                       set timeout 60
                       spawn su - {self.u_name}
