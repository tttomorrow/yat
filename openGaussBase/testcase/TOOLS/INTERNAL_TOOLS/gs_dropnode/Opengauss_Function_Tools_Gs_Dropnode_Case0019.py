...
class Tools(unittest.TestCase):
    ...
    def test_gs_dropnode(self):
        ...
        text = '----step6:构建数据库Degrade状态 expect:成功----'
        self.log.info(text)
        add_cmd = f"echo -e {self.pri_host} > /etc/hosts;" \
                  f"cat /etc/hosts;"
        self.log.info(add_cmd)
        add_res = self.primary_root_node.sh(add_cmd).result()
        self.log.info(add_res)
        self.assertEqual(add_res, self.pri_host, '主机ip写入失败')
        ...
        execute_cmd = f'su - {self.u_name} <<EOF\n' \
                      ...
                      f'EOF'
        self.log.info(execute_cmd)
        drop_res = self.primary_root_node.sh(execute_cmd).result()
    ...
    def tearDown(self):
        text = "----step8:恢复hosts、卸载数据库、清理用户及文件 expect:成功----"
        self.log.info(text)
        self.log.info('----恢复hosts文件----')
        if self.hosts_bac_flag == 1:
            res = self.primary_root_node.sh(f'rm -rf /etc/hostsbac').result()
            self.log.info(res)
        else:
