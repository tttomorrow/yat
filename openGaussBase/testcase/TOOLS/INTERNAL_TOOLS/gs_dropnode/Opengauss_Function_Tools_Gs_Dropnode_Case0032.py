...
class Tools(unittest.TestCase):
    ...
    def test_gs_dropnode(self):
        ...
        text = '----step6:构建非待扩容备机ping不通故障 expect:成功----'
        self.log.info(text)
        ping_cmd = f'route add -host {self.sta1_host} reject;' \
                   f'ping {self.sta1_host}'
        self.log.info(ping_cmd)
        check_res = self.primary_root_node.sh(ping_cmd).result()
        self.log.info(check_res)
        self.assertIn('connect: No route to host', check_res, '构造ping故障失败')

        text = '----step7:执行gs_dropnode进行备机减容 expect:失败'
        self.log.info(text)
        ...
        drop_cmd = f'su - {self.u_name} <<EOF\n' \
                   f'source {self.env_path};' \
                   f'{shell_path};\n' \
                   f'EOF'
        self.log.info(drop_cmd)
        drop_res = self.primary_root_node.sh(drop_cmd).result()
        self.log.info(drop_res)
        check_msg = f"[GAUSS-35807] The host {standby1_node_name} which " \
                    f"still exist in the cluster can't be connected"
        self.assertIn(check_msg, drop_res, text + '减容错误')
    ...
    def tearDown(self):
        ...
        self.log.info(text)
        self.log.info('----恢复ping故障----')
        ping_cmd = f'route del -host {self.sta1_host} reject;' \
                   f'ping -c 6 {self.sta1_host}'
        self.log.info(ping_cmd)
        ping_res = self.primary_root_node.sh(ping_cmd).result()
        self.log.info(ping_res)
        ping_assert = re.search('[1-6] received', ping_res)
        ...
        self.assertIsNotNone(ping_assert, 'ping故障恢复失败')
