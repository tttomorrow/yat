...
class Tools(unittest.TestCase):
    ...
    def test_gs_dropnode(self):
        ...
        text = '----step6:在对备1减容时停止备2 expect:失败----'
        self.log.info(text)
        ...
        session_1 = ComThread(self.common.get_sh_result, args=(
            self.standby1_root_node, drop_cmd,))
        session_1.setDaemon(True)
        self.log.info('在备1减容同时停止备2')
        stop_cmd = f'su - {self.u_name} <<EOF\n' \
                   f'source {self.env_path};' \
                   f'gs_om -t stop -h {self.sta2_host};\n' \
                   f'EOF'
        session_2 = ComThread(self.common.get_sh_result, args=(
            self.standby1_root_node, stop_cmd,))
        ...
        self.log.info('备2停止结果')
        session_2.join(120)
        session_2_result = session_2.get_result()
        self.log.info(session_2_result)
        self.log.info('备1减容及备2停止结果')
        check_msg = f'[GAUSS-35803] The IP of local host {self.sta1_host} ' \
                    f'is in the target node list'
        self.assertIn(check_msg, session_1_result, '备1减容同时停止备2，减容错误')
        self.assertIn(Constant.STOP_NODE_SUC_MSG, session_2_result, '备2停止失败')
