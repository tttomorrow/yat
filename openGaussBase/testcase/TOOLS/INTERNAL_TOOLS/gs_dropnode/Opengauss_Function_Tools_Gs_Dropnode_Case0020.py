...
Description :
    ...
    6.集群正在switchover时进行备机减容
    7.集群强制进行failover时进行备机减容
    8.清理环境
...
class Tools(unittest.TestCase):
    ...
    def test_gs_dropnode(self):
        ...
        text = '----step2:配置一主两备xml文件 expect:配置成功----'
        self.log.info(text)
        ...
        text = '----step6:集群正在switchover时进行备机减容 expect:失败----'
        self.log.info(text)
        ...
        session_1 = ComThread(self.common.get_sh_result, args=(
            self.standby1_root_node, switch_cmd,))
        ...
        session_2 = ComThread(self.common.get_sh_result, args=(
            self.primary_root_node, execute_cmd,))
        ...
        self.assertIn(check_msg, session_2_result, '备2减容错误')
        self.log.info('刷新配置文件并重启数据库')
        refresh_cmd = f'su - {self.u_name} <<EOF\n' \
                      f'source {self.env_path};' \
        ...
        refresh_res = self.standby1_root_node.sh(refresh_cmd).result()
        self.log.info(refresh_res)
        self.assertIn('Normal', refresh_res, '刷新配置文件重启失败')

        text = '----step7:集群强制failover时执行备机减容 expect:失败'
        self.log.info(text)
        self.log.info('执行failover操作')
        failover_cmd = f'su - {self.u_name} <<EOF\n' \
                       f'source {self.env_path};' \
                       f'gs_ctl failover -D {self.instance_path};\n' \
                       f'EOF'
        session_3 = ComThread(self.common.get_sh_result, args=(
            self.primary_root_node, failover_cmd,))
        ...
        session_4 = ComThread(self.common.get_sh_result, args=(
            self.standby1_root_node, execute_cmd,))
        ...
        self.assertIn(Constant.FAILOVER_SUCCESS_MSG, session_3_result,
                      'switchover失败')
        check_msg = f'[GAUSS-35806] Current status of cluster is Unavailable'
        self.assertIn(check_msg, session_4_result, '备2减容错误')
        self.log.info('刷新配置文件并重启数据库')
        refresh_cmd = f'su - {self.u_name} <<EOF\n' \
                      f'source {self.env_path};' \
                      f'gs_om -t refreshconf;' \
    ...
    def tearDown(self):
        ...
        self.assertIn(Constant.uninstall_success_msg, uninstall_msg,
                      '执行失败:' + text_1)
        self.assertEqual('not exists', pri_del_dir_msg, '目录删除失败:' + text_3)
        self.assertEqual('not exists', sta1_del_dir_msg, '目录删除失败:' + text_3)
        self.assertEqual('not exists', sta2_del_dir_msg, '目录删除失败:' + text_3)
