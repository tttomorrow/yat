...
Description :
    ...
    6.一主两备在减容备1开始后立即使用gs_om进行重启数据库
    7.清理环境
...
class Tools(unittest.TestCase):
    ...
    def test_gs_dropnode(self):
        ...
        text = '----step6:在对备1减容开始后立即重启数据库 expect:减容失败----'
        self.log.info(text)
        ...
        check_msg = f"It is required that all the nodes should be normal " \
                    f"except the target ones"
        self.assertIn(check_msg, session_1_result, text + '减容失败')
