...
"""
Case Type   : 系统内部使用工具
Case Name   : 待扩容节点已安装数据库情况下，建立主备关系成功
Description :
    1.下载解压数据库安装包
    2.配置主节点单机及带扩容节点单机及扩容主备xml文件
    3.创建用户并修改文件属性
    4.gs_preinstall预安装：
    ...
    6.建立扩容备机root及用户互信
    7.gs_expansion备机扩容建立主备关系：
    ./gs_expansion -U {usrname} -G {grp_name} -X {xml_path} -h {hostname} -L
    8.清理环境
...
class Tools(unittest.TestCase):
    ...
    def test_gs_install(self):
        ...
        self.common.check_load_msg(self.standby_root_node, self.standby_node,
                                   self.package_path, self.conf_path)
        text = '----查看主机名----'
        self.log.info(text)
        ...
        text = '----step2:配置主节点单机及带扩容节点单机及扩容主备xml文件 expect:配置成功----'
        self.log.info(text)
        ...
        self.common.scp_file(self.primary_root_node, 'single.xml',
                             self.conf_path)
        self.common.scp_file(self.standby_root_node, 'single.xml',
                             self.conf_path)
        self.common.scp_file(self.primary_root_node, 'primary_standby.xml',
                             self.conf_path)

        self.log.info('----配置主节点单机xml----')
        xml_pri_single_cmd = \
        ...
        self.log.info(xml_pri_single_cmd)
        single_xml_result = self.primary_root_node.sh(
            xml_pri_single_cmd).result()
        ...
        for content in msg_list:
            self.assertTrue(single_xml_result.find(content) > -1,
                            '主节点配置失败:' + text)
        self.log.info('----配置待扩容节点单机xml----')
        xml_sta_single_cmd = \
            f"sed -i 's#v_nodeNames#{standby_node_name}#g' " \
            f"{self.xml_single_path};" \
            f"sed -i 's#v_gaussdbAppPath#{gaussdb_app_path}#g' " \
            f"{self.xml_single_path};" \
            f"sed -i 's#v_gaussdbLogPath#{gaussdb_log_path}#g' " \
            ...
            f"sed -i 's#v_corePath#{macro.DB_CORE_PATH}#g' " \
            f"{self.xml_single_path};" \
            f"sed -i 's#v_db_host#{self.sta_host}#g' " \
            f"{self.xml_single_path};" \
            f"sed -i 's#v_dataPortBase#{port}#g' " \
            f"{self.xml_single_path};" \
            f"sed -i 's#v_dataNode1#{self.instance_path}#g' " \
            f"{self.xml_single_path};" \
            f"cat {self.xml_single_path}"
        self.log.info(xml_sta_single_cmd)
        single_xml_result = self.standby_root_node.sh(
            xml_sta_single_cmd).result()
        self.log.info(single_xml_result)
        msg_list = [standby_node_name, self.data_path, f"{port}",
                    self.sta_host]
        for content in msg_list:
            self.assertTrue(single_xml_result.find(content) > -1,
                            '待扩容节点配置失败:' + text)
        self.log.info('----配置扩容主备xml----')
        xml_expansion_cmd = \
            f"sed -i " \
        ...
        pri_file_msg = self.primary_root_node.sh(file_own).result()
        sta_file_msg = self.standby_root_node.sh(file_own).result()
        self.log.info(pri_file_msg)
        self.log.info(sta_file_msg)
        self.assertEqual('', pri_file_msg, '主节点执行失败:' + text)
        self.assertEqual('', sta_file_msg, '备节点执行失败:' + text)
        ...
        pri_msg = self.primary_root_node.sh(preinstall_cmd).result()
        sta_msg = self.standby_root_node.sh(preinstall_cmd).result()
        self.log.info(pri_msg)
        self.log.info(sta_msg)
        self.assertIn(Constant.preinstall_success_msg, pri_msg,
                      '主节点执行失败:' + text)
        self.assertIn(Constant.preinstall_success_msg, sta_msg,
                      '备节点执行失败:' + text)
        ...
        pri_exec_msg = self.primary_root_node.sh(execute_cmd).result()
        self.log.info(pri_exec_msg)
        self.assertIn(Constant.install_success_msg, pri_exec_msg,
                      text + '主节点执行失败')
        sta_exec_msg = self.standby_root_node.sh(execute_cmd).result()
        self.log.info(sta_exec_msg)
        self.assertIn(Constant.install_success_msg, sta_exec_msg,
                      text + '备节点执行失败')
        ...
        self.log.info(text)
        expansion_cmd = f'cd {self.script_path};' \
                        f'source {self.env_path};' \
                        f'./gs_expansion -U {self.u_name} -G {self.u_name} ' \
                        f'-X {self.xml_expansion_path} -h {self.sta_host} -L'
    ...
    def tearDown(self):
        text = "----step8:卸载数据库，清理用户及文件 expect:成功----"
        self.log.info(text)
