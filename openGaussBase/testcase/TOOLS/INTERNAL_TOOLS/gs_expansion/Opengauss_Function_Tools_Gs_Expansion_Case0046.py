...
Case Name   : 内核增加了连接参数Replconninfo由不空变为空的告警-4
Description :
    1.使用方式1设置其中一个连接参数由不空变为空的情况，观察打印报警信息，然后恢复连接参数
    2.使用方式2设置其中一个连接参数由不空变为空的情况，观察打印报警信息，然后恢复连接参数
    3.使用方式3设置其中一个连接参数由不空变为空的情况，观察打印报警信息，然后恢复连接参数
    4.使用方式4设置其中一个连接参数由不空变为空的情况，观察打印报警信息
Expect      :
    1.方式1不打印报警信息
    2.方式2不打印报警信息
    3.方式3设置不生效
...
class GsExpansion46(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.cons = Constant()
        self.com = Common()
    ...
    def test_1(self):
        text = '----step1: 使用方式1设置其中一个连接参数由不空变为空的情况，观察打印报警信息，然后恢复连接参数' \
               ' expect: 方式1不打印报警信息----'
        self.log.info(text)
    ...
    def test_2(self):
        text = '----step2: 使用方式2设置其中一个连接参数由不空变为空的情况，观察打印报警信息，然后恢复连接参数' \
               ' expect: 方式2不打印报警信息----'
        self.log.info(text)
        sql = f"show replconninfo{Node_Num - 1}; "
        res = Pri_U_SH.execut_db_sql(sql)
    ...
    def test_3(self):
        text = '----step3: 使用方式3设置其中一个连接参数由不空变为空的情况，观察打印报警信息，然后恢复连接参数' \
               ' expect: 方式3设置不生效----'
        self.log.info(text)
        sql = f"show replconninfo{Node_Num - 1}; "
        res = Pri_U_SH.execut_db_sql(sql)
        ...
        expect = f'parameter "replconninfo{Node_Num - 1}" cannot ' \
            f'be changed now'
        sql = f"set replconninfo{Node_Num - 1} to ''; " \
    ...
    def test_4(self):
        text = '----step4: 使用方式4设置其中一个连接参数由不空变为空的情况，观察打印报警信息' \
               ' expect: 方式4不打印报警信息----'
        self.log.info(text)
        sql = f"show replconninfo{Node_Num - 1}; "
        res = Pri_U_SH.execut_db_sql(sql)
