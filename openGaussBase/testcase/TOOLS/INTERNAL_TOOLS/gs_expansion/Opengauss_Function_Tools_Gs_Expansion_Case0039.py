...
Case Name   : 内核能根据连接参数自动切换状态-集群保持不变-删除
Description :
    1.删除数据库连接参数，但是保证删除后的连接参数大于一条
...
class GsExpansion39(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.cons = Constant()
        self.com = Common()
    ...
    def test_1(self):
        text = '----step1: 删除数据库连接参数，但是保证删除后的连接参数大于一条 expect: 成功----'
        self.log.info(text)
        cmd = f"sed -i '/replconninfo1/d' {self.conf_path}"
        self.log.info(cmd)
        res = Pri_U_SH.node.sh(cmd).result()
        self.log.info(res)
        self.assertEqual(len(res), 0, f'执行失败: {text}')
        ...
        text = '----step2: 使用gs_ctl命令查看主节点状态 expect: 主节点状态为Primary----'
        self.log.info(text)
        res = Pri_U_SH.get_db_cluster_status('all')
        self.log.info(res)
