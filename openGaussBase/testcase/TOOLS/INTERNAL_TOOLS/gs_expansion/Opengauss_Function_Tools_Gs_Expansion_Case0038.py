...
Case Name   : 内核能根据连接参数自动切换状态-单机N切P
Description :
    1.新增数据库连接参数
...
class GsExpansion38(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.cons = Constant()
        self.com = Common()
        ...
        text = '----删除主节点所有连接参数,以满足前置条件:不包含连接参数 expect: 成功----'
        self.log.info(text)
        ...
        text = '----检查节点状态 expect: 节点状态为Normal----'
        self.log.info(text)
        res = Pri_U_SH.get_db_cluster_status('all')
        self.log.info(res)
    ...
    def test_1(self):
        text = '----step1: 新增数据库连接参数 expect: 成功----'
        self.log.info(text)
        ...
        self.log.info(res3)
        self.assertEqual(len(res3), 0, f'执行失败: {text}')
        ...
        text = '----step2: 使用gs_ctl命令查看主节点状态 expect: 主节点状态切换为Primary----'
        self.log.info(text)
        res = Pri_U_SH.get_db_cluster_status('all')
        self.log.info(res)
        start = res.index('instance_role')
    ...
    def tearDown(self):
        ...
        self.log.info(text3)
        cmd = f'if [ -f {self.conf_path}_bak ]; ' \
            f'then \\cp -r {self.conf_path}_bak {self.conf_path} && ' \
            f'rm -rf {self.conf_path}_bak; else echo "not exists"; fi'
        self.log.info(cmd)
        res3 = Pri_U_SH.node.sh(cmd).result()
        self.log.info(res3)

        text4 = '----step4: 查看集群状态 expect: 状态正常----'
        self.log.info(text4)
