...
class GsExpansion51(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step2: 执行扩容流程 expect: 成功----'
        self.log.info(text)
        text = '----step2.1: 破坏备2节点用户互信，使部分节点用户互信不正常 expect: 成功----'
        self.log.info(text)
        ssh_path = os.path.join("/", "home", Pri_U_SH.node.ssh_user, ".ssh",
                                "*")
        cmd = f'rm -rf {ssh_path}'
        self.log.info(cmd)
        res = self.com.get_sh_result(self.sh['sta2_user'].node, cmd)
        self.assertEqual(len(res), 0, f'执行失败: {text}')

        text = '----step2.2: 执行扩容流程 expect: 扩容失败，提示个人互信不正常----'
        self.log.info(text)
        ...
        res = self.com.get_sh_result(self.sh.get("pri_root").node,
                                     self.expansion_cmd)
        expect = 'Failed to verify SSH trust on these nodes:.*' \
                 'by individual user'
    ...
    def tearDown(self):
        ...
        text1 = '----后置操作: 恢复互信 expect: 成功----'
        self.log.info(text1)
        script_path = os.path.join(os.path.dirname(macro.DB_INSTANCE_PATH),
                                   'tool', 'script')
        ssh_res1 = Pri_U_SH.exec_gs_sshexkey(
            script_path, *self.hosts, **self.params)

        ssh_res2 = self.sh['pri_root'].exec_gs_sshexkey(macro.DB_SCRIPT_PATH,
                                                        *self.hosts,
                                                        **self.params)

        text2 = '----后置操作: 执行扩容流程 expect: 成功----'
        self.log.info(text2)
        self.log.info(self.expansion_cmd)
        res = self.com.get_sh_result(self.sh.get("pri_root").node,
                                     self.expansion_cmd)
        ...
        self.assertIn('Successfully created SSH trust', ssh_res1,
                      f'执行失败: {text1}')
        self.assertIn('Successfully created SSH trust', ssh_res2,
                      f'执行失败: {text1}')
        self.assertIsNotNone(regex_res1, f'执行失败: {text2}')
