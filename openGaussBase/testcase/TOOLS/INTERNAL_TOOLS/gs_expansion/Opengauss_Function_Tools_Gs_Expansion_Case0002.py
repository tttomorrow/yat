...
"""
Case Type   : 系统内部使用工具
Case Name   : 安装包方式安装一主N备数据库，执行备机扩容，扩容成功
Description :
    1.下载解压数据库安装包
    2.配置单节点及备机扩容xml文件
    3.创建用户并修改文件属性
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.sta1_host = self.standby1_node.db_host
        self.sta2_host = self.standby2_node.db_host
    ...
    def test_gs_install(self):
        ...
        xml_cmd = \
            f"sed -i " \
            f"'s#v_nodeNames#{primary_node_name}, {standby1_node_name}#g' " \
            f"{self.xml_path};" \
            ...
            f"{self.xml_path};" \
            f"sed -i 's#v_db_host#{self.pri_host}, {self.sta1_host}#g' " \
            f"{self.xml_path};" \
            ...
            f"{self.xml_path};" \
            f"sed -i 's#v_sta_nodeNames#{standby1_node_name}#g' " \
            f"{self.xml_path};" \
            f"sed -i 's#v_sta_db_host#{self.sta1_host}#g' " \
            f"{self.xml_path};" \
            f"cat {self.xml_path}"
        ...
        msg_list = [primary_node_name, standby1_node_name, self.data_path,
                    f"{port}", self.pri_host, self.sta1_host]
        ...
        xml_expansion_cmd = \
            ...
            f"{self.instance_path}#g' {self.xml_expansion_path};" \
        ...
        self.common.create_user(self.standby1_root_node, self.u_name)
        ...
        self.assertIn(self.pri_host and self.sta1_host and self.sta2_host,
        ...
        expansion_cmd = f'./gs_expansion -U {self.u_name} -G {self.u_name} ' \
                        f'-X {self.xml_expansion_path} -h {self.sta2_host}'
    ...
    def tearDown(self):
        ...
        self.assertEqual('not exists', sta1_del_dir_msg, '执行失败:' + text_3)
        self.assertEqual('not exists', sta2_del_dir_msg, '执行失败:' + text_3)
