...
Description :
    ...
    6.建立扩容备机root及用户互信
    7.gs_expansion备机扩容指定不存在用户及用户组：
    ./gs_expansion -U {usrname} -G {grp_name} -X {xml_path} -h {hostname}
    8.gs_expansion备机扩容 -h 待安装节点信息错误：
    ./gs_expansion -U {usrname} -G {grp_name} -X {xml_path} -h {hostname}
    9.gs_expansion备机扩容，xml文件不存在：
    ./gs_expansion -U {usrname} -G {grp_name} -X {xml_path} -h {hostname}
    10.清理环境
...
class Tools(unittest.TestCase):
    ...
    def test_gs_install(self):
        ...
        text = '----step7:gs_expansion备机扩容指定不存在用户及用户组 expect:失败'
        self.log.info(text)
        expansion_cmd = f'cd {self.script_path};' \
                        f'source {self.env_path};' \
                        f'./gs_expansion -U no_exists_usr -G no_exists_grp ' \
        ...
        error_msg = f'User no_exists_usr does not exist'
        self.assertIn(error_msg, expan_result, text + '执行失败')

        text = '----step8:执行gs_expansion进行备机扩容，-h节点信息错误 expect:失败'
        self.log.info(text)
        err_host = '123' + '.' + '456' + '.' + '789' + '123'
        expansion_cmd = f'cd {self.script_path};' \
                        f'source {self.env_path};' \
                        f'./gs_expansion -U {self.u_name} -G {self.u_name} ' \
                        f'-X {self.xml_expansion_path} -h {err_host}'
        self.log.info(expansion_cmd)
        expan_result = self.primary_root_node.sh(expansion_cmd).result()
        self.log.info(expan_result)
        error_msg = f'standby host backip {err_host} is not in the ' \
                    f'XML configuration file'
        self.assertIn(error_msg, expan_result, text + '执行事变')

        text = '----step9:执行gs_expansion进行备机扩容，xml文件不存在 expect:失败'
        self.log.info(text)
        expansion_cmd = f'rm -rf {self.xml_expansion_path};' \
        ...
        expan_result = self.primary_root_node.sh(expansion_cmd).result()
        self.log.info(expan_result)
        error_msg = f'The {self.xml_expansion_path} does not exist or the ' \
                    f'permission on the upper-layer directory is insufficient'
        self.assertIn(error_msg, expan_result, text + '执行失败')

    def tearDown(self):
        text = "----step8:卸载数据库，清理用户及文件互信 expect:成功----"
        self.log.info(text)
