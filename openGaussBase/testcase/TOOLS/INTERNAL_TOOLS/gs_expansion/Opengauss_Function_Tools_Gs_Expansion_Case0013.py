...
class Tools(unittest.TestCase):
    ...
    def test_gs_install(self):
        ...
        check_lis = [self.pri_host, self.sta2_host]
        for check in check_lis:
            self.assertIn(check, check_res, '扩容后检查失败')

        text = '----step8:再次执行gs_expansion进行备机扩容 expect:失败'
        self.log.info(text)
        ...
        self.log.info(expan_result)
        error_msg = f'These nodes [{self.sta2_host}] are already ' \
                    f'in the cluster. Skip expand these nodes.'
