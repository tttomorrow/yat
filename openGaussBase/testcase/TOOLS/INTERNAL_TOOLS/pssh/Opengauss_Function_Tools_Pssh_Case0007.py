...
"""
Case Type   : 系统内部使用工具
Case Name   : pssh -h指定正确主机ip列表文件与正确命令
Description :
    1.创建主机ip列表文件
    2.执行pssh -h指定正确主机ip列表文件与正确命令
    3.清理环境
Expect      :
    1.创建成功
    2.执行成功，返回相应结果
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.pri_host = self.primary_root_node.db_host
        self.sta1_host = self.s_root_node1.db_host
    ...
    def test_standby(self):
        text = '-----step1:创建主机ip列表文件 expect:成功-----'
        test = '-----查看主机名-----'
        self.log.info(test)
        ...
        s1_node_name = self.s_root_node1.sh("uname -n").result()
        self.log.info(s1_node_name)
        self.assertIsNotNone(s1_node_name, '执行失败:' + text)
        ...
        self.assertIn(self.pri_host and self.sta1_host, add_res, '主机列表文件创建失败')

        text = '-----step2:执行pssh -h正确主机ip列表与正确的命令 expect:成功-----'
        self.log.info(text)
        ...
        for name in [pri_node_name, s1_node_name]:
            self.assertIn(name, pssh_res, '执行失败' + text)
