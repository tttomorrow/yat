...
"""
Case Type   : 系统内部使用工具
Case Name   : pssh指定-p参数为小数或非数字等
Description :
    1.pssh指定-p参数为小数或非数字等
    2.清理环境
Expect      :
    1.执行失败，合理报错
    2.无需清理环境
...
class Tools(unittest.TestCase):
    ...
    def test_standby(self):
        text = '-----step1:pssh指定-p参数为小数或非数字等 expect:失败-----'
        self.log.info(text)
        ...
        par_lis = [0.5, 'asdf', '@']
        for par in par_lis:
            pssh_cmd = f"pssh -s -t {par} -H {node_name} hostname"
            ...
            self.assertIn(f"pssh: error: option -t: invalid integer "
                          f"value: '{par}'", pssh_res, '执行失败' + text)
