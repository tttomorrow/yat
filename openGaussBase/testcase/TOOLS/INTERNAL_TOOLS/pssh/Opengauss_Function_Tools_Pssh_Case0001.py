...
"""
Case Type   : 系统内部使用工具
Case Name   : 查看pssh工具帮助信息
Description :
    1.执行命令查询pssh帮助信息
Expect      :
    1.显示pssh帮助信息成功，共12个选项
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Tools_Pssh_Case0001 开始执行-')
    ...
    def test_pssh(self):
        text = '--step1:查询pssh帮助信息;expect:显示pssh帮助信息，' \
               '共12个选项--'
        self.log.info(text)
        ...
        parameter = ['--help', '-H', '-h', '-t', '-p', '-o', '-e', '-P', '-s',
                     '-x', '-i', '-O']
        self.assertEqual(parameter.sort(), options_list.sort(),
                         '执行失败:' + text)

    def tearDown(self):
        self.log.info('-Opengauss_Function_Tools_Pssh_Case0001 执行完成-')
