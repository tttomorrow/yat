...
"""
Case Type   : 系统内部使用工具
Case Name   : pssh -H输入错误主机名与正确的命令
Description :
    1.执行pssh -H输入错误主机名与正确的命令
    2.清理环境
Expect      :
    1.执行失败，报错code 255
    2.无需清理环境
...
class Tools(unittest.TestCase):
    ...
    def test_standby(self):
        text = '-----step1:执行pssh -H输入错误主机名与正确的命令 expect:失败-----'
        self.log.info(text)
        ...
        pssh_cmd = f"pssh -H asdfsasf hostname"
        pssh_res = self.primary_root_node.sh(pssh_cmd).result()
        self.log.info(pssh_res)
        self.assertIn('error code 255', pssh_res, '执行失败' + text)
