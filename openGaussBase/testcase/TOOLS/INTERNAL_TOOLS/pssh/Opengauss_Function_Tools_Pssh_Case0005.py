...
"""
Case Type   : 系统内部使用工具
Case Name   : pssh -H输入正确主机名与错误的命令
Description :
    1.执行pssh -H输入正确主机名与错误的命令
    2.清理环境
Expect      :
    1.执行失败，合理报错
    2.无需清理环境
...
class Tools(unittest.TestCase):
    ...
    def test_standby(self):
        text = '-----step1:执行pssh -H输入错误主机名与错误的命令 expect:失败-----'
        self.log.info(text)
