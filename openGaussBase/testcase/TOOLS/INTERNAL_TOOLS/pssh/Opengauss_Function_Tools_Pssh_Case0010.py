...
"""
Case Type   : 系统内部使用工具
Case Name   : pssh -h指定权限为000主机名列表文件与正确命令
Description :
    1.创建主机名文件，更改权限为000
    2.pssh -h指定权限为000主机名列表文件与正确命令
    3.清理环境
Expect      :
    1.创建成功
    2.执行成功，返回相应结果
...
class Tools(unittest.TestCase):
    ...
    def test_standby(self):
        text = '-----step1:创建主机名列表文件 expect:成功-----'
        test = '-----查看主机名-----'
        self.log.info(test)
        ...
        add_cmd = f'rm -rf {self.ssh_file};touch {self.ssh_file};' \
                  ...
                  f"echo -e '{pri_node_name}\n{s1_node_name}' " \
                  f"> {self.ssh_file}; chmod 000 {self.ssh_file};" \
                  f"cat {self.ssh_file}"
        ...
        for name in [pri_node_name, s1_node_name]:
            self.assertIn(name, add_res, '主机列表文件创建失败')

        text = '-----step2:执行pssh -h指定权限为000主机列表与正确的命令 expect:成功-----'
        self.log.info(text)
