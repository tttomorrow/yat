...
"""
Case Type   : 系统内部使用工具
Case Name   : pssh指定-o并指定不存在的输出结果目录
Description :
    1.pssh指定-o并指不存在的输出结果目录
    2.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        ...
        self.out_path = os.path.join('/', 'home', 'output_path')
        self.com_root = CommonSH("PrimaryRoot")
        self.common = Common()

    def test_standby(self):
        text = '-----step1:pssh指定-o并指定不存在的输出结果目录 expect:成功-----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(text)
        del_res = self.primary_root_node.sh(f'rm -rf {self.out_path}').result()
