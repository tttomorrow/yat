...
"""
Case Type   : 系统内部使用工具
Case Name   : trace未开启，解析dump codepath信息
Description :
    1.查看gaussdb进程号
    2.启动trace
...
Expect      :
    ...
    4.停止trace成功
    5.解析dump codepath信息成功，对应目录下生成codepath文件
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_Gs_Trace0010开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info(stop_msg)
        self.assertIn(self.constant.trace_stop_success, stop_msg)

        LOG.info('--------------解析codepath信息-------------')
        codepath_cmd = f'''source {macro.DB_ENV_PATH};
            gstrace codepath -f {macro.DB_BACKUP_PATH}/gs_trace.dump -o \
            {macro.DB_BACKUP_PATH}/gs_trace.codepath;
            '''
        LOG.info(codepath_cmd)
        codepath_msg = self.PrimaryNode.sh(codepath_cmd).result()
        LOG.info(codepath_msg)
        self.assertIn('GAUSS-TRACE', codepath_msg)
    ...
    def tearDown(self):
        ...
        stop_cmd = f'''source {macro.DB_ENV_PATH};
            ...
            rm -rf {macro.DB_BACKUP_PATH}/gs_trace.codepath;
            '''
        LOG.info(stop_cmd)
        stop_msg = self.PrimaryNode.sh(stop_cmd).result()
        LOG.info(stop_msg)
        LOG.info('---Opengauss_Function_Tools_Gs_Trace0010执行完成---')
