...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_Gs_Trace0014开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('-------------查看分片文件是否存在-------------')
        step_cmd = f"ls |grep perf.step"
        LOG.info(step_cmd)
        step_msg = self.PrimaryNode.sh(step_cmd).result()
        LOG.info(step_msg)
        self.assertIn('perf.step', step_msg)
    ...
    def tearDown(self):
        ...
        stop_cmd = f'''source {macro.DB_ENV_PATH};
            ...
            rm -rf {macro.DB_BACKUP_PATH}/gs_trace.perf.step;
            '''
        LOG.info(stop_cmd)
        stop_msg = self.PrimaryNode.sh(stop_cmd).result()
        LOG.info(stop_msg)
        LOG.info('---Opengauss_Function_Tools_Gs_Trace0014执行完成---')
