...
"""
Case Type   : 系统内部使用工具
Case Name   : 使用不存在的port启动trace
Description :
    1.查看gaussdb端口号
    2.指定不存在的port启动trace
Expect      :
    1.查看gaussdb端口号成功
    2.启动trace失败，报错Attach to trace config failed
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_Gs_Trace0016开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        start_cmd = f'''source {macro.DB_ENV_PATH};
            source /etc/profile
            gstrace start -p {self.new_port};
            '''
        LOG.info(start_cmd)
        start_msg = self.PrimaryNode.sh(start_cmd).result()
        self.assertIn('Attach to trace config failed', start_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(stop_msg)
        LOG.info('---Opengauss_Function_Tools_Gs_Trace0016执行完成---')
