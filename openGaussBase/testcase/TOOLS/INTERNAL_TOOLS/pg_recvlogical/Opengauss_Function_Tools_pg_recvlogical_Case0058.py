...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 同步间隔时间参数不指定-F
              --fsync-interval=INTERVAL，默认值10
Description :
        1.修改参数值
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.主机创建逻辑复制槽
        4.连接数据库，创建列存表并查看当前lsn位置并插入数据
        5.启动逻辑复制槽的流复制, 不指定参数-F/--fsync-interval
        6.删除复制槽
        7.清理环境
Expect      :
        1.修改成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0058 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0058"
        self.slot_name = "slot_pg_recvlogical_0058"
        self.t_name = "t_pg_recvlogical_0058"
    ...
    def test_standby(self):
        ...
        text = '--step5:逻辑复制槽流式解码, 不指定参数-F/--fsync-interval;' \
               'expect:启动流复制成功，每次心跳缓存刷盘一次--'
        self.log.info(text)
        ...
        self.assertEqual(write_to_xlog[-2][:-1], write_to_xlog[-1],
                         '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0058 finish--')
