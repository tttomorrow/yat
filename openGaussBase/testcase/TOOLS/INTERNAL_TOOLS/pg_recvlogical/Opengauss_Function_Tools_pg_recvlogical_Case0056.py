...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 同步间隔时间参数指定-F
Description :
        1.修改参数值
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.主机创建逻辑复制槽
        4.连接数据库，创建列存表并查看当前lsn位置,并插入数据
        5.启动逻辑复制槽的流复制, 指定参数--F
        6.删除复制槽
        7.清理环境
Expect      :
        1.修改成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0056 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0056"
        self.slot_name = "slot_pg_recvlogical_0056"
        self.t_name = "t_pg_recvlogical_0056"
    ...
    def test_standby(self):
        ...
        self.log.info(text)
        check_res = self.pri_sh.execut_db_sql(f"select slot_name from "
                                              f"pg_replication_slots where "
        ...
        self.assertTrue(exec_msg.count(exc_msg) == 5 or exec_msg.count(
            exc_msg) == 4 or exec_msg.count(exc_msg2) == 5,
                        '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0056 finish--')
