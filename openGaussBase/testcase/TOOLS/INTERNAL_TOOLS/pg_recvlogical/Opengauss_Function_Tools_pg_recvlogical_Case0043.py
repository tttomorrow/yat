...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 指定初始复制的LSN位置--startpos=PTR
Description :
        ...
        8.启动逻辑复制槽的流复制，指定初始复制的LSN位置 --startpos=PTR
        9.删除复制槽
        10.清理环境
Expect      :
        ...
        6.获取lsn位置成功
        7.插入数据成功
        8.启动流复制成功，从指定lsn位置复制，无指定lsn之前插入的数据
        9.删除复制槽成功
        10.清理环境完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0043 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0043"
        self.slot_name = "slot_pg_recvlogical_0043"
        self.t_name = "t_pg_recvlogical_0043"
    ...
    def test_standby(self):
        ...
        text = '--step6:获取当前lsn位置;expect:成功--'
        self.log.info(text)
        ...
        startpos = execute_cmd.splitlines()[-2].strip()
        ...
        text = '--step8:启动逻辑复制槽的流复制, 指定初始复制的LSN位置' \
               '--startpos=PTR;expect:启动流复制成功，从指定lsn位置复制，' \
               '无指定lsn之前插入的数据--'
        self.log.info(text)
        ...
        self.assertTrue(
            exec_msg.find('BEGIN ' + insert_msg) > -1 and exec_msg.find(
                'BEGIN ' + old_ins_msg) == -1)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0043 finish--')
