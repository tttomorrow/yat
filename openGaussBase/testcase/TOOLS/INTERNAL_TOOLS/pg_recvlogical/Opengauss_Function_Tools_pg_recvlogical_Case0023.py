...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 指定参数-?,输出帮助信息，随后立即退出
Description :
        1.指定参数-?,查看帮助信息
        pg_recvlogical -?
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0023 start-')
        self.primary_node = Node('PrimaryDbUser')

    def test_standby(self):
        text = '--step1:逻辑复制pg_recvlogical 指定参数-?;expect:输出帮助信息--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0023 finish--')
