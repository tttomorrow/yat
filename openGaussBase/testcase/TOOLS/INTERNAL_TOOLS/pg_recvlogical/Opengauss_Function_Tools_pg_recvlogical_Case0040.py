...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 指定参数--no-loop,连接失败后不重试
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.重启数据库
        4.主机创建逻辑复制槽
        5.启动逻辑复制槽的流复制, 指定参数--no-loop
        6.删除复制槽
        7.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0040 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0040"
        self.slot_name = "slot_pg_recvlogical_0040"
    ...
    def test_pg_recvlogical(self):
        ...
        text = '--step4:启动逻辑复制槽的流复制, 指定参数--no-loop;' \
               'expect:流复制启动失败，失败后不重试--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0040 finish--')
