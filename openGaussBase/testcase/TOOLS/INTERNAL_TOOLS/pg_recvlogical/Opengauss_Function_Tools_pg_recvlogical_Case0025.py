...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 指定发送的心跳包间隔时间
              --status-interval=2
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.主机创建逻辑复制槽
        4.启动逻辑复制槽的流复制, 指定发送的心跳包间隔时间--status-interval=2
        5.删除复制槽
        6.清理环境
Expect      :
        ...
        3.创建逻辑复制槽成功
        4.启动流复制成功，linux超时时间30s内，发送心跳包大于10条
        5.删除复制槽成功
        6.清理环境完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0025 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0025'
        self.slot_name = 'solt_pg_recvlogical_case0025'
    ...
    def test_standby(self):
        ...
        text = '--step4:启动逻辑复制槽的流复制, 指定发送的心跳包间隔时间' \
               '--status-interval=2;expect:启动流复制成功，linux超时时间30s' \
               '内，发送心跳包大于10条--'
        self.log.info(text)
        ...
        exec_msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(exec_msg)
        self.assertTrue(exec_msg.count("confirming write up to") > 10,
                        '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(restart_msg, '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0025 finish--')
