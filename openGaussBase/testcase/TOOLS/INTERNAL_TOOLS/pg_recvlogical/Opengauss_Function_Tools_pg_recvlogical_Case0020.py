...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical -?, –help -V, –version 期望:显示参数信息和版本信息
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.重启数据库
        4.主机创建逻辑复制槽, 不指定参数-v/--verbose
        5.启动逻辑复制槽的流复制, 不指定参数-v/--verbose
        6.删除复制槽, 不指定参数-v/--verbose
        7.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0020 start-')
        ...
        self.u_name = 'u_pg_recvlogical_0020'
        self.slot_name = 'solt_pg_recvlogical_case0020'
    ...
    def test_standby(self):
        ...
        text = '--step3:主机创建逻辑复制槽, 不指定参数-v/--verbose，' \
               ';expect:无详细信息--'
        self.log.info(text)
        ...
        self.assertNotIn(f'initializing replication slot "{self.slot_name}"',
                         exec_msg, '执行失败：' + text)

        text = '--step4:启动逻辑复制槽的流复制, 不指定参数-v/--verbose;' \
               'expect:无详细信息--'
        self.log.info(text)
        ...
        exec_msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(exec_msg)
        self.assertNotIn('confirming write up to', exec_msg,
                         '执行失败：' + text)

        text = '--step5:主机删除复制槽, 不指定参数-v/--verbose;' \
               'expect:无详细信息--'
        self.log.info(text)
        ...
        exec_msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(exec_msg)
        self.assertNotIn(f'freeing replication slot "{self.slot_name}"',
                         exec_msg, '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(restart_msg, '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0020 finish--')
