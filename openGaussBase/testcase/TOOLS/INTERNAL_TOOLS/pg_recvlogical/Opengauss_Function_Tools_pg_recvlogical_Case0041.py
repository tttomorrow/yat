...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 不指定参数-n/--no-loop,连接失败后重试
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.主机创建逻辑复制槽
        4.启动逻辑复制槽的流复制, 不指定参数-n/--no-loop
        5.删除复制槽
        6.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0041 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0041"
        self.slot_name = "slot_pg_recvlogical_0041"
    ...
    def test_pg_recvlogical(self):
        ...
        param_value = ['wal_level=logical', 'enable_slot_log=on',
                       'wal_sender_timeout=12', 'wal_receiver_timeout=12']
        ...
        text = '--step4:启动逻辑复制槽的流复制, 不指定参数-n/--no-loop;' \
               'expect:流复制启动失败，失败后重试--'
        self.log.info(text)
        ...
        exec_msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(exec_msg)
        self.assertTrue(exec_msg.count('pg_recvlogical: disconnected.') > 1,
                        '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0041 finish--')
