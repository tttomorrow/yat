...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0027 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0027'
        self.slot_name = 'solt_pg_recvlogical_case0027'
    ...
    def test_pg_recvlogical(self):
        ...
        text = '--step4:启动逻辑复制槽的流复制, 不指定发送的心跳包间隔时间' \
               '-s/--status-interval;expect:启动流复制成功，默认心跳包间隔' \
               '时间10s--'
        self.log.info(text)
        ...
        exec_msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(exec_msg)
        self.assertTrue(exec_msg.count("confirming write up to") < 5,
                        '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0027 finish--')
