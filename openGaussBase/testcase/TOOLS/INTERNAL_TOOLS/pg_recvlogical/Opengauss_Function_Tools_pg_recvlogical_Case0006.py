...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Tools_pg_recvlogical_Case0006 start----')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0006'
        self.slot_name = 'solt_pg_recvlogical_0006'
    ...
    def test_standby(self):
        ...
        text = '----step3:连接参数指定正确-U/-p/-h/-d/-W;expect:连接成功----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(text)
        self.log.info('----删除用户,逻辑复制槽;expect:成功----')
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '----Opengauss_Function_Tools_pg_recvlogical_Case0006 finish----')
