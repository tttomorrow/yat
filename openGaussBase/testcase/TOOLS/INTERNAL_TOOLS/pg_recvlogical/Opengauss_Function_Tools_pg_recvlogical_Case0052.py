...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 输出插件添加指定以外参数
              -o pretty-print=1，报错
Description :
...
Expect      :
        ...
        3.创建逻辑复制槽成功
        4.启动流复制失败，输出插件添加指定以外参数-o pretty-print=1
        FATAL:  option "pretty-print" = "1" is unknown
        5.删除复制槽成功
        6.清理环境完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0052 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0052"
        self.slot_name = "slot_pg_recvlogical_0052"
    ...
    def test_standby(self):
        ...
        text = '--step4 :启动逻辑复制槽的流复制, 输出插件添加指定以外参数' \
               '-o pretty-print=1;expect:启动流复制失败，输出插件添加指定' \
               '以外参数-o pretty-print=1--'
        self.log.info(text)
        ...
        exec_msg = self.primary_node.sh(execute_cmd).result()
        self.log.info(exec_msg)
        self.assertIn('FATAL:  option "pretty-print" = "1" is unknown',
                      exec_msg, '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0052 finish--')
