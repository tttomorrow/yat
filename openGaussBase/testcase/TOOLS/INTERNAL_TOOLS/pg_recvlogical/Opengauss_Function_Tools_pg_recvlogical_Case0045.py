...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 设置输出到指定文件-f -,输出到stdout
Description :
        ...
        6.启动逻辑复制槽的流复制，设置输出到指定文件-f -,输出到stdout
        7.删除复制槽
        8.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0045 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0045"
        self.slot_name = "slot_pg_recvlogical_0045"
        self.t_name = "t_pg_recvlogical_0045"
    ...
    def test_standby(self):
        ...
        text = '--step6:启动逻辑复制槽的流复制, 设置输出到指定文件-f -,' \
               '输出到stdout;expect:启动流复制成功，解码信息输出到stdout--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0045 finish--')
