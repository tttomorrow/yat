...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 行为参数指定--start，启动流复制成功
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.主机创建逻辑复制槽并查询
        4.启动逻辑复制槽的流复制, 指定行为参数--start
        5.删除复制槽
        6.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0011start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0011'
        self.slot_name = 'solt_pg_recvlogical_case0011'
    ...
    def test_standby(self):
        ...
        text = '--step3:主机创建逻辑复制槽并查询;expect:成功--'''
        self.log.info(text)
        ...
        self.assertIn(f'{self.slot_name}', check_res, '执行失败：' + text)

        text = '--step4:启动逻辑复制槽的流复制, 指定行为参数--start;' \
               'expect:开始流复制--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(restart_msg, '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0011finish--')
