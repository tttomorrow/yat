...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 行为参数指定多个 --start --drop，提示异常
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.逻辑复制pg_recvlogical 指定行为参数--start --drop
        4.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0016 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0016'
        self.slot_name = 'solt_pg_recvlogical_case0016'
    ...
    def test_standby(self):
        ...
        text = '--step3:逻辑复制pg_recvlogical 指定行为参数--start --drop;' \
               'expect:报错 --'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(restart_msg, '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0016 finish--')
