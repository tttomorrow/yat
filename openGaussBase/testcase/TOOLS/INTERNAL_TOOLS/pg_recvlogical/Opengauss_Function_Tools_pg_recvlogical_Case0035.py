...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0035 start-')
        ...
        self.u_name = "u_pg_recvlogical_0035"
        self.slot_name = "test@001"
    ...
    def test_pg_recvlogical(self):
        ...
        text = '--step3:主机创建逻辑复制槽，包含(_?-.)以外特殊字符--;' \
               'expect:合理报错--'
        self.log.info(text)
        ...
        self.assertIn(f'FATAL:  replication slot name "{self.slot_name}" '
                      f'contains invalid character', exec_msg,
                      '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0035 finish--')
