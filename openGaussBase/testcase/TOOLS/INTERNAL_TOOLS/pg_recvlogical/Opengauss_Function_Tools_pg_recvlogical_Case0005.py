...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical -W/--password 参数指定，输入密码错误，
              连接失败
Description :
    1.修改参数wal_level为logical;enable_slot_log为on
    2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
    3.指定参数-W/--password ,输入错误密码
    4.清理环境
Expect      :
    1.修改成功
    2.配置成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Tools_pg_recvlogical_Case0005开始执行----')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pg_hba = os.path.join(macro.DB_INSTANCE_PATH,
                                   macro.PG_HBA_FILE_NAME)
        self.u_name = 'u_pg_recvlogical_0005'
        self.slot_name = 'solt_pg_recvlogical_0005'
        self.ha_port = str(int(self.primary_node.db_port) + 1)
        self.error_password = f'abc + {macro.COMMON_PASSWD}'
    ...
    def test_standby(self):
        ...
        text = '----step3:指定参数-W/--password,输入错误密码;' \
               'expect: 连接失败----'
        self.log.info(text)
        connect_options = ['-W', '--password']
