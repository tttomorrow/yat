...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 指定参数--verbose,输出详细信息
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.重启数据库
        4.主机创建逻辑复制槽, 指定参数--verbose,查看详细信息
        5.启动逻辑复制槽的流复制, 指定参数--verbose,查看详细信息
        6.删除复制槽, 指定参数--verbose,查看详细信息
        7.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0019 start-')
        ...
        self.u_name = 'u_pg_recvlogical_0019'
        self.slot_name = 'solt_pg_recvlogical_case0019'
    ...
    def test_standby(self):
        ...
        text = '--step3:主机创建逻辑复制槽, 指定参数--verbose,查看详细信息;' \
               'expect:成功--'
        self.log.info(text)
        ...
        text = '--step4:启动逻辑复制槽的流复制, 指定参数--verbose,查看详细' \
               '信息;expect:成功--'
        self.log.info(text)
        ...
        text = '--step5:主机删除复制槽, 指定参数--verbose,查看详细信息;' \
               'expect:成功--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(restart_msg, '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0019 finish--')
