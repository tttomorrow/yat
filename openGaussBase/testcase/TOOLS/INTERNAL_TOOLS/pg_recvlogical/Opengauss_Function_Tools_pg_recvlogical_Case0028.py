...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 指定逻辑复制槽的名称-S
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.主机创建逻辑复制槽, 指定逻辑复制槽的名称-S
        4.启动逻辑复制槽的流复制, 指定逻辑复制槽的名称-S
        5.删除复制槽, 指定逻辑复制槽的名称-S
        6.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0028 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0028'
        self.slot_name = 'solt_pg_recvlogical_case0028'
    ...
    def test_pg_recvlogical(self):
        ...
        text = '--step4:启动逻辑复制槽的流复制, 指定逻辑复制槽的名称-S;' \
               'expect:成功--'
        self.log.info(text)
        ...
        text = '--step5:主机删除复制槽, 指定逻辑复制槽的名称-S;expect:成功--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0028 finish--')
