...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 不指定初始复制的LSN位置-I/--startpos=PTR
Description :
        ...
        6.启动逻辑复制槽的流复制，不指定初始复制的LSN位置-I/--startpos=PTR
        7.向列存表中插入数据，获取当前事务ID
        8.启动逻辑复制槽的流复制，不指定初始复制的LSN位置-I/--startpos=PTR
        9.删除复制槽
        10.清理环境
Expect      :
        ...
        4.创建表成功
        5.插入数据成功
        6.启动流复制成功，从初始lsn位置复制
        7.插入数据成功
        8.启动流复制成功，从上次lsn位置复制
        9.删除复制槽成功
        10.清理环境完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0044 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0044"
        self.slot_name = "slot_pg_recvlogical_0044"
        self.t_name = "t_pg_recvlogical_0044"
    ...
    def test_standby(self):
        ...
        self.assertTrue(exec_msg.find('BEGIN ' + old_ins_msg) > -1,
                        '执行失败：' + text)
        ...
        text = '--step8:启动逻辑复制槽的流复制, 不指定初始复制的LSN位置' \
               '-I/--startpos=PTR;expect:启动流复制成功，从上次lsn位置复制--'
        self.log.info(text)
        start_cmd = f"pg_recvlogical " \
                    f"-d postgres " \
                    f"-U {self.u_name} " \
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0044 finish--')
