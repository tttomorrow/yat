...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical --no-password 参数指定时，不使用密码进行
              连接，连接成功
Description :
    1.修改参数wal_level为logical;enable_slot_log为on
    2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
    3.逻辑复制pg_recvlogical --no-password 参数指定
    4.清理环境
Expect      :
    1.修改成功
    2.配置成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Tools_pg_recvlogical_Case0009开始执行----')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.username = self.primary_node.ssh_user
        self.slot_name = 'solt_pg_recvlogical_0009'
    ...
    def test_standby(self):
        ...
        text = '----step4:逻辑复制pg_recvlogical --no-password 参数指定;' \
               'expect:不输入密码连接成功----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '----Opengauss_Function_Tools_pg_recvlogical_Case0009 finish----')
