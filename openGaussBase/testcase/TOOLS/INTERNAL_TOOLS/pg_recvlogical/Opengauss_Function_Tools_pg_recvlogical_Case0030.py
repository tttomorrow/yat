...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 不指定逻辑复制槽的名称-s/--slot=SLOT
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.重启数据库
        4.主机创建逻辑复制槽, 不指定逻辑复制槽的名称-s/--slot=SLOT
        5.启动逻辑复制槽的流复制, 不指定逻辑复制槽的名称-s/--slot=SLOT
        6.删除复制槽, 不指定逻辑复制槽的名称-s/--slot=SLOT
        7.清理环境
Expect      :
        1.修改参数wal_level为logical;enable_slot_log为on成功
        2.pg_hba.conf 配置逻辑复制的用户白名单成功
        3.重启数据库成功
        4.创建逻辑复制槽,不指定逻辑复制槽的名称,提示没有指定逻辑复制槽
        pg_recvlogical: no slot specified
        5.启动流复制,不指定逻辑复制槽的名称,提示没有指定逻辑复制槽
        pg_recvlogical: no slot specified
        6.删除复制槽,不指定逻辑复制槽的名称,提示没有指定逻辑复制槽
        pg_recvlogical: no slot specified
        7.清理环境完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0030 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0030'
        self.slot_name = 'solt_pg_recvlogical_case0030'
    ...
    def test_pg_recvlogical(self):
        ...
        text = '--step3:主机创建逻辑复制槽, 不指定逻辑复制槽的名称' \
               '-s/--slot=SLOT;expect:报错--'
        self.log.info(text)
        ...
        self.assertIn('pg_recvlogical: no slot specified', exec_msg,
                      '执行失败：' + text)

        text = '--step4:启动逻辑复制槽的流复制, 不指定逻辑复制槽的名称' \
               '-s/--slot=SLOT;expect:报错--'
        self.log.info(text)
        ...
        text = '--step5:主机删除复制槽, 不指定逻辑复制槽的名称' \
               '-s/--slot=SLOT;expect:报错--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0030 finish--')
