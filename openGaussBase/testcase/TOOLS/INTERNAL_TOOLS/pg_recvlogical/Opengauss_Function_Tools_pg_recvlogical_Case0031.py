...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 指定不存在的逻辑复制槽的名称，提示不存在
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.启动逻辑复制槽的流复制, 指定不存在逻辑复制槽的名称-S
        4.删除复制槽, 指定不存在逻辑复制槽的名称-S
        5.清理环境
Expect      :
        1.修改参数wal_level为logical;enable_slot_log为on成功
        2.pg_hba.conf 配置逻辑复制的用户白名单成功
        3.启动流复制,提示指定逻辑复制槽"solt_pg_recvlogical_case0031"不存在
        FATAL:  replication slot "solt_pg_recvlogical_case0031" does not exist
        4.删除复制槽,提示指定逻辑复制槽"solt_pg_recvlogical_case0031"不存在
        FATAL:  replication slot "solt_pg_recvlogical_case0031" does not exist
        5.清理环境完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0031 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0031'
        self.slot_name = 'solt_pg_recvlogical_case0031'
    ...
    def test_pg_recvlogical(self):
        ...
        text = '--step3:启动逻辑复制槽的流复制, 指定不存在的逻辑复制槽的名称' \
               '-s;expect:报错--'
        self.log.info(text)
        ...
        self.assertIn(
            f'FATAL:  replication slot "{self.slot_name}" does not exist',
            exec_msg, '执行失败：' + text)

        text = '--step4:主机删除复制槽, 指定不存在的逻辑复制槽的名称-s;' \
               'expect:报错--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0031 finish--')
