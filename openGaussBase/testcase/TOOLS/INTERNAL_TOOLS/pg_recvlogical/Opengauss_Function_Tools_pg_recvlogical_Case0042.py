...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 指定初始复制的LSN位置-I
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0042 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0042"
        self.slot_name = "slot_pg_recvlogical_0042"
        self.t_name = "t_pg_recvlogical_0042"
    ...
    def test_standby(self):
        ...
        text = '--step8:启动逻辑复制槽的流复制, 指定初始复制的LSN位置-I;' \
               'expect:启动流复制成功，从指定lsn位置复制，' \
               '无指定lsn之前插入的数据--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0042 finish--')
