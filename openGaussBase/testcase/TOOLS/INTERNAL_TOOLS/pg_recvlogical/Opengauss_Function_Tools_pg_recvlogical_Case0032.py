...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 逻辑复制槽的名称-S 包含数字，创建成功
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.主机创建逻辑复制槽, 逻辑复制槽的名称-S 包含数字
        4.启动逻辑复制槽的流复制, 逻辑复制槽的名称-S 包含数字
        5.删除复制槽, 逻辑复制槽的名称-S 包含数字
        6.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0032 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0032'
        self.slot_name = '001'
    ...
    def test_pg_recvlogical(self):
        ...
        text = '--step3:主机创建逻辑复制槽, 逻辑复制槽的名称-S 包含数字;' \
               'expect:成功--'
        self.log.info(text)
        ...
        text = '--step4:启动逻辑复制槽的流复制, 逻辑复制槽的名称-S 包含数字;' \
               'expect:成功--'
        self.log.info(text)
        ...
        text = '--step5:主机删除复制槽, 逻辑复制槽的名称-S 包含数字;' \
               'expect:成功--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0032 finish--')
