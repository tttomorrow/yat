...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 行为参数指定--drop，删除逻辑复制槽成功
Description :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0012 start-')
        ...
        self.u_name = 'u_pg_recvlogical_0012'
        self.slot_name = 'solt_pg_recvlogical_case0012'
    ...
    def test_standby(self):
        ...
        text = '--step3:主机创建逻辑复制槽,指定行为参数--create;expect:成功 --'
        self.log.info(text)
        ...
        text = '--step4:查询复制槽;expect:查询成功--'
        self.log.info(text)
        query_cmd = self.pri_sh.execut_db_sql('select slot_name,plugin from'
                                              ' pg_get_replication_slots();')
        self.log.info(query_cmd)
        self.assertIn(f'{self.slot_name}', query_cmd, '执行失败：' + text)

        text = '--text5:主机删除复制槽;expect:成功--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(restart_msg, '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0012 finish--')
