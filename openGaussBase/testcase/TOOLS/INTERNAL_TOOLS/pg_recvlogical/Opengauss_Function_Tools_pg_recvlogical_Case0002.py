...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical -h/--host=HOSTNAME 参数指定错误时，
              连接服务失败
Description :
...
Expect      :
    1.修改成功
    2.配置成功
    3.合理报错failed to connect pgrecvlogical0002:60507
    4.清理环境完成
History     :
"""
import unittest
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Tools_pg_recvlogical_Case0002开始执行----')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.u_name = 'u_pg_recvlogical_0002'
        self.slot_name = 'solt_pg_recvlogical_0002'
        self.ha_port = str(int(self.primary_node.db_port) + 1)
        self.not_exist_host = 'pgrecvlogical0002'
        self.error_msg = f'failed to connect {self.not_exist_host}:' \
                         f'{self.ha_port}'
    ...
    def test_standby(self):
        ...
        text = '----step3:指定不存在主机名-h/--host;expect:连接失败----'
        self.log.info(text)
        connect_options = [f'-h {self.not_exist_host}',
                           f'--host={self.not_exist_host}']
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '--Opengauss_Function_Tools_pg_recvlogical_Case0002 finish--')
