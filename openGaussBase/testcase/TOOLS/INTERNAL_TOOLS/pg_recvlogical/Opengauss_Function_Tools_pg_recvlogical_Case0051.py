...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 输出插件添加参数-o/--option=include
              -timestamp，解码信息是否包含commit时间戳
Description :
        ...
        4.连接数据库，创建列存表
        5.向列存表中插入数据，获取当前时间
        6.启动逻辑复制槽的流复制，添加参数-o include-timestamp=0
        7.向列存表中插入数据，获取当前时间
        8.启动逻辑复制槽的流复制，添加参数--option=include-timestamp=1
        9.删除复制槽
        10.清理环境
Expect      :
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0051 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0051"
        self.slot_name = "slot_pg_recvlogical_0051"
        self.t_name = "t_pg_recvlogical_0051"
    ...
    def test_standby(self):
        ...
        text = '--step6:启动逻辑复制槽的流复制, ' \
               '添加参数-o include-timestamp=0;' \
               'expect:启动流复制成功，解码出的data列不包含xid信息--'
        self.log.info(text)
        ...
        text = '--step8:启动逻辑复制槽的流复制, ' \
               '添加参数--option=include-timestamp=1;' \
        ...
        start_cmd = f"pg_recvlogical " \
                    ...
                    f"-p {str(int(self.primary_node.db_port)+1)} " \
                    f"--start -s 2 " \
                    f"-v -f - -n --option=include-timestamp=1"
        execute_cmd = f'''source {macro.DB_ENV_PATH}
                   expect <<EOF
                   set timeout 30
                   spawn {start_cmd}
        ...
        self.assertIn('confirming write up to', exec_msg, '执行失败：' + text)
        self.assertNotIn('disconnected', exec_msg, '执行失败：' + text)
        self.assertIn('(at', exec_msg, '执行失败：' + text)
        self.log.info('------------杀掉pg_recvlogical进程-----------')
        kill_msg = self.primary_node.sh(kill_cmd).result()
        self.log.info(kill_msg)
        sleep(5)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0051 finish--')
