...
"""
Case Type   : 系统内部使用工具
Case Name   : 逻辑复制pg_recvlogical 不指定使用的逻辑复制槽输出插件，
             默认值mppdb_decoding
Description :
        1.修改参数wal_level为logical;enable_slot_log为on
        2.主机pg_hba.conf文件中配置逻辑复制的用户白名单
        3.主机创建逻辑复制槽,不指定使用的逻辑复制槽输出插件-P/--plugin
        4.查询复制槽信息
        5.删除复制槽
        6.清理环境
Expect      :
        ...
        3.创建逻辑复制槽成功
        4.查询复制槽信息，创建的逻辑复制槽输出插件为mppdb_decoding
        5.删除复制槽成功
        6.清理环境完成
History     :
"""
...
class Tools(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0055 start-')
        self.constant = Constant()
        self.common = Common()
        self.primary_node = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.u_name = "u_pg_recvlogical_0055"
        self.slot_name = "slot_pg_recvlogical_0055"
    ...
    def test_standby(self):
        ...
        text = '--step4:查询复制槽信息;expect:复制槽存在--'
        self.log.info(text)
        query_cmd = f"select plugin from pg_replication_slots where " \
                    f"slot_name = '{self.slot_name}';"
        self.log.info(query_cmd)
        execute_cmd = self.pri_sh.execut_db_sql(sql=query_cmd,
                                                dbname="postgres")

        self.log.info(execute_cmd)
        self.assertIn('mppdb_decoding', execute_cmd, '执行失败：' + text)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '-Opengauss_Function_Tools_pg_recvlogical_Case0055 finish--')
