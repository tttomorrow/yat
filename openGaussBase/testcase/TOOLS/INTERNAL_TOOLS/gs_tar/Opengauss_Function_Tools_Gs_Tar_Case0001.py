...
"""
Case Type   : 基础功能
Case Name   : gs_basebackup指定压缩格式备份后使用gs_tar直接解压
Description :
    1.新建备份目录与解压目录
    2.不指定压缩级别进行备份
    3.使用gs_tar进行解压
...
class Tarclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_Tools_Gs_Tar_Case0001 start")
    ...
    def test_tar0001(self):
        try:
            ...
            text = '----step2: 不指定压缩级别进行备份 expect:不指定压缩级别进行备份成功----'
            self.log.info(text)
            ...
            cmd = f"chmod -R 700 {self.tar_path};" \
                ...
                f"gs_ctl start -D {self.tar_path} -M primary -t 300"
            self.log.info(cmd)
            result = self.primary_user_node.sh(cmd).result()
            self.log.info(result)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_Gs_Tar_Case0001 end-")
