...
Case Name   : 不自定义表空间，本地备份，开启进度报告，开启冗余模式
Description :
    1、开始备份：gs_basebackup -D /usr2/chenchen/basebackup/bak_Pv  -Fp
        -Xstream -p 18333 -l gauss_10.bak -P -v -U sysadmin -W
    2、使用备份目录启动数据库
Expect      :
...
@unittest.skipIf(1 == Primary_SH.get_node_num(),
                 '需主备环境，若为单机环境则不执行')
class GsBaseBackUpCase9(unittest.TestCase):
    def setUp(self):
        self.Primary_User_Node = Node('PrimaryDbUser')
        self.Primary_Root_Node = Node('PrimaryRoot')
        self.LOG = Logger()
        self.Constant = Constant()
        self.T_NAME = 't_basebackup_9'
        self.backup_bak_path = os.path.join(macro.DB_BACKUP_PATH,
                                            'gs_basebackup')
        self.gs_basebackup_bak_name = 'gs_basebackup_Case0009.bak'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0009 start----')
    ...
    def tearDown(self):
        ...
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0009 end----')
