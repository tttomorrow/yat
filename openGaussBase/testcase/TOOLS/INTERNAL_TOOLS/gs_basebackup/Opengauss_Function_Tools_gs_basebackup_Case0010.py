...
Description :
    1、开始备份：gs_basebackup -D /usr2/chenchen/basebackup/bak_Pvfast -Fp
        -Xstream -p 18333 -l gauss_11.bak -P -v -U sysadmin
        -W --checkpoint=fast
    2、使用备份目录启动数据库
Expect      :
...
@unittest.skipIf(1 == Primary_SH.get_node_num(),
                 '需主备环境，若为单机环境则不执行')
class GsBaseBackUpCase10(unittest.TestCase):
    def setUp(self):
        self.Primary_User_Node = Node('PrimaryDbUser')
        self.Primary_Root_Node = Node('PrimaryRoot')
        self.LOG = Logger()
        self.Constant = Constant()
        self.T_NAME = 't_basebackup_10'
        self.backup_bak_path = os.path.join(macro.DB_BACKUP_PATH,
                                            'gs_basebackup')
        self.gs_basebackup_bak_name = 'gs_basebackup_Case0010.bak'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0010 start----')
    ...
    def test_server_tools(self):
        ...
        text = '----step2: 修改参数data_directory expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step2.1: 使用备份目录启动数据库 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step2.3: 查询数据库状态确认是否启动成功 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step2.4: 查询测试表是否存在 expect: 成功----'
        self.LOG.info(text)
    ...
    def tearDown(self):
        ...
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0010 end----')
