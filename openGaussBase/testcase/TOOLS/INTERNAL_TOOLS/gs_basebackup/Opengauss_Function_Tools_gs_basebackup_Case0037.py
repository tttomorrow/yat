...
Description :
    ...
    3.打开两个gsql会话，执行checkpoint和vacuum
    4.使用备份目录启动数据库
Expect      :
    1.创建备份目录成功
    2.备份提示成功，备份目录下生成备份文件
    3.checkpoint和vacuum成功
...
class GsBaseBackUpCase37(unittest.TestCase):
    def setUp(self):
        ...
        self.backup_name = 'gs_basebackup_Case0037.bak'
        self.LOG.info(f'-----{os.path.basename(__file__)} start-----')
        text = '----调大wal_sender_timeout值----'
        self.LOG.info(text)
        guc_flag = Primary_SH.execute_gsguc('reload',
                                            self.Constant.GSGUC_SUCCESS_MSG,
                                            'wal_sender_timeout=180s')
        self.assertTrue(guc_flag)
    ...
    def test_server_tools(self):
        ...
        bak_text = '----step2.2: 执行备份 expect: 成功----'
        self.LOG.info(bak_text)
        ...
        text = '----step3: 打开两个gsql会话，执行checkpoint和vacuum expect: 成功----'
        self.LOG.info(text)
        sql_cmd = "checkpoint;"
