...
Case Name   : 不自定义表空间，本地备份，开启进度报告，开启冗余模式，检查点模式设置为fast,不出现输入密码提示,omm用户备份（初始用户）
Description :
    1、开始备份：gs_basebackup -D /usr2/chenchen/basebackup/bak_Pvfast_womm
        -Fp -Xstream -p 18333 -l gauss_13.bak -P -v -U omm -w
    2、使用备份目录启动数据库
Expect      :
...
@unittest.skipIf(1 == Primary_SH.get_node_num(),
                 '需主备环境，若为单机环境则不执行')
class GsBaseBackUpCase12(unittest.TestCase):
    def setUp(self):
        self.Primary_User_Node = Node('PrimaryDbUser')
        self.Primary_Root_Node = Node('PrimaryRoot')
        self.LOG = Logger()
        self.Constant = Constant()
        self.T_NAME = 't_basebackup_12'
        self.backup_bak_path = os.path.join(macro.DB_BACKUP_PATH,
                                            'gs_basebackup')
        self.backup_bak_name = 'gs_basebackup_Case0012.bak'
        self.backup_Expect_Msg = 'gs_basebackup: base backup completed'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0012 start----')
    ...
    def test_server_tools(self):
        ...
        text = '----step2.5: 查询数据库状态确认是否启动成功 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step2.6: 查询测试表是否存在 expect: 成功----'
        self.LOG.info(text)
    ...
    def tearDown(self):
        ...
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0012 end----')
