...
Case Name   : 不自定义表空间，远程备份，开启进度报告，开启冗余模式，检查点模式设置为fast,t模式压缩备份,压缩级别5
Description :
    1.创建备份目录
    2.开始备份：gs_basebackup -D /usr2/chenchen/basebackup/bak_hPvfast_t
        -Ft -Xfetch -Z5 -p 18333 -h ip -l gauss_25.bak -U sysadmin -W
    3.解压1gzip -d base.tar.gz
    4.解压2：gs_tar -D /wyc/tmp/backup/tmp -F /wyc/tmp/backup/base.tar
    5.使用解压后的目录启动数据库
Expect      :
    1.创建备份目录成功
    2.备份提示成功，生成base.tar.gz备份文件
    3.解压出base.tar
    4.解压出openGauss数据目录
    5.使用备份目录启动数据库失败
History     :
...
@unittest.skipIf(1 == Primary_SH.get_node_num(),
                 '需主备环境，若为单机环境则不执行')
class GsBaseBackUpCase24(unittest.TestCase):
    def setUp(self):
        ...
        self.T_NAME = 't_basebackup_24'
        self.T_Insert_Value = 'gs_basebackup_24'
        self.gs_basebackup_bak_name = 'gs_basebackup_Case0024.bak'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0024 start----')
    ...
    def test_server_tools(self):
        ...
        text = '----step5.1: 主机停止数据库 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step5.3: 主机使用备份目录启动数据库 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step5.6: 主机查询数据库状态确认是否启动成功 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step5.7: 主机查询测试表是否存在 expect: 成功----'
        self.LOG.info(text)
    ...
    def tearDown(self):
        ...
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0024 end----')
