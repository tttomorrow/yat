...
Case Name   : 备份包含内存表的数据库
Description :
    1.创建备份目录
    2.关掉enable_incremental_checkpoint
    3.建mot表并插入数据create foreign table mot_table(a int);，插入数据
    4.执行备份：gs_basebackup -D /usr2/chenchen/basebackup/bak_default
...
@unittest.skipIf(1 == Primary_SH.get_node_num(),
                 '需主备环境，若为单机环境则不执行')
class GsBaseBackUpCase36(unittest.TestCase):
    def setUp(self):
        self.Primary_User_Node = Node('PrimaryDbUser')
        self.Primary_Root_Node = Node('PrimaryRoot')
        self.LOG = Logger()
        self.Constant = Constant()
        self.T_NAME = 't_basebackup_36'
        self.backup_bak_path = os.path.join(macro.DB_BACKUP_PATH,
                                            'gs_basebackup')
        self.backup_bak_name = 'gs_basebackup_Case0036.bak'
        self.Backup_Expect_Msg = 'gs_basebackup: base backup completed'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0036 start----')
    ...
    def test_server_tools(self):
        ...
        text = '----step2.1: 关掉enable_incremental_checkpoint expect: 成功----'
        self.LOG.info(text)
        gs_guc_result = Primary_SH.execute_gsguc(
            'set',
            self.Constant.GSGUC_SUCCESS_MSG,
            'enable_incremental_checkpoint=off')
        self.assertTrue(gs_guc_result, '执行失败:' + text)

        text = '----step2.2: 重启数据库 expect: 成功----'
        self.LOG.info(text)
        Primary_SH.restart_db_cluster()
        db_status = Primary_SH.get_db_cluster_status()
        self.assertTrue('Degraded' in db_status or 'Normal' in db_status,
                        '执行失败:' + text)

        text = '----step3: 创建测试表 expect: 成功----'
        self.LOG.info(text)
        sql_cmd = f"drop foreign table if exists {self.T_NAME}; " \
            f"create foreign table {self.T_NAME}(a int); " \
            f"insert into  {self.T_NAME} select generate_series(1,1000);"
        self.LOG.info(sql_cmd)
        sql_result = Primary_SH.execut_db_sql(sql_cmd)
        self.LOG.info(sql_result)
        self.assertIn('CREATE FOREIGN TABLE', sql_result, '执行失败:' + text)
        self.assertIn(self.Constant.INSERT_SUCCESS_MSG,
                      sql_result,
                      '执行失败:' + text)

        text = '----step4.1: 执行备份 expect: 成功----'
        self.LOG.info(text)
        ...
        self.assertIn(self.Backup_Expect_Msg, backup_msg, '执行失败:' + text)

        text = '----step2.1: 停止数据库 expect: 成功----'
        self.LOG.info(text)
    ...
    def tearDown(self):
        ...
        cp_res = Primary_SH.execut_db_sql('checkpoint;')
        self.LOG.info(cp_res)

        time.sleep(5)

        self.LOG.info('----还原enable_incremental_checkpoint---')
        gs_guc_result = Primary_SH.execute_gsguc(
            'set',
            self.Constant.GSGUC_SUCCESS_MSG,
            'enable_incremental_checkpoint=on')
        self.assertTrue(gs_guc_result)

        self.LOG.info('----重启数据库---')
        Primary_SH.restart_db_cluster()

        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0036 end----')
