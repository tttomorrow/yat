...
Case Name   : 不自定义表空间，本地备份，开启进度报告
Description :
    1.创建备份目录
    2.创建测试表
    create table testzl(a int);
    3.执行备份，并开启进度报告
    gs_basebackup -D /opt/openGauss_lu1208/bak -Fp -Xstream
        -p 18333 -l gauss_9.bak -P -U sysadmin -W
    4.使用备份目录启动数据库
...
@unittest.skipIf(1 == Primary_SH.get_node_num(),
                 '需主备环境，若为单机环境则不执行')
class GsBaseBackUpCase8(unittest.TestCase):
    def setUp(self):
        self.Primary_User_Node = Node('PrimaryDbUser')
        self.Primary_Root_Node = Node('PrimaryRoot')
        self.LOG = Logger()
        self.Constant = Constant()
        self.T_NAME = 't_basebackup_8'
        self.backup_bak_path = os.path.join(macro.DB_BACKUP_PATH,
                                            'gs_basebackup')
        self.gs_basebackup_bak_name = 'gs_basebackup_Case0008.bak'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0008 start----')
    ...
    def test_server_tools(self):
        ...
        text = '----step5.3: 查询数据库状态确认是否启动成功 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step6: 查询测试表是否存在 expect: 成功----'
        self.LOG.info(text)
    ...
    def tearDown(self):
        self.LOG.info('----step7: run_teardown expect: 成功----')
        self.LOG.info('----停止数据库----')
        ...
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0008 end----')
