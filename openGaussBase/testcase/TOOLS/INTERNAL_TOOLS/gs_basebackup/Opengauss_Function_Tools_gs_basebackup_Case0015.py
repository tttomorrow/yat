...
Case Name   : 不自定义表空间，本地备份，开启进度报告，开启冗余模式，检查点模式设置为fast,
    不出现输入密码提示,t模式压缩备份,指定压缩级别0
Description :
    1、创建备份目录
    2、开始备份：gs_basebackup -D /usr2/chenchen/basebackup/bak_Pvfast_wt0
        -Ft -Xfetch -Z0 -p 18333 -l gauss_16.bak -P -v -U sysadmin -w
...
@unittest.skipIf(1 == Primary_SH.get_node_num(),
                 '需主备环境，若为单机环境则不执行')
class GsBaseBackUpCase15(unittest.TestCase):
    def setUp(self):
        ...
        self.T_NAME = 't_basebackup_15'
        self.T_Insert_Value = 'gs_basebackup_15'
        self.backup_bak_name = 'gs_basebackup_Case0015.bak'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0015 start----')
    ...
    def tearDown(self):
        ...
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0015 end----')
