...
Case Name   : 自定义绝对路径表空间，本地默认模式备份
Description :
    1.创建备份目录
    2.创建表空间tablespace_test1、tablespace_test2
    3.开始备份：gs_basebackup -D /home/function0226/gs_basebackup -p 30226 -T
...
Expect      :
    1.创建备份目录成功
    2.创建表空间成功
    3.备份提示成功，/data/function0226/cluster/tablespace_test2目录不为空
...
@unittest.skipIf(1 == Primary_SH.get_node_num(),
                 '需主备环境，若为单机环境则不执行')
class GsBaseBackUpCase26(unittest.TestCase):
    def setUp(self):
        ...
        self.backup_bak_path = os.path.join(macro.DB_BACKUP_PATH,
                                            'gs_basebackup')
        self.backup_bak_name = 'gs_basebackup_Case0026.bak'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0026 start----')
    ...
    def test_server_tools(self):
        ...
        text = '----step2.1: 创建表空间目录及重定向目录 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step2.2: 创建测试表空间及表 expect: 成功----'
        self.LOG.info(text)
        ...
        text = '----step2.3: 等待备机完成数据同步 expect: 成功----'
        self.LOG.info(text)
        node_num = self.Common.get_node_num(self.Primary_User_Node)
        self.LOG.info(node_num)
        cons_flag = Primary_SH.check_location_consistency('primary',
                                                          node_num,
                                                          300)
        self.assertTrue(cons_flag, '执行失败:' + text)
        ...
        text = '----step3.2: 查看重定向目录 expect: 成功----'
        self.LOG.info(text)
    ...
    def tearDown(self):
        ...
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0026 end----')
