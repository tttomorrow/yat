...
Description :
    1、创建备份目录
    2、开始备份：gs_basebackup -D /usr2/chenchen/basebackup/bak_Pvfast_wt9 -Ft
...
History     :
    2021-3-12 wal_sender_timeout值太小导致网络波动时偶现失败，故用例开始执行时调大该值
"""
...
class GsBaseBackUpCase17(unittest.TestCase):
    def setUp(self):
        ...
        self.backup_bak_name = 'gs_basebackup_Case0017.bak'
        self.LOG.info(
            '----Opengauss_Function_Tools_gs_basebackup_Case0017 start----')

        text = '----step1: 调大wal_sender_timeout值 expect: 成功----'
        self.LOG.info(text)
    ...
    def test_server_tools(self):
        ...
        self.LOG.info(text)
        build_msg_list = Primary_SH.get_standby_and_build(options='-t 600')
