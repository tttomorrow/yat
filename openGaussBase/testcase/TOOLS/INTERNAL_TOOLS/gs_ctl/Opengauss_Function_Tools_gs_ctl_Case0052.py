...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl stop指定-m的值为fast，stop主机后活跃事务是否回滚
Description :
    1.开启事务
    2.执行事务，不做提交
    3.开启新的session指定-m设置参数为fast，stop主机
    4.重新开启数据库
    5.查看事务是否回滚
Expect      :
    1.开启事务成功
    2.执行事务，不做提交成功
    3.开启新的session指定-m设置参数为fast，stop主机成功
    4.重新开启数据库
    5.查看事务，事务回滚
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0052开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        restart_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl stop -D {macro.DB_INSTANCE_PATH} -m fast ;
            '''
        LOG.info(restart_cmd)
        restart_msg = self.PrimaryNode.sh(restart_cmd).result()
        LOG.info(restart_msg)
        self.assertIn(self.constant.GS_CTL_STOP_SUCCESS_MSG, restart_msg)
    ...
    def tearDown(self):
        ...
        restart_msg = self.PrimaryNode.sh(restart_cmd).result()
        LOG.info(restart_msg)
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0052执行完成---')
