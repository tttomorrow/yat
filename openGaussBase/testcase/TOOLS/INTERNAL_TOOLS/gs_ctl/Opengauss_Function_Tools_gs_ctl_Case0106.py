...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0106开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        progress_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl querybuild -D {macro.DB_INSTANCE_PATH} ;
            '''
        LOG.info(progress_cmd)
        progress_msg = self.StandbyNode.sh(progress_cmd).result()
        LOG.info(progress_msg)
        self.assertIn('Build completed', progress_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0106执行完成---')
