...
"""
Case Type   : 系统内部使用工具
Case Name   : 在主节点执行failover是否成功
Description :
    1.在主机上执行failover（主机执行，不stop备机）
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0074开始执行-----')
    ...
    def tearDown(self):
        ...
        if 'Standby' in self.node_msg:
            ...
            msg1 = self.PrimaryNode.sh(recover_cmd).result()
            LOG.info(msg1)
        else:
            return '主备节点正常'
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0074执行完成----')
