...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl start指定-l FILENAME，当指定不存在的文件且该文件不在
                数据库实例目录下主机是否可以启动成功
Description :
    1.关闭运行正常的集群
    2.使用-l参数并指定不存在的文件，且该文件不在数据库实例目录下启动数据库
    3.查看gsctl_test.dat文件中是否有记录信息
    4.查看集群状态，主机正常启动
Expect      :
    1.关闭运行正常的集群成功
    2.使用-l参数并指定不存在的文件，且该文件不在数据库实例目录下启动数据库成功
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0018开始执行-----')
    ...
    def tearDown(self):
        ...
        LOG.info(msg)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0018执行完成---')
