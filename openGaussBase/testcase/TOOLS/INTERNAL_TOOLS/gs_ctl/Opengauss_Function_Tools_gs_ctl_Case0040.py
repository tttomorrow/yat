...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl build使用incremental增量重建备机是否可以成功
Description :
    1.gs_ctl指定-b为incremental，增量重建备机
    2.查看集群状态是否正常
Expect      :
    1.gs_ctl指定-b为incremental，增量重建备机成功
    2.集群状态正常
History     :
"""
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0040开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''
                ...
                gs_ctl build -D {self.instance_path} -b incremental ; 
                    '''
        LOG.info(excute_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(is_start)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0040执行完成---')
