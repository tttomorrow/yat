...
"""
Case Type   : 系统内部使用工具
Case Name   : 备机重建不使用-q参数重建结束后数据库是否重启
Description :
    1.不指定参数-q进行备机重建
    2.查看集群状态备机状态是否为Normal
Expect      :
    1.不指定参数-q进行备机重建成功
    2.查看集群状态成功，备机状态为Normal
History     :
"""
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0043开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''
                ...
                gs_ctl build -D {self.instance_path} ; 
                    '''
        LOG.info(excute_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(is_start)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0043执行完成---')
