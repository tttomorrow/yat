...
Description :
    1.开启事务
    2.执行事务，不做提交
    3.开启新的session使用gs_ctl restart进行重启（不指定-w）
    4.查看事务是否回滚
Expect      :
    1.开启事务成功
    2.执行事务，不做提交成功
    3.开启新的session使用gs_ctl restart进行重启成功，未等待事务
    4.查看事务，事务回滚
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0024开始执行-----')
    ...
    def tearDown(self):
        ...
        restart_msg = self.PrimaryNode.sh(restart_cmd).result()
        LOG.info(restart_msg)
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0024执行完成---')
