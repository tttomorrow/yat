...
"""
Case Type   : 系统内部使用工具
Case Name   : 单节点关闭数据库后gs_ctl start -M在备节点指定pending启动
Description :
    1.关闭正在运行的备机
    2.指定-M设置值为pending启动备机
    3.查看备机状态pending模式是否启动正常
    4.恢复备节点
Expect      :
    1.关闭正在运行的备机成功
    2.指定-M设置值为pending启动备机成功
    3.查看备机状态pending模式启动正常
    4.恢复备节点
History     :
"""
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0015开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('---------关闭正在运行的备机------------')
        is_stop = self.sh_standby.stop_db_instance()
    ...
    def tearDown(self):
        ...
        LOG.info(msg)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0015执行完成---')
