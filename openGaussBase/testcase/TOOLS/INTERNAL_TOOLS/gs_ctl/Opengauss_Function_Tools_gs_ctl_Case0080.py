...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_ctl switchover是否支持-m的值为smart
Description :
    1.指定-m设置参数为smart，在备机switchover
    2.是否有提示信息显示不支持smart参数
Expect      :
    1.指定-m设置参数为smart，在备机switchover失败
    2.有提示信息：unrecognized shutdown mode "smart"
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0080开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl switchover -D {macro.DB_INSTANCE_PATH} -m smart ;
            '''
        LOG.info(excute_cmd)
        excute_msg = self.StandbyNode.sh(excute_cmd).result()
        LOG.info(excute_msg)
        self.assertIn(self.constant.UNRECOGNIZED_SHUTDOWN_MSG, excute_msg)
    ...
    def tearDown(self):
        ...
        if 'Standby' in self.node_msg:
            ...
            return '主备节点正常'
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0080执行完成------')
