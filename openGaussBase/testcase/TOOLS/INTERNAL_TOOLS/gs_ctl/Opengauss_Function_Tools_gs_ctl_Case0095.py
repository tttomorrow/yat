...
"""
Case Type   : 系统内部使用工具
Case Name   : 主机执行gs_ctl notify使用-D指定不正确数据库实例目录是否成功
Description :
    1.以pending的方式启动主机
...
Expect      :
    ...
    3.主机指定不正确的数据库实录目录执行notify失败。提示找不到pid文件
    4.查看集群状态，主节点为pending状态
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0095开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('---------主机指定不正确的数据库实录目录执行notify------')
        query_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl notify  -D {macro.DB_BACKUP_PATH} -M primary ;
            '''
        LOG.info(query_cmd)
        query_msg = self.StandbyNode.sh(query_cmd).result()
        LOG.info(query_msg)
        self.assertIn(self.constant.NOT_EXIST, query_msg)
    ...
    def tearDown(self):
        ...
        query_msg = self.PrimaryNode.sh(query_cmd).result()
        LOG.info(query_msg)
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0095执行完成---')
