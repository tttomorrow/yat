...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_ctl failover时添加-W参数，是否显示failover开始而不显示结束
Description :
    1.关闭主数据库(主机执行)
    2.在备机上执行failover，与-W结合使用，是否只有failover开始提示无
    完成提示（备机执行）
...
Expect      :
    1.关闭主数据库成功
    2.在备机上执行failover成功,无failover完成的提示信息
    3.执行refrashconf进行信息写入
    4.重启集群成功
    5.检查主备状态，主备切换成功
History     :
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0066开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl failover -W -D {macro.DB_INSTANCE_PATH} -m fast;
            '''
        LOG.info(excute_cmd)
        excute_msg = self.StandbyNode.sh(excute_cmd).result()
        LOG.info(excute_msg)
        self.assertNotIn(self.constant.FAILOVER_SUCCESS_MSG, excute_msg)
    ...
    def tearDown(self):
        ...
        if 'Standby' in self.node_msg:
            ...
            return '主备节点正常'
        LOG.info('-----Opengauss_Function_Tools_gs_ctl_Case0066执行完成---')
