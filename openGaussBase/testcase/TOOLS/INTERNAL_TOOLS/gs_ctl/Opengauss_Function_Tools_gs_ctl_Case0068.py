...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_ctl failover时添加-t参数，并设置该参数的值为整型
Description :
    1.关闭主数据库(主机执行)
    2.在备机上执行failover，添加-t参数并设置参数的值为5
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0068开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl failover -D {macro.DB_INSTANCE_PATH} -m fast -t 5;
            '''
        LOG.info(excute_cmd)
    ...
    def tearDown(self):
        ...
        if 'Standby' in self.node_msg:
            ...
            return '主备节点正常'
        LOG.info('----Opengauss_Function_Tools_gs_ctl_Case0068执行完成----')
