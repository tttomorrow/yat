...
"""
Case Type   : 系统内部使用工具
Case Name   : 备机执行gs_ctl promote使用-D指定正确数据库实例目录是否成功
Description :
    1.创建恢复文件
    2.执行gs_ctl promote指定-D设置参数为正确的数据库实例目录(备机执行)
Expect      :
    1.创建恢复文件成功
    2.执行gs_ctl promote指定-D设置参数为正确的数据库实例目录失败
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0062开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        msg = self.user_node.sh(excute_cmd).result()
        LOG.info(msg)
        self.assertIn('server promoting', msg)
    ...
    def tearDown(self):
        ...
        LOG.info(rm_msg)
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0062执行完成------')
