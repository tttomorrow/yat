...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl restart指定-o "OPTIONS"启动是否报错
Description :
    1.关闭运行正常的集群
    2.使用-o option参数重启数据库
    3.恢复集群状态
Expect      :
    1.关闭运行正常的集群成功
    2.使用-o option参数重启数据库失败
    3.恢复集群状态成功
History     :
"""
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0036开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('-------使用-o option参数重启数据库------')
        excute_cmd = f'''
                source {self.env_path};
                gs_ctl restart -D {self.instance_path} -M primary -o option; 
                    '''
        LOG.info(excute_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(is_start)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0036执行完成---')
