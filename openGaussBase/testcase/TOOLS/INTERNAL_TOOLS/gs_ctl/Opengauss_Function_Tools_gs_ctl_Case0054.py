...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl stop指定-m的值为immediate，stop备机是否成功
Description :
    1.gs_ctl指定-m设置参数为immediate，stop备机(备机执行)
    2.查看集群状态是否正常,备机是否已关闭
Expect      :
    1.gs_ctl指定-m设置参数为immediate，stop备机成功
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0054开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''
                    ...
                    gs_ctl stop -D {self.instance_path} -m immediate;
                        '''
        LOG.info(excute_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(is_start)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0054执行完成---')
