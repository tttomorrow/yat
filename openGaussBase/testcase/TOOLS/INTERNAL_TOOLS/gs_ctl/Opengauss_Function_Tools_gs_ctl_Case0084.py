...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl failover与-T参数结合使用是否成功
Description :
    1.关闭主数据库(主机执行)
    2.在备机上执行failover，添加-T参数并设置参数的值为5（备机执行）
    3.停留两秒后执行refrashconf进行信息写入
    4.重启集群
    5.检查主备是否切换成功
Expect      :
    1.关闭主数据库成功
    2.在备机上执行failover，添加-T参数并设置参数的值为5成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0084开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl failover -D {macro.DB_INSTANCE_PATH} -T 5;
            '''
        LOG.info(excute_cmd)
    ...
    def tearDown(self):
        ...
        if 'Standby' in self.node_msg:
            ...
            return '主备节点正常'
        LOG.info('--Opengauss_Function_Tools_gs_ctl_Case0084执行完成---')
