...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl start指定-t的值足够大，启动备机
Description :
    1.关闭正在运行的备机
    2.指定-t的值足够大启动备机
    3.查看集群状态
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0005开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''
                    source {self.env_path};
                    gs_ctl start -D {self.instance_path} -M standby -t 120; 
                        '''
        LOG.info(excute_cmd)
        msg = self.user_node.sh(excute_cmd).result()
        LOG.info(msg)
    ...
    def tearDown(self):
        ...
        LOG.info(msg)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0005执行完成---')
