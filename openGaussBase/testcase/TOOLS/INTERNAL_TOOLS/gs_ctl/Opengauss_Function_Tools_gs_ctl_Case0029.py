...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl restart只使用-D指定正确数据库实例目录主机是否可以启动成功
Description :
    1.查看集群状态是否正常
    2.指定-D设置参数为正确的数据库实例目录，restart启动主机
    3.查看集群状态主机是否正常
Expect      :
    1.查看集群状态正常成功
    2.指定-D设置参数为正确的数据库实例目录，restart启动主机成功
    3.查看集群状态主机状态为正常
History     :
"""
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0029开始执行-----')
    ...
    def tearDown(self):
        ...
        LOG.info(is_start)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0029执行完成---')
