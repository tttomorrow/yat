...
"""
Case Type   : 系统内部使用工具
Case Name   : 备机执行gs_ctl query查询lsn并展示最大长度是否成功
Description :
    1.查询lsn
    2.备机执行query并指定-L参数,查询lsn并展示最大长度
Expect      :
    1.查询lsn
    2.备机执行query并指定-L参数,查询lsn并展示最大长度成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0093开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        LOG.info('-----------------查询备机状态---------------------')
        query_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl query -D {macro.DB_INSTANCE_PATH} -L {self.pv} ;
            '''
        LOG.info(query_cmd)
    ...
    def tearDown(self):
        ...
        # 无需清理环境
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0093执行完成---')
