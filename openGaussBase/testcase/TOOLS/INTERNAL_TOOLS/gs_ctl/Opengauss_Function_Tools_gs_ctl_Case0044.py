...
"""
Case Type   : 系统内部使用工具
Case Name   : 运行事务不提交，gs_ctl stop指定-W关闭数据库，事务是否回滚
Description :
    1.开启事务
    2.执行事务，不做提交
    3.开启新的session指定-W参数，关闭数据库
    4.查看事务是否回滚
Expect      :
    1.开启事务成功
    2.执行事务，不做提交成功
    3.开启新的session指定-W参数，关闭数据库成功
    4.查看事务，事务回滚
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0044开始执行-----')
    ...
    def tearDown(self):
        ...
        restart_msg = self.PrimaryNode.sh(restart_cmd).result()
        LOG.info(restart_msg)
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0044执行完成---')
