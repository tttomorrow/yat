...
"""
Case Type   : 系统内部使用工具
Case Name   : 运行事务不提交，gs_ctl restart指定-w启动数据库，事务是否回滚
Description :
    1.开启事务
    2.执行事务，不做提交
    3.开启新的session使用gs_ctl restart结合-w参数进行重启
    4.查看事务是否回滚
Expect      :
    1.开启事务成功
    2.执行事务，不做提交成功
    3.开启新的session使用gs_ctl restart结合-w参数进行重启成功，未等待事务
    4.查看事务，事务回滚
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0023开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        restart_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl restart -D {macro.DB_INSTANCE_PATH} -M primary -w;
            '''
    ...
    def tearDown(self):
        ...
        restart_msg = self.PrimaryNode.sh(restart_cmd).result()
        LOG.info(restart_msg)
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0023执行完成---')
