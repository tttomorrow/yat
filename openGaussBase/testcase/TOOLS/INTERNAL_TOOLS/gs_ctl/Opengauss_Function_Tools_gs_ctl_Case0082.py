...
"""
Case Type   : 系统内部使用工具
Case Name   : 指定用户名与密码执行switchover
Description :
    1.指定用户名与密码，在备机switchover
    2.执行refrashconf进行信息写入
    3.重启集群
    4.查看集群状态，主备是否切换成功
Expect      :
    1.指定用户名与密码，在备机switchover成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0082开始执行-----')
    ...
    def tearDown(self):
        ...
        if 'Standby' in self.node_msg:
            ...
            return '主备节点正常'
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0082执行完成---')
