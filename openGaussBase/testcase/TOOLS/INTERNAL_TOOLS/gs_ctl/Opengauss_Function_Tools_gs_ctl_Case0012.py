...
"""
Case Type   : 系统内部使用工具
Case Name   : 数据库集群关闭后gs_ctl start -M在主节点指定pending启动
Description :
    1.关闭正在运行的集群
    2.指定-M设置值为pending启动主机
    3.查看集群状态主机pending模式是否启动正常
    4.恢复主节点
    5.恢复集群状态
Expect      :
    1.关闭正在运行的集群成功
    2.指定-M设置值为pending启动主机成功
    3.查看集群状态主机pending模式启动正常
    4.恢复主节点
    5.恢复集群状态
History     :
"""
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0012开始执行-----')
    ...
    def tearDown(self):
        ...
        LOG.info(is_start)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0012执行完成---')
