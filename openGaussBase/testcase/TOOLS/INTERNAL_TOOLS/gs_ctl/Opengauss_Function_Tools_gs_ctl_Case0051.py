...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl stop指定-m的值为smart，stop备机时是否会报错
Description :
    1.查看集群状态是否正常
    2.指定-m设置参数为smart，stop备机(备机执行)
    3.查看集群状态备机是否正常
Expect      :
    1.查看集群状态正常成功
    2.指定-m设置参数为smart，stop备机失败,已不支持
    3.查看集群状态备机状态为正常
History     :
"""
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Tools_gs_ctl_Case0051开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''
                    ...
                    gs_ctl stop -D {self.instance_path} -m smart;
                        '''
        LOG.info(excute_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(is_start)
        LOG.info(
            '-----Opengauss_Function_Tools_gs_ctl_Case0051执行完成---')
