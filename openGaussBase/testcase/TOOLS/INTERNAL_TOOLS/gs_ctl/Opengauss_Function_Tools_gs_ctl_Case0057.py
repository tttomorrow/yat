...
"""
Case Type   : 系统内部使用工具
Case Name   : gs_ctl reload使用-D指定不正确数据库实例目录主机是否reload成功
Description :
    1.手动修改备机配置文件session_timeout参数的值
    2.gs_ctl reload指定-D设置参数为不正确的数据库实例目录
    3.查看该参数是否生效
Expect      :
    1.手动修改备机配置文件session_timeout参数的值成功
    2.执行gs_ctl reload指定-D设置参数为不正确的数据库实例目录失败
    3.查看该参数的值,该参数已生效
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0057开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        value_cmd = f"sed -i 's/session_timeout = {self.pv}/session_timeout" \
        ...
        excute_cmd = f'''source {macro.DB_ENV_PATH};
            gs_ctl reload -D {macro.DB_BACKUP_PATH} ;
            '''
        LOG.info(excute_cmd)
        excute_msg = self.user_node.sh(excute_cmd).result()
        LOG.info(excute_msg)
        self.assertNotIn(self.constant.gs_ctl_reload_success, excute_msg)

        LOG.info('------------------查看参数值----------------------')
        value_msg = self.sh_standby.execut_db_sql('show session_timeout;')
        LOG.info(value_msg)
        self.assertIn(self.pv, value_msg)
    ...
    def tearDown(self):
        ...
        LOG.info(msg)
        LOG.info('------Opengauss_Function_Tools_gs_ctl_Case0057执行完成---')
