...
"""
Case Type   : 系统内部使用工具
Case Name   : 执行gs_ctl failover时指定用户名和密码，failover是否成功
Description :
    1.关闭主数据库(主机执行)
    2.在备机上执行failover，并指定用户名与密码
    3.执行refrashconf进行信息写入
    4.重启集群
    5.检查主备是否切换成功
Expect      :
    1.关闭主数据库成功
    2.在备机上执行failover，并指定用户名与密码成功
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0070开始执行-----')
    ...
    def test_system_internal_tools(self):
        ...
        excute_cmd = f'''source {macro.DB_ENV_PATH};
            ...
            -U {self.StandbyNode.ssh_user} -P {self.StandbyNode.ssh_password};
            '''
        LOG.info(excute_cmd)
    ...
    def tearDown(self):
        ...
        if 'Standby' in self.node_msg:
            ...
            return '主备节点正常'
        LOG.info('----Opengauss_Function_Tools_gs_ctl_Case0070执行完成----')
