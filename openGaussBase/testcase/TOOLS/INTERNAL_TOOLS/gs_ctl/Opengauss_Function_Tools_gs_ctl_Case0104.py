...
"""
Case Type   : 系统内部使用工具
Case Name   : 主机指定SIGNALNAME PID为USR1，执行gs_ctl kill是否成功
Description :
    1.查看数据库用户PID
    2.对USR1执行gs_ctl kill
    3.查看数据库进程(可能需要稍等一会)
    4.重启数据库
Expect      :
    1.查看数据库用户的PID成功
    2.对USR1执行gs_ctl kill成功，gs_ctl kill ,datadir is (null)
    3.数据库进程被kill掉
    4.重启数据库成功
History     :
"""
...
class SystemInternalTools(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0104开始执行-----')
    ...
    def tearDown(self):
        ...
        LOG.info('---Opengauss_Function_Tools_gs_ctl_Case0104执行完成---')
