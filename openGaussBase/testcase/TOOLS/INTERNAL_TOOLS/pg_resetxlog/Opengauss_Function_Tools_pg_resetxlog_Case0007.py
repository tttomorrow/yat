...
"""
Case Type   : tools
Case Name   : 指定-e参数执行pg_resetxlog命令，设置下一个事务id，重置日志是否成功
Description :
    ...
    ll
    3.指定-e参数执行pg_resetxlog命令(参数值为长整型数值)
    pg_resetxlog -e 533689 /opt/openGauss_zl/cluster/dn1/
    4.查看执行结果，是否有提示信息
    ...
    7.进行备机重建
    gs_ctl build -D /opt/openGauss_zl/cluster/dn2 -b full
Expect      :
    1.检查数据库状态成功
    2.在对应目录下查看xlog列表成功
...
class Tools(unittest.TestCase):
    def setUp(self):
        Logger.info(
            '---Opengauss_Function_Tools_pg_resetxlog_Case0007 start---')
    ...
    def test_systools(self):
        ...
        Logger.info('----指定-e参数执行pg_resetxlog命令----')
        excute_cmd2 = f'source {self.DB_ENV_PATH};' \
                      f'pg_resetxlog -e 533689 {self.DB_INSTANCE_PATH}'
        Logger.info(excute_cmd2)
        execute_msg2 = self.userNode.sh(excute_cmd2).result()
        Logger.info(execute_msg1)
        self.assertTrue('Transaction log reset' in execute_msg2)
    ...
    def tearDown(self):
        Logger.info(
            'Opengauss_Function_Tools_pg_resetxlog_Case0007 finish')
