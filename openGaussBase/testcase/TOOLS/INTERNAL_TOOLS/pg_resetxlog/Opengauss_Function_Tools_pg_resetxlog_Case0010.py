...
"""
Case Type   : 基础功能
Case Name   : 指定-O参数执行pg_resetxlog命令，设置下一个multitransaction的偏移量是否成功,日志重置是否成功
Description :
    1.查看数据库状态，若为开启状态则关闭数据库
    2.在对应目录下查看原有xlog列表
    3.查看下一个multitransaction的偏移量
    4.指定-m参数执行pg_resetxlog命令
    5.查看multitransaction ID(Latest checkpoint's NextMultiXactId)
...
Expect      :
    1.检查数据库状态成功
    2.在对应目录下查看xlog列表成功
    3.查看下一个multitransaction的偏移量成功
    4.执行pg_resetxlog命令成功
    5.查看下一个multitransaction的偏移量为2
...
class Resetxlogclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_Tools_pg_resetxlog_Case0010 start")
    ...
    def test_resetxlog0006(self):
        ...
        text = '----step3: 查看下一个multitransaction的偏移量 expect:' \
               '查看下一个multitransaction的偏移量成功----'
        self.log.info(text)
        cmd = f"source {macro.DB_ENV_PATH};" \
            f"pg_controldata  {macro.DB_INSTANCE_PATH} | grep NextMultiOffset"
        self.log.info(cmd)
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.offset = result.split(' ')[-1]
        self.log.info(f"NextMultiOffset is {self.offset}")

        text = '----step4: 指定-O参数执行pg_resetxlog命令 expect:执行pg_resetxlog命令成功-'
        self.log.info(text)
        cmd = f"source {macro.DB_ENV_PATH};" \
            f"pg_resetxlog {macro.DB_INSTANCE_PATH} -O " \
            f"{str(int(self.offset)+2)}"
        self.log.info(cmd)
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        ...
        text = '----step5: 查看下一个multitransaction的偏移量是否为设置值 ' \
               'expect:查看下一个multitransaction的偏移量为2----'
        self.log.info(text)
        cmd = f"source {macro.DB_ENV_PATH};" \
            f"pg_controldata {macro.DB_INSTANCE_PATH} | grep NextMultiOffset"
        self.log.info(cmd)
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.assertEqual(result.split(' ')[-1], str(int(self.offset)+2),
                         '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.log.info('------------------清理环境-------------')
        if self.flg:
            self.log.info("----还原NextMultiOffset-----------")
        ...
        self.log.info("-Opengauss_Function_Tools_pg_resetxlog_Case0010 end-")
