...
class Tools(unittest.TestCase):
    def setUp(self):
        Logger.info(
            '---Opengauss_Function_Tools_pg_resetxlog_Case0002 start---')
    ...
    def test_systools(self):
        ...
        close_db_exe = f'source {self.DB_ENV_PATH};' \
                       f'gs_ctl stop -D {self.DB_INSTANCE_PATH} -M primary'
        ...
        self.assertTrue(execute_msg3 != execute_msg2)
        Logger.info('-------开启主数据库-------')
    ...
    def tearDown(self):
        Logger.info(
            'Opengauss_Function_Tools_pg_resetxlog_Case0002 finish')
