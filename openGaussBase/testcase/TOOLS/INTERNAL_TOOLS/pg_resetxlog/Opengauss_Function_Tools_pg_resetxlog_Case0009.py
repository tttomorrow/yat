...
Expect      :
    1.检查数据库状态成功
    2.在对应目录下查看原有xlog列表成功
    3.查看下一个oid成功
    4.执行pg_resetxlog命令成功，命令执行成功的提示信息为：Transaction log reset
    5.设置成功
...
class Resetxlogclass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_Tools_pg_resetxlog_Case0009 start")
    ...
    def test_resetxlog0006(self):
        ...
        text = '----step3: 查看下一个oid expect:查看下一个oid成功----'
        self.log.info(text)
        cmd = f"source {macro.DB_ENV_PATH};" \
            f"pg_controldata {macro.DB_INSTANCE_PATH} | grep NextOID"
        self.log.info(cmd)
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.next_oid = result.split(' ')[-1]
        self.log.info(f"next oid is {self.next_oid}")

        text = '----step4: 指定-o参数执行pg_resetxlog命令 expect:执行pg_resetxlog命令成功-'
        self.log.info(text)
        cmd = f"source {macro.DB_ENV_PATH};" \
            f"pg_resetxlog {macro.DB_INSTANCE_PATH} " \
            f"-o {str(int(self.next_oid)+1)}"
        self.log.info(cmd)
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.assertIn("Transaction log reset", result, '执行失败:' + text)

        text = '----step5: 查看下一个oid是否为设置值 expect:设置成功----'
        self.log.info(text)
        cmd = f"source {macro.DB_ENV_PATH};" \
            f"pg_controldata {macro.DB_INSTANCE_PATH} | grep NextOID"
        self.log.info(cmd)
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.new_next_oid = result.split(' ')[-1]
        self.assertGreater(self.new_next_oid, self.next_oid, '执行失败:' + text)
        ...
        text = '--step8: 在对应目录下查看现有xlog列表，是否重置成功 ' \
               'expect:对应目录下查看xlog列表成功，日志重置成功--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_Tools_pg_resetxlog_Case0009 end-")
