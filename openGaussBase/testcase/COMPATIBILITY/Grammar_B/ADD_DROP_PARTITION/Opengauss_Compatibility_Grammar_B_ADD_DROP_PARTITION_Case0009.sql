...
create index on b_interval_t1 (c1) global;
create index on b_interval_t1 (c2) local;
insert into b_interval_t1 values
...
alter table b_interval_t1 add partition (partition pa1 values less than ('1990-02-01 00:00:00'));
alter table b_interval_t1 add partition (partition pa1 values less than ('1990-02-01 00:00:00'),partition pa2 values less than ('1990-02-02 00:00:00'));
-- @testpoint: 原语法 add partition 合理报错，间隔分区表不支持手动添加分区
alter table b_interval_t1 add partition pa1 values less than ('1990-02-01 00:00:00');
alter table b_interval_t1 add partition pa1 values less than ('1990-02-01 00:00:00'),add partition pa2 values less than ('1990-02-02 00:00:00');
drop table b_interval_t1;
