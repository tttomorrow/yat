...
drop table if exists b_interval_t3;
-- segment on
create table b_interval_t3(c1 int primary key,c2 timestamp)
with (segment=on)
...
create index on b_interval_t3 (c1) global;
create index on b_interval_t3 (c2) local;
insert into b_interval_t3 values
...
alter table b_interval_t3 add partition (partition pa1 values less than ('1990-02-01 00:00:00'));
alter table b_interval_t3 add partition (partition pa1 values less than ('1990-02-01 00:00:00'),partition pa2 values less than ('1990-02-02 00:00:00'));
-- @testpoint: 原语法 add partition 合理报错，间隔分区表不支持手动添加分区
alter table b_interval_t3 add partition pa1 values less than ('1990-02-01 00:00:00');
alter table b_interval_t3 add partition pa1 values less than ('1990-02-01 00:00:00'),add partition pa2 values less than ('1990-02-02 00:00:00');
drop table b_interval_t3;
