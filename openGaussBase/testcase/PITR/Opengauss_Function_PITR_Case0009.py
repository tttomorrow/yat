...
"""
Case Type   : 基础功能
Case Name   : recovery_target_inclusive为false时，设置recovery_target_xid恢复
Description :
    1、主节点开启归档
    2、创建表
    3、备份
...
Expect      :
    ...
    6、成功
    7、成功
    8、recovery.conf文件变成revocery.done,且数据正确
History     :
...
class Pitrclass(unittest.TestCase):
    ...
    def test_pitr(self):
        ...
        text = '----step8: 查询数据库恢复情况 ' \
               'expect:recovery.conf文件变成revocery.done,且数据正确；----'
        self.log.info(text)
        cmd = f"select * from {self.tbname} order by i;"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        ...
        self.assertNotIn('transcation test begin...', result, '执行失败:' + text)
        self.assertNotIn('transcation test commit...', result, '执行失败:' + text)
