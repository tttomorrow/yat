...
"""
Case Type   : 基础功能
Case Name   : 使用归档文件恢复到recovery_target_lsn节点
Description :
    1.主节点开启归档
    2.创建表
    3.备份
...
class Pitrclass(unittest.TestCase):
    ...
    def test_pitr(self):
        ...
        text = '----step4: 再次插入数据，查询当前lsn expect:成功----'
        self.log.info(text)
        ...
        text = '----step6: 恢复数据到step4中查询的lsn节点 expect:成功----'
        self.log.info(text)
        ...
        cmd = f"select count(*) from {self.tbname}_01;" \
            ...
            f"select count(*) from {self.tbname}_03;"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        ...
        text = '----step10: tab3插入数据 expect:成功----'
        self.log.info(text)
        ...
        time.sleep(3)
        cmd = f"insert into {self.tbname}_03 values(1);"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
    ...
    def tearDown(self):
        ...
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, result1, '执行失败'+text)
