...
"""
Case Type   : 基础功能
Case Name   : recovery文件中恢复时间点不存在
Description :
    1.主节点开启归档
    2.创建表
    3.备份
    4.再次插入数据，并创建laber_name
    5.破坏数据库
    6.恢复数据
    7.重启数据库
    8.查询数据库恢复情况
...
class Pitrclass(unittest.TestCase):
    ...
    def test_pitr(self):
        ...
        text = '----step4: 再次插入数据，并创建laber_name expect:成功----'
        self.log.info(text)
        cmd = f"create table {self.tbname}_02(i int);" \
            ...
            f"create table {self.tbname}_03(i int);" \
        ...
        self.assertIn(f"{self.tbname}_03", result, '执行失败:' + text)
        cmd = f"select * from {self.tbname}_01;" \
            f"select * from {self.tbname}_02;" \
            f"select * from {self.tbname}_03;"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        ...
        time.sleep(3)
        cmd = f"INSERT INTO {self.tbname}_03 VALUES (11);"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
