...
class Pitrclass(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info("--------Opengauss_Function_PITR_Case0004 start-------")
    ...
    def test_backup0026(self):
        ...
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        self.assertEqual(result.count('3'), 3, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.log.info("-Opengauss_Function_PITR_Case0004 end-")
