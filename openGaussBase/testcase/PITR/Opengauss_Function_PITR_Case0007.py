...
"""
Case Type   : 基础功能
Case Name   : 多事务并行时，xid大的先提交时，恢复至xid大的节点
Description :
    1.主节点开启归档
    2.创建表
    3.备份
...
class Pitrclass(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info("--------Opengauss_Function_PITR_Case0007 start-------")
    ...
    def test_backup0026(self):
        ...
        self.xid = result.splitlines()[4].split('|')[1].strip()
        ...
        self.assertIn('session2 inserting...', result, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.log.info("-Opengauss_Function_PITR_Case0007 end-")
