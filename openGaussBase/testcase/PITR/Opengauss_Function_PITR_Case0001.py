...
class Pitrclass(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        result1 = self.commonshpri.stop_db_cluster()
        result2 = self.commonshpri.start_db_cluster()
        ...
        result = self.primary_user_node.sh(cmd).result()
        self.log.info(result)
        self.assertTrue(result1 and result2, '执行失败'+text)
