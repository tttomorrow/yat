...
"""
Case Type   : GSC功能模块
Case Name   : enable_global_syscache参数设置为off，对应系统函数、视图查询功能验证
Description :
    0、查看enable_global_syscache初始配置值;
    1、修改参数enable_global_syscache为off;
    2、重启数据库，使参数生效;
    3、查询gsc相关系统函数、视图;
...
Expect      :
    0、查看enable_global_syscache初始配置值;
    1、修改参数enable_global_syscache为off; 修改成功
    2、重启数据库，使参数生效; 重启成功
    3、查询gsc相关系统函数、视图;
    3.1、查询gs_gsc_memory_detail; 查询结果为空
    3.2、查询gs_lsc_memory_detail; 查询结果为空
    3.3、查询gs_gsc_dbstat_info; 查询结果为空
    3.4、查询gs_gsc_table_detail; 查询结果为空
    3.5、查询gs_gsc_catalog_detail; 查询结果为空
    3.6、查询gs_gsc_clean; 查询结果为f
History     :
"""
import os
import unittest
...
class GscTestCase(unittest.TestCase):
    ...
    def test_main(self):
        step_txt = '----step1:修改参数enable_global_syscache为off; expect:修改成功----'
        self.log.info(step_txt)
        msg = self.sh.execute_gsguc('set',
                                    self.constant.GSGUC_SUCCESS_MSG,
                                    f"enable_global_syscache= off")
        ...
        self.log.info(step_txt)
        step_txt = '----step3.1:查询gs_gsc_memory_detail; expect:查询结果为空----'
        self.log.info(step_txt)
        select_result = self.check_select('gs_gsc_memory_detail')
        self.assertEqual(select_result, 0, '执行失败:' + step_txt)

        step_txt = '----step3.2:查询gs_lsc_memory_detail; expect:查询结果为空----'
        self.log.info(step_txt)
        select_result = self.check_select('gs_lsc_memory_detail')
        self.assertEqual(select_result, 0, '执行失败:' + step_txt)

        step_txt = '----step3.3:查询gs_gsc_dbstat_info; expect:查询结果为空----'
        self.log.info(step_txt)
        select_result = self.check_select('gs_gsc_dbstat_info()')
        self.assertEqual(select_result, 0, '执行失败:' + step_txt)

        step_txt = '----step3.4:查询gs_gsc_table_detail; expect:查询结果为空----'
        self.log.info(step_txt)
        select_result = self.check_select('gs_gsc_table_detail()')
        self.assertEqual(select_result, 0, '执行失败:' + step_txt)

        step_txt = '----step3.5:查询gs_gsc_catalog_detail; expect:查询结果为空----'
        self.log.info(step_txt)
        select_result = self.check_select('gs_gsc_catalog_detail()')
        self.assertEqual(select_result, 0, '执行失败:' + step_txt)

        step_txt = '----step3.6:查询gs_gsc_clean; expect:查询结果为f----'
        self.log.info(step_txt)
        select_result = self.check_select('gs_gsc_clean()', False)
        self.assertEqual(select_result.splitlines()[-2].strip().lower(), 'f',
                         '执行失败:' + step_txt)
    ...
    def tearDown(self):
        ...
        self.log.info(step_txt)
        step1_txt = '----还原参数enable_global_syscache为初始值; ' \
                    'expect:修改成功----'
