...
"""
Case Type   : GSC功能模块
Case Name   : 非线程池模式场景下，global_syscache_threshold内存阈值修改验证，已占用大于修改值
Description :
    ...
    7、并发对5个库所有表进行查询;
    8、查看syscache占用情况;
    9、修改global_syscache_threshold为当前占用空间的一半
    10、查看syscache占用情况;
Expect      :
    ...
    9、修改global_syscache_threshold为当前占用空间的一半，设置成功
    10、查看syscache占用情况; 比步骤8占用减少,符合设置阈值范围
History     :
"""
import os
import time
...
class GscTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.table_num = 1000
        self.db_num = 5
        self.fun_create_name = 'fun_create_tb_gsc0013'
        self.fun_drop_name = 'fun_drop_tb_gsc0013'
        self.db_ex_name = 'db_gsc0013_'
        self.tb_ex_name = 'tb_gsc0013_'
    ...
    def test_main(self):
        ...
        self.log.info(step_txt)
        self.init_para3 = self.com.show_param('global_syscache_threshold')
        ...
        step_txt = f'----step9:修改global_syscache_threshold为' \
            f'当前占用空间的一半; expect:成功----'
        self.log.info(step_txt)
        msg = self.sh.execute_gsguc('reload',
                                    self.constant.GSGUC_SUCCESS_MSG,
                                    f"global_syscache_threshold= "
                                    f"{int(total_size1 / 1024 / 2)}")
        self.assertTrue(msg, '执行失败:' + step_txt)
        ...
        self.assertLess(total_size2, total_size1, '执行失败:' + step_txt)
    ...
    def tearDown(self):
        ...
        msg2 = self.sh.execute_gsguc('reload',
                                     ...
                                     f"{self.init_para2}")
        msg3 = self.sh.execute_gsguc('reload',
                                     self.constant.GSGUC_SUCCESS_MSG,
                                     f"global_syscache_threshold="
                                     f"{self.init_para3}")
