...
"""
Case Type   : GSC功能模块
Case Name   : gsc打开，某个库中某张表未进行缓存/已缓场景下，验证该库相应系统缓存信息变化
Description :
...
Expect      :
    ...
    4、创建10个表; 创建成功
    5、查询所有库并进行连接; 连接成功
    6、表未缓存时，查看syscache占用情况; 系统缓存增加
    7、表已缓存时，查看syscache占用情况; 系统缓存不增加
History     :
"""
import os
import unittest
...
class GscTestCase(unittest.TestCase):
    ...
    def test_main(self):
        ...
        step_txt = '----step6:表未缓存时，查看syscache占用情况; expect:系统缓存增加----'
        self.log.info(step_txt)
        gsc_size2 = 0
        for i in range(self.tb_num):
        ...
        step_txt = '----step7:表已缓存时，查看syscache占用情况; expect:系统缓存不增加----'
        self.log.info(step_txt)
        for i in range(self.tb_num):
            check_sql = f"select * from {self.tb_ex_name}{i};" \
                f"select * from gs_gsc_memory_detail;" \
