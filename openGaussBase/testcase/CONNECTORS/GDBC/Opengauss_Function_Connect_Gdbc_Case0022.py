...
class ConnGO22(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，聚合查询 expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f"{clean_sql}" \
            ...
            f"insert into {self.t_name} values(1, 'red'), (2, 'red'), " \
            f"(3, 'yello'), (4, 'blue'), (5, 'blue');"
        query_sql = f"select sum(a) from {self.t_name};\n" \
            f"select max(a) from {self.t_name};\n" \
            f"select min(a) from {self.t_name};\n" \
            f"select first(a) from {self.t_name};\n" \
            f"select last(a) from {self.t_name};\n" \
            f"select count(*) from {self.t_name};"
        query_sql = ','.join([f'"{i}"' for i in query_sql.splitlines()])
        query_sql1 = self.go.query(
            f"select count(b) as num, b from {self.t_name} "
            f"group by b order by num desc;",
            **{"num1": "int", "res1": "string"})
        query_sql2 = self.go.query(
            f"select count(b) as num, b from {self.t_name} "
            f"group by b having num>1;",
            False,
            **{"num2": "int", "res2": "string"})
        query_sql3 = self.go.query_row(f"select avg(a) from {self.t_name};",
                                       **{"avg_res": "float64"})
