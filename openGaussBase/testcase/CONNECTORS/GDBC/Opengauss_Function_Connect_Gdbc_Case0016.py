...
"""
Case Type   : go驱动
Case Name   : 连接数据库，copy
Description :
    1.新建用户并赋权
    2.配置pg_hba入口, sha256认证方法
    3.配置enable_copy_server_files参数值为on，允许使用copy to/from
    4.连接数据库，执行copy to/copy from
    5.还原pg_hba文件
...
class ConnGO16(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_16'
        self.go_file = 'go_conn.go'
        self.t_name = 't_go_16'
        self.copy_path = os.path.join(macro.DB_BACKUP_PATH, 'go_copy.dat')
    ...
    def test_1(self):
        ...
        text = '----step3: 配置enable_copy_server_files参数值为on，' \
        ...
        text = '----step4: ，执行copy to/copy from expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            f'create table {self.t_name}(a int,b int,c text);' \
            f'insert into {self.t_name} values(generate_series(1,10000),' \
            f'generate_series(1,10000),\'c-\'||generate_series(1,5));' \
            f'copy {self.t_name} to \'{self.copy_path}\';' \
            f'copy {self.t_name} from \'{self.copy_path}\';'
        query_sql = self.go.query(
        ...
        self.assertIn(' 10000}\n20000', res, f'执行失败: {text}')

    def tearDown(self):
        text = '----run teardown----'
        self.log.info(text)

        s5_text = '----step5: 还原pg_hba文件  expect: 成功----'
        ...
        s7_text = '----step7: 删除用户  expect: 成功----'
        self.log.info(s7_text)
        sql = f'drop user if exists {self.db_user} cascade;'
        sql_res = self.pri_sh.execut_db_sql(sql)
        self.log.info(sql_res)

        s8_text = '----step8: 刪除go_conn.go、备份文件 expect: 成功----'
        self.log.info(s8_text)
        rm_cmd = f'rm -rf {self.go_file} {self.copy_path}'
        self.log.info(rm_cmd)
        rm_res = self.pri_root.sh(rm_cmd).result()
        self.log.info(rm_res)

        self.assertEqual(len(cmd_res), 0, f'执行失败: {s5_text}')
        self.assertTrue(is_restart, f'执行失败: {s5_text}')
        self.assertTrue(guc_res, f'执行失败: {s6_text}')
        self.assertIn(self.cons.DROP_ROLE_SUCCESS_MSG,
                      sql_res,
                      f'执行失败: {s7_text}')
        self.assertIs(rm_res, '', f'执行失败: {s8_text}')
