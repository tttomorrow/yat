...
class ConnGO27(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_27'
        self.go_file = 'go_conn.go'
        self.new_user = 'u_go_27'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义用户&授权 expect: 成功----'
        self.log.info(text)
        ...
        new_conn_str = f'newConnStr := "host={self.pri_user.db_host} ' \
            f'port={self.pri_user.db_port} ' \
            f'user={self.new_user} ' \
            f'password={macro.COMMON_PASSWD} ' \
            f'dbname={self.pri_user.db_name} ' \
            f'sslmode=disable"'

        clean_sql = f"drop user if exists {self.new_user} cascade;"
        s_sql = f"{clean_sql}" \
            f"create user {self.new_user} password '{macro.COMMON_PASSWD}';" \
            f"alter user {self.new_user} createrole;" \
            f"grant connect on database {self.pri_user.db_name} " \
            f"to {self.new_user};"
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    {new_conn_str}\n' \
            f'    new_conn, err := sql.Open("opengauss", newConnStr)\n' \
            f'    if err != nil {{\n' \
            f'        panic(err)\n' \
            f'    }}\n\n' \
            f'    new_conn.Close()' \
        ...
        self.assertEqual(res.count(' 0}'), 2, f'执行失败: {text}')
