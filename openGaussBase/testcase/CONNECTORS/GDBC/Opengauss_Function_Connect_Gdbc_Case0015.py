...
class ConnGO15(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，upsert expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            f'create table {self.t_name}(a int,b int primary key,c text);' \
            f'insert into {self.t_name} values(generate_series(1,5),' \
            f'generate_series(1,5),\'c-\'||generate_series(1,5));'
        query_sql1 = self.go.query(
            f"explain (costs off, verbose) insert into {self.t_name} "
            f"values(1,1,3) on duplicate key update c='new-c';",
            **{"q_plan ": "string"})
        s_sql1 = f'insert into {self.t_name} values(1,1,3) ' \
            f'on duplicate key update c=\'new-c\';'
        query_sql2 = self.go.query_row(
            f'select * from {self.t_name} where b=1;',
        ...
        pattern = f'Conflict Resolution: UPDATE.*' \
            f'Conflict Arbiter Indexes: {self.t_name}_pkey.*' \
            f'Output: 1, 1, \'3\'::text.*1 1 new-c'
        reg_res = re.search(pattern, res, re.S)
        self.assertIsNotNone(reg_res, f'执行失败: {text}')
