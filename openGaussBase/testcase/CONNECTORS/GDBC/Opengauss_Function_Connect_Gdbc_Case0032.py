...
class ConnGO32(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_32'
        self.go_file = 'go_conn.go'
        self.db_name = 'db_go_32'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义数据库 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop database if exists {self.db_name}_new;"
        s_sql = f'drop database if exists {self.db_name};'
        s_sql1 = f"create database {self.db_name};"
        s_sql2 = f"alter database {self.db_name} rename to " \
            f"{self.db_name}_new;"
