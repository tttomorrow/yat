...
class ConnGO23(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，select..into expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop table if exists {self.t_name} cascade;" \
            f"drop table if exists {self.t_name}_new cascade;"
        s_sql = f"{clean_sql}" \
            f"create table {self.t_name}(a int, b text);" \
            f"insert into {self.t_name} values(1, 'red'), (2, 'red'), " \
            f"(3, 'yello'), (4, 'blue'), (5, 'blue');"
        query_sql = f"select * into {self.t_name}_new from " \
            f"{self.t_name} where b='red';" \
            f"select count(*) from {self.t_name}_new;"
