...
"""
Case Type   : go驱动
Case Name   : 连接数据库，insert..return
Description :
    1.新建用户并赋权
    2.配置pg_hba入口, sha256认证方法
    3.连接数据库，insert..return
    4.还原pg_hba文件
    5.删除用户
    6.刪除go_conn.go
Expect      :
...
class ConnGO11(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，insert..return expect: 成功----'
        self.log.info(text)
        ...
        query_sql1 = self.go.query_row(
            f'insert into {self.t_name} values (2,\'小明\') returning *;',
            **{"id ": "int", "name": "string"})
        query_sql2 = self.go.query_row(
            f'insert into {self.t_name} values (3,\'小明\') returning id;',
            **{"id1 ": "int"})
        query_sql3 = self.go.query_row(
            f'insert into {self.t_name} values (4,\'小明\') returning id,name;',
            **{"id2 ": "int", "name2": "string"})
