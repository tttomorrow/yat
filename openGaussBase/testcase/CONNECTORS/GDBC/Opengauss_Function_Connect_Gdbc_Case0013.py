...
class ConnGO13(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_13'
        self.go_file = 'go_conn.go'
        self.schema_name = 'schema_go_13'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，set expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            ...
            f'set session current_schema to {self.schema_name};'
        select_sql1 = "select current_schema;"
        select_sql2 = "select setting from pg_settings where " \
                      "name = 'current_schema';"
        select_sql3 = "set session current_schema to default; " \
        ...
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    q_ary := [...]string{{"{select_sql1}", ' \
            f'"{select_sql2}", "{select_sql3}"}}\n\n' \
        ...
        self.assertEqual(res.count(self.schema_name), 2, f'执行失败: {text}')
