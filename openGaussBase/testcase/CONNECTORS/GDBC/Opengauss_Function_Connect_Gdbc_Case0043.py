...
class ConnGO43(unittest.TestCase):
    def setUp(self):
        ...
        self.proc_name = 'proc_go_43'
        self.pkg_name = 'pkg_go_43'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义包 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop table if exists {self.table_name};" \
            ...
            f"drop package if exists {self.pkg_name};"
        # 创建包头
        s_sql = f"{clean_sql}" \
            f"create or replace package {self.pkg_name} is " \
            f"var1 int:=1;" \
            f"var2 int:=2;" \
            f"procedure {self.proc_name}(var3 int);" \
            f"end {self.pkg_name};"
        # 创建包体
        s_sql1 = f'q_str := "drop table if exists {self.table_name};\\n"\n' \
            f'    q_str += "create or replace package body {self.pkg_name} ' \
            f'is\\n"\n' \
            f'    q_str += "    var4 int:=4;\\n"\n' \
            f'    q_str += "    procedure {self.proc_name}(var3 int)\\n"\n' \
            f'    q_str += "    is\\n"\n' \
            f'    q_str += "    begin\\n"\n' \
            f'    q_str += "        create table if not exists ' \
            f'{self.table_name}(col1 int);\\n"\n' \
            f'    q_str += "        insert into {self.table_name} values(' \
            f'var1);\\n"\n' \
            f'    q_str += "        insert into {self.table_name} values(' \
            f'var3);\\n"\n' \
            f'    q_str += "    end;\\n"\n' \
            f'    q_str += "begin\\n"\n' \
            f'    q_str += "    var4:=9;\\n"\n' \
            f'    q_str += "    {self.proc_name}(var4);\\n"\n' \
            f'    q_str += "end {self.pkg_name};"'
        # 调用包定义函数
        s_sql2 = f"call {self.pkg_name}.{self.proc_name}(1);"
        # 查询
        s_sql3 = f"select * from {self.table_name};"
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    {s_sql1}\n' \
            f'    fmt.Println(q_str)\n\n' \
            f'    res, err = db.Exec(q_str)\n' \
            f'    if err != nil {{\n' \
            f'        panic(err)\n' \
            f'    }}\n' \
            f'    fmt.Println(res)\n\n' \
            f'    {self.go.query(s_sql2, **{"result": "string"})}\n\n' \
            f'    {self.go.query(s_sql3, False, **{"result1": "int"})}\n' \
            f'}}'
