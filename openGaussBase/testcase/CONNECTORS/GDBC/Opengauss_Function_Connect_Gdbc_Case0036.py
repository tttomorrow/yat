...
"""
Case Type   : go驱动
Case Name   : 连接数据库，定义表，表数据不写入预写日志
Description :
    1.新建用户并赋权
    2.配置pg_hba入口, sha256认证方法
    3.连接数据库，定义表，表数据不写入预写日志
    4.还原pg_hba文件
    5.删除用户
    6.刪除go_conn.go
Expect      :
...
class ConnGO36(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义表，表数据不写入预写日志 expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            f'create unlogged table {self.t_name}(' \
            f'w_warehouse_sk      integer            not null,' \
            f'w_warehouse_name    varchar(20),' \
            f'w_gmt_offset        decimal(5,2)' \
            f');'
        s_sql1 = f"insert into {self.t_name} values (101, 'nw', 10.1);"
        s_sql2 = f"select * from {self.t_name};"
        param_dict = {"col1": "int", "col2": "string", "col3": "float64"}
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    {self.go.query(s_sql2, **param_dict)}\n' \
            f'}}'
        ...
        self.assertIn('101 nw 10.1', res, f'执行失败: {text}')
