...
class ConnGO38(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义列存表 expect: 成功----'
        self.log.info(text)
        ...
        s_sql1 = f"create table {self.t_name}_2(" \
            ...
            f" w_warehouse_name varchar(20)," \
            f" w_gmt_offset     decimal(5,2)," \
            f" partial cluster key(w_warehouse_sk, w_warehouse_id)" \
            f") with (orientation = column);"
        # 创建一个带压缩的列存表
