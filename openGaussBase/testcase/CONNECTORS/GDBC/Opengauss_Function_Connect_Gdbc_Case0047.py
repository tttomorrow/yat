...
class ConnGO47(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_47'
        self.go_file = 'go_conn.go'
        self.table_name = 't_go_47'
        self.type_name = 'type_go_47'
        self.func_name = 'func_go_47'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义数据类型转换 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop table if exists {self.table_name};" \
            f"drop cast if exists (text as {self.type_name});" \
            f"drop function if exists {self.func_name};" \
            f"drop type if exists {self.type_name};"
        # 创建自定义类型
        s_sql = f"drop type if exists {self.type_name};" \
            f"create type {self.type_name} as (length int, content text);"
        # 创建转换函数
        s_sql1 = f"drop function if exists {self.func_name};" \
            f"create or replace function {self.func_name} (text) " \
            f"returns {self.type_name} as \\$\\$" \
            f"    select char_length(\\$1), \\$1;" \
            f"\\$\\$ language sql strict;"
        # 定义类型转换
        s_sql2 = f"drop cast if exists (text as {self.type_name});" \
            f"create cast (text as {self.type_name}) with function" \
            f" {self.func_name}(text) as implicit;"
        # 使用
        s_sql3 = f"drop table if exists {self.table_name};" \
            f"create table {self.table_name} (a {self.type_name});" \
            f"insert into {self.table_name} values ('amy'::text);"
        ...
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    {self.go.query(query_sql, **{"result": "string"})}\n' \
            f'}}'
        ...
        self.assertIn("(3,amy)", res, f'执行失败: {text}')
