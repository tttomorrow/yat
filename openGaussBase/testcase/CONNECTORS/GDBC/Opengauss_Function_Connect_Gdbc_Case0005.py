...
class ConnGO5(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，用户名错误，方式一 expect: 成功----'
        self.log.info(text)
        ...
        cmd = cmd.replace('{', '{{')
        cmd = cmd.replace('}', '}}')
        cmd = cmd.replace(self.db_user, 'er_user')
        self.log.info(cmd)
        res = self.pri_root.sh(cmd).result()
        self.log.info(res)
        self.assertIn('panic: failed to connect', res, f'执行失败: {text}')

        text = '----step3: 连接数据库，密码错误,方式二 expect: 成功----'
        self.log.info(text)
        ...
        cmd = cmd.replace(macro.COMMON_PASSWD, 'er_pwd')
        self.log.info(cmd)
        res = self.pri_root.sh(cmd).result()
        self.log.info(res)
        self.assertIn('panic: failed to connect', res, f'执行失败: {text}')

        text = '----step3: 连接数据库，端口错误,方式三 expect: 成功----'
        self.log.info(text)
        ...
        cmd = cmd.replace(self.pri_user.db_port, '11111')
        self.log.info(cmd)
        res = self.pri_root.sh(cmd).result()
        self.log.info(res)
        self.assertIn('panic: failed to connect', res, f'执行失败: {text}')
