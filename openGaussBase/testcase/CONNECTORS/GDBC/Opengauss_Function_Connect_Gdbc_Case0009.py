...
class ConnGO9(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，多表join expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop table if exists {self.t_name}_1;" \
            f"drop table if exists {self.t_name}_2;"
        s_sql = f'{clean_sql}' \
            f'create table {self.t_name}_1(id int,name varchar(10));' \
            f'create table {self.t_name}_2(id int,name varchar(10));' \
            f'insert into {self.t_name}_1 values(1, \'apple\'), ' \
            f'(3, \'orange\'), (4, \'grape\');' \
            f'insert into t_go_9_2 values(1, \'a\'), (2, \'b\'), (3, \'p\');'

        join_sql1 = f"select a.*,  b.* from {self.t_name}_1 a, " \
            f"{self.t_name}_2 b where a.id=b.id;"
        join_sql2 = f"select a.*,  b.* from {self.t_name}_1 a " \
            f"left join {self.t_name}_2 b on a.id=b.id;"
        join_sql3 = f"select a.*,  b.* from {self.t_name}_1 a " \
            f"right join {self.t_name}_2 b on a.id=b.id;"
        join_sql4 = f"select a.*,  b.* from {self.t_name}_1 a " \
            f"cross join {self.t_name}_2 b;"
        join_sql5 = f"select a.*,  b.* from {self.t_name}_1 a " \
            f"inner join {self.t_name}_2 b on a.id=b.id;"
        join_sql6 = f"select a.*,  b.* from {self.t_name}_1 a " \
            f"full join {self.t_name}_2 b on a.id=b.id;"
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    join_sql := [...]string{{"{join_sql1}", "{join_sql2}", ' \
            f'"{join_sql3}", "{join_sql4}", "{join_sql5}", ' \
            f'"{join_sql6}"}}\n\n' \
            f'    for _, v := range join_sql {{\n' \
            f'        fmt.Println(v)\n' \
            f'        var id_a, name_a, id_b, name_b string\n' \
            f'        err = db.QueryRow(v).Scan(&id_a, &name_a, ' \
            f'&id_b, &name_b)\n' \
            f'        if err != nil {{\n' \
            f'            panic(err)\n' \
            f'        }}\n' \
            f'        fmt.Println(id_a, name_a, id_b, name_b)\n' \
            f'    }}\n' \
            f'}}'
        ...
        self.assertEqual(res.count('1 apple 1 a'), 6, f'执行失败: {text}')
