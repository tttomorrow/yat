...
class ConnGO26(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，列分区表 expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f"{clean_sql}" \
            f"create table {self.t_name} (c1 dec(4,2),c2 int) " \
            f"with (orientation=column, compression=no) " \
            f"partition by range (c1) " \
            f"( " \
            f"    partition p1 values less than(10)," \
            f"    partition p2 values less than(20)," \
            f"    partition p3 values less than(30)," \
            f"    partition p4 values less than(maxvalue));" \
            f"insert into {self.t_name} values (11.11,1);"
        query_sql = f"select * from {self.t_name};"
        param_dict = {"a": "float64", "b": "int"}
