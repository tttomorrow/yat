...
class ConnGO10(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_10'
        self.go_file = 'go_conn.go'
        self.t_name = 't_go_10'
        self.index_name = 'i_go_10'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，hint expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            f'create table {self.t_name}(id int,name varchar(10));' \
            f'create index {self.index_name} on {self.t_name}(id);' \
            f'insert into {self.t_name} values(generate_series(1, 100000), ' \
            f'generate_series(1, 100000)||\'test\');'
        query_sql = self.go.query(
            f'explain select /*+ indexscan({self.t_name} {self.index_name})'
            f' */ id, name from {self.t_name} where id > 100;',
            **{"q_plan ": "string"})
        ...
        pattern = f'[Bypass].*Index Scan using {self.index_name}' \
            f' on {self.t_name}'
