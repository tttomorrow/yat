...
class ConnGO46(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_46'
        self.go_file = 'go_conn.go'
        self.t_name = 't_go_46'
        self.func_name = 'agg_func_go_46'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义聚合函数 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop aggregate if exists {self.func_name} " \
            f"(anyelement);" \
            f"drop table if exists {self.t_name} cascade;"
        s_sql = f"{clean_sql}" \
            f"create table {self.t_name}(id int, name varchar(20));" \
            f"insert into {self.t_name} values (generate_series(1,100)," \
            f" generate_series(1,100)||'test');"
        # 定义聚合函数
        s_sql1 = f"create aggregate {self.func_name} (anyelement) " \
            f"(" \
            f"sfunc = array_append," \
            f"stype = anyarray," \
            f"initcond = '{{}}'" \
            f");"
        # 使用聚合函数
        query_sql = f"select {self.func_name}(id)::text from {self.t_name};"
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    {self.go.query(query_sql, **{"id": "string"})}\n' \
            f'}}'
        ...
        expect = [str(i) for i in range(1, 101)]
        self.assertIn(f"{{{','.join(expect)}}}", res, f'执行失败: {text}')
