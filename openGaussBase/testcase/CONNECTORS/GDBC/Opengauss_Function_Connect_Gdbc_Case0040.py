...
class ConnGO40(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_40'
        self.go_file = 'go_conn.go'
        self.index_name = 'i_go_40'
        self.table_name = 't_go_40'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义索引 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop index if exists new_{self.index_name};" \
            f"drop index if exists {self.index_name}_2;" \
            f"drop index if exists {self.index_name}_3;" \
            f"drop index if exists {self.index_name}_4;" \
            f"drop index if exists {self.index_name}_6;" \
            f"drop index if exists {self.index_name}_7;" \
            f"drop table if exists {self.table_name}_1 cascade;" \
            f"drop table if exists {self.table_name}_2 cascade;"
        s_sql = f"{clean_sql}" \
            f"create table {self.table_name}_1(" \
            f" t_id integer(20)," \
            f" t_name varchar(20) not null default ''" \
            f");"
        sql_list = list()
        sql_list.append(f"create table {self.table_name}_2(t_id integer(20),"
                        f"t_name varchar(20) not null default '') "
        ...
        # 创建普通的唯一索引
        sql_list.append(f"create unique index {self.index_name}_1 on "
                        f"{self.table_name}_1(t_id);")
        # 创建指定B-tree索引
        sql_list.append(f"create index {self.index_name}_2 on "
                        f"{self.table_name}_1 using btree(t_id);")
        # 创建hash索引
        sql_list.append(f"create index {self.index_name}_3 on "
                        f"{self.table_name}_1 using hash(t_id);")
        # 创建表达式索引
        sql_list.append(f"create index {self.index_name}_4 on "
                        f"{self.table_name}_1(substr(t_name, 1, 4));")
        # 创建global全局索引
        sql_list.append(f"create index {self.index_name}_5 on "
                        f"{self.table_name}_2(t_id) global;")
        # 删除global全局索引，全局分区索引和本地分区索引不应位于同一列;创建local全局索引
        sql_list.append(f"drop index if exists {self.index_name}_5;"
                        f"create index {self.index_name}_6 on "
                        f"{self.table_name}_2(t_id) local (partition p1,"
                        f"partition p2,partition p3,partition p4);")
        # 创建gin索引
        sql_list.append(f"create index {self.index_name}_7 on "
                        f"{self.table_name}_1 using gin(to_tsvector("
                        f"'english', t_name));")
        # 重建索引
        sql_list.append(f"reindex index {self.index_name}_1;")
        # 重命名索引
        sql_list.append(f"alter index if exists {self.index_name}_1 rename"
                        f" to new_{self.index_name};")
        ...
        self.assertEqual(res.count(' 0}'), 12, f'执行失败: {text}')
