...
class ConnGO20(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，普通查询 expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            ...
            f'(2, \'test2\'), (3, \'test3\');'
        query_sql = f"select * from {self.t_name};\n" \
            f"select a, b from {self.t_name};\n" \
            f"select a, b as c from {self.t_name};\n" \
            f"select * from {self.t_name} where a>1;"
        ...
        self.assertEqual(res.count('1 test1\n2 test2\n3 test3'), 3,
                         f'执行失败: {text}')
        self.assertEqual(res.count('2 test2\n3 test3'), 4,
                         f'执行失败: {text}')
