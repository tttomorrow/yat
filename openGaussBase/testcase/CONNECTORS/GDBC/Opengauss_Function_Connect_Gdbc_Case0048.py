...
class ConnGO48(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_48'
        self.go_file = 'go_conn.go'
        self.table_name = 't_go_48'
        self.type_name = 'type_go_48'
        self.oper_name = 'operator_go_48'
        self.func_name = 'type_go_48_smaller'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义操作符 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop operator if exists < ({self.type_name}, " \
            f"{self.type_name});" \
            f"drop function if exists {self.func_name};" \
            f"drop table if exists {self.table_name};" \
            f"drop type if exists {self.type_name};"
        # 定义数据类型
        s_sql = f"drop type if exists {self.type_name};" \
            f"create type {self.type_name} as (content text, length int);"
        sql_list = list()
        # 建表插入数据，使用type_go_48类型
        sql_list.append(f"drop table if exists {self.table_name};"
                        f"create table {self.table_name} (id serial, "
                        f"o {self.type_name});"
                        f"insert into {self.table_name} (o) values "
                        f"(row('Amy', 18));"
                        f"insert into {self.table_name} (o) values "
                        f"(row('Nico', 19));")
        # 定义比较函数
        sql_list.append(f"drop function if exists {self.func_name};"
                        f"create or replace function {self.func_name}("
                        f"left_o {self.type_name}, right_o {self.type_name})"
                        f" returns boolean as \\$\\$"
                        f"select (left_o).length < (right_o).length"
                        f"\\$\\$ language sql strict;")
        # 定义操作符
        sql_list.append(f"create operator < (procedure = {self.func_name}, "
                        f"leftarg = {self.type_name}, "
                        f"rightarg = {self.type_name}, "
                        f"commutator = <);")
        # 使用操作符
        query_sql = f"select (select o from {self.table_name} where id = 1)" \
            f" < (select o from {self.table_name} where id = 2);"
        ...
        f_contant += f'    {self.go.query(query_sql, **params)}\n' \
            f'}}'
        ...
        self.log.info(res)
        self.assertIn('panic: pq: OPERATOR is not yet supported.', res,
                      f'执行失败: {text}')
        self.assertIn("\ntrue\n", res, f'执行失败: {text}')
        self.assertEqual(res.count(' 0}'), 3, f'执行失败: {text}')
        self.assertEqual(res.count(' 1}'), 1, f'执行失败: {text}')
