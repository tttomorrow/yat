...
"""
Case Type   : go驱动
Case Name   : 连接数据库，定义模式
Description :
    1.新建用户并赋权
    2.配置pg_hba入口, sha256认证方法
    3.连接数据库，创建角色及同名schema
    4.还原pg_hba文件
    5.删除用户
    6.刪除go_conn.go
Expect      :
...
class ConnGO33(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_33'
        self.go_file = 'go_conn.go'
        self.schema_name = 'schema_go_33'
        self.user_name = 'u_go_33'
        self.view_name = 'view_go_33'
        self.table_name = 't_go_33'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，创建角色r_go_33及同名schema expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop schema if exists {self.schema_name} cascade;" \
            f"drop user if exists {self.user_name} cascade;" \
            f"drop view if exists {self.view_name};" \
            f"drop table if exists {self.table_name};" \
            f"drop user if exists {self.user_name} cascade;"
        s_sql = f'{clean_sql}' \
            f'create role {self.user_name} identified by ' \
            f'\'{macro.PASSWD_INITIAL}\';' \
            f'create schema authorization {self.user_name} ' \
            f'create table {self.table_name} (title text, release date,' \
            f' awards text[]) ' \
            f'create view {self.view_name} as ' \
            f'select title, release from {self.table_name} where ' \
            f'awards is not null;'
        s_sql1 = f"alter schema {self.user_name} rename to" \
            f" {self.schema_name};"
