...
class ConnGO49(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_49'
        self.go_file = 'go_conn.go'
        self.table_name = 't_go_49'
        self.type_name = 'type_go_49'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义数据类型 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop table if exists {self.table_name};" \
            f"drop type if exists {self.type_name}_new;"
        # 定义数据类型
        s_sql = f"drop type if exists {self.type_name};" \
            f"create type {self.type_name} as (f1 int, f2 text);"
        sql_list = list()
        # 建表插入数据，使用type_go_49类型
        sql_list.append(f"drop table if exists {self.table_name};"
                        f"create table {self.table_name} (id int, "
                        f"properties {self.type_name});"
                        f"insert into {self.table_name} values "
                        f"(1, (1, 'test_1'));")
        # 重命名数据类型
        sql_list.append(f"alter type {self.type_name} rename to "
                        f"{self.type_name}_new;")
        # 给数据类型增加一个新的属性
        sql_list.append(f"alter type {self.type_name}_new add attribute"
                        f" f3 int;")
        # 使用数据类型
        sql_list.append(f"insert into {self.table_name} values (2, (2,"
                        f" 'test_2', 2));")
        ...
        self.assertEqual(res.count(' 0}'), 4, f'执行失败: {text}')
        self.assertEqual(res.count(' 1}'), 2, f'执行失败: {text}')
