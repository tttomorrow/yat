...
class ConnGO21(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，子查询 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop table if exists {self.t_name} cascade;" \
            f"drop table if exists {self.t_name}_sub cascade;"
        s_sql = f'{clean_sql}' \
            f'create table {self.t_name}(a int, b text);' \
            f'insert into {self.t_name} values(generate_series(1,1000), ' \
            f'generate_series(1,1000)||\'test\');' \
            f'create table {self.t_name}_sub as table {self.t_name};'
        query_sql = f"select count(*) from {self.t_name} where exists " \
            f"(select a from {self.t_name}_sub where a is not null);\n" \
            f"select count(*) from {self.t_name} where not exists " \
            f"(select a from {self.t_name}_sub where a is not null);\n" \
            f"select count(*) from {self.t_name} where " \
            f"a in (select a from {self.t_name}_sub where a<100);\n" \
            f"select count(*) from {self.t_name} where " \
            f"a not in (select a from {self.t_name}_sub where a<100);\n" \
            f"select count(*) from {self.t_name} where " \
            f"a = any (select a from {self.t_name}_sub " \
            f"where a is not null);\n" \
            f"select count(*) from {self.t_name} where " \
            f"a = any (select a from {self.t_name}_sub where a is null);\n" \
            f"select count(*) from {self.t_name} where " \
            f"a = some (select a from {self.t_name}_sub " \
            f"where a is not null);\n" \
            f"select count(*) from {self.t_name} where " \
            f"a = some (select a from {self.t_name}_sub where a is null);\n" \
            f"select count(*) from {self.t_name} where " \
            f"a = all (select a from {self.t_name}_sub " \
            f"where a is not null);\n" \
            f"select count(*) from {self.t_name} where " \
            f"a = all (select a from {self.t_name}_sub where a is null);"
        query_sql = ','.join([f'"{i}"' for i in query_sql.splitlines()])
