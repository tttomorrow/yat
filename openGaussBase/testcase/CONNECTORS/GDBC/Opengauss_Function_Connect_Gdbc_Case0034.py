...
class ConnGO34(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义表空间 expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop tablespace if exists {self.ts_name}_new;"
        clean_sql1 = f"drop role if exists {self.user_name};"
        s_sql = f'{clean_sql1}' \
            f'create role {self.user_name} identified by ' \
            f'\'{macro.COMMON_PASSWD}\';'
        s_sql1 = f"create tablespace {self.ts_name} relative location " \
            f"'{self.ts_name}';"
        s_sql2 = f"alter tablespace {self.ts_name} rename to " \
            f"{self.ts_name}_new;" \
            f"alter tablespace {self.ts_name}_new owner to {self.user_name};"
        f_contant = f'{self.go.get_head()}\n\n' \
            f'func cleanup(db *sql.DB) {{\n' \
            f'    {self.go.exec_sql(clean_sql)}\n\n' \
            f'    {self.go.exec_sql(clean_sql1, False)}\n' \
            f'    db.Close()\n' \
            f'}}\n\n' \
            f'func main() {{\n' \
            f'    {conn_str}\n' \
        ...
        self.assertEqual(res.count(' 0}'), 5, f'执行失败: {text}')
