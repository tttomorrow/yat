...
class ConnGO24(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，集合运算 expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f"{clean_sql}" \
            f"create table {self.t_name}_1(a int, b text);" \
            f"insert into {self.t_name}_1 values(1, 'red'), (2, 'red'), " \
            f"(3, 'yello'), (4, 'blue'), (5, 'blue');" \
            f"create table {self.t_name}_2(a int, b text);" \
            f"insert into {self.t_name}_2 values (2, 'red'), (6, 'blue'), " \
            f"(7, 'orange'), (8, 'gray');"
        query_sql = f"select * from {self.t_name}_1 union " \
            f"select * from {self.t_name}_2;\n" \
            f"select * from {self.t_name}_1 union all " \
            f"select * from {self.t_name}_2;\n" \
            f"select * from {self.t_name}_1 except all " \
            f"select * from {self.t_name}_2;\n"
        query_sql = ','.join([f'"{i}"' for i in query_sql.splitlines()])
        query_sql1 = self.go.query(
            f"select * from {self.t_name}_1 a inner join "
            f"{self.t_name}_2 b on a.b=b.b;",
            **{"a_a": "int", "a_b": "string", "b_a": "int", "b_b": "string"})
