...
"""
Case Type   : go驱动
Case Name   : 连接数据库，explain plan
Description :
    1.新建用户并赋权
    2.配置pg_hba入口, sha256认证方法
    3.连接数据库，explain plan
    4.还原pg_hba文件
    5.删除用户
    6.刪除go_conn.go
Expect      :
...
class ConnGO8(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，explain plan expect: 成功----'
        self.log.info(text)
        ...
        clean_sql = f"drop table if exists {self.t_name};"
        s_sql = f'{clean_sql}' \
            f'create table {self.t_name}(id int,name varchar(10));' \
            f'insert into {self.t_name} values(generate_series(1,500),' \
            f'generate_series(1,500)||\'go\');' \
            f'explain plan set statement_id=\'TPCH-Q4\' for ' \
            f'select count(*) from t_go_8;'
        query_sql = self.go.query_row(
        ...
        self.assertIn('TPCH-Q4 AGGREGATE', res, f'执行失败: {text}')
