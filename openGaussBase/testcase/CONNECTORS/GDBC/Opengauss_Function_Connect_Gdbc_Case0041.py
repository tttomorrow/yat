...
class ConnGO41(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_41'
        self.go_file = 'go_conn.go'
        self.table_name = 't_go_41'
        self.proc_name = 'proc_go_41'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，定义存储过程 expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f"{clean_sql}" \
            ...
            f");"
        s_sql += f"create or replace procedure {self.proc_name}(a int," \
            f" b varchar(20)) as " \
            f"declare" \
            f"    id int := a;" \
            f"    name varchar(20) := b;" \
            f"begin" \
            f"    insert into {self.table_name} values(id, name);" \
            f"end;"
        s_sql1 = f"select {self.proc_name}(1, 'test1');"
        s_sql2 = f"call {self.proc_name}(2, 'test2');"
        ...
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    {self.go.query(s_sql1, **{"proc": "string"})}\n\n' \
            f'    {self.go.query(s_sql2, False, **{"proc1": "string"})}\n\n' \
            f'    {self.go.query(s_sql3, False, **params)}\n\n' \
            f'}}'
