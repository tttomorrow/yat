...
class ConnGO14(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，update expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            f'create table {self.t_name}(id int, name varchar(50));' \
            f'insert into {self.t_name} values(1, \'aaa\'),(2,\'bbb\'),' \
            f'(3,\'ccccc\'),(4,\'ddddddd\'),(5,\'eeeeeeee\');' \
            f'update t_go_14 set (name, id) = (\'abc\',1);'
        query_sql = self.go.query(
            f"select count(*) from {self.t_name} where name='abc'",
            **{"num": "int"})
        ...
        reg_res = re.search(' 5}.*5.* 0}', res, re.S)
        self.assertIsNotNone(reg_res, f'执行失败: {text}')
