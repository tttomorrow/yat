...
"""
Case Type   : go驱动
Case Name   : 连接数据库，modify..unique
Description :
    1.新建用户并赋权
    2.配置pg_hba入口, sha256认证方法
    3.连接数据库，modify..unique
    4.还原pg_hba文件
    5.删除用户
    6.刪除go_conn.go
Expect      :
...
class ConnGO12(unittest.TestCase):
    def setUp(self):
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，modify..unique expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            ...
            f'alter table {self.t_name} add constraint studentkey ' \
            f'primary key(a);' \
            f'alter table {self.t_name} add constraint unkey unique (a);' \
            f'alter table {self.t_name} drop constraint studentkey;' \
            f'alter table {self.t_name} add constraint unkey1 unique (a);'
