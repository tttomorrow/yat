...
class ConnGO6(unittest.TestCase):
    def setUp(self):
        ...
        self.go = ComGo()
        self.cons = Constant()
        self.db_user = 'go_user_6'
        self.go_file = 'go_conn.go'
        self.func_name = 'func_go_6'
    ...
    def test_1(self):
        ...
        text = '----step3: 连接数据库，调用函数 expect: 成功----'
        self.log.info(text)
        ...
        s_sql = f'{clean_sql}' \
            f'create or replace function {self.func_name} (int) ' \
            f'returns int as \\$\\$' \
            f'    select 1 + \\$1;' \
        ...
        f_contant = f'{self.go.get_head()}\n\n' \
            ...
            f'    {self.go.exec_sql(f"call {self.func_name}(1);", False)}\n' \
            f'}}'
