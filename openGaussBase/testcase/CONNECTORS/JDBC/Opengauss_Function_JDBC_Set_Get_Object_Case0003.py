...
"""
Case Type   : 数据库系统
Case Name   : 数据库兼容PG模式，localDate类型setObject插入Date类型数据，并使用getObject获取结果
Description :
    1.写配置文件
    2.编译java工具
    3.运行java工具
...
class Jdbcisreadonly(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0003 start")
        ...
        self.java_name = "jdbc_set_get_object_case0001"
        self.common = Common()
    ...
    def test_index(self):
        ...
        self.assertIn(f'第10行结果：{today}', result)
        self.assertIn(f'第14行结果：{today}', result)
        self.assertIn(f'第16行结果：{today}', result)
        ...
        self.assertIn('第13行结果：2020-02-29', result)
        self.assertIn('第15行结果：2021-01-02', result)
    ...
    def tearDown(self):
        ...
        result = self.commsh.execut_db_sql(
            f"drop database if exists {self.db_name};")
        self.log.info(result)
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0003 end")