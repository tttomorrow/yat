...
"""
Case Type   : 数据库系统
Case Name   : LocalTime类型geiObject并发用例
Description :
    1.写配置文件
    2.编译java工具
    3.建表并插入数据
...
class Jdbcisreadonly(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0017 start")
        ...
        self.java_name = "jdbc_set_get_object_case0017"
        self.tb_name = "jdbc_set_get_object_case0017"
        self.common = Common()
    ...
    def test_index(self):
        ...
        cmd = f"insert into {self.tb_name} " \
            f"values('allballs'), ('23:59:59'), (null);"
        result = self.commonshpri.execut_db_sql(cmd)
        self.log.info(result)
        ...
        for index in range(50):
            ...
            for i in range(9):
                ...
                self.assertIn(f"第0行结果：null", select_result)
                self.assertIn(f"第1行结果：23:59:59", select_result)
                self.assertIn(f"第2行结果：00:00", select_result)
            time.sleep(3)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_JDBC_Set_Get_Object_Case0017 end-")
