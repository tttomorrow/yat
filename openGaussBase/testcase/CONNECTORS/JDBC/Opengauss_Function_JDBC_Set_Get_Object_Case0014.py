...
"""
Case Type   : 数据库系统
Case Name   : LocalDate类型setObject并发用例
Description :
    1.写配置文件
    2.编译java工具
    3.建表
...
class Jdbcisreadonly(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0014 start")
        ...
        self.java_name = "jdbc_set_get_object_case0014"
        self.tb_name = "jdbc_set_get_object_case0014"
        self.common = Common()
    ...
    def test_index(self):
        ...
        for index in range(50):
            ...
            self.assertIn("(144 rows)", insert_result)
            ...
            self.assertEqual(insert_result.count('1970-01-02 00:00:00'), 9)
            self.assertEqual(insert_result.count('2020-02-29 00:00:00'), 9)
            self.assertEqual(insert_result.count('2021-01-02 00:00:00'), 18)
            ...
            self.assertGreaterEqual((insert_result.count(f'{now[0]}') +
                              insert_result.count(f'{now[1]}') +
                              insert_result.count(f'{now[2]}') +
                              insert_result.count(f'{today}') +
                              insert_result.count(f'{tomorrow}') +
                              insert_result.count(f'{yesterday}')), 54)
    ...
    def tearDown(self):
        ...
        self.log.info("-Opengauss_Function_JDBC_Set_Get_Object_Case0014 end-")
