...
"""
Case Type   : 数据库系统
Case Name   : 预编译sql为查询*所有列数据，执行预编译sql语句过程中修改表结构-删除字段
Description :
    1.创建配置文件
    2.编译java文件
...
class JDBCPrepareStatement(unittest.TestCase):
    ...
    def test_main(self):
        ...
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} ' \
            f'DROP COLUMN c_customer_sk;\d {self.tb_name}'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        self.assertIn(self.constant.ALTER_TABLE_MSG, result,
                      "执行失败" + step_txt)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0022:end----')
