...
"""
Case Type   : 数据库系统
Case Name   : 预编译sql为查询*所有列数据，执行预编译sql语句过程中修改表结构-修改字段名称
Description :
    1.创建配置文件
    2.编译java文件
    3.运行java预编译代码（查询*所有列预编译重复执行）
...
Expect      :
    1.创建配置文件成功
    2.编译java文件成功
    3.运行java预编译代码（查询*所有列预编译重复执行），开始执行
...
class JDBCPrepareStatement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0025:start----')
        self.target_path = os.path.join(macro.DB_BACKUP_PATH, "JDBC_test")
        self.properties = os.path.join(self.target_path,
                                       "jdbc_preparestat_Case0025.properties")
        self.java_name = "jdbc_PrepareStatement_Case0021_25"
        self.tb_name = "tb_jdbc_PrepareStatement_Case0025"
        self.com = Common()
    ...
    def test_main(self):
        ...
        step_txt = '----step3:运行java预编译代码（查询*所有列预编译重复执行）; expect:开始运行----'
        self.log.info(step_txt)
        java_cmd = f"java -cp " \
            f"{os.path.join(self.target_path, 'postgresql.jar')}" \
            f":{self.target_path} " \
        ...
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} ' \
            f'rename c_customer_sk to c_customer_sk1;'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        self.assertIn(self.constant.ALTER_TABLE_MSG, result,
                      "执行失败" + step_txt)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0025:end----')
