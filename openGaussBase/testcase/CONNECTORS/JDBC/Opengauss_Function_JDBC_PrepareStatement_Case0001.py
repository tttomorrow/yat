...
"""
Case Type   : 数据库系统
Case Name   : 预编译sql为插入数据，执行预编译sql语句过程中修改表结构-增加字段
Description :
    1.创建配置文件
    2.编译java文件
    3.运行java预编译代码（插入预编译重复执行）
    4.步骤3过程中，进行表结构变更-增加字段可以为空
    5.步骤3过程中，进行表结构变更-增加字段不可以为空
    6.验证步骤3结果
Expect      :
    1.创建配置文件成功
    2.编译java文件成功
    3.运行java预编译代码（插入预编译重复执行），开始执行
    4.步骤3过程中，进行表结构变更-增加字段可以为空，修改成功
    5.步骤3过程中，进行表结构变更-增加字段不可以为空，修改失败
    6.验证步骤3结果，正常执行完成
History     :
"""
import os
...
class JDBCPrepareStatement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0001:start----')
        self.target_path = os.path.join(macro.DB_BACKUP_PATH, "JDBC_test")
        self.properties = os.path.join(self.target_path,
                                       "jdbc_preparestat_case0001.properties")
        self.java_name = "jdbc_PrepareStatement_Case0001_05"
        self.tb_name = "tb_jdbc_PrepareStatement_Case0001"
        self.com = Common()
        self.constant = Constant()
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.match_msg = 'ERROR:  column ".*" contains null values'
    ...
    def test_main(self):
        ...
        step_txt = '----step4:步骤3过程中，进行表结构变更-增加字段可以为空; expect:修改成功----'
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} add ' \
            f'column c_customer_bak3 char(40) ;\d {self.tb_name}'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        self.assertIn(self.constant.ALTER_TABLE_MSG, result,
                      "执行失败" + step_txt)

        step_txt = '----step5:步骤3过程中，进行表结构变更-增加字段不可以为空; expect:修改失败----'
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} add ' \
            f'column c_customer_bak2 char(40) not null;'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        result = re.match(self.match_msg, result)
        self.assertIsNotNone(result, "执行失败" + step_txt)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0001:end----')
