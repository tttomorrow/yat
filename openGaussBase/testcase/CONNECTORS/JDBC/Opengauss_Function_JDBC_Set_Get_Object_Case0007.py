...
"""
Case Type   : 数据库系统
Case Name   : 数据库兼容PG模式，localdatetime类型getObject读取Date类型数据
Description :
    1.写配置文件
    2.编译java工具
    3.运行java工具
...
class Jdbcisreadonly(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0007 start")
        ...
        self.java_name = "jdbc_set_get_object_case0007"
        self.common = Common()
    ...
    def test_index(self):
        ...
        result = self.db_primary_root_node.sh(cmd).result()
        self.log.info(result)
        self.assertIn('conversion to class java.time.LocalDateTime '
                      'from date not supported', result)
    ...
    def tearDown(self):
        ...
        result = self.commsh.execut_db_sql(
            f"drop database if exists {self.db_name};")
        self.log.info(result)
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0007 end")