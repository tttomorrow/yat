...
"""
Case Type   : 数据库系统
Case Name   : 预编译sql为插入数据，执行预编译sql语句过程中修改表结构-修改字段长度
Description :
    1.创建配置文件
    2.编译java文件
    3.运行java预编译代码（插入预编译重复执行）
    4.步骤3过程中，进行表结构变更-修改字段长度与预编译sql相关
    5.步骤3过程中，进行表结构变更-修改字段长度与预编译sql无关
    6.步骤3过程中，进行表结构变更-修改字段长度小于已存在数据
    7.验证步骤3结果
Expect      :
    1.创建配置文件成功
    2.编译java文件成功
    3.运行java预编译代码（插入预编译重复执行），开始执行
    4.步骤3过程中，进行表结构变更-修改字段长度与预编译sql相关，修改成功
    5.步骤3过程中，进行表结构变更-修改字段长度与预编译sql无关，修改成功
    6.步骤3过程中，进行表结构变更-修改字段长度小于已存在数据，修改失败
    7.验证步骤3结果，由于步骤4修改长度小于后续预编译插入长度，预编译插入中断
History     :
"""
import os
import time
...
class JDBCPrepareStatement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0003:start----')
        self.target_path = os.path.join(macro.DB_BACKUP_PATH, "JDBC_test")
        self.properties = os.path.join(self.target_path,
                                       "jdbc_preparestat_case0003.properties")
        self.java_name = "jdbc_PrepareStatement_Case0001_05"
        self.tb_name = "tb_jdbc_PrepareStatement_Case0003"
        self.com = Common()
        self.constant = Constant()
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.err_msg = 'ERROR:  value too long for type character varying(3)'
        self.err_msg1 = 'ERROR: value too long for type character varying(9)'
    ...
    def test_main(self):
        ...
        step_txt = '----step4:步骤3过程中，进行表结构变更-修改字段长度与预编译sql相关; expect:修改成功----'
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} ' \
            f'alter COLUMN c_customer_name type VARCHAR(9);\d {self.tb_name}'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        self.assertIn(self.constant.ALTER_TABLE_MSG, result,
                      "执行失败" + step_txt)

        step_txt = '----step5:步骤3过程中，进行表结构变更-修改字段长度与预编译sql无关； expect:修改成功----'
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} ' \
            f'alter COLUMN c_customer_bak1 type char(40);\d {self.tb_name}'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        self.assertIn(self.constant.ALTER_TABLE_MSG, result,
                      "执行失败" + step_txt)

        step_txt = '----step6:步骤3过程中，进行表结构变更-修改字段长度小于已存在数据； expect:修改失败----'
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} ' \
            f'alter COLUMN c_customer_name type VARCHAR(3);'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        self.assertIn(self.err_msg, result, "执行失败" + step_txt)

        step_txt = '----step7:验证步骤3结果; expect:步骤4修改长度小于后续预编译插入长度，预编译插入中断----'
        self.log.info(step_txt)
        java_thread.join(600)
        java_result = java_thread.get_result()
        self.assertIn(self.err_msg1, java_result, "执行失败" + step_txt)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0003:end----')
