...
"""
Case Type   : 数据库系统
Case Name   : OffSetDateTime类型setObject插入TIMESTAMPtz类型数据，并使用getObject获取结果
Description :
    1.写配置文件
    2.编译java工具
    3.运行java工具
...
class Jdbcisreadonly(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0012 start")
        ...
        self.java_name = "jdbc_set_get_object_case0012"
        self.common = Common()
    ...
    def test_index(self):
        ...
        today.append((datetime.datetime.strptime(result_time,
                                            '%Y-%m-%d %H:%M:%S') -
                 datetime.timedelta(hours=+8, seconds=+5)
                      ).strftime('%Y-%m-%dT%H'))
        self.log.info(today)
    ...
    def tearDown(self):
        ...
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0012 end")
