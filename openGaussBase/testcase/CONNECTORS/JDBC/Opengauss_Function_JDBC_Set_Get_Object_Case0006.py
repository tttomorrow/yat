...
"""
Case Type   : 数据库系统
Case Name   : localTime类型setObject插入Time with time zone类型数据
Description :
    1.写配置文件
    2.编译java工具
    3.运行java工具
...
class Jdbcisreadonly(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0006 start")
        ...
        self.java_name = "jdbc_set_get_object_case0006"
        self.common = Common()
    ...
    def test_index(self):
        ...
        result = self.db_primary_root_node.sh(cmd).result()
        self.log.info(result)
        self.assertIn('第1行结果：24:00:00+08', result)
        self.assertIn('第2行结果：00:00:00+08', result)
        self.assertIn('第3行结果：null', result)
        self.assertIn('第6行结果：00:00:00+08', result)
        self.assertIn('第7行结果：12:00:00+08', result)
        self.assertIn('第8行结果：00:00:00+00', result)
        self.assertIn('第9行结果：23:59:59+08', result)
    ...
    def tearDown(self):
        ...
        self.log.info("Opengauss_Function_JDBC_Set_Get_Object_Case0006 end")
