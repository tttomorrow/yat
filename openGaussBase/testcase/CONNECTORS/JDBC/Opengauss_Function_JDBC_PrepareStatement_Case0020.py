...
"""
Case Type   : 数据库系统
Case Name   : 预编译sql为查询指定列数据，执行预编译sql语句过程中修改表结构-修改字段名称
Description :
    1.创建配置文件
    2.编译java文件
    3.运行java预编译代码（查询指定列预编译重复执行）
    4.步骤3过程中，进行表结构变更-修改字段名称与预编译sql无关
    5.步骤3过程中，进行表结构变更-修改字段名称与预编译sql相关
    6.验证步骤3结果
Expect      :
    1.创建配置文件成功
    2.编译java文件成功
    3.运行java预编译代码（查询指定列预编译重复执行），开始执行
    4.步骤3过程中，进行表结构变更-修改字段名称与预编译sql无关，修改成功
    5.步骤3过程中，进行表结构变更-修改字段名称与预编译sql相关，修改成功
    6.验证步骤3结果，由于步骤5中断
History     :
"""
import os
...
class JDBCPrepareStatement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0020:start----')
        self.target_path = os.path.join(macro.DB_BACKUP_PATH, "JDBC_test")
        self.properties = os.path.join(self.target_path,
                                       "jdbc_preparestat_Case0020.properties")
        self.java_name = "jdbc_PrepareStatement_Case0016_20"
        self.tb_name = "tb_jdbc_PrepareStatement_Case0020"
        self.com = Common()
        self.constant = Constant()
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.match_msg = 'ERROR: column ".*" does not exist'
    ...
    def test_main(self):
        ...
        step_txt = '----step3:运行java预编译代码（查询指定列预编译重复执行）; expect:开始运行----'
        self.log.info(step_txt)
        java_cmd = f"java -cp " \
            f"{os.path.join(self.target_path, 'postgresql.jar')}" \
            f":{self.target_path} " \
        ...
        step_txt = '----step4:步骤3过程中，进行表结构变更-修改字段名称与预编译sql无关; expect:修改成功----'
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} ' \
            f'rename c_customer_bak1 to bak1;\d {self.tb_name}'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        self.assertIn(self.constant.ALTER_TABLE_MSG, result,
                      "执行失败" + step_txt)

        step_txt = '----step5:步骤3过程中，进行表结构变更-修改字段名称与预编译sql相关； expect:修改成功----'
        self.log.info(step_txt)
        alter_sql = f'alter table {self.tb_name} ' \
            f'rename c_customer_name to c_customer_name1;'
        result = self.pri_sh.execut_db_sql(alter_sql)
        self.log.info(result)
        self.assertIn(self.constant.ALTER_TABLE_MSG, result,
                      "执行失败" + step_txt)

        step_txt = '----step6:验证步骤3结果; expect:由于步骤5中断----'
        self.log.info(step_txt)
    ...
    def tearDown(self):
        ...
        self.log.info(
            '----Opengauss_Function_JDBC_PrepareStatement_Case0020:end----')
