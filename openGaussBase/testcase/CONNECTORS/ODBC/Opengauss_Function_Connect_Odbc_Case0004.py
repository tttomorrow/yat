...
class ConnODBC4(unittest.TestCase):
    def setUp(self):
        self.LOG = Logger()
    ...
    def test_conn(self):
        ...
        self.LOG.info(text)
        pwd = quote(macro.PASSWD_INITIAL + special, 'utf-8')
        ini_content = [self.pri_user.db_host, self.db_name, self.db_user,
                       pwd, self.pri_user.db_port]
        ...
        self.LOG.info(text)
        source_cmd = f'source {self.sourcefile};' \
            f'isql -v gaussodbc &'
        self.LOG.info(source_cmd)
        source_msg = self.pri_root.sh(source_cmd).result()
