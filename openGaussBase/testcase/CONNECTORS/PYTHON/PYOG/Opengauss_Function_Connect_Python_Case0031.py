...
Case Name   : openGauss模式连接数据库，行分区表
Description :
    1.配置pg_hba入口
    2.连接数据库
    3.创建行存分区表，精度在合理范围值，插入数据
    --3.1建表插入数据
    --3.2查询
    --3.3删除表
Expect      :
...
class ConnPython31(unittest.TestCase):
    def setUp(self):
        self.pri_user = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.LOG = Logger()
        text = '----Opengauss_Function_Connect_Python_Case0031 start----'
        self.LOG.info(text)
    ...
    def test_conn(self):
        ...
        text = '----step3: 创建行存分区表，精度在合理范围值，插入数据 expect: 成功----'
        self.LOG.info(text)

        text = '----step3.1: 建表插入数据 expect: 成功----'
        self.LOG.info(text)
        cmd = '''drop table if exists t_py_31;
           create table t_py_31 (c1 dec(4,2),c2 int) with \
           ...
           (
            ...
            insert into t_py_31 values (0.0,1);
            insert into t_py_31 values (10.01,1);
            insert into t_py_31 values (20.01,1);
            insert into t_py_31 values (30.01,1);
            insert into t_py_31 values (40.01,1);'''
        self.LOG.info(cmd)
        res = db.execute(cmd)
        ...
        self.LOG.info(text)
        cmd = "select count(*) from t_py_31;"
        self.LOG.info(cmd)
        sql = db.prepare(cmd)
        self.assertEqual(sql(), [(5,)], text)

        cmd = "select * from t_py_31 partition (P2);"
        self.LOG.info(cmd)
        sql = db.prepare(cmd)
    ...
    def tearDown(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0031 end----'
        self.LOG.info(text)
