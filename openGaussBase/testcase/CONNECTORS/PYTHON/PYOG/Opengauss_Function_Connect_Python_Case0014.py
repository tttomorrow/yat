...
Case Name   : openGauss模式连接数据库，explain plan
Description :
    1.配置pg_hba入口
    2.连接数据库
    3.explain plan
...
class ConnPython14(unittest.TestCase):
    def setUp(self):
        self.pri_user = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.LOG = Logger()
        text = '----Opengauss_Function_Connect_Python_Case0014 start----'
        self.LOG.info(text)
    ...
    def test_conn(self):
        ...
        text = '----step3: explain plan expect: 成功----'
        self.LOG.info(text)
        ...
        cmd = 'create table t_py_1(a int, b int)'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.CREATE_TABLE_SUCCESS,
                                 None), text)

        cmd = "insert into t_py_1 values(generate_series(1,500), " \
              "generate_series(500,1000))"
        sql = db.prepare(cmd)
        self.assertEqual(sql(), ('INSERT', 250500), text)

        cmd = "explain plan set statement_id='py_14' for " \
              "select count(*) from t_py_1"
        sql = db.prepare(cmd)
        self.assertEqual(sql(), ('EXPLAIN SUCCESS', None), text)

        cmd = "select count(*) from t_py_1"
        sql = db.prepare(cmd)
        self.assertEqual(sql(), [(250500,)], text)

        cmd = "select statement_id, operation from plan_table"
        sql = db.prepare(cmd)
        expect = [('py_14', 'AGGREGATE'), ('py_14', 'TABLE ACCESS')]
        self.assertEqual(sql(), expect, text)

        cmd = 'delete from plan_table'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), ('DELETE', 2), text)
    ...
    def tearDown(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0014 end----'
        self.LOG.info(text)
