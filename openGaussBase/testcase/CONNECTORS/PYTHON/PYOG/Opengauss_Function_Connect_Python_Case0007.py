...
Case Name   : openGauss模式连接数据库，密码含特殊字符，如@
Description :
    1.创建库、用户，用户密码含特殊字符，并赋权
    2.配置pg_hba入口
    3.连接数据库
    4.断开连接
Expect      :
...
class ConnPython7(unittest.TestCase):
    def setUp(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0007 start----'
        self.LOG.info(text)

    def test_conn(self):
        text = '----step1: 创建库、用户，用户密码含特殊字符，并赋权 expect: 成功----'
        self.LOG.info(text)
        special = '@&%$'
        sql_cmd = f"drop database if exists {self.db_name}; " \
            f"create database {self.db_name}; " \
            f"create user {self.db_user} with password " \
            f"'{macro.PASSWD_INITIAL}{special}'; " \
        ...
        text = '----step3: 连接数据库，密码含@ expect: 成功----'
        self.LOG.info(text)
        pwd = macro.PASSWD_INITIAL + special
        conn_info = f'opengauss://{self.db_user}:{pwd}@' \
    ...
    def tearDown(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0007 end----'
        self.LOG.info(text)
