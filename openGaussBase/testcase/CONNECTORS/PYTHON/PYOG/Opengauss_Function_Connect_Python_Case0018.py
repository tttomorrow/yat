...
Case Name   : openGauss模式连接数据库，modify..unique
Description :
    1.配置pg_hba入口
    2.连接数据库
    3.删除主键之后，再次更新unique约束
    4.断开连接
Expect      :
    1.执行成功
...
class ConnPython18(unittest.TestCase):
    def setUp(self):
        self.pri_user = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.LOG = Logger()
        text = '----Opengauss_Function_Connect_Python_Case0018 start----'
        self.LOG.info(text)
    ...
    def test_conn(self):
        ...
        text = '----step3: 删除主键之后，再次更新unique约束 expect: 成功----'
        self.LOG.info(text)
        ...
        cmd = 'create table t_py_1(id int)'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.CREATE_TABLE_SUCCESS,
                                 None), text)

        cmd = 'alter table t_py_1 add constraint p_key primary key(id)'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.ALTER_TABLE_MSG, None), text)

        cmd = 'alter table t_py_1 add constraint unkey unique (id)'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.ALTER_TABLE_MSG, None), text)

        cmd = 'alter table t_py_1 drop constraint p_key'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.ALTER_TABLE_MSG, None), text)

        cmd = 'alter table t_py_1 add constraint unkey1 unique (id)'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.ALTER_TABLE_MSG, None), text)
    ...
    def tearDown(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0018 end----'
        self.LOG.info(text)
