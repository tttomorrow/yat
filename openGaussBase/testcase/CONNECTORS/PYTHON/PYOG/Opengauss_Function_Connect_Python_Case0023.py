...
Case Name   : openGauss模式连接数据库，匿名块
Description :
    1.配置pg_hba入口
    2.连接数据库
    3.执行只含有DML语句的匿名块
...
class ConnPython23(unittest.TestCase):
    def setUp(self):
        self.pri_user = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.LOG = Logger()
        text = '----Opengauss_Function_Connect_Python_Case0023 start----'
        self.LOG.info(text)
    ...
    def test_conn(self):
        ...
        cmd = 'drop table if exists t_py_anonymous23;'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.TABLE_DROP_SUCCESS,
                                 None), text)

        cmd = 'create table t_py_anonymous23 (a int,b int,c text);'
        sql = db.prepare(cmd)
        ...
        cmd = '''declare 
           begin
               insert into t_py_anonymous23 values(generate_series(1,100000),\
               generate_series(1,5),'c-'||generate_series(1,10));
           end;'''
        sql = db.prepare(cmd)
        self.assertEqual(sql(), ('ANONYMOUS BLOCK EXECUTE', None), text)

        cmd = "select count(*) from t_py_anonymous23;"
        sql = db.prepare(cmd)
        self.assertEqual(sql(), [(100000,)], text)
    ...
    def tearDown(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0023 end----'
        self.LOG.info(text)
