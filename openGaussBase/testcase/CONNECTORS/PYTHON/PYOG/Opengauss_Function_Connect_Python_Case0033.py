...
Case Name   : openGauss模式连接数据库，定义用户&授权
Description :
    1.新建用户并授权
    2.配置pg_hba入口
    3.连接数据库
...
class ConnPython33(unittest.TestCase):
    def setUp(self):
        ...
        self.LOG = Logger()
        self.user = 'u_py_33'
        text = '----Opengauss_Function_Connect_Python_Case0033 start----'
        self.LOG.info(text)

    def test_conn(self):
        text = '----step1: 新建用户并授权 expect: 成功----'
        self.LOG.info(text)
        sql_cmd = f"drop user if exists {self.user} cascade;" \
            f"create user {self.user} password " \
            f"'{macro.PASSWD_INITIAL}';" \
            f"alter user {self.user} createrole;" \
        ...
        pattern = f"{self.constant.DROP_ROLE_SUCCESS_MSG}.*" \
            ...
            f"{self.constant.ALTER_ROLE_SUCCESS_MSG}.*" \
    ...
    def tearDown(self):
        ...
        text1 = '----step5: 删除pg_hba入口 expect: 成功----'
        self.LOG.info(text1)
        guc_cmd = f'source {macro.DB_ENV_PATH}; ' \
            f'gs_guc reload -D {macro.DB_INSTANCE_PATH} ' \
        ...
        text = '----Opengauss_Function_Connect_Python_Case0033 end----'
        self.LOG.info(text)

        text = '----断言teardown执行成功----'
        self.LOG.info(text)
        self.assertIn(self.constant.GSGUC_SUCCESS_MSG, guc_res, text1)
        self.assertIn(self.constant.DROP_ROLE_SUCCESS_MSG, sql_res, text2)
