...
Case Name   : openGauss模式连接数据库, 多ip（不含主机）连接
Description :
...
class ConnPython10(unittest.TestCase):
    def setUp(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0010 start----'
        self.LOG.info(text)
    ...
    def test_conn(self):
        ...
        conn_info = f'opengauss://{self.db_user}:{macro.PASSWD_INITIAL}@' \
            ...
            f'{self.sta2_user.db_host}:{self.sta2_user.db_port}/' \
            f'{self.db_name}'
    ...
    def tearDown(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0010 end----'
        self.LOG.info(text)
