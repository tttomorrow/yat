...
Case Name   : openGauss模式连接数据库，upsert
Description :
    1.配置pg_hba入口
    2.连接数据库
    3.upsert插入
...
class ConnPython21(unittest.TestCase):
    def setUp(self):
        self.pri_user = Node('PrimaryDbUser')
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.LOG = Logger()
        text = '----Opengauss_Function_Connect_Python_Case0021 start----'
        self.LOG.info(text)
    ...
    def test_conn(self):
        ...
        text = '----step3: upsert插入 expect: 成功----'
        self.LOG.info(text)

        cmd = 'drop table if exists t_py_upsert21;'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.TABLE_DROP_SUCCESS,
                                 None), text)

        cmd = 'create table t_py_upsert21 (a int,b int primary key,c text);'
        sql = db.prepare(cmd)
        self.assertEqual(sql(), (self.constant.CREATE_TABLE_SUCCESS,
                                 None), text)

        cmd = "insert into t_py_upsert21 values(generate_series(1,10)," \
              "generate_series(1,10),'c-'||generate_series(1,10));"
        sql = db.prepare(cmd)
        self.assertEqual(sql(), ('INSERT', 10), text)

        cmd = "select * from t_py_upsert21 where b=1;"
        sql = db.prepare(cmd)
        self.assertEqual(sql(), [(1, 1, 'c-1')], text)

        cmd = "explain (costs off, verbose) insert into t_py_upsert21 " \
              "values(1,1,3) on duplicate key update c='new-c';"
        sql = db.prepare(cmd)
        expect = [('Insert on public.t_py_upsert21',),
                  ('  Conflict Resolution: UPDATE',),
                  ('  Conflict Arbiter Indexes: t_py_upsert21_pkey',),
                  ('  ->  Result',), ("        Output: 1, 1, '3'::text",)]
        self.assertEqual(sql(), expect, text)

        cmd = "insert into t_py_upsert21 values(1,1,3) on duplicate key " \
              "update c='new-c';"
        sql = db.prepare(cmd)
        ...
        cmd = "select * from t_py_upsert21 where b=1;"
        sql = db.prepare(cmd)
        self.assertEqual(sql(), [(1, 1, 'new-c')], text)
    ...
    def tearDown(self):
        ...
        text = '----Opengauss_Function_Connect_Python_Case0021 end----'
        self.LOG.info(text)
