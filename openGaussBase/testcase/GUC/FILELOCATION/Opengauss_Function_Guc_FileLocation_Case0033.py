...
"""
Case Type   : GUC
Case Name   : ident_file参数使用gs_guc set设置
Description :
        1.查询ident_file默认值
        2.创建测试文件夹,将pg_ident.conf文件拷贝至测试目录
        3.使用gs_guc set设置ident_file为自定义路径并重启数据库
        4.查询参数修改后的值
        5.清理环境
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0033start-')
    ...
    def test_guc(self):
        ...
        text = '---step3:使用gs_guc set设置ident_file为自定义路径;' \
               'expect:设置成功'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.pg_file, sql_cmd.splitlines()[-2].strip(),
                         '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0033finish-')
