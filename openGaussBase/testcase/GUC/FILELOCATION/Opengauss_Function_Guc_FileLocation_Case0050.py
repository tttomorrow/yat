...
"""
Case Type   : GUC
Case Name   : external_pid_file参数使用gs_guc reload 设置
Description :
        1.查询external_pid_file默认值
        2.使用gs_guc reload设置external_pid_file参数值为指定路径
        3.查询参数修改后的值
        4.清理环境
Expect      :
        1.默认值为空
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0050start-')
    ...
    def test_guc(self):
        ...
        text = '---step3:使用gs_guc reload设置external_pid_file;' \
               'expect:设置成功--'
        self.log.info(text)
        result = self.pri_sh.execute_gsguc('reload',
                                           self.constant.GSGUC_SUCCESS_MSG,
                                           f"external_pid_file="
                                           f"'{self.file_path}/external.pid'")
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0050finish-')
