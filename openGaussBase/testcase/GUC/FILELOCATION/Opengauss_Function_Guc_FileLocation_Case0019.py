...
"""
Case Type   : GUC
Case Name   : 修改指定数据库，用户，会话级别的参数hba_file
Description :
        1.查询hba_file默认值
        2.创建测试文件
        3.在gsql中设置数据库级别hba_file
        4.在gsql中设置用户级别hba_file
        5.在gsql中设置会话级别hba_file并查询
        6.清理环境
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0019start-')
    ...
    def test_guc(self):
        text = '---step1:查询默认值;expect:默认值是pg_hba.conf(---'
        self.log.info(text)
        ...
        text = '---step2:创建测试文件并复制pg_hba.conf文件至测试路径;' \
               'expect:创建成功，复制成功---'
        self.log.info(text)
        ...
        text = '---step3:在gsql中设置数据库级别hba_file;' \
               'expect:合理报错---'
        ...
        text = '---step4:在gsql中设置用户级别hba_file;' \
               'expect:合理报错--'
        ...
        text = '---step5:在gsql中设置会话级别hba_file;' \
               'expect:合理报错--'
        self.log.info(text)
        sql_cmd = self.pri_sh.execut_db_sql(f"set hba_file to "
    ...
    def tearDown(self):
        ...
        self.log.info(msg)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0019finish-')
