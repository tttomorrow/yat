...
"""
Case Type   : GUC
Case Name   : ident_file参数使用gs_guc set设置为非字符类型
Description :
              1、查看ident_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ident_file
              2、使用设置gs_guc set设置ident_file为12345
              gs_guc set -D {cluster/dn1} -c "ident_file=12345"
Expect      :
              1、显示默认值；
              2、参数修改失败；
History     :
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0043开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("ident_file参数使用gs_guc set设置为非字符类型")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0043完成==')
