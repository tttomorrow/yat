...
"""
Case Type   : GUC
Case Name   : 使用gs_guc reload修改参数data_directory为空值，观察预期结果；
Description :
              1、查看data_directory默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c data_directory
              2、使用设置s_guc reload设置data_directory为空值
              gs_guc reload -D {cluster/dn1} -c "data_directory=' '"
              gs_guc reload -N all  -D {cluster/dn1} -c "data_directory=' '"
              3、重启使其生效，观察预期结果；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0015开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("使用设置gs_guc reload设置data_directory为空值")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0015完成==')
