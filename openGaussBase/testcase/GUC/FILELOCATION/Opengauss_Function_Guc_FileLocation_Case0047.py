...
"""
Case Type   : GUC
Case Name   : ident_file参数使用gs_guc reload设置为空值
Description :
              1、查看ident_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ident_file
              2、使用设置s_guc set设置ident_file为空值
              gs_guc reload -D {cluster/dn1} -c "ident_file="
              3、重启使其生效，观察预期结果；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0047开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("ident_file参数使用gs_guc reload设置为空值")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0047完成==')
