...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数ident_file为不存在目录
Description :
        1.查询ident_file默认值
        2.使用ALTER SYSTEM SET修改数据库参数ident_file为不存在目录并重启数据库
        3.查询参数修改后的值
        4.清理环境
Expect      :
        1.默认值为pg_ident.conf
...
class GUC(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0039start-')
        self.constant = Constant()
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.user_node = Node('PrimaryDbUser')
        self.pg_file = os.path.join(macro.DB_INSTANCE_PATH, 'pg_ident.conf')

    def test_external_pid_file(self):
        ...
        text = '---step2:使用ALTER SYSTEM SET修改数据库参数ident_file' \
               '为不存在的路径并重启数据库;expect:重启成功'
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0039finish-')
