...
"""
Case Type   : GUC
Case Name   : ident_file参数使用gs_guc reload设置为不存在目录
Description :
        1.查询ident_file默认值
        2.使用gs_guc reload设置ident_file为不存在的目录
        3.查询参数修改后的值
        4.清理环境
Expect      :
...
class GUC(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0038start-')
    ...
    def test_external_pid_file(self):
        ...
        text = '---step2:使用gs_guc reload设置ident_file为不存在的' \
               ';expect:设置成功--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0038finish-')
