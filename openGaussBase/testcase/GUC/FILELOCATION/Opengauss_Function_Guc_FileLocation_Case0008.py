...
"""
Case Type   : GUC
Case Name   : data_directory参数使用gs_guc set设置为问题路径（如路径分隔符为\)
Description :
        1.查询data_directory默认值
        2.创建测试目录
        3.使用gs_guc set设置data_directory为问题路径并重启
        4.kill -9
        5.恢复参数默认值
        6.连接数据库执行ddl操作
        7.清理环境
...
class GUC(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0008 start-')
        ...
        self.tb_name = "tb_guc_filelocation_case0008"
    ...
    def test_data_directory(self):
        ...
        text = '---step3:使用设置gs_guc set设置data_directory为问题路径并' \
               '重启;expect:重启失败'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, drop_cmd,
                      '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0008 finish-')
