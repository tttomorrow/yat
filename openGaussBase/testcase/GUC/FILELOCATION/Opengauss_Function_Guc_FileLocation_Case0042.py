...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数ident_file为问题路径
              （如路径分隔符为\）
Description :
        1.查询ident_file默认值
        2.创建测试文件夹,将pg_ident.conf文件拷贝至测试目录
        3.使用ALTER SYSTEM SET修改数据库参数ident_file为问题路径并重启数据库
        4.查询参数修改后的值
        5.清理环境
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0042start-')
    ...
    def test_guc(self):
        ...
        text = '---step3:使用ALTER SYSTEM SET修改数据库参数ident_file' \
        ...
        sql_cmd = self.pri_sh.execut_db_sql('show ident_file;')
        self.log.info(sql_cmd)
        self.assertIn(f'{self.file_path}pg_ident.conf', sql_cmd,
                      '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0042finish-')
