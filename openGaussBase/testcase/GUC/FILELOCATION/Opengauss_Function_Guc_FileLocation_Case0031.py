...
"""
Case Type   : GUC
Case Name   : hba_file参数使用gs_guc reload设置为空值
Description :
              1、查看hba_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c hba_file
              2、使用设置s_guc set设置hba_file为空值
              gs_guc reload -D {cluster/dn1} -c "hba_file="
              3、重启使其生效，观察预期结果；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0031开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("hba_file参数使用gs_guc reload设置为空值")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0031完成==')
