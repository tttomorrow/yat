...
"""
Case Type   : GUC
Case Name   : external_pid_file参数使用gs_guc reload 为问题路径
             （如路径分隔符为\）
Description :
        1.查询external_pid_file默认值
        2.创建测试文件
        3.使用gs_guc reload设置external_pid_file到问题路径下
        4.查询参数修改后的值
        5.清理环境
Expect      :
        1.默认值为空
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0057start-')
    ...
    def test_guc(self):
        ...
        text = '---step3:使用gs_guc reload设置external_pid_file到问题路径;' \
               'expect:设置成功'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0057finish-')
