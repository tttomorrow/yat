...
"""
Case Type   : GUC
Case Name   : ident_file参数使用gs_guc reload 设置为问题路径（如路径分隔符为\）
Description :
        1.查询ident_file默认值
        2.创建测试文件夹,将pg_ident.conf文件拷贝至测试目录
        3.使用gs_guc reload设置external_pid_file为问题路径
        4.查询参数修改后的值
        5.清理环境
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0041start-')
    ...
    def test_guc(self):
        ...
        text = '---step3:使用设置gs_guc reload设置ident_file为问题路径;' \
               'expect:设置成功'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0041finish-')
