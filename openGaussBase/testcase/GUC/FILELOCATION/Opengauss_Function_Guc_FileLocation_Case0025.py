...
"""
Case Type   : GUC
Case Name   : hba_file参数使用gs_guc reload 设置为问题路径（如路径分隔符为\）
Description :
        1.查询hba_file默认值
        2.创建测试目录,并将hba_file目录拷贝至测试目录下
        3.hba_file参数使用gs_guc reload 设置为问题路径
        4.查询hba_file参数值
        5.清理环境
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0025start-')
    ...
    def test_guc(self):
        ...
        text = '---step3:使用gs_guc reload设置pg_hba.conf为问题路径;' \
               'expect:修改成功'
        self.log.info(text)
        ...
        text = '---step4:查询修改后的值;expect:参数值不变---'
        self.log.info(text)
        sql_cmd = self.pri_sh.execut_db_sql('show hba_file;')
        self.log.info(sql_cmd)
        self.assertEqual(self.pg_file, sql_cmd.splitlines()[-2].strip(),
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0025finish-')
