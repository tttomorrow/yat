...
"""
Case Type   : GUC
Case Name   : hba_file参数使用gs_guc set设置为不存在目录
Description :
        1.查询hba_file默认值
        2.hba_file参数使用gs_guc set 设置为不存在路径并重启数据库
        3.恢复参数值并查询
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0021start-')
    ...
    def test_guc(self):
        ...
        text = '---step2:使用gs_guc set设置hba_file为不存在目录并重启;' \
               'expect:重启失败'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.pg_file, sql_cmd.splitlines()[-2].strip(),
                      '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0021finish-')
