...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数data_directory为问题路径
             （如路径分隔符为\）
Description :
        1.查询data_directory默认值
        2.创建测试文件
        3.使用ALTER SYSTM SET修改数据库参数data_directory为问题路径
        （如路径分隔符为\）
        4.kill -9
        5.恢复参数值
        6.连接数据库执行ddl操作
...
class GUC(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0010 start-')
        ...
        self.new_dn1 = os.path.join(self.file_path, '\\testdir')
        self.tb_name = "tb_guc_filelocation_case0010"
    ...
    def test_data_directory(self):
        ...
        text = '---step2:创建测试文件夹;expect:创建成功---'
        self.log.info(text)
        ...
        text = '---step3使用ALTER SYSTM SET修改数据库参数data_directory' \
               '为问题路径（如路径分隔符为\）;expect:重启失败---'
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, drop_cmd,
                      '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0010 finish-')
