...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数hba_file为问题路径
Description :
        1.查询hba_file默认值
        2.创建测试文件
        3.使用ALTER SYSTEM SET修改数据库参数hba_file为问题路径并重启数据库
        4.清理环境
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0026start-')
    ...
    def test_guc(self):
        ...
        text = '---step3:在gsql中设置数据库级别hba_file为不存在路径并重启;' \
               'expect:重启失败---'
    ...
    def tearDown(self):
        ...
        self.assertIn(self.pg_file, sql_cmd.splitlines()[-2].strip(),
                      '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0026finish-')
