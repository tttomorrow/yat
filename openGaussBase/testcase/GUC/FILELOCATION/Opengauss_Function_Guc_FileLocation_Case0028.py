...
"""
Case Type   : GUC
Case Name   : hba_file参数使用gs_guc reload设置为为非字符类型
Description : 
              1、查看hba_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c hba_file
              2、使用设置s_guc set设置hba_file为12345
              gs_guc reload -D {cluster/dn1} -c "hba_file=12345"
Expect      :
              1、显示默认值；
              2、参数修改失败；
History     :
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0028开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("hba_file参数使用gs_guc reload设置为非字符类型")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0028完成==')
