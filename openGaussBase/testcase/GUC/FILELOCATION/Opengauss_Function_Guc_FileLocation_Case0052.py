...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数external_pid_file
Description :
        1.查询external_pid_file默认值
        2.创建测试文件
        3.使用ALTER SYSTEM SET修改数据库参数external_pid_file为自定义路径并
        重启数据库
        4.查询参数修改后的值
        5.清理环境
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0052start-')
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0052finish-')
