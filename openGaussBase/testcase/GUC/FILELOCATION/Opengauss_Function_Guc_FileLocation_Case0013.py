...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改参数data_directory为非字符类型，
Description : 
              1、查看data_directory默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c data_directory
              2、使用ALTER SYSTM SET修改数据库参数data_directory;
              为非字符类型123456
              ALTER SYSTEM set data_directory to 123456;
              3、重启使其生效；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0013开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0013完成==')
