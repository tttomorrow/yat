...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set修改参数data_directory为非字符类型，观察预期结果；
Description :
              1、查看data_directory默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c data_directory
              2、使用设置s_guc set设置data_directory为12345
              gs_guc set -D {cluster/dn1} -c "data_directory=12345"
Expect      : 
              1、显示默认值；
              2、参数修改失败；
History     :
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0011开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("gs_guc set设置data_directory为非字符类型12345")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0011完成==')
