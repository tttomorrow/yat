...
"""
Case Type   : GUC
Case Name   : hba_file参数使用gs_guc set设置
Description :
        1.查询hba_file默认值
        2.创建测试目录,并将hba_file目录拷贝至测试目录下
        3.hba_file参数使用gs_guc set设置为自定义路径并重启数据库
        4.主节点查询hba_file参数值
        5.清理环境
Expect      :
...
class FilePosition(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0017start-')
    ...
    def test_guc(self):
        ...
        text = '---step3:使用gs_guc set设置pg_hba.conf为自定义路径并重启;' \
               'expect:修改成功'
        self.log.info(text)
        result = self.pri_sh.execute_gsguc('set',
                                           self.constant.GSGUC_SUCCESS_MSG,
                                           f"hba_file='{self.file_path}"
                                           f"/pg_hba.conf'")
        ...
        text = '---step4:查询修改后的值;expect:修改成功---'
        self.log.info(text)
        sql_cmd = self.pri_sh.execut_db_sql('show hba_file;')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.pg_file, sql_cmd.splitlines()[-2].strip(),
                      '执行失败:' + text)
        self.log.info(
            '-Opengauss_Function_Guc_FileLocation_Case0017finish-')
