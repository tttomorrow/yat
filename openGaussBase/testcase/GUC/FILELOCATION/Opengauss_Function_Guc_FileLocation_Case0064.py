...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改参数external_pid_file为空值
Description :
              1、查看external_pid_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c external_pid_file
              2、 使用ALTER SYSTEM SET修改数据库参数external_pid_file为空值
              ALTER SYSTEM SET external_pid_file to ;
              3、重启使其生效，观察预期结果
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0064开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置external_pid_file为空值")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0064完成==')
