...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改参数hba_file为空值
Description :
              1、查看hba_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c hba_file
              2、使用ALTER SYSTEM SET修改数据库参数hba_file为空值
              ALTER SYSTEM SET hba_file to ;
              3、重启使其生效，观察预期结果；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_FileLocation_Case0032开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_FileLocation_Case0032完成==')
