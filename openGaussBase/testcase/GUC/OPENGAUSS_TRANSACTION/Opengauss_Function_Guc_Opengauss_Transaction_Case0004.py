...
"""
Case Type   : GUC
Case Name   : 修改transaction_read_only为'abc'，观察预期结果；
Description :
    1、查询transaction_read_only默认值；
    show transaction_read_only;
    2、修改transaction_read_only为'abc'，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c "transaction_read_only='abc'"
...
class Guctestcase(unittest.TestCase):
    ...
    def test_guc(self):
        LOGGER.info("查询transaction_read_only 期望：默认值on")
        sql_cmd = COMMONSH.execut_db_sql("show transaction_read_only;")
        LOGGER.info(sql_cmd)
        self.assertIn("off", sql_cmd)

        LOGGER.info("方式一修改transaction_read_only为'abc'，"
                    "重启使其生效，期望：设置成功")
        result = COMMONSH.execute_gsguc("set",
                                        self.constant.GSGUC_SUCCESS_MSG,
                                       "transaction_read_only='abc'")
