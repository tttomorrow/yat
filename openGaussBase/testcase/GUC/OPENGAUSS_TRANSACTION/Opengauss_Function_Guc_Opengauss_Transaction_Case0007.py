...
Description :
    ...
    gs_guc set -D {cluster/dn1} -c "allow_concurrent_tuple_update=on"
...
class Guctestcase(unittest.TestCase):
    ...
    def test_guc(self):
        ...
        LOGGER.info(sql_cmd)
        self.assertIn("on", sql_cmd.split("\n")[-2].strip())

        LOGGER.info("方式三修改allow_concurrent_tuple_update为off，"
                    "重启使其生效，期望：设置成功")
        result = COMMONSH.execut_db_sql('''set allow_concurrent_tuple_update
            to off;show allow_concurrent_tuple_update;
            ''')
        LOGGER.info(result)
