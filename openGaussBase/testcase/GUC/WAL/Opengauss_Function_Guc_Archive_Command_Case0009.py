...
Expect      :
    ...
    5.归档命令设置成功,执行select pg_switch_xlog()，不产生归档日志
    6.删除归档日志路径
    7.恢复默认值
History     : 
"""
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0009_开始----")
        ...
        self.sh_path1 = os.path.join(
            os.path.dirname(macro.DB_INSTANCE_PATH), 'og1.sh')
        self.sh_path2 = os.path.join(
            os.path.dirname(macro.DB_INSTANCE_PATH), 'og2.sh')
    ...
    def test_guc(self):
        ...
        self.log.info("---4.2将归档命令写入脚本pg.sh---")
        echo_cmd = f'echo "test ! -f {self.archive_path}/\$2 && cp ' \
            f'\$1 {self.archive_path}/\$2">{self.sh_path1};'
        self.log.info(echo_cmd)
        echo_msg = self.userNode.sh(echo_cmd).result()
        self.log.info(echo_msg)
        self.log.info("---4.3配置参数---")
        guc_cmd = f'''source {macro.DB_ENV_PATH};
                   gs_guc set -N {hostname} \
                   -I all -c "archive_command ='sh \
                   {self.sh_path2} %p %f'";
                    '''
        self.log.info(guc_cmd)
        guc_msg = self.userNode.sh(guc_cmd).result()
        ...
        text = '-------step5:查看归档命令是否设置成功,并触发归档:expect:不生成归档日志--------'
        self.log.info(text)
        alter_cmd = f'show archive_command;select pg_switch_xlog();'
        ...
        text = '----查看是否生成归档日志----'
        self.log.info(text)
        ls_cmd = f'''ls  {self.archive_path};'''
        self.log.info(ls_cmd)
        ls_msg = self.userNode.sh(ls_cmd).result()
        self.log.info(ls_msg)
        self.assertEqual('', ls_msg, '执行失败:' + text)

    def tearDown(self):
        text = '----step6:删除归档目录;expect:删除成功----'
        self.log.info(text)
        clear_cmd = f'''source {macro.DB_ENV_PATH};
                                rm -rf {self.archive_path};
                                rm -rf {self.sh_path1};
                                '''
        self.log.info(clear_cmd)
        clear_msg = self.userNode.sh(clear_cmd).result()
        self.log.info(clear_msg)
        text = '----step7:恢复默认值;expect:恢复成功----'
        self.log.info(text)
        ...
        self.assertEqual(self.default_value4, self.default_value2,
                         '执行失败:' + text)
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0009_结束----")
