...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0008_开始----")
        self.pri_sh = CommonSH('PrimaryDbUser')
        self.userNode = Node('dbuser')
        self.constant = Constant()
        self.com = Common()
        self.archive_path1 = os.path.join(
            os.path.dirname(macro.DB_INSTANCE_PATH), 'archive1')
        self.archive_path2 = os.path.join(
            os.path.dirname(macro.DB_INSTANCE_PATH), 'archive2')
    ...
    def test_guc(self):
        ...
        mkdir_cmd = f'''if [ ! -d "{self.archive_path1}" ]
                                then
                                    mkdir {self.archive_path1}
                                fi'''
        self.log.info(mkdir_cmd)
        mkdir_result = self.userNode.sh(mkdir_cmd).result()
        self.log.info(mkdir_result)
        self.log.info("---4.2将归档命令写入脚本---")
        echo_cmd = f'echo "test ! -f {self.archive_path2}\$2 ' \
            f'&& cp \$1 {macro.DB_INSTANCE_PATH}/\$2">' \
            f'{self.sh_path};'
        ...
        text = '----查看是否生成归档日志----'
        self.log.info(text)
        ls_cmd = f'''ls  {self.archive_path1};'''
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.default_value4, self.default_value2,
                         '执行失败:' + text)
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0008_结束----")