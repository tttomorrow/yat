...
"""
Case Type   : guc参数
Case Name   : 修改参数synchronous_commit为on，并校验其预期结果
Description :
...
class Guc(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Guc_WAL_Case0006_开始---')
        self.st_root = Node('Standby1Root')
        self.sh_user = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.com = Common()
        self.t_name = 't_guc_06'
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.recovery_value, self.default_value,
                         '执行失败:' + text)
        self.log.info('---Opengauss_Function_Guc_WAL_Case0006_结束---')
