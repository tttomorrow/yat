...
"""
Case Type   : guc参数
Case Name   : 修改参数synchronous_commit为off，并校验其预期结果
Description :
    1、查看synchronous_commit默认值
    2、修改synchronous_commit为off，重启使其生效
...
class Guc(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Guc_WAL_Case0007_开始---')
        self.st_root = Node('Standby1Root')
        self.sh_user = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.com = Common()
        self.t_name = 't_guc_07'
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.recovery_value, self.default_value,
                         '执行失败:' + text)
        self.log.info('---Opengauss_Function_Guc_WAL_Case0007_结束---')
