...
"""
Case Type   : 服务端工具
Case Name   : 归档命令包含多条shell语句（归档命令写在脚本中）
Description :
    1.修改配置文件将日志归档模式打开
    2.重启数据库使参数设置生效:gs_om -t restart;
    3.查看归档日志模式
    4.配置归档路径和参数
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0004_开始----")
    ...
    def test_guc(self):
        ...
        guc_msg = self.userNode.sh(guc_cmd).result()
        self.log.info(guc_msg)
        self.log.info("----4.4.重启数据库使配置参数生效----")
        result = self.pri_sh.restart_db_cluster()
        self.log.info(result)
        result = self.pri_sh.get_db_cluster_status('detail')
        self.log.info(result)
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.default_value4, self.default_value2,
                         '执行失败:' + text)
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0004_结束----")
