...
"""
Case Type   : GUC
Case Name   : 1主2备 主机不开启归档，2同步/异步备机开启归档模式 期望：2备机按照archive_timeout归档成功
Description :
    1.主备查询参数默认值
    ...
    gs_guc reload -N all -I all -c "synchronous_commit=off"
    8.查看主备归档路径,等待30后再次查看主备归档路径
    ll /home/$user/xlog
    select pg_sleep(30);
    ll /home/$user/xlog
    9.1主2备恢复并清理环境
...
Expect      :
    ...
    2.创建归档地址成功
    3.设置成功，重启成功
    4.主机不归档 2备机归档成功 归档路径下有归档文件生成
    5.sleep成功
    6.主机不归档 2备机归档成功 归档路径xlog数量+1
    7.重启成功
    8.主机不归档 2备机归档成功 归档路径下有归档文件生成,30s后2备归档路径xlog数量+1
    8.清理环境
History     :
"""
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0094 start==")
    ...
    def test_standby_xlog(self):
        ...
        self.set_guc("synchronous_standby_names", "'dn_6002,dn_6003'",
        ...
        sql_cmd = COMMONSH.execut_db_sql("show synchronous_standby_names;"
                                         ...
                                         "show archive_dest;")
        LOGGER.info(sql_cmd)
        self.assertIn("dn_6002,dn_6003", sql_cmd)
        ...
        LOGGER.info("查看主机归档")
        result = int(self.userNode.sh(sql).result())
        LOGGER.info(result)
        self.assertEqual(0, result)

        LOGGER.info("步骤7：设置2备机为异步")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status("detail")
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0094 finish==")
