...
"""
Case Type   : GUC
Case Name   : 修改wal_log_hints为off，观察预期结果；
Description :
    1、查询wal_log_hints默认值；
    show wal_log_hints;
    2、修改wal_log_hints为off，重启使其生效，并校验其预期结果；
    gs_guc set -D /openGauss/zyn1026_gauss/cluster/dn1 -c "wal_log_hints=off"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------Opengauss_Function_Guc_WAL_Case0034开始执行-----------------------------")
    ...
    def test_guc_wal(self):
        ...
        logger.info("方式一修改wal_log_hints为off，重启使其生效，期望：设置成功")
        sql_cmd = f'''source {self.DB_ENV_PATH};gs_guc set -D {self.DB_INSTANCE_PATH} -c "wal_log_hints=off"'''
        logger.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        logger.info("-------------------------Opengauss_Function_Guc_WAL_Case0034执行结束---------------------------")