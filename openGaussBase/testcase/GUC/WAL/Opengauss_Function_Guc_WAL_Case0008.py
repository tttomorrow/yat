...
"""
Case Type   : guc参数
Case Name   : 修改参数synchronous_commit为local，并校验其预期结果
Description :
    1、查看synchronous_commit默认值
    2、修改synchronous_commit为local，重启使其生效
...
Expect      :
    1、显示默认值
    2、参数修改成功，重启生效
    3、主机创建表成功，备机kill进程成功，主机插入数据后系统返回插入成功信息
    4、恢复默认值成功
    5、重启数据库成功
History     :
"""
...
class Guc(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Guc_WAL_Case0008_开始---')
        self.st_root = Node('Standby1Root')
        self.sh_user = CommonSH('PrimaryDbUser')
        self.constant = Constant()
        self.com = Common()
        self.t_name = 't_guc_08'
    ...
    def test_guc(self):
        ...
        self.assertIn('local', sql_cmd, '执行失败:' + text)

        text = '--step3.主机创建表，同时kill掉备机数据库进程，主机向表中插入数据;' \
               'expect:主机创建表成功，备机kill进程成功，主机插入数据后系统返回插入成功信息--'
        self.log.info(text)
        text = '--step3.1.主机创建表--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.recovery_value, self.default_value,
                         '执行失败:' + text)
        self.log.info('---Opengauss_Function_Guc_WAL_Case0008_结束---')
