...
"""
Case Type   : GUC
Case Name   : 1主2备 主机开启归档，1同步备机开启归档 执行select pg_switch_xlog(); 期望：1主1备归档成功
Description :
    1.1主2备创建xlog归档地址
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0091 start==")
    ...
    def test_standby_xlog(self):
        ...
        LOGGER.info("步骤2+3：主机+备1设置开启归档")
        test_com_list = [COMMONSH, self.s_com1]
        ...
        test_node_list = [self.userNode, self.s_node1]
        ...
        for node in test_node_list:
            ...
            for j in range(60):
                result = COMMONSH.execut_db_sql("select pg_is_in_recovery()")
                LOGGER.info(result)
                if "f\n" in result:
                    result = COMMONSH.execut_db_sql("select pg_switch_xlog()")
                    LOGGER.info(result)
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status("detail")
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0091 finish==")
