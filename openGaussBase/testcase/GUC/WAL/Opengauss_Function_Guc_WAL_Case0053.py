...
Description :
    ...
    2、修改enable_double_write为test,'test'、9999999999等，期望：合理报错
    gs_guc set -D {cluster/dn1} -c "enable_double_write=test";
    gs_guc set -D {cluster/dn1} -c "enable_double_write='test'";
    gs_guc set -D {cluster/dn1} -c "enable_double_write=9999999999";
    3、恢复默认值 无需恢复
Expect      :
    1、查看enable_double_write默认值 期望：on；
    2、修改enable_double_write为test,'test'、9999999999等，期望：合理报错
...
class Guctestcase(unittest.TestCase):
    ...
    def test_guc_wal(self):
        ...
        logger.info("-----------修改enable_double_write为test,'test'、9999999999等，期望：合理报错-------------")
        ...
        logger.info("-----------修改enable_double_write为9999999999，期望：修改失败-------------")
        result = commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'enable_double_write=9999999999')
        self.assertFalse(result)
        sql_cmd = commonsh.execut_db_sql(f'''show enable_double_write;''')
        logger.info(sql_cmd)
        self.assertIn(self.Constant.OPEN_STATUS_MSG[0], sql_cmd)
