...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0003_开始----")
    ...
    def test_guc(self):
        ...
        self.log.info("---4.2.配置参数---")
        guc_cmd = f'''source {macro.DB_ENV_PATH};
            gs_guc reload -N {hostname} -I all -c \
            "archive_command = 'test ! -f {self.archive_path}/%f && \
            cp  %p {self.archive_path}/%f'";'''
        self.log.info(guc_cmd)
        guc_msg = self.userNode.sh(guc_cmd).result()
        self.log.info(guc_msg)
        self.log.info(
            "-----4.3.查看归档命令是否设置成功并触发归档-----")
        ...
        self.assertIn('1 row', excute_msg, '执行失败:' + text)

        text = '----step5:查看是否生成归档日志;expect:无法生成归档日志----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info(text)
        clear_cmd = f'source {macro.DB_ENV_PATH}; rm -rf {self.archive_path};'
        ...
        self.assertEqual(self.default_value4, self.default_value2,
                         '执行失败:' + text)
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0003_结束----")
