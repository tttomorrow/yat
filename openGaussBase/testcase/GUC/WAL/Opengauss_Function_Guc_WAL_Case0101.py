...
Case Name   : 1主2备机potential 时执行xlog归档 期望：归档成功
Description :
    1.主备查询参数默认值
    ...
    3.设置同步备 开启归档设置
    主机执行
    gs_guc reload -D {dn1} -c "synchronous_standby_names='dn_6003,dn_6002'"
    备机1执行
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0101 start==")
    ...
    def test_standby_xlog(self):
        ...
        result = COMMONSH.execute_gsctl("query", "Potential", get_detail=True)
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status("detail")
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0101 finish==")
