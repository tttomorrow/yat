...
"""
Case Type   : GUC
Case Name   : 归档过程中 执行build 期望：归档成功
Description :
    1.主备查询参数默认值
    ...
    5.同时执行日志切换 查看归档路径 归档文件
    select pg_switch_xlog();
    ll /home/$user/xlog
    6.起线程对备2执行build
    gs_ctl build -D dn1 -b full
    7.同时执行日志切换 查看归档路径 归档文件
    select pg_switch_xlog();
    ll /home/$user/xlog
    8.恢复环境
    rm -rf /home/$user/xlog
...
Expect      :
    ...
    3.设置成功，重启成功
    4.build成功
    5.pg_switch_xlog后备1归档文件不+1
    6.build成功
    7.pg_switch_xlog后备1归档文件+1
    8.恢复环境
History     :
"""
import time
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0126 start==")
    ...
    def count_xlog_increase(self, node, sql, com=COMMONSH):
        for i in range(3):
    ...
    def test_standby_xlog(self):
        ...
        self.set_guc("synchronous_standby_names", "''", "reload",
                     COMMONSH, True)
        self.set_guc("synchronous_commit", "on", "reload", COMMONSH)
        ...
        LOGGER.info("步骤4:build备1")
        session = ComThread(self.s_com1.execute_gsctl,
                            args=("build", "server started", "-b full"))
        ...
        LOGGER.info("步骤6:build备2")
        session = ComThread(self.s_com2.execute_gsctl,
                            args=("build", "server started", "-b full"))
        session.setDaemon(True)
        session.start()
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status("detail")
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0126 finish==")
