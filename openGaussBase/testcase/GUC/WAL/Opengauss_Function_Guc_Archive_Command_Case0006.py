...
"""
Case Type   : 服务端工具
Case Name   : 多条shell语句换行写入脚本中，触发归档并产生归档日志
Description :
    1.修改配置文件将日志归档模式打开
    2.重启数据库使参数设置生效
    3.查看归档日志模式
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0006_开始----")
    ...
    def test_guc(self):
        ...
        echo_cmd = f'''echo 'cp --remove-destination $1 \
            {self.archive_path}/$2\n
            mkdir {self.temp_path1}/arch1\n
            mv {self.temp_path1}/arch1 {self.temp_path1}/arch2'\
            >{self.sh_path}'''
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.default_value4, self.default_value2,
                         '执行失败:' + text)
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0006_结束----")
