...
"""
Case Type   : GUC
Case Name   : 1主1备1级联 在级联备执行switchover切换备和级联备 备+级联备开启归档设置 期望：备机归档成功 级联备不归档
Description :
    1.主备查询参数默认值
    ...
    3.级联备执行执行switchover 进行备/级联备切换
    gs_ctl switchover -D dn1
    gs_om -t refreshconf
    4.设置新主机和现备1+2 开启归档设置
...
Expect      :
    1.显示默认值
    2.1主1备1级联创建xlog归档地址成功
    3.switchover成功
    4.开启归档设置成功
    5.pg_switch_xlog后备1归档文件+1 级联备不归档
    6.清理环境
History     :
"""
import time
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0124 start==")
        ...
        self.test_node_list = [self.s_node1, self.s_node2]
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status("detail")
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0124 finish==")
