...
"""
Case Type   : 服务端工具
Case Name   : 参数值设为remote_apply，rollback前停止备机，检查rollback后的主机状态；再启动备机
Description :
    ...
    5.主机执行rollback,无需等待，返回成功
    rollback;
    6.启动备机
    gs_ctl start  -D /xxx/dn1/ -M standby
    7.检查主机和备机的日志，日志文件大小是否一致，日志文件路径：
    /xxx/dn1/pg_xlog
    查询表，主备机表数据是否一致
...
class Guc(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Guc_WAL_Case0087_开始---')
    ...
    def test_guc(self):
        ...
        self.log.info(text)
        result1 = self.sta_com.execute_gsctl("stop", "server stopped")
        self.log.info(result1)
        result2 = self.sta_com.execute_gsctl("start", "server started",
                                             "-M standby")
        self.log.info(result2)
        self.assertTrue(result2, '执行失败' + text)

        text = '--step3.主机开启事务并建表并插入数据,之后rollback;expect:操作成功--'
        self.log.info(text)
        ...
        text = '--step4.停止备机;expect:备机停止成功--'
        self.log.info(text)
        result = self.sta_com.execute_gsctl("stop", "server stopped")
        self.log.info(result)
        self.assertTrue(result, '执行失败' + text)

        text = '--step5.主机执行rollback,;expect:查询表信息不存在--'
        self.log.info(text)
        ...
        text = '-----step6.1.启动备机;except:备机启动成功-----'
        self.log.info(text)
        result = self.sta_com.execute_gsctl("start", "server started",
                                            "-M standby")
        self.log.info(result)
        self.assertTrue(result, '执行失败' + text)
        text = '-----step6.2.获取线程执行结果;except:获取成功-----'
        self.log.info(text)
        ...
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, thread_result,
                      '执行失败:' + text)
        self.assertIn(self.constant.ROLLBACK_MSG, thread_result,
                      '执行失败:' + text)

        text = '--step7.1.检查主备表信息;expect:表信息一致:表不存在--'
        self.log.info(text)
        sql_pri = self.pri_com.execut_db_sql(
            f'select count(*) from {self.tb_name};')
        self.log.info(sql_pri)
        ...
        text = '--step7.2.检查主备日志信息;expect:日志信息一致--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        text = '--step9.2重启数据库;expect:重启数据库成功--'
        self.log.info(text)
        ...
        self.assertEqual(self.recovery_value, self.default_value,
                         '执行失败:' + text)
        self.log.info('---Opengauss_Function_Guc_WAL_Case0087_结束---')
