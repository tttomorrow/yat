...
"""
Case Type   : 服务端工具
Case Name   : 使用gs_guc set方式修改参数但不重启数据库，设置未生效，不产生归档日志
Description :
    1.修改配置文件将日志归档模式打开
    2.重启数据库使参数设置生效
    3.查看归档日志模式
...
Expect      :
    ...
    5.归档命令设置成功,执行select pg_switch_xlog()，日志归档失败
    6.删除归档日志
    7.恢复默认值
History     :
"""
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0010_开始----")
    ...
    def test_guc(self):
        ...
        echo_cmd = f'echo "test ! -f {self.archive_path}/\$2 &&\
        cp  \$1 {self.archive_path}/\$2">{self.sh_path};'
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.default_value4, self.default_value2,
                         '执行失败:' + text)
        self.log.info(
            "----Opengauss_Function_Guc_Archive_Command_Case0010_结束----")
