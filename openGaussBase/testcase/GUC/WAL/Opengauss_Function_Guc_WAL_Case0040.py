...
"""
Case Type   : GUC
Case Name   : 修改commit_delay为20，观察预期结果；
Description :
    1、查询commit_delay默认值；
    show commit_delay;
    2、修改commit_delay为20，重启使其生效，并校验其预期结果；
    gs_guc set -D /openGauss/zyn1026_gauss/cluster/dn1 -c "commit_delay=20"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------Opengauss_Function_Guc_WAL_Case0040开始执行-----------------------------")
    ...
    def test_guc_wal(self):
        ...
        logger.info("方式一修改commit_delay为20，重启使其生效，期望：设置成功")
        sql_cmd = f'''source {self.DB_ENV_PATH};gs_guc set -D {self.DB_INSTANCE_PATH} -c "commit_delay=20"'''
        logger.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        logger.info("-------------------------Opengauss_Function_Guc_WAL_Case0040执行结束---------------------------")