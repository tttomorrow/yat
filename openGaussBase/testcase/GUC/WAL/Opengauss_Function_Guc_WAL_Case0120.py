...
"""
Case Type   : GUC
Case Name   : 1+2switchover后，新主机开启归档模式，设置1同步不开启归档 1异步开启归档 期望：异步备归档成功
Description :
    1.1主2备创建xlog归档地址
    rm -rf /home/$user/xlog
    mkdir /home/$user/xlog
    2.备1上执行switchover 进行主备切换
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        ...
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0120 start==")
    ...
    def test_standby_xlog(self):
        ...
        LOGGER.info("步骤2：执行switchover")
        result = self.s_com2.execute_gsctl("switchover",
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status("detail")
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_WAL_Case0120 finish==")
