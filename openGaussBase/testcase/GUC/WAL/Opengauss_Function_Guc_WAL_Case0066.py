...
"""
Case Type   : GUC
Case Name   : 修改recovery_redo_workers为2，观察预期结果；
Description :
    1、查询recovery_redo_workers默认值；
    show recovery_redo_workers;
    2、修改recovery_redo_workers为2，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c "recovery_redo_workers=2"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("==Opengauss_Function_Guc_WAL_Case0066开始执行==")
    ...
    def test_guc_wal(self):
        ...
        sql_cmd = COMMONSH.execut_db_sql("show recovery_redo_workers;")
        logger.info(sql_cmd)
        self.assertIn("1", sql_cmd)

        logger.info("方式一修改recovery_redo_workers为2，"
                    "重启使其生效，期望：设置成功")
    ...
    def tearDown(self):
        logger.info("恢复默认值")
        sql_cmd1 = COMMONSH.execut_db_sql("show recovery_redo_workers;")
        ...
        is_started = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in is_started or "Normal" in is_started)
        logger.info("==Opengauss_Function_Guc_WAL_Case0066执行结束==")
