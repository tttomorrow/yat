...
Description :
    ...
    2、修改commit_siblings为test,'test'、9999999999等，期望：合理报错
    gs_guc set -D {cluster/dn1} -c "commit_siblings=test";
    gs_guc set -D {cluster/dn1} -c "commit_siblings='test'";
    gs_guc set -D {cluster/dn1} -c "commit_siblings=9999999999";
    3、恢复默认值 无需恢复
Expect      :
    1、查看commit_siblings默认值 期望：5；
    2、修改commit_siblings为test,'test'、9999999999等，期望：合理报错
...
class Guctestcase(unittest.TestCase):
    ...
    def test_guc_wal(self):
        ...
        logger.info("-----------修改commit_siblings为test,'test'、9999999999等，期望：合理报错-------------")
        ...
        logger.info("-----------修改commit_siblings为9999999999，期望：修改失败-------------")
        sql_cmd = f'''source {self.DB_ENV_PATH};gs_guc reload -D {self.DB_INSTANCE_PATH} -c "commit_siblings=9999999999"'''
        logger.info(sql_cmd)
        msg = self.userNode.sh(sql_cmd).result()
        logger.info(msg)
        self.assertIn(self.Constant.SQL_WRONG_MSG[1], msg)
