...
"""
Case Type   : 服务端工具
Case Name   : 参数值设为remote_apply，abort前停止备机，检查abort后的主机状态；再启动备机
Description :
...
class Guc(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Guc_WAL_Case0090_开始---')
    ...
    def test_guc(self):
        ...
        text = '--step3.主机开启事务并建表并插入数据,之后abort;expect:操作成功--'
        self.log.info(text)
        ...
        text = '--step5.主机执行abort;expect:查询表信息不存在--'
        self.log.info(text)
        ...
        text = '--step7.检查主备表信息;expect:表信息一致--'
        self.log.info(text)
        sql_pri = self.pri_com.execut_db_sql(
            f'select count(*) from {self.tb_name};')
        self.log.info(sql_pri)
    ...
    def tearDown(self):
        ...
        text = '--step10.2重启数据库;expect:重启数据库成功--'
        self.log.info(text)
        ...
        self.assertEqual(self.recovery_value, self.default_value,
                         '执行失败:' + text)
        self.log.info('---Opengauss_Function_Guc_WAL_Case0090_结束---')
