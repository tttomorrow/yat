...
"""
Case Type   : 服务端工具
Case Name   : 参数值设为remote_apply，commit前停止备机，检查commit后的主机状态；再启动备机
Description :
...
class Guc(unittest.TestCase):
    ...
    def setUp(self):
        self.log = Logger()
        self.log.info('---Opengauss_Function_Guc_WAL_Case0084_开始---')
    ...
    def test_guc(self):
        ...
        text = '--step3.主机开启事务并建表并插入数据,之后提交;expect:操作成功--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertEqual(self.recovery_value, self.default_value,
                         '执行失败:' + text)
        self.log.info('---Opengauss_Function_Guc_WAL_Case0084_结束---')
