...
"""
Case Type   : GUC
Case Name   : 修改enable_xlog_prune为off，观察预期结果；
Description :
    1、查询enable_xlog_prune默认值；
    show enable_xlog_prune;
    2、修改enable_xlog_prune为off，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c "enable_xlog_prune=off"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("==Opengauss_Function_Guc_WAL_Case0056开始执行==")
    ...
    def test_guc_wal(self):
        ...
        sql_cmd = COMMONSH.execut_db_sql("show enable_xlog_prune;")
        logger.info(sql_cmd)
        self.assertIn(self.constant.OPEN_STATUS_MSG[0], sql_cmd)

        logger.info("方式一修改enable_xlog_prune为off，"
                    "重启使其生效，期望：设置成功")
    ...
    def tearDown(self):
        ...
        is_started = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in is_started or "Normal" in is_started)
        logger.info("==Opengauss_Function_Guc_WAL_Case0056执行结束==")
