...
"""
Case Type   : GUC
Case Name   : 设置max_active_global_temporary_table为0，观察预期结果
Description :
    1、查询max_active_global_temporary_table默认值
       show max_active_global_temporary_table;
    2、设置max_active_global_temporary_table为0,
       gs_guc set -D {cluster/dn1} -c "max_active_global_temporary_table=0"
       重启并校验其结果，show max_active_global_temporary_table;
    3、创建全局临时表;
    4、恢复默认值;
Expect      :
    1、显示默认值，1000;
    2、参数修改成功，重启数据库，校验参数值为0;
    3、创建全局临时表失败;
    4、恢复默认值成功;
History     :
"""
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        logger.info("===Opengauss_Function_Guc_Globaltemptab_Case0004开始执行===")
    ...
    def test_guc(self):
        ...
        logger.info("===修改max_active_global_temporary_table为0，期望：重启设置成功===")
        re = self.comsh.execute_gsguc('set',
                                      self.constant.GSGUC_SUCCESS_MSG,
                                      "max_active_global_temporary_table=0")
        self.assertTrue(re)
        result = self.comsh.restart_db_cluster()
        logger.info(result)

        logger.info("======期望：查询结果为修改值0======")
        sql_cmd2 = self.comsh.execut_db_sql(self.config_param)
        logger.info(sql_cmd2)
        self.assertEqual('0', sql_cmd2.splitlines()[-2].strip())
    ...
    def tearDown(self):
        ...
        status = self.comsh.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        logger.info("===Opengauss_Function_Guc_Globaltemptab_Case0004执行结束===")
