...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方式修改max_active_global_temporary_table为100，观察预期结果
Description :
    1、查询max_active_global_temporary_table默认值
       show max_active_global_temporary_table;
    2、使用gs_guc set方式修改max_active_global_temporary_table为100，
       gs_guc set -D {cluster/dn1} -c "max_active_global_temporary_table=100";
       重启并校验其结果，show max_active_global_temporary_table;
    3、创建全局临时表;
    4、恢复默认值;
Expect      :
    1、显示默认值，1000;
    2、参数修改成功，重启数据库，校验参数值为100;
    3、创建全局临时表成功;
    4、恢复默认值成功;
History     :
"""
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        logger.info("===Opengauss_Function_Guc_Globaltemptab_Case0001开始执行===")
    ...
    def test_guc(self):
        ...
        logger.info("===修改max_active_global_temporary_table为100，期望：重启设置成功===")
        re = self.comsh.execute_gsguc('set',
                                      self.constant.GSGUC_SUCCESS_MSG,
                                      "max_active_global_temporary_table=100")
        self.assertTrue(re)
        restart = self.comsh.restart_db_cluster()
        logger.info(restart)

        logger.info("======期望：查询结果为修改值100======")
        sql_cmd2 = self.comsh.execut_db_sql(self.config_param)
        logger.info(sql_cmd2)
        self.assertEqual('100', sql_cmd2.splitlines()[-2].strip())
    ...
    def tearDown(self):
        ...
        status = self.comsh.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        logger.info("===Opengauss_Function_Guc_Globaltemptab_Case0001执行结束===")
