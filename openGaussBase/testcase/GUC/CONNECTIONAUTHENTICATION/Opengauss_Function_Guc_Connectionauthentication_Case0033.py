...
"""
Case Type   : GUC
Case Name   : local_bind_address参数使用gs_guc reload设置为非字符类型
Description : 1、查看local_bind_address默认值；
              source {env}
              show local_bind_address;
              2、使用设置gs_guc reload设置local_bind_address为非字符类型
              gs_guc reload -D {cluster/dn1} -c "local_bind_address=123456"
Expect      : 1、显示默认值；
              2、修改失败；
History     :
"""
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        ...
        self.name = "Opengauss_Function_Guc_Connectionauthentication_Case0033"
    ...
    def test_guc(self):
        ...
        self.assertIn(ipadd, sql_cmd)

        self.log.info("步骤2：gs_guc reload设置local_bind_address为非字符类型")
