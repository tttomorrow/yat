...
"""
Case Type   : GUC
Case Name   : max_connections参数使用gs_guc reload设置为超边界值
Description : 
              1、查看max_connections默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c max_connections
              2、使用设置gs_guc reload设置max_connections为超边界值
              gs_guc reload -D {cluster/dn1} -c "max_connections=666666"
              3、校验是否修改成功；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0043开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置max_wal_senders设置max_connections为"
                      "超边界值，重启使其生效")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Guc_Connectionauthentication_Case0043完成==')
