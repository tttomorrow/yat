...
"""
Case Type   : GUC
Case Name   : listen_addresses参数使用gs_guc set设置（单节点与多节点）
Description :
    1、查看listen_addresses默认值
        source [环境变量路径]
        gs_guc check -D [数据库实例路径] -c listen_addresses
    2、使用设置gs_guc set设置listen_addresses，以'55.66.77.88'为例
        单机（单节点）：
        gs_guc set -D [数据库实例路径] -c "listen_addresses='55.66.77.88'"
        主备（多节点）：
        gs_guc set -N all -D [数据库实例路径] -c "listen_addresses='55.66.77.88'"
...
Expect      :
    ...
    5、恢复默认值成功；
    6、重启数据库成功
History     : 2021/2/24 修改单词拼写错误
              2021/8/18 为适配listen_addresses默认值变更，修改正则写法
"""
import re
import unittest
...
class GucSetListenAddresses(unittest.TestCase):
    def setUp(self):
        logger.info(
            "Opengauss_Function_Guc_Connectionauthentication_Case0015开始执行"
        )
    ...
    def tearDown(self):
        ...
        if self.default_ips_list:
            ...
            if not self.commonsh_list[0].restart_db_cluster():
                ...
                logger.info("数据库重启成功")
        logger.info(
            "Opengauss_Function_Guc_Connectionauthentication_Case0015执行结束"
        )
