...
Case Name   : gs_guc set修改参数listen_addresses为0.0.0.0,使用套接字和非套接字
              连接数据库
Description :
    1、查看listen_addresses默认值；
    2、设置listen_addresses参数为0.0.0.0
    3、使用套接字和非套接字连接
    4、恢复默认值
Expect      :
    1、listen_addresses默认值为监听本机TCP/IP地址
    2、修改listen_addresses参数为0.0.0.0成功
    3、使用套接字连接成功;非套接字连接失败
    4、恢复默认值成功
History     :
"""
import unittest
...
class GUC(unittest.TestCase):
    def setUp(self):
        LOGGER.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0301start-')
    ...
    def test_listen_addresses(self):
        ...
        LOGGER.info("步骤2:设置listen_addresses参数为0.0.0.0并重启数据库")
    ...
    def tearDown(self):
        ...
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0301finish-')
