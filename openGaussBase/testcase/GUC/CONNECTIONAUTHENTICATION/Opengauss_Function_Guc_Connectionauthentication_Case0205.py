...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数ssl_cert_file为其他数据类型
Description : 1、查看ssl_cert_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_cert_file
              2、使用ALTER SYSTM SET修改数据库参数ssl_cert_file为其他数据类型;
              ALTER SYSTEM set ssl_cert_file to 12345;
Expect      : 1、显示默认值；
              2、参数修改成功；
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0205开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0205完成==')
