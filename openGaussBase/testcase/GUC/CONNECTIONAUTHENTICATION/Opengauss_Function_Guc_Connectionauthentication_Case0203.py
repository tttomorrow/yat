...
"""
Case Type   : GUC
Case Name   : ssl_cert_file参数使用gs_guc set设置为其他数据类型
Description : 1、查看ssl_cert_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_cert_file
              2、使用设置gs_guc set设置ssl_cert_file
              gs_guc set -D {cluster/dn1} -c "ssl_cert_file=12345"
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0203开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0203完成==')
