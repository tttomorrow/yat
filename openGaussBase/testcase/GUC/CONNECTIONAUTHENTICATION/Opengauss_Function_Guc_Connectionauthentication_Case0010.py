...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改参数port为超边界值，观察预期结果；
Description : 1、查看port默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c port
              2、使用ALTER SYSTEM SET修改数据库参数port为超边界值;
              ALTER SYSTEM set port to 77777;
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0010.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        self.log.info("使用aLTER SYSTEM SET设置port为超边界值")
        sql_cmd = COMMONSH.execut_db_sql(
            f'''ALTER SYSTEM set port to '77777';''')
    ...
    def tearDown(self):
        ...
        self.log.info('==Connectionauthentication_Case0010.py finish==')
