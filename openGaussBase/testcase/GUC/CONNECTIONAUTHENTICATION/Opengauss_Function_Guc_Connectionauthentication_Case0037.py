...
"""
Case Type   : GUC
Case Name   : 使用alter system set修改参数local_bind_address为空值
Description : 1、查看local_bind_address默认值；
              source {env}
              show local_bind_address;
              2、使用alter system set修改数据库参数local_bind_address;为空值
              alter system set  local_bind_address to ' ';
              3、重启数据库使其生效，观察预期结果
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        ...
        self.name = "Opengauss_Function_Guc_Connectionauthentication_Case0037"
    ...
    def test_guc(self):
        ...
        self.log.info("使用alter system set设置local_bind_address为空值")

        sql_cmd = COMMONSH.execut_db_sql(f"alter system "
            f"set local_bind_address to ' ';")
        self.log.info(sql_cmd)
