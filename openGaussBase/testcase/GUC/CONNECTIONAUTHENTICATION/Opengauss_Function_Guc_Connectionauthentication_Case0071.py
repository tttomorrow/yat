...
"""
Case Type   : GUC
Case Name   : sysadmin_reserved_connections参数gs_guc reload设置为其他数据类型
Description : 1、查看sysadmin_reserved_connections默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c sysadmin_reserved_connections
              2、gs_guc reload设置sysadmin_reserved_connections为其他数据类型
              gs_guc reload -D {cluster/dn1}
              -c "sysadmin_reserved_connections='test'"
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0071.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        self.log.info("设置sysadmin_reserved_connections，观察预期结果")
        result = COMMONSH.execute_gsguc(
            'reload', self.constant.TPCC_ERROR,
            f'sysadmin_reserved_connections=\'test\'')
        self.assertTrue(result)
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Connectionauthentication_Case0071.py finish==')
