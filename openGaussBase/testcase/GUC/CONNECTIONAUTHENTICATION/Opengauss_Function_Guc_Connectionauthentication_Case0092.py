...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数unix_socket_directory为空值
Description :
              ...
              ALTER SYSTEM set unix_socket_directory to ' ';
              4、重启使其生效。
Expect      : 
              1、显示默认值；
              2、创建成功；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0092开始==')
    ...
    def test_startdb(self):
        ...
        sql_cmd = COMMONSH.execut_db_sql(
            f'''ALTER SYSTEM set unix_socket_directory to ' ';''')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Guc_Connectionauthentication_Case0092完成==')
