...
"""
Case Type   : GUC
Case Name   : sysadmin_reserved_connections参数使用gs_guc reload设置为超边界值
Description : 1、查看sysadmin_reserved_connections默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c sysadmin_reserved_connections
              2、使用gs_guc reload设置sysadmin_reserved_connections为超边界值
              gs_guc reload -D {cluster/dn1}
              -c "sysadmin_reserved_connections='666666'"
              3、重启数据库使其生效
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0074.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        self.log.info("设置sysadmin_reserved_connections为666666")
        result = COMMONSH.execute_gsguc(
            'reload', self.constant.GSGUC_SUCCESS_MSG,
            'sysadmin_reserved_connections=\'666666\'')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Connectionauthentication_Case0074.py finish==')
