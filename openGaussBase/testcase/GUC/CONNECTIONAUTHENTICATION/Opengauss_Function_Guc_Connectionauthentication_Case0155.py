...
"""
Case Type   : GUC
Case Name   : 修改指定数据库，用户，会话级别的参数session_timeout
Description : 1、查看session_timeout默认值；
              ...
              alter database postgres set session_timeout to 601s;
              alter user env109 set session_timeout to 601s;
              set session_timeout to 601s;
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0155开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置session_timeout，校验预期结果")
        sql_cmd = COMMONSH.execut_db_sql(
            f'''alter database postgres set session_timeout to 601s;''')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0155完成==')
