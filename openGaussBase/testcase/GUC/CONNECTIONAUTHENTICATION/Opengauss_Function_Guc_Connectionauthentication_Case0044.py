...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数max_connections为超边界值
Description : 1、查看max_connections默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c max_connections
              2、使用ALTER SYSTEM SET修改数据库参数max_connections为超边界值;
              ALTER SYSTEM set max_connections to 666666;
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0044.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        result = COMMONSH.execut_db_sql('show max_wal_senders;')
        self.log.info(result)
        self.default_value = result.splitlines()[-2].strip()
        self.log.info(self.default_value)

        self.log.info("设置max_wal_senders参数，"
                      "并设置max_connections为超边界值")
        ...
        sql_cmd = COMMONSH.execut_db_sql(
            f'''ALTER SYSTEM set max_connections to '666666';''')
    ...
    def tearDown(self):
        ...
        self.log.info('==Connectionauthentication_Case0044.py finish==')
