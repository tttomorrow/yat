...
Case Name   : gs_guc set修改参数max_connections值小于等于max_wal_senders值
Description :
...
class GUC(unittest.TestCase):
    def setUp(self):
        LOGGER.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0307start-')
    ...
    def test_listen_addresses(self):
        ...
        LOGGER.info("步骤3:设置max_connections为16并重启数据库")
        sql_cmd = COMMONSH.execute_gsguc("set",
                                         self.constant.GSGUC_SUCCESS_MSG,
                                         "max_connections=16")
        LOGGER.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        LOGGER.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0307finish-')
