...
"""
Case Type   : GUC
Case Name   : max_connections参数使用gs_guc reload设置为空值
Description : 1、查看max_connections默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c max_connections
              2、使用设置gs_guc reload设置max_connections为空值
              gs_guc reload -D {cluster/dn1} -c "max_connections=' '"
              gs_guc reload -N all  -D {cluster/dn1} -c "max_connections=' '"
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0049.py start==')
        self.rootNode = Node()
    ...
    def tearDown(self):
        ...
        self.log.info('==Connectionauthentication_Case0049.py finish==')
