...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数max_connections为空值
Description : 1、查看max_connections默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c max_connections
              2、使用ALTER SYSTEM SET修改数据库参数max_connections为空值;
              ALTER SYSTEM set max_connections to ' ';
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0050.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        self.log.info("设置max_wal_senders参数，"
                      "并设置max_connections为空值")
        sql_cmd = COMMONSH.execut_db_sql(
            f'''ALTER SYSTEM set max_connections to ' ';''')
    ...
    def tearDown(self):
        ...
        self.log.info('==Connectionauthentication_Case0050.py finish==')
