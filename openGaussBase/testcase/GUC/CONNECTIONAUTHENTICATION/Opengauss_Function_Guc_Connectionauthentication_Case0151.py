...
"""
Case Type   : GUC
Case Name   : auth_iteration_count参数使用gs_guc reload设置为超边界值
Description : 1、查看auth_iteration_count默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c auth_iteration_count
              2、使用设置gs_guc reload设置auth_iteration_count为超边界值
              gs_guc reload -D {cluster/dn1}
              -c "auth_iteration_count='134217729'"
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0151开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置auth_iteration_count为超边界值，重启使其生效")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0151完成==')
