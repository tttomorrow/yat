...
Case Name   : 修改指定数据库，用户，会话级别的参数application_name为其他数据
              类型
Description :
        1.查询application_name默认值
        2.在gsql中分别设置数据库、用户、会话、级别application_name为其他数据
        类型
        3.清理环境
Expect      :
        1.显示默认值gsql
...
class ClientConnection(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0122start-')
    ...
    def test_application_name(self):
        ...
        sql_cmd = commonsh.execut_db_sql("set application_name to 12345;"
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0122finish-')
