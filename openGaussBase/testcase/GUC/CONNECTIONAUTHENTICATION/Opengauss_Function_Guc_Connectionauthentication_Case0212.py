...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数ssl_key_file为其他数据类型
Description : 1、查看ssl_key_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_key_file
              2、使用ALTER SYSTM SET修改数据库参数ssl_key_file为其他数据类型;
              ALTER SYSTEM set ssl_key_file to 12345;
Expect      : 1、显示默认值；
              2、参数修改成功；
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0212开始==')
    ...
    def test_startdb(self):
        ...
        sql_cmd = COMMONSH.execut_db_sql('show ssl_key_file;')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0212完成==')
