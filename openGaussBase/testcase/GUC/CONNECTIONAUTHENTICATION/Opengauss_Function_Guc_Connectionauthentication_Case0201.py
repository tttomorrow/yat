...
"""
Case Type   : GUC
Case Name   : 修改指定数据库，用户，会话级别的参数ssl_cert_file
Description : 1、查看ssl_cert_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_cert_file
              2、在gsql中分别设置数据库、用户、会话、级别ssl_cert_file；
              alter database postgres set ssl_cert_file to 'server1.crt';
              alter user env109 set ssl_cert_file to 'server1.crt';
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0201开始==')
    ...
    def test_startdb(self):
        ...
        sql_cmd = COMMONSH.execut_db_sql('''show ssl_cert_file;''')
        self.log.info(sql_cmd)
        self.default_value = sql_cmd.split("\n")[-2].strip()

        self.log.info("设置ssl_cert_file，校验预期结果")
        ...
        sql_cmd2 = COMMONSH.execut_db_sql(
            f'''set ssl_cert_file to 'server1.crt';''')
    ...
    def tearDown(self):
        ...
        gs_guc set -N all -I all -c "ssl_cert_file='{self.default_value}'"'''
        self.log.info(gucsetcmd2)
        self.db_user_node.sh(gucsetcmd2)
        COMMONSH.restart_db_cluster()
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0201完成==')
