...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数ssl_key_file
Description : 1、查看ssl_key_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_key_file
              2、使用ALTER SYSTM SET修改数据库参数ssl_key_file;
              ALTER SYSTEM set ssl_key_file to 'server1.key';
Expect      : 1、显示默认值；
              2、参数修改成功，重启后参数生效；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0209开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0209完成==')
