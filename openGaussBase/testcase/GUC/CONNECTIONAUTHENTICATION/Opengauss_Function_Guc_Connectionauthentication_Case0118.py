...
Case Name   : 方式三设置参数application_name
Description :
        1.查询application_name默认值
        2.set方法修改参数值为dn_6001
        3.创建测试用户
        4.修改用户级别参数值为dn_6001
        5.修改数据库级别参数值为dn_6001
        6.清理环境
Expect      :
        1.显示默认值gsql
        2.设置成功
...
class ClientConnection(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0118start-')
    ...
    def test_application_name(self):
        ...
        sql_cmd = commonsh.execut_db_sql("set application_name to 'dn_6001';"
                                         "show application_name ")
        self.log.info(sql_cmd)
        self.assertIn('dn_6001', sql_cmd)
    ...
    def tearDown(self):
        ...
        sql_cmd = commonsh.execut_db_sql(f'''drop user if exists test_spur0223 
            cascade;''')
        self.log.info(sql_cmd)
        sql_cmd = commonsh.execut_db_sql('''alter database postgres 
            reset application_name;''')
        self.log.info(sql_cmd)
        self.log.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0118finish-')
