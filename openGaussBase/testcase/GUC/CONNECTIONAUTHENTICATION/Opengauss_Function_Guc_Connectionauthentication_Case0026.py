...
"""
Case Type   : GUC
Case Name   : listen_addresses参数使用gs_guc reload设置为空值（单节点与多节点）
Description :
    1、查看listen_addresses默认值
        source [环境变量路径]
        gs_guc check -D [数据库实例路径] -c listen_addresses
    2、使用设置gs_guc reload设置listen_addresses为''
        单机（单节点）：
...
Expect      :
    ...
    3、show listen_addresses的值和第2步设置的值不一样，且等于默认值，说明reload设置不生效；
    4、恢复默认值成功
...
class GucReloadListenAddresses(unittest.TestCase):
    def setUp(self):
        logger.info(
            "Opengauss_Function_Guc_Connectionauthentication_Case0026开始执行")
    ...
    def tearDown(self):
        ...
        logger.info(
            "Opengauss_Function_Guc_Connectionauthentication_Case0026执行结束")
