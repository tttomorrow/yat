...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数ssl_ciphers为空值
Description : 1、查看ssl_ciphers默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_ciphers
              2、使用ALTER SYSTM SET修改数据库参数ssl_ciphers为空值;
              ALTER SYSTEM set ssl_ciphers to ' ';
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0185开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置ssl_ciphers为空值")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0185完成==')
