...
"""
Case Type   : GUC
Case Name   : 修改指定数据库，用户，会话级别的参数ssl
Description : 1、查看ssl默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl
              2、在gsql中分别设置数据库、用户、会话、级别ssl；
              alter database postgres set ssl to off;
              alter user env109 set ssl to off;
              set ssl to off;
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0168开始==')
    ...
    def test_startdb(self):
        ...
        sql_cmd = COMMONSH.execut_db_sql(
            f'''alter database postgres set ssl to off;''')
        self.log.info(sql_cmd)
        self.assertIn("ERROR", sql_cmd)
        sql_cmd1 = COMMONSH.execut_db_sql(
            f'''alter user {self.db_user_node.db_user} set ssl to off;''')
        self.log.info(sql_cmd1)
        self.assertIn("ERROR", sql_cmd1)
        sql_cmd2 = COMMONSH.execut_db_sql(f'''set ssl to off;''')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0168完成==')
