...
"""
Case Type   : GUC
Case Name   : ssl_renegotiation_limit参数使用gs_guc set设置为其他数据类型
Description : 1、查看ssl_renegotiation_limit默认值；
              ...
              gs_guc set -D {cluster/dn1} -c "ssl_renegotiation_limit='test'"
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0190开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0190完成==')
