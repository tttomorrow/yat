...
"""
Case Type   : GUC
Case Name   : ssl_renegotiation_limit参数使用gs_guc set设置为空值
Description : 1、查看ssl_renegotiation_limit默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_renegotiation_limit
              2、使用设置gs_guc set设置ssl_renegotiation_limit为空值
              gs_guc set -D {cluster/dn1} -c "ssl_renegotiation_limit=' '"
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0193开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0193完成==')
