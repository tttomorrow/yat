...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数unix_socket_group为空值
Description : 1、查看unix_socket_group默认值；
              source /opt/opengauss810/env
              gs_guc check -D /opt/opengauss810/cluster/dn1
              -c unix_socket_group
              2、使用ALTER SYSTM SET修改数据库参数unix_socket_group为空值;
              ALTER SYSTEM set unix_socket_group to ' ';
              3、重启使其生效。
Expect      : 1、显示默认值；
              2、参数修改失败；
              3、重启失败。
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0103.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        sql_cmd = COMMONSH.execut_db_sql(
            f'''ALTER SYSTEM set unix_socket_group to ' ';''')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Connectionauthentication_Case0103.py finish==')
