...
"""
Case Type   : GUC
Case Name   : 修改参数comm_tcp_mode为其他类型参数及超边界值，并校验其预期结果。
Description :
              1、查看comm_tcp_mode默认值；
              show comm_tcp_mode;
              2、修改comm_tcp_mode分别为-1、'test'，校验其预期结果；
Expect      :
              1、显示默认值；
              2、参数修改失败；
History     :
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0238开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("修改comm_tcp_mode分别为其他类型参数及超边界值等，"
                      "校验其预期结果；")
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0238完成==')
