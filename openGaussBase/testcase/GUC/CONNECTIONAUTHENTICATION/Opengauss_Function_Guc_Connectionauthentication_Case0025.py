...
"""
Case Type   : GUC
Case Name   : listen_addresses参数使用gs_guc set设置为空值（单节点与多节点）
Description :
    1、查看listen_addresses默认值
        source [环境变量路径]
        gs_guc check -D [数据库实例路径] -c listen_addresses
    2、使用设置gs_guc set设置listen_addresses为空值
        单机（单节点）：
...
class GucSetListenAddresses(unittest.TestCase):
    def setUp(self):
        logger.info(
            "Opengauss_Function_Guc_Connectionauthentication_Case0025开始执行"
        )
    ...
    def test_guc_set_listen_addresses(self):
        ...
        logger.info(body2)
        if "listen_addresses = ''" not in body2:
            raise Exception("postgressql.conf中listen_addresses不为空，"
                            "guc_set设置失败")
        logger.info("postgressql.conf中listen_addresses和修改值一致")
        logger.info("重启数据库使之生效")
        if self.commonsh_list[0].restart_db_cluster():
            logger.info("修改默认值为''后，重启数据库成功")
        else:
            raise Exception("修改默认值为''后，重启数据库失败")
        ...
        for i in range(len(self.node_list)):
            ...
            logger.info(body5)
            if "listen_addresses = ''" not in body5:
                raise Exception("postgressql.conf中listen_addresses不为空，"
                                "guc_set设置失败")
        logger.info("postgressql.conf中listen_addresses和修改值一致")
        logger.info("重启数据库使之生效")
        if self.commonsh_list[0].restart_db_cluster():
            ...
            raise Exception("修改默认值为''后，重启数据库失败")
        # 多节点设置的数据库恢复统一在tearDown()中做
        logger.info("在多节点使用set设置listen_addresses为''成功")
    ...
    def tearDown(self):
        ...
        if self.default_ips_list:
            ...
            if not self.commonsh_list[0].restart_db_cluster():
                ...
                logger.info("数据库重启成功")
        logger.info(
            "Opengauss_Function_Guc_Connectionauthentication_Case0025执行结束"
        )
