...
"""
Case Type   : GUC
Case Name   : ssl_key_file参数使用gs_guc reload设置为其他数据类型
Description : 1、查看ssl_key_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_key_file
              2、使用设置gs_guc reload设置ssl_key_file
              gs_guc reload -D {cluster/dn1} -c "ssl_key_file=12345"
              gs_guc reload -N all  -D {cluster/dn1} -c "ssl_key_file=12345"
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0211开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0211完成==')
