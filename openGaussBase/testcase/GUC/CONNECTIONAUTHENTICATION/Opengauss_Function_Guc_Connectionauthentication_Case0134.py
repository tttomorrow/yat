...
"""
Case Type   : GUC
Case Name   : authentication_timeout参数使用gs_guc set设置为空值
Description :
              1、查看authentication_timeout默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c authentication_timeout
              2、使用设置gs_guc set设置authentication_timeout为空值
              gs_guc set -D {cluster/dn1} -c "authentication_timeout=' '"
Expect      :
              1、显示默认值；
              2、参数修改失败；
History     :
...
class GucTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0134开始==')
    ...
    def test_startdb(self):
        ...
        result = COMMONSH.execute_gsguc(
            'set', self.constant.TPCC_ERROR, f"authentication_timeout=' '")
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.assertEqual("1min", sql_cmd.split("\n")[-2].strip())
        self.log.info('==Guc_Connectionauthentication_Case0134完成==')
