...
"""
Case Type   : GUC
Case Name   : 修改参数comm_sctp_port,观察预期结果
Description :
              1、查看comm_sctp_port默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c comm_sctp_port
              2、使用设置gs_guc set设置comm_sctp_port
              gs_guc set -D {cluster/dn1} -c "comm_sctp_port=30"
              3、校验是否修改成功；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0239开始==')
    ...
    def test_startdb(self):
        ...
        showcmd = "source " + macro.DB_ENV_PATH \
                  ...
                  + " -c \"show comm_sctp_port;\""
        self.log.info(showcmd)
        check = self.db_user_node.sh(showcmd).result()
        self.log.info(check)
        self.assertIn('7000', check)
        self.log.info("设置comm_sctp_port，重启使其生效")
    ...
    def tearDown(self):
        ...
        gs_guc set -N all -I all -c "comm_sctp_port=\'7000\'"'''
        self.log.info(gucsetcmd2)
        self.db_user_node.sh(gucsetcmd2)
        COMMONSH.restart_db_cluster()
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0239完成==')
