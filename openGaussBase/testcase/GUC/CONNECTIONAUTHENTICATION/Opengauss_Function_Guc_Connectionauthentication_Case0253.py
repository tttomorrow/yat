...
"""
Case Type   : GUC
Case Name   : 修改参数comm_no_delay,观察预期结果
Description :
              1、查看comm_no_delay默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c comm_no_delay
              2、使用设置gs_guc set设置comm_no_delay
              gs_guc set -D {cluster/dn1} -c "comm_no_delay=on"
              3、校验是否修改成功；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0253开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0253完成==')
