...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数ssl为其他数据类型
Description : 1、查看ssl默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl
              2、使用ALTER SYSTM SET修改数据库参数ssl为其他数据类型;
              ALTER SYSTEM set ssl to 12345;
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0172开始==')
    ...
    def test_startdb(self):
        ...
        altersql1 = "source " + macro.DB_ENV_PATH \
                    ...
                    + " -c \"ALTER SYSTEM set ssl to 12345;\""
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0172完成==')
