...
"""
Case Type   : GUC
Case Name   : session_timeout参数使用gs_guc reload设置
Description : 1、查看session_timeout默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c session_timeout
              2、使用设置gs_guc reload设置session_timeout
              gs_guc reload -D {cluster/dn1} -c "session_timeout=601s"
              3、校验是否修改成功；
...
class GucTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0154开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0154完成==')
