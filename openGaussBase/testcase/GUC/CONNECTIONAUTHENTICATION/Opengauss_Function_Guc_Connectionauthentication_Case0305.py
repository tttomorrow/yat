...
Case Name   :gs_guc set修改参数listen_address为多个ip，ip间使用分号分隔，
             重启数据库失败
Description :
    1、查看listen_addresses默认值
    2、设置listen_addresses参数为多个ip,ip间使用分号分隔并重启数据库
    3、恢复默认值
Expect      :
...
class GUC(unittest.TestCase):
    def setUp(self):
        LOGGER.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0305start-')
    ...
    def tearDown(self):
        ...
        LOGGER.info(
            '-Opengauss_Function_Guc_Connectionauthentication_Case0305finish-')
