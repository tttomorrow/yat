...
"""
Case Type   : GUC
Case Name   : 修改指定数据库，用户，会话级别的参数ssl_ca_file
Description : 1、查看ssl_ca_file默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_ca_file
              2、在gsql中分别设置数据库、用户、会话、级别ssl_ca_file；
              alter database postgres set ssl_ca_file to 'cacert1.pem';
              alter user env109 set ssl_ca_file to 'cacert1.pem';
              set ssl_ca_file to 'cacert1.pem';
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0215开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置ssl_ca_file，校验预期结果")
        ...
        sql_cmd2 = COMMONSH.execut_db_sql(
            f'''set ssl_ca_file to 'cacert1.pem';''')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0215完成==')
