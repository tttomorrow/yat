...
"""
Case Type   : GUC
Case Name   : ssl_renegotiation_limit参数使用gs_guc set设置
Description : 1、查看ssl_renegotiation_limit默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c ssl_renegotiation_limit
              2、使用设置gs_guc set设置ssl_renegotiation_limit
              gs_guc set -D {cluster/dn1} -c "ssl_renegotiation_limit=1"
              3、校验是否修改成功；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0186开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置ssl_renegotiation_limit，重启使其生效")
        gucsetcmd2 = '''source ''' + macro.DB_ENV_PATH \
                     + ''';gs_guc set -D ''' + macro.DB_INSTANCE_PATH \
                     + ''' -c "ssl_renegotiation_limit=1"'''
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0186完成==')
