...
"""
Case Type   : GUC
Case Name   : 修改参数comm_timer_mode,观察预期结果
Description :
              1、查看comm_timer_mode默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c comm_timer_mode
              2、使用设置gs_guc set设置comm_timer_mode
              gs_guc set -D {cluster/dn1} -c "comm_timer_mode=on"
              3、校验是否修改成功；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0259开始==')
    ...
    def tearDown(self):
        ...
        gs_guc set -N all -I all -c "comm_timer_mode=\'off\'"'''
        self.log.info(gucsetcmd2)
        self.db_user_node.sh(gucsetcmd2)
        COMMONSH.restart_db_cluster()
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0259完成==')
