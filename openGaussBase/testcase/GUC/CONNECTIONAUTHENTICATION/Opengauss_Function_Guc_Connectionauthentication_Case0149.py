...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数auth_iteration_count为空值
Description : 1、查看auth_iteration_count默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c auth_iteration_count
              2、使用ALTER SYSTM SET修改数据库参数auth_iteration_count为空值;
              ALTER SYSTEM set auth_iteration_count to ' ';
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class GucTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0149开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0149完成==')
