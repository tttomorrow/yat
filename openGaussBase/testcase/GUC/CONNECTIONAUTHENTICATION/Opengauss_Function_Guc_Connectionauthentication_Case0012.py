...
"""
Case Type   : GUC
Case Name   : port参数使用gs_guc reload设置为空值
Description : 1、查看port默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c port
              2、使用设置gs_guc reload设置port为空值
              gs_guc reload -D {cluster/dn1} -c "port= ' '"
              gs_guc reload  -N all -D {cluster/dn1} -c "port= ' '"
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0012.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        self.log.info("使用设置gs_guc reload设置port为空值")
    ...
    def tearDown(self):
        ...
        self.log.info('==Connectionauthentication_Case0012.py finish==')
