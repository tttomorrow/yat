...
"""
Case Type   : GUC
Case Name   : 修改参数comm_tcp_mode,观察预期结果
Description :
              1、查看comm_tcp_mode默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c comm_tcp_mode
              2、使用设置gs_guc set设置comm_tcp_mode
              gs_guc set -D {cluster/dn1} -c "comm_tcp_mode=off"
              gs_guc set -N all  -D {cluster/dn1} -c "comm_tcp_mode=off"
              3、校验是否修改成功；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0237开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info(res1)
        self.assertIn('off', res1)

    def tearDown(self):
        self.log.info("恢复默认值")
        gucsetcmd2 = f'''source {macro.DB_ENV_PATH};
        gs_guc set -N all -I all -c "comm_tcp_mode=\'on\'"'''
        self.log.info(gucsetcmd2)
        self.db_user_node.sh(gucsetcmd2)
        COMMONSH.restart_db_cluster()
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0237完成==')
