...
"""
Case Type   : GUC
Case Name   : unix_socket_permissions参数使用gs_guc set设置为其他数据类型
Description : 1、查看unix_socket_permissions默认值；
              ...
              -c "unix_socket_permissions='test'"
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0109.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        result = COMMONSH.execute_gsguc(
            'set', self.constant.TPCC_ERROR,
            f'unix_socket_permissions=\'test\'')
        self.assertTrue(result)
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Connectionauthentication_Case0109.py finish==')
