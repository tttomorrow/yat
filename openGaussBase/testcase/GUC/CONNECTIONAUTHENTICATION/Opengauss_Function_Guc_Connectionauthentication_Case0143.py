...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数auth_iteration_count
Description : 1、查看auth_iteration_count默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c auth_iteration_count
              2、使用ALTER SYSTM SET修改数据库参数auth_iteration_count;
              ALTER SYSTEM set auth_iteration_count to 11111;
              3、重启使其生效，校验预期结果；
Expect      : 1、显示默认值；
              2、参数修改成功；
              3、重启成功，参数修改成功。
...
class GucTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0143开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置auth_iteration_count")
        sql_cmd = COMMONSH.execut_db_sql(
            f'''ALTER SYSTEM set auth_iteration_count to 11111;''')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0143完成==')
