...
"""
Case Type   : GUC
Case Name   : 修改指定数据库，用户，会话级别的参数session_timeout为空值
Description : 1、查看session_timeout默认值；
              ...
              alter database postgres set session_timeout to ' ';
              alter user env109 set session_timeout to ' ';
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0162开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置session_timeout为空值，校验预期结果")
        sql_cmd = COMMONSH.execut_db_sql(
            f'''alter database postgres set session_timeout to ' ';''')
        ...
        sql_cmd2 = COMMONSH.execut_db_sql(f'''set session_timeout to ' ';''')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0162完成==')
