...
"""
Case Type   : GUC
Case Name   : 修改指定数据库，用户，会话级别的参数unix_socket_group
Description : 1、查看unix_socket_group默认值；
              source /opt/opengauss810/env
              gs_guc check -D /opt/opengauss810/cluster/dn1
              -c unix_socket_group
              2、在gsql中分别设置数据库、用户、会话、级别unix_socket_group；
              alter database postgres set unix_socket_group to 'test';
              alter user env109 set unix_socket_group to 'test';
              set unix_socket_group to 'test';
Expect      : 1、显示默认值；
              2、参数修改失败；
History     :
"""
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0096.py start==')
        self.rootNode = Node()
    ...
    def test_startdb(self):
        ...
        self.log.info("设置unix_socket_group，校验预期结果")
        ...
        sql_cmd2 = COMMONSH.execut_db_sql(
            f'''set unix_socket_group to 'test';''')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Connectionauthentication_Case0096.py finish==')
