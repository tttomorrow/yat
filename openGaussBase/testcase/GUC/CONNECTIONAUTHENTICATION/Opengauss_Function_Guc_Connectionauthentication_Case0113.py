...
"""
Case Type   : GUC
Case Name   : unix_socket_permissions参数使用gs_guc reload设置为空值
Description : 1、查看unix_socket_permissions默认值；
              source /opt/opengauss810/env
              gs_guc check -D /opt/opengauss810/cluster/dn1
              -c unix_socket_permissions
              2、使用设置gs_guc reload设置unix_socket_permissions为空值
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0113.py start==')
        self.rootNode = Node()
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Connectionauthentication_Case0113.py finish==')
