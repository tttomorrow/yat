...
"""
Case Type   : GUC
Case Name   : unix_socket_directory参数使用gs_guc reload设置为其他数据类型
Description : 
              ...
              3、使用设置gs_guc reload设置unix_socket_directory
              gs_guc reload -D {cluster/dn1} -c "unix_socket_directory=12345"
              gs_guc reload -N all  -D {cluster/dn1}
              -c "unix_socket_directory=12345"
Expect      : 
              1、显示默认值；
              2、创建成功；
              3、参数修改失败；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0088开始==')
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Guc_Connectionauthentication_Case0088完成==')
