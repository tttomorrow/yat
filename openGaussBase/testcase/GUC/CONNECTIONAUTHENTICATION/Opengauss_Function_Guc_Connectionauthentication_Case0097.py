...
"""
Case Type   : GUC
Case Name   : 使用ALTER SYSTEM SET修改数据库参数unix_socket_group
Description : 1、查看unix_socket_group默认值；
              source /opt/opengauss810/env
              gs_guc check -D /opt/opengauss810/cluster/dn1
              -c unix_socket_group
              2、使用ALTER SYSTM SET修改数据库参数unix_socket_group;
              ALTER SYSTEM set unix_socket_group to 'test';
Expect      : 1、显示默认值；
              2、参数修改成功；
History     :
"""
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Connectionauthentication_Case0097.py start==')
        self.rootNode = Node()
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info('==Connectionauthentication_Case0097.py finish==')
