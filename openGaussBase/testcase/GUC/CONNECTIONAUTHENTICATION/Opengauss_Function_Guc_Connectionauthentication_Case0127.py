...
"""
Case Type   : GUC
Case Name   : authentication_timeout参数使用gs_guc set设置
Description : 1、查看authentication_timeout默认值；
              source /opt/opengauss810/env
              gs_guc check -D {cluster/dn1} -c authentication_timeout
              2、使用设置gs_guc set设置authentication_timeout
              gs_guc set -D {cluster/dn1} -c "authentication_timeout=10min"
              3、校验是否修改成功；
...
class GucTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info('==Guc_Connectionauthentication_Case0127开始==')
    ...
    def test_startdb(self):
        ...
        self.log.info("设置authentication_timeout，重启使其生效")
        ...
        result = COMMONSH.execute_gsguc('check', '10min',
    ...
    def tearDown(self):
        self.log.info("恢复默认值")
        result = COMMONSH.execute_gsguc('set', self.constant.GSGUC_SUCCESS_MSG,
                                        f"authentication_timeout='{self.default_value}'")
        self.assertTrue(result)
        COMMONSH.restart_db_cluster()
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue("Normal" in status or "Degraded" in status)
        self.log.info('==Guc_Connectionauthentication_Case0127完成==')
