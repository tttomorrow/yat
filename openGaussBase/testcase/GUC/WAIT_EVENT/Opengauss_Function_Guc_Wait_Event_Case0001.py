...
"""
Case Type   : GUC
Case Name   : 修改enable_instr_track_wait为off，观察预期结果；
Description :
    1、查询enable_instr_track_wait默认值；
    show enable_instr_track_wait;
    2、修改enable_instr_track_wait为off，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c "enable_instr_track_wait=off"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("==Opengauss_Function_Guc_Wait_Event_Case0001"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        logger.info("方式一修改enable_instr_track_wait为off，"
                    "重启使其生效，期望：设置成功")
