...
"""
Case Type   : GUC
Case Name   : 使用alter database方法设置参数partition_lock_upgrade_timeout为
              3000,观察预期结果
Description :
...
class ClientConnection(unittest.TestCase):
    def setUp(self):
        LOG.info(
            '--Opengauss_Function_Guc_ClientConnection_Case0196start----')
    ...
    def test_partition_lock_upgrade_timeout(self):
        ...
        # 创建数据库
        sql_cmd = commonsh.execut_db_sql('''drop database if exists test_spdb196;
            create database test_spdb196;
            ''')
        LOG.info(sql_cmd)
        self.assertIn(self.constant.CREATE_DATABASE_SUCCESS, sql_cmd)
        # 修改数据库级别参数
        sql_cmd = commonsh.execut_db_sql('''alter database test_spdb196 set
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '---Opengauss_Function_Guc_ClientConnection_Case0196执行完成----')
