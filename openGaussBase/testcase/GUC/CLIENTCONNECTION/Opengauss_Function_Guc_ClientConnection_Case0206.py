...
"""
Case Type   : GUC
Case Name   : 使用gs_guc reload方法设置参数enable_online_ddl_waitlock为on ,
              观察预期结果
Description :
        1.查询enable_online_ddl_waitlock默认值
        2.修改参数值为on
        3.查看修改后的值
        4.恢复参数默认值
Expect      :
...
class ClientConnection(unittest.TestCase):
    def setUp(self):
        LOG.info(
            '---Opengauss_Function_Guc_ClientConnection_Case0206start--')
    ...
    def tearDown(self):
        ...
        if "off" != sql_cmd.split('\n')[-2].strip():
            ...
            LOG.info(msg)
        LOG.info(
            '--Opengauss_Function_Guc_ClientConnection_Case0206执行完成--')
