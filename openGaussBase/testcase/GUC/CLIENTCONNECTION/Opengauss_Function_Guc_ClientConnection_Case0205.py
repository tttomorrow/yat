...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数enable_online_ddl_waitlock为on ,
              观察预期结果
Description :
...
class ClientConnection(unittest.TestCase):
    def setUp(self):
        LOG.info(
            '---Opengauss_Function_Guc_ClientConnection_Case0205start----')
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status)
        LOG.info(
            '---Opengauss_Function_Guc_ClientConnection_Case0205执行完成----')
