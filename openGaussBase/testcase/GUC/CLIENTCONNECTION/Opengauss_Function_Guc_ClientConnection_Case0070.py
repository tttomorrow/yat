...
"""
Case Type   : GUC
Case Name   : 使用gs_guc reload方法设置参数default_transaction_deferrable为on,观察预期结果
Description :
...
class ClientConnection(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Guc_ClientConnection_Case0070start------')
    ...
    def tearDown(self):
        ...
        if self.res not in sql_cmd:
            msg = self.commonsh.execute_gsguc('reload',
                                              self.constant.GSGUC_SUCCESS_MSG,
                                              f'default_transaction_deferrable'
                                              f'={self.res}')
            self.log.info(msg)
        self.log.info(
            '----Opengauss_Function_Guc_ClientConnection_Case0070执行完成--')
