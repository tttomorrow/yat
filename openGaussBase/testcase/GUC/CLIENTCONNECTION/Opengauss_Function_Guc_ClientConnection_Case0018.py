...
"""
Case Type   : GUC参数
Case Name   : 使用gs_guc reload方法设置参数current_schema,观察预期结果
Description :
    1.show参数默认值
    2.创建模式
    3.修改参数默认值为自定义模式
...
class Guc(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Guc_ClientConnection_Case0018开始')
        self.dbuser = Node('PrimaryDbUser')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')
        self.sc_name = "sc_guc_0018"
    ...
    def test_current_schema(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'show current_schema;')
        self.log.info(sql_cmd)
        self.assertIn(f'{self.sc_name}', sql_cmd)
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(f'drop schema {self.sc_name} '
                                              f'cascade; ')
        self.log.info(sql_cmd)
        self.log.info('Opengauss_Function_Guc_ClientConnection_Case0018结束')
