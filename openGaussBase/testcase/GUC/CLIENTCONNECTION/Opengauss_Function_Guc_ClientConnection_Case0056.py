...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数default_transaction_isolation为serializable,观察预期结果
Description :
        1.查询default_transaction_isolation默认值
        2.修改参数值为serializable
        3.恢复参数默认值
Expect      :
        1.显示默认值为read committed
        2.设置成功,显示repeatable read
        3.默认值恢复成功
History     :
"""
import unittest
...
class ClientConnection(unittest.TestCase):
    def setUp(self):
        LOG.info(
            '----Opengauss_Function_Guc_ClientConnection_Case0056start----')
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '---Opengauss_Function_Guc_ClientConnection_Case0056执行完成--')
