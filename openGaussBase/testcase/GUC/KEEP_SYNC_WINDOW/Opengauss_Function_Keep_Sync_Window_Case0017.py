...
Case Name   : 1主2备下，1主2同步备，同步备机多次异常,在时间窗口内执行DML操作，
              测试时间窗口参数
              执行DML操作
Description :
        1.设置以下guc参数
...
Expect      :
        ...
        13.数据同步完成
        14.清理环境完成
              可用模式打印warning提示不确定，导致断言获取时间索引不对，修改用例
"""
import os
import time
import unittest
...
class GucParameters(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = "tb_keep_sync_window_0017"
    ...
    def test_keep_sync_window(self):
        ...
        self.assertIn('ANY 2(dn_6002,dn_6003)', result1, '执行失败' + text)
        ...
        self.log.info('等待180s后，同步备未恢复连接，数据插入成功')
        self.log.info(text)
        ...
        text = '--step11:删除数据;expect:同步备在超时窗口未恢复连接,' \
               '进入最大可用模式，删除成功--'
        self.log.info(text)
        sql_cmd = f"select timenow();" \
