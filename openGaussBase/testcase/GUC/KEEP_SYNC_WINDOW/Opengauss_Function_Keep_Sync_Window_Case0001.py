...
Case Name   : gs_guc set方法设置参数keep_sync_window为有效值
Description :
        1.修改参数值0
        2.重启数据库成功并查询集群状态
        3.查询参数值
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0001 start-')
    ...
    def test_keep_sync_window(self):
        ...
        text = '--step6:查询参数值;expect:参数值为2147483647--'
        self.log.info(text)
        sql_cmd = self.common.show_param('keep_sync_window')
        self.log.info(sql_cmd)
        self.assertEqual('2147483647s', sql_cmd, '执行失败' + text)

        text = '--step7:修改参数值1min;expect:修改成功--'
        self.log.info(text)
        result = self.pri_sh.execute_gsguc("set",
                                           self.constant.GSGUC_SUCCESS_MSG,
        ...
        text = '--step9:查询参数值;expect:参数值为1min--'
        self.log.info(text)
        sql_cmd = self.common.show_param('keep_sync_window')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        text = '--step10:恢复默认值;expect:恢复默认值成功--'
        self.log.info(text)
        restore = self.pri_sh.execute_gsguc("set",
                                            self.constant.GSGUC_SUCCESS_MSG,
                                            f"keep_sync_window="
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败' + text)
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0001 finish-')
