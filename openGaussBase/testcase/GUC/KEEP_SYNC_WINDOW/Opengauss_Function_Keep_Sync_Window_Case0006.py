...
Case Name   : synchronous_commit参数为off时，验证keep_sync_window参数是否有效
Description :
        1.设置以下三个guc参数
...
class GucParameters(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0006 start-')
        ...
        self.tb_name = "tb_keep_sync_window_0006"
    ...
    def test_keep_sync_window(self):
        ...
        text = '--step6:插入数据;expect:插入成功，主节点不阻塞，' \
               '不会进入最大可用模式,keep_sync_window参数无效--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.assertTrue(restart_cmd, '执行失败' + text)
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0006 finish-')
