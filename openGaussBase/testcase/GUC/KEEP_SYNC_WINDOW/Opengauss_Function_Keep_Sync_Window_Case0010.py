...
Case Name   : 1主2同步备(1个同步，1个潜在同步)，一个备机异常，
             测试keep_sync_window参数是否生效
Description :
        1.设置以下四个guc参数
        1.1设置synchronous_standby_names='dn_6002,dn_6003'
...
Expect      :
        ...
        3.建表成功
        4.停止备1节点成功
        5.主节点不阻塞,数据插入成功，没有进入最大可用模式，时间参数不生效
        6.启动成功
        7.同步成功
        8.清理环境完成
"""
...
class GucParameters(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0010 start-')
        ...
        self.tb_name = "tb_keep_sync_window_0010"
    ...
    def test_keep_sync_window(self):
        ...
        self.assertEqual('3min', result3, '执行失败' + text)
        result4 = self.common.show_param('synchronous_standby_names')
        self.assertIn(f"{macro.DN_NODE_NAME.split('/')[1]},"
                      f"{macro.DN_NODE_NAME.split('/')[2]}", result4,
                      '执行失败' + text)
        ...
        text = '--step5:插入数据;expect:主节点插入数据，主节点不阻塞,' \
               '数据插入成功，没有进入最大可用模式，时间参数不生效--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0010 finish-')
