...
Case Name   : 1主2备环境下，1主1同步1异步，同步备为备1节点，同步备异常，
              测试时间窗是否生效
Description :
        1.设置以下四个guc参数
...
Expect      :
        ...
        3.建表成功
        4.停止备1节点成功
        5.主节点插入数据，主节点阻塞,60s后，进入最大可用模式，数据插入成功，
        主机事务提交，时间参数生效
        6.60s后，插入成功
        7.启动成功
        8.同步成功
...
class GucParameters(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0011 start-')
        ...
        self.tb_name = "tb_keep_sync_window_0011"

    def test_keep_sync_window(self):
        text = '--step1:设置以下四个guc参数;expect:设置成功--'
        self.log.info(text)
        self.log.info('设置synchronous_standby_namest=dn_6002')
        ...
        text = '--step5:插入数据;expect:主节点插入数据，主节点阻塞,60s后，' \
               '进入最大可用模式，数据插入成功，主机事务提交，时间参数生效--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0011 finish-')
