...
Case Name   : 1主2备环境下，1主1同步1异步，时间窗内，同步备kill -9
Description :
        1.设置以下guc参数
...
Expect      :
        ...
        4.成功
        5.kill成功
        6.时间窗内同步备未恢复连接;180s后进入最大可用模式，数据修改成功
        7.启动成功
        8.同步成功
        9.清理环境完成
"""
...
class GucParameters(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0025 start-')
        ...
        self.tb_name = "tb_keep_sync_window_0025"
    ...
    def test_keep_sync_window(self):
        ...
        text = '--step5:kill -9同步备;expect:kill成功--'
        self.log.info(text)
        msg = self.common.kill_pid(self.standby_root_node1, '9')
        self.assertEqual('', msg, '执行失败:' + text)

        text = '--step6:主节点修改数据;expect:时间窗内同步备未恢复连接；' \
               '180s后进入最大可用模式，数据修改成功--'
        self.log.info(text)
        sql_cmd = f"update {self.tb_name} set id =id +1;"
        self.log.info(sql_cmd)
        ...
        self.assertIn(self.constant.UPDATE_SUCCESS_MSG, insert_thread_result,
                      '执行失败:' + text)
        time.sleep(200)

        text = '--step7:gs_ctl方式启动备1节点;expect:启动成功--'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0025 finish-')
