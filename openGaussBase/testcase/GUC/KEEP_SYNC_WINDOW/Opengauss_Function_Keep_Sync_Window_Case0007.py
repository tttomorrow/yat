...
Case Name   : synchronous_commit参数为on，most_available_sync为off，
              keep_sync_window大于0，测试时间窗口参数是否有效
Description :
        1.设置以下三个guc参数
        1.1.设置synchronous_commit=on
        1.2.设置most_available_sync=off
        1.3设置keep_sync_window=180
        2.查询修改后的参数值
        3.建表
        4.停止集群
        5.gs_ctl方式启动主节点
...
Expect      :
        ...
        4.停止集群成功
        5.启动成功
        6.主节点阻塞,180s后，主节点依然阻塞，不会进入最大可用模式
        7.启动成功
        8.同步备启动后，主节点数据插入成功
        9.同步成功
        10.清理环境完成
...
class GucParameters(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0007 start-')
        ...
        self.tb_name = "tb_keep_sync_window_0007"
    ...
    def test_keep_sync_window(self):
        ...
        text = '--step6:插入数据;expect:主节点阻塞,180s后，主节点依然阻塞，' \
               '不会进入最大可用模式--'
        self.log.info(text)
        ...
        text = '--step8:获取step5插入结果;expect:同步备启动后，' \
               '主节点数据插入成功---'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0007 finish-')
