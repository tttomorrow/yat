...
Case Name   : 1主2备环境下，1主1同步备1异步备，同步模式与最大可用模式切换
              (kill -9同步备->同步模式->kill -9同步备->进入最大可用模式)
Description :
        1.设置以下guc参数
...
Expect      :
        ...
        5.成功
        6.阻塞
        7.重启成功，同步备在时间窗口内恢复，事物自动提交
        8.成功
        9.阻塞，同步备在时间窗口内未恢复，进入最大可用模式
        10.重启成功
        11.数据同步完成
        12.清理环境完成
"""
...
class GucParameters(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0031 start-')
        ...
        self.tb_name = "tb_keep_sync_window_0031"
    ...
    def test_keep_sync_window(self):
        ...
        self.assertEqual('5min', result4, '执行失败' + text)
        ...
        self.assertLess(f'{num}', '05:00', '执行失败:' + text)

        text = '--step8:再次kill -9同步备;expect:成功--'
        self.log.info(text)
        msg = self.common.kill_pid(self.standby_root_node2, '9')
        self.assertEqual('', msg, '执行失败:' + text)

        text = '--step9:插入数据;expect:阻塞--'
        self.log.info(text)
        ...
        insert_thread.setDaemon(True)
        insert_thread.start()
        self.log.info('获取step9结果,同步备在时间窗口内未恢复,'
                      '进入最大可用模式，数据插入成功')
        insert_thread.join(10 * 600)
        self.assertTrue(self.constant.INSERT_SUCCESS_MSG in
                        insert_thread_result, '执行失败:' + text)
        ...
        time.sleep(360)
        text = '--step10:启动同步备;expect:启动成功--'
        self.log.info(text)
        start_cmd = self.standby_sh2.start_db_instance(mode="standby")
        self.log.info(start_cmd)
        self.assertTrue(start_cmd, '执行失败' + text)

        text = '----step11:查看数据是否同步;expect:数据同步完成----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0031 finish-')
