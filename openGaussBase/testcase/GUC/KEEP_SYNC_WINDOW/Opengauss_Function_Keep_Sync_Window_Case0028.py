...
Case Name   : 1主2备下，1同步1异步，synchronous_commit=remote_apply,时间窗内，
              同步备及主机均异常
Description :
        1.设置以下guc参数
...
class GucParameters(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0028 start-')
        ...
        self.tb_name = "tb_keep_sync_window_0028"
    ...
    def test_keep_sync_window(self):
        ...
        text = '--step7:同步时间窗内kill -9主机;expect:成功--'
        self.log.info(text)
        msg = self.common.kill_pid(self.pri_root_node, '9')
        self.assertEqual('', msg, '执行失败:' + text)

        self.log.info('获取step6结果，kill-9主节点后，插入数据报连接丢失')
        insert_thread.join(10 * 600)
        insert_thread_result = insert_thread.get_result()
        self.log.info(insert_thread_result)
        self.assertNotIn(self.constant.INSERT_SUCCESS_MSG,
        ...
        text = '----step10:查看数据是否丢失;expect:数据未丢失----'
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        self.log.info('-Opengauss_Function_Keep_Sync_Window_Case0028 finish-')
