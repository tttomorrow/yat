...
"""
Case Type   : GUC
Case Name   : 修改minimum_pool_size，观察其预期结果
Description :
            1、查询minimum_pool_size默认值；
            show minimum_pool_size；
            2、修改minimum_pool_size为0，重启使其生效，观察预期结果；
            gs_guc set -D {cluster/dn1}  -c "minimum_pool_size=300"
            3、恢复默认值；
Expect      :
            1、显示默认值；
            2、修改成功，预期结果正常；
...
class GuctestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Connection_Pool_Case0003"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("修改minimum_pool_size为300重启生效，期望设置成功")
        result = COMMONSH.execute_gsguc("set",
                                        self.constant.GSGUC_SUCCESS_MSG,
                                       "minimum_pool_size=300")
        self.assertTrue(result)

        LOGGER.info("期望：重启后查询结果为300")
