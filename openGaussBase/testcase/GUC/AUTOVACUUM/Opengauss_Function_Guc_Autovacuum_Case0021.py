...
"""
Case Type   : GUC
Case Name   : 修改autovacuum_analyze_scale_factor，观察其预期结果
Description :
            1、查询autovacuum_analyze_scale_factor默认值；
            show autovacuum_analyze_scale_factor；
            2、修改autovacuum_analyze_scale_factor为0，
            重启使其生效，观察预期结果；
            gs_guc set -D {cluster/dn1}
            -c "autovacuum_analyze_scale_factor=0.2"
            3、恢复默认值；
Expect      :
            1、显示默认值；
            2、修改成功并生效
...
class GuctestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Guc_Autovacuum_Case0021开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("修改autovacuum_analyze_scale_factor为0.2"
                    "重启生效，期望设置成功")
    ...
    def tearDown(self):
        ...
        if "0.1" != sql_cmd.split("\n")[-2].strip():
            ...
            COMMONSH.restart_db_cluster()
        status = COMMONSH.get_db_cluster_status()
        result1 = COMMONSH.execute_gsguc('check', '0.1',
                                        'autovacuum_analyze_scale_factor')
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.assertTrue(result1)
        LOGGER.info("==Guc_Autovacuum_Case0021执行结束==")
