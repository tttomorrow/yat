...
"""
Case Type   : GUC
Case Name   : 修改enable_resource_track为on，观察预期结果；
Description :
    1、查询enable_resource_track默认值 ；
    show enable_resource_track;
    2、修改enable_resource_track为on，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c 'enable_resource_track=off'
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info('==Guc_Load_Management_Case0011开始执行==')
    ...
    def test_guc(self):
        ...
        LOGGER.info('修改enable_resource_track为off，'
                    '重启使其生效，期望：设置成功')
        ...
        result1 = COMMONSH.execut_db_sql(f"select count(*) "
            f"from gs_wlm_plan_operator_info "
    ...
    def tearDown(self):
        ...
        status = COMMONSH.get_db_cluster_status()
        self.assertTrue('Degraded' in status or 'Normal' in status)
        LOGGER.info('==-Guc_Load_Management_Case0011执行结束==')
