...
Description :
            ...
            2、修改IsInplaceUpgrade为off，校验其预期结果；
            gs_guc set -D {cluster/dn1} -c "IsInplaceUpgrade=on";
            3、恢复默认值；
Expect      :
            1、显示默认值；
            2、参数修改失败，预期结果正常；
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("Opengauss_Function_Guc_Upgrade_Case0001开始执行")
    ...
    def test_guc(self):
        ...
        LOGGER.info("修改IsInplaceUpgrade为on 期望设置失败")
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.assertEqual("off", sql_cmd.split("\n")[-2].strip())
        LOGGER.info("Opengauss_Function_Guc_Upgrade_Case0001执行结束")
