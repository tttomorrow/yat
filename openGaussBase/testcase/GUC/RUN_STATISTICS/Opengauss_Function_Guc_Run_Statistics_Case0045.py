...
"""
Case Type   : GUC
Case Name   : 普通用户guser方式三修改track_functions为all，校验其预期结果
Description :
    步骤1:查询track_functions默认值
    show track_functions;
    步骤2:创建用户guser
    create user guser password {password};
    步骤3:普通用户guser方式三修改track_functions为all，校验其预期结果
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Run_Statistics_Case0045"
            "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("步骤3:普通用户方式三修改参数track_functions为all")
