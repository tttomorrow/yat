...
"""
Case Type   : GUC
Case Name   : 方式四修改参数stats_temp_directory为pg_stat_tmp1
Description :
    步骤1:查询stats_temp_directory默认值
    show stats_temp_directory;
    步骤2:方式四修改参数stats_temp_directory为pg_stat_tmp1
    alter system set stats_temp_directory to pg_stat_tmp1;
    show stats_temp_directory;
    步骤3:执行DML
    步骤4:恢复默认值
    alter system set stats_temp_directory to pg_stat_tmp;
Expect      :
    步骤1:显示默认值pg_stat_tmp
    步骤2:参数设置成功，校验参数值为pg_stat_tmp1
    步骤3:DML不报错
    步骤4:恢复默认值成功
History     :
"""
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Run_Statistics_Case0057"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("步骤2:修改stats_temp_directory为pg_stat_tmp1，重启，期望：设置成功")
        result = COMMONSH.execut_db_sql("alter system set "
            "stats_temp_directory to 'pg_stat_tmp1'")
    ...
    def tearDown(self):
        ...
        if "pg_stat_tmp" != sql_cmd.splitlines()[-2].strip():
            COMMONSH.execut_db_sql("alter system "
