...
"""
Case Type   : GUC
Case Name   : 管理员用户方式三修改参数track_activities为0/true/no,设置成功生效
Description :
    1、查询track_activities默认值
    show track_activities;
    2、管理员用户方式三修改track_activities为0
    alter database postgres set track_activities to 0;
    show track_activities;
    管理员用户方式三修改track_activities为true
    alter database postgres set track_activities to true;
    show track_activities;
    管理员用户方式三修改track_activities为no
    alter database postgres set track_activities to no;
    show track_activities;
    3、恢复默认值,清理环境
Expect      :
    1、显示默认值
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Run_Statistics_Case0031"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("管理员用户方式三修改参数track_activities为true")
        sql_cmd = COMMONSH.execut_db_sql(f"alter database {self.db_name} "
                                       f"set track_activities to true;")
        self.assertIn(self.constant.ALTER_DATABASE_SUCCESS_MSG, sql_cmd)
        ...
        LOGGER.info("管理员用户方式三修改参数track_activities为no")
        sql_cmd = COMMONSH.execut_db_sql(f"alter database {self.db_name} "
            f"set track_activities to no;")
        self.assertIn(self.constant.ALTER_DATABASE_SUCCESS_MSG, sql_cmd)
