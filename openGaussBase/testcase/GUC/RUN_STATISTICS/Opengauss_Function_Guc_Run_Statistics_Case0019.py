...
"""
Case Type   : GUC
Case Name   : 修改track_sql_count为off，观察预期结果；
Description :
    1、查询track_sql_count默认值；
    show track_sql_count;
    2、修改track_sql_count为on，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c "track_sql_count=off"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------Opengauss_Function_Guc_Run_Statistics_Case0019开始执行-----------------------------")
    ...
    def test_guc(self):
        ...
        logger.info("修改track_sql_count为on，重启使其生效，期望：设置成功")
        result = commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'track_sql_count=off')
        ...
        result = commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'track_sql_count=on')
        self.assertTrue(result)
    ...
    def tearDown(self):
        ...
        logger.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        logger.info("-------------------------Opengauss_Function_Guc_Run_Statistics_Case0019执行结束---------------------------")