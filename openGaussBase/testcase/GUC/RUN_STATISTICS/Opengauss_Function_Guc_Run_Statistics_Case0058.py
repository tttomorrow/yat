...
"""
Case Type   : GUC
Case Name   : 修改参数stats_temp_directory为嵌套目录pg_stat_tmp/pg_stat_tmp，修改成功并生效
Description :
    步骤1:查询stats_temp_directory默认值
    show stats_temp_directory;
    步骤2:方式二修改stats_temp_directory为pl，校验其预期结果
    gs_guc reload -N all -I all -c "stats_temp_directory=
...
Expect      :
    ...
    步骤2:参数修改成功，校验修改后参数值为pg_stat_tmp/pg_stat_tmp
    步骤3:DML无报错，查询pg_stat_tmp/pg_stat_tmp下有临时文件生成
    步骤4:恢复默认值成功
History     :
"""
import unittest
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Run_Statistics_Case0058"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        sql_cmd = self.user_node.sh(f'''rm -rf \
            {macro.DB_INSTANCE_PATH}/pg_stat_tmp/pg_stat_tmp;\
            mkdir {macro.DB_INSTANCE_PATH}/pg_stat_tmp/pg_stat_tmp;
            ''').result()
        LOGGER.info(sql_cmd)
        self.assertNotIn("bash", sql_cmd)

        LOGGER.info("步骤2:修改为pg_stat_tmp/pg_stat_tmp，重启，期望：设置成功")
        ...
        LOGGER.info("期望：重启后查询结果为pg_stat_tmp/pg_stat_tmp")
        ...
        sql_cmd = COMMONSH.execut_db_sql("show stats_temp_directory;")
        LOGGER.info(sql_cmd)
        self.assertEqual("pg_stat_tmp/pg_stat_tmp",
        ...
        sql_cmd = self.user_node.sh(f'''rm -rf \
            {macro.DB_INSTANCE_PATH}/pg_stat_tmp/pg_stat_tmp''').result()
        LOGGER.info(sql_cmd)
        self.assertNotIn("bash", sql_cmd)
        result = COMMONSH.execute_gsguc("reload",
                                        self.constant.GSGUC_SUCCESS_MSG,
