...
"""
Case Type   : GUC
Case Name   : 修改log_planner_stats为on，观察预期结果；
Description :
    1、查询log_planner_stats默认值；
    show log_planner_stats;
    2、修改log_planner_stats为on，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c "log_planner_stats=on"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("------------------------Opengauss_Function_Guc_Run_Statistics_Case0023开始执行-----------------------------")
    ...
    def test_guc(self):
        ...
        logger.info("修改log_planner_stats为on，重启使其生效，期望：设置成功")
        result = commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'log_planner_stats=on')
        ...
        result = commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'log_planner_stats=off')
        self.assertTrue(result)
    ...
    def tearDown(self):
        ...
        logger.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        logger.info("-------------------------Opengauss_Function_Guc_Run_Statistics_Case0023执行结束---------------------------")