...
"""
Case Type   : GUC
Case Name   : 方式二修改参数track_activities为no/false/0,设置成功并生效
Description :
    1、查询track_activities默认值
    show track_activities;
    2、方式二修改track_activities为no，校验其预期结果
    gs_guc reload -N all -I all -c "track_activities=no"
    show track_activities;
    查询gs_sql_count视图为空
    3、方式二修改track_activities为false，校验其预期结果
    gs_guc reload -N all -I all -c "track_activities=false"
    show track_activities;
    查询gs_sql_count视图为空
    4、方式二修改track_activities为0，校验其预期结果
    gs_guc reload -N all -I all -c "track_activities=0"
    show track_activities;
    查询gs_sql_count视图为空
    5、做简单DML
    6、恢复默认值
Expect      :
    1、显示默认值on
    2、参数修改成功，校验修改后参数值为no，查询gs_sql_count视图为空
    3、参数修改成功，校验修改后参数值为false，查询gs_sql_count视图为空
    4、参数修改成功，校验修改后参数值为0，查询gs_sql_count视图为空
    5、DML无报错
    6、恢复默认值成功
History     :
"""
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Run_Statistics_Case0029"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("2、修改track_activities为no，重启生效 期望设置成功")
        ...
        LOGGER.info("3、修改track_activities为false，重启 期望设置成功")
        result = COMMONSH.execute_gsguc("reload",
                                       self.constant.GSGUC_SUCCESS_MSG,
        ...
        LOGGER.info("4、修改track_activities为0，重启，期望：设置成功")
        result = COMMONSH.execute_gsguc("reload",
                                       self.constant.GSGUC_SUCCESS_MSG,
                                       "track_activities=0")
        self.assertTrue(result)
