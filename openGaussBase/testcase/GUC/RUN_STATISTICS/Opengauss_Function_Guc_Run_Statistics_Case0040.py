...
"""
Case Type   : GUC
Case Name   : 普通用户方式三修改参数track_io_timing为on,合理报错不生效
Description :
    1、查询track_io_timing默认值
    show track_io_timing;
    2、创建用户guser
    create user guser password {password};
    3、普通用户guser方式三修改track_io_timing为on，校验其预期结果
    alter database postgres set track_io_timing to on;
    show track_io_timing;
    4、恢复默认值,清理环境
    alter database postgres set track_io_timing to off;
    drop user guser;
Expect      :
    1、显示默认值off
    2、创建用户成功
    3、参数设置失败合理报错，校验修改后参数值为off
    4、恢复默认值成功
History     :
"""
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Run_Statistics_Case0040"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("3、普通用户方式三修改参数track_io_timing为on")
        sql_cmd = (f"alter database  {self.db_name} "
                   f"set track_io_timing to on;")
