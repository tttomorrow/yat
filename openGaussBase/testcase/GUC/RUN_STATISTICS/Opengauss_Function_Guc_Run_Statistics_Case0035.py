...
"""
Case Type   : GUC
Case Name   : 普通用户方式三修改参数track_counts为off,合理报错不生效
Description :
    1、查询track_counts默认值
    show track_counts;
    2、创建用户guser
    create user guser password {password};
    3、普通用户guser方式三修改track_counts为off，校验其预期结果
    alter database postgres set track_counts to off;
    show track_counts;
    4、恢复默认值,清理环境
Expect      :
    1、显示默认值on
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Run_Statistics_Case0035"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("3、普通用户方式三修改参数track_counts为off")
        sql_cmd = (f"alter database {self.db_name} "
                   f"set track_counts to off;")
