...
"""
Case Type   : GUC参数
Case Name   : enable_hashjoin参数使用gs_guc set设置为其他数据类型，验证其预期结果
Description :
    1.查询enable_hashjoin默认值
    2.修改enable_hashjoin为99999
    3.修改enable_hashjoin为test
    4.修改enable_hashjoin为'test'
Expect      :
    1.查询enable_hashjoin默认值成功
    2.修改enable_hashjoin为99999失败
    3.修改enable_hashjoin为test失败
    4.修改enable_hashjoin为'test'失败
History     :
"""
...
class GucQueryplan(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '------Opengauss_Function_Guc_Queryplan_Case0008-------')
    ...
    def test_Guc_queryplan(self):
        ...
        LOG.info(
            '--------修改enable_hashjoin为test------')
        msg = self.comsh.execute_gsguc('set', self.constant.GSGUC_SUCCESS_MSG,
                                      'enable_hashjoin=test')
        LOG.info(msg)
    ...
    def tearDown(self):
        ...
        LOG.info(startmsg)
        LOG.info(
            '-------Opengauss_Function_Guc_Queryplan_Case0008执行完成------')
