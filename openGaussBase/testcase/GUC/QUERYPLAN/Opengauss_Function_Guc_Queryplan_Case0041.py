...
"""
Case Type   : GUC参数
Case Name   : expected_computing_nodegroup参数使用gs_guc set设
                置并验证其预期结果
Description :
    1.查询expected_computing_nodegroup默认值
    2.修改expected_computing_nodegroup为optimal
    3.重启使其生效
    4.校验其预期结果
    5.恢复默认值
Expect      :
    1.查询expected_computing_nodegroup默认值成功
    2.修改expected_computing_nodegroup为optimal成功
    3.重启集群成功
    4.该参数值为optimal，达到预期效果
    5.恢复默认值成功
History     :
"""
...
class GucQueryplan(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Guc_Queryplan_Case0041开始执行----')
        self.com = Common()
    ...
    def test_Guc_queryplan(self):
        ...
        LOG.info('-------修改expected_computing_nodegroup为optimal---------')
        msg = self.comsh.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            "expected_computing_nodegroup='optimal'")
        LOG.info(msg)
    ...
    def tearDown(self):
        ...
        LOG.info(startmsg)
        LOG.info('---Opengauss_Function_Guc_Queryplan_Case0041执行完成----')
