...
"""
Case Type   : GUC参数
Case Name   : 修改参数allocate_mem_cost，观察预期结果
Description :
    1.show参数默认值
    2.修改参数默认值为2
    3.show参数值
...
class Guc(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Guc_Parameter_Case1243开始')
    ...
    def test_guc_parameter(self):
        ...
        text = '--step2:修改参数值为2;expect:修改成功--'
        self.log.info(text)
        ...
        text = '--step3:show参数值;expect:参数值为2--'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(f'show allocate_mem_cost;')
        self.log.info(sql_cmd)
        self.assertIn('2', sql_cmd, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info('Opengauss_Function_Guc_Parameter_Case1243结束')
