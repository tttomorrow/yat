...
"""
Case Type   : GUC参数
Case Name   : enable_hashagg参数使用gs_guc set设置并验证其预期结果
Description :
    1.查询enable_hashagg默认值
    2.修改enable_hashagg为off
    3.重启使其生效
    4.校验其预期结果
    5.恢复默认值
Expect      :
    1.查询enable_hashagg默认值成功
    2.修改enable_hashagg为off成功
    3.重启集群成功
    4.该参数值为off，达到预期效果
    5.恢复默认值成功
History     :
...
class GucQueryplan(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '------Opengauss_Function_Guc_Queryplan_Case0005-------')
    ...
    def tearDown(self):
        ...
        LOG.info(startmsg)
        LOG.info(
            '----Opengauss_Function_Guc_Queryplan_Case0005执行完成------')
