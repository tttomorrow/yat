...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数enable_sonic_hashagg为无效值,合理报错
Description :
...
class QueryPlan(unittest.TestCase):
    ...
    def setUp(self):
        ...
        LOG.info(
            '-------Opengauss_Function_Guc_Queryplan_Case0106start------')

    def test_enable_sonic_hashagg(self):
        LOG.info('--步骤1:查看默认值--')
        sql_cmd = commonsh.execut_db_sql(f'''show enable_sonic_hashagg;''')
        LOG.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '-----Opengauss_Function_Guc_Queryplan_Case0106执行完成-------')
