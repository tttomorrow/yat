...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数constraint_exclusion为无效值,合理报错
Description :
...
class QueryPlan(unittest.TestCase):
    ...
    def setUp(self):
        ...
        LOG.info(
            '-----Opengauss_Function_Guc_Queryplan_Case0078start------')

    def test_constraint_exclusion(self):
        LOG.info('--步骤1:查看默认值--')
        sql_cmd = commonsh.execut_db_sql(f'''show constraint_exclusion;''')
        LOG.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '-----Opengauss_Function_Guc_Queryplan_Case0078执行完成-----')
