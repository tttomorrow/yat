...
"""
Case Type   : GUC参数
Case Name   : 修改参数effective_cache_size为其他数据类型，观察预期结果
Description :
    1.show参数默认值
    2.修改参数默认值为test
...
class Guc(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_Guc_Parameter_Case1242开始')
    ...
    def test_guc_parameter(self):
        ...
        error_msg1 = 'ERROR: The value "test" for parameter "effective_' \
                     'cache_size" is incorrect, requires a integer value'
        error_msg2 = 'ERROR: The value "9999999999" for parameter ' \
                     '"effective_cache_size" is incorrect, ' \
                     'requires a integer value'

        text = '--step2:修改参数默认值为test;expect:合理报错--'
        self.log.info(text)
        mod_msg = self.commonsh.execute_gsguc('set',
                                              error_msg1,
                                              'effective_cache_size =test')
        self.log.info(mod_msg)
        self.assertTrue(mod_msg, '执行失败:' + text)

        text = '--step3:修改参数默认值为9999999999;expect:合理报错--'
        self.log.info(text)
        mod_msg = self.commonsh.execute_gsguc('set',
                                              error_msg2,
    ...
    def tearDown(self):
        ...
        self.log.info('Opengauss_Function_Guc_Parameter_Case1242结束')
