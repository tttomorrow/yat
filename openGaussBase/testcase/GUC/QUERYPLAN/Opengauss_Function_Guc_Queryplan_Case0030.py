...
"""
Case Type   : GUC参数
Case Name   : max_recursive_times参数使用gs_guc set设置为其他数据类型，
                验证其预期结果
Description :
    1.查询max_recursive_times默认值
    2.修改max_recursive_times为test
    3.修改max_recursive_times为'test'
Expect      :
    1.查询max_recursive_times默认值成功
    2.修改max_recursive_times为test失败
    3.修改max_recursive_times为'test'失败
History     :
"""
...
class GucQueryplan(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('-----Opengauss_Function_Guc_Queryplan_Case0030-------')
        self.com = Common()
    ...
    def tearDown(self):
        ...
        LOG.info(startmsg)
        LOG.info('--Opengauss_Function_Guc_Queryplan_Case0030执行完成-----')
