...
"""
Case Type   : GUC参数
Case Name   : cpu_operator_cost参数使用gs_guc set设置并验证其预期结果
Description :
    1.查询cpu_operator_cost默认值
    2.修改cpu_operator_cost为2
    3.重启使其生效
    4.校验其预期结果
    5.恢复默认值
Expect      :
    1.查询cpu_operator_cost默认值成功
    2.修改cpu_operator_cost为2成功
    3.重启集群成功
    4.该参数值为2，达到预期效果
    5.恢复默认值成功
History     :
...
class GucQueryplan(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info('---Opengauss_Function_Guc_Queryplan_Case0059开始执行--')
        self.com = Common()
    ...
    def test_Guc_queryplan(self):
        ...
        LOG.info('-------修改cpu_operator_cost为2---------')
    ...
    def tearDown(self):
        ...
        LOG.info(startmsg)
        LOG.info('---Opengauss_Function_Guc_Queryplan_Case0059执行完成----')
