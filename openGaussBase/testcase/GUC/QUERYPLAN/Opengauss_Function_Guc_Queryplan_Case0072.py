...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数geqo_selection_bias为无效值,合理报错
Description :
        1.查询geqo_selection_bias默认值
        2.依次修改参数值为test,1.4,空串,2.1
        3.恢复参数默认值
Expect      :
        1.显示默认值为2
        2.合理报错
...
class QueryPlan(unittest.TestCase):
    ...
    def setUp(self):
        ...
        LOG.info(
            '-----Opengauss_Function_Guc_Queryplan_Case0072start----')
    ...
    def test_geqo_selection_bias(self):
        ...
        LOG.info('--步骤二：依次修改参数值为test,1.4,"''",2.1--')
        invalid_value = ['test', 1.4, "''", 2.1]
        for i in invalid_value:
            result = commonsh.execute_gsguc("set",
                                            self.constant.GSGUC_SUCCESS_MSG,
                                            f"geqo_selection_bias={i}")
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '--Opengauss_Function_Guc_Queryplan_Case0072执行完成--')
