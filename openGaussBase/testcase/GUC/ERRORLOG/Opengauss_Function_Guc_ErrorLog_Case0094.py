...
Case Name   : 查看参数debug_pretty_print的默认值为on
Description :
    1.查询debug_pretty_print参数默认值
    2.设置debug_print_parse、debug_print_rewritten和debug_print_plan的值均为on
    3.进行简单的SQL操作，查看pg日志
Expect      :
    1.查询debug_pretty_print参数，结果为on
    2.设置成功
    3.进行简单的SQL操作，查看pg日志，打印的执行计划、解析树、重写结果信息缩进展示
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0094'
    ...
    def test_main(self):
        ...
        step_txt = '----step1:查询debug_pretty_print参数默认值 expect:结果为on----'
        self.log.info(step_txt)
        ...
        step_txt = '----step2:设置ebug_print_parse、debug_print_rewritten和' \
        ...
        self.log.info(result)
        self.assertIn(self.suc_flag4, result, '执行失败:' + step_txt)
        self.assertIn(self.suc_flag5, result, '执行失败:' + step_txt)
        self.assertIn(self.suc_flag6, result, '执行失败:' + step_txt)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.TABLE_DROP_SUCCESS, drop_result,
                      '执行失败:' + step2_txt)
        self.assertTrue(msg4, '执行失败:' + step3_txt)
