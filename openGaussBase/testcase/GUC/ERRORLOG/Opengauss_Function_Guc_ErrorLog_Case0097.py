...
Case Name   : 查看参数log_checkpoints的默认值为off
Description :
    1.查看参数log_checkpoints的默认值
...
Expect      :
    ...
    2.设置enable_incremental_checkpoint、incremental_checkpoint_timeout
    3.等待incremental_checkpoint_timeout时间，查看pg日志，不打印检查点和重启点的统计
History     :
...
class ErrorLog(unittest.TestCase):
    ...
    def test_main(self):
        ...
        self.assertNotIn(self.match_msg1, result, "执行失败" + step_txt)
        match_result = len(re.findall(self.match_msg2, result))
        self.assertEqual(0, match_result, "执行失败" + step_txt)
    ...
    def tearDown(self):
        ...
        msg1 = self.pri_sh.execute_gsguc(
            'reload',
            self.constant.GSGUC_SUCCESS_MSG,
            f'enable_incremental_checkpoint={self.para2}')
        msg2 = self.pri_sh.execute_gsguc(
            'reload',
            self.constant.GSGUC_SUCCESS_MSG,
            f'incremental_checkpoint_timeout={self.para3}')
