...
Case Name   : 设置参数syslog_facility值为local4
Description :
    1.设置参数syslog_facility值为local1：gs_guc reload -N all -I all -c
    "syslog_facility=local4"
    2.修改系统配置文件，/etc/rsyslog.conf文件：local1.* /var/log/postgresql4,
...
class Errorlog(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0045 start--')
    ...
    def test_errorlog(self):
        ...
        excute_cmd6 = f'ls /var/log/postgresql4'
        msg6 = self.primaryRoot.sh(excute_cmd6).result()
        self.logger.info(msg6)
        self.assertTrue(msg6.find("/var/log/postgresql4") > -1)
    ...
    def tearDown(self):
        ...
        execute_cmd3_1 = 'rm -rf /var/log/postgresql4'
        ...
        execute_cmd4 = 'ls /var/log/postgresql4'
        ...
        execute_cmd5 = 'cat /etc/*syslog.conf |grep "local4.*  ' \
                       '/var/log/postgresql4"'
        msg5 = self.primaryRoot.sh(execute_cmd5).result()
        self.logger.info(msg5)
        self.assertFalse(msg5.find("local4.*  /var/log/postgresql4") > -1)
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0045 finish--')
