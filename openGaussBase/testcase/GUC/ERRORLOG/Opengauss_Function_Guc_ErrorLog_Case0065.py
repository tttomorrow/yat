...
Case Name   : 修改参数log_min_error_statement的默认值为debug/panic
Description :
    1.修改log_min_error_statement参数为debug
    2.执行产生error级别日志的SQL语句；“create table test;”查看pg日志
    3.修改log_min_error_statement参数为panic
    4.执行产生error级别日志的SQL语句；“create table test;”查看pg日志
Expect      :
    1.修改log_min_error_statement参数为error，修改成功
    2.执行产生errorSQL语句；“create table test;”’；查看pg日志，打印错误SQL信息
    3.修改log_min_error_statement参数为panic，修改成功
    4.执行产生error级别日志的SQL语句；“create table test;”查看pg日志，不打印错误SQL信息
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = 'tb_Guc_ErrorLog_Case0065_1'
        self.tb_name2 = 'tb_Guc_ErrorLog_Case0065_2'
        self.dir_new = os.path.join(macro.PG_LOG_PATH, 'pg_bak',
                                    'pg_errorlog_case0065')
    ...
    def test_main(self):
        ...
        step_txt = '----step1:修改log_min_error_statement参数为debug; ' \
                   'expect:修改成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
        ...
        step_txt = '----step2:执行产生errorSQL语句；查看pg日志; ' \
                   'expect:打印错误SQL信息----'
        self.log.info(step_txt)
        ...
        shell_cmd = f"cat {file_name} | grep '{sql}' -C 5"
        self.log.info(shell_cmd)
        result = self.pri_root.sh(shell_cmd).result()
        self.log.info(result)
        self.assertIn('ERROR', result, '执行失败:' + step_txt)

        step_txt = '----step3:修改log_min_error_statement参数为panic; ' \
                   'expect:修改成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
    ...
    def tearDown(self):
        ...
        step_txt1 = '----恢复参数log_min_error_statement值为初始值; expect:修改成功----'
