...
Description :
    ...
    "log_destination='csvlog'"，查看参数设置结果：show log_destination;
    2.执行重启数据库操作：gs_om - t stop && gs_om -t start
    5.查看是否在 $gaussdb_log/$username/pg_log/下生成.scv的log日志
Expect      :
    1.参数设置成功，返回值为csvlog
    2.重启数据库完成
    3.在 $gaussdb_log/$username/pg_log/下生成.scv的log日志
History     :
"""
import os
import unittest
...
class Errorlog(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('==Opengauss_Function_Guc_ErrorLog_Case0003 start==')
    ...
    def test_errorlog(self):
        ...
        execute_cmd4 = f'source {self.DB_ENV_PATH};' \
                       f'gs_guc reload -D {macro.DB_INSTANCE_PATH} -c ' \
                       f'"log_directory=\'{self.log_path}\'"'
        execute_msg4 = self.userNode.sh(execute_cmd4).result()
        ...
        self.logger.info('-------查看log_destination修改后的值为csvlog-------')
        sql_cmd2 = 'show log_destination;'
        msg2 = self.sh_primy.execut_db_sql(sql_cmd2)
        self.logger.info(msg2)
        self.common.equal_sql_mdg(msg2, 'log_destination', 'csvlog', '(1 row)',
                                  flag='1')
    ...
    def tearDown(self):
        ...
        self.assertTrue(int(msg4) == 0)
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0003 finish--')
