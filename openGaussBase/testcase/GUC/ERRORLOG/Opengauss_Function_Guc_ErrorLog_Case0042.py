...
Case Name   : 设置参数syslog_facility值为local1
Description :
    1.设置参数syslog_facility值为local1：gs_guc reload -N all -I all -c
    "syslog_facility=local1"
    2.修改系统配置文件，/etc/rsyslog.conf文件：local1.* /var/log/postgresql11,
...
class Errorlog(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0042 start--')
    ...
    def test_errorlog(self):
        ...
        excute_cmd6 = f'ls /var/log/postgresql1'
        msg6 = self.primaryRoot.sh(excute_cmd6).result()
        self.logger.info(msg6)
        self.assertTrue(msg6.find('/var/log/postgresql1') > -1)
    ...
    def tearDown(self):
        ...
        excute_cmd1 = f'source {self.DB_ENV_PATH};gs_guc reload -N all -I ' \
                      f'all -c "log_destination=\'stderr\'"'
        self.logger.info(excute_cmd1)
        msg1 = self.userNode.sh(excute_cmd1).result()
        self.logger.info(msg1)
        ...
        execute_cmd3_1 = 'rm -rf /var/log/postgresql1'
        ...
        execute_cmd4 = 'ls /var/log/postgresql1'
        ...
        execute_cmd5 = 'cat /etc/*syslog.conf |grep "local1.*  ' \
                       '/var/log/postgresql1"'
        msg5 = self.primaryRoot.sh(execute_cmd5).result()
        self.logger.info(msg5)
        self.assertFalse(msg5.find('local1.*  /var/log/postgresql1') > -1)
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0042 finish--')
