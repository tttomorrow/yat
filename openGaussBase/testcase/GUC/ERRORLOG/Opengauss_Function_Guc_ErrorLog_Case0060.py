...
Case Name   : 修改参数log_min_messages的值为fatal
Description :
    1.修改log_min_messages参数为fatal
    2.执行产生error级别日志的SQL语句；
    3.执行产生fatal级别日志的操作；
    4.修改log_min_messages参数为error
    5.执行产生error级别日志的SQL语句；
Expect      :
    1.修改log_min_messages参数为fatal，修改成功
    2.执行产生error级别日志的SQL语句；查看pg日志，不打印ERROR级别日志信息
    3.执行产生fatal级别日志的操作；打印fatal级别日志
    4.修改log_min_messages参数为error，修改成功
    5.执行产生error级别日志的SQL语句；查看pg日志，打印ERROR级别日志信息
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0060'
        self.u_name = 'u_guc_errorlog_case0060'
        self.password = macro.PASSWD_INITIAL
        self.assert_msg = 'FATAL:  Invalid username/password,login denied'
        self.dir_new = os.path.join(macro.PG_LOG_PATH, 'pg_bak',
                                    'pg_errorlog_case0060')
    ...
    def test_main(self):
        ...
        step_txt = '----step1:修改log_min_messages参数为fatal; expect:修改成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
        ...
        step_txt = '----step2:执行产生error级别日志的SQL语句；' \
                   '查看pg日志; expect:不打印ERROR级别日志----'
        self.log.info(step_txt)
        sql = f'create table {self.tb_name};'
        result = self.pri_sh.execut_db_sql(sql)
        self.log.info(result)
        self.assertTrue('ERROR' in result, '执行失败:' + step_txt)
        ...
        step_txt = '----step3:执行产生fatal级别日志的操作；' \
                   '查看pg日志; expect:打印fatal级别日志----'
        self.log.info(step_txt)
        self.log.info('----用户名密码不存在，进行连接数据库操作，产生fatal信息----')
        sql = f'show log_min_messages;'
        connect_info = f'-U {self.u_name} -W {self.password}'
        result = self.pri_sh.execut_db_sql(sql, sql_type=connect_info)
        self.log.info(result)
        self.assertTrue('FATAL' in result, '执行失败:' + step_txt)
        self.log.info('----查看pg_log----')
        shell_cmd = f'''ls -t {self.dir_new} | head -1'''
        file_name = os.path.join(self.dir_new,
                                 self.pri_root.sh(shell_cmd).result())
        self.log.info(file_name)
        shell_cmd = f"cat {file_name} | grep '{self.assert_msg}' -B 1"
        self.log.info(shell_cmd)
        result = self.pri_root.sh(shell_cmd).result()
        self.log.info(result)
        self.assertNotEqual('', result, '执行失败:' + step_txt)

        step_txt = '----step4:修改log_min_messages参数为error; ' \
                   'expect:修改成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
        ...
        step_txt = '----step5:执行产生error级别日志的SQL语句；查看pg日志; ' \
                   'expect:打印ERROR级别日志----'
        self.log.info(step_txt)
        sql = f'create table {self.tb_name};'
        result = self.pri_sh.execut_db_sql(sql)
    ...
    def tearDown(self):
        ...
        step_txt1 = '----恢复参数log_min_messages值为初始值; expect:修改成功----'
        ...
        self.assertTrue(msg2, '执行失败:' + step_txt1)
        self.assertTrue(msg3, '执行失败:' + step_txt2)
