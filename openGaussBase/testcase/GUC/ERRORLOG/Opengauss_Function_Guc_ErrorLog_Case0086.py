...
Case Name   : 修改参数debug_print_rewritten的值为on
Description :
    1.修改参数debug_print_rewritten为on
    2.进行简单的SQL操作，查看pg日志
Expect      :
    1.修改参数debug_print_rewritten为on,修改成功
    2.进行简单的SQL操作，查看pg日志，打印重写结果关键信息“LOG:  rewritten parse tree”
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0086'
        self.suc_flag = 'LOG:  rewritten parse tree'
        self.dir_new = os.path.join(macro.PG_LOG_PATH, 'pg_bak',
                                    'pg_errorlog_case0086')

    def test_main(self):
        step_txt = '----查看参数debug_print_rewritten的默认值----'
        ...
        step_txt = '----step1:修改参数debug_print_rewritten的值为on; ' \
                   'expect:修改成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
        ...
        step_txt = '----step2:进行简单的SQL操作，查看pg日志 expect:打印重写结果关键信息----'
        self.log.info(step_txt)
