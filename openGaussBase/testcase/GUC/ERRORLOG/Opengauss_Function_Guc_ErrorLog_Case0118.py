...
Description :
    1.创建新用户
    2.连接新用户，执行修改参数logging_module值为on(ALL)
    3.步骤2会话，查看参数值
...
Expect      :
    1.创建新库成功
    2.连接新用户，执行修改参数logging_module值为on(ALL)，设置成功
    3.步骤2会话，查看参数值，未生效
    4.步骤2会话进行事务操作（以XACT模块进行验证）操作成功
    5.查看pg日志，数据库会话内未产生XACT日志信息
    6.重新连接新用户进行参数查看，参数值已修改
    7.步骤6会话进行事务操作，操作成功
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.user_name = 'u_guc_errorlog_case0118'
    ...
    def test_main(self):
        ...
        step_txt = '----连接新用户，执行修改参数logging_module值为on(ALL)，进行会话内事务操作'
        self.log.info(step_txt)
        ...
        step_txt = '----step2:连接新用户，修改参数logging_module值为on(ALL) expect：成功----'
        self.log.info(step_txt)
        ...
        step_txt = '----step6:重新连接新用户进行参数查看 expect：参数值已修改----'
        self.log.info(step_txt)
