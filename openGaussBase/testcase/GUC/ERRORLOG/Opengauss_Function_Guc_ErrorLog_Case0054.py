...
Description :
    1.修改参数client_min_messages值为fatal：gs_guc reload -N all -I all -c
     "client_min_messages=fatal"
    2.查看修改后参数：show client_min_messages;
Expect      :
    1.修改成功
    2.返回值：error
History     :
...
class Errorlog(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0054 start--')
    ...
    def test_errorlog(self):
        ...
        self.common.equal_sql_mdg(msg0, 'client_min_messages', 'notice',
                                '(1 row)', flag='1')
        self.logger.info('--------设置参数client_min_messages值为fatal-------')
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c ' \
                      f'"client_min_messages=fatal"'
    ...
    def tearDown(self):
        ...
        self.common.equal_sql_mdg(msg2, 'client_min_messages', 'notice',
                                '(1 row)', flag='1')
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0054 finish--')
