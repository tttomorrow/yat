...
Case Name   : 修改参数log_disconnections的值为on
Description :
    1.修改参数log_disconnections的值为on: gs_guc reload -N all -I all -c
...
class Security(unittest.TestCase):
    ...
    def test_errorlog(self):
        ...
        if value1_return != 'on':
            ...
            self.common.equal_sql_mdg(sql_msg1, 'log_disconnections', 'on',
                                      '(1 row)', flag='1')
        ...
        excute_msg4 = self.userNode.sh(excute_cmd4).result()
        Logger.info(excute_msg4)
        self.assertTrue(excute_msg4.find('LOG:  disconnection:') > -1)
    ...
    def tearDown(self):
        ...
        Logger.info('-----恢复log_disconnections的默认值-----')
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c ' \
                      f'"log_disconnections=off"'
        msg1 = self.userNode.sh(excute_cmd1).result()
        Logger.info(msg1)
        Logger.info('-------查看log_disconnections是否恢复-------')
        sql_cmd2 = 'show log_disconnections;'
        msg2 = self.sh_primy.execut_db_sql(sql_cmd2)
        Logger.info(msg2)
        self.common.equal_sql_mdg(msg2, 'log_disconnections', 'off',
                                  '(1 row)', flag='1')
