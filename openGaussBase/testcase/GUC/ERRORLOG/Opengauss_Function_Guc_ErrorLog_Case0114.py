...
Case Name   : 修改参数log_statement的值为all
Description :
    1.修改参数log_statement的值为all
    2.连接数据库执行不同类型SQL语句
    3.查看pg_log日志文件中是否记录步骤2中语句
Expect      :
    1.修改参数log_statement的值为all，修改成功
    2.连接数据库执行不同类型SQL语句成功
    3.查看pg_log日志文件中记录步骤2中语句
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0114'
        self.dir_new = os.path.join(macro.PG_LOG_PATH, 'pg_bak',
                                    'pg_errorlog_case0114')
    ...
    def test_main(self):
        ...
        step_txt = '----step6: 查看pg_log日志文件中是否记录步骤2中语句' \
                   'expect: 打印等待记录消息---'
        self.log.info(step_txt)
