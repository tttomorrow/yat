...
Case Name   : 设置参数syslog_facility值为local6
Description :
    1.设置参数syslog_facility值为local1：gs_guc reload -N all -I all -c
    "syslog_facility=local6"
    2.修改系统配置文件，/etc/rsyslog.conf文件：local1.* syslog_facility=local6,
...
class Errorlog(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0047 start--')
    ...
    def test_errorlog(self):
        ...
        excute_cmd3 = f'echo "local6.*  /var/log/postgresql6" >> ' \
        ...
        excute_cmd6 = f'ls /var/log/postgresql6'
        msg6 = self.primaryRoot.sh(excute_cmd6).result()
        self.logger.info(msg6)
        self.assertTrue(msg6.find('/var/log/postgresql6') > -1)
    ...
    def tearDown(self):
        ...
        execute_cmd3_1 = 'rm -rf /var/log/postgresql6'
        ...
        execute_cmd4 = 'ls /var/log/postgresql6'
        ...
        execute_cmd5 = 'cat /etc/*syslog.conf |grep "local6.*  ' \
                       '/var/log/postgresql6"'
        msg5 = self.primaryRoot.sh(execute_cmd5).result()
        self.logger.info(msg5)
        self.assertFalse(msg5.find('local6.*  /var/log/postgresql6') > -1)
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0047 finish--')
