...
Case Name   : 修改参数log_statement的值为ddl
Description :
    1.修改参数log_statement的值为ddl
    2.连接数据库执行不同类型SQL语句
    3.查看pg_log日志文件中是否记录步骤2中语句
Expect      :
    1.修改参数log_statement的值为ddl，修改成功
    2.连接数据库执行不同类型SQL语句成功
    3.查看pg_log日志文件中记录步骤2中ddl语句
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0112'
        self.dir_new = os.path.join(macro.PG_LOG_PATH, 'pg_bak',
                                    'pg_errorlog_case0112')
    ...
    def test_main(self):
        ...
        step_txt = '----step1:修改参数log_statement的值为ddl expect:设置成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
                                        'log_statement=ddl')
        ...
        self.log.info('----不包含除ddl语句其他打印信息----')
        sql = f"insert into {self.tb_name} values(4,'a');" \
            f"prepare insert_sql(int,varchar(10)) as " \
            f"insert into {self.tb_name} values($1,$2);" \
            f"execute insert_sql(2,'b');" \
            f"deallocate prepare insert_sql;" \
    ...
    def tearDown(self):
        ...
        step2_txt = "----还原参数log_statement； expect: 成功----"
