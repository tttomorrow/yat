...
Case Name   : 修改参数debug_print_parse的值为on
Description :
    1.修改参数debug_print_parse为on
    2.进行简单的SQL操作，查看pg日志
Expect      :
    1.修改参数debug_print_parse为on,修改成功
    2.进行简单的SQL操作，查看pg日志，打印解析树关键信息“LOG:  parse tree”
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0083'
        self.suc_flag = 'LOG:  parse tree'
        self.dir_new = os.path.join(macro.PG_LOG_PATH, 'pg_bak',
                                    'pg_errorlog_case0083')

    def test_main(self):
        step_txt = '----查看参数debug_print_parse的默认值----'
        ...
        step_txt = '----step1:修改参数debug_print_parse的值为on; ' \
                   'expect:修改成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
        ...
        step_txt = '----step2:进行简单的SQL操作，查看pg日志 expect:打印解析树关键信息----'
        self.log.info(step_txt)
    ...
    def tearDown(self):
        ...
        step_txt1 = '----恢复参数debug_print_parse; expect:修改成功----'
        self.log.info(step_txt1)
        msg1 = self.pri_sh.execute_gsguc('reload',
                                         self.constant.GSGUC_SUCCESS_MSG,
                                         f'debug_print_parse={self.para1}')
