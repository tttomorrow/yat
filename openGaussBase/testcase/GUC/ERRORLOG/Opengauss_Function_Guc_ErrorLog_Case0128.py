...
Description :
    1.连接数据库，查看log_connections默认参数：show log_connections;
    2.重启数据库生成日志文件
    3.登录并退出数据库，后查看pg_log日志：tail -f postgresql-xxx-xx-xx.log
Expect      :
    1.返回值：on
    2.生成名为postgresql的日志文件
    3.日志中有记录客户端的连接请求信息，例如：登录用户、登录数据库等
History     :
"""
import os
import unittest
...
class Security(unittest.TestCase):
    def setUp(self):
        Logger.info('Opengauss_Function_Guc_ErrorLog_Case0128 start')
    ...
    def test_errorlog(self):
        ...
        self.common.equal_sql_mdg(sql_msg1, 'log_connections', 'off',
                                  '(1 row)', flag='1')
        ...
        excute_msg4 = self.userNode.sh(excute_cmd4).result()
        Logger.info(excute_msg4)
        self.assertFalse(excute_msg4.find('LOG:  connection authorized:') > -1)
    ...
    def tearDown(self):
        ...
        Logger.info('--Opengauss_Function_Guc_ErrorLog_Case0128 finish--')
