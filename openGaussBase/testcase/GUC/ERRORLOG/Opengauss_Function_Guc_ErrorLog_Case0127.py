...
Case Name   : 修改参数log_connections的值为on
Description :
    1.修改参数log_connections的值为on: gs_guc reload -N all -I all -c
     "log_connections=on"
    2.重启数据库生成日志文件
...
class Security(unittest.TestCase):
    def setUp(self):
        Logger.info('Opengauss_Function_Guc_ErrorLog_Case0130 start')
    ...
    def test_errorlog(self):
        ...
        sql_cmd0 = 'show log_connections;'
        sql_msg0 = self.sh_primy.execut_db_sql(sql_cmd0)
        Logger.info(sql_msg0)
        value1_return = sql_msg0.splitlines()[2].strip()
        if value1_return != 'on':
            Logger.info('-------修改参数log_connections的值为on------')
            ...
            sql_cmd1 = 'show log_connections;'
            sql_msg1 = self.sh_primy.execut_db_sql(sql_cmd1)
            Logger.info(sql_msg1)
            self.common.equal_sql_mdg(sql_msg1, 'log_connections', 'on',
                                      '(1 row)', flag='1')
        ...
        excute_msg4 = self.userNode.sh(excute_cmd4).result()
        Logger.info(excute_msg4)
        self.assertTrue('LOG:  connection authorized:' in excute_msg4)
    ...
    def tearDown(self):
        ...
        Logger.info('-----恢复log_connections的默认值-----')
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c ' \
                      f'"log_connections=off"'
        msg1 = self.userNode.sh(excute_cmd1).result()
        Logger.info(msg1)
        Logger.info('-------查看log_connections是否恢复-------')
        sql_cmd2 = 'show log_connections;'
        msg2 = self.sh_primy.execut_db_sql(sql_cmd2)
        Logger.info(msg2)
        self.common.equal_sql_mdg(msg2, 'log_connections', 'off',
                                  '(1 row)', flag='1')
        ...
        Logger.info('--Opengauss_Function_Guc_ErrorLog_Case0130 finish--')
