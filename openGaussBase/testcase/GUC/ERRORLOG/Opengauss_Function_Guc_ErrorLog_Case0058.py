...
Case Name   : 参数log_min_messages设置为warning
Description :
    1.修改log_min_messages参数为warning;
    2.执行产生warning级别日志的SQL语句；“commit”；
    3.执行产生error级别日志的SQL语句；“create table test;”’
Expect      :
    1.修改成功
    2.执行产生warning级别日志的SQL语句；“commit”；查看pg日志，打印warning级别日志
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0058'
        self.dir_new = os.path.join(macro.PG_LOG_PATH, 'pg_bak',
                                    'pg_errorlog_case0058')

    def test_main(self):
        step_txt = '----查看log_min_messages及相关参数初始配置值-----'
        ...
        step_txt = '----step1:修改log_min_messages参数为warning; expect:修改成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
