...
Case Name   : 设置参数log_rotation_size值为2097150KB
Description :
    1.设置参数log_rotation_size值为2097150：gs_guc reload -N all -I all -c
    "log_rotation_size=2097150"
    2.清空log_directory，重启数据库，查看日志文件
    3.构造日志文件大于2097150KB，执行sql语句，查看日志文件
Expect      :
    1.设置成功
    2.重启数据库成功，生成新的日志文件
    3.生成新的日志文件
...
class Security(unittest.TestCase):
    def setUp(self):
        Logger.info('----Opengauss_Function_Guc_ErrorLog_Case0039 start----')
    ...
    def test_errorlog(self):
        ...
        self.common.equal_sql_mdg(sql_msg0, 'log_statement', 'mod',
                                  '(1 row)', flag='1')
        Logger.info('-------修改log_rotation_size的值为2097150kB------')
        excute_cmd1 = f'source {self.DB_ENV_PATH};' \
                      f'gs_guc reload -N all -I all -c ' \
                      f'\'log_rotation_size=2097150kB\''
        ...
        self.common.equal_sql_mdg(sql_msg1, 'log_rotation_size', '2097150kB',
                                  '(1 row)', flag='1')
        ...
        Logger.info('------构造文件大小大于2097150kB,查看是否生成新的日志文件------')
        ...
        sleep(5)
        sql_cmd5 = 'create table errorlog0039(id int);' \
                   'insert into errorlog0039 values(5);' \
                   'drop table errorlog0039;'
    ...
    def tearDown(self):
        ...
        Logger.info('--Opengauss_Function_Guc_ErrorLog_Case0039 finish--')
