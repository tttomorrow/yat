...
Case Name   : 设置log_hostname的值为on
Description :
    1.设置log_hostname的值为on
    2.设置log_connections、log_disconnections参数为on
    3.主机gsql -h连接主机、结束连接，查看主机pg日志
    4.备机gsql -h连接主机、结束连接，查看主机pg日志
Expect      :
    1.设置log_hostname的值为on设置成功
    2.设置log_connections、log_disconnections参数为on，设置成功
    3.主机gsql -h连接主机、结束连接，查看主机pg日志，连接、断连信息展示host为主机hostname
    4.备机gsql -h连接主机、结束连接，查看主机pg日志，连接、断连信息展示host为备机hostname
History     :
...
class ErrorLog(unittest.TestCase):
    ...
    def test_main(self):
        ...
        self.log.info(f"log_hostname is {result}")
        self.para1 = result.strip().splitlines()[-2].strip()

        step_txt = '----step1:设置log_connections、log_disconnections' \
                   '参数为on expect:设置成功----'
        self.log.info(step_txt)
        msg = pri_sh.execute_gsguc('reload',
                                   self.constant.GSGUC_SUCCESS_MSG,
        ...
        self.log.info(result)
        self.assertIn('host=' + self.pri_hostname, result,
                      '执行失败:' + step_txt)
        self.assertNotIn('host=' + self.pri_ip, result,
                         '执行失败:' + step_txt)
        ...
        result = self.pri_root.sh(shell_cmd).result()
        self.log.info(result)
        self.assertIn('host=' + self.sta1_hostname, result,
                      '执行失败:' + step_txt)
        self.assertNotIn('host=' + self.sta1_ip, result,
                         '执行失败:' + step_txt)
    ...
    def tearDown(self):
        ...
        msg4 = pri_sh.execute_gsguc('reload',
                                         self.constant.GSGUC_SUCCESS_MSG,
