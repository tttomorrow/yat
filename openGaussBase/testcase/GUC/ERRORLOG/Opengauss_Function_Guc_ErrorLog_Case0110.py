...
Case Name   : 修改参数log_lock_waits的值为on
Description :
    1.修改参数log_lock_waits的值为on
...
Expect      :
    1.修改参数log_lock_waits的值为on,修改成功
    2.设置deadlock_timeout参数为10S成功
    3.创建测试表并插入数据成功
    4.打开会话，进行表相关数据update操作并等待30s
    5.打开另一会话，对步骤3的表记录再次进行update操作，并等待30s
    6.等待deadlocke_timeout时间，查看pg日志打印锁等待记录消息
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0110'
    ...
    def test_main(self):
        ...
        step_txt = '----查看参数log_lock_waits的默认值----'
        self.log.info(step_txt)
        ...
        step_txt = '----step1:修改参数log_lock_waits的值为on expect:设置成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
                                        'log_lock_waits=on')
        ...
        self.assertEqual(1, match_result1, "执行失败" + step_txt)
        self.assertEqual(1, match_result2, "执行失败" + step_txt)
    ...
    def tearDown(self):
        ...
        self.assertTrue(msg3, '执行失败:' + step3_txt)
