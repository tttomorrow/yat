...
Case Name   : 设置参数syslog_facility值为local7
Description :
    1.设置参数syslog_facility值为local1：gs_guc reload -N all -I all -c
    "syslog_facility=local7"
    2.修改系统配置文件，/etc/rsyslog.conf文件：local7.* syslog_facility=local7,
...
class Errorlog(unittest.TestCase):
    def setUp(self):
        self.logger = Logger()
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0048 start--')
    ...
    def test_errorlog(self):
        ...
        excute_cmd1 = f'source {self.DB_ENV_PATH};gs_guc reload -N all -I ' \
                      f'all -c "log_destination=\'syslog\'"'
        ...
        excute_cmd6 = 'ls  /var/log/postgresql7'
        msg6 = self.primaryRoot.sh(excute_cmd6).result()
        self.logger.info(msg6)
        self.assertTrue(msg6.find('/var/log/postgresql7') > -1)
    ...
    def tearDown(self):
        ...
        self.common.equal_sql_mdg(msg0_1, 'syslog_facility', 'local7', '(1 row)',
                                flag='1')
        ...
        execute_cmd3_1 = 'rm -rf /var/log/postgresql7'
        ...
        execute_cmd4 = 'ls /var/log/postgresql7'
        ...
        execute_cmd5 = 'cat /etc/*syslog.conf |grep "local7.*  ' \
                       '/var/log/postgresql7"'
        msg5 = self.primaryRoot.sh(execute_cmd5).result()
        self.logger.info(msg5)
        self.assertFalse(msg5.find('local7.*  /var/log/postgresql7') > -1)
        self.logger.info('--Opengauss_Function_Guc_ErrorLog_Case0048 finish--')
