...
Case Name   : 修改参数log_min_error_statement值为error
Description :
    1.修改log_min_error_statement参数为error
    2.执行产生warning级别日志的SQL语句；“commit”；
    3.执行产生error级别日志的SQL语句；“create table test;”’
Expect      :
    1.修改log_min_error_statement参数为error，修改成功
    2.执行产生warning的SQL语句；“commit”；查看pg日志，不打印“commit”SQL信息
    3.执行产生errorSQL语句；“create table test;”’；查看pg日志，打印SQL信息
History     :
...
class ErrorLog(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = 'tb_Guc_ErrorLog_Case0064'
        self.dir_new = os.path.join(macro.PG_LOG_PATH, 'pg_bak',
                                    'pg_errorlog_case0064')
    ...
    def test_main(self):
        ...
        step_txt = '----step1:修改log_min_error_statement参数为error; ' \
                   'expect:修改成功----'
        self.log.info(step_txt)
        msg = self.pri_sh.execute_gsguc('reload',
                                        self.constant.GSGUC_SUCCESS_MSG,
        ...
        step_txt = '----step2:执行产生warning的SQL语句；“commit”；查看pg日志；' \
                   '查看pg日志; expect:不打印“commit”SQL信息----'
        self.log.info(step_txt)
        sql = 'commit;'
        result = self.pri_sh.execut_db_sql(sql)
        self.log.info(result)
        self.assertTrue('COMMIT' in result, '执行失败:' + step_txt)
