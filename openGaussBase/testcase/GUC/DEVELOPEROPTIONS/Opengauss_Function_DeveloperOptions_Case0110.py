...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数enable_opfusion为无效值,合理报错
Description :
        1.查询enable_opfusion默认值
        2.修改参数值为test
        3.修改参数值为12345
        4.修改参数值为null
        5.修改参数值为空串
...
class DeveloperOption(unittest.TestCase):
    def setUp(self):
        LOG.info(
            '----Opengauss_Function_DeveloperOptions_Case0110start-----')
        self.constant = Constant()

    def test_enable_opfusion(self):
        LOG.info('--步骤1:查看默认值--')
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '----Opengauss_Function_DeveloperOptions_Case0110执行完成-----')
