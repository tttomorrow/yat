...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数advance_xlog_file_num为10,观察预期结果
Description :
        1.查询advance_xlog_file_num默认值
        2.修改参数值为10并重启数据库（一般不用配置）
        3.恢复参数默认值
Expect      :
        1.显示默认值为0
        2.设置成功
...
class DeveloperOptions(unittest.TestCase):
    def setUp(self):
        LOG.info(
            '-----Opengauss_Function_DeveloperOptions_Case0103start----')
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '-----Opengauss_Function_DeveloperOptions_Case0103finish-----')
