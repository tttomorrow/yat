...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数sql_beta_feature为无效值,合理报错
Description :
...
class DeveloperOption(unittest.TestCase):
    def setUp(self):
        LOG.info(
            '----Opengauss_Function_DeveloperOptions_Case0114start-----')
        self.constant = Constant()

    def test_sql_beta_feature(self):
        LOG.info('--步骤1:查看默认值--')
        sql_cmd = commonsh.execut_db_sql(f'''show sql_beta_feature;''')
        LOG.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '----Opengauss_Function_DeveloperOptions_Case0114执行完成-----')
