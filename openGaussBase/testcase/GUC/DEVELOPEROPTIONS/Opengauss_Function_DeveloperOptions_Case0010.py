...
class Guc(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DeveloperOptions_Case0010开始')
    ...
    def test_guc_developeroptions(self):
        ...
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(f'show ignore_checksum_failure;')
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info('Opengauss_Function_DeveloperOptions_Case0010结束')
