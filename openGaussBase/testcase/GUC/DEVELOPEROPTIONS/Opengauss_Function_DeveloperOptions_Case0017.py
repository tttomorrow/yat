...
"""
Case Type   : GUC参数
Case Name   : 使用gs_guc set方法设置参数enable_force_vector_engine为on,观察预期结果
Description :
    1.show参数默认值
    2.修改参数默认值为on
    3.show参数值
...
class Guc(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('Opengauss_Function_DeveloperOptions_Case0017开始')
        self.dbuser = Node('PrimaryDbUser')
        self.constant = Constant()
        self.commonsh = CommonSH('PrimaryDbUser')

    def test_guc_developeroptions(self):
        text = '--step1:show参数默认值;expect:参数值为off--'
        self.log.info(text)
        sql_cmd = self.commonsh.execut_db_sql(f'show '
                                              f'enable_force_vector_engine;')
    ...
    def tearDown(self):
        text = '--step4:恢复参数默认值;expect:恢复成功--'
        self.log.info(text)
        mod_msg = self.commonsh.execute_gsguc('set',
                                              self.constant.GSGUC_SUCCESS_MSG,
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info('Opengauss_Function_DeveloperOptions_Case0017结束')
