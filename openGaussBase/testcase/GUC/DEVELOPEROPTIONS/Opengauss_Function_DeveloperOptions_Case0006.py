...
class DeveloperOptions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        LOG.info(
            '------Opengauss_Function_DeveloperOptions_Case0006start-----')
    ...
    def test_allow_system_table_mods(self):
        ...
        LOG.info('--步骤4:查询oid大于一万的系统表并给表重命名--')
        sql_cmd = '''select oid,relname from pg_class where
            oid >=10000 and relkind='r';
            alter table gs_wlm_instance_history rename to gs_history_new;
            '''
        ...
        LOG.info('--步骤5:恢复表原名--')
        sql_cmd = '''alter table  gs_history_new rename to 
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '-----Opengauss_Function_DeveloperOptions_Case0006执行完成----')
