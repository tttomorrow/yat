...
class DeveloperOptions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        LOG.info(
            '------Opengauss_Function_DeveloperOptions_Case0003start----')
    ...
    def test_allow_system_table_mods(self):
        ...
        LOG.info('--步骤4:查询oid大于一万的系统表并给系统表添加约束--')
        sql_cmd = '''select oid,relname from pg_class where oid >=10000 
            and relkind='r';
            alter table gs_wlm_instance_history add constraint u_constraint03 
            unique (instancename);
            '''
        ...
        LOG.info('--步骤5:系统表gs_wlm_instance_history删除约束--')
        sql_cmd = '''alter table gs_wlm_instance_history drop 
            constraint u_constraint03;
            '''
        excute_cmd1 = f'''source {self.DB_ENV_PATH};
            gsql -d postgres -p {self.user_node.db_port} -c "{sql_cmd}" 
            '''
    ...
    def tearDown(self):
        ...
        LOG.info(sql_cmd)
        LOG.info(
            '----Opengauss_Function_DeveloperOptions_Case0003执行完成------')
