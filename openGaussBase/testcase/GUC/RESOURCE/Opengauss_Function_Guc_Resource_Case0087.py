...
"""
Case Type   : GUC
Case Name   : 修改cstore_backwrite_quantity，观察其预期结果（因当前版本暂不支持开启异步IO功能，目前正向用例只校验默认值与修改方式，预期结果后期补全）；
Description :
        1、查询cstore_backwrite_quantity默认值；
           show cstore_backwrite_quantity；
        2、恢复默认值；
Expect      :
        1、显示默认值；
        2、恢复默认值成功；
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-----------Opengauss_Function_Guc_Resource_Case0087.py start------------')
    ...
    def tearDown(self):
        ...
        self.log.info('--------------Opengauss_Function_Guc_Resource_Case0087.py执行完成---------------')
