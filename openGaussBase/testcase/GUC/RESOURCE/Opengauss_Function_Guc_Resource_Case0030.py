...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-----------Opengauss_Function_Guc_Resource_Case00030.py start------------')
    ...
    def test_startdb(self):
        ...
        # 使用设置gs_guc set设置temp_buffers为false
        msg = self.commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'temp_buffers=false')
        self.log.info(msg)
        ...
        # 使用设置gs_guc set设置temp_buffers为test
        msg = self.commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'temp_buffers=test')
        self.log.info(msg)
        self.assertFalse(msg)
        # 修改失败后重启后查看默认值
        msg = self.commonsh.restart_db_cluster()
    ...
    def tearDown(self):
        ...
        status = self.commonsh.get_db_cluster_status()
        self.assertTrue("Normal" in status)
        self.log.info( '--------------Opengauss_Function_Guc_Resource_Case0030.py执行完成---------------')
