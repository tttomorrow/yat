...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-----------Opengauss_Function_Guc_Resource_Case00026.py start------------')
    ...
    def test_startdb(self):
        ...
        # 使用设置gs_guc set设置bulk_write_ring_size为false
        msg = self.commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'bulk_write_ring_size=false')
        self.log.info(msg)
        ...
        # 使用设置gs_guc set设置bulk_write_ring_size为test
        msg = self.commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'bulk_write_ring_size=test')
        self.log.info(msg)
        self.assertFalse(msg)
        # 修改失败后重启后查看默认值
        msg = self.commonsh.restart_db_cluster()
    ...
    def tearDown(self):
        ...
        if self.res not in sql_cmd:
            msg = self.commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, f'''bulk_write_ring_size={self.res}''')
            self.log.info(msg)
            msg = self.commonsh.restart_db_cluster()
            self.log.info(msg)
        status = self.commonsh.get_db_cluster_status()
        self.assertTrue("Normal" in status)
        self.log.info( '--------------Opengauss_Function_Guc_Resource_Case0026.py执行完成---------------')
