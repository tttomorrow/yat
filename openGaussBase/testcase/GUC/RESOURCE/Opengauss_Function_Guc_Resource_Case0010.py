...
Description :
        1.查看默认值show enable_memory_limit
        2.改为空值gs_guc set -D /cluster/dn1 -c "enable_memory_limit=''"
Expect      :
        1、显示默认值；
        2、参数修改失败；
History     : 
...
class Deletaduit(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info('-----------Opengauss_Function_Guc_Resource_Case00010.py start------------')
    ...
    def test_startdb(self):
        ...
        # 设置enable_memory_limit为空值
        msg = self.commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'enable_memory_limit=none')
        self.log.info(msg)
    ...
    def tearDown(self):
        ...
        status = self.commonsh.get_db_cluster_status()
        self.assertTrue("Normal" in status)
        self.log.info('---------------------Opengauss_Function_Guc_Resource_Case00010.py执行完成-----------------------------')
