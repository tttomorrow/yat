...
Description :
         ...
         2、修改temp_file_limit为false，"test",999999999、空值等，并校验其预期结果；
Expect      :
        1、显示默认值；
        2、参数修改失败；
History     : 
...
class Deletaduit(unittest.TestCase):
    ...
    def test_startdb(self):
        ...
        self.log.info(sql_cmd)
        self.assertIn(self.res, sql_cmd)
        # 使用设置gs_guc set设置temp_file_limit为9999999999
        msg = self.commonsh.execute_gsguc('set', self.Constant.GSGUC_SUCCESS_MSG, 'temp_file_limit=9999999999')
        self.log.info(msg)
        self.assertFalse(msg)
        msg = self.commonsh.restart_db_cluster()
        self.log.info(msg)
