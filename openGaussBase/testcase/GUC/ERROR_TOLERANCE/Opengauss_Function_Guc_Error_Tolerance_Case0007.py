...
Description :
            ...
            2、修改max_query_retry_times为abc，观察预期结果；
            set max_query_retry_times to abc;
Expect      :
            1、显示默认值0；
            2、修改max_query_retry_times失败，预期结果正常；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Error_Tolerance_Case0007"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("==修改max_query_retry_times为abc期望合理报错==")
        LOGGER.info("==期望：修改失败，show参数为默认值==")
        result = COMMONSH.execut_db_sql("set max_query_retry_times to abc;")
        LOGGER.info(result)
