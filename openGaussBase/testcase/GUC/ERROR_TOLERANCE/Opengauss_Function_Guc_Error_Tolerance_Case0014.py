...
Description :
            ...
            2、修改data_sync_retry为on，观察预期结果；
            gs_guc set -D {cluster/dn1}  -c "data_sync_retry=on"
            3、恢复默认值
Expect      :
            1、显示默认值off；
            2、修改data_sync_retry成功，预期结果正常；
            3、恢复默认值成功；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Error_Tolerance_Case0014"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("修改data_sync_retry为on重启生效，期望设置成功")
