...
Description :
            ...
            2、修改max_cn_temp_file_size为16，观察预期结果；
            gs_guc set -D {cluster/dn1}  -c "max_cn_temp_file_size=16"
            3、恢复默认值
Expect      :
            1、显示默认值5GB；
            2、修改max_cn_temp_file_size成功，预期结果正常；
            3、恢复默认值成功；
History     :
"""
...
class GucTest(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Error_Tolerance_Case0010"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("修改max_cn_temp_file_size为16重启生效，期望设置成功")
        result = COMMONSH.execute_gsguc("set",
                                        self.constant.GSGUC_SUCCESS_MSG,
                                       "max_cn_temp_file_size=16")
