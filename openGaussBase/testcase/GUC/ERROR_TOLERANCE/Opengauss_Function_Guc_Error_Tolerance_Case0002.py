...
"""
Case Type   : GUC
Case Name   : 修改restart_after_crash，观察其预期结果
Description :
            1、查询restart_after_crash默认值；
            show restart_after_crash；
            2、修改restart_after_crash为0，重启使其生效，观察预期结果；
            gs_guc set -D {cluster/dn1}  -c "restart_after_crash=off"
            3、恢复默认值；
Expect      :
            1、显示默认值；
            2、修改成功 预期结果正常；
...
class GuctestCase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Error_Tolerance_Case0002"
                    "开始执行==")
    ...
    def test_guc(self):
        ...
        LOGGER.info("修改restart_after_crash为off重启生效，期望设置成功")
