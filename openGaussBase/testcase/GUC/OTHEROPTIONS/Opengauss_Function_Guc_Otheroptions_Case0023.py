...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方式修改job_queue_processes为100，观察预期结果；
Description :
    1、查询job_queue_processes默认值,show job_queue_processes;
    2、使用gs_guc set方式修改job_queue_processes为100，
       gs_guc set -D {cluster/dn1} -c "job_queue_processes=100";
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        logger.info("===Opengauss_Function_Guc_Otheroptions_Case0023开始执行===")
    ...
    def test_guc(self):
        ...
        res_set = self.commonsh.execute_gsguc('set',
                                              self.constant.GSGUC_SUCCESS_MSG,
    ...
    def tearDown(self):
        logger.info("======恢复默认值======")
        sql_cmd4 = self.commonsh.execut_db_sql('''show job_queue_processes; ''')
        logger.info(sql_cmd4)
        if "10" not in sql_cmd4:
        ...
        status = self.commonsh.get_db_cluster_status()
        self.assertTrue("Degraded" in status or "Normal" in status)
        logger.info("===Opengauss_Function_Guc_Otheroptions_Case0023执行结束===")
