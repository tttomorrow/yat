...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数wal_receiver_timeout为无效值,
              合理报错
Description :
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '--Opengauss_Function_Guc_HA_Replication_Case0048start----')
    ...
    def test_wal_receiver_timeout(self):
        ...
        self.log.info('--步骤2:修改参数值test, -1,"''",125.55,2147483648')
        invalid_value = ['test', -1, "''", 125.55, 2147483648]
        for i in invalid_value:
            result = self.commonsh.execute_gsguc("set",
                                                 constant.GSGUC_SUCCESS_MSG,
                                                 f"wal_receiver_timeout={i}")
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0048执行完成-----')
