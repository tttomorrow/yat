...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数max_standby_streaming_delay为无效值,
             合理报错
Description :
        1.查询max_standby_streaming_delay默认值
        2.修改参数值为字符test
        3.修改参数值为空串
        4.修改参数值为-2
        5.修改参数值为2147483648
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '--Opengauss_Function_Guc_HA_Replication_Case0042start----')
        self.commonsh = CommonSH('PrimaryDbUser')

    def test_max_standby_streaming_delay(self):
        self.log.info('--步骤1:查看默认值--')
        sql_cmd = self.commonsh.execut_db_sql('show'
                                              ' max_standby_streaming_delay;')
        self.log.info(sql_cmd)
        self.assertEqual('3s', sql_cmd.split('\n')[2].strip())
        self.log.info('--步骤2:修改参数值test, "''", -2,2147483648,255.252--')
        invalid_value = ['test', "''", -2, 2147483648, 255.252]
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0042执行完成-----')
