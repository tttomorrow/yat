...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数wal_receiver_connect_retries为无效值,
              合理报错
Description :
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '--Opengauss_Function_Guc_HA_Replication_Case0052start----')
    ...
    def test_wal_receiver_connect_retries(self):
        ...
        for i in invalid_value:
            result = self.commonsh.execute_gsguc("set",
                                               ...
                                               f"'wal_receiver_connect_retries"
                                                 f"={i}")
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0052执行完成-----')
