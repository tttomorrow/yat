...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数wait_dummy_time为500,观察预期结果
Description :
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0033start-----')
    ...
    def test_wait_dummy_time(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql('show wait_dummy_time;')
        self.log.info(sql_cmd)
        self.assertEqual('500', sql_cmd.split('\n')[2].strip())
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0033finish---')
