...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数vacuum_defer_cleanup_age为无效值,
              合理报错
Description :
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '--Opengauss_Function_Guc_HA_Replication_Case0022start----')
    ...
    def test_vacuum_defer_cleanup_age(self):
        ...
        self.log.info('步骤2:修改参数值test,-1, "''",1000001,125.255')
        invalid_value = ['test', -1, "''", 1000001, 125.255]
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0022执行完成-----')
