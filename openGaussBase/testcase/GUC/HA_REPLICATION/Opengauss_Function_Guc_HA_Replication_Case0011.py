...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数max_cached_tuplebufs为5000,观察预期结果
Description :
        1.查询max_cached_tuplebufs默认值
        2.主机修改参数值为5000并重启数据库
        3.恢复参数默认值
Expect      :
        1.显示默认值为8192
        2.设置成功显示5000
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '--Opengauss_Function_Guc_HA_Replication_Case0011start---')
        self.commonsh = CommonSH('PrimaryDbUser')

    def test_max_cached_tuplebufs(self):
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '-Opengauss_Function_Guc_HA_Replication_Case0011finish--')
