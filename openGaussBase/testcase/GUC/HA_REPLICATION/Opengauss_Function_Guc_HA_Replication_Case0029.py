...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数ha_module_debug为on,
             观察预期结果
Description :
        1.查询ha_module_debug默认值
        2.修改参数值为on并重启数据库
        3.恢复参数默认值
Expect      :
        1.显示默认值为off
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0029start-----')
        self.commonsh = CommonSH('PrimaryDbUser')

    def test_ha_module_debug(self):
        self.log.info('--步骤1:查看默认值--')
        sql_cmd = self.commonsh.execut_db_sql('show ha_module_debug;')
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0029finish---')
