...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数most_available_sync为无效值,合理报错
Description :
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '-Opengauss_Function_Guc_HA_Replication_Case0016start----')
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0016执行完成-----')
