...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数enable_incremental_catchup为off,
             观察预期结果
Description :
        1.查询enable_incremental_catchup默认值
        2.修改参数值为off并重启数据库
        3.恢复参数默认值
Expect      :
        1.显示默认值为on
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0031start-----')
        self.commonsh = CommonSH('PrimaryDbUser')

    def test_enable_incremental_catchup(self):
    ...
    def tearDown(self):
        ...
        sql_cmd = self.commonsh.execut_db_sql(
            'show enable_incremental_catchup;')
        self.log.info(sql_cmd)
        if "on" != sql_cmd.split('\n')[-2].strip():
            mod_msg = self.commonsh.execute_gsguc('set',
                                                  constant.GSGUC_SUCCESS_MSG,
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0031finish---')
