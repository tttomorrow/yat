...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数enable_mix_replication为无效值,
              合理报错
Description :
        1.查询enable_mix_replication默认值
        2.修改参数值为字符test
        3.修改参数值为1234
        4.修改参数值为空串
        5.恢复参数默认值
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '--Opengauss_Function_Guc_HA_Replication_Case0020start----')
        self.commonsh = CommonSH('PrimaryDbUser')

    def test_enable_mix_replication(self):
        self.log.info('--步骤1:查看默认值--')
        sql_cmd = self.commonsh.execut_db_sql('show enable_mix_replication;')
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0020执行完成-----')
