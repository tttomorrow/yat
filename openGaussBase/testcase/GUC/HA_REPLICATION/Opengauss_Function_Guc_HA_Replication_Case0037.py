...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数hot_standby为off,观察预期结果
Description :
        1.查询hot_standby默认值
        2.修改参数值为off并重启数据库
        3.恢复参数默认值
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0037start-----')
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0037finish---')
