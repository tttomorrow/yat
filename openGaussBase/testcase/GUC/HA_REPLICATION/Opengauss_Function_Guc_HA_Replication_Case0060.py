...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set方法设置参数replconninfo4为无效值,
              合理报错
Description :
        1.查询replconninfo4默认值
        2.修改参数值为字符test;1234
        3.恢复参数默认值
Expect      :
        1.显示默认值为空
...
class HAReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '--Opengauss_Function_Guc_HA_Replication_Case0060start----')
        self.commonsh = CommonSH('PrimaryDbUser')

    def test_replconninfo1(self):
        self.log.info('--步骤1:查看默认值--')
        sql_cmd = self.commonsh.execut_db_sql('show replconninfo4;')
    ...
    def tearDown(self):
        ...
        self.log.info(sql_cmd)
        self.log.info(
            '---Opengauss_Function_Guc_HA_Replication_Case0060执行完成-----')
