...
Description :
    ...
    show unique_sql_clean_ratio;
    3、清空记录，主机执行100 unique_sql，备机执行1 不触发自动淘汰，查看hash table记录条数
...
Expect      :
    1、显示默认值
    2、参数修改成功，校验修改后系统参数
    3、主机查看hash table记录条数为100，备机查看为1
    4、恢复默认值成功；
History     :
"""
import unittest
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0030 start==")
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0030 finish=")
