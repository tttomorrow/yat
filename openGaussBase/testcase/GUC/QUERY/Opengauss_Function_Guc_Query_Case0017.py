...
"""
Case Type   : GUC
Case Name   : 使用gs_guc set设置track_stmt_session_slot为边界值
Description :
    ...
    2、设置log_min_duration_statement为0;
       gs_guc set -D {cluster/dn1} -c "log_min_duration_statement=0";
    3、使用gs_guc set方式修改为边界值0,重启校验是否生效;
       gs_guc set -D {cluster/dn1} -c "track_stmt_session_slot=0";
...
Expect      :
    1、显示默认值，1000 & 30min;
    2、参数设置成功;
    3、参数设置成功，重启数据库校验参数生效;
    4、创建表成功，查询表记录，track_stmt_session_slot为0，语句执行将不会被跟踪
    5、参数设置成功;
    6、恢复默认值成功;
"""
...
class GucTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name = "tb_query_case0017"
    ...
    def test_guc(self):
        ...
        logger.info("======步骤五：设置track_stmt_session_slot为2147483647===")
        set_cmd = self.comsh.execute_gsguc('set',
                                           self.constant.GSGUC_SUCCESS_MSG,
