...
"""
Case Type   : GUC
Case Name   : 手动执行reset_unique_sql清空记录 再执行unique_sql触发自动淘汰
Description :
    1、查询参数默认值；
    ...
    2、修改unique_sql_clean_ratio为0.15，重启使其生效，并校验其预期结果；
    gs_guc set -N all -I all -c "enable_auto_clean_unique_sql=on"
    gs_guc set -N all -I all -c "unique_sql_clean_ratio=0.15"
    gs_om -t stop && gs_om -t start
    show unique_sql_clean_ratio;
    3、清空记录，执行100 unique_sql，不触发自动淘汰，查看hash table记录条数
    select reset_unique_sql('GLOBAL','ALL',100);
    select count(va) from (select get_instr_unique_sql() as va);
    gsql -r -d -p -f test.sql
    select count(va) from (select get_instr_unique_sql() as va);
    4、手动执行reset_unique_sql清空记录
    select reset_unique_sql('GLOBAL','ALL',100);
    5、执行unique_sql 100+1条 触发自动淘汰 查看记录条数
    gsql -r -d -p -f test.sql
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0025 start==")
    ...
    def test_unique_sql(self):
        ...
        LOGGER.info("步骤3：清空记录后执行unique_sql 100+1条 触发自动淘汰 查看记录条数")
        ...
        LOGGER.info("步骤5：清空记录后执行unique_sql 100+1条 触发自动淘汰 查看记录条数")
        result = COMMONSH.execut_db_sql("select "
            "reset_unique_sql('GLOBAL','ALL',100);"
            "select count(*) from dbe_perf.statement;")
        LOGGER.info(result)
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0025 finish=")
