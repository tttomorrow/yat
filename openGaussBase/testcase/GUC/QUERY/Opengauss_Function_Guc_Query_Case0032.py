...
"""
Case Type   : GUC
Case Name   : 条目满后执行非unique_sql，不触发自动淘汰，查看记录条数
Description :
    1、查询参数默认值；
    ...
    show unique_sql_clean_ratio;
    3、清空记录，执行100 unique_sql，再执行5非unique_sql，不触发自动淘汰，查看hash table记录条数
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0032 start==")
    ...
    def test_unique_sql(self):
        ...
        self.assertIn("1\n", result)
        for i in range(2):
            LOGGER.info(f"第{i}次执行大量unique sql")
            for j in range(48):
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0032 finish=")
