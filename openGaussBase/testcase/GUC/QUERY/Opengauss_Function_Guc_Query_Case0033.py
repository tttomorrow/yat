...
"""
Case Type   : GUC
Case Name   : 2参数设置off，自动淘汰被禁用，查看记录条数
Description :
    1、查询参数默认值；
    ...
    2、设置enable_resource_track为off,修改unique_sql_clean_ratio为0.1，
    重启使其生效，并校验其预期结果；
    ...
    show enable_resource_track;
    show instr_unique_sql_count;
    3、清空记录，执行1 unique_sql,查看hash table记录条数
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0033 start==")
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0033 finish=")
