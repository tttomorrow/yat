...
"""
Case Type   : GUC
Case Name   : enable_auto_clean_unique_sql=off 不触发自动淘汰
Description :
    1、查询参数默认值；
    ...
    2、修改enable_auto_clean_unique_sql为off，重启使其生效，并校验其预期结果；
    gs_guc reload -D {dn1} -c "enable_auto_clean_unique_sql=off"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0044 start==")
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0044 finish=")
