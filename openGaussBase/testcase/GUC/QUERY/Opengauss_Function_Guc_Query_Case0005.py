...
"""
Case Type   : GUC
Case Name   : 修改enable_instr_rt_percentile为off，观察预期结果；
Description :
    1、查询enable_instr_rt_percentile默认值；
    show enable_instr_rt_percentile;
    2、修改enable_instr_rt_percentile为off，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c "enable_instr_rt_percentile=off"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("==Opengauss_Function_Guc_Query_Case0005开始执行==")
    ...
    def test_guc(self):
        ...
        logger.info("方式一修改enable_instr_rt_percentile为off，"
                    "重启使其生效，期望：设置成功")
    ...
    def tearDown(self):
        ...
        is_started = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in is_started or "Normal" in is_started)
        logger.info("==Opengauss_Function_Guc_Query_Case0005执行结束==")
