...
"""
Case Type   : GUC
Case Name   : 修改instr_rt_percentile_interval为60，观察预期结果；
Description :
    1、查询instr_rt_percentile_interval默认值；
    show instr_rt_percentile_interval;
    2、修改instr_rt_percentile_interval为60，重启使其生效，并校验其预期结果；
    gs_guc set -D {cluster/dn1} -c "instr_rt_percentile_interval=60"
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        logger.info("==Opengauss_Function_Guc_Query_Case0009开始执行==")
    ...
    def test_guc(self):
        ...
        logger.info("方式一修改instr_rt_percentile_interval为60，"
                    "重启使其生效，期望：设置成功")
        result = COMMONSH.execute_gsguc("set",
                                        self.constant.GSGUC_SUCCESS_MSG,
                                       "instr_rt_percentile_interval=60")
    ...
    def tearDown(self):
        ...
        is_started = COMMONSH.get_db_cluster_status()
        self.assertTrue("Degraded" in is_started or "Normal" in is_started)
        logger.info("==Opengauss_Function_Guc_Query_Case0009执行结束==")
