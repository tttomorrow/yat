...
"""
Case Type   : GUC
Case Name   : 主备分别设置不同最大条目+淘汰比例 触发自动淘汰
Description :
    1、主备查询参数默认值；
...
Expect      :
    1、显示默认值；
    2、参数修改成功，校验修改后系统参数
    3、主机查看hash table记录条数为81，备机查看hash table记录条数为17
    4、恢复默认值成功；
History     :
"""
import unittest
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0029 start==")
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0029 finish=")
