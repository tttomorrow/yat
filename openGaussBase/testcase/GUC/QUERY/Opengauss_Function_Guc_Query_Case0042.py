...
"""
Case Type   : GUC
Case Name   : 打开自动淘汰，多次修改淘汰比例 查看生效
Description :
    1、查询参数默认值；
    ...
    2、修改unique_sql_clean_ratio为0.2，重启使其生效，并校验其预期结果；
    gs_guc set -N all -I all -c "enable_auto_clean_unique_sql=on"
    gs_guc set -N all -I all -c "unique_sql_clean_ratio=0.2"
    ...
    select count(va) from (select get_instr_unique_sql() as va);
    4、修改unique_sql_clean_ratio为0.15，并校验其预期结果；
    gs_guc reload -N all -I all -c "unique_sql_clean_ratio=0.15"
    gs_om -t stop && gs_om -t start
    show unique_sql_clean_ratio;
    5、执行100+1 unique_sql，查看hash table记录条数
    select reset_unique_sql('GLOBAL','ALL',100);
    select count(va) from (select get_instr_unique_sql() as va);
    gsql -r -d -p -f test.sql
    select count(va) from (select get_instr_unique_sql() as va);
    6、alter system set修改unique_sql_clean_ratio为0，并校验其预期结果；
    alter system set unique_sql_clean_ratio to 0;
    show unique_sql_clean_ratio;
    7、执行100+1 unique_sql，查看hash table记录条数
    select reset_unique_sql('GLOBAL','ALL',100);
    select count(va) from (select get_instr_unique_sql() as va);
    gsql -r -d -p -f test.sql
    select count(va) from (select get_instr_unique_sql() as va);
...
Expect      :
    1、显示默认值；
    2、参数修改成功，校验修改后系统参数
    3、执行成功 触发自动淘汰 淘汰0.1 查询记录条数91
    4、参数修改成功，校验修改后系统参数
    5、执行成功 触发自动淘汰 淘汰0.15 查询记录条数86
    6、参数修改成功，校验修改后系统参数
    7、执行成功 触发自动淘汰 淘汰0.1 查询记录条数91
    8、恢复默认值成功
History     :
"""
import unittest
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0042 start==")
    ...
    def test_unique_sql(self):
        ...
        self.set_gs_guc("unique_sql_clean_ratio", "0.2", "reload")
        ...
        LOGGER.info("步骤4：修改unique_sql_clean_ratio为0.15")
        self.set_gs_guc("unique_sql_clean_ratio", "0.15", "reload")
        sql_cmd = COMMONSH.execut_db_sql("show unique_sql_clean_ratio;")
        LOGGER.info(sql_cmd)
        self.assertIn("0.15", sql_cmd)

        LOGGER.info("步骤5：清空记录后触发自动淘汰")
        result = COMMONSH.execut_db_sql("select "
            "reset_unique_sql('GLOBAL','ALL',100);"
            "select count(*) from dbe_perf.statement;")
        LOGGER.info(result)
        ...
        LOGGER.info("步骤6：修改unique_sql_clean_ratio为0.15")
        self.set_gs_guc("unique_sql_clean_ratio", "0", "reload")
        sql_cmd = COMMONSH.execut_db_sql("show unique_sql_clean_ratio;")
        LOGGER.info(sql_cmd)
        self.assertIn("0.1", sql_cmd)

        LOGGER.info("步骤7：清空记录后触发自动淘汰")
        result = COMMONSH.execut_db_sql("select "
            "reset_unique_sql('GLOBAL','ALL',100);"
            "select count(*) from dbe_perf.statement;")
        LOGGER.info(result)
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0042 finish=")
