...
"""
Case Type   : GUC
Case Name   : unique_sql_clean_ratio=0.2 触发自动淘汰
Description :
    1、查询参数默认值；
    ...
    show unique_sql_clean_ratio;
    3、清空hsah table，执行100+1 unique_sql，触发自动淘汰,查看hash table记录条数
    ...
    select count(va) from (select get_instr_unique_sql() as va);
    4.再次执行被清理的unique_sql 1条，查看记录条数
    gsql -r -d -p -f test.sql
    select count(va) from (select get_instr_unique_sql() as va);
    5、恢复默认值；
...
Expect      :
    ...
    3、触发自动淘汰,查看hash table记录条数为81
    4、再次执行被清理的unique_sql，查看记录条数为82
    5、恢复默认值成功；
History     :
"""
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0022 start==")
    ...
    def test_unique_sql(self):
        ...
        LOGGER.info(f"步骤4：再次执行被清理的unique_sql 1条，查看记录条数")
        result = COMMONSH.execut_db_sql("select count(*) "
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0022 finish=")
