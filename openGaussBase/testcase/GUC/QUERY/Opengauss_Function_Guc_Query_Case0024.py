...
"""
Case Type   : GUC
Case Name   : unique_sql_clean_ratio=0.1 instr_unique_sql_count小改大后 触发自动淘汰
Description :
    1、查询参数默认值；
    ...
    gs_guc set -N all -I all -c "instr_unique_sql_count=50"
    ...
    select count(va) from (select get_instr_unique_sql() as va);
    4、修改参数instr_unique_sql_count为100 查看记录条数
    gs_guc reload -N all -I all -c "instr_unique_sql_count=100"
    show instr_unique_sql_count;
    select count(va) from (select get_instr_unique_sql() as va);
...
Expect      :
    1、显示默认值；
    2、参数修改成功，校验修改后系统参数值为0.2；
    3、查看hash table记录条数为50
    4、修改成功 查询为100 记录条数为50
    5、执行成功 触发自动淘汰 查看hash table记录条数为81
    6、恢复默认值成功；
History     :
"""
import unittest
...
class Guctestcase(unittest.TestCase):
    def setUp(self):
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0024 start==")
    ...
    def test_unique_sql(self):
        ...
        LOGGER.info("步骤3：清空记录后执行unique sql")
        ...
        LOGGER.info(f"步骤4：修改参数instr_unique_sql_count为100 查看记录条数")
        self.set_gs_guc("instr_unique_sql_count", "100", "reload")
        sql_cmd = COMMONSH.execut_db_sql("show instr_unique_sql_count;")
        LOGGER.info(sql_cmd)
        self.assertTrue("100", sql_cmd.splitlines()[-2].strip())
        result = COMMONSH.execut_db_sql("select count(*) "
            "from dbe_perf.statement;")
        LOGGER.info(result)
        self.assertIn("42\n", result)

        LOGGER.info("步骤5: 执行unique_sql 100+1条 触发自动淘汰 查看记录条数")
    ...
    def tearDown(self):
        ...
        LOGGER.info(status)
        self.assertTrue("Normal" in status or "Degraded" in status)
        LOGGER.info("==Opengauss_Function_Guc_Query_Case0024 finish=")
