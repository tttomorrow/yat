...
Case Name   : 创建ustore表,组合使用where条件,比较运算符,right join...using,
              order by排序,逻辑运算符与模糊查询like进行查询索引推荐
Description :
    ...
    8.使用where条件,比较运算符,right join...using，
      order by排序进行查询索引推荐
    9.使用where条件,比较运算符,right join...using，order by排序,逻辑运算符与
       模糊查询like进行查询索引推荐
    10.清理环境
Expect      :
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.common = Common()
        self.Con = Constant()
        self.table1 = 't_table_ai_indexadv_ustore_0007'
        self.table2 = 't_table_ai_indexadv1_ustore_0007'
        self.procedure1 = 'p_procedure_ustore_insert_0007'
        self.procedure2 = 'p_procedure_ustore1_insert_0007'
    ...
    def test_ai(self):
        ...
        self.assertIn('p_procedure_ustore_insert_0007', insert_data,
                      text + step)
        self.assertIn('p_procedure_ustore1_insert_0007', insert_data,
                      text + step)
        self.assertIn('100000', insert_data, text + step)
        self.assertIn('98001', insert_data, text + step)

        step = 'step8:使用where条件，比较运算符，right join...using连接，' \
               'order by排序进行查询索引推荐 ' \
               'expect:返回推荐的索引列'
        self.logger.info(step)
        index_advise = self.primary_sh.execut_db_sql(f'''
            ...
            from {self.table1} right join {self.table2} 
            using(col_int)
        ...
        step = 'step9:使用where条件,比较运算符,right join...using,order ' \
               'by排序,逻辑运算符与模糊查询like进行查询索引推荐 ' \
               'expect:返回推荐的索引列'
        self.logger.info(step)
        index_advise = self.primary_sh.execut_db_sql(f'''
            select * from {self.index_advise}('select {self.table1}.col_int,
