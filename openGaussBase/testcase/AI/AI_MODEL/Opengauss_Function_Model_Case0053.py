...
Case Name   : predict by使用指定训练特征是表不存在的列名
Description :
    1.建表并插入数据
    2.创建model
    3.查找系统表gs_model_warehouse
    4.预测数据表数据,指定训练特征是表不存在的列名
    5.清理环境
Expect      :
    1.建表并插入数据成功
    2.创建model成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.Con = Constant()
        self.table = 't_Model_tab_0053'
        self.model = 'm_model_predict_0053'
    ...
    def test_ai(self):
        ...
        step = 'step4:预测数据表数据,指定训练特征是表不存在的列名' \
               'expect:测数据表数据失败'
        self.logger.info(step)
        predict_data = self.primary_sh.execut_db_sql(
            f'''select predict by {self.model}(features column1) from 
                {self.table};''')
        self.logger.info(predict_data)
        self.assertIn('ERROR:  column "column1" does not exist',
                      predict_data, "执行失败" + step)
