...
class AI(unittest.TestCase):
    ...
    def test_ai_snapshot(self):
        ...
        step = 'step6:使用svm算子创建model expect:使用svm算子创建model成功'
        self.logger.info(step)
        create_model = self.primary_sh.execut_db_sql(
            f'''create model {self.model} using svm_classification features 
            second_attack target trait_anxiety > 10 from {self.snapshot1};''')
        ...
        predict_data = self.primary_sh.execut_db_sql(
            f'''select predict by {self.model} (features
            sum(second_attack)) from {self.snapshot1};''')
    ...
    def tearDown(self):
        ...
        self.assertIn(self.Con.TABLE_DROP_SUCCESS, clean_environment,
                      "执行失败" + step)
        self.assertIn(self.snapshot, clean_environment, "执行失败" + step)
        self.assertIn(self.snapshot1, clean_environment, "执行失败" + step)
        self.assertIn(self.snapshot2, clean_environment, "执行失败" + step)
