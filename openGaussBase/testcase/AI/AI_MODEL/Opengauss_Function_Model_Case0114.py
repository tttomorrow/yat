...
Case Name   : linear_regression创建mode,带超参seed
Description :
    1.建表并插入数据
    2.linear_regression创建mode,带不同值的超参seed
...
Expect      :
    1.建表并插入数据成功
    2.创建model成功
    3.返回hyperparametersvalues值中含超参seed值1
    4.返回hyperparametersvalues值中含超参seed值2
...
class AI(unittest.TestCase):
    def setUp(self):
        ...
        self.Con = Constant()
        self.table = 't_table_model_mot_0114'
        self.model1 = 'm_model_true_seed_0114'
        self.model2 = 'm_model_true_seed1_0114'
    ...
    def test_ai(self):
        ...
        step = 'step2:linear_regression创建mode带不同值的超参seed' \
               'expect:创建成功'
        self.logger.info(step)
        ...
        step = 'step3:查找系统表gs_model_warehouse中新建model1的' \
               ...
               'expect:返回hyperparametersvalues值中含超参seed值100'
        self.logger.info(step)
        select_model1 = self.primary_sh.execut_db_sql(
            f'''select hyperparametersvalues from gs_model_warehouse 
                where modelname='{self.model1}';''')
        self.logger.info(select_model1)
        self.assertEqual(len(select_model1.split(',')), 9, "执行失败" + step)
        self.assertIn('100', select_model1.split(',')[7], "执行失败" + step)

        step = 'step4:查找系统表gs_model_warehouse中新建model2的' \
               'hyperparametersvalues值 ' \
               'expect:返回hyperparametersvalues值中含超参seed值2'
        self.logger.info(step)
        select_model2 = self.primary_sh.execut_db_sql(
            f'''select hyperparametersvalues from gs_model_warehouse 
                where modelname='{self.model2}';''')
        self.logger.info(select_model2)
        self.assertEqual(len(select_model2.split(',')), 9, "执行失败" + step)
        self.assertIn('2', select_model2.split(',')[7], "执行失败" + step)
