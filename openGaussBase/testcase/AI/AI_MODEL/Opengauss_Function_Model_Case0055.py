...
Case Name   : predict by使用指定训练特征为数据
Description :
    1.建表并插入数据
    2.创建model
    3.查找系统表gs_model_warehouse
    4.预测数据表数据,使用指定训练特征为数据
    5.清理环境
Expect      :
    1.建表并插入数据成功
    2.创建model成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.Con = Constant()
        self.table = 't_Model_tab_0055'
        self.model = 'm_model_predict_0055'
    ...
    def test_ai(self):
        ...
        step = 'step4:预测数据表数据,使用指定训练特征为数据' \
               'expect:返回预测的表数据'
        self.logger.info(step)
        predict_data = self.primary_sh.execut_db_sql(
            f'''select predict by {self.model}(features 
                array[135.529122074100002,1.38508608310000003,
                75.1363695022999991,-155.490917820600004,
                -23.7378253056999995,33.6490592628000016,
                -7.9599672791999998]::double  precision[]) 
                from (select array[135.529122074100002,1.38508608310000003,
                75.1363695022999991,155.490917820600004,
                23.7378253056999995,33.6490592628000016, 
                -7.9599672791999998]::double  precision[]);''')
