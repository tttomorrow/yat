...
Case Name   : predict by使用数据结构等与model中不一致
Description :
    1.建表并插入数据
    2.创建model
    3.查找系统表gs_model_warehouse
    4.预测数据表数据,使用数据结构等与model中不一致
    5.清理环境
Expect      :
    1.建表并插入数据成功
    2.创建model成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.Con = Constant()
        self.table = 't_Model_tab_0078'
        self.model = 'm_model_predict_0078'
    ...
    def test_ai(self):
        ...
        step = 'step4:预测数据表数据,使用数据结构等与model中不一致' \
               'expect:测数据表数据失败'
        self.logger.info(step)
        predict_data = self.primary_sh.execut_db_sql(
            f'''select predict by {self.model} 
                (features array[fixed_acidity,volatile_acidity]) 
                from {self.table};

                select predict by {self.model}(features fixed_acidity,
                volatile_acidity) from {self.table};

                select predict by {self.model}(features fixed_acidity) 
                from {self.table};''')
        self.logger.info(predict_data)
        str1 = 'ERROR:  column "fixed_acidity" does not exist'
        self.assertIn(str1, predict_data, "执行失败" + step)
        self.assertEqual(predict_data.count(str1), 3, "执行失败" + step)
