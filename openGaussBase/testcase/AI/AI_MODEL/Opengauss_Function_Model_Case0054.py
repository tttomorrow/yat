...
Case Name   : predict by使用多余model中的features数量
Description :
    1.建表并插入数据
    2.创建model
    3.查找系统表gs_model_warehouse
    4.预测数据表数据,使用多余model中的features数量
    5.清理环境
Expect      :
    1.建表并插入数据成功
    2.创建model成功
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.Con = Constant()
        self.table = 't_Model_tab_0054'
        self.model = 'm_model_predict_0054'
    ...
    def test_ai(self):
        ...
        step = 'step4:预测数据表数据,使用多余model中的features数量' \
               'expect:预测数据表数据失败'
        self.logger.info(step)
        predict_data = self.primary_sh.execut_db_sql(
            f'''select predict by {self.model}(features treatment,
                trait_anxiety,id) from {self.table};''')
        self.logger.info(predict_data)
        str1 = 'ERROR:  Invalid number of features'
        str2 = 'for prediction, provided 3, expected 2'
        self.assertIn(str1, predict_data, "执行失败" + step)
        self.assertIn(str2, predict_data, "执行失败" + step)
