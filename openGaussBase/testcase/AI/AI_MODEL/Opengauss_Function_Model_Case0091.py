...
Case Name   : svm创建mode带正确的超参optimizer和错误的超参optimizer
Description :
    1.建表并插入数据
    2.svm创建mode带正确的超参optimizer
    3.svm创建mode带错误的超参optimizer
...
class SQL(unittest.TestCase):
    def setUp(self):
        ...
        self.Con = Constant()
        self.table = 't_table_model_mot_0091'
        self.model1 = 'm_model_true_optimizer_0091'
        self.model2 = 'm_model_true_optimizer1_0091'
        self.model3 = 'm_model_wrong_optimizer_0091'
    ...
    def test_explain(self):
        ...
        step = 'step2:svm创建mode带正确的超参optimizer' \
               'expect:创建成功'
        self.logger.info(step)
        create_model = self.primary_sh.execut_db_sql(
            f'''create model {self.model1} using  svm_classification
                ...
                {self.table} with optimizer = 'gd';

                create model {self.model2} using svm_classification
                features treatment,trait_anxiety target second_attack from 
                {self.table} with optimizer = 'ngd';''')
        self.logger.info(create_model)
        str1 = 'MODEL CREATED. PROCESSED 1'
        self.assertTrue(create_model.count(str1) == 2, "执行失败" + step)

        step = 'step3:svm创建mode带错误的超参optimizer' \
               'expect:创建失败'
        self.logger.info(step)
        create_model = self.primary_sh.execut_db_sql(
            f'''create model {self.model3} using svm_classification
                features treatment,trait_anxiety target second_attack from 
                {self.table} with optimizer = 'abc';''')
        self.logger.info(create_model)
        self.assertIn('ERROR:  Invalid hyperparameter value for optimizer.',
        ...
        self.assertIn('gd', select_model, "执行失败" + step)
        self.assertIn('ngd', select_model, "执行失败" + step)
