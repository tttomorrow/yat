...
Case Name   : linear_regression创建mode,带超参optimizer
Description :
    1.建表并插入数据
    2.linear_regression创建mode,带超参optimizer
...
Expect      :
    1.建表并插入数据成功
    2.创建model成功
    3.返回hyperparametersvalues值中含超参optimizer值1
    4.返回hyperparametersvalues值中含超参optimizer值2
...
class AI(unittest.TestCase):
    def setUp(self):
        ...
        self.Con = Constant()
        self.table = 't_table_model_mot_0110'
        self.model1 = 'm_model_true_optimizer_0110'
        self.model2 = 'm_model_true_optimizer1_0110'
    ...
    def test_ai(self):
        ...
        step = 'step2:linear_regression创建mode带超参optimizer' \
               'expect:创建成功'
        self.logger.info(step)
        ...
        step = 'step3:查找系统表gs_model_warehouse中新建model1的' \
               ...
               'expect:返回hyperparametersvalues值中含超参optimizer值gd'
        self.logger.info(step)
        ...
        self.assertIn('ngd', select_model.split(',')[5], "执行失败" + step)

        step = 'step4:查找系统表gs_model_warehouse中新建model2的' \
               'hyperparametersvalues值' \
               'expect:返回hyperparametersvalues值中含超参optimizer值ngd'
        self.logger.info(step)
        select_model = self.primary_sh.execut_db_sql(
            f'''select hyperparametersvalues from gs_model_warehouse 
        ...
        self.logger.info(select_model)
        self.assertEqual(len(select_model.split(',')), 9, "执行失败" + step)
        self.assertIn('gd', select_model.split(',')[5], "执行失败" + step)

        step = 'step5:查询系统表 expect:返回全部新建model的modelname值'
        self.logger.info(step)
        select_model = self.primary_sh.execut_db_sql(
