...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case091_1'
        self.subname1 = "sub_case091_1"
        self.pubname1 = "pub_case091_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:两个集群均创建表_UUID的类型 expect:成功--'
        self.log.info(text)
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(i int,i2 UUID primary key);'
        ...
        sql = f"insert into {self.tb_name1} values" \
            f"(1,'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11');" \
        ...
        sql = f"UPDATE {self.tb_name1}  SET " \
            f"i2='A0EEBC99-9C0B-4EF8-BB6D-6BB9BD380A12';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('1 | a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a12',
                      result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
