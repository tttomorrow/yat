...
"""
Case Type   : 基础功能
Case Name   : 创建发布订阅后，修改订阅端表结构，仅列位置置换
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case019_1'
        self.subname = "sub_case019"
        self.pubname = "pub_case019"
    ...
    def test_pubsub(self):
        ...
        sql = f"drop table {self.tb_name1};" \
            f"create table {self.tb_name1}(t text , i int);"
        ...
        sql = f"insert into {self.tb_name1} values(NULL,2) ;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        ...
        self.log.info(text)
        sql = f"drop table {self.tb_name1};" \
            f"create table {self.tb_name1}(i int , t text);"
        result = self.commsh_sub.execut_db_sql(sql,
                                               self.user_param_sub,
                                               None,
                                               macro.DB_ENV_PATH_REMOTE1)
        ...
        sql = f"insert into {self.tb_name1} values(3, 'new') ;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('3 | new', result, '执行失败' + text)
        self.assertIn('1 row', result, '执行失败' + text)
