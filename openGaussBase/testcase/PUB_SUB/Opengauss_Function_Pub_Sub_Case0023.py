...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = 'tb_pubsub_case023_1'
        self.subname = "sub_case023"
        self.pubname = "pub_case023"
    ...
    def test_pubsub(self):
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(2, 'conflict-B');"
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(2, 'conflict-A');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        ...
        self.assertIn('2 | conflict-B', result, '执行失败' + text)
        self.assertIn('1 | first', result, '执行失败' + text)
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(3, 'ok');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        ...
        self.log.info(result)
        self.assertIn('1 | first', result, '执行失败' + text)
        self.assertIn('2 | conflict-A', result, '执行失败' + text)
        self.assertIn('3 | ok', result, '执行失败' + text)
