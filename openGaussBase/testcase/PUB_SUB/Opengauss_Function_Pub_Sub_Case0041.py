...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case041_1'
        self.tb_name2 = 'tb_pubsub_case041_2'
        self.tb_name3 = 'tb_pubsub_case041_3'
        self.subname1 = "sub_case041_1"
        self.pubname1 = "pub_case041_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step5:查询是否同步  expect:数据均更新--"
        self.log.info(text)
        ...
        self.assertIn('1 | 66666^^&     | 中文   | test', result, '执行失败' + text)

        text = '--step6:修改发布端  expect:失败--'
        self.log.info(text)
        sql = f"ALTER PUBLICATION {self.pubname1} " \
            f"SET(publish='insert', publish='delete');" \
            f"ALTER PUBLICATION {self.pubname1} SET(publish=NULL);" \
            f"ALTER PUBLICATION {self.pubname1} SET(publish_=NULL);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn('unrecognized "publish" value: "null"',
                      result, '执行失败:' + text)
        self.assertIn('conflicting or redundant options',
                      result, '执行失败:' + text)
        self.assertIn('unrecognized publication parameter: publish_',
                      result, '执行失败:' + text)

        text = '--step7:修改发布端  expect:成功--'
        self.log.info(text)
        sql = f"ALTER PUBLICATION {self.pubname1} SET (publish='update');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} " \
            f"values(2, '66^^&', '中文', 'test');" \
            f"update {self.tb_name2} set use_filename='update' where id=1;" \
            f"delete from {self.tb_name3} ;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(self.constant.INSERT_SUCCESS_MSG),
        ...
        text = "--step9:查询是否同步  expect:tb_pubsub_case041_2更新，其余未更新--"
        self.log.info(text)
        sql_select = f"select * from {self.tb_name1};" \
            f"select * from {self.tb_name2};" \
            f"select * from {self.tb_name3};"
        ...
        self.log.info(result)
        self.assertEqual(result.count('1 row'), 3, '执行失败' + text)
        self.assertIn('1 | update       | 中文   | test',
