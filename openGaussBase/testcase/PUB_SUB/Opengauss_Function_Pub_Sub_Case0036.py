...
Expect      :
    ...
    8.成功
    9.成功
    10.s_pubsub_case036_1.tb_pubsub_case036_1同步，其余未同步
    11.失败is already member of publication "pub1"
    12.成功
    13.成功
    14.未同步
    15.成功
    16.tb_pubsub_case036_1更新，其余未更新
History     :
"""
import unittest
import os
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case036_1'
        self.schema_name1 = 's_pubsub_case036_1'
        self.subname1 = "sub_case036_1"
        self.pubname1 = "pub_case036_1"
        self.pubname2 = "pub_case036_2"
    ...
    def test_pubsub(self):
        ...
        sql = f"create schema {self.schema_name1};" \
            ...
            f"(i int primary key, t boolean) WITH (FILLFACTOR=20);" \
            f"create table {self.tb_name1}(i int primary key, t boolean) " \
            f"WITH (FILLFACTOR=20);"
        ...
        text = '--step6:修改发布端名称不存在 expect:失败--'
        self.log.info(text)
        sql = f"ALTER PUBLICATION not_exits add table " \
            f"{self.schema_name1}.{self.tb_name1};"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertNotIn(self.constant.alter_pub_succ_msg,
                         result, '执行失败' + text)

        text = '--step7:修改发布端，添加不存在表 expect:失败--'
        self.log.info(text)
        sql = f"ALTER PUBLICATION {self.pubname1} add table tb_not_exist;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertNotIn(self.constant.alter_pub_succ_msg,
        ...
        self.log.info(text)
        sql = f"insert into {self.schema_name1}.{self.tb_name1} " \
            f"values(2,true);select pg_sleep(5.5);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = "--step10:查询是否同步 " \
               "expect:s_pubsub_case036_1.tb_pubsub_case036_1同步，其余未同步--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
                                               None,
        ...
        self.assertIn('2 | t', result, '执行失败' + text)

        text = '--step11:修改发布，添加表已存在在发布中 ' \
               'expect:失败is already member of publication "pub1"--'
        self.log.info(text)
        sql = f"ALTER PUBLICATION {self.pubname1} add table " \
            f"{self.schema_name1}.{self.tb_name1};"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn("is already member of publication",
                      result, '执行失败' + text)

        text = '--step12:创建发布端 expect:成功--'
        self.log.info(text)
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(2,true);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)

        text = "--step14:查询是否同步 expect:未同步--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
                                               None,
        ...
        text = '--step15:添加存在于其他发布中表 expect:成功--'
        self.log.info(text)
        ...
        text = '--step15:修改表数据 expect:成功--'
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(3,true);" \
            f"insert into {self.schema_name1}.{self.tb_name1} values(3,true);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
