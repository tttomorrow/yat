...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.sta2_userdb_pub = Node(node='Standby2DbUser')
        ...
        self.tb_name1 = 'tb_pubsub_case082_1'
        self.tb_name2 = 'tb_pubsub_case082_2'
        self.subname1 = "sub_case082_1"
        self.pubname1 = "pub_case082_1"
        ...
        self.log.info(result)
        self.values_path = os.path.join(self.parent_path_pub, 'case082.sql')
    ...
    def test_pubsub(self):
        ...
        self.log.info(text)
        create_sql = f'CREATE TABLE {self.tb_name1}(i int ,i2 json);' \
            f'CREATE TABLE {self.tb_name2}(i int ,i2 jsonb);' \
            f'alter table {self.tb_name1} REPLICA IDENTITY full;' \
            f'alter table {self.tb_name2} REPLICA IDENTITY full;'
        ...
        self.log.info(text)
        self.sta2_userdb_pub.sh(f"touch {self.values_path}")
        f = open(self.values_path, "w", encoding="utf-8")
        f.write(f"INSERT INTO {self.tb_name1} VALUES(10,'[\"abcd\"]');")
        f.write(f"INSERT INTO {self.tb_name2} VALUES(1,'   "
                f"[1, \" a \", {{\"a\"   :1    }}]  ');")
        f.close()
        cmd = f"scp {self.pri_userdb_pub.ssh_user}@" \
            f"{self.sta2_userdb_pub.ssh_host}:{self.values_path} " \
            f"{self.parent_path_pub}"
        self.log.info(cmd)
        self.pri_userdb_pub.sh(cmd)
        cmd = f"source {macro.DB_ENV_PATH};" \
            f"gsql -p{self.pri_userdb_pub.db_port} " \
            f"-d{self.pri_userdb_pub.db_name} -f {self.values_path} " \
        ...
        text = "--step5:修改1列数据 集群A:发布端 " \
               "expect:成功（i2='{\"name\":\"zhangsan\"}'，其他数据均不变）--"
        self.log.info(text)
        sql = f"UPDATE {self.tb_name1} set  i2=" \
              + "'{{\\\"name\\\":\\\"zhangsan\\\"}}' WHERE i=10;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = "--step6:查询数据 集群B:订阅端 expect:数据无法同步--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
                                               None,
        ...
        flg = f'FATAL:  could not identify an equality operator for type json'
    ...
    def tearDown(self):
        ...
        cmd = f"mv " \
            f"{os.path.join(self.parent_path_pub, 'pg_hba.conf')} "\
            f"{os.path.join(macro.DB_INSTANCE_PATH, 'pg_hba.conf')};" \
            f"rm -rf {self.values_path};"
        self.log.info(cmd)
        result = self.pri_userdb_pub.sh(cmd).result()
        self.log.info(result)
        cmd = f"rm -rf {self.values_path};"
        result = self.sta2_userdb_pub.sh(cmd).result()
        self.log.info(result)
        cmd = f"mv " \
            f"{os.path.join(self.parent_path_sub, 'pg_hba.conf')} "\
            f"{os.path.join(macro.DB_INSTANCE_PATH_REMOTE1, 'pg_hba.conf')} "
