...
"""
Case Type   : 基础功能
Case Name   : 发布指定表不存在关联表，创建发布订阅后，修改发布端及订阅端表结构
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case021_1'
        self.subname = "sub_case021"
        self.pubname = "pub_case021"
    ...
    def test_pubsub(self):
        ...
        sql = f"drop table {self.tb_name1};" \
            f"create table {self.tb_name1}(t text primary key, i int);"
        ...
        text = '--step9:修改表结构,并插入数据 expect:成功--'
        self.log.info(text)
        sql = f"drop table {self.tb_name1};" \
        ...
        sql = f"insert into {self.tb_name1} values( 2, 'NULL');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        ...
        self.log.info(result)
        self.assertIn('1 row', result, '执行失败' + text)
        self.assertIn('2 | NULL', result, '执行失败' + text)
