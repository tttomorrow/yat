...
"""
Case Type   : 基础功能
Case Name   : 发布内容为二级分区（range-range）
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case136_1'
        self.subname1 = "sub_case136_1"
        self.pubname1 = "pub_case136_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:两个集群均创建二级分区表range-range expect:成功--'
        self.log.info(text)
        create_sql = f"CREATE TABLE {self.tb_name1}( " \
            f" month_code VARCHAR2 ( 30 ) NOT NULL primary key," \
            f"    dept_code  VARCHAR2 ( 30 ) NOT NULL ," \
            f"    user_no    VARCHAR2 ( 30 ) NOT NULL ," \
            f"    sales_amt  int)" \
            f"PARTITION BY RANGE (month_code) " \
            f"SUBPARTITION BY RANGE (dept_code)(" \
            f"  PARTITION p_201901 VALUES LESS THAN( '201903' )  (" \
            f"    SUBPARTITION p_201901_a VALUES LESS THAN( '6' )," \
            f"    SUBPARTITION p_201901_b VALUES LESS THAN( '9' )  )," \
            f"  PARTITION p_201902 VALUES LESS THAN( '201908' )  " \
            f"(    SUBPARTITION p_201902_a VALUES LESS THAN( '6' )," \
            f"    SUBPARTITION p_201902_b VALUES LESS THAN( '9' )  ));"
        ...
        sql = f"insert into {self.tb_name1} values('201902', '1', '1', 1);" \
            f"insert into {self.tb_name1} values('201903', '8', '1', 1);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = "--step5:查询是否同步 expect:数据按顺序同步--"
        self.log.info(text)
        sql = f"update {self.tb_name1} set month_code='201907'" \
            f" where month_code='201902';" \
            f"update {self.tb_name1} set month_code='201906' " \
            f"where month_code='201903';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('201906', result, '执行失败:' + text)
        self.assertIn('201907', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)
