...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case086_1'
        self.subname1 = "sub_case086_1"
        self.pubname1 = "pub_case086_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:两个集群均创建表_范围型 expect:成功--'
        self.log.info(text)
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(i int ,i2 int4range primary key,i3 int8range,' \
            f'i4 numrange,i5 tsrange,i6 tstzrange,i7 daterange )'
        ...
        sql = f"insert into {self.tb_name1} values" \
            f"(1108,int4range(10,20),int8range(100,200), numrange(1,5)," \
            f"'[2010-01-01 14:30, 2010-01-01 15:30)'," \
            f"'(2013-12-11 16:00:00+08,  2014-01-01 15:30)'," \
            f"'[12-10-2010,1-1-2011]');select pg_sleep(5.5);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        sql = f"UPDATE {self.tb_name1} SET " \
            f"i5='[2010-01-01 14:30, 2010-01-01 15:30]' " \
            f"WHERE  i5 = '[2010-01-01 14:30, 2010-01-01 15:30)';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('["2010-01-01 14:30:00","2010-01-01 15:30:00"]',
                      result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
