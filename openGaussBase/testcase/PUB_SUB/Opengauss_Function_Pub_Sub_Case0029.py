...
"""
Case Type   : 基础功能
Case Name   : 创建发布，仅发布insert,update操作
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case029_1'
        self.tb_name2 = 'tb_pubsub_case029_2'
        self.subname = "sub_case029"
        self.pubname = "pub_case029"
    ...
    def test_pubsub(self):
        ...
        sql = f"create table {self.tb_name1}(i int primary key, t text);" \
            ...
            f"insert into {self.tb_name1} values(1, 'first'),(2, 'second');" \
        ...
        self.log.info(text)
        sql = f"update {self.tb_name2} set c= 'update' where i <2;" \
        ...
        self.assertIn('1 |  1 | update', result, '执行失败' + text)
        self.assertIn('2 |  2 | second', result, '执行失败' + text)
        self.assertEqual(result.count('2 rows'), 2, '执行失败' + text)

        text = '--step6:修改表数据 expect:成功--'
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(3, 'not fresh');" \
            f"insert into {self.tb_name2} values(3, 3, 'not fresh');" \
            f"{sql_select}"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        self.assertEqual(result.count('3 | not fresh'), 2, '执行失败' + text)
        ...
        self.assertIn('1 | update', result, '执行失败' + text)
        self.assertIn('2 | second', result, '执行失败' + text)
        self.assertIn('3 | not fresh', result, '执行失败' + text)
        self.assertIn('1 |  1 | update', result, '执行失败' + text)
        self.assertIn('2 |  2 | second', result, '执行失败' + text)
        self.assertIn('3 |  3 | not fresh', result, '执行失败' + text)
        self.assertEqual(result.count('3 rows'), 2, '执行失败' + text)

        text = '--step8:修改表数据expect:成功--'
        self.log.info(text)
        sql = f"delete from {self.tb_name2} where c='second';" \
            f"delete from {self.tb_name1} where t='second';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.DELETE_SUCCESS_MSG, result, '执行失败' + text)
        self.assertNotIn('second', result, '执行失败' + text)

        text = "--step9:查询是否同步 expect:不同步--"
        self.log.info(text)
