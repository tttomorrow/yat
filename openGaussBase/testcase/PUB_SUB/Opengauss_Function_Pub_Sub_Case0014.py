...
"""
Case Type   : 基础功能
Case Name   : FOR ALL TABLES 创建发布后，再创建表
Description :
    1.创建发布端
    2.创建订阅
    3.在两个集群创建表
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case014_1'
        self.tb_name2 = 'tb_pubsub_case014_2'
        self.tb_name3 = 'tb_pubsub_case014_3'
        self.subname = "sub_case014"
        self.pubname = "pub_case014"
    ...
    def test_pubsub(self):
        ...
        self.assertIn('1 | first', result.splitlines()[7], '执行失败' + text)

        text = '--step6:在两个集群创建表 expect:成功--'
        self.log.info(text)
        sql = f"create table {self.tb_name3}(i int primary key, t text);"
        result = self.commsh_pub.execut_db_sql(
            sql, sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(self.constant.TABLE_CREATE_SUCCESS),
        ...
        sql = f"insert into {self.tb_name1} values(2, 'second');" \
            f"insert into {self.tb_name2} values(2, 'second');" \
            f"insert into {self.tb_name3} values(2, 'second');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(self.constant.INSERT_SUCCESS_MSG),
        ...
        self.assertEqual(result.count('2 | second'), 3, '执行失败' + text)
