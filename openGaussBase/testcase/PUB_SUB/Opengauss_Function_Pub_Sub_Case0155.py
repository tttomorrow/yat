...
"""
Case Type   : 基础功能
Case Name   : 密文创建订阅，且密码与发布端用户密码不符
Description :
    1.两个集群创建用户
    2.两个集群创建表
    3.创建发布(发布端执行)
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case155_1"
        self.pubname1 = "pub_case155_1"
        ...
        self.user_name1 = "u_case155_1"
        self.user_name2 = "u_case155_2"
        self.passwd = "!@#$%^&*<>?:!_1231sf"
        ...
        self.tb_name = 'public.t_pubsub_case155'
    ...
    def test_pubsub(self):
        ...
        text = "--step5:创建订阅(订阅端执行，端口只能是DN端口+1) expect:失败--"
        self.log.info(text)
        ...
        self.assertIn('WARNING:  apply worker could not connect to '
                      'the remote server', result, '执行失败:' + text)
        ...
        text = "--step7:订阅端查询数据 expect:数据不同步--"
        self.log.info(text)
        sql = f"select * from {self.tb_name};"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertNotIn('999', result, '执行失败:' + text)
