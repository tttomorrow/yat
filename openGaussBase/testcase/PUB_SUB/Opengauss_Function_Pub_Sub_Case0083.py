...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case083_1'
        self.subname1 = "sub_case083_1"
        self.pubname1 = "pub_case083_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:两个集群均创建表_HLL类型 expect:成功--'
        self.log.info(text)
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(i int primary key,set hll(14))'
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values (1, hll_empty());" \
        ...
        text = "--step5:修改1列数据 集群A:成功  --"
        self.log.info(text)
        sql = f"UPDATE {self.tb_name1} set  set= " \
            f"hll_add(set, hll_hash_integer(12345)) where i = 1;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
                      'dbb2b8496d654cb1dbb2b8496d6500',
                      result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
