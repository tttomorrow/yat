...
Expect      :
    ...
    3.成功
    4.成功
    5.(1, 'first')同步，(2, 'new')未同步，订阅端pg_log提示
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = 'tb_pubsub_case016_1'
        self.subname = "sub_case016"
        self.pubname = "pub_case016"
    ...
    def test_pubsub(self):
        ...
        self.assertEqual(result.count(self.constant.TABLE_CREATE_SUCCESS),
                         2, '执行失败:' + text)
        sql = f"create table {self.tb_name1}(i int primary key, t text);" \
            f"insert into {self.tb_name1} values(2, 'second^');"
        ...
        text = "--step5:查询是否同步 expect:(1, 'first')同步，(2, 'new')未同步，" \
               "订阅端pg_log提示FATAL:  duplicate key value " \
        ...
        text = '--step6:删除发布端造成订阅端主键冲突数据，再修改数据 expect成功--'
        self.log.info(text)
        sql = f"delete from {self.tb_name1} where i=2;" \
        ...
        text = '--step7:订阅端删除冲突数据 expect:删除冲突后同步--'
        self.log.info(text)
        sql = f"delete from {self.tb_name1} where i=2;" \
        ...
        self.assertEqual(result.count('1 row'),
                         2, '执行失败' + text)
