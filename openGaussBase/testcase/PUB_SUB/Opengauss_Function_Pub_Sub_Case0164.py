...
"""
Case Type   : 基础功能
Case Name   : 用户是运维管理员operator admin
Description :
    1.发布端创建运维管理员
    2.两个集群创建用户并创建表（发布端使用user3用户，订阅端使用tpcc用户创建表）
    3.创建发布(发布端执行)
    4.创建订阅，订阅信息中发布用户为运维管理员(订阅端执行，端口只能是DN端口+1)
Expect      :
    1.成功
    2.成功
    3.成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case164_1"
        self.pubname1 = "pub_case164_1"
        ...
        self.user_name1 = "u_case164_1"
        self.user_name2 = "u_case164_2"
        ...
        self.tb_name = 't_pub_sub_case164'
    ...
    def test_pubsub(self):
        ...
        text = "--step1:发布端创建运维管理员 expect:成功--"
        self.log.info(text)
        sql = f"create user {self.user_name2} with OPRADMIN  " \
        ...
        text = "--step2:两个集群创建用户并创建表（发布端使用user3用户，订阅端使用tpcc用户创建表） " \
               "expect:成功--"
        self.log.info(text)
        ...
        result = self.commsh_pub.execut_db_sql(
            sql, f"-U {self.user_name2} -W {self.passwd}")
        self.log.info(result)
        self.assertIn(self.constant.TABLE_CREATE_SUCCESS, result,
                      '执行失败:' + text)
        sql = f"create schema {self.user_name2};" \
            f"create table {self.user_name2}.{self.tb_name}" \
            f"(i int primary key);"
        ...
        text = "--step4:创建订阅，订阅信息中发布用户为运维管理员(订阅端执行，端口只能是DN端口+1) expect:失败--"
        self.log.info(text)
    ...
    def tearDown(self):
        ...
        sql = f"DROP SUBSCRIPTION if exists {self.subname1};" \
            f"drop table if exists {self.user_name2}.{self.tb_name};" \
            f"drop schema {self.user_name2}"
        ...
        sql = f"drop user {self.user_name1} cascade;" \
