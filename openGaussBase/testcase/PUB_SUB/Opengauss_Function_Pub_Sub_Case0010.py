...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case010_1'
        self.tb_name2 = 'tb_pubsub_case010_2'
        self.tb_name3 = 'tb_pubsub_case010_3'
        self.tb_name4 = 'tb_pubsub_case010_4'
        self.subname = "sub_case010"
        self.pubname = "pub_case010"
    ...
    def test_pubsub(self):
        ...
        self.log.info(text)
        sql = f"CREATE TABLE {self.tb_name1}(id int " \
            ...
            f"create table {self.tb_name2}( c_id integer primary key, " \
            f"c_name varchar(16) default 'omm' NOT NULL, " \
            f"c_class varchar(20) not null)partition " \
            f"by LIST(c_id)(partition p1 values (1,2,3)," \
            f"partition p2 values (4,5,6),partition p3 values (7,8,9)," \
            f"partition p4 values (10,11,12),partition p5 values (13,14,15)," \
            f"partition p6 values (16,17,18));" \
            f"create table {self.tb_name3}(c_id integer primary key," \
            f"c_name varchar(16) default 'omm',c_class varchar(20) not null" \
            f")partition by hash(c_id)(partition p1," \
            f"partition p2,partition p3," \
            f"partition p4,partition p5,partition p6);" \
            f"create table {self.tb_name4}(c_id integer ," \
            f"c_date DATE primary key,c_info varchar(20) not null)" \
            f"partition by range(c_date)interval('2 day')" \
            f"(partition p1 values less than ('2021-06-01 00:00:00')," \
            f"partition p2 values less than ('2021-06-03 00:00:00')," \
            f"partition p3 values less than ('2021-06-05 00:00:00')," \
            f"partition p4 values less than ('2021-06-07 00:00:00')," \
            f"partition p5 values less than ('2021-06-09 00:00:00'));"
        ...
        sql = f"insert into {self.tb_name1} values" \
            f"(1, 'first', '%一', ''),(60, 'first', '%二', '')," \
            f"(90, 'first', '%三', '');select pg_sleep(5.5);" \
            f"insert into {self.tb_name2} values(1, 'first', '%一')," \
            f"(9, 'first', '%二'),(12, 'first', '%三'); " \
            f"insert into {self.tb_name3} values(1, 'first', '%一')," \
            f"(9, 'first', '%二'),(12, 'first', '%三');" \
            f"insert into {self.tb_name4} values" \
            f"(1, '2021-05-30 00:00:00', '1-1')," \
        ...
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertEqual(result.count('3 rows'), 3, '执行失败' + text)
        self.assertEqual(result.count('2 rows'), 1, '执行失败' + text)
        result_pub = self.commsh_pub.execut_db_sql(sql)
        self.log.info(result_pub)
        self.assertEqual(result_pub, result, '执行失败' + text)
