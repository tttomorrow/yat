...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case167_1"
        self.pubname1 = "pub_case167_1"
        ...
        self.user_name1 = "u_case167_1"
        self.user_name2 = "u_case167_2"
    ...
    def test_pubsub(self):
        ...
        text = "--step5:查询进程中是否有密码相关 expect:最后转成的log文件中，无密码--"
        self.log.info(text)
        cmd = f"lsof -i:{self.pri_userdb_sub.db_port}"
        self.log.info(cmd)
        result =  self.com_pub.get_sh_result(self.pri_root_sub, cmd)
        pid = result.splitlines()[1].split()[1].strip()
        cmd = f"cd {self.parent_path_sub};" \
            f"gcore -o {pid}.data {pid} > /dev/null 2>&1"
        self.log.info(cmd)
        self.com_pub.get_sh_result(self.pri_userdb_sub, cmd)
        cmd = f"cd {self.parent_path_sub};" \
            f"strings {pid}.data.{pid} > {pid}.log"
        self.log.info(cmd)
        self.com_pub.get_sh_result(self.pri_userdb_sub, cmd)
        cmd = f"cd {self.parent_path_sub};" \
            f"cat {pid}.log|grep {self.passwd} ; " \
            f"cat {pid}.log|grep {macro.COMMON_PASSWD} ; " \
            f"rm -rf {pid}.log;rm -rf {pid}.data;"
        self.log.info(cmd)
        result = self.com_pub.get_sh_result(self.pri_userdb_sub, cmd)
