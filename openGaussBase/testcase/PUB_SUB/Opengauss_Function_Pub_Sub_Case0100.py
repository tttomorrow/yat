...
Description :
    ...
    2、在两个集群中创建相同字段表
    3、创建发布端(集群A)
    4、创建订阅，其他配置信息正常，发布名称存在，子句指定订阅参数enable，验证默认值true（集群B）
    4.1:创建订阅   expect:创建订阅成功
    4.2:向发布端表中插入数据   expect:成功
    4.3:查询集群B中表中数据是否同步   expect:数据已同步
    5、创建订阅，其他配置信息正常，发布名称存在，子句指定订阅参数enable，验证值false（集群B）
    5.1:创建订阅   expect:创建订阅成功
    5.2:向发布端表中插入数据   expect:成功
    5.3:查询集群B中表中数据是否同步   expect:数据未同步（订阅只设置，未启动）
    6、创建订阅，其他配置信息正常，发布名称存在，子句指定订阅参数enable，验证不符合规范值（集群B）   expect:创建订阅失败
Expect      :
    1：成功
    2：成功
    3：成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case100_1"
        self.subname2 = "sub_case100_2"
        self.subname3 = "sub_case100_3"
        self.pubname1 = "pub_case100_1"
        self.pubname2 = "pub_case100_2"
        self.tb_name1 = "t_case100_1"
        self.tb_name2 = "t_case100_2"
    ...
    def test_pubsub(self):
        ...
        sql = f"CREATE PUBLICATION {self.pubname1} " \
            ...
            f"CREATE PUBLICATION {self.pubname2} for table {self.tb_name2} ;"
        result = self.commsh_pub.execut_db_sql(sql,
        ...
        text = "--step4.2:向发布端表中插入数据   expect:成功--"
        self.log.info(text)
        ...
        text = "--step4.3:查询集群B中表中数据是否同步   expect:数据已同步--"
        self.log.info(text)
        ...
        sql = f"CREATE SUBSCRIPTION {self.subname2} CONNECTION " \
            ...
            f"dbname={self.pri_userdb_pub.db_name} " \
            f"password={macro.COMMON_PASSWD}' " \
            f"PUBLICATION {self.pubname2} with(enabled=false);"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertNotIn(self.constant.SQL_WRONG_MSG[1], result,
        ...
        text = "--step5.2:向发布端表中插入数据   expect:成功--"
        self.log.info(text)
        ...
        text = "--step5.3:查询集群B中表中数据是否同步   expect:数据未同步（订阅只设置，未启动）--"
        self.log.info(text)
        sql_select = f"select * from {self.tb_name2}"
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_u,
                                               None,
                                               macro.DB_ENV_PATH_REMOTE1)
        ...
        text = "--step6: 创建订阅，其他配置信息正常，发布名称存在，子句指定订阅参数enable，验证不符合规范值（集群B）" \
               "expect:创建订阅失败--"
        self.log.info(text)
        result = self.commsh_sub.execute_generate(
            macro.COMMON_PASSWD, env_path=macro.DB_ENV_PATH_REMOTE1)
        ...
        sql = f"CREATE SUBSCRIPTION {self.subname3} CONNECTION " \
            ...
            f"dbname={self.pri_userdb_pub.db_name} " \
            f"password={macro.COMMON_PASSWD}' " \
            f"PUBLICATION {self.pubname2} with(enabled=abc);"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('ERROR:  enabled requires a Boolean value', result,
                      '执行失败:' + text)
