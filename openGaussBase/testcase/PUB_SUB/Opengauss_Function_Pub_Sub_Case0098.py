...
"""
Case Type   : 基础功能
Case Name   : 创建新订阅，连接发布者端dbname验证
Description :
    1、创建用户，赋予所有权限
    2、创建发布端(集群A)
    3、创建订阅（集群B）
    3.1:订阅名正常，发布端连接port正常，连接订阅端user具有流复制权限，密码为加密密码，数据库为发布库   expect:成功
    3.2:订阅名正常，发布端连接port正常，连接订阅端user具有流复制权限，密码为加密密码，数据库为非发布库  expect:成功
Expect      :
    1：成功
    2：成功
    3.1：成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case098_1"
        self.pubname1 = "pub_case098_1"
        self.tb_name1 = "t_case098_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step3.1:订阅名正常，发布端连接port正常，连接订阅端user具有流复制权限，" \
               "密码为加密密码，数据库为发布库   expect:成功--"
        self.log.info(text)
