...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case080_1'
        self.subname1 = "sub_case080_1"
        self.pubname1 = "pub_case080_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(i int ,i2 bit(3) primary key ,i3 bit varying(5))'
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values (10,B'101',B'00');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = "--step5:修改1列数据 集群A:发布端 " \
               "expect:成功（i2=B'10'::bit(3)，其他数据均显示正确）--"
        self.log.info(text)
        sql = f"select pg_sleep(5.5);" \
            f"UPDATE {self.tb_name1} SET i2=B'10'::bit(3) WHERE i2=B'101';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('10 | 100 | 00', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)
