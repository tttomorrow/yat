...
"""
Case Type   : 基础功能
Case Name   : 修改订阅，验证重命名订阅
Description :
    1:创建用户，赋予所有权限
    2:在两个集群中创建相同字段表
    3:创建发布端(集群A)
    4.1:创建订阅（集群B）  expect:创建订阅成功
    4.2:修改订阅，重命名订阅名称已存在   expect:修改订阅失败
    4.3:修改订阅，重命名订阅名称不符合规范   expect:修改订阅失败
    4.4:修改订阅，重命名订阅名称不存在且符合规范   expect:修改订阅成功
    4.5:向发布端表中插入数据   expect:成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case114_1"
        self.pubname1 = "pub_case114_1"
        self.tb_name1 = "public.t_case114_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step4.2:修改订阅，重命名订阅名称已存在   expect:修改订阅失败--"
        self.log.info(text)
        ...
        text = "--step4.3:修改订阅，重命名订阅名称不符合规范   expect:修改订阅失败--"
        self.log.info(text)
        sql = f"alter SUBSCRIPTION {self.subname1} " \
            f"rename to @sub22_new@;select pg_sleep(5);"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        ...
        text = "--step4.4:修改订阅，重命名订阅名称不存在且符合规范   expect:修改订阅成功--"
        self.log.info(text)
        sql = f"alter SUBSCRIPTION {self.subname1} " \
