...
"""
Case Type   : 基础功能
Case Name   : 以二进制模式发布订阅普通行存表，astore表
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅指定已二进制模式传输
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case170_1'
        self.tb_name2 = 'tb_pubsub_case170_2'
        self.tb_name3 = 'tb_pubsub_case170_3'
        self.subname = "sub_case170"
        self.pubname = "pub_case170"
    ...
    def test_pubsub(self):
        ...
        text = '--step3:创建订阅指定已二进制模式传输 expect:成功--'
        self.log.info(text)
        ...
        self.assertEqual(result.count('1 | first'), 3, '执行失败' + text)
        self.assertEqual(result.count('1 |  1 | first'), 1, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        sql = f"DROP table  {self.tb_name2},{self.tb_name1},{self.tb_name3};"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        result = self.commsh_pub.execut_db_sql(sql, 
