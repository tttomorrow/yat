...
"""
Case Type   : 基础功能
Case Name   : 修改订阅，验证connection连接信息dbname参数
Description :
    1、创建用户，赋予所有权限
    2、在两个集群中创建相同字段表
    3、创建发布端(集群A)
    4、创建订阅   expect:创建订阅成功
    5.修改订阅connection连接信息dbname参数为非发布库   expect:成功
    6.修改订阅connection连接信息dbname参数为不符合规范库   expect:失败
    7.修改订阅connection连接信息dbname参数为发布库   expect:成功
Expect      :
    1.成功
    2.成功
    3.成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = "public.t_case100_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step5: 修改订阅connection连接信息dbname参数为非发布库   expect:成功--"
        self.log.info(text)
        sql = f"select pg_sleep(10);" \
            ...
            f"user={self.user_name} " \
            f"dbname=postgres " \
            f"password={macro.COMMON_PASSWD}' ;select * from pg_SUBSCRIPTION;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        ...
        text = "--step6: 修改订阅connection连接信息dbname参数为不符合规范库   expect:失败--"
        self.log.info(text)
        sql = f"select pg_sleep(10);" \
            f"alter SUBSCRIPTION {self.subname1} CONNECTION " \
            f"'host={self.pri_userdb_pub.db_host} " \
        ...
        text = "--step7: 修改订阅connection连接信息dbname参数为发布库   expect:成功--"
        self.log.info(text)
