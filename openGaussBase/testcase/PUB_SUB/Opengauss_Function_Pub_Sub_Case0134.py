...
"""
Case Type   : 基础功能
Case Name   : 发布订阅--pg_replication_origin_oid
Description :
    1.创建发布订阅
    2.查询复制源名称
    3.查询复制源oid
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname = "sub_case134"
        self.pubname = "pub_case134"
        ...
        self.log.info(result)
        self.tb_name = "t_pub_sub_case134"
        self.repli_name = "rep_pub_sub_case134"
    ...
    def test_pubsub(self):
        ...
        roident = result.splitlines()[2].split('|')[0].strip()
        self.assertIn('1 row', result, '执行失败:' + text)

        text = '--step3:查询复制源oid expect:成功，返回oid正确--'
        self.log.info(text)
        sql = f"select pg_replication_origin_oid('{roname}');"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('1 row', result, '执行失败:' + text)
        self.assertEqual(roident, result.splitlines()[-2].strip(),
                         '执行失败:' + text)

        text = '--step4:查询不存在复制源 expect:返回结果为空--'
        self.log.info(text)
        sql = f"select pg_replication_origin_oid('not_exist_pubsub');"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('1 row', result, '执行失败:' + text)
        self.assertEqual('', result.splitlines()[-2].strip(), '执行失败:' + text)
