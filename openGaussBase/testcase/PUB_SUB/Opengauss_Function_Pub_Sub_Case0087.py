...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case087_1'
        self.subname1 = "sub_case087_1"
        self.pubname1 = "pub_case087_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(i int ,i2 OID, i3 CID,i4 XID,i5 TID primary key,' \
            f'i6 REGCONFIG,i7 REGDICTIONARY,i8 REGOPER,i9 REGOPERATOR,' \
            f'i10 REGPROC,i11 REGPROCEDURE,i12 REGCLASS,i13 REGTYPE)'
        ...
        sql = f"insert into {self.tb_name1} values" \
            f"(1,564182,null,null,'(0,3)','english','simple',null," \
            f"'*(integer,integer)',null,'sum(int4)','pg_type','integer');" \
        ...
        self.log.info(text)
        sql = f"UPDATE {self.tb_name1}  SET i3='10';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('1 | 564182 | 10 |   ', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)
