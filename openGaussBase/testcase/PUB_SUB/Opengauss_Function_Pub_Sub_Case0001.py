...
"""
Case Type   : 基础功能
Case Name   : 创建发布且发布内容为空
Description :
    1.两个集群均创建表
    2.创建发布订阅，无选择值
    3.修改表tb_pubsub_case001_1
    4.查询订阅端数据
    5.修改发布内容
    6.修改表数据
    7.查询订阅端数据
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case001_1'
        self.tb_name2 = 'tb_pubsub_case001_2'
        self.tb_name3 = 'tb_pubsub_case001_3'
        self.subname = "sub_case001"
        self.pubname = "pub_case001"
    ...
    def test_pubsub(self):
        ...
        text = '--step2:创建发布订阅，无选择值 expect:成功--'
        self.log.info(text)        
        ...
        text = '--step3:修改表tb_pubsub_case001_1 expect:成功--'
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(1, 'no table');" \
        ...
        text = '--step4:查询订阅端数据 expect:集群B未同步A数据--'
        self.log.info(text)
        ...
        text = '--step5:修改发布内容 expect:成功--'
        self.log.info(text)
        sql = f"ALTER PUBLICATION {self.pubname} " \
            f"add table only {self.tb_name1}, {self.tb_name2};"
        result = self.commsh_pub.execut_db_sql(sql, 
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        self.log.info(text)
        sql = f"update {self.tb_name1} set t='after alter' where i<5;" \
            f"insert into {self.tb_name2} values(1, 'no table');" \
            f"insert into {self.tb_name3} values(1, 1, 'no table');"
        result = self.commsh_pub.execut_db_sql(sql, 
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        ...
        text = '--step7:查询订阅端数据 expect:tb_pubsub_case001_2同步，其余未同步--'
        self.log.info(text)
        sql = f"select * from {self.tb_name1};" \
        ...
        self.assertIn('1 | no table', result.splitlines()[6])
    ...
    def tearDown(self):
        ...
        text = '--清理环境，删除表--'
        self.log.info(text)
        sql = f"DROP table  {self.tb_name3},{self.tb_name2},{self.tb_name1};"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        result = self.commsh_pub.execut_db_sql(sql, 
