...
"""
Case Type   : 基础功能
Case Name   : 同一数据库创建多个发布内容相同且publish不同的订阅发布
Description :
    1.在两个集群创建表
    2.创建发布端发布内容相同且publish不同
    3.创建订阅
    4.修改表数据
    5.查询是否同步
    6.修改表数据
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case033_1'
        self.tb_name2 = 'tb_pubsub_case033_2'
        self.schema_name1 = 's_pubsub_case033_1'
        self.schema_name2 = 's_pubsub_case033_2'
        self.tbs_name = 'tbs_pubsub_case033'
        self.subname1 = "sub_case033_1"
        self.subname2 = "sub_case033_2"
        self.pubname1 = "pub_case033_1"
        self.pubname2 = "pub_case033_2"
    ...
    def test_pubsub(self):
        ...
        sql = f"create schema {self.schema_name1};" \
            ...
            f"values (1, 's_pubsub_case033_2', '%一', '[]');" \
        ...
        self.assertIn('1 | s_pubsub_case033_2 | update123 | []',
                      result, '执行失败' + text)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertEqual(result.count('0 rows'), 4, '执行失败' + text)
        ...
        self.assertIn('2 |  2 | second', result, '执行失败' + text)
        self.assertIn('1 | s_pubsub_case033_2 | %一     | []',
                      result, '执行失败' + text)
        self.assertEqual(result.count('1 | first'),
                         3, '执行失败' + text)
        self.assertEqual(result.count('2 rows'), 2, '执行失败' + text)
        self.assertEqual(result.count('1 row'), 2, '执行失败' + text)
