...
"""
Case Type   : 基础功能
Case Name   : 创建发布，仅发布delete,update操作
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case147_1'
        self.subname1 = "sub_case147_1"
        self.subname2 = "sub_case147_2"
        self.pubname1 = "pub_case147_1"
        self.pubname2 = "pub_case147_2"
    ...
    def test_pubsub(self):
        ...
        sql = f"CREATE PUBLICATION {self.pubname1} " \
            f"FOR  all TABLEs with (publish='delete,insert');" \
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(1, 'first-1');" \
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertEqual(result.count('1 | first-1'), 2, '执行失败' + text)
        self.assertIn('2 rows', result, '执行失败' + text)

        text = '--step6:修改表数据expect:成功--'
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(1, 'first-2');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        ...
        self.log.info(result)
        self.assertEqual(result.count('1 | first-1'), 2, '执行失败' + text)
        self.assertEqual(result.count('1 | first-2'), 2, '执行失败' + text)
        self.assertIn('4 rows', result, '执行失败' + text)

        text = '--step8:修改表属性 expect:成功--'
        self.log.info(text)
        ...
        self.log.info(text)
        sql = f"delete from  {self.tb_name1};select pg_sleep(5);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.DELETE_SUCCESS_MSG, result, '执行失败' + text)

        text = "--step10:查询是否同步 expect:数据同步--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
                                               None,
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(1, 'first-2');" \
            f"update {self.tb_name1} set i=2 where t='first-2';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        ...
        text = "--step11:查询是否同步 expect:update未同步--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
                                               None,
