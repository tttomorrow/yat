...
"""
Case Type   : 基础功能
Case Name   : 发布内容为二级分区（range-list）
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case138_1'
        self.subname1 = "sub_case138_1"
        self.pubname1 = "pub_case138_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f"CREATE TABLE {self.tb_name1}(" \
            ...
            f"    dept_code  VARCHAR2 ( 30 ) NOT NULL primary key," \
            f"    user_no    VARCHAR2 ( 30 ) NOT NULL ," \
            f"    sales_amt  int)PARTITION BY RANGE " \
            f"(month_code) SUBPARTITION BY LIST (dept_code)(" \
            f"  PARTITION p_201901 VALUES LESS THAN( '201903' )  (" \
            f"    SUBPARTITION p_201901_a values ('1')," \
            f"    SUBPARTITION p_201901_b values ('10')  )," \
            f"  PARTITION p_201902 VALUES LESS THAN( '201909' )" \
            f"  (    SUBPARTITION p_201902_a values ('1')," \
            f"    SUBPARTITION p_201902_b values ('10')  ));"
        ...
        sql = f"insert into {self.tb_name1} values('201902', '1', '1', 1);" \
            f"insert into {self.tb_name1} values('201903', '10', '1', 1);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
