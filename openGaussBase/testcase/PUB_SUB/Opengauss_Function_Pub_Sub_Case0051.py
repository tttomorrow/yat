...
"""
Case Type   : 基础功能
Case Name   : 级联订阅，中间发布端被订阅内容与其作为订阅端订阅内容不相同
Description :
    1.三个集群创建表
    2.创建发布端订阅端
    3.修改数据
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = 'tb_pubsub_case051_1'
        self.tb_name2 = 'tb_pubsub_case051_2'
        self.sub_name = "sub_case051_1"
        self.pub_name = "pub_case051_1"
    ...
    def test_pubsub(self):
        ...
        self.log.info(text)
        sql = f"CREATE PUBLICATION {self.pub_name} for table {self.tb_name1};"
        ...
        sql = f"CREATE PUBLICATION {self.pub_name} " \
        ...
        self.assertEqual(result.count(self.constant.INSERT_SUCCESS_MSG),
                         2, '执行失败' + text)
        sql = f"insert into {self.tb_name1} values(1, " \
            f"'2', '{self.tb_name1}2', 'equal');" \
            f"insert into {self.tb_name2} values(1, " \
            f"'2', '{self.tb_name2}2', 'equal');" \
            f"select pg_sleep(10);"
        result = self.commsh_node2.execut_db_sql(sql,
                                                 self.user_param_node2, None,
                                                 macro.DB_ENV_PATH_REMOTE1)
        ...
        text = "--step4:查看数据是否更新 expect:集群B：tb1更新 集群C：tb2更新"
        self.log.info(text)
        ...
        self.assertNotIn(f'1 | 1            | {self.tb_name2} | equal',
                      result, '执行失败:' + text)
        self.assertIn(f'1 | 1            | {self.tb_name1}  | equal',
                      result, '执行失败:' + text)
        self.assertIn(f'1 | 2            | {self.tb_name1}2 | equal',
                      result, '执行失败:' + text)
        self.assertIn(f'1 | 2            | {self.tb_name2}2 | equal',
                      result, '执行失败:' + text)
        ...
        self.assertNotIn(f'1 | 1            | {self.tb_name2} | equal',
                         result, '执行失败:' + text)
        self.assertNotIn(f'1 | 1            | {self.tb_name1}  | equal',
                      result, '执行失败:' + text)
        self.assertNotIn(f'1 | 2            | {self.tb_name1}2 | equal',
                      result, '执行失败:' + text)
