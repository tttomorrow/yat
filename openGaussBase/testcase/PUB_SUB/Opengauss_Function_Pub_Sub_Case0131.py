...
"""
Case Type   : 基础功能
Case Name   : 发布订阅--pg_replication_origin_status
Description :
    1.创建发布订阅
    2.查看 pg_replication_origin_status
    3.修改订阅端信息
    4.查询系统表内容
Expect      :
    1.成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname = "sub_case131"
        self.pubname = "pub_case131"
        ...
        self.log.info(result)
        self.tb_name = "tb_pub_sub_case131"
    ...
    def test_pubsub(self):
        ...
        sql1 = f"create table {self.tb_name}(i int primary key);"
        ...
        text = '--step2:查看 pg_replication_origin_status  expect:类型与资料一致--'
        self.log.info(text)
        sql = "\d+ pg_replication_origin_status"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('local_id    | oid  |           | plain',
                      result, '执行失败:' + text)
        self.assertIn('external_id | text |           | extended',
                      result, '执行失败:' + text)
        self.assertIn('remote_lsn  | text |           | extended',
                      result, '执行失败:' + text)
        self.assertIn('local_lsn   | text |           | extended',
                      result, '执行失败:' + text)
        ...
        self.log.info(text)
        sql = "select * from pg_replication_origin_status;" \
              "select pg_current_xlog_location();"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(result.splitlines()[-2].strip(),
                      result.splitlines()[2].strip(), '执行失败:' + text)
