...
"""
Case Type   : 基础功能
Case Name   : 发布订阅--pg_replication_origin
Description :
    1.创建发布订阅
    2.查看 pg_replication_origin
Expect      :
    1.成功
    2.类型与资料一致且内容正确,表中存在2条记录，且roname为pg_oid
History     :
"""
import unittest
import os
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname = "sub_case129"
        self.subname2 = "sub_case129_2"
        self.pubname = "pub_case129"
    ...
    def test_pubsub(self):
        ...
        text = '--step2:查看 pg_replication_origin ' \
               'expect:类型与资料一致且内容正确,表中存在2条记录，且roname为pg_oid--'
        self.log.info(text)
        sql = "\d+ pg_replication_origin"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('roident | oid  | not null  | plain',
                      result, '执行失败:' + text)
        self.assertIn('roname  | text |           | extended',
                      result, '执行失败:' + text)

        sql = "select oid,* from pg_subscription ;" \
              "select * from pg_replication_origin;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(f"pg_{result.splitlines()[2].split()[0].strip()}",
                      result, '执行失败:' + text)
        self.assertIn(f"pg_{result.splitlines()[3].split()[0].strip()}",
                      result, '执行失败:' + text)
