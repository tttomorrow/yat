...
Expect      :
    ...
    4.成功
    5.全部同步
    6.失败，提示it does not have a replica identity and publishes updates
    7.成功
    8.tb_pubsub_case015_2成功tb_pubsub_case015_1提示
    it does not have a replica identity and publishes updates
    9.tb_pubsub_case015_2更新，tb_pubsub_case015_1未更新
History     :
"""
import unittest
import os
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case015_1'
        self.tb_name2 = 'tb_pubsub_case015_2'
        self.subname = "sub_case015"
        self.pubname = "pub_case015"
    ...
    def test_pubsub(self):
        ...
        self.log.info(text)
        sql = f"create table {self.tb_name1}(i int , t text);" \
            f"create table {self.tb_name2}(i int , t text);"
        ...
        self.assertEqual(result.count('1 | first'), 2, '执行失败' + text)

        text = '--step6:更新表数据 expect:失败，提示' \
               'it does not have a replica identity and publishes--'
        self.log.info(text)
        sql = f"update {self.tb_name1} set t='Update%' where i=1; " \
            f"delete from {self.tb_name2} where i<10;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        flg = 'it does not have a replica identity and publishes'
        self.assertEqual(result.count(flg), 2, '执行失败' + text)

        text = '--step7:两个集群修改表2为REPLICA IDENTITY expect:成功--'
        self.log.info(text)
        sql = f"alter table {self.tb_name2} REPLICA IDENTITY full;"
        result = self.commsh_pub.execut_db_sql(
            sql, sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = '--step8:更新表数据 expect:tb_pubsub_case015_2成功' \
               'tb_pubsub_case015_1提示it does not have a replica ' \
               'identity and publishes updates--'
        self.log.info(text)
        sql = f"delete from {self.tb_name2} where i<10;" \
            f"update {self.tb_name1} set t='Update---$' where i=1;" \
            f"insert into {self.tb_name2} values(2, 'first');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(flg), 1, '执行失败' + text)
        ...
        text = '--step9:查询是否同步 expect:tb_pubsub_case015_2更新，' \
               'tb_pubsub_case015_1未更新--'
        self.log.info(text)
        sql = f"select * from {self.tb_name1};" \
            f"select * from {self.tb_name2};"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
        ...
        self.assertEqual(result.count('2 | first'), 1, '执行失败' + text)
