...
Expect      :
    ...
    4.失败
    5.数据均更新
    6.创建成功，调用提示CREATE PUBLICATION is not allowed in a non-volatile function
History     :
"""
import unittest
import os
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case043_1'
        self.tb_name2 = 'tb_pubsub_case043_2'
        self.tb_name3 = 'tb_pubsub_case043_3'
        self.subname1 = "sub_case043_1"
        self.pubname1 = "pub_case043_1"
        self.pubname2 = "pub_case043_2"
    ...
    def test_pubsub(self):
        ...
        text = '--step6:修改发布端 ' \
               'expect:失败，DETAIL:  Tables cannot be added to or ' \
               'dropped from FOR ALL TABLES publications.--'
        self.log.info(text)

        text = "--step6:创建发布端  " \
               "expect:创建成功，调用提示CREATE PUBLICATION is not allowed " \
               "in a non-volatile function--"
        self.log.info(text)
        sql = f"CREATE FUNCTION func_add_sql(integer, integer)" \
            f" RETURNS integer" \
            f"    AS 'CREATE PUBLICATION {self.pubname2} for ALL TABLES;" \
            f"    select \$1 + \$2;" \
            f"    '" \
            f"    LANGUAGE SQL" \
            f"    IMMUTABLE" \
            f"    RETURNS NULL ON NULL INPUT;" \
            f"select func_add_sql(1,1);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        flg = " CREATE PUBLICATION is not allowed in a non-volatile function"
