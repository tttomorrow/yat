...
"""
Case Type   : 基础功能
Case Name   : 发布内容为二级分区（list-range）
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
    4.修改表数据
    5.增加REPLICA IDENTITY full属性
    6.修改数据，并检查是否同步
Expect      :
    1.成功
    2.成功
    3.成功
    4.insert成功且同步，update失败
    5.成功
    6.成功
History     :
"""
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = 'tb_pubsub_case144_1'
        self.subname1 = "sub_case144_1"
        self.pubname1 = "pub_case144_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:两个集群均创建表_对象标识符型 expect:成功--'
        self.log.info(text)
        create_sql = f"CREATE TABLE {self.tb_name1}(   " \
            f" month_code VARCHAR2 ( 30 ) NOT NULL ,    " \
            f"dept_code  VARCHAR2 ( 30 ) NOT NULL ," \
            f"    user_no    VARCHAR2 ( 30 ) NOT NULL ," \
            f"    sales_amt  int)" \
            f"PARTITION BY LIST (month_code) SUBPARTITION " \
            f"BY RANGE (dept_code)" \
            f"(  PARTITION p_201901 VALUES ( '201902' )" \
            f"  (    SUBPARTITION p_201901_a values less than ('4')," \
            f"    SUBPARTITION p_201901_b values less than ('6')  )," \
            f"  PARTITION p_201902 VALUES ( '201903' )  (" \
            f"   SUBPARTITION p_201902_a values less than ('3')," \
            f"    SUBPARTITION p_201902_b values less than ('6')  ));"
        ...
        text = "--step4:修改表数据，并检查是否同步: expect:insert成功且同步，update失败--"
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values('201902', '4', '1', 1);" \
            f"insert into {self.tb_name1} values('201902', '3', '1', 1);" \
            f"update {self.tb_name1} set month_code='201903';" \
        ...
        self.assertIn(self.constant.INSERT_SUCCESS_MSG,
                      result, '执行失败:' + text)
        self.assertIn('it does not have a replica identity and publishes '
                      'updates', result, '执行失败:' + text)
        ...
        self.assertNotIn('201903', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)

        text = "--step5:增加REPLICA IDENTITY full属性 expect:成功--"
        self.log.info(text)
        sql = f"alter table {self.tb_name1} REPLICA IDENTITY full;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = "--step6:修改数据，并检查是否同步 expect:成功--"
        self.log.info(text)
        sql = f"update {self.tb_name1} set month_code='201903';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('201903', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)
