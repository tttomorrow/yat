...
"""
Case Type   : 基础功能
Case Name   : 修改订阅，验证不同用户权限修改订阅
Description :
    1:创建用户，赋予所有权限
    2:在两个集群中创建相同字段表
    3:创建发布端(集群A)
    4.1:创建订阅（集群B）  expect:创建订阅成功
    4.2:连接创建订阅用户，修改订阅，重命名订阅名称   expect:修改订阅成功
    4.3:连接具有sysadmin权限的其他用户，修改订阅，重命名订阅名称   expect:修改订阅成功
    4.4:连接无权限用户，修改订阅，重命名订阅名称   expect:修改订阅失败
    4.5:向发布端表中插入数据   expect:成功（针对4.2步骤操作）
    4.6:查询集群B中表中数据是否同步   expect:数据已同步
Expect      :
    1：成功
    2：成功
    3：成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case115_1"
        self.pubname1 = "pub_case115_1"
        self.tb_name1 = "public.t_case115_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step4.1:创建订阅（集群B）,指定参数enabled为false  expect:创建订阅成功--"
        self.log.info(text)
        ...
        text = "--step4.2:连接创建订阅用户，修改订阅，重命名订阅名称   expect:修改订阅成功--"
        self.log.info(text)
        sql = f"alter SUBSCRIPTION {self.subname1} " \
            f"rename to {self.subname1}_new;select pg_sleep(5);"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        ...
        text = "--step4.3:连接具有sysadmin权限的其他用户，修改订阅，重命名订阅名称   " \
               "expect:修改订阅成功--"
        self.log.info(text)
        sql = f"alter SUBSCRIPTION {self.subname1}_new " \
            f"rename to {self.subname1};select pg_sleep(5);"
        ...
        text = "--step4.4:连接无权限用户，修改订阅，重命名订阅名称   expect:修改订阅失败--"
        self.log.info(text)
        sql = f"create user {self.user_name2} password " \
            f"'{macro.COMMON_PASSWD}';"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.CREATE_ROLE_SUCCESS_MSG,
                      result, '执行失败:' + text)
        sql = f"alter SUBSCRIPTION {self.subname1} " \
            f"rename to {self.subname1}_new;select pg_sleep(20);"
        user_param = f"-U {self.user_name2} -W {macro.COMMON_PASSWD}"
        result = self.commsh_sub.execut_db_sql(sql, user_param, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.SQL_WRONG_MSG[1], result,
                         '执行失败:' + text)
        self.assertIn('must be owner of subscription', result, '执行失败:' + text)

        text = "--step4.5:向发布端表中插入数据   expect:成功--"
        self.log.info(text)
        ...
        text = "--step4.6:查询集群B中表中数据是否同步   expect:数据同步--"
        self.log.info(text)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('1 | a_', result, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        sql = f"DROP SUBSCRIPTION  {self.subname1};" \
            f"DROP SUBSCRIPTION  {self.subname1}_new;"
