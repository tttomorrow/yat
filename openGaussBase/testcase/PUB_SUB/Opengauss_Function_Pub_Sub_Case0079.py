...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case079_1'
        self.subname1 = "sub_case079_1"
        self.pubname1 = "pub_case079_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(i2 cidr primary key,i3 inet,i4 macaddr)'
        ...
        sql = f"insert into {self.tb_name1} values" \
            f"('192.168.100.128/25','10.1.2.3/32','08:00:2b:01:02:03');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = "--step5:修改1列数据 集群A:发布端 expect:成功 （i2='192.168/24'，其他数据均不变）--"
        self.log.info(text)
        sql = f"select pg_sleep(5.5);" \
            f"UPDATE {self.tb_name1} SET i2='192.168/24' " \
            f"WHERE i2='192.168.100.128/25';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('192.168.0.0/24 | 10.1.2.3 | 08:00:2b:01:02:03',
                      result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
