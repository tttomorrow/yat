...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case077_1'
        self.subname1 = "sub_case077_1"
        self.pubname1 = "pub_case077_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f'CREATE TABLE {self.tb_name1}(i int primary key,' \
            f'i2 CHAR(4),i3 VARCHAR2(5),i4 NVARCHAR2(6),i5 TEXT,i6 name,' \
            f'i7 "char",i8 CHARACTER(4),i9 NCHAR(4)' \
            f',i10 CHARACTER VARYING(5),i11 CLOB,i12 VARCHAR(4) );'
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(1,'ok'," \
            f"'good','longw','文本类型数据','jjjjjjjjj','p'," \
            f"'my','lw','well','数据','yo');select pg_sleep(5.5);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = "--step5:修改1列数据 集群A:发布端 expect:成功  （i3='too'，其他数据均不变）--"
        self.log.info(text)
        sql = f"UPDATE {self.tb_name1} SET i3='too' WHERE i3='good';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('1 | ok   | too | ', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)
