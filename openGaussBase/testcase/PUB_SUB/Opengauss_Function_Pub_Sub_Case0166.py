...
"""
Case Type   : 基础功能
Case Name   : 系统表中普通用户不能查看到别的用户的用户名密码(即使是加密后的密码)
Description :
    1.两个集群创建用户
    2.创建发布(发布端执行)
    3.创建订阅，订阅信息中发布用户为超级用户(订阅端执行，端口只能是DN端口+1)
    4.订阅端创建普通用户
    5.普通用户查询订阅表
Expect      :
    1.成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case166_1"
        self.pubname1 = "pub_case166_1"
        ...
        self.user_name1 = "u_case166_1"
        self.user_name2 = "u_case166_2"
    ...
    def test_pubsub(self):
        ...
        text = "--step3:创建订阅，订阅信息中发布用户为超级用户(订阅端执行，端口只能是DN端口+1) " \
               "expect:成功--"
        self.log.info(text)
        ...
        text = "--step4:订阅端创建普通用户 expect:成功--"
        self.log.info(text)
        ...
        text = "--step5:普通用户查询订阅表 expect:成功--"
        self.log.info(text)
