...
"""
Case Type   : 基础功能
Case Name   : 创建新订阅，连接发布者端ip验证
Description :
    1、创建发布端(集群A)
    2、创建订阅（集群B）
    2.1:订阅名正常，发布端连接ip正常   expect:成功
    2.2:订阅名正常，发布端连接集群备机ip  expect:失败
    2.3:订阅名正常，发布端连接不符合规范ip  expect:失败
    2.4:订阅名正常，发布端连接ip使用hostname
Expect      :
    1：成功
    2.1：成功
    2.2：失败
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case094_1"
        self.subname2 = "sub_case094_2"
        self.pubname1 = "pub_case094_1"
        ...
        self.log.info(result)
        self.pub_hostname = self.pri_userdb_pub.sh('hostname').result()
    ...
    def test_pubsub(self):
        ...
        guc_res = self.commsh_sub.execute_gsguc(
            'reload', self.constant.GSGUC_SUCCESS_MSG, '',
            'all', False, False, macro.DB_INSTANCE_PATH_REMOTE1,
            f'host    replication  {self.sta_userdb_pub.db_user} '
        ...
        text = "--step2.1:订阅名正常，发布端连接ip正常   expect:成功--"
        self.log.info(text)
        ...
        text = "--step2.2:订阅名正常，发布端连接集群备机ip  expect:失败--"
        self.log.info(text)
        result = self.commsh_sub.execute_generate(
            macro.COMMON_PASSWD, env_path=macro.DB_ENV_PATH_REMOTE1)
        self.assertIn('', result, '执行失败:' + text)
        sql = f"CREATE SUBSCRIPTION {self.subname2} CONNECTION " \
            f"'host={self.sta_userdb_pub.db_host} " \
            f"port={self.port} " \
            f"user={self.pri_userdb_pub.db_user} " \
            f"dbname={self.pri_userdb_pub.db_name} " \
            f"password={self.pri_userdb_pub.ssh_password}' " \
        ...
        self.assertIn(self.constant.SQL_WRONG_MSG[1], result,
                         '执行失败:' + text)
        flg = "SSL connection has been closed unexpectedly" in result or \
              "FATAL:  can't decode in pmState is not run or recovery " \
              "in progress" in result or \
              self.constant.can_not_connect_pub_msg in result
        self.assertTrue(flg, '执行失败:' + text)

        text = "--2.3:订阅名正常，发布端连接不符合规范ip  expect:失败--"
        self.log.info(text)
        result = self.commsh_sub.execute_generate(
            macro.COMMON_PASSWD, env_path=macro.DB_ENV_PATH_REMOTE1)
        self.assertIn('', result, '执行失败:' + text)
        ...
        text = "--2.4:订阅名正常，发布端连接ip使用hostname  expect:成功--"
        self.log.info(text)
        result = self.commsh_sub.execute_generate(
            macro.COMMON_PASSWD, env_path=macro.DB_ENV_PATH_REMOTE1)
        self.assertIn('', result, '执行失败:' + text)
        sql = f"CREATE SUBSCRIPTION {self.subname2} CONNECTION " \
            f"'host={self.pub_hostname} " \
            f"port={self.port} " \
            f"user={self.pri_userdb_pub.db_user} " \
            f"dbname={self.pri_userdb_pub.db_name} " \
            f"password={self.pri_userdb_pub.ssh_password}' " \
