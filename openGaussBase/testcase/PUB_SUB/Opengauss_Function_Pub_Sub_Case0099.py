...
"""
Case Type   : 基础功能
Case Name   : 创建新订阅，发布端发布名称publication_name验证
Description :
    1、创建用户，赋予所有权限
    2、创建发布端(集群A)
    3、创建订阅（集群B）
    3.1:其他配置信息正常，发布名称存在   expect:成功
    3.2:其他配置信息正常，发布名称不存在  expect:失败
    3.3:其他配置信息正常，修改发布名称，订阅端更新发布名称   expect:成功
Expect      :
    1：成功
    2：成功
    3.1：成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case099_1"
        self.pubname1 = "pub_case099_1"
        self.tb_name1 = "t_case099_1"
    ...
    def test_pubsub(self):
        ...
        self.log.info(text)
        sql = f"create user {self.user_name} " \
        ...
        text = "--step3.1:其他配置信息正常，发布名称存在   expect:成功--"
        self.log.info(text)
        ...
        text = "--step3.2:其他配置信息正常，发布名称不存在  expect:成功--"
        self.log.info(text)
        result = self.commsh_sub.execute_generate(
            macro.COMMON_PASSWD, env_path=macro.DB_ENV_PATH_REMOTE1)
        self.assertIn('', result, '执行失败:' + text)
        ...
        text = "--step3.3:其他配置信息正常，修改发布名称，订阅端更新发布名称 expect:失败--"
        self.log.info(text)
        sql = f"alter publication {self.pubname1} rename " \
            f"to new_{self.pubname1};"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_u)
        self.log.info(result)
        ...
        sql = f"alter subscription {self.subname1} " \
            f"set publication new_{self.pubname1};" \
        ...
        self.assertIn(self.constant.alter_sub_succ_msg, result,
                      '执行失败:' + text)
        sql_insert = f"insert into {self.tb_name1} values(9999, 'd');"
        result = self.commsh_pub.execut_db_sql(sql_insert,
                                               sql_type=self.user_param_u)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        sql = f"DROP PUBLICATION  if exists {self.pubname1};" \
            f"DROP PUBLICATION  if exists new_{self.pubname1};"
