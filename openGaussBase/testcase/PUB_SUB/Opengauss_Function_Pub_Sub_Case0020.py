...
"""
Case Type   : 基础功能
Case Name   : 发布指定表存在关联表，创建发布订阅后，修改订阅端表结构
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case020_1'
        self.tb_name2 = 'tb_pubsub_case020_2'
        self.subname = "sub_case020"
        self.pubname = "pub_case020"
    ...
    def test_pubsub(self):
        ...
        sql = f"create table {self.tb_name1}(i int primary key, t text);" \
            f"create table {self.tb_name2}(i int primary key, id int, " \
        ...
        sql = f"insert into {self.tb_name2} values(2, 1, 'second--');" \
        ...
        self.assertIn('1 |  1 | first', result, '执行失败' + text)
        self.assertIn('2 |  1 | second--', result, '执行失败' + text)
        self.assertIn('1 row', result, '执行失败' + text)
        self.assertIn('2 rows', result, '执行失败' + text)

        text = '--step8:修改表结构,并插入数据 expect:成功--'
        self.log.info(text)
        sql = f"alter table {self.tb_name2} add  CONSTRAINT " \
        ...
        sql = f"insert into {self.tb_name1} values(3, 'three');" \
        ...
        text = "--step9:查询是否同步 expect:新数据被同步--"
        self.log.info(text)
        time.sleep(10)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
        ...
        self.assertIn('1 |  1 | first', result, '执行失败' + text)
        self.assertIn('2 |  1 | second--', result, '执行失败' + text)
        self.assertIn('3 | three', result, '执行失败' + text)
