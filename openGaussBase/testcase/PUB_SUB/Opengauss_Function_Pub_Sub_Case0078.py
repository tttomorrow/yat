...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = 'tb_pubsub_case078_1'
        self.subname1 = "sub_case078_1"
        self.pubname1 = "pub_case078_1"
    ...
    def test_pubsub(self):
        ...
        self.log.info(text)
        create_sql = f"CREATE TABLE {self.tb_name1}(i int, i2 point ," \
            f"i3 lseg,i4 box,i5 path,i6 path,i7 polygon,i8 circle);" \
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(1,point'2,3'," \
            f"lseg'[(1,2),(2,2)]',box'(2,4),(4,4)'," \
            f"path'((1,2),(1,3),(3,2),(3,3))',path'[(1,1),(1,2),(1,3)]'," \
            f"polygon'(1,2),(1,3),(3,2),(3,3)',circle'4,4,2');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        sql = f"select pg_sleep(5.5);" \
            f"UPDATE {self.tb_name1} SET i8=circle'5,5,1' " \
            f"WHERE i8=circle'4,4,2';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        flg = f'FATAL:  could not identify an equality ' \
            f'operator for type point'
