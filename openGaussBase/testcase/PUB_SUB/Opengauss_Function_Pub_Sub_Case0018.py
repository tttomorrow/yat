...
"""
Case Type   : 基础功能
Case Name   : 发布指定表存在关联表，创建发布订阅后，修改发布端表结构
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case018_1'
        self.tb_name2 = 'tb_pubsub_case018_2'
        self.subname = "sub_case018"
        self.pubname = "pub_case018"
    ...
    def test_pubsub(self):
        ...
        text = '--step6:修改表结构,并插入数据 expect:成功--'
        self.log.info(text)
        sql = f"alter table {self.tb_name2} DROP CONSTRAINT fk_id;" \
            f"insert into {self.tb_name2} values(2, 2, 'second--');" \
            f"update {self.tb_name1} set t='update1' where i=1;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(self.constant.INSERT_SUCCESS_MSG),
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('1 | update1', result, '执行失败' + text)
        self.assertIn('1 row', result, '执行失败' + text)
        self.assertIn('2 rows', result, '执行失败' + text)
        self.assertIn('1 |  1 | first', result, '执行失败' + text)
        self.assertIn('2 |  2 | second--', result, '执行失败' + text)

        text = '--step8:修改表结构,并插入数据 expect:成功--'
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(2, 'second');" \
            f"alter table {self.tb_name2} add  CONSTRAINT " \
            f"fk_id foreign key(id) references {self.tb_name1}(i);" \
            f"insert into {self.tb_name1} values(3, 'three');" \
            f"insert into {self.tb_name2} values(3, 3, 'three1--');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(self.constant.INSERT_SUCCESS_MSG),
        ...
        text = "--step9:查询是否同步 expect:所有表更新数据均被同步--"
        self.log.info(text)
        time.sleep(10)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
        ...
        self.log.info(result)
        self.assertIn('1 | update1', result, '执行失败' + text)
        self.assertEqual(result.count('2 | second'), 2, '执行失败' + text)
        self.assertEqual(result.count('3 | three'), 2, '执行失败' + text)
        self.assertIn('1 |  1 | first', result, '执行失败' + text)
        self.assertIn(' 2 |  2 | second--', result, '执行失败' + text)
        self.assertIn('3 |  3 | three1--', result, '执行失败' + text)
