...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info("-Opengauss_Function_Pub_Sub_Case0031 start-")
        self.pri_userdb_pub = Node(node='PrimaryDbUser')
        self.constant = Constant()
        self.commsh_pub = CommonSH('PrimaryDbUser')
        self.com_pub = Common()
        self.pubname = "pub_case031"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:创建发布端 expect:失败--'
        self.log.info(text)        
        sql = f"CREATE PUBLICATION {self.pubname} " \
            f"FOR  all TABLEs with (publish='alte');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn('unrecognized "publish"', result, '执行失败:' + text)
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.drop_pub_succ_msg, drop_pub_result,
                      '执行失败' + text)
        self.log.info("-Opengauss_Function_Pub_Sub_Case0031 end-")
