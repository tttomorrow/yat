...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case076_1'
        self.subname1 = "sub_case076_1"
        self.pubname1 = "pub_case076_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f"CREATE TABLE {self.tb_name1}" \
            f"(i int , i2 money primary key,i3 BOOLEAN);"
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(1,567,TRUE);" \
        ...
        text = "--step5:修改数据_集群A expect:成功  （i3=FALSE，其他数据均显示正确）--"
        self.log.info(text)
        sql = f"UPDATE {self.tb_name1} SET  i3=FALSE WHERE i3=TRUE;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(' 1 | $567.00 | f', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)
