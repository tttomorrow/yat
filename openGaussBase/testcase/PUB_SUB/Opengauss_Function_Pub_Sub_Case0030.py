...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case030_1'
        self.tb_name2 = 'tb_pubsub_case030_2'
        self.subname = "sub_case030"
        self.pubname = "pub_case030"
    ...
    def test_pubsub(self):
        ...
        self.log.info(text)
        sql = f"delete from {self.tb_name1} where t='second';" \
            f"delete from {self.tb_name2} where c='second';" \
            f"{sql_select}"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.DELETE_SUCCESS_MSG, result, '执行失败' + text)
        self.assertEqual(result.count('second'), 1, '执行失败' + text)
        ...
        text = "--step9:查询是否同步 expect:数据不同步--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
                                               None,
