...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case112_1"
        self.pubname1 = "pub_case112_1"
        self.tb_name1 = "public.t_case112_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step4.3:查询集群B中表中数据是否同步   expect:数据未同步--"
        self.log.info(text)
        ...
        text = "--step4.4:修改订阅，使订阅可用   expect:修改订阅成功--"
        self.log.info(text)
        sql = f"alter SUBSCRIPTION {self.subname1} enable;" \
            f"select pg_sleep(50);"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertNotIn(self.constant.SQL_WRONG_MSG[1], result,
        ...
        sql_insert = f"select pg_sleep(10);" \
            f"select * from pg_replication_slots ;" \
            f"insert into {self.tb_name1} values(2,'a_');"
        result = self.commsh_pub.execut_db_sql(sql_insert,
                                               sql_type=self.user_param_u)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)

        text = "--step4.6查询集群B中表中数据是否同步   expect:数据同步--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
                                               None,
        ...
        self.assertIn('2 | a_', result, '执行失败' + text)
