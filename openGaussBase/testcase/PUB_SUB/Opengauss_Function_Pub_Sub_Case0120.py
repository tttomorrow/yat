...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case120_1'
        self.tb_name2 = 'tb_pubsub_case120_2'
        self.tb_name3 = 'tb_pubsub_case120_3'
        self.subname = "sub_case120"
        self.pubname = "pub_case120"
    ...
    def test_pubsub(self):
        ...
        sql = f"CREATE TABLE {self.tb_name1}(id NUMBER(7) CONSTRAINT " \
            f"s_longtext_id_nn NOT NULL, " \
            f"use_filename VARCHAR2(20) primary key, " \
            f"filename VARCHAR2(255), text VARCHAR2(2000));" \
            f"CREATE TABLE {self.tb_name2}(like {self.tb_name1} " \
            f"including all);create table {self.tb_name3}(" \
            f"i int primary key, use_filename VARCHAR2(2000), " \
        ...
        sql = f"CREATE PUBLICATION {self.pubname} " \
            f"FOR  TABLE {self.tb_name1},{self.tb_name2},{self.tb_name3};"
        ...
        self.log.info(text)
        sql = f"ALTER PUBLICATION {self.pubname} set table {self.tb_name1};" \
        ...
        sql = f"insert into {self.tb_name1} values" \
            ...
            f"insert into {self.tb_name2} values" \
            f"(2, '666^^&', '中文', 'test');" \
            f"insert into {self.tb_name3} values(2, '666^^&');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(self.constant.INSERT_SUCCESS_MSG),
        ...
        text = "--step8:查询是否同步 expect:.tb_pubsub_case039_1更新，其余未更新--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
                                               None,
