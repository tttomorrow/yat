...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case140_1'
        self.subname1 = "sub_case140_1"
        self.pubname1 = "pub_case140_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f"CREATE TABLE {self.tb_name1}(    " \
            ...
            f"    user_no    VARCHAR2 ( 30 ) NOT NULL primary key," \
            f"    sales_amt  int)" \
            f"PARTITION BY hash (month_code) SUBPARTITION BY LIST (dept_code)" \
            f"(  PARTITION p_201901  " \
            f"(    SUBPARTITION p_201901_a VALUES ( '1' )," \
            f"    SUBPARTITION p_201901_b VALUES ( '2' )  )," \
            f"  PARTITION p_201902  (" \
            f"    SUBPARTITION p_201902_a VALUES ( '1' )," \
            f"    SUBPARTITION p_201902_b VALUES ( '2' )  ));"
        ...
        sql = f"insert into {self.tb_name1} values('201901', '1', '1', 1);" \
            f"insert into {self.tb_name1} values('201906', '2', '2', 2);" \
        ...
        self.log.info(text)
        sql = f"update {self.tb_name1}  set month_code='201902' ;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('201902', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)
