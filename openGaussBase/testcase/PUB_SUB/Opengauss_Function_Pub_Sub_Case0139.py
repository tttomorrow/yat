...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case139_1'
        self.subname1 = "sub_case139_1"
        self.pubname1 = "pub_case139_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f"CREATE TABLE {self.tb_name1}(" \
            ...
            f"    sales_amt  int)" \
            f"PARTITION BY hash (month_code) SUBPARTITION BY hash (dept_code)" \
            f"(  PARTITION p_201901  (" \
            f"    SUBPARTITION p_201901_a," \
            f"    SUBPARTITION p_201901_b  )," \
            f"  PARTITION p_201902  (" \
            f"    SUBPARTITION p_201902_a," \
            f"    SUBPARTITION p_201902_b  ));"
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values('201901', '1', '1', 1);" \
            f"insert into {self.tb_name1} values('201906', '2', '1', 1);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        self.log.info(text)
        sql = f"update {self.tb_name1} set dept_code='6' " \
            f"where month_code='201901';" \
            f"update {self.tb_name1} set dept_code='2' " \
            f"where month_code='201906';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('201901     | 6         | 1       |         1',
                      result, '执行失败:' + text)
        self.assertIn('201906     | 2         | 1       |         1',
                      result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
