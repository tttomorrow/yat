...
"""
Case Type   : 基础功能
Case Name   : 修改订阅，验证connection连接信息host参数
Description :
    1、创建用户，赋予所有权限
    2、在两个集群中创建相同字段表
    3、创建发布端(集群A)
    4、创建订阅   expect:创建订阅成功
    5、修改订阅connection连接信息host参数为不符合规范IP   expect:失败
    6、修改订阅connection连接信息host参数为发布端集群备机IP   expect:成功，但日志有错误提示
    7、向发布端表中插入数据   expect:成功
    8、查询集群B中表中数据是否同步   expect:数据未同步
    9、修改订阅connection连接信息host参数为发布端IP   expect:成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.subname1 = "sub_case105_1"
        self.pubname1 = "pub_case105_1"
        self.tb_name1 = "public.t_case105_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step5: 修改订阅connection连接信息host参数为不符合规范IP   expect:失败--"
        self.log.info(text)
        ...
        text = "--step6:修改订阅connection连接信息host参数为发布端集群备机IP expect:失败--"
        self.log.info(text)
        sql = f"alter SUBSCRIPTION {self.subname1} CONNECTION " \
        ...
        self.assertIn(self.constant.can_not_conn_newpub_msg, result,
                         '执行失败:' + text)
        flg = f'WARNING:  apply worker could not connect to the remote server'
        ...
        text = "--step7:向发布端表中插入数据   expect:成功--"
        self.log.info(text)
        ...
        text = "--step8:查询集群B中表中数据是否同步   expect:数据同步--"
        self.log.info(text)
        ...
        text = "--step9:修改订阅connection连接信息host参数为发布端IP   expect:成功--"
        self.log.info(text)
        sql = f"alter SUBSCRIPTION {self.subname1} CONNECTION " \
            f"'host={self.pri_userdb_pub.db_host} " \
            f"port={self.port} " \
        ...
        text = "--step10:查询集群B中表中数据是否同步   expect:数据已同步--"
        self.log.info(text)
        sql_select = f"select count(*) from {self.tb_name1}"
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
