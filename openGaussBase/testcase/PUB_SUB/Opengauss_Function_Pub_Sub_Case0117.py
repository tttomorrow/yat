...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case117_1"
        self.pubname1 = "pub_case117_1"
        self.tb_name1 = "public.t_case117_1"
        ...
        self.user_name = "u_case117_1"
        self.user_name1 = "u_case117_2"
    ...
    def test_pubsub(self):
        ...
        text = "--step7:连接创建订阅用户，删除订阅 expect:成功--"
        self.log.info(text)
        sql = f"drop SUBSCRIPTION  {self.subname1} cascade;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_sub, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertNotIn(self.constant.SQL_WRONG_MSG[1], result,
        ...
        text = "--step8:连接无权限用户，删除订阅 expect:失败--"
        self.log.info(text)
        sql = f"create user {self.user_name1} " \
            f"password '{macro.COMMON_PASSWD}';" \
        ...
        result = self.commsh_sub.execut_db_sql(
            sql, f"-U {self.user_name1} -W {macro.COMMON_PASSWD}",
            None, macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.SQL_WRONG_MSG[1], result,
                      '执行失败:' + text)
        self.assertIn('must be owner of subscription', result,
                      '执行失败:' + text)

        text = "--step9:连接具有sysadmin权限的其他用户，删除订阅  expect:失败--"
        self.log.info(text)
        result = self.commsh_sub.execut_db_sql(
    ...
    def tearDown(self):
        ...
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_u)
        self.log.info(result)
        sql = f"drop user if exists {self.user_name};" \
