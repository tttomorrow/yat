...
"""
Case Type   : 基础功能
Case Name   : 修改订阅，验证name参数
Description :
    1:创建用户，赋予所有权限
    2:在两个集群中创建相同字段表
    3:创建发布端(集群A)
    4:创建订阅   expect:创建订阅成功
    5:修改订阅名称，订阅名称不存在   expect:失败
    6:修改订阅名称，订阅名称存在   expect:成功
    7:向发布端表中插入数据   expect:成功
    8:查询集群B中表中数据是否同步   expect:数据已同步
Expect      :
    1：成功
    2：成功
    3：成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case104_1"
        self.pubname1 = "pub_case104_1"
        self.tb_name1 = "public.t_case104_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step5:修改订阅名称，订阅名称不存在   expect:失败--"
        self.log.info(text)
        sql = f"alter SUBSCRIPTION {self.subname1}_222 CONNECTION " \
            f"'host={self.pri_userdb_pub.db_host} " \
            f"port={self.port} " \
        ...
        text = "--step6:修改订阅名称，订阅名称存在   expect:成功--"
        self.log.info(text)
        ...
        text = "--step8:查询集群B中表中数据是否同步   expect:数据已同步--"
        self.log.info(text)
