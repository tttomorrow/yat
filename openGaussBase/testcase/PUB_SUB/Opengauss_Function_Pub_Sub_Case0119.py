...
Description :
    ...
    2:在两个集群中创建相同字段表
    3:创建发布端(集群A)
    4:开启事务，在事务中创建订阅（集群B）  expect:创建订阅失败，cannot run inside a transaction block
Expect      :
    1：成功
    2：成功
    3：成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case119_1"
        self.pubname1 = "pub_case119_1"
        self.tb_name1 = "public.t_case119_1"
    ...
    def test_pubsub(self):
        ...
        sql = f"start transaction;" \
            ...
            f"PUBLICATION {self.pubname1}; commit;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn("cannot run inside a transaction block",
                      result, '执行失败:' + text)
