...
"""
Case Type   : 基础功能
Case Name   : 使用gs_dump/gs_resotre备份恢复订阅
Description :
    1.在两个集群创建表
    2.发布端打开增量备份开关
    3.订阅端备份
    ...
    6.订阅端向其他数据库导入数据
    7.修改属性
    8.切换到tpcc数据库删除sub1，之后在postgres数据库再修改sub1属性
    9.更新数据
    10.查询数据是否更新
Expect      :
    1.成功
    ...
    4.成功
    5.更新
    6.成功，提示enabled=false
    7.失败， FATAL:  replication slot "sub1" already exists
    8.成功
    9.成功
    10.tpcc数据库中表更新，postgres数据中表未更新
History     :
"""
import unittest
import os
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info("-Opengauss_Function_Pub_Sub_Case0071 start-")
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case071_1'
        self.tb_name2 = 'tb_pubsub_case071_2'
        self.tbs_name = 'tps_pubsub_case071'
        self.schema_name1 = 's_pubsub_case071_1'
        self.subname1 = "sub_case071_1"
        self.pubname1 = "pub_case071_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step3:订阅端备份 expect:成功--'
        self.log.info(text)
        result = self.commsh_sub.exec_gs_dump(
            self.backup_path, env_path=macro.DB_ENV_PATH_REMOTE1)
        self.assertTrue(result, '执行失败:' + text)
        ...
        text = "--step6:订阅端向其他数据库导入数据 expect:成功，提示enabled=false--"
        self.log.info(text)
        result = self.commsh_sub.restore_file(
            self.backup_path, dbname='postgres',
            env_path=macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.RESTORE_SUCCESS_MSG,
                      result, '执行失败' + text)
        self.assertIn('enabled=false', result, '执行失败' + text)

        text = '--step7:修改属性 ' \
               'expect:失败， FATAL:  replication slot "sub1" already exists--'
        self.log.info(text)
        ...
        result = self.commsh_sub.execut_db_sql(sql,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(f'FATAL:  replication slot "{self.subname1}" '
                      f'already exists', result, '执行失败' + text)

        text = '--step8:切换到tpcc数据库删除sub1，之后在postgres数据库再修改sub1属性 expect:成功--'
        self.log.info(text)
        ...
        result = self.commsh_sub.execut_db_sql(sql,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.alter_sub_succ_msg, result, '执行失败' + text)
        sql = f"select pg_sleep(30);select * from pg_replication_slots;"
        result = self.commsh_pub.execut_db_sql(sql,
                                              sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        self.assertNotIn('2 | first        | %一     | new ',
                         result, '执行失败' + text)
        ...
        self.assertIn('2 | first        | %一     | new',
                      result, '执行失败' + text)
        self.assertIn(' 201 |   1 |   1', result, '执行失败' + text)
    ...
    def tearDown(self):
        ...
        drop_sub_result = self.commsh_sub.execut_db_sql(
            sql, self.user_param_sub, 'postgres', macro.DB_ENV_PATH_REMOTE1)
        self.log.info(drop_sub_result)
        ...
        self.assertIn(self.constant.drop_sub_succ_msg, drop_sub_result,
                      '执行失败' + text)
        self.log.info("-Opengauss_Function_Pub_Sub_Case0071 end-")
