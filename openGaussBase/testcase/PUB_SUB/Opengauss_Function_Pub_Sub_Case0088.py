...
"""
Case Type   : 基础功能
Case Name   : 发布内容_账本数据库使用的类型
Description :
    1.两个集群均创建表_账本数据库使用的类型
    2.创建发布订阅
    3.插入数据_集群A:
    4.查询数据_集群B:订阅端
    5.修改1列数据 集群A:发布端
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case088_1'
        self.subname1 = "sub_case088_1"
        self.pubname1 = "pub_case088_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:两个集群均创建表_账本数据库使用的类型 expect:成功--'
        self.log.info(text)
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(a int  primary key,b HASH16,c HASH32)'
        ...
        sql = f"insert into {self.tb_name1} values" \
            f"(10,'e697da2eaa3a775b','685847ed1fe38e18f6b0e2b18c00edee');" \
        ...
        self.log.info(text)
        sql = f"UPDATE {self.tb_name1}  SET b='fffffffffffff';" \
            f"UPDATE {self.tb_name1}  SET c='ccc847ed1fe38e18f6b0e2b18c00edee';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        self.log.info(result)
        self.assertIn(
            '10 | 000fffffffffffff | ccc847ed1fe38e18f6b0e2b18c00edee',
            result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
