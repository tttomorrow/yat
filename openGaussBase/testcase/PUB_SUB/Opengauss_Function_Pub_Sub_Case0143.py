...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case143_1'
        self.subname1 = "sub_case143_1"
        self.pubname1 = "pub_case143_1"
    ...
    def test_pubsub(self):
        ...
        create_sql = f"CREATE TABLE {self.tb_name1}(    " \
            ...
            f"    sales_amt  int)PARTITION BY LIST (month_code) " \
            f"SUBPARTITION BY HASH (dept_code)(  " \
            f"PARTITION p_201901 VALUES ( '201902' )  (" \
            f"    SUBPARTITION p_201901_a," \
            f"    SUBPARTITION p_201901_b  )," \
            f"  PARTITION p_201902 VALUES ( '201903' )  " \
            f"(    SUBPARTITION p_201902_a,    SUBPARTITION p_201902_b  ));"
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values('201902', '1', '1', 1);" \
            f"insert into {self.tb_name1} values('201902', '2', '1', 1);" \
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('201903     | 2         | 1       |         1',
                      result, '执行失败:' + text)
        self.assertIn('201903     | 1         | 1       |         1',
                      result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
