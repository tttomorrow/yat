...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case064_1'
        self.tb_name2 = 'tb_pubsub_case064_2'
        self.schema_name1 = 's_pubsub_case064_1'
        self.subname1 = "sub_case064_1"
        self.pubname1 = "pub_case064_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step5:重启订阅端 expect:成功--'
        self.log.info(text)
        result = self.commsh_sub.start_db_cluster(True,
            env_path=macro.DB_ENV_PATH_REMOTE1)
        flg = self.constant.START_SUCCESS_MSG in result or 'Degrade' in result
        self.assertTrue(flg, '执行失败:' + text)
        ...
        self.log.info(text)
        sql = f"delete from {self.schema_name1}.{self.tb_name1} " \
            f"where id1 <10;"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('91', result, '执行失败' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败' + text)
