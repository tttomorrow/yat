...
"""
Case Type   : 基础功能
Case Name   : 删除订阅，验证参数name
Description :
    1:创建用户，赋予所有权限
    2:在两个集群中创建相同字段表
    3:创建发布端(集群A)
    4:创建订阅（集群B）  expect:创建订阅成功
    5:向发布端表中插入数据   expect:成功
    6:查询集群B中表中数据是否同步   expect:数据已同步
    7.1:删除订阅，订阅名称符合规范但不存在(含if exists参数验证)   expect:删除订阅失败
    7.2:删除订阅，订阅名称不符合规范(含if exists参数验证)   expect:删除订阅失败
    7.3:删除订阅，订阅名称已存在(含if exists参数验证)    expect:删除订阅成功
    8:删除订阅，订阅名称已存在   expect:删除订阅成功
    9:删除订阅，订阅名称已存在,restrict参数验证   expect:删除订阅成功
    10:删除订阅，订阅名称已存在,cascade参数验证   expect:删除订阅成功
Expect      :
    1：成功
    2：成功
    3：成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case116_1"
        self.pubname1 = "pub_case116_1"
        self.tb_name1 = "public.t_case116_1"
    ...
    def test_pubsub(self):
        ...
        text = "--step5:向发布端表中插入数据   expect:成功--"
        self.log.info(text)
        ...
        text = "--step6:查询集群B中表中数据是否同步   expect:数据同步--"
        self.log.info(text)
        ...
        text = "--step7.1:删除订阅，订阅名称符合规范但不存在(含if exists参数验证)  expect:删除订阅失败--"
        self.log.info(text)
        sql = f"drop subscription if exists not_exists;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        ...
        self.assertNotIn(self.constant.SQL_WRONG_MSG[1], result,
                         '执行失败:' + text)
        sql = f"drop subscription not_exists;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.NOT_EXIST, result, '执行失败:' + text)

        text = "--step7.2:删除订阅，订阅名称不符合规范(含if exists参数验证)   expect:删除订阅失败--"
        self.log.info(text)
        sql = f"drop subscription if exists @sub24@;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.SQL_WRONG_MSG[1], result,
                         '执行失败:' + text)
        sql = f"drop subscription  @sub24@;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.SQL_WRONG_MSG[1], result,
        ...
        text = "--step7.3:删除订阅，订阅名称已存在(含if exists参数验证)    expect:删除订阅成功--"
        self.log.info(text)
        ...
        text = "--step8:删除订阅，订阅名称已存在   expect:删除订阅成功--"
        self.log.info(text)
        sql = f"CREATE SUBSCRIPTION {self.subname1} CONNECTION " \
            f"'host={self.pri_userdb_pub.db_host} " \
            f"port={self.port} " \
        ...
        text = "--step9:删除订阅，订阅名称已存在,restrict参数验证   expect:删除订阅成功--"
        self.log.info(text)
        sql = f"CREATE SUBSCRIPTION {self.subname1} CONNECTION " \
            f"'host={self.pri_userdb_pub.db_host} " \
            f"port={self.port} " \
            ...
            f"PUBLICATION {self.pubname1};" \
            f"select pg_sleep(5); " \
            f"drop subscription {self.subname1} restrict;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.drop_sub_succ_msg, result, '执行失败:' + text)

        text = "--step10:删除订阅，订阅名称已存在,cascade   expect:删除订阅成功--"
        self.log.info(text)
        sql = f"CREATE SUBSCRIPTION {self.subname1} CONNECTION " \
            f"'host={self.pri_userdb_pub.db_host} " \
            f"port={self.port} " \
            ...
            f"PUBLICATION {self.pubname1};" \
            f"select pg_sleep(5); " \
            f"drop subscription {self.subname1} cascade;"
        result = self.commsh_sub.execut_db_sql(sql, self.user_param_u, None,
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(self.constant.drop_sub_succ_msg, result, '执行失败:' + text)
