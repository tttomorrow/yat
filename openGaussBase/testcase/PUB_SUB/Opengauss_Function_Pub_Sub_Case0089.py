...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case089_1'
        self.subname1 = "sub_case089_1"
        self.pubname1 = "pub_case089_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:两个集群均创建表_二进制型 expect:成功--'
        self.log.info(text)
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(i int,i2 BLOB,i3 RAW,i4 BYTEA primary key);'
        ...
        sql = f"insert into {self.tb_name1} values" \
            f"(1,empty_blob(),HEXTORAW('DEADBEEF'),E'\\\\xDEADBEEF');" \
        ...
        self.log.info(text)
        sql = f"UPDATE {self.tb_name1}  SET i2='00010';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn('1 | 000010 | DEADBEEF | \\xde414442454546',
                      result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
