...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case090_1'
        self.tb_name2 = 'tb_pubsub_case090_2'
        self.tb_name3 = 'tb_pubsub_case090_3'
        self.subname1 = "sub_case090_1"
        self.pubname1 = "pub_case090_1"
    ...
    def test_pubsub(self):
        ...
        text = '--step1:两个集群均创建表_时间/日期型 expect:成功--'
        self.log.info(text)
        create_sql = f'CREATE TABLE {self.tb_name1}' \
            f'(i int ,i2 DATE primary key,i3 TIME WITHOUT TIME ZONE,' \
            f'i4 TIME WITH TIME ZONE,i5 TIMESTAMP WITHOUT TIME ZONE,' \
            f'i6 TIMESTAMP WITH TIME ZONE,i7 SMALLDATETIME);' \
            f'CREATE TABLE {self.tb_name2}(i int ,' \
            f'i8 INTERVAL DAY(3) TO SECOND (4) primary key,' \
            f'i9 interval year (6));CREATE TABLE {self.tb_name3}(i int ,' \
            f'i11 date,i12 timestamp primary key,i13 timestamp,' \
            f'i14 date,i15 date,i16 date,i17 date,i18 time);'
        ...
        sql = f"insert into {self.tb_name1} values" \
            f"(1,'2021-12-15','21:21:21','21:21:21 pst'," \
            f"'2010-12-12','2013-12-11 pst','2003-04-12 04:05:06');" \
            f"insert into {self.tb_name2} values" \
            f"(10,INTERVAL '3' DAY,INTERVAL '2' YEAR);" \
            f"insert into {self.tb_name3} values(100,'epoch'," \
            f"'infinity','-infinity','now','today','tomorrow'," \
            f"'yesterday','allballs');select pg_sleep(5.5);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        text = "--step5:修改1列数据 集群A:发布端 expect:成功--"
        self.log.info(text)
        sql = f"UPDATE {self.tb_name1}  SET " \
            f"i2='2020-10-18' WHERE i2='2021-12-15';"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               ...
                                               macro.DB_ENV_PATH_REMOTE1)
        self.log.info(result)
        self.assertIn(' 2020-10-18 00:00:00', result, '执行失败:' + text)
        result_a = self.commsh_pub.execut_db_sql(sql_select,
                                                 sql_type=self.user_param_pub)
        self.log.info(result_a)
        self.assertEqual(result_a, result, '执行失败:' + text)
