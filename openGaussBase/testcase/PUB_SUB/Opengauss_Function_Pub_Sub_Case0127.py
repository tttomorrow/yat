...
"""
Case Type   : 基础功能
Case Name   : 发布订阅系统视图--pg_publication_tables_table
Description :
    1.创建发布端
    2.查询pg_publication_tables_table结构，及内容
    3.修改发布端信息
    4.查询系统表内容 
Expect      :
    1.成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case127_1'
        self.pubname = "pub_case127"
        self.parent_path_pub = os.path.dirname(macro.DB_INSTANCE_PATH)
        self.user_param_pub = f'-U {self.pri_userdb_pub.db_user} ' \
            f'-W {self.pri_userdb_pub.db_password}'
        self.schema_name = 's_pubsub_case127'
    ...
    def test_pubsub(self):
        ...
        text = '--step2:查询pg_publication_tables结构，及内容 expect:类型与资料一致且内容正确--'
        self.log.info(text)
        sql = f"\d+  pg_publication_tables"
        result = self.commsh_pub.execut_db_sql(sql)
        self.log.info(result)
        self.assertIn('pubname    | name', result, '执行失败:' + text)
        self.assertIn('schemaname | name |', result, '执行失败:' + text)
        self.assertIn('tablename  | name ', result, '执行失败:' + text)
        sql = f"select * from pg_publication_tables;"
        result = self.commsh_pub.execut_db_sql(sql)
        self.log.info(result)
        flg = re.search(rf'{self.pubname}(.*)public(.*){self.tb_name1}',
                        result, re.M | re.I)
        ...
        result = self.commsh_pub.execut_db_sql(sql)
        self.log.info(result)
        self.assertIn(f'{self.pubname} | public           | {self.tb_name1}',
                      result, '执行失败:' + text)
        self.assertIn(f'{self.pubname} | {self.schema_name} | '
                      f'{self.tb_name1}', result, '执行失败:' + text)
