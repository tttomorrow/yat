...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.num_sub = int(self.commsh_sub.get_node_num(
            macro.DB_ENV_PATH_REMOTE1)) - 1
        self.log.info(str(self.num_pub))
        self.log.info(str(self.num_sub))
    ...
    def test_pubsub(self):
        ...
        text = '--step10:更新并查询数据是否更新 expect:插入数据成功，不更新，复制槽不存在--'
        self.log.info(text)
        sql_insert = f"insert into {self.schema_name1}.{self.tb_name1} " \
            f"values(200, 1, 1);" \
    ...
    def tearDown(self):
        ...
        if not result:
            ...
            self.commsh_sub.restart_db_cluster(True,
                                               macro.DB_ENV_PATH_REMOTE1)
        for i in range(self.num_sub):
            self.comsh_sta_sub[i].build_standby(
                "-t 3600", macro.DB_ENV_PATH_REMOTE1,
                macro.DB_INSTANCE_PATH_REMOTE1)
