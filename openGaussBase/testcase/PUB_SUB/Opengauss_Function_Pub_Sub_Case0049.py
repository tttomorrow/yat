...
"""
Case Type   : 基础功能
Case Name   : max_replication_slots小于物理+逻辑+订阅数
Description :
    1.修改max_replication_slots为5
    2.两个集群创建表
    3.创建逻辑复制槽
    4.创建发布端订阅端（假设集群为1主2备，则需逻辑复制槽3个）
    5.修改数据
    6.读取复制槽slot1解码结果
    7.查看数据是否更新
Expect      :
    ...
    5.成功
    6.成功
    7.tb_pubsub_case049_1/tb_pubsub_case049_2更新，其余未更新
History     :
"""
import unittest
import os
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info("-Opengauss_Function_Pub_Sub_Case0049 start-")
        ...
        self.com = Common()
        self.tb_name = ['tb_pubsub_case049_1', 'tb_pubsub_case049_2',
                        'tb_pubsub_case049_3']
        self.subname = ["sub_case049_1", "sub_case049_2", "sub_case049_3"]
        self.pubname = "pub_case049_1"
        self.slot_name = "slot_case049"
    ...
    def test_pubsub(self):
        ...
        sql_table = f"CREATE TABLE {self.tb_name[0]}(id NUMBER(7) " \
            ...
            f"CREATE TABLE {self.tb_name[1]}" \
            f"(like {self.tb_name[0]} including all);" \
            f"CREATE TABLE {self.tb_name[2]}(i int primary key, " \
            f"text varchar(1024)) WITH (ORIENTATION = COLUMN, " \
            f"COMPRESSION=HIGH);"
        ...
        text = '--step4:创建发布端订阅端（假设集群为1主2备，则需再逻辑复制槽3个） expect:成功--'
        self.log.info(text)
        ...
        for i in range(3):
            ...
            if 2 == i:
                sql = f"CREATE SUBSCRIPTION {self.subname[i]} CONNECTION " \
                    ...
                    f"dbname={self.pri_userdb_pub.db_name} " \
                    f"password={self.pri_userdb_pub.ssh_password}' " \
                    f"PUBLICATION {self.pubname} with (enabled=false);" \
                    f"select pg_sleep(30);" \
                    f"select * from pg_SUBSCRIPTION;"
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name[0]} " \
        ...
        text = "--step6:读取复制槽slot1解码结果  " \
               "expect:成功,tb_pubsub_case049_1/tb_pubsub_case049_2存在于解码记录中--"
        self.log.info(text)
        ...
        text = '--step7:查看数据是否更新 ' \
               'expect:tb_pubsub_case049_1/tb_pubsub_case049_2更新，其余未更新--'
        self.log.info(text)
        for i in range(3):
    ...
    def tearDown(self):
        ...
        self.assertIn(self.constant.drop_sub_succ_msg, drop_sub_result,
                      '执行失败' + text)
        self.log.info("-Opengauss_Function_Pub_Sub_Case0049 end-")
