...
"""
Case Type   : 基础功能
Case Name   : 用户是超级管理员superuser
Description :
    1.两个集群创建用户
    2.创建发布(发布端执行)
    3.发布端创建用户且非超级管理员也不是运维管理员
    4.创建订阅，订阅信息中发布用户为超级用户(订阅端执行，端口只能是DN端口+1)
Expect      :
    1.成功
    2.成功
    3.成功
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case165_1"
        self.pubname1 = "pub_case165_1"
        ...
        self.user_name1 = "u_case165_1"
        self.user_name2 = "u_case165_2"
    ...
    def test_pubsub(self):
        ...
        text = "--step3:发布端创建用户且非超级管理员也不是运维管理员 expect:成功--"
        self.log.info(text)
        sql = f"create user {self.user_name2} with Replication  " \
        ...
        text = "--step4:创建订阅，订阅信息中发布用户为超级用户(订阅端执行，端口只能是DN端口+1) " \
               "expect:创建失败，提示无法连接--"
        self.log.info(text)
