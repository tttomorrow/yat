...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.tb_name1 = 'tb_pubsub_case026_1'
        self.tb_name2 = 'tb_pubsub_case026_2'
        self.subname = "sub_case026"
        self.pubname = "pub_case026"
    ...
    def test_pubsub(self):
        ...
        sql = f"update {self.tb_name1} set t='update' where i=1;" \
            ...
            f"update {self.tb_name2} set c='update' where i=1;"
        result = self.commsh_pub.execut_db_sql(sql, 
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        ...
        self.log.info(text)
        sql = f"insert into {self.tb_name1} values(2, 'not fresh');" \
            f"{sql_select}"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertIn(self.constant.INSERT_SUCCESS_MSG, result, '执行失败' + text)
        self.assertIn('2 | not fresh', result, '执行失败' + text)
        ...
        self.log.info(text)
        sql = f"delete from {self.tb_name2};{sql_select}"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
