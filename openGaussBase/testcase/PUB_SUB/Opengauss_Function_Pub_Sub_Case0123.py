...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = 'tb_pubsub_case122_1'
        self.tb_name2 = 'tb_pubsub_case122_2'
        self.tb_name3 = 'tb_pubsub_case122_3'
        self.subname1 = "sub_case122_1"
        self.pubname1 = "pub_case122_1"
        ...
        self.replication_type_sub = self.com_sub.show_param(
            "replication_type", macro.DB_ENV_PATH_REMOTE1)
        self.hot_standby_sub = self.com_sub.show_param(
            "hot_standby", macro.DB_ENV_PATH_REMOTE1)
        self.recovery_redo_workers_sub = self.com_sub.show_param(
            "recovery_redo_workers", macro.DB_ENV_PATH_REMOTE1)
        self.recovery_parse_workers_sub = self.com_sub.show_param(
            "recovery_parse_workers", macro.DB_ENV_PATH_REMOTE1)
        self.replication_type_pub = self.com_pub.show_param(
            "replication_type")
        self.hot_standby_pub = self.com_pub.show_param("hot_standby")
        self.recovery_redo_workers_pub = self.com_pub.show_param(
            "recovery_redo_workers")
        self.recovery_parse_workers_pub = self.com_pub.show_param(
    ...
    def tearDown(self):
        ...
        result = self.commsh_sub.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            f'replication_type={self.replication_type_sub}',
            dn_path=macro.DB_INSTANCE_PATH_REMOTE1,
            env_path=macro.DB_ENV_PATH_REMOTE1)
        self.assertTrue(result, '执行失败:' + text)
        result = self.commsh_pub.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            f'replication_type={self.replication_type_pub}')
        self.assertTrue(result, '执行失败:' + text)
        result1_p = self.commsh_pub.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            f'recovery_parse_workers={self.recovery_parse_workers_pub}')
        result2_p = self.commsh_pub.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            f'recovery_redo_workers={self.recovery_redo_workers_pub}')
        result3_p = self.commsh_pub.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            f'hot_standby={self.hot_standby_pub}')
        result1 = self.commsh_sub.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            f'recovery_parse_workers={self.recovery_parse_workers_sub}',
            dn_path=macro.DB_INSTANCE_PATH_REMOTE1,
            env_path=macro.DB_ENV_PATH_REMOTE1)
        result2 = self.commsh_sub.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            f'recovery_redo_workers={self.recovery_redo_workers_sub}',
            dn_path=macro.DB_INSTANCE_PATH_REMOTE1,
            env_path=macro.DB_ENV_PATH_REMOTE1)
        result3 = self.commsh_sub.execute_gsguc(
            'set', self.constant.GSGUC_SUCCESS_MSG,
            f'hot_standby={self.hot_standby_sub}',
            dn_path=macro.DB_INSTANCE_PATH_REMOTE1,
            env_path=macro.DB_ENV_PATH_REMOTE1)
        self.assertTrue(result1 and result2 and result3, '执行失败:' + text)
        self.assertTrue(result1_p and result2_p and result3_p, '执行失败:' + text)
