...
Description :
    ...
    7.2、修改订阅connection连接信息dbname参数为password参数为加密后的修改密码    expect:成功
...
class Pubsubclass(unittest.TestCase):
    ...
    def test_pubsub(self):
        ...
        text = "--step5: 修改订阅connection连接信息password参数为修改后的明文密码   expect:失败--"
        self.log.info(text)
        ...
        text = "--step6: 修改订阅connection连接信息password参数为错误密码   expect:失败--"
        self.log.info(text)
        result = self.commsh_sub.execute_generate(
            self.common_passwd, env_path=macro.DB_ENV_PATH_REMOTE1)
        self.assertIn('', result, '执行失败:' + text)
        ...
        text = "--step7.2:修改订阅connection连接信息dbname参数为password参数为加密后的修改密码 " \
               " expect:成功--"
        self.log.info(text)
        sql = f"select pg_sleep(10);" \
            f"alter SUBSCRIPTION {self.subname1} CONNECTION " \
