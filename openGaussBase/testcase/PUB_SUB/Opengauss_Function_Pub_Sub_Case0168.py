...
"""
Case Type   : 基础功能
Case Name   : 发布端用户不具有数据库登录权限
Description :
    1.两个集群创建用户
    2.创建发布,并创建无登录权限用户(发布端执行)
    3.创建订阅(订阅端执行，端口只能是DN端口+1)
Expect      :
    1.成功
    2.成功
    3.创建失败，提示WARNING:  apply worker could not connect to the remote server
History     :
"""
import unittest
import os
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.com_pub = Common()
        self.subname1 = "sub_case168_1"
        self.pubname1 = "pub_case168_1"
        ...
        self.user_name1 = "u_case168_1"
        self.user_name2 = "u_case168_2"
    ...
    def test_pubsub(self):
        ...
        text = "--step2:创建发布,并创建无登录权限用户(发布端执行) expect:成功--"
        self.log.info(text)
        sql = f"create role {self.user_name2} " \
            f"identified by '{macro.COMMON_PASSWD}';" \
        ...
        text = "--step3:创建订阅(订阅端执行，端口只能是DN端口+1) " \
               "expect:创建失败，提示WARNING:  apply worker " \
        ...
        self.assertIn('WARNING:  apply worker could not '
                      'connect to the remote server', result, '执行失败:' + text)
