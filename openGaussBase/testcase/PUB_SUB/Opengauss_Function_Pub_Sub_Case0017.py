...
"""
Case Type   : 基础功能
Case Name   : 发布指定表不存在关联表，创建发布订阅后，修改发布端表结构
Description :
    1.在两个集群创建表
    2.创建发布端
    3.创建订阅
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.tb_name1 = 'tb_pubsub_case017_1'
        self.subname = "sub_case017"
        self.pubname = "pub_case017"
    ...
    def test_pubsub(self):
        ...
        sql = f"drop table {self.tb_name1};" \
            f"create table {self.tb_name1}(i text primary key,t int);" \
            f"insert into {self.tb_name1} values('NULL', 2);"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(self.constant.INSERT_SUCCESS_MSG),
        ...
        sql = f"drop table {self.tb_name1};" \
            ...
            f"insert into {self.tb_name1} values(2, 'new');"
        result = self.commsh_pub.execut_db_sql(sql,
                                               sql_type=self.user_param_pub)
        self.log.info(result)
        self.assertEqual(result.count(self.constant.INSERT_SUCCESS_MSG),
        ...
        text = "--step9:查询是否同步 expect:数据不同步，由于step6导致，pg_log提示--"
        self.log.info(text)
        time.sleep(10)
        result = self.commsh_sub.execut_db_sql(sql_select,
                                               self.user_param_sub,
        ...
        flg = 'FATAL:  invalid input syntax for integer:'
