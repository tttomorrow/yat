...
Expect      :
    ...
    2.成功
    3.成功
    4.失败，pg_log日志中相关报错日志，确保无明文密码落盘
History     :
"""
import unittest
import os
...
class Pubsubclass(unittest.TestCase):
    def setUp(self):
        ...
        self.subname1 = "sub_case156_1"
        self.pubname1 = "pub_case156_1"
        ...
        self.user_name1 = "u_case156_1"
        self.user_name2 = "u_case156_2"
    ...
    def test_pubsub(self):
        ...
        text = "--step4:创建订阅(订阅端执行，端口只能是DN端口+1)   " \
               "expect:失败，pg_log日志中相关报错日志，确保无明文密码落盘--"
        self.log.info(text)
        ...
        self.assertIn(self.constant.can_not_connect_pub_msg,
                      result, '执行失败:' + text)
        flg = [self.constant.can_not_connect_pub_msg,
               f'STATEMENT:  CREATE SUBSCRIPTION {self.subname1} CONNECTION',
               self.passwd, macro.COMMON_PASSWD]
        for i in range(4):
            result = self.com_pub.find_pglog_content(
                ...
                path=self.dir_new)
            if i < 1:
                self.assertIn(flg[i], result, '执行失败:' + text)
            elif i == 1:
                content = f"STATEMENT:  CREATE SUBSCRIPTION {self.subname1} " \
                    f"CONNECTION *******"
                self.assertIn(content, result, '执行失败:' + text)
            else:
                self.assertNotIn(flg[i], result, '执行失败:' + text)
            result = self.com_pub.find_pglog_content(
