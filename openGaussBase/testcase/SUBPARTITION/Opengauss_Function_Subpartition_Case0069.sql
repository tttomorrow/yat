-- @testpoint: list_list二级分区表：自治事务/匿名块

--step1: 创建表空间; expect:成功
drop table if exists t_subpartition_0069;
drop tablespace if exists ts_subpartition_0069;
create tablespace ts_subpartition_0069 relative location 'subpartition_tablespace/subpartition_tablespace_0069';
--test1: 自治事务
--step2: 创建二级分区表; expect:成功
create table t_subpartition_0069
(
    col_1 int ,
    col_2 int ,
    col_3 varchar2 ( 30 ) ,
...
tablespace ts_subpartition_0069
...
insert into t_subpartition_0069 values(1,2);
--step4: 查询数据; expect:成功，1条数据
select * from t_subpartition_0069;
...
begin
insert into t_subpartition_0069 values(num3, num4);
end;
/
...
declare
begin
insert into t_subpartition_0069 values(66, 66);
autonomous_4(a,b);
rollback;
end;
/
...
select * from t_subpartition_0069 order by col_1;
...
pragma autonomous_transaction;
begin
insert into t_subpartition_0069 values(1,2,'you are so cute,will commit!');
end;
/
--step11: 插入数据; expect:成功
insert into t_subpartition_0069 values(1,4,'you will rollback!');
...
pragma autonomous_transaction;
begin
insert into t_subpartition_0069 values(a, b, c);
return 1;
end;
/
...
--step19: 删除表和表空间; expect:成功
drop table if exists t_subpartition_0069_01;
drop table if exists t_subpartition_0069;
drop tablespace if exists ts_subpartition_0069;