...
"""
Case Type   : MOT
Case Name   : MOT设置max_mot_global_memory=-100%进行SQL语句测试
Description :
    1、修改配置文件mot.conf中max_mot_global_memory=-100%
...
CONFIG_PARAM = 'max_mot_global_memory = -100%'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0197开始执行======")
        LOG.info("===步骤1：修改配置文件，设置max_mot_global_memory=-100%,重启数据库===")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0197执行结束======')
