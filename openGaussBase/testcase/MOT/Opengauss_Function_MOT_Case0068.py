...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("------------------Opengauss_Function_MOT_Case0068开始执行------------------")
        ...
        self.sql_cmd = f'''
                        ...
                        CREATE FOREIGN TABLE {self.schema}.mixindex1(t1 float4 not null,t2 float4 not null,t3 float4 not null,t4 varchar(232) not null);
                        --总列宽256
                        CREATE INDEX index1 ON {self.schema}.mixindex1(t1,t2,t4);
                        --对列t1,t3,t4创建唯一索引
                        CREATE FOREIGN TABLE {self.schema}.mixindex2(t1 float4 not null,t2 float4 not null,t3 float4 not null,t4 varchar(240) not null);
                        --总列宽256
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0068执行结束---------------')
