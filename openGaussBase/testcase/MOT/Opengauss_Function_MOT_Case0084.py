...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("------------------Opengauss_Function_MOT_Case0084开始执行-----------------")
        ...
        self.sql_cmd = f'''
                        ...
                        CREATE FOREIGN TABLE {self.schema}.mixindex1(t1 float4 not null,t2 float4 not null,t3 float4 not null,t4 varchar(233) not null);
                        --总列宽257
                        CREATE INDEX index1 ON {self.schema}.mixindex1(t1,t2,t4);
                        --对列t1,t3,t4创建唯一索引
                        CREATE FOREIGN TABLE {self.schema}.mixindex2(t1 float4 not null,t2 float4 not null,t3 float4 not null,t4 varchar(241) not null);
                        --总列宽257
                        CREATE UNIQUE INDEX index4 ON {self.schema}.mixindex2(t1,t3,t4);
                        DROP SCHEMA {self.schema} CASCADE;
                     '''
        logger.info("-------------------开始用例测试:mot不支持在总列宽>256的表上创建索引，数据类型为float4-------------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0084执行结束---------------')
