...
"""
Case Type   : MOT
Case Name   : MOT设置min_mot_local_memory=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中min_mot_local_memory=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep min_mot_local_memory
...
CONFIG_PARAM = 'min_mot_local_memory = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0214开始执行======")
        LOG.info("======步骤1：修改配置文件，设置min_mot_local_memory=xxx启数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep min_mot_local_memory'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep min_mot_local_memory'''
        LOG.info(cat_cmd)
        cmd_msg = self.user_node.sh(cat_cmd).result()
        LOG.info(cmd_msg)
        self.assertIn('WARNING', cmd_msg)
        self.assertIn('Invalid min_mot_local_memory memory value'
                      ' format: xxx', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0214执行结束======')
