...
Case Name： MOT支持的数据类型binary_integer
'''
...
class Mot_datatype_test(unittest.TestCase):
    ...
    def test_mot_datatype(self):
        logger.info("-----------------------Opengauss_Function_MOT_Case0004开始执行----------------")
        self.sql_cmd = f'''
                        create schema schema_mot_test;
                        create FOREIGN table schema_mot_test.bigint01 (name binary_integer);
                        insert into schema_mot_test.bigint01 values (120);
                        select * from schema_mot_test.bigint01;
                        drop schema schema_mot_test CASCADE;
                                              '''
        logger.info("-------------------------开始用例测试:MOT支持binary_integer数据类型--------------------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0004执行结束---------------')
