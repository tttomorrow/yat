...
"""
Case Type   : MOT
Case Name   : MOT设置enable_mot_codegen=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_mot_codegen=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep enable_mot_codegen
...
CONFIG_PARAM = 'enable_mot_codegen = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0256开始执行======")
        LOG.info("======步骤1：修改配置文件设置enable_mot_codegen=xxx启动数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep enable_mot_codegen'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep enable_mot_codegen'''
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0256执行结束======')
