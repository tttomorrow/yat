...
"""
Case Type   : MOT
Case Name   : MOT设置enable_gc= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_gc=
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0239开始执行======")
        LOG.info("======步骤1：修改配置文件设置enable_gc= 启动数据库======")
        ...
        self.assertIn(
            'Empty value specified for configuration item enable_gc', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0239执行结束======')
