...
"""
Case Type   : MOT
Case Name   : MOT设置reclaim_threshold=-1KB进行SQL语句测试
Description :
    1、修改配置文件mot.conf中reclaim_threshold=-1KB
...
CONFIG_PARAM = 'reclaim_threshold = -1KB'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0242开始执行======")
        LOG.info("======步骤1：修改配置文件设置reclaim_threshold=-1KB启动数据库======")
        ...
        self.assertIn(
            'Configuration of reclaim_threshold=18446744073709550592'
            ' is out of bounds [1024, 67108864]', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0242执行结束======')
