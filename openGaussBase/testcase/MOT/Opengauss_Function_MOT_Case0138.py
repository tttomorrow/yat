...
"""
Case Type   : MOT
Case Name   : MOT设置enable_stats=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_stats=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看mot.conf配置文件参数是否生效,cat cluster/dn1/mot.conf | grep enable_stats
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0138开始执行======")
        LOG.info("======步骤1：修改mot.conf配置文件，设置enable_stats=xxx,重启数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep enable_stats'''
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0138执行结束======')
