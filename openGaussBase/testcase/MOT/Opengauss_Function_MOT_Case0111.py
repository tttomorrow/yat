...
"""
Case Type   : MOT
Case Name   : MOT设置enable_group_commit= true进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_group_commit= true
...
Expect      :
    ...
    2、重启数据库成功；
    3、查看配置文件，参数修改生效；
    4、查看日志，无enable_group_commit无效信息
    5、连接数据库成功，创建内存表成功；
    6、清理环境成功；
History     :
"""
...
CONFIG_PARAM = 'enable_group_commit = true'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0111开始执行======")
        LOG.info("===步骤1：修改mot.conf配置文件，设置enable_group_commit=true,重启数据库===")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0111执行结束======')
