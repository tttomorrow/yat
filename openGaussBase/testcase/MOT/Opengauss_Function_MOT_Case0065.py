...
'''
Case Type:  mot支持的索引
Case Name:  MOT表支持在总列宽=256的表上创建索引，数据类型为smallint
...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("----------------------Opengauss_Function_MOT_Case0065开始执行---------------------")
        ...
        self.sql_cmd = f'''
                        ...
                        CREATE FOREIGN TABLE {self.schema}.mixindex1(t1 smallint not null,t2 smallint not null,t3 smallint not null,t4 varchar(238) not null);
                        --总列宽256
                        CREATE INDEX index1 ON {self.schema}.mixindex1(t1,t2,t4);
                        --对列t1,t3,t4创建唯一索引
                        CREATE FOREIGN TABLE {self.schema}.mixindex2(t1 smallint not null,t2 smallint not null,t3 smallint not null,t4 varchar(246) not null);
                        --总列宽256
                        CREATE UNIQUE INDEX index4 ON {self.schema}.mixindex2(t1,t3,t4);
                        DROP SCHEMA {self.schema} CASCADE;
                     '''
        logger.info("----------------开始用例测试:mot支持在总列宽=256的表上创建索引，数据类型为smallint----------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0065执行结束---------------')
