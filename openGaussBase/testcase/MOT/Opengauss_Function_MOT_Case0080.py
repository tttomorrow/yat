...
'''
Case Type:  mot不支持的索引
Case Name:  MOT表不支持在总列宽>256的表上创建索引，数据类型为tinyint
...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("------------------Opengauss_Function_MOT_Case0080开始执行-----------------")
        self.schema = 'schema_mot_test'
        self.sql_cmd = f'''
                        DROP SCHEMA IF EXISTS {self.schema} CASCADE;
                        CREATE SCHEMA {self.schema};
                        --对列t1,t2,t4创建普通索引，tinyint类型附加大小为1，非唯一索引需额外的8字节
                        CREATE FOREIGN TABLE {self.schema}.mixindex1(t1 tinyint not null,t2 tinyint not null,t3 tinyint not null,t4 varchar(242) not null);
                        --总列宽258
                        CREATE INDEX index1 ON {self.schema}.mixindex1(t1,t2,t4);
                        --对列t1,t3,t4创建唯一索引
                        CREATE FOREIGN TABLE {self.schema}.mixindex2(t1 tinyint not null,t2 tinyint not null,t3 tinyint not null,t4 varchar(250) not null);
                        --总列宽258
                        CREATE UNIQUE INDEX index2 ON {self.schema}.mixindex2(t1,t3,t4);
                        DROP SCHEMA {self.schema} CASCADE;
                     '''
        logger.info("------------------开始用例测试:mot不支持在总列宽>256的表上创建索引，数据类型为tinyint------------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0080执行结束---------------')
