...
"""
Case Type   : MOT
Case Name   : MOT设置lazy_load_chunk_directory= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中lazy_load_chunk_directory=
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep lazy_load_chunk_directory
...
CONFIG_PARAM = 'lazy_load_chunk_directory = '
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0176开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置lazy_load_chunk_directory= ,重启数据库")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep lazy_load_chunk_directory'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep lazy_load_chunk_directory'''
        LOG.info(cat_cmd)
        ...
        self.assertIn('Empty value specified for configuration item'
                      ' lazy_load_chunk_directory', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0176执行结束======')
