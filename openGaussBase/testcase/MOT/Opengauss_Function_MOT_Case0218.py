...
"""
Case Type   : MOT
Case Name   : MOT设置max_mot_session_memory= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中max_mot_session_memory= 
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep max_mot_session_memory
...
CONFIG_PARAM = 'max_mot_session_memory =  '
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0218开始执行======")
        LOG.info("======步骤1：修改配置文件，设置max_mot_session_memory= 启数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep max_mot_session_memory'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep max_mot_session_memory'''
        LOG.info(cat_cmd)
        ...
        self.assertIn('Empty value specified for configuration item'
                      ' max_mot_session_memory', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0218执行结束======')
