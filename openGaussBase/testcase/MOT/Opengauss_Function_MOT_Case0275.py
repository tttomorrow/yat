...
"""
Case Type   : MOT
Case Name   : MOT设置 checkpoint_dir= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中 checkpoint_dir= 
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,
       cat cluster/dn1/mot.conf | grep  checkpoint_dir
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0275开始执行======")
        LOG.info("======步骤1：修改配置文件设置 checkpoint_dir= 启动数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep checkpoint_dir'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep  checkpoint_dir
            '''
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info("======Opengauss_Function_MOT_Case0275执行结束======")
