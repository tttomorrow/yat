...
"""
Case Type   : MOT
Case Name   : gs_dump/gsql备份恢复多个内存表数据(纯文本)
Description :
    1、修改配置文件增量检查点为off,
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0107开始执行======")
        ...
        LOG.info("======步骤2：gs_dump以纯文本格式导出多个内存表======")
        ...
        LOG.info("======步骤3：以gsql方式导入多个纯文本格式内存表======")
        sql_cmd = f'''source {macro.DB_ENV_PATH};
            gsql {self.user_node.db_name} \
            -p {self.user_node.db_port} \
            -c "drop foreign table if exists {self.mot_table1};
            drop foreign table if exists {self.mot_table2}";
            gsql {self.user_node.db_name} \
            -p {self.user_node.db_port} \
            -f {self.mot_path}        
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0107执行结束======')
