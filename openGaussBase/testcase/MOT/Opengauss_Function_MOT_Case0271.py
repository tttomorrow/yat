...
"""
Case Type   : MOT
Case Name   : MOT设置session_large_buffer_store_size=-1MB进行SQL语句测试
Description :
    1、修改配置文件mot.conf中session_large_buffer_store_size=-1MB
...
CONFIG_PARAM = 'session_large_buffer_store_size = -1MB'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0271开始执行======")
        LOG.info("===步骤1：修改配置文件设置session_large_buffer_store_size=-1MB启动数据库==")
        ...
        self.assertIn('Configuration of '
                      'session_large_buffer_store_size=17592186044415'
                      ' is out of bounds [0, 131072]', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0271执行结束======')
