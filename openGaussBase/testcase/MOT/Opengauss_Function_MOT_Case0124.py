...
"""
Case Type   : MOT
Case Name   : MOT设置checkpoint_segsize= 1GB进行SQL语句测试
Description :
    1、修改配置文件mot.conf中checkpoint_segsize= 1GB
...
CONFIG_PARAM = 'checkpoint_segsize = 1GB'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0124开始执行======")
        LOG.info("====步骤1：修改mot.conf配置文件，设置checkpoint_segsize=1GB,重启数据库====")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0124执行结束======')
