...
"""
Case Type   : MOT
Case Name   : MOT设置high_reclaim_threshold= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中high_reclaim_threshold=
...
CONFIG_PARAM = 'high_reclaim_threshold =  '
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0254开始执行======")
        LOG.info("======步骤1：修改配置文件设置high_reclaim_threshold= 启动数据库======")
        ...
        self.assertIn('Empty value specified for configuration'
                      ' item high_reclaim_threshold', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0254执行结束======')
