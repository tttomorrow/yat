...
"""
Case Type   : MOT
Case Name   : MOT设置reclaim_batch_size=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中reclaim_batch_size=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep reclaim_batch_size
...
CONFIG_PARAM = 'reclaim_batch_size = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0248开始执行======")
        LOG.info("======步骤1：修改配置文件设置reclaim_batch_size=xxx启动数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep reclaim_batch_size'''
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0248执行结束======')
