...
"""
Case Type   : MOT
Case Name   : MOT设置session_large_buffer_store_size=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中session_large_buffer_store_size=xxx
...
CONFIG_PARAM = 'session_large_buffer_store_size = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0272开始执行======")
        LOG.info("===步骤1：修改配置文件设置session_large_buffer_store_size=xxx启动数据库==")
        ...
        self.assertIn('Invalid session_large_buffer_store_size memory '
                      'value format: xxx', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info("======Opengauss_Function_MOT_Case0272执行结束======")
