...
"""
Case Type   : MOT
Case Name   : MOT设置affinity_mode=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中affinity_mode=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep affinity_mode
...
CONFIG_PARAM = 'affinity_mode = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0171开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置affinity_mode=xxx,重启数据库")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep affinity_mode'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep affinity_mode'''
        LOG.info(cat_cmd)
        cmd_msg = self.user_node.sh(cat_cmd).result()
        LOG.info(cmd_msg)
        self.assertIn('ERROR', cmd_msg)
        self.assertIn('Failed to convert string configuration item '
                      'affinity_mode to user type, using default value:'
                      ' fill-physical-first', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0171执行结束======')
