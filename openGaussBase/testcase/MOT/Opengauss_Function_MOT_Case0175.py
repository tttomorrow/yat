...
"""
Case Type   : MOT
Case Name   : MOT设置lazy_load_chunk_directory=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中lazy_load_chunk_directory=xxx
...
CONFIG_PARAM = 'lazy_load_chunk_directory = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0175开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置lazy_load_chunk_directory=xxx,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0175执行结束======')
