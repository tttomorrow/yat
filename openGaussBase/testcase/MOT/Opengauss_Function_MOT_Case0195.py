...
"""
Case Type   : MOT
Case Name   : MOT设置max_mot_global_memory=5GB进行SQL语句测试
Description :
    1、修改配置文件mot.conf中max_mot_global_memory=5GB
...
CONFIG_PARAM = 'max_mot_global_memory = 5GB'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0195开始执行======")
        LOG.info("步骤1：修改配置文件，设置max_mot_global_memory=5GB,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0195执行结束======')
