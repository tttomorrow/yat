...
"""
Case Type   : MOT
Case Name   : MOT设置max_mot_global_memory=200%进行SQL语句测试
Description :
    1、修改配置文件mot.conf中max_mot_global_memory=200%
...
CONFIG_PARAM = 'max_mot_global_memory = 200%'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0198开始执行======")
        LOG.info("===步骤1：修改配置文件，设置max_mot_global_memory=200%,重启数据库===")
        ...
        self.assertIn('Invalid max_mot_global_memory memory format:'
                      ' illegal percent specification', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0198执行结束======')
