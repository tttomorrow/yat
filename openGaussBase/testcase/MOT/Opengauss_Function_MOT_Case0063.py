...
'''
Case Type:  mot支持的索引
Case Name:  MOT表支持在总列宽=256的表上创建索引，数据类型为varchar
...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("---------------------Opengauss_Function_MOT_Case0063开始执行---------------------")
        ...
        self.sql_cmd = f'''
                        ...
                        CREATE FOREIGN TABLE {self.schema}.mixindex1(t1 varchar(140) not null,t2 varchar(100) not null,t3 varchar(1) not null);
                        --总列宽256
                        CREATE INDEX index1 ON {self.schema}.mixindex1(t1,t2);
                        --对列t1,t3创建唯一索引
                        CREATE FOREIGN TABLE {self.schema}.mixindex2(t1 varchar(247) not null,t2 varchar(1) not null,t3 varchar(1) not null);
                        --总列宽256
                        CREATE UNIQUE INDEX index2 ON {self.schema}.mixindex2(t1,t3);
                        DROP SCHEMA {self.schema} CASCADE;
                     '''
        logger.info("-----------------开始用例测试:mot支持在总列宽=256的表上创建索引，数据类型为varchar---------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0063执行结束---------------')
