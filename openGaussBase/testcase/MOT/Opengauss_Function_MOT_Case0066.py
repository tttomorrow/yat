...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("--------------------Opengauss_Function_MOT_Case0066开始执行------------------")
        ...
        self.sql_cmd = f'''
                        ...
                        CREATE FOREIGN TABLE {self.schema}.mixindex1(t1 integer not null,t2 integer not null,t3 integer not null,t4 varchar(234) not null);
                        --总列宽256
                        CREATE INDEX index1 ON {self.schema}.mixindex1(t1,t2,t4);
                        --对列t1,t3,t4创建唯一索引
                        CREATE FOREIGN TABLE {self.schema}.mixindex2(t1 integer not null,t2 integer not null,t3 integer not null,t4 varchar(242) not null);
                        --总列宽256
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0066执行结束---------------')
