...
"""
Case Type   : MOT
Case Name   : MOT设置checkpoint_workers= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中checkpoint_workers=
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看mot.conf配置文件参数是否生效,cat cluster/dn1/mot.conf | grep checkpoint_workers
...
CONFIG_PARAM = 'checkpoint_workers =  '
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0132开始执行======")
        LOG.info("======步骤1：修改mot.conf配置文件，设置checkpoint_workers= ,重启数据库=====")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep checkpoint_workers'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep checkpoint_workers'''
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0132执行结束======')
