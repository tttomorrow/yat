...
"""
Case Type   : MOT
Case Name   : MOT设置enable_mot_codegen_print=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_mot_codegen_print=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep enable_mot_codegen_print
...
MOT_CONF = os.path.join(macro.DB_INSTANCE_PATH, macro.MOT_CONF_NAME)
MOT_LOG = os.path.join(macro.PG_LOG_PATH, macro.DN_NODE_NAME.split('/')[0])
CONFIG_PARAM = 'enable_mot_codegen_print = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.show_param = 'show enable_incremental_checkpoint;'

        LOG.info("======检查参数，修改配置，并重启数据库======")
        self.config_item = "enable_incremental_checkpoint=off"
        check_res = self.sh_primysh.execut_db_sql(self.show_param)
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0262开始执行======")
        LOG.info("======步骤1：修改配置文件设置enable_mot_codegen_print=xxx启动数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep enable_mot_codegen_print'''
        msg = self.user_node.sh(cat_cmd).result()
        LOG.info(msg)
        self.assertIn(CONFIG_PARAM, msg.split('\n')[-1].strip())

        LOG.info("======步骤3：查看pg_log日志，存在参数ERROR信息======")
        ls_cmd = f'''ls -t {MOT_LOG} | head -1'''
        LOG.info(ls_cmd)
        log_msg = self.user_node.sh(ls_cmd).result()
        LOG.info(log_msg)

        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep enable_mot_codegen_print
            '''
        LOG.info(cat_cmd)
        cmd_msg = self.user_node.sh(cat_cmd).result()
        LOG.info(cmd_msg)
        self.assertIn('ERROR', cmd_msg)
        self.assertIn('Unexpected configuration item value type: '
                      'enable_mot_codegen_print', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info("======Opengauss_Function_MOT_Case0262执行结束======")
