...
"""
Case Type   : MOT
Case Name   : MOT设置store_memory_policy=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中store_memory_policy=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep store_memory_policy
...
CONFIG_PARAM = 'store_memory_policy = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0181开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置store_memory_policy=xxx,重启数据库")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep store_memory_policy'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep store_memory_policy'''
        LOG.info(cat_cmd)
        msg = self.user_node.sh(cat_cmd).result()
        LOG.info(msg)
        self.assertIn('ERROR', msg)
        self.assertIn('Failed to convert string configuration item'
                      ' store_memory_policy to user type', msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0181执行结束======')
