...
class Mot_datatype_test(unittest.TestCase):
    ...
    def test_mot_none_datatype_udf(self):
        logger.info("------------------------Opengauss_Function_MOT_Case0017开始执行----------------------")
        self.schema = 'schema_mot_test'
        self.tablename = 'MOTTable'
        self.datatype = 'typeUDF'
        self.sql_cmd = f'''CREATE SCHEMA {self.schema};
                        CREATE TYPE {self.datatype} AS (d1 int,d2 text);
                        CREATE FOREIGN TABLE {self.schema}.{self.tablename}(t1 {self.datatype});
                        DROP TYPE {self.datatype};
                        DROP SCHEMA {self.schema} CASCADE;
                      '''
        logger.info("-------------------------开始用例测试:MOT不支持用户自定义数据类型--------------------------")
        msg = self.sh_primysh.execut_db_sql(self.sql_cmd)
        logger.info(msg)
        self.assertIn(self.constant.CREATE_TYPE_SUCCESS_MSG, msg)
        self.assertIn(self.constant.NOT_SUPPORTED_TYPE, msg)
        self.assertIn(self.constant.DROP_TYPE_SUCCESS_MSG, msg)
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0017执行结束---------------')
