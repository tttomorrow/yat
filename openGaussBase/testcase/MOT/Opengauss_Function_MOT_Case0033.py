...
"""
Case Type   : 功能测试
Case Name   : MOT不支持的数据类型，line
Description :
    1.设置enable_incremental_checkpoint参数为off,并重启数据库;
    2.创建内存表，字段类型为line，插入数据，查看执行结果，清理环境，删除内存表；
...
class Mot_datatype_test(unittest.TestCase):
    ...
    def test_mot_none_datatype_array(self):
        logger.info("------------------------Opengauss_Function_MOT_Case0033开始执行-------------------")
        ...
        self.sql_cmd = f'''CREATE SCHEMA {self.schema};
                      ...
                      '''
        logger.info("-------------------------开始用例测试:MOT不支持数据类型line--------------------------")
        msg = self.sh_primysh.execut_db_sql(self.sql_cmd)
        logger.info(msg)
        self.assertIn(self.constant.NOT_YET_SUPPORTED_TYPE, msg)
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0033执行结束---------------')
