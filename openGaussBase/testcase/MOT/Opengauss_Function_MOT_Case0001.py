...
"""
Case Type   : 功能测试
Case Name   : MOT支持的数据类型，bigint
Description :
    1.设置enable_incremental_checkpoint参数为off,并重启数据库;
    2.创建内存表，字段类型为bigint，插入数据，查看执行结果，清理环境，删除内存表；
    3.恢复环境默认配置，修改enable_incremental_checkpoint为on，并重启数据库；
Expect      :
    1.设置成功，重启数据库成功；
    2.创建内存表成功，插入数据成功，查看执行结果成功，清理环境成功；
    3.修改参数成功，重启数据库成功；
History     : 
"""
...
class Mot_datatype_test(unittest.TestCase):
    ...
    def test_mot_datatype(self):
        logger.info("-----------------------Opengauss_Function_MOT_Case0001开始执行---------------------")
        self.sql_cmd = f'''
                            create schema schema_mot_test;
                            create FOREIGN table schema_mot_test.bigint01 (name bigint);
                            insert into schema_mot_test.bigint01 values (120);
                            select * from schema_mot_test.bigint01;
                            drop schema schema_mot_test CASCADE;
                      '''
        logger.info("-------------------------开始用例测试:MOT支持bigint数据类型-----------------------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('------------------------Opengauss_Function_MOT_Case0001执行结束-----------------------')
