...
"""
Case Type   : MOT
Case Name   : MOT设置affinity_mode=fill-socket-first进行SQL语句测试
Description :
    1、修改配置文件mot.conf中affinity_mode=fill-socket-first
...
CONFIG_PARAM = 'affinity_mode = fill-socket-first'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0172开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置affinity_mode=fill-socket-first,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0172执行结束======')
