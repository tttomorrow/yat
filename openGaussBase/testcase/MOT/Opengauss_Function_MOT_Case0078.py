...
'''
Case Type:  mot支持的索引
Case Name:  MOT表支持在单字段列宽<256的表上创建索引，数据类型为varchar
...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("------------------Opengauss_Function_MOT_Case0078开始执行------------------")
        self.schema = 'schema_mot_test'
        self.sql_cmd = f'''
                        DROP SCHEMA IF EXISTS {self.schema} CASCADE;
                        CREATE SCHEMA {self.schema};
                        --对列t1创建普通索引，varchar附加大小为4，非唯一索引需额外的8字节
                        CREATE FOREIGN TABLE {self.schema}.mixindex1(t1 varchar(200) not null,t2 varchar(200) not null);
                        --列宽为212
                     ...
                     '''
        logger.info("---------------开始用例测试:mot支持在单字段列宽<256的表上创建索引，数据类型为varchar---------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0078执行结束---------------')
