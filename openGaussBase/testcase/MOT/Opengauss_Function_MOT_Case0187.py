...
"""
Case Type   : MOT
Case Name   : MOT设置chunk_alloc_policy=native进行SQL语句测试
Description :
    1、修改配置文件mot.conf中chunk_alloc_policy=native
...
CONFIG_PARAM = 'chunk_alloc_policy = native'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0187开始执行======")
        LOG.info("步骤1：修改配置文件，设置chunk_alloc_policy=native,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0187执行结束======')
