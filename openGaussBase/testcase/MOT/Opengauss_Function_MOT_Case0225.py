...
"""
Case Type   : MOT
Case Name   : MOT设置session_large_buffer_store_max_object_size=0MB进行SQL语句测试
Description :
    1、修改配置文件mot.conf中session_large_buffer_store_max_object_size=0MB
...
CONFIG_PARAM = 'session_large_buffer_store_max_object_size = 0MB'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0225开始执行======")
        LOG.info("======步骤1：修改配置文件，"
                 "设置session_large_buffer_store_max_object_size=0MB启动数据库=====")
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep session_large_buffer_store_max_object_size'''
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0225执行结束======')
