...
"""
Case Type   : MOT
Case Name   : MOT设置chunk_prealloc_worker_count=1进行SQL语句测试
Description :
    1、修改配置文件mot.conf中chunk_prealloc_worker_count=1
...
CONFIG_PARAM = 'chunk_prealloc_worker_count = 1'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0190开始执行======")
        LOG.info("步骤1：修改配置文件，设置chunk_prealloc_worker_count=1,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0190执行结束======')
