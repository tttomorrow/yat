...
"""
Case Type   : MOT
Case Name   : MOT设置group_commit_size=0进行SQL语句测试
Description :
    1、修改配置文件mot.conf中group_commit_size=0
...
CONFIG_PARAM = 'group_commit_size = 0'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0116开始执行======")
        LOG.info("======步骤1：修改mot.conf配置文件，设置group_commit_size=0,重启数据库======")
        ...
        self.assertIn('Configuration of group_commit_size=0 '
                      'is out of bounds [2, 1000]', cat_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0116执行结束======')
