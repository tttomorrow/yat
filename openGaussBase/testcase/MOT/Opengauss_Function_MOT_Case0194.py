...
"""
Case Type   : MOT
Case Name   : MOT设置chunk_prealloc_worker_count=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中chunk_prealloc_worker_count=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep chunk_prealloc_worker_count
...
CONFIG_PARAM = 'chunk_prealloc_worker_count = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0194开始执行======")
        LOG.info("====步骤1：修改配置文件，设置chunk_prealloc_worker_count=xxx,重启数据库====")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep chunk_prealloc_worker_count'''
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0194执行结束======')
