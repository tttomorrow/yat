...
"""
Case Type   : MOT
Case Name   : MOT设置checkpoint_segsize=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中checkpoint_segsize=xxx
...
CONFIG_PARAM = 'checkpoint_segsize = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0127开始执行======")
        LOG.info("=====步骤1：修改mot.conf配置文件，设置checkpoint_segsize=xxx,重启数据库====")
        ...
        self.assertIn('Invalid checkpoint_segsize memory value format:'
                      ' xxx (expecting value digits)', cat_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0127执行结束======')
