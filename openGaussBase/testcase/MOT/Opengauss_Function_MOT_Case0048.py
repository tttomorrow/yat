...
'''
Case Type： mot不支持DDL
Case Name： mot不支持的DDL，创建including表
'''
...
class Mot_DDL_test(unittest.TestCase):
    ...
    def test_mot_usddl(self):
        logger.info("-------------------Opengauss_Function_MOT_Case0048开始执行---------------------")
        self.schema = 'schema_mot_test'
        self.sql_cmd = f''' 
                        DROP SCHEMA IF EXISTS {self.schema} CASCADE;
                        CREATE SCHEMA {self.schema};
                        CREATE FOREIGN TABLE {self.schema}.inc_tab(a int);
                        --创建including表
                        CREATE FOREIGN TABLE {self.schema}.inc_tab_new (like inc_tab INCLUDING DEFAULTS);
                        DROP SCHEMA {self.schema} CASCADE;
                      '''
        logger.info("--------------------开始用例测试:MOT不支持的DDL，创建including表---------------------")
        ...
        self.assertIn(self.constant.INCLUDING_DEFAULTS, msg)
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0048执行结束---------------')
