...
"""
Case Type   : MOT
Case Name   : MOT设置enable_system_stats=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_system_stats=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep enable_system_stats
...
CONFIG_PARAM = 'enable_system_stats = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0165开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置enable_system_stats=xxx,重启数据库")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep enable_system_stats'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep enable_system_stats'''
        LOG.info(cat_cmd)
        msg = self.user_node.sh(cat_cmd).result()
        LOG.info(msg)
        self.assertIn('ERROR', msg)
        self.assertIn('Unexpected configuration item value type:'
                      ' enable_system_stats', msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0165执行结束======')
