...
"""
Case Type   : MOT
Case Name   : MOT设置session_max_huge_object_size= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中session_max_huge_object_size=
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,
    cat cluster/dn1/mot.conf | grep session_max_huge_object_size
...
CONFIG_PARAM = 'session_max_huge_object_size =  '
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0232开始执行======")
        LOG.info("=====步骤1：修改配置文件，设置session_max_huge_object_size= 启动数据库====")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep session_max_huge_object_size'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep session_max_huge_object_size'''
        LOG.info(cat_cmd)
        ...
        self.assertIn('Empty value specified for configuration item'
                      ' session_max_huge_object_size', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0232执行结束======')
