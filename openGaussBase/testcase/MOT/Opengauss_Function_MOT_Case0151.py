...
"""
Case Type   : MOT
Case Name   : MOT设置enable_db_session_stats= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_db_session_stats=
...
CONFIG_PARAM = 'enable_db_session_stats =  '
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0151开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置enable_db_session_stats= ,重启数据库")
        ...
        self.assertIn('Empty value specified for configuration'
                      ' item enable_db_session_stats', cat_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0151执行结束======')
