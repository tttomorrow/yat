...
"""
Case Type   : MOT
Case Name   : MOT设置enable_group_commit=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_group_commit=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看mot.conf配置文件参数是否生效,cat cluster/dn1/mot.conf | grep enable_group_commit
...
CONFIG_PARAM = 'enable_group_commit = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0112开始执行======")
        LOG.info("===步骤1：修改mot.conf配置文件，设置enable_group_commit=xxx,重启数据库===")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep enable_group_commit'''
        ...
        cat_cmd = f'''
                    cd {MOT_LOG}
                    cat {log_msg} | grep enable_group_commit'''
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0112执行结束======')
