...
"""
Case Type   : MOT
Case Name   : MOT设置session_max_huge_object_size=1GB进行SQL语句测试
Description :
    1、修改配置文件mot.conf中session_max_huge_object_size=1GB
...
CONFIG_PARAM = 'session_max_huge_object_size = 1GB'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0231开始执行======")
        LOG.info("====步骤1：修改配置文件设置session_max_huge_object_size=1GB启动数据库====")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0231执行结束======')
