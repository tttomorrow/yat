...
"""
Case Type   : MOT
Case Name   : MOT设置mot_codegen_limit=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中mot_codegen_limit=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep mot_codegen_limit
...
CONFIG_PARAM = 'mot_codegen_limit = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0267开始执行======")
        LOG.info("======步骤1：修改配置文件设置mot_codegen_limit=xxx启动数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep mot_codegen_limit'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep mot_codegen_limit
            '''
        LOG.info(cat_cmd)
        cmd_msg = self.user_node.sh(cat_cmd).result()
        LOG.info(cmd_msg)
        self.assertIn('ERROR', cmd_msg)
        self.assertIn('Unexpected non-integer item value type'
                      ' mot_codegen_limit', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info("======Opengauss_Function_MOT_Case0267执行结束======")
