...
"""
Case Type   : MOT
Case Name   : MOT设置enable_memory_stats=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_memory_stats=xxx
...
CONFIG_PARAM = 'enable_memory_stats = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0159开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置enable_memory_stats=xxx,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0159执行结束======')
