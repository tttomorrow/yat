...
"""
Case Type   : MOT
Case Name   : gs_dump/gs_restore备份恢复多个内存表数据(目录归档格式)
Description :
    1、修改配置文件增量检查点为off,
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0109开始执行======")
        ...
        LOG.info("======步骤2：gs_dump以目录归档格式导出多个内存表======")
        ...
        LOG.info("======步骤3：gs_restore导入多个目录归档格式内存表======")
        sql_cmd = f'''source {macro.DB_ENV_PATH};                    
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0109执行结束======')
