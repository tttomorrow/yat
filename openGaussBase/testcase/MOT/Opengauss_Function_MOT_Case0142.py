...
"""
Case Type   : MOT
Case Name   : MOT设置print_stats_period= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中print_stats_period=
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看mot.conf配置文件参数是否生效,cat cluster/dn1/mot.conf | grep print_stats_period
...
CONFIG_PARAM = 'print_stats_period =  '
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0142开始执行======")
        LOG.info("======步骤1：修改mot.conf配置文件，设置print_stats_period= ,重启数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep print_stats_period'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep print_stats_period'''
        LOG.info(cat_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0142执行结束======')
