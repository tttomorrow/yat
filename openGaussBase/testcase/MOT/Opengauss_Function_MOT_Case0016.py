...
class Mot_datatype_test(unittest.TestCase):
    ...
    def test_mot_none_datatype_uuid(self):
        logger.info("------------------------Opengauss_Function_MOT_Case0016开始执行-------------------")
        ...
        self.sql_cmd = f'''create schema {self.schema};
                      ...
                      '''
        logger.info("-------------------------开始用例测试:MOT不支持数据类型UUID--------------------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0016执行结束---------------')
