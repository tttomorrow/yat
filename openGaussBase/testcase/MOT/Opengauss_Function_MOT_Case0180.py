...
"""
Case Type   : MOT
Case Name   : MOT设置store_memory_policy=expanding进行SQL语句测试
Description :
    1、修改配置文件mot.conf中store_memory_policy=expanding
...
CONFIG_PARAM = 'store_memory_policy = expanding'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0180开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置store_memory_policy=expanding,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0180执行结束======')
