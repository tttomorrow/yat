...
'''
Case Type:  mot不支持的索引
Case Name:  MOT表不支持在总列宽>256的表上创建索引，数据类型为int
...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("------------------Opengauss_Function_MOT_Case0082开始执行------------------")
        self.schema = 'schema_mot_test'
        self.sql_cmd = f'''
                        DROP SCHEMA IF EXISTS {self.schema} CASCADE;
                        CREATE SCHEMA {self.schema};
                        --对列t1,t2,t4创建普通索引，int类型4字节，附加大小为1，非唯一索引需额外的8字节
                        CREATE FOREIGN TABLE {self.schema}.mixindex1(t1 integer not null,t2 integer not null,t3 integer not null,t4 varchar(235) not null);
                        --总列宽257
                        CREATE INDEX index1 ON {self.schema}.mixindex1(t1,t2,t4);
                        --对列t1,t3,t4创建唯一索引
                        CREATE FOREIGN TABLE {self.schema}.mixindex2(t1 integer not null,t2 integer not null,t3 integer not null,t4 varchar(243) not null);
                        --总列宽257
                        CREATE UNIQUE INDEX index4 ON {self.schema}.mixindex2(t1,t3,t4);
                        DROP SCHEMA {self.schema} CASCADE;
                     '''
        logger.info("-------------------开始用例测试:mot不支持在总列宽>256的表上创建索引，数据类型为int-------------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0082执行结束---------------')
