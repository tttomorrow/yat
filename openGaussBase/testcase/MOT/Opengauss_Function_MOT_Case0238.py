...
"""
Case Type   : MOT
Case Name   : MOT设置enable_gc=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中enable_gc=xxx
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep enable_gc
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0238开始执行======")
        LOG.info("======步骤1：修改配置文件设置enable_gc=xxx启动数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep enable_gc'''
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0238执行结束======')
