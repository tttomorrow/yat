...
"""
Case Type   : MOT
Case Name   : MOT设置session_large_buffer_store_max_object_size=XXX进行SQL语句测试
Description :
    1、修改配置文件mot.conf中session_large_buffer_store_max_object_size=XXX
...
CONFIG_PARAM = 'session_large_buffer_store_max_object_size = XXX'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0229开始执行======")
        LOG.info("步骤1:修改参数session_large_buffer_store_max_object_size=XX启动数据库")
        ...
        self.assertIn('Invalid session_large_buffer_store_max_object_size'
                      ' memory value format: XXX', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0229执行结束======')
