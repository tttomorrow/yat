...
"""
Case Type   : MOT
Case Name   : MOT设置checkpoint_recovery_workers=-1进行SQL语句测试
Description :
    1、修改配置文件mot.conf中checkpoint_recovery_workers=-1
...
CONFIG_PARAM = 'checkpoint_recovery_workers = -1'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0135开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置checkpoint_recovery_workers=-1,重启数据库")
        ...
        self.assertIn('Configuration of checkpoint_recovery_workers=4294967295'
                      ' is out of bounds [1, 1024]', cat_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0135执行结束======')
