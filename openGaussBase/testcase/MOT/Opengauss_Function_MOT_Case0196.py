...
"""
Case Type   : MOT
Case Name   : MOT设置max_mot_global_memory=30%进行SQL语句测试
Description :
    1、修改配置文件mot.conf中max_mot_global_memory=30%
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep max_mot_global_memory
...
CONFIG_PARAM = 'max_mot_global_memory = 30%'
...
class MotParamTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if 'off' != check_res.splitlines()[-2].strip():
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0196开始执行======")
        LOG.info("步骤1：修改配置文件，设置max_mot_global_memory=30%,重启数据库")
        add_cmd = f'''sed -i '$a\\{CONFIG_PARAM}' {MOT_CONF}'''
        LOG.info(add_cmd)
        self.user_node.sh(add_cmd)
        self.sh_primysh.restart_db_cluster()
        result = self.sh_primysh.get_db_cluster_status()
        self.assertTrue("Degraded" in result or "Normal" in result)

        LOG.info("======步骤2：查看mot.conf文件参数修改是否生效======")
        cat_cmd = f'''cat {MOT_CONF} | grep max_mot_global_memory'''
        msg = self.user_node.sh(cat_cmd).result()
        LOG.info(msg)
        self.assertIn(CONFIG_PARAM, msg.split('\n')[-1].strip())

        LOG.info("======步骤3：查看pg_log日志，参数修改生效======")
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep max_mot_global_memory'''
        LOG.info(cat_cmd)
        cmd_msg = self.user_node.sh(cat_cmd).result()
        LOG.info(cmd_msg)
        self.assertIn('Loaded max_mot_global_memory: 30%', cmd_msg)
        ...
        LOG.info("======步骤5：用例执行成功，删除日志信息======")
        del_cmd = f'''rm -rf {MOT_LOG}/{log_msg}'''
        LOG.info(del_cmd)
    ...
    def tearDown(self):
        ...
        LOG.info(del_cmd)
        self.user_node.sh(del_cmd)
        LOG.info('======Opengauss_Function_MOT_Case0196执行结束======')
