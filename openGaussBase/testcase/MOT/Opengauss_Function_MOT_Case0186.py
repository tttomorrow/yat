...
"""
Case Type   : MOT
Case Name   : MOT设置chunk_alloc_policy=chunk-interleaved进行SQL语句测试
Description :
    1、修改配置文件mot.conf中chunk_alloc_policy=chunk-interleaved
...
CONFIG_PARAM = 'chunk_alloc_policy = chunk-interleaved'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0186开始执行======")
        LOG.info("步骤1：修改配置文件，设置chunk_alloc_policy=chunk-interleaved,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0186执行结束======')
