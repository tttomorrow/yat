...
"""
Case Type   : MOT
Case Name   : MOT设置group_commit_size=16KB进行SQL语句测试
Description :
    1、修改配置文件mot.conf中group_commit_size=16KB
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看mot.conf配置文件参数是否生效,cat cluster/dn1/mot.conf | grep group_commit_size
...
CONFIG_PARAM = 'group_commit_size = 16KB'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0114开始执行======")
        LOG.info("===步骤1：修改mot.conf配置文件，设置group_commit_size=16KB ,重启数据库===")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep group_commit_size'''
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0114执行结束======')
