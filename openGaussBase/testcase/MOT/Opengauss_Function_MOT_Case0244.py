...
"""
Case Type   : MOT
Case Name   : MOT设置reclaim_threshold= 进行SQL语句测试
Description :
    1、修改配置文件mot.conf中reclaim_threshold=
    2、重启数据库,gs_om -t stop && gs_om -t start
    3、查看配置文件参数是否生效,cat cluster/dn1/mot.conf | grep reclaim_threshold
...
CONFIG_PARAM = 'reclaim_threshold =  '
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0244开始执行======")
        LOG.info("======步骤1：修改配置文件设置reclaim_threshold= 启动数据库======")
        ...
        cat_cmd = f'''cat {MOT_CONF} | grep reclaim_threshold'''
        ...
        cat_cmd = f'''cd {MOT_LOG}
            cat {log_msg} | grep reclaim_threshold'''
        LOG.info(cat_cmd)
        ...
        self.assertIn('Empty value specified for configuration'
                      ' item reclaim_threshold', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0244执行结束======')
