...
"""
Case Type   : MOT
Case Name   : gs_dump/gs_restore备份恢复内存表数据(tar归档格式)
Description :
    1、修改配置文件增量检查点为off,gs_guc set -D cluster/dn1 -c "enable_incremental_checkpoint=off"
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0106开始执行======")
        LOG.info("======步骤1：创建内存表======")
        ...
        LOG.info("======步骤2：gs_dump以tar归档格式导出内存表======")
        ...
        LOG.info("======步骤3：gs_restore导入tar归档格式内存表======")
        sql_cmd = f'''source {macro.DB_ENV_PATH};
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0106执行结束======')
