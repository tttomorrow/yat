...
class Mot_datatype_test(unittest.TestCase):
    ...
    def test_mot_datatype(self):
        logger.info("-----------------------Opengauss_Function_MOT_Case0002开始执行----------------")
        self.sql_cmd = f'''
                            create schema schema_mot_test;
                            create FOREIGN table schema_mot_test.bigserial01 (name bigserial);
                            insert into schema_mot_test.bigserial01 values (120);
                            select * from schema_mot_test.bigserial01;
                            drop schema schema_mot_test CASCADE;
                      '''
        logger.info("-------------------------开始用例测试:MOT支持bigserial数据类型--------------------------")
    ...
    def tearDown(self):
        ...
        # stopstatus, startstatus = self.sh_primy.configSetModify(self.configitem)
        # self.assertTrue(stopstatus)
        # self.assertTrue(startstatus)
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('-------------------Opengauss_Function_MOT_Case0002执行结束----------------------')
