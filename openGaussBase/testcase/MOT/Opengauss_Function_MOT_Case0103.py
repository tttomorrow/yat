...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_dump(self):
        LOG.info("======Opengauss_Function_MOT_Case0103开始执行======")
        LOG.info("======步骤1：创建内存表======")
        self.mot_table = 'test_dump'
        self.mot_path = os.path.join(macro.DB_INSTANCE_PATH, 'mot_test.sql')
        ...
        LOG.info("======步骤3：以gsql方式导入纯文本格式内存表======")
        sql_cmd = f'''source {macro.DB_ENV_PATH}
            gsql {self.user_node.db_name} \
            -p {self.user_node.db_port} \
            -c "drop foreign table if exists {self.mot_table}";
            gsql {self.user_node.db_name} \
            -p {self.user_node.db_port} \
            -f {self.mot_path}        
            '''
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0103执行结束======')
