...
"""
Case Type   : MOT
Case Name   : MOT设置reserve_memory_mode=physical进行SQL语句测试
Description :
    1、修改配置文件mot.conf中reserve_memory_mode=physical
...
CONFIG_PARAM = 'reserve_memory_mode = physical'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0177开始执行======")
        LOG.info("步骤1：修改mot.conf配置文件，设置reserve_memory_mode=physical,重启数据库")
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0177执行结束======')
