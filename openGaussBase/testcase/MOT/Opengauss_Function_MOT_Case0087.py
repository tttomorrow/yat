...
'''
Case Type:  mot不支持的索引
Case Name:  MOT表不支持在DECIMAL类型上创建索引
...
class Mot_index_test(unittest.TestCase):
    ...
    def test_mot_none_index(self):
        logger.info("------------------Opengauss_Function_MOT_Case0087开始执行---------------------")
        self.schema = 'schema_mot_test'
        self.sql_cmd = f'''
                        DROP SCHEMA IF EXISTS {self.schema} CASCADE;
                        CREATE SCHEMA {self.schema};
                        CREATE FOREIGN TABLE {self.schema}.decimal_type_t1(DT_COL1 DECIMAL(10,4) not null);
                        CREATE INDEX index1 ON {self.schema}.decimal_type_t1(DT_COL1);
                        DROP SCHEMA {self.schema} CASCADE;
                      '''
        logger.info("-------------------------开始用例测试:mot不支持在DECIMAL类型上创建索引--------------------------")
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0087执行结束---------------')
