...
'''
Case Type： MOT
Case Name： 结合MOT表，以BEGIN启动事务以END结束事务测试
...
class MOT_TRANSACTION_TEST(unittest.TestCase):
    ...
    def setUp(self):
        logger.info('----------------------------Opengauss_Function_MOT_Case0095开始执行-----------------------------')
    ...
    def test_MOT_Transaction(self):
        ...
        logger.info(msg1)
        self.assertIn(self.constant.CREATE_ANONYMOUS_BLOCK_SUCCESS_MSG, msg1)
        ...
        logger.info(msg2)
        self.assertIn(self.constant.INDEX_BITMAP_SUCCESS_MSG, msg2)
        self.assertIn(self.constant.DROP_FOREIGN_SUCCESS_MSG, msg2)
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('----------------------------Opengauss_Function_MOT_Case0095执行完成-----------------------------')
