...
"""
Case Type   : MOT
Case Name   : MOT设置max_mot_session_memory=xxx进行SQL语句测试
Description :
    1、修改配置文件mot.conf中max_mot_session_memory=xxx
...
CONFIG_PARAM = 'max_mot_session_memory = xxx'
...
class MotParamTest(unittest.TestCase):
    ...
    def test_mot_param(self):
        LOG.info("======Opengauss_Function_MOT_Case0219开始执行======")
        LOG.info("======步骤1：修改配置文件，设置max_mot_session_memory=xxx启数据库======")
        ...
        self.assertIn('Invalid max_mot_session_memory memory value'
                      ' format: xxx', cmd_msg)
    ...
    def tearDown(self):
        ...
        LOG.info('======Opengauss_Function_MOT_Case0219执行结束======')
