...
class Mot_DDL_test(unittest.TestCase):
    ...
    def test_mot_usddl(self):
        logger.info("-------------------------Opengauss_Function_MOT_Case0050开始执行---------------------")
        self.schema = 'schema_mot_test'
        self.sql_cmd = f'''
                        DROP SCHEMA IF EXISTS {self.schema} CASCADE;
                        CREATE SCHEMA {self.schema};
                        --创建范围分区表
                        CREATE FOREIGN TABLE {self.schema}.range_tab(r1 integer NOT NULL,r2 character(16) NOT NULL,r3 character(10)) PARTITION BY RANGE (r1)
                         (PARTITION P1 VALUES LESS THAN(5000),
                         PARTITION P2 VALUES LESS THAN(10000),
                         PARTITION P8 VALUES LESS THAN(MAXVALUE));
                        DROP SCHEMA {self.schema} CASCADE;
                      '''
        logger.info("--------------------------开始用例测试:MOT不支持的DDL，创建范围分区表---------------------------")
        msg = self.sh_primysh.execut_db_sql(self.sql_cmd)
        logger.info(msg)
        self.assertIn(self.constant.CREATE_SCHEMA_SUCCESS_MSG, msg)
        self.assertIn(self.constant.NOT_RANGE, msg)
    ...
    def tearDown(self):
        ...
        # self.assertTrue(stopmsg)
        # self.assertTrue(startmsg)
        logger.info('---------------Opengauss_Function_MOT_Case0050执行结束---------------')
