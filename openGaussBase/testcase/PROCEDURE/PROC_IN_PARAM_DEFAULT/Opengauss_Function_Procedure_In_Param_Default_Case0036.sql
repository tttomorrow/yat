-- @testpoint: 测试入参为char default,输出数据集的存储过程
...
--创建存储过程
create or replace procedure proc_in_param_default_0036(
p1 in char default 'abcdg',
p2 out int
)
as
v1 test_001.t1%type;
begin
	for i in(select t1 from test_001 where t13=p1 order by t1)
	loop
...
declare
	v_p2 int;
begin
	proc_in_param_default_0036('abcdg',v_p2);
end;
/
--清理环境
drop table if exists test_001;
drop procedure proc_in_param_default_0036;
