-- @testpoint: 测试入参为decimal default,输出数据集的存储过程
...
--创建存储过程
create or replace procedure proc_in_param_default_0032(
p1 in binary_double default 12345.46,
p2 out int
)
as
v1 test_001.t1%type;
begin
	for i in(select t1 from test_001 where t9=p1 order by t1)
	loop
...
declare
	v_p2 int;
begin
	proc_in_param_default_0032(12345.46,v_p2);
end;
/
--清理环境
drop table if exists test_001;
drop procedure proc_in_param_default_0032;
