...
--创建存储过程
create or replace procedure proc_in_param_default_009(p1 in binary_double default 12345.57)
as
v1 test_001.t1%type;
begin
	select t1 into v1 from test_001 where t9=p1;
	raise info 'v1:%',v1;
	exception
	when no_data_found 
...
--调用存储过程
begin
	proc_in_param_default_009();
end;
/
--清理环境
drop table if exists test_001;
drop procedure proc_in_param_default_009;
