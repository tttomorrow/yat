-- @testpoint: 测试入参为date default,输出数据集的存储过程
...
--测试入参为date default,输出数据集的存储过程
create or replace procedure proc_in_param_default_041(
p1 in date default '2002-08-11 00:00:00',
p2 out int
)
as
v1 test_001.t2%type;
begin
	for i in(select t2 from test_001 where t19=p1 order by t2)
	loop
...
declare
	v_p2 int;
begin
	proc_in_param_default_041('2002-08-11 00:00:00',v_p2);
end;
/
--清理环境
drop table if exists test_001;
drop procedure proc_in_param_default_041;
