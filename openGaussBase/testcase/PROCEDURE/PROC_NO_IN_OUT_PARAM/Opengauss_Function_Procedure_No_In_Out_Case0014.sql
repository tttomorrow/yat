-- @testpoint: 测试无出入参存储过程（测试变量为VARCHAR(4000)型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_013 AS
V_VARCHAR VARCHAR(4000);
BEGIN
SELECT C_VARCHAR INTO V_VARCHAR FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_VARCHAR;
EXCEPTION
WHEN NO_DATA_FOUND THEN raise info 'NO_DATA_FOUND';
END;
/

Call PROC_NO_IN_OUT_PARAM_013();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_013;