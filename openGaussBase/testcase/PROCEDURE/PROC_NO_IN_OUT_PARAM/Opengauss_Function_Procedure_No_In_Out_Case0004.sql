-- @testpoint: 测试无出入参存储过程（测试变量为BIGINT型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_003 AS

V_BIGINT BIGINT;
BEGIN

SELECT C_BIGINT INTO V_BIGINT FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
...
Call PROC_NO_IN_OUT_PARAM_003();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_003;