-- @testpoint: 测试无出入参存储过程（测试变量为DATE型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_019 AS
V_DATE DATE;
BEGIN
SELECT C_DATE INTO V_DATE FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_DATE;
EXCEPTION
...
Call PROC_NO_IN_OUT_PARAM_019();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_019;
