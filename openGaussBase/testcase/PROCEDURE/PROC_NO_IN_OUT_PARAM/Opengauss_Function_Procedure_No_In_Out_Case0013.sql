-- @testpoint: 测试无出入参存储过程（测试变量为CHAR(4000)型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_012 AS
V_CHAR CHAR(4000);
BEGIN
SELECT C_CHAR INTO V_CHAR FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_CHAR;
EXCEPTION
...
Call PROC_NO_IN_OUT_PARAM_012();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_012;
