-- @testpoint: 测试无出入参存储过程（测试变量为VARCHAR2(4000)型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_014 AS
V_VARCHAR2 VARCHAR2(4000);
BEGIN
SELECT C_VARCHAR2 INTO V_VARCHAR2 FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_VARCHAR2;
EXCEPTION
WHEN NO_DATA_FOUND THEN raise info 'NO_DATA_FOUND';
END;
/

Call PROC_NO_IN_OUT_PARAM_014();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_014;