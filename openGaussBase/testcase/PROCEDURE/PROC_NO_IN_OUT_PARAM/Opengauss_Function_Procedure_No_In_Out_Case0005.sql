-- @testpoint: 测试无出入参存储过程（测试变量为NUMBER型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_004 AS

V_NUMBER NUMBER;

BEGIN
SELECT C_NUMBER INTO V_NUMBER FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_NUMBER;
EXCEPTION
...
Call PROC_NO_IN_OUT_PARAM_004();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_004;