-- @testpoint: 测试无出入参存储过程（测试变量为CHAR(100)型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_016 AS
V_CHAR1 CHAR(100);
BEGIN
SELECT C_CHAR1 INTO V_CHAR1 FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_CHAR1;
EXCEPTION
...
Call PROC_NO_IN_OUT_PARAM_016();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_016;