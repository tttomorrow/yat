-- @testpoint: 测试无出入参存储过程（测试变量为DOUBLE型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_007 AS
V_DOUBLE BINARY_DOUBLE;
BEGIN
SELECT C_BINARY INTO V_DOUBLE FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_DOUBLE;
EXCEPTION
...
Call PROC_NO_IN_OUT_PARAM_007();
drop procedure PROC_NO_IN_OUT_PARAM_007;
