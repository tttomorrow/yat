-- @testpoint: 测试无出入参存储过程（测试变量为VARCHAR(100)型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_015 AS
V_VARCHAR1 VARCHAR(100);
BEGIN
SELECT C_VARCHAR1 INTO V_VARCHAR1 FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_VARCHAR1;
EXCEPTION
WHEN NO_DATA_FOUND THEN raise info 'NO_DATA_FOUND';
END;
/

Call PROC_NO_IN_OUT_PARAM_015();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_015;