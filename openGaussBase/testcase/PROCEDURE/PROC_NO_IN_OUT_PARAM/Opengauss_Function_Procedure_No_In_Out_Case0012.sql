-- @testpoint: 测试无出入参存储过程（测试变量为REAL型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_011 AS
V_REAL REAL;
BEGIN
SELECT C_REAL INTO V_REAL FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_REAL;
EXCEPTION
...
Call PROC_NO_IN_OUT_PARAM_011();
drop procedure PROC_NO_IN_OUT_PARAM_011;
