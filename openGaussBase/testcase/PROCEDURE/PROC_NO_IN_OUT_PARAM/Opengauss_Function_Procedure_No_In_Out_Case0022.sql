-- @testpoint: 测试无出入参存储过程（测试变量为TIMESTAMP(6)型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_021 AS
V_TIMESTAMP1 TIMESTAMP(6);
BEGIN
SELECT C_TIMESTAMP1 INTO V_TIMESTAMP1 FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_TIMESTAMP1;
EXCEPTION
WHEN NO_DATA_FOUND THEN raise info 'NO_DATA_FOUND';
END;
/

Call PROC_NO_IN_OUT_PARAM_021();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_021;
drop table if exists PROC_NO_IN_OUT_PARAM_001;