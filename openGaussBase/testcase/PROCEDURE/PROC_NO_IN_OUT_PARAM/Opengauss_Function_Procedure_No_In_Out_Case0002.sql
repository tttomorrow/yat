-- @testpoint: 测试无出入参存储过程（测试变量为int型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_001 AS

V_INT INT;
BEGIN

SELECT C_INT INTO V_INT FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_INT;
EXCEPTION
...
Call PROC_NO_IN_OUT_PARAM_001();
drop procedure PROC_NO_IN_OUT_PARAM_001;
