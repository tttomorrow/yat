-- @testpoint: 测试无出入参存储过程（测试变量为TIMESTAMP型无入出参存储过程）
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_020 AS
V_TIMESTAMP TIMESTAMP;
BEGIN
SELECT C_TIMESTAMP INTO V_TIMESTAMP FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;
raise info ':%',V_TIMESTAMP;
EXCEPTION
WHEN NO_DATA_FOUND THEN raise info 'NO_DATA_FOUND';
END;
/

Call PROC_NO_IN_OUT_PARAM_020();
drop PROCEDURE PROC_NO_IN_OUT_PARAM_020;