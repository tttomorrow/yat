-- @testpoint: 测试无出入参存储过程（测试变量为INTEGER型无入出参存储过程）
-- @dts:
...
CREATE OR REPLACE PROCEDURE PROC_NO_IN_OUT_PARAM_002 AS

V_INTEGER INTEGER;
BEGIN
SELECT C_INTEGER INTO V_INTEGER FROM PROC_NO_IN_OUT_PARAM_001 WHERE C_INT=12;

raise info ':%',V_INTEGER;
EXCEPTION
...
Call PROC_NO_IN_OUT_PARAM_002();
drop procedure PROC_NO_IN_OUT_PARAM_002;
