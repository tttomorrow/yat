-- @testpoint: 测试入参类型为char(4000)，出参类型为char(100)的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为char(4000)，出参类型为char(100)的存储过程）
create or replace procedure proc_in_out_param_0019(
p1 in char,
p2 out varchar
)
as
begin
	select t13 into p2 from test_001 where t12=p1;
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 char(100);
begin
	proc_in_out_param_0019('dbce',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0019;
drop table test_001;