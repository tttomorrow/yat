-- @testpoint: 测试入参类型为int，出参类型为numeric,带min()函数的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为int，出参类型为numeric,带min()函数的存储过程）
create or replace procedure proc_in_out_param_0041(
p1 in int,
p2 out numeric
)
as
begin
	select min(t17) into p2 from test_001 where t1>p1;
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 numeric;
begin
	proc_in_out_param_0041('12',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0041;
drop table test_001;