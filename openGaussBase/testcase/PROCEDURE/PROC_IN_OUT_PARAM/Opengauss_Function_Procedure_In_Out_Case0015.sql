-- @testpoint: 测试入参类型为decimal，出参类型为decimal(8,2)的存储过程
...
--创建存储过程
create or replace procedure proc_in_out_param_0015(
p1 in decimal,
p2 out decimal
)
as
begin
	select t9 into p2 from test_001 where t8=p1;
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 decimal(8,2);
begin
	proc_in_out_param_0015('2345.67',v_p2);
end;
/
--删除存储过程
drop procedure proc_in_out_param_0015;
drop table test_001;