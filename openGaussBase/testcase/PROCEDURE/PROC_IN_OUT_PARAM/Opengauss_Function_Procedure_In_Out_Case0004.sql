-- @testpoint: 测试入参类型为integer，出参类型为number default 0.2332的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为integer，出参类型为number default 0.2332的存储过程）
create or replace procedure proc_in_out_param_0004(
p1 in integer,
p2 out number
)
as
begin
	select t4 into p2 from test_001 where t2=p1;
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 number;
begin
	proc_in_out_param_0004('58812',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0004;
drop table test_001;