-- @testpoint: 测试入参类型为bigint，出参类型为number(12,2)的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为bigint，出参类型为number(12,2)的存储过程）
create or replace procedure proc_in_out_param_0006(
p1 in bigint,
p2 out number
)
as
begin
	select t5 into p2 from test_001 where t3=p1;
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 number(12,2);
begin
	proc_in_out_param_0006('546223079',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0006;
drop table test_001;