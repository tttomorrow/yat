-- @testpoint: 测试入参类型为real，出参类型为timestamp,返回结果为数据集的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为real，出参类型为timestamp,返回结果为数据集的存储过程）
create or replace procedure proc_in_out_param_0036(
p1 in real,
p2 out timestamp
)
as
begin
	for i in(select t20 from test_001 where t11>p1 group by t20) 
	loop
		raise info 'result:%',i.t20;
		p2:=i.t20;
	end loop;
...
declare
	v_p2 timestamp;
begin
	proc_in_out_param_0036('12.44',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0036;
drop table test_001;