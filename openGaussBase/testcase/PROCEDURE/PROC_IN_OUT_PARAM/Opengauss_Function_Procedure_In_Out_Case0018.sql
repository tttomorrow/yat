-- @testpoint: 测试入参类型为real，出参类型为char(4000)的存储过程
...
create index indx_t2 on test_001(t2,t19,t20);
...
--带入参和出参的存储过程测试（测试入参类型为real，出参类型为char(4000)的存储过程）
create or replace procedure proc_in_out_param_0018(
p1 in real,
p2 out varchar
)
as
begin
	select t12 into p2 from test_001 where t11=p1;
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 char(4000);
begin
	proc_in_out_param_0018('12.33',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0018;
drop table test_001;