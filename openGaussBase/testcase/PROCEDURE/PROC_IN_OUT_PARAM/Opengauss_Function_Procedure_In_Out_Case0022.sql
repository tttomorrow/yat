-- @testpoint: 测试入参类型为varchar(100)，出参类型为varchar2(4000)的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为varchar(100)，出参类型为varchar2(4000)的存储过程）
create or replace procedure proc_in_out_param_0022(
p1 in varchar,
p2 out varchar2
)
as
	begin
	select t16 into p2 from test_001 where t15=p1;
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 varchar2(4000);
begin
	proc_in_out_param_0022('ac',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0022;
drop table test_001;