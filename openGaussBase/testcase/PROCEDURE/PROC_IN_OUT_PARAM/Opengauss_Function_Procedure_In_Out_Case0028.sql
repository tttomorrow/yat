-- @testpoint: 测试入参类型为timestamp(6)，出参类型为bool的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为timestamp(6)，出参类型为bool的存储过程）
create or replace procedure proc_in_out_param_0028(
p1 in timestamp,
p2 out bool
)
as
begin
	select t22 into p2 from test_001 where t21=p1;
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 bool;
begin
	proc_in_out_param_0028('2012-02-01 15:12:11.320000',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0028;
drop table test_001;