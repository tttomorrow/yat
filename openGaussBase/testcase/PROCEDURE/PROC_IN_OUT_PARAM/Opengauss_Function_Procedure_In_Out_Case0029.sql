-- @testpoint: 测试入参类型为int，出参类型为number(12,2)的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为int，出参类型为number(12,2)的存储过程）
create or replace procedure proc_in_out_param_0029(
p1 in int,
p2 out number
)
as
begin
	insert into test_001 values(21,58815,546223077,1234567.67,12345.4567,12.1234567,1234.78,2345.89,12345.4567,12.1234567,12.55,'dbcf','abcdg','abcdeh','ad','adf',123.47,'2002-08-11','2000-03-01 15:42:21.11','2008-08-12 15:13:21.32',true);
	raise info 'p2:%',p2;
	exception
	when no_data_found 
	then 
...
declare
	v_p2 number(12,2);
begin
	proc_in_out_param_0029(1,v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0029;
drop table test_001;