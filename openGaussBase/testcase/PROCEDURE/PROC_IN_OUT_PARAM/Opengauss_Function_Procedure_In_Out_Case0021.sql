-- @testpoint: 测试入参类型为varchar(4000)，出参类型为varchar(100)的存储过程
...
--带入参和出参的存储过程测试（测试入参类型为varchar(4000)，出参类型为varchar(100)的存储过程）
create or replace procedure proc_in_out_param_0021(
p1 in varchar,
p2 out varchar
)
as
begin
select t15 into p2 from test_001 where t14=p1;
 raise info 'p2:%',p2;
exception
...
v_p2 varchar(100);
begin
proc_in_out_param_0021('abcdeg',v_p2);
end;
/
--清理环境
drop procedure proc_in_out_param_0021;
