-- @testpoint: 测试出参类型为NUMBER(12,6),并且将出参的值赋给类型为NUMBER(12,2)的变量输出的存储过程
...
--仅带出参的存储过程测试（测试出参类型为NUMBER(12,6),并且将出参的值赋给类型为NUMBER(12,2)的变量输出的存储过程）
CREATE OR REPLACE PROCEDURE PROC_OUT_PARAM_001(P1 OUT NUMBER) AS
V1 NUMBER(12,2);
BEGIN
