-- @testpoint: 仅带出参，出参类型为DECIMAL，并且将出参的值赋给类型为DECIMAL(8,1)的变量输出的存储过程
...
--仅带出参的存储过程测试（测试出参类型为DECIMAL，并且将出参的值赋给类型为DECIMAL(8,1)的变量输出的存储过程）
CREATE OR REPLACE PROCEDURE PROC_OUT_PARAM_001(P1 OUT DECIMAL) 
AS
V1 DECIMAL(8,1);
BEGIN
SELECT T8 INTO V1 FROM TEST_001 WHERE T1=12;
raise info 'V1:%',V1;
EXCEPTION
