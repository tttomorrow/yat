-- @testpoint: 测试仅带出参，出参类型为BIGINT的存储过程
...
--仅带出参的存储过程测试（测试出参类型为BIGINT的存储过程）
CREATE OR REPLACE PROCEDURE PROC_OUT_PARAM_001(P1 OUT BIGINT) 
AS
BEGIN
SELECT T3 INTO P1 FROM TEST_001 WHERE T1=12;
raise info ':%',P1;
EXCEPTION
