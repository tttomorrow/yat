-- @testpoint: 测试仅带入参，入参1类型为INT,入参2类型为INTEGER,执行DELETE的存储过程
...
--test procedure
--#测试仅带入参的存储过程（仅有入参1类型为INT,入参2类型为INTEGER,执行DELETE的存储过程）
CREATE OR REPLACE PROCEDURE PROC_IN_PARAM_0029(P1 IN INT,P2 IN INTEGER)
AS
V1 INT;
V2 INTEGER;
BEGIN
DELETE FROM TEST_001 WHERE T1=P1 AND T2=P2;
...
--调用存储过程
call PROC_IN_PARAM_0029(15,58814);
drop PROCEDURE PROC_IN_PARAM_0029;
drop table if exists test_001;