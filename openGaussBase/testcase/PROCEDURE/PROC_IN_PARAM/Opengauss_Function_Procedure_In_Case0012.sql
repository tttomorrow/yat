-- @testpoint: 测试仅带入参，入参类型为CHAR(4000)的存储过程
...
--test procedure
--测试仅带入参的存储过程（仅有入参类型为CHAR(4000)的存储过程）
CREATE OR REPLACE PROCEDURE PROC_IN_PARAM_0012(P1 IN CHAR) AS
V1 REAL;
BEGIN
SELECT T11 INTO V1 FROM TEST_001 WHERE T12=P1;
raise info 'V1:%',V1;
EXCEPTION
...
--调用存储过程
call PROC_IN_PARAM_0012('dbcd');
drop PROCEDURE PROC_IN_PARAM_0012;
drop table if exists test_001;