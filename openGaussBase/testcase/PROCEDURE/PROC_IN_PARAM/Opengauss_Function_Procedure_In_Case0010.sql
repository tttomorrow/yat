-- @testpoint: 测试仅带入参，入参类型为DECIMAL(8,4)的存储过程
...
--test procedure
--测试仅带入参的存储过程（仅有入参类型为DECIMAL(8,4)的存储过程）
CREATE OR REPLACE PROCEDURE PROC_IN_PARAM_00010(P1 IN DECIMAL) AS
V1 DECIMAL(8,2);
BEGIN
SELECT T9 INTO V1 FROM TEST_001 WHERE T10=P1;
raise info 'V1:%',V1;
EXCEPTION
...
--调用存储过程
call PROC_IN_PARAM_00010(12.2346);
drop PROCEDURE PROC_IN_PARAM_00010;
drop table if exists test_001;
