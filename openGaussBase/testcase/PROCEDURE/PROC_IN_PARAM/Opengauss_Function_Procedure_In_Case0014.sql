-- @testpoint: 测试仅带入参，入参为VARCHAR(4000)的存储过程
...
--test procedure
--测试仅带入参的存储过程（仅有入参类型为VARCHAR(4000)的存储过程）
CREATE OR REPLACE PROCEDURE PROC_IN_PARAM_0014(P1 IN VARCHAR) AS
V1 CHAR(100);
BEGIN
SELECT T13 INTO V1 FROM TEST_001 WHERE T14=P1;
raise info 'V1:%',V1;
EXCEPTION
...
--调用存储过程
call PROC_IN_PARAM_0014('1999-01-01');
drop PROCEDURE PROC_IN_PARAM_0014;
drop table if exists test_001;