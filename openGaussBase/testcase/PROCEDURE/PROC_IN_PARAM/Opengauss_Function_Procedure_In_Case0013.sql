-- @testpoint: 测试仅带入参，入参为CHAR(100)的存储过程
...
--test procedure
--测试仅带入参的存储过程（仅有入参类型为CHAR(100)的存储过程）
CREATE OR REPLACE PROCEDURE PROC_IN_PARAM_0013(P1 IN CHAR) AS
V1 CHAR(4000);
BEGIN
SELECT T12 INTO V1 FROM TEST_001 WHERE T13=P1;
raise info 'V1:%',V1;
EXCEPTION
...
--调用存储过程
call PROC_IN_PARAM_0013('abcde');
drop PROCEDURE PROC_IN_PARAM_0013;
drop table if exists test_001;