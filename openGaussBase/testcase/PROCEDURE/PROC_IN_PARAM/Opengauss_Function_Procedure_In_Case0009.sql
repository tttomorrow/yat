-- @testpoint: 测试仅带入参，入参类型为DECIMAL(8,2)的存储过程
...
--test procedure
--测试仅带入参的存储过程（仅有入参类型为DECIMAL(8,2)的存储过程）
CREATE OR REPLACE PROCEDURE PROC_IN_PARAM_0009(P1 IN DECIMAL) AS
V1 DECIMAL;
BEGIN
SELECT T8 INTO V1 FROM TEST_001 WHERE T9=P1;
raise info 'V1:%',V1;
EXCEPTION
...
--调用存储过程
call PROC_IN_PARAM_0009(12345.68);
drop PROCEDURE PROC_IN_PARAM_0009;
drop table if exists test_001;
