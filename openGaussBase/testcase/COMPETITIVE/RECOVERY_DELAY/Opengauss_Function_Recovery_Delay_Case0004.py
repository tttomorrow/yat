...
"""
Case Type   : 数据库系统
Case Name   : 1主2备事务同步方式remote_write时备机备份延迟
Description :
    1.设置synchronous_commit=remote_write,
    ...
    11.等待2min查询备机
    12.删除表
    13.等待1min设置recovery_min_apply_delay=0
    14.重启数据库
    15.备机查询
Expect      :
    1.设置成功
...
class RecoveryDelay(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("---Opengauss_Function_Recovery_Delay_Case0004 start---")
    ...
    def test_recovery_delay(self):
        if self.node_num > 2:
            ...
            self.log.info('-------更新表，等待5s再更新------------')
            sql = f"update {self.tb_name} set s='update1';"
            result = self.commshpri.execut_db_sql(sql)
            self.log.info(result)
            self.assertIn(self.constant.UPDATE_SUCCESS_MSG, result)
            time.sleep(5)
            sql = f"insert into {self.tb_name} values (11,'test');" \
                f"update {self.tb_name} set s='update11' where i=11;"
            result = self.commshpri.execut_db_sql(sql)
            self.log.info(result)
            self.assertIn(self.constant.UPDATE_SUCCESS_MSG, result)
    ...
    def tearDown(self):
        ...
        self.log.info("---Opengauss_Function_Recovery_Delay_Case0004 end--")