...
"""
Case Type   : 数据库系统
Case Name   : 级联备switchover后检测备机延迟
Description :
    1.创建表
    2.切换备节点为级联备
    3.设置延迟为1min
...
class RecoveryDelay(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("---Opengauss_Function_Recovery_Delay_Case0017 start--")
    ...
    def test_recovery_delay(self):
        if self.node_num > 2:
            ...
            self.log.info('-------切换级联备为备节点---------')
            result = self.comshsta[1].execute_gsctl(
    ...
    def tearDown(self):
        ...
        self.log.info('--------还原集群-------')
        if self.node_num > 2:
            result = self.commshpri.execute_gsctl(
                'switchover', self.constant.SWITCHOVER_SUCCESS_MSG)
            self.log.info(result)
            result = self.commshpri.exec_refresh_conf()
            self.log.info(result)
            for i in range(int(self.node_num) - 1):
        ...
        self.log.info("---Opengauss_Function_Recovery_Delay_Case0017 end--")