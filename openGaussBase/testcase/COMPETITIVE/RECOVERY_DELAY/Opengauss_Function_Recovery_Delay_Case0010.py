...
class RecoveryDelay(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.info("---Opengauss_Function_Recovery_Delay_Case0010 start---")
        ...
        if self.node_num > 2:
            ...
            for exec_time in range(5):
                ...
                for i in range(int(self.node_num) - 1):
                    result = self.comshsta[i].execut_db_sql(sql)
                    self.log.info(result)
                    if ('5 | test' in result) and ('2' in result) \
    ...
    def tearDown(self):
        ...
        self.log.info("---Opengauss_Function_Recovery_Delay_Case0010 end--")