...
Case Name   : 创建hash分区表,设置query_dop=2,执行explain，查看是否启用并行查询
Description :
    1、创建Hash分区表
    2、向分区表中插入数据
    3、设置query_dop参数值为2
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0017开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.common = Common()
        self.initial_query_dop = self.common.show_param("query_dop")
        self.t_name_sc = "t_score_0017"
        self.t_name_stu = "t_student_0017"
        self.t_name_tea = "t_teacher_0017"
        self.t_name_cou = "t_course_0017"
    ...
    def test_query_dop(self):
        ...
        sql_cmd1 = f'''drop table if exists {self.t_name_sc};
            ...
            s_id int,
            s_name char(8));
            drop table if exists {self.t_name_cou};
            create table {self.t_name_cou}(
            t_id int,
            c_id int,
            c_name char(8));
            drop table if exists {self.t_name_tea};
            create table {self.t_name_tea}(
            t_id int,
            t_name char(8));'''
        ...
        sql_cmd2 = f'''insert into {self.t_name_sc} values(
            ...
            generate_series(1, 1000000),
            '李四');
            insert into {self.t_name_cou} values(
            generate_series(1, 100),
            generate_series(1, 1000000),
            'Math');
            insert into {self.t_name_tea} values(
            generate_series(1, 100),
            '张三');'''
        ...
        analyse_res = self.commonsh.execut_db_sql(
            f'''analyze {self.t_name_sc};
                ...
                analyze {self.t_name_cou};
                analyze {self.t_name_tea};''')
        ...
        explain_res = self.commonsh.execut_db_sql(
            f'''explain select * from {self.t_name_stu} c where c.s_id not in(
                select s3.s_id from {self.t_name_stu} s3
                join {self.t_name_sc} b
                on s3.s_id=b.s_id where b.c_id in(
                    select c3.c_id from {self.t_name_cou} c3
                    join {self.t_name_tea} t3
                    on c3.t_id = t3.t_id where t3.t_name ='张三'));''')
    ...
    def tearDown(self):
        ...
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0017执行结束---")
