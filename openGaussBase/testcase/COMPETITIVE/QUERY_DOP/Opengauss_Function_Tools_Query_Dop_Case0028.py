...
Case Name   : 设置query_dop=2,enable_hashjoin=on,enable_nestloop=on,
            enable_mergejoin=off,执行explain，查看是否启用并行查询
Description :
    1、创建测试表
    2、向测试表中插入数据
    3、分别设置各参数的值query_dop=2,enable_hashjoin=on,enable_nestloop=on,
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0028开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.t_name_sc = "t_score_0028"
        self.t_name_stu = "t_student_0028"
    ...
    def tearDown(self):
        ...
        self.assertTrue(drop_res.count(self.constant.TABLE_DROP_SUCCESS)
                        == 2, '执行失败:' + drop_text)
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0028执行结束---")
