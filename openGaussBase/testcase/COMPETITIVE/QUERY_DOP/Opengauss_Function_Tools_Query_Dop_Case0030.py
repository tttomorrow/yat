...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0030开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.t_name_sc = "t_score_0030"
        self.t_name_stu = "t_student_0030"
    ...
    def tearDown(self):
        ...
        self.assertTrue(drop_res.count(self.constant.TABLE_DROP_SUCCESS)
                        == 2, '执行失败:' + drop_text)
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0030执行结束---")
