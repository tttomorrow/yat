...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0002开始执行---")
        self.commonsh = CommonSH("PrimaryDbUser")

    def test_query_dop(self):
        step_text = "---step1:更改query_dop参数的值为0;expect:更改参数失败，报错---"
        self.logger.info(step_text)
        assert_info = "The value 0 is outside the valid range"
    ...
    def tearDown(self):
        ...
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0002执行结束---")
