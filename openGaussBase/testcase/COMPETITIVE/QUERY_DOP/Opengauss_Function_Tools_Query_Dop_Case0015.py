...
Case Name   : 创建范围分区表,设置query_dop=2，使用有group by 和join的查询，执行explain，查看是否启用并行查询
Description :
    1、创建范围分区表
    2、向分区表中插入数据
    3、设置query_dop参数值为2
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0015开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.common = Common()
        self.initial_query_dop = self.common.show_param("query_dop")
        self.t_name_sc = "t_score_0015"
        self.t_name_stu = "t_student_0015"
    ...
    def test_query_dop(self):
        ...
        assert_info = "Streaming(type: LOCAL REDISTRIBUTE dop: 2/2)"
    ...
    def tearDown(self):
        ...
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0015执行结束---")
