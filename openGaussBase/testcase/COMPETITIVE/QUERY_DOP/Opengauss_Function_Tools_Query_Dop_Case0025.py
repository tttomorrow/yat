...
Case Name   : 设置query_dop=2,创建并执行存储过程查询（若能正确执行就说明不支持并行查询）
Description :
    1、创建测试表
    2、向表中插入数据
    3、设置query_dop参数值为2
    4、重启数据库集群使参数生效
    5、创建并执行procedure查询，是否启用并行查询
    6、清理环境
Expect      :
    1、创建测试表成功
    2、插入数据成功
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0025开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.common = Common()
        self.initial_query_dop = self.common.show_param("query_dop")
        self.t_name_sc = "t_score_0025"
        self.proc_name = "proc_0025"
    ...
    def test_query_dop(self):
        ...
        step5_text = "---step5:创建并执行procedure查询;expect:成功执行,不支持并行查询---"
        self.logger.info(step5_text)
        procedure_cmd = self.commonsh.execut_db_sql(
            f'''create or replace procedure {self.proc_name}
                AS
                p_sql varchar2(2000);
                BEGIN
                p_sql:='select count(*) from {self.t_name_sc}';
                EXECUTE IMMEDIATE p_sql;
                END;''')
        self.logger.info(procedure_cmd)
        procedure_res = self.commonsh.execut_db_sql(
            f'call {self.proc_name}();')
        assert_info = f'{self.proc_name}'
        self.assertIn(self.constant.CREATE_PROCEDURE_SUCCESS_MSG,
                      procedure_cmd, "执行失败:" + step5_text)
        self.assertIn(assert_info, procedure_res, "执行失败:" + step5_text)

    def tearDown(self):
        self.logger.info("---清理环境---")

        drop_text = "---step:删除表及procedure;expect:删除成功---"
        self.logger.info(drop_text)
        drop_cmd = f'''drop procedure if exists {self.proc_name};
