...
Case Name   : 创建list分区表,设置query_dop=2,执行explain，查看是否启用并行查询
Description :
    1、创建list分区表
    2、向分区表中插入数据
    3、设置query_dop参数值为2
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0018开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.common = Common()
        self.initial_query_dop = self.common.show_param("query_dop")
        self.t_name_sc = "t_score_0018"
        self.t_name_stu = "t_student_0018"

    def test_query_dop(self):
        step1_text = "---step1:创建list分区表;expect:建表成功---"
        ...
        sql_cmd1 = f'''drop table if exists {self.t_name_sc};
            ...
            partition p3 values (300000),
            partition p4 values (400000)
            );'''
        ...
        sql_cmd2 = f'''insert into {self.t_name_sc} values(
            ...
            random()*100,
            generate_series(1, 1000000));
            insert into {self.t_name_stu} values(100000, '张三'),
            (200000, '李四'),(300000, '王五'),(400000, '赵六');'''
        ...
        explain_res = self.commonsh.execut_db_sql(
            f'''explain select a.* ,b.s_score as score01,c.s_score as score02 
                ...
                on a.s_id=b.s_id and b.c_id=01 left join {self.t_name_sc} c 
    ...
    def tearDown(self):
        ...
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0018执行结束---")
