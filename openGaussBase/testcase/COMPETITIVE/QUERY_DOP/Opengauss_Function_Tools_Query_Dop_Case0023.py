...
Case Name   : 设置query_dop=2,创建并执行median查询（若能正确执行就说明不支持并行查询）
Description :
    1、创建测试表
    2、向表中插入数据
    3、设置query_dop参数值为2
    4、重启数据库集群使参数生效
    5、创建并执行median查询，是否启用并行查询
    6、清理环境
Expect      :
    1、创建测试表成功
    2、插入数据成功
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0023开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.common = Common()
        self.initial_query_dop = self.common.show_param("query_dop")
        self.t_name_sc = "t_score_0023"
    ...
    def test_query_dop(self):
        ...
        step5_text = "---step5:创建并执行median查询;expect:成功执行,不支持并行查询---"
        self.logger.info(step5_text)
        median_res = self.commonsh.execut_db_sql(
            f'''start transaction;
                select median(s_id) from {self.t_name_sc};''')
        self.logger.info(median_res)
        self.common.equal_sql_mdg(median_res, "START TRANSACTION", "median",
                                  "----------", "500000.5", "(1 row)")

    def tearDown(self):
        self.logger.info("---清理环境---")

        drop_text = "---step:提交事务、删除表;expect:删除成功---"
        self.logger.info(drop_text)
        drop_cmd = f'end;drop table if exists {self.t_name_sc};'
        ...
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0023执行结束---")
