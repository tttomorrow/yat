...
Case Name   : 创建范围分区表,设置query_dop=2,执行explain，查看是否启用并行查询
Description :
    1、创建范围分区表
    2、向分区表中插入数据
    3、设置query_dop参数值为2
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0021开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.common = Common()
        self.initial_query_dop = self.common.show_param("query_dop")
        self.t_name_sco = "t_score_0021"
        self.t_name_stu = "t_student_0021"

    def test_query_dop(self):
        step1_text = "---step1:创建范围分区表;expect:建表成功---"
        self.logger.info(step1_text)
        sql_cmd1 = f'''drop table if exists {self.t_name_sco};
            create table {self.t_name_sco}(
            s_id int,
            s_score int,
            s_course char(30))
            partition by range (s_score)
            (partition p1 values less than (60),
            partition p2 values less than (1000001));
        ...
        sql_cmd2 = f'''insert into {self.t_name_sco} values(
        ...
        explain_res = self.commonsh.execut_db_sql(
            f'''explain select b.s_id,b.s_name,ROUND(AVG(a.s_score),2) as avg_score 
                from {self.t_name_stu} b left join {self.t_name_sco} a 
                on b.s_id = a.s_id 
                GROUP BY b.s_id,b.s_name HAVING avg_score <60 
                union select a.s_id,a.s_name,0 as avg_score 
                from {self.t_name_stu} a where a.s_id not in (
                select distinct s_id from {self.t_name_sco});''')
    ...
    def tearDown(self):
        ...
        drop_text = "---step:删除分区表;expect:删除成功---"
        self.logger.info(drop_text)
        drop_cmd = f'''drop table if exists {self.t_name_sco};
            drop table if exists {self.t_name_sco};'''
        ...
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0021执行结束---")
