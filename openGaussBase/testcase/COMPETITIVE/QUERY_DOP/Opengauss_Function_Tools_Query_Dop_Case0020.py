...
Case Name   : 创建间隔分区表,设置query_dop=2,执行explain，查看是否启用并行查询
Description :
    1、创建间隔分区表
    2、向分区表中插入数据
    3、设置query_dop参数值为2
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0020开始执行---")
        self.constant = Constant()
        self.commonsh = CommonSH("PrimaryDbUser")
        self.common = Common()
        self.initial_query_dop = self.common.show_param("query_dop")
        self.t_name_sc = "t_score_0020"
        self.t_name_stu = "t_student_0020"

    def test_query_dop(self):
        step1_text = "---step1:创建间隔分区表;expect:建表成功---"
        ...
        sql_cmd1 = f'''drop table if exists {self.t_name_sc};
            ...
            s_score int,
            c_id int,
            s_time TIMESTAMP(0) WITHOUT TIME ZONE)
            partition by range (s_time) interval ('1 day')
        ...
        sql_cmd2 = f'''insert into {self.t_name_sc} values(
            ...
            generate_series(500001, 1000000), 
            random()*100,
            generate_series(500001, 1000000),
            '2018-01-01');
        ...
        explain_res = self.commonsh.execut_db_sql(
            f'''explain select b.s_id,b.s_name,ROUND(AVG(a.s_score),2) as avg_score 
                ...
                on b.s_id = a.s_id GROUP BY b.s_id,b.s_name 
                HAVING avg_score <60 
    ...
    def tearDown(self):
        ...
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0020执行结束---")
