...
Case Name   : 将并行查询参数设置为64
Description :
    1、更改query_dop参数的值为64
    2、重新启动数据库集群
    3、查看query_dop参数是否生效
    4、清理环境
Expect      :
...
class QueryDopCase(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = Logger()
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0004开始执行---")
    ...
    def test_query_dop(self):
        step1_text = "---step1:更改query_dop参数的值为64;expect:更改参数成功---"
        ...
        self.common.equal_sql_mdg(sql_cmd, "query_dop", "64",
                                  "(1 row)", flag="1")
    ...
    def tearDown(self):
        ...
        self.commonsh.execute_gsguc("set",
                                    self.constant.GSGUC_SUCCESS_MSG,
                                    f"query_dop = {self.initial_query_dop}")
        ...
        self.logger.info(
            "---Opengauss_Function_Tools_Query_Dop_Case0004执行结束---")
