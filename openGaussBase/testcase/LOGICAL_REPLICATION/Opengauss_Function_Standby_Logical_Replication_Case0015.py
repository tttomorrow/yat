...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        self.decode_file = os.path.join(macro.DB_INSTANCE_PATH,
                                        'logical15.txt')
        self.us_name = "us_logical_replication_case0015"
        self.slot_name = "slot_logical_replication_case0015"
        self.tb_name = "tb_logical_replication_case0015"
    ...
    def test_standby_logical(self):
        ...
        text = '--step8:创建表并修改属性后进行DML操作;expect:创建成功--'
        self.log.info(text)
        sql_cmd = Pri_SH.execut_db_sql(f'''drop table if exists {self.tb_name};
            ...
            c_16 blob,
            c_17 bytea);
            alter table {self.tb_name} REPLICA IDENTITY using index \
            tb_logical_replication_case0015_pkey;
            ...
            update {self.tb_name} set c_1 = 100000;
        ...
        text = '-step10:查看解码文件;expect:update和delete操作均会记录' \
               'index name列的旧值-'
        self.log.info(text)
