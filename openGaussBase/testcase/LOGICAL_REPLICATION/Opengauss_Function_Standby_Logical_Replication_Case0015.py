...
"""
Case Type   : 逻辑复制
Case Name   : 创建逻辑复制槽后，修改表为REPLICA IDENTITY INDEX属性，
              执行DML操作（表无主键），备机进行逻辑复制操作
Description :
        ...
        6.备机创建解码文件
        7.备机执行逻辑复制槽流式解码
        8.创建表后修改表的属性为REPLICA IDENTITY INDEX并进行DML操作
        9.备机查看解码文件
        10.停止解码
        11.主机删除逻辑复制槽
        12.清理环境
Expect      :
        ...
        4.主机创建逻辑复制槽成功
        5.显示slot_test008复制槽信息
        6.备机创建解码文件成功
        7.备机逻辑复制槽流式解码过程
        8.创建表并进行DML操作成功
        9.备机解码文件update和delete操作均会记录index name列的旧值
        10.停止解码成功
        11.删除成功
        12.清理环境完成
History     :
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Logical_Replication_Case0015start-----')
    ...
    def test_standby_logical(self):
        ...
        if 'slot_test015' in check_res.split('\n')[-2].strip():
        ...
        self.assertIn('slot_test015', query_cmd)
        ...
        self.log.info('步骤8:创建表并修改属性后进行DML操作')
        sql_cmd = Primary_SH.execut_db_sql('''drop table if exists test015;
            create table test015(c_1 integer primary key,
            c_2 bigint,
            c_3 smallint,
            c_4 tinyint,
            c_5 serial,
            ...
            c_16 blob,
            c_17 bytea);
            alter table test015 REPLICA IDENTITY using index test015_pkey;
            insert into test015 values(1,10,5,25,default,default,default,\
            1237.127,123456.1234,date '12-10-2010','21:21:21','2010-12-12',\
            '测试','测试工程师','西安',empty_blob(),E'\\xDEADBEEF');
            update test015 set c_15 = '数据库';
            update test015 set c_1 = 100000;
            delete from test015;''')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info(
            '--Opengauss_Function_Logical_Replication_Case0015finish----')
