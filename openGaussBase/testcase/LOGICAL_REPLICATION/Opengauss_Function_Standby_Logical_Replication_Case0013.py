...
"""
Case Type   : 逻辑复制
Case Name   : 备机进行逻辑复制操作，MOT表进行DML操作无法解析
Description :
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Logical_Replication_Case0013start-----')
    ...
    def test_standby_logical(self):
        ...
        if 'slot_test013' in check_res.split('\n')[-2].strip():
        ...
        self.assertIn('slot_test013', query_cmd)
        ...
        self.log.info('步骤8:创建MOT表并进行DML操作')
        sql_cmd = Primary_SH.execut_db_sql('''drop foreign table if exists 
            mot_0013;
            create FOREIGN table MOT_0013(a int);
            insert into mot_0013 values(generate_series(1,2000));
            delete from mot_0013;''')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        sql_cmd = Primary_SH.execut_db_sql('''drop role if exists rep;
            drop  foreign table if exists mot_0013;''')
        self.log.info(sql_cmd)
        ...
        restore_cmd = Primary_SH.execute_gsguc('set',
                                               ...
                                               'enable_slot_log=off')
        self.log.info(restore_cmd)
        restore_msg = Primary_SH.execute_gsguc('set',
                                               self.constant.GSGUC_SUCCESS_MSG,
        ...
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info(
            '--Opengauss_Function_Logical_Replication_Case0013finish----')
