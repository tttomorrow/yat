...
"""
Case Type   : 逻辑复制
Case Name   : 列存表，备机进行逻辑复制操作，DML语句无法解析
Description :
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Logical_Replication_Case0011start-----')
    ...
    def test_standby_logical(self):
        ...
        if 'slot_test011' in check_res.split('\n')[-2].strip():
        ...
        self.assertIn('slot_test011', query_cmd)
        ...
        self.log.info('步骤8:创建列存表并进行DML操作')
        sql_cmd = Primary_SH.execut_db_sql('''drop table if exists test011;
        create table test011(c_1 INTEGER,
                  c_2 BIGINT,
                  c_3 SMALLINT,
                  c_4 TINYINT,
                  c_5 SERIAL,
                  ...
                  c_14 VARCHAR(20),
                  c_15 TEXT,
                  c_16 BYTEA) with (orientation =column);
            insert into test011 values(1,10,5,25,default,default,default,\
            1237.127,123456.1234,date '12-10-2010','21:21:21','2010-12-12',\
            '测试','测试工程师','西安',empty_blob(),E'\\xDEADBEEF');
            update test011 SET c_15 = '数据库';
            delete from test011 where c_16 = empty_blob();''')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info(
            '--Opengauss_Function_Logical_Replication_Case0011finish----')
