...
class LogicalReplication(unittest.TestCase):
    ...
    def test_standby(self):
        ...
        if 'slot_test017' in check_res.split('\n')[-2].strip():
        ...
        self.assertIn('slot_test017', query_cmd)
        self.log.info('--步骤6:备机使用pg_logical_slot_get_changes函数--')
        sql_cmd = self.Standby_SH.execut_db_sql("select * from "
                                                "pg_logical_slot_get_changes"
                                                "('slot_test017',NULL,NULL,"
                                                "'include-timestamp',1);")
        self.log.info(sql_cmd)
