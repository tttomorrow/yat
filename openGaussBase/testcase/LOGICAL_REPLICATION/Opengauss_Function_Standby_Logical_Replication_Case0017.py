...
"""
Case Type   : 备机支持逻辑复制
Case Name   : 备机使用pg_logical_slot_get_changes函数，合理报错
Description :
        ...
        4.主机创建逻辑复制槽
        5.主机上查询逻辑复制槽
        6.备机使用pg_logical_slot_get_changes函数
        7.清理环境
Expect      :
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Logical_Replication_Case0017start-----')
        self.constant = Constant()
        self.Standby_SH = CommonSH('Standby1DbUser')
        self.primary_node = Node('PrimaryDbUser')
        self.standby_node = Node('Standby1DbUser')
        self.us_name = "us_logical_replication_case0017"
        self.slot_name = "slot_logical_replication_case0017"
    ...
    def test_standby(self):
        ...
        text = '--step6:备机使用pg_logical_slot_get_changes函数;' \
               'expect:合理报错--'
        self.log.info(text)
        sql_cmd = self.Standby_SH.execut_db_sql(f"select * from "
                                                f"pg_logical_slot_get_changes"
                                                f"('{self.slot_name}',"
                                                f"NULL,NULL,"
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status,
                        '执行失败:' + text)
        self.log.info(
            '--Opengauss_Function_Logical_Replication_Case0017finish----')
