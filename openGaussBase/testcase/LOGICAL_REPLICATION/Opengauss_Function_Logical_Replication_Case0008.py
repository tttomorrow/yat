...
"""
Case Type   : 逻辑复制
Case Name   : 使用JDBC创建名称中含有汉字的逻辑复制槽
Description :
    1.创建名称中含有汉字的逻辑复制槽
Expect      :
    1.创建名称中含有汉字的逻辑复制槽失败
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        ...
        LOG.info(
            '---Opengauss_Function_Logical_Replication_Case0008开始执行-----')
    ...
    def tearDown(self):
        ...
        msg = self.comsh.execut_db_sql('''
                    SELECT * FROM pg_drop_replication_slot('slot复制槽');
                    ''')
        LOG.info(msg)
        msg = self.comsh.execute_gsguc('set', self.constant.GSGUC_SUCCESS_MSG,
        ...
        LOG.info(startmsg)
        LOG.info(
            '-----Opengauss_Function_Logical_Replication_Case0008执行完成---')
