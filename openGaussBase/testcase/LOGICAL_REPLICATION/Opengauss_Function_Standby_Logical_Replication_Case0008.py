...
Description :
        ...
        6.备机创建解码文件
        7.备机执行逻辑复制槽流式解码
        8.创建表(无主键)后修改表的属性为REPLICA IDENTITY NOTHING并进行DML操作
        9.备机查看解码文件
        10.停止解码
        11.主机删除逻辑复制槽
        12.清理环境
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Logical_Replication_Case0008start-----')
        self.constant = Constant()
        self.primary_node = Node('PrimaryDbUser')
        self.standby_node = Node('Standby1DbUser')
        self.standby_node1 = Node('Standby1DbUser')
        self.root_node = Node('Standby1Root')
        self.decode_file = os.path.join(macro.DB_INSTANCE_PATH, 'logical8.txt')
    ...
    def test_standby_logical(self):
        ...
        if 'slot_test008' in check_res.split('\n')[-2].strip():
        ...
        self.assertIn('slot_test008', query_cmd)
        ...
        sql_cmd = Primary_SH.execut_db_sql('''drop table if exists test008;
            create table test008(c_1 integer,
            c_2 bigint,
            c_3 smallint,
            c_4 tinyint,
            c_5 serial,
            ...
            c_16 blob,
            c_17 bytea);
            alter table test008 REPLICA IDENTITY NOTHING;
            insert into test008 values(1,10,5,25,default,default,default,\
            1237.127,123456.1234,date '12-10-2010','21:21:21','2010-12-12',\
            '测试','测试工程师','西安',empty_blob(),E'\\xDEADBEEF');
            update test008 set c_15 = '数据库';
            delete from test008 where c_16 = empty_blob();''')
        self.log.info(sql_cmd)
    ...
    def tearDown(self):
        ...
        self.assertTrue("Degraded" in status or "Normal" in status)
        self.log.info(
            '--Opengauss_Function_Logical_Replication_Case0008finish----')
