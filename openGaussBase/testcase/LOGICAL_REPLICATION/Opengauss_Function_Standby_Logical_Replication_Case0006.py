...
"""
Case Type   : 逻辑复制
Case Name   : 创建逻辑复制槽后，修改表为REPLICA IDENTITY FULL属性，
...
Description :
        ...
        6.备机创建解码文件
        7.备机执行逻辑复制槽流式解码
        8.创建表(无主键)后修改表的属性为REPLICA IDENTITY full并进行DML操作
        9.备机查看解码文件
        10.停止解码
        11.主机删除逻辑复制槽
        12.清理环境
Expect      :
        ...
        4.主机创建逻辑复制槽成功
        5.显示slot_test006复制槽信息
        6.备机创建解码文件成功
        7.屏幕输出备机逻辑复制槽流式解码过程
        8.创建表(无主键)并进行DML操作成功
        9.备机解码文件update和delete操作均会记录所有列的旧值
        10.停止解码成功
        11.删除成功
        12.清理环境完成
History     :
...
class LogicalReplication(unittest.TestCase):
    def setUp(self):
        self.log = Logger()
        self.log.info(
            '----Opengauss_Function_Logical_Replication_Case0006start-----')
        self.constant = Constant()
        self.primary_node = Node('PrimaryDbUser')
        self.standby_node = Node('Standby1DbUser')
        self.standby_node1 = Node('Standby1DbUser')
        self.root_node = Node('Standby1Root')
        self.decode_file = os.path.join(macro.DB_INSTANCE_PATH, 'logical6.txt')
...
"\'2010-12-12 00:00:00\'","\'测试    \'","\'测试工程师\'","\'西安\'",\
    ...
    def test_standby_logical(self):
        ...
        if 'slot_test006' in check_res.split('\n')[-2].strip():
        ...
        self.assertIn('slot_test006', query_cmd)
        ...
        sql_cmd = Primary_SH.execut_db_sql('''drop table if exists test006;
            create table test006(c_1 integer,
            c_2 bigint,
            c_3 smallint,
            c_4 tinyint,
            c_5 serial,
            ...
            c_16 blob,
            c_17 bytea);
            alter table test006 REPLICA IDENTITY full;
            insert into test006 values(1,10,5,25,default,default,default,\
            1237.127,123456.1234,date '12-10-2010','21:21:21','2010-12-12',\
            '测试','测试工程师','西安',empty_blob(),E'\\xDEADBEEF');
            update test006 set c_15 = '数据库';
            delete from test006 where c_16 = empty_blob();''')
        self.log.info(sql_cmd)
