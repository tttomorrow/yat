...
class GoScript(private val output: File, private val env: Map<String, String> = mapOf()) : FileOutputScript() {
    private val commander = ProcessCommander()

    fun execute(scriptFile: File): Boolean {
        makeParentPathExists(output)
        return try {
            0 == commander.ffexec("go", "run", scriptFile.absolutePath, output = output, env = env)
        } catch (e: IOException) {
