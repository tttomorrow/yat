...
class GSqlScript(builder: Builder) : BaseCmdSqlScript(builder) {
    override fun getCmdPathFromMacro(): String? {
        return env["GSQL_PATH"]
    }
    ...
    override fun makeMirrorFile(relative: Path): File {
        return Paths.get(tempDir, "gsql", relative.subpath(1, relative.nameCount).toString()).toFile()
    }

    override fun makeCommander(): CmdCommander {
        return CmdCommander("gsql", "-d \"${dbname}\" -h \"${host}\" -U \"${user}\" -p \"${port}\" -W \"${password}\"")
    }
