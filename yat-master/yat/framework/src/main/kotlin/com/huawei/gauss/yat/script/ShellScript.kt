...
class ShellScript(private val output: File, private val env: Map<String, String> = mapOf()) : FileOutputScript() {
    ...
    fun execute(script: String): Boolean {
        ...
        return try {
            0 == shell.sfexec(script, output = output, env = env)
        } catch (e: IOException) {
    ...
    fun execute(scriptFile: File): Boolean {
        makeParentPathExists(output)
        return try {
            0 == shell.ffexec(scriptFile, output = output, env = env)
        } catch (e: IOException) {
            output.writeText(e.message!!)
            false
        }
