...
class EntryNode(val name: String, val values: List<EntryValue>) {
    fun toTestGroup(suite: TestSuite): TestGroup {
        val group = TestGroup(suite)
        values.forEach { entryValue ->
            group.addTestCase(entryValue.value, entryValue.attrs)
        }
        return group
    }
}